...
inline std::vector<std::uint32_t> distinctQubits(core::Random& rng, std::uint32_t n,
                                                 std::size_t k) {
...
inline Circuit randomClifford(std::uint32_t n, std::size_t gates, std::uint64_t seed) {
    ...
    const std::vector<std::pair<std::string, Matrix>> one{{"h", H()}, {"s", S()}, {"sdg", Sdg()},
                                                          {"x", X()}, {"y", Y()}, {"z", Z()}};
    const std::vector<std::pair<std::string, Matrix>> two{
    ...
    for (std::size_t g = 0; g < gates; ++g) {
        ...
        if (r < 0.45) {
            const std::vector<std::pair<std::string, Matrix>> one{
                {"h", H()},     {"t", T()},     {"sx", SX()}, {"rx", RX(th)},
                {"ry", RY(th)}, {"rz", RZ(th)}, {"y", Y()}};
            const auto& [name, u] = one[rng.uniformInt(one.size())];
            c.push_back({name, u, q({static_cast<std::uint32_t>(rng.uniformInt(n))}), {}});
        } else if (r < 0.75) {
            const std::vector<std::pair<std::string, Matrix>> two{
            ...
            auto t =
                distinctQubits(rng, n, 3); // dense 8x8: RX ⊗ RY ⊗ RZ, exercises the k-qubit kernel
...
inline double maxHermitianDefect(const Matrix& rho) {
    ...
    for (std::size_t i = 0; i < rho.rows; ++i)
        for (std::size_t j = i; j < rho.cols; ++j)
            worst = std::max(worst, std::abs(rho(i, j) - std::conj(rho(j, i))));
    return worst;
}
inline std::vector<QubitIndex> allQubits(std::uint32_t n) {
    std::vector<QubitIndex> v;
    for (std::uint32_t i = 0; i < n; ++i)
        v.push_back(QubitIndex{i});
    return v;
}
