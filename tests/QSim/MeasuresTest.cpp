...
namespace {
std::vector<Complex> amplitudes(const StateVectorBackend& sv) {
    return {sv.amplitudes().begin(), sv.amplitudes().end()};
}
...
Circuit bellCircuit() {
    return {{"h", H(), q({0}), {}}, {"cx", CX(), q({0, 1}), {}}};
}
Circuit ghz3Circuit() {
    return {{"h", H(), q({0}), {}}, {"cx", CX(), q({0, 1}), {}}, {"cx", CX(), q({1, 2}), {}}};
}
...
TEST_CASE("Measures: von Neumann entropy and mutual information") {
    ...
    REQUIRE(measures::mutualInformation(*p0, *p1, *prod.reducedDensityMatrix(q({0, 1}))) ==
    ...
    REQUIRE(measures::mutualInformation(
                *ghz.reducedDensityMatrix(q({0})), *ghz.reducedDensityMatrix(q({1})),
...
TEST_CASE("Measures: concurrence of Bell, product, general pure and Werner states") {
    ...
    for (const auto& [name, extra] : std::vector<std::pair<const char*, Matrix>>{
             {"Phi+", I2()}, {"Phi-", Z()}, {"Psi+", X()}, {"Psi-", Y()}}) {
        INFO(name);
        Circuit c = bell;
    ...
    REQUIRE(measures::concurrence(num::projector(amplitudes(zero))).value() ==
            Approx(0.0).margin(1e-7));
    // General pure state a|00⟩ + b|01⟩ + c|10⟩ + d|11⟩: C = 2|ad − bc|. The eigenvalue square roots
    // of the rank-one ρρ̃ amplify round-off to ~1e-8, hence the tolerance.
    ...
    for (int trial = 0; trial < 20; ++trial) {
        ...
        REQUIRE(measures::concurrence(num::projector(psi)).value() ==
                Approx(expected).margin(1e-7));
    }
    ...
    for (double p : {0.0, 0.2, 1.0 / 3.0, 0.5, 0.8, 1.0}) {
        INFO("p = " << p);
        Matrix werner =
            num::add(num::scale(phi, p), num::scale(Matrix::identity(4), 0.25 * (1.0 - p)));
        REQUIRE(measures::concurrence(werner).value() ==
                Approx(std::max(0.0, 1.5 * p - 0.5)).margin(1e-7));
    }
...
TEST_CASE("Measures: Schmidt coefficients") {
    ...
    const auto prod =
        amplitudes(prepared(3, {{"ry", RY(1.3), q({0}), {}}, {"h", H(), q({2}), {}}}));
    ...
    for (std::size_t k = 0; k < lam->size(); ++k) {
        sum += (*lam)[k] * (*lam)[k];
        if (k > 0)
            REQUIRE((*lam)[k] <= (*lam)[k - 1]);
        if ((*lam)[k] > 1e-12)
            entropy -= (*lam)[k] * (*lam)[k] * std::log2((*lam)[k] * (*lam)[k]);
    }
    REQUIRE(sum == Approx(1.0).margin(1e-12));
    StateVectorBackend sv;
    REQUIRE(sv.allocate(4).has_value());
    REQUIRE(sv.setAmplitudes(psi).has_value());
    REQUIRE(measures::entropyBits(*sv.reducedDensityMatrix(q({0, 3}))) ==
            Approx(entropy).margin(1e-10));
...
TEST_CASE("Measures: Bloch vector, purity and fidelity") {
    ...
    auto near = [](std::array<double, 3> r, std::array<double, 3> e) {
        for (int k = 0; k < 3; ++k)
            REQUIRE(r[k] == Approx(e[k]).margin(1e-12));
    };
    ...
    REQUIRE(measures::fidelityTo(num::projector(plus), num::projector(plus)) ==
            Approx(1.0).margin(1e-9));
    REQUIRE(measures::fidelityTo(num::scale(Matrix::identity(2), 0.5), num::projector(zeroKet)) ==
