...
TEST_CASE("placeholders substitute from the node instance") {
    ...
    CHECK(substitutePath("wiring.line[$line].attn[$k].P_diss", p).value() ==
          "wiring.line[3].attn[0].P_diss");
...
TEST_CASE("binding roots split and round-trip") {
    for (auto [name, root] : {std::pair{"cryo", BindingRoot::Cryo},
                              {"wiring", BindingRoot::Wiring},
                              {"device", BindingRoot::Device},
                              {"instr", BindingRoot::Instr},
                              {"static", BindingRoot::Static},
                              {"run", BindingRoot::Run}}) {
...
TEST_CASE("the registry resolves a row through a fake provider") {
    ...
    SpecRow diss = row("dissipated power", "W", "wiring.line[$line].attn[$k].P_diss",
    ...
    reg.registerProvider(
        BindingRoot::Wiring, [](std::string_view path) -> std::optional<BindingValue> {
            if (path == "line[3].attn[0].P_diss")
                return BindingValue::number(1.25e-9, {}, data::FidelityClass::Model);
            if (path == "line[3].attn[0].n_th")
                return BindingValue::number(3.6e-3, "", data::FidelityClass::Model);
            return std::nullopt;
        });
    ...
    SpecRow bloch =
        row("Bloch vector", "", "device.qubit[$i].bloch[0]", data::FidelityClass::Exact);
    params.setIndex("i", 7);
    reg.registerProvider(
        BindingRoot::Device, [](std::string_view path) -> std::optional<BindingValue> {
            if (path == "qubit[7].bloch[0]")
                return BindingValue::number(0.42, {}, data::FidelityClass::Numerical);
            return std::nullopt;
        });
    ...
    CHECK(b->cls ==
          data::FidelityClass::Numerical); // Lindblad probe is weaker than the row's Exact
    CHECK(b->available());

    // a NaN reading (probe disabled) is "not available" so the overlay hides
    reg.registerProvider(BindingRoot::Run, [](std::string_view) {
        return std::optional<BindingValue>{
            BindingValue::number(std::numeric_limits<double>::quiet_NaN())};
    });
