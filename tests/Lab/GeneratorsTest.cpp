...
std::vector<Case> cases() {
    ...
    for (const auto& p : centre)
        amp = std::max(amp, std::abs(p.y));
    return {
        {"Box",
         {{"w_m", 0.03}, {"h_m", 0.02}, {"d_m", 0.04}, {"ports", 3}, {"marking", "arrow"}},
         1.0,
         {0.03, 0.02, 0.04}},
        {"Box",
         {{"w_m", 0.03}, {"h_m", 0.01}, {"d_m", 0.03}, {"wireframe", true}},
         1.0,
         {0.03, 0.01, 0.03}},
        {"Box",
         {{"w_m", 0.8}, {"h_m", 1.9}, {"d_m", 0.7}, {"front", "valve_mimic"}},
         1.0,
         {0.8, 1.9, 0.7}},
        {"Box",
         {{"w_m", 0.03}, {"h_m", 0.02}, {"d_m", 0.05}, {"marking", "heatsink"}},
         1.0,
         {0.03, 0.02, 0.05}},
        {"Plate",
         {{"r_m", 0.19}, {"t_m", 0.012}, {"holes", "bolt_circle"}},
         1.0,
         {0.38, 0.012, 0.38}},
        {"Plate",
         {{"r_m", 0.3},
         ...
         {0.6, 0.02, 0.6}},
        {"Plate",
         {{"r_m", 0.014}, {"t_m", 0.0016}, {"shape", "square"}},
         1.0,
         {0.028, 0.0016, 0.028}},
        {"PlateWithHoles",
         {{"w_um", 8000},
          {"d_um", 8000},
          {"t_um", 2},
          {"holes_um", {{0, 0, 300}, {1000, 500, 200}}}},
         1e6,
         {8000, 2, 8000}},
        {"RackUnit",
         {{"u", 2}, {"front", "sg_mw"}},
         1.0,
         {kRackPanelWidth_m, 2 * kRackUnit_m, kRackUnitDepth_m}},
        {"RackUnit",
         {{"u", 6}, {"front", "awg"}},
         1.0,
         {kRackPanelWidth_m, 6 * kRackUnit_m, kRackUnitDepth_m}},
        {"Cylinder",
         {{"r_m", 0.06}, {"h_m", 0.45}, {"caps", true}, {"attachments", {"rotary_valve_box"}}},
         1.0,
         {0.12, 0.45, 0.12}},
        {"Cylinder",
         {{"r_m", 0.025}, {"h_m", 0.05}, {"attachments", {"sma_bulkheads"}}},
         1.0,
         {0.05, 0.05, 0.05}},
        {"Cylinder", {{"r_m", 0.03}, {"h_m", 0.02}, {"front", "dial"}}, 1.0, {0.06, 0.02, 0.06}},
        {"Cylinder",
         {{"r_m", 0.015}, {"h_m", 0.02}, {"attachments", {"handle"}}},
         1.0,
         {0.03, 0.02, 0.03}},
        {"Cylinder",
         {{"r_m", 0.12}, {"h_m", 0.4}, {"attachments", {"coil"}}},
         1.0,
         {0.24, 0.4, 0.24}},
        {"Cylinder",
         {{"r_m", 0.06}, {"h_m", 0.15}, {"attachments", {"flange"}}},
         1.0,
         {0.12, 0.15, 0.12}},
        ...
        {"SmaConnector",
         core::Json::object(),
         1.0,
         {kSmaHexAcrossCorners_m, kSmaLength_m, kSmaHexAcrossCorners_m * 0.866}},
        {"Tube",
         {{"r_m", 0.00043}, {"points_m", {{0, 0, 0}, {0, -0.5, 0}}}},
         1.0,
         {0.00086, 0.5, 0.00086}},
        {"Tube", {{"r_m", 0.01}, {"path", "straight_0.1m"}}, 1.0, {0.02, 0.1, 0.02}},
        {"Can",
         {{"r_m", 0.28}, {"h_m", 1.35}, {"t_m", 0.006}, {"base", "hemi"}, {"open", "top"}},
         1.0,
         {0.56, 1.63, 0.56}},
        {"Can",
         {{"r_m", 0.28},
         ...
         {0.59, 1.49, 0.59}},
        {"Post",
         {{"d_m", 0.02}, {"L_m", 0.22}, {"wall_m", 0.0005}, {"collar_m", 0.008}},
         1.0,
         {0.02, 0.22, 0.02}},
        {"Plate",
         {{"r_m", 0.25},
          {"t_m", 0.010},
          {"holes", "bolt_circle"},
          {"through_hole_m", {-0.087, 0.023, 0.03}}},
         1.0,
         {0.5, 0.010, 0.5}},
        {"Plate",
         {{"r_m", 0.3},
        ...
        {"Tube",
         {{"r_m", 0.0012},
          {"bundle", 12},
          {"bundle_pitch_m", 0.0026},
          {"points_m", {{0, 0, 0}, {0, -0.3, 0}}}},
         1.0,
         {0.0024, 0.3, 0.0024 + 11 * 0.0026}}, // a vertical run spreads its ribbon along z
        {"Spiral",
         {{"r_m", 0.04},
          {"pitch_m", 0.01},
          {"turns", 12},
         ...
         {0.086, 0.126, 0.086}},
        {"Can",
         {{"r_m", 0.245}, {"h_m", 1.15}, {"t_m", 0.002}, {"base", "flat"}, {"open", "top"}},
         1.0,
         {0.49, 1.15, 0.49}},
        {"Torus", {{"r_m", 0.15}, {"tube_r_m", 0.01}, {"count", 2}}, 1.0, {0.32, 0.17, 0.32}},
        {"Sphere", {{"r_um", 10}}, 1e6, {20, 20, 20}},
        {"Frame",
         {{"width_m", 1.4}, {"depth_m", 1.4}, {"height_m", 2.6}, {"profile_mm", 60}},
         1.0,
         {1.4, 2.6, 1.4}},
        {"Frame", {{"span_m", 1.4}, {"travel_m", 1.6}}, 1.0, {1.4, kGantryHeight_m, 1.6}},
        {"Octagon", {{"r_m", 0.12}, {"h_m", 0.1}}, 1.0, {0.24, 0.1, 0.24}},
        {"ExtrudedU",
         {{"points_m", {{0, 0, 0}, {2, 0, 0}}}},
         1.0,
         {2.0 + kTrayWidth_m, kTrayHeight_m, kTrayWidth_m}},
        {"Spiral",
         {{"r_m", 0.05}, {"pitch_m", 0.01}, {"turns", 20}, {"tube_r_m", 0.003}},
         1.0,
         {0.106, 0.206, 0.106}},
        {"Cpw",
         {{"path_um", {{0, 0}, {1000, 0}, {1000, 500}}}, {"w_um", 10}, {"s_um", 6}},
         1e6,
         {1022, kFilmMetalTop_m * 1e6, 522}},
        {"Meander",
         {{"f_r_hz", 7e9}, {"pitch_um", 50}, {"amplitude_um", 150}, {"lead_um", 50}},
         1e6,
         {lenX, kFilmMetalTop_m * 1e6, 2 * amp + 22}},
        {"Xmon",
         {{"arm_length_um", 150}, {"arm_width_um", 24}, {"gap_um", 24}},
         1e6,
         {348, kFilmMetalTop_m * 1e6, 348}},
        {"Xmon",
         {{"arm_length_um", 150}, {"gap_um", 24}, {"scale", 0.6}},
         1e6,
         {2 * 0.6 * 174, kFilmMetalTop_m * 1e6, 2 * 0.6 * 174}},
        {"Junction",
         {{"w_lead_um", 1.0}, {"overlap_um", 0.15}},
         1e6,
         {10.65, 2 * kFilmJunction_m * 1e6, 10.65}},
        {"SquidLoop",
         {{"area_um2", 60}, {"w_um", 1.0}},
         1e6,
         {std::sqrt(60.0) + 1.0, kFilmJunction_m * 1e6, std::sqrt(60.0) + 1.0}},
        {"Airbridge", {{"span_um", 30}, {"w_um", 10}, {"h_um", 3}}, 1e6, {30, 3.3, 10}},
        {"WirebondArc", {{"h_um", 300}}, 1e6, {557.5, 312.5, 90}},
        {"TrapChip",
         {{"electrodes", 40}},
         1.0,
         {kTrapChipSize_m[0], kTrapChipSize_m[1], kTrapChipSize_m[2]}},
    };
}
...
TEST_CASE("every generator builds a valid mesh of the requested size") {
    for (const auto& c : cases()) {
        ...
        for (int k = 0; k < 3; ++k)
            CHECK(std::abs(ssize[k] - c.expected[k]) <= 0.35 * c.expected[k]);
...
TEST_CASE("the generator table covers the spec 17 §6 names and rejects unknown ones") {
    ...
    for (const char* n : {"Box",         "Plate",        "PlateWithHoles", "RackUnit",  "Cylinder",
                          "CylinderSma", "SmaConnector", "Tube",           "Can",       "Torus",
                          "Sphere",      "Frame",        "Octagon",        "ExtrudedU", "Spiral",
                          "Cpw",         "Meander",      "Xmon",           "Junction",  "SquidLoop",
                          "Airbridge",   "WirebondArc",  "TrapChip",       "Post"})
...
TEST_CASE("a plate's through-hole is a real opening and its rim is chamfered") {
    GenParams p(core::Json{{"r_m", 0.25},
                           {"t_m", 0.010},
                           {"holes", "bolt_circle"},
                           {"through_hole_m", {-0.087, 0.023, 0.03}}});
    ...
    for (const auto& v : full->vertices) {
        ...
        if (rho > 0.248 && std::abs(std::abs(v.normal.y) - 0.7071) < 1e-3)
            ++chamferVerts;
    }
