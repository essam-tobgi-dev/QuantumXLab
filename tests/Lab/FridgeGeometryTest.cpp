...
const Scene& scene() {
    static Scene s = [] {
        ...
        if (!built)
            FAIL(built.error().format());
        return std::move(*built);
    }();
    return s;
}
const Node& plate(cryo::Stage s) {
    return *scene().node(scene().stageNodes()[static_cast<std::size_t>(s)]);
}
...
TEST_CASE("every stage hangs from the plate above on six posts that touch both plates") {
    ...
    for (int k = 1; k < cryo::kStageCount; ++k) {
        ...
        for (ComponentId id : sc.findByDescriptor("support_post")) {
            ...
            if (n->params.token("stage") && *n->params.token("stage") == stageLayoutName(upper))
                posts.push_back(n);
        }
        ...
        for (const Node* p : posts) {
            ...
            CHECK(std::hypot(c.x, c.z) ==
                  Catch::Approx(
                      plateBoltRing_m(sc.layout().stages[static_cast<std::size_t>(k)].radius_m))
                      .margin(1e-6));
            azimuths.insert(static_cast<int>(std::lround(glm::degrees(std::atan2(c.z, c.x)))) %
                            360);
...
TEST_CASE("the vacuum can carries a flange with 48 hex bolts on the flange radius") {
    ...
    const double r = d->geometryNumber("r_m", 0.28), fw = d->geometryNumber("flange_w_m", 0.015),
                 ft = d->geometryNumber("flange_t_m", 0.012);
    ...
    for (const auto& v : m.vertices) {
        ...
        if (v.position.y < -ft - 1e-4 && rho > r + 0.15 * fw && rho < r + 0.85 * fw)
            heads.insert(static_cast<int>(std::floor(std::atan2(v.position.z, v.position.x) /
                                                     (2.0 * 3.14159265358979) * 48.0 * 4.0)));
    }
    ...
    CHECK(h ==
          Catch::Approx(d->geometryNumber("flange_t_m", 0.012) * 0.0 + (-(ovc->localBounds.min.y)))
              .margin(1e-6));
...
TEST_CASE("the pulse-tube stages end on the 50 K and 4 K plates with copper braids to each") {
    ...
    for (std::size_t i = 0; i < 2; ++i) {
        ...
        INFO(n->instanceName);
        CHECK(std::abs(n->worldBounds.min.y - (plate(ends[i]).world[3].y + 0.5 * plateT(ends[i]))) <
              1e-3);
        CHECK(std::abs(n->worldBounds.max.y -
                       (plate(starts[i]).world[3].y - 0.5 * plateT(starts[i]))) < 1e-3);
        CHECK(sc.inspect(n->id).function.find("1.4 Hz") != std::string::npos);
    }
    std::map<std::string, int> braids;
    for (ComponentId id : sc.findByDescriptor("thermal_braid"))
        braids[*sc.node(id)->params.token("stage")]++;
    CHECK(braids["s50"] == 3);
    CHECK(braids["s4"] == 3);
}
...
TEST_CASE("the still pumping line rises through a real hole in every plate above the still") {
    ...
    CHECK(line->worldBounds.max.y >
          plate(cryo::Stage::RT).world[3].y + 0.5 * plateT(cryo::Stage::RT));
    for (cryo::Stage s : {cryo::Stage::PT2, cryo::Stage::PT1, cryo::Stage::RT}) {
        ...
        double nearest = 1e9;
        for (const auto& v : m.vertices)
            nearest = std::min(nearest, std::hypot(v.position.x - local.x, v.position.z - local.z));
    ...
    CHECK(sc.node(sc.findByDescriptor("mixing_chamber").front())->worldBounds.min.y ==
          Catch::Approx(plate(cryo::Stage::MXC).world[3].y + 0.5 * plateT(cryo::Stage::MXC))
              .margin(1e-6));
}
...
TEST_CASE("the layout's light panels become at most eight point lights on the ceiling plane") {
    ...
    CHECK(lights.front().color.w ==
          Catch::Approx(4000.0 / (4.0 * 3.14159265358979) / (1500.0 * 0.45)).epsilon(0.02));
    std::size_t panels = 0;
    for (const Node& n : sc.nodes())
        panels += n.instanceName.rfind("light_panel", 0) == 0 ? 1 : 0;
    CHECK(panels == 4);
...
TEST_CASE("the spec and theory cross-references resolve") {
    ...
    const std::string cmd =
        "cd \"" QXL_SOURCE_DIR "\" && python3 tools/check_xrefs.py > /dev/null 2>&1";
