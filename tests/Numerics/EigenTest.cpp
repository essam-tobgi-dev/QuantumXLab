    ...
    for (std::size_t n : {2u, 5u, 12u, 40u, 80u}) {
        ...
        for (bool forceOwn : {true, false}) {
            ...
            for (std::size_t i = 1; i < n; ++i)
                REQUIRE(e->values[i - 1] <= e->values[i] + 1e-12);
            Matrix D(n, n);
            for (std::size_t i = 0; i < n; ++i)
                D(i, i) = e->values[i];
...
TEST_CASE("svd reconstructs and orders singular values") {
    ...
    auto s = svd(A);
    REQUIRE(s);
    for (std::size_t i = 1; i < s->singular.size(); ++i)
        REQUIRE(s->singular[i - 1] >= s->singular[i] - 1e-12);
...
TEST_CASE("structure checks and distances") {
    ...
    std::vector<Matrix> kraus = {scale(X, std::sqrt(0.3)),
                                 scale(Matrix::identity(2), std::sqrt(0.7))};
...
TEST_CASE("equalUpToGlobalPhase") {
    ...
    // Traceless case: X vs e^{iφ}X has Tr(X†X e^{iφ}) ≠ 0, but X vs Z has zero trace overlap →
    // fallback path.
