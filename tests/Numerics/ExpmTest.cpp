...
TEST_CASE("expm matches closed form for Pauli rotations and the Hermitian path") {
    ...
    // A = -i θ/2 (n·σ)/|n|
    Matrix nsig = scale(pauli::X.toMatrix(), nx / L);
    nsig += scale(pauli::Y.toMatrix(), ny / L);
    nsig += scale(pauli::Z.toMatrix(), nz / L);
...
TEST_CASE("expmTimesVector agrees with dense expm") {
    ...
    Vector out2 = expmTimesVector(
        n, [&](std::span<const Complex> x, std::span<Complex> y) { S.matvecInto(x, y); }, v, 1e-14);
    err = 0;
    for (std::size_t i = 0; i < n; ++i)
        err = std::max(err, std::abs(ref[i] - out2[i]));
    REQUIRE(err < 1e-9);
}
...
TEST_CASE("sparse CSR operations") {
    ...
    Vector x = {1, 2, 3};
    Vector y = S.matvec(x);
    REQUIRE(y[0] == Complex(7, 0));
    REQUIRE(y[1] == Complex(9, 0));
    REQUIRE(y[2] == Complex(4, 0));
