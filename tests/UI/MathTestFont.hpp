...
struct DrawnLine { double x0 = 0, y0 = 0, x1 = 0, y1 = 0, thickness = 0; };
struct DrawnRect { double x = 0, y = 0, w = 0, h = 0; bool filled = false; };
...
public:
    ...
    std::string plain() const {
        ...
        for (const auto& g : glyphs) sorted.push_back(&g);
        std::stable_sort(sorted.begin(), sorted.end(),
                         [](const DrawnGlyph* a, const DrawnGlyph* b) { return a->x < b->x; });
        std::string s;
        for (auto* g : sorted) s += g->text;
        return s;
    }
