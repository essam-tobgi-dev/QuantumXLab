...
TEST_CASE("Panels: the spec 19 §3 catalog is complete and every panel is identifiable") {
    ...
    for (std::size_t i = 0; i < panels.size(); ++i) {
        ...
        CHECK(p->windowTitle().find(std::string("###") + std::string(p->key())) !=
              std::string::npos);
...
TEST_CASE("Workspaces: every panel appears in a preset and the presets follow spec 19 §2") {
    ...
    // Spec 19 §9: "every panel in §3 exists, appears in its workspace preset, and persists its
    // state".
    ...
    for (PanelId id : {PanelId::Viewport, PanelId::Inspector, PanelId::ComponentTree,
                       PanelId::Instruments, PanelId::Fridge, PanelId::Log, PanelId::Tour})
    ...
    for (PanelId id : {PanelId::StateViews, PanelId::Plots, PanelId::Fits, PanelId::Results,
                       PanelId::Estimates, PanelId::Theory})
...
TEST_CASE("Physical lab hides every Simulator-only panel and nothing else (spec 19 §2, 00 §6)") {
    ...
    for (std::size_t w = 0; w < kWorkspaceCount; ++w) {
        ...
        for (const PanelPtr& p : shell.panels())
            if (shell.visible(*p))
                shownSimulated.emplace_back(p->key());
        ...
        for (const PanelPtr& p : shell.panels())
            if (shell.visible(*p))
                shownPhysical.emplace_back(p->key());
        std::sort(shownSimulated.begin(),
                  shownSimulated.end()); // set_difference wants sorted input
        std::sort(shownPhysical.begin(), shownPhysical.end());
        std::vector<std::string> removed;
        std::set_difference(shownSimulated.begin(), shownSimulated.end(), shownPhysical.begin(),
                            shownPhysical.end(), std::back_inserter(removed));
...
TEST_CASE("Layout: the project JSON round-trips through the envelope (spec 19 §4, 23 §2)") {
    ...
    state.panels["code_editor"] =
        core::Json::object({{"text", "OPENQASM 3.0;\n"}, {"caret_line", 1}});
...
TEST_CASE("Shell: a layout restores the workspace, the visibility set and each panel's state") {
    ...
    editor->deserialize(
        core::Json::object({{"text", "qubit[2] q;\n"}, {"caret_line", 1}, {"caret_column", 1}}));
    ...
    CHECK(fresh.panel(PanelId::CodeEditor)->serialize().at("text").get<std::string>() ==
          "qubit[2] q;\n");
