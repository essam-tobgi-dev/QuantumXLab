...
TEST_CASE("slug rule matches the shipped anchors") {
    ...
    REQUIRE(slugify("6.4 Purcell decay and the Purcell filter") ==
            "6.4-purcell-decay-and-the-purcell-filter");
    REQUIRE(slugify("1.1 RF confinement and the pseudopotential") ==
...
TEST_CASE("link classification covers every target form") {
    REQUIRE(classifyLink("T05-superconducting-qubits.md#6.3-dispersive-readout").kind ==
            LinkKind::Theory);
    REQUIRE(classifyLink("T05-superconducting-qubits.md#6.3-dispersive-readout").doc == "T05");
    REQUIRE(classifyLink("T05-superconducting-qubits.md#6.3-dispersive-readout").anchor ==
...
TEST_CASE("fixture document parses every block type") {
    ...
    for (const auto& s : intro->spans) {
        ...
        if (s.link.kind == LinkKind::Theory) {
            link = true;
            REQUIRE(s.link.doc == "T05");
        }
    }
    REQUIRE(bold);
    REQUIRE(ital);
    ...
    for (const auto& b : d.blocks)
        if (b.kind == BlockKind::Table) {
            ...
            bool hasMath = false;
            for (const auto& s : b.rows[1][1].spans)
                hasMath |= s.math;
            REQUIRE(hasMath);
        }
    ...
    for (const auto& b : d.blocks)
        for (const auto& s : b.spans)
            if (s.link.kind != LinkKind::None)
                kinds.push_back(s.link.kind);
