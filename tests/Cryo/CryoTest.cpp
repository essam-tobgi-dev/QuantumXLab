...
const MaterialCatalog& mats() {
    static MaterialCatalog m = [] {
        auto r = MaterialCatalog::load();
        REQUIRE(r);
        return *r;
    }();
    return m;
...
TEST_CASE("coax conduction load per metre matches the T08 table order") {
    CoaxCatalog cc;
    auto ss = cc.get("SS_086");
    REQUIRE(ss);
    auto q = conductionLoad(**ss, 1.0, 4.0, 300.0, mats());
    REQUIRE(q);
    // T08 §4.1: 0.86 mm stainless coax 300→4 K per metre ≈ 1–2 mW (inner+outer+PTFE); accept 0.5–4
    // mW.
    REQUIRE(q->total() > 0.5e-3);
    REQUIRE(q->total() < 4e-3);
    auto nbti = cc.get("NbTi_086");
    REQUIRE(nbti);
...
TEST_CASE("attenuator dissipation and thermal photon helpers") {
    ...
    REQUIRE(thermalPhotons(5e9, 293.0) ==
            Approx(1220).epsilon(0.03)); // T07 §9 quotes 1250 for 300 K
...
TEST_CASE("reference wiring loads, validates, and has the standard chains") {
    ...
    const WiringLine* d = w.find("drive_q0");
    REQUIRE(d);
    ...
    auto ro = ChainCatalog::instantiate("readout_in_std", "ri", "m[0]");
    REQUIRE(ro);
    REQUIRE(ro->totalAttenuation_dB() == Approx(70.0));
    auto d60 = ChainCatalog::instantiate("drive_60dB", "d60", "d[9]");
    REQUIRE(d60);
...
TEST_CASE("wiring validation rejects misplaced elements") {
    ...
    Element att;
    att.kind = ElementKind::Attenuator;
    att.stage = Stage::PT2;
    att.attenuation_dB = 20;
    att.id = "att_bad";
    ...
    // Duplicate channel.
    Wiring w5;
    w5.lines = {*w.find("drive_q3"), *w.find("drive_q3")};
