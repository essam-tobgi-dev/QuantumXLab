...
struct Fixture {
    ...
    StageArray nominalT() const {
        StageArray T{};
        for (Stage s : kStages)
            T[stageIndex(s)] = nominalTemperature(s);
        return T;
    }
};
} // namespace
...
TEST_CASE("steady state with the 27-qubit reference wiring reaches nominal stage temperatures") {
    ...
    REQUIRE(s);
    INFO("T = " << s->T_K[1] << " " << s->T_K[2] << " " << s->T_K[3] << " " << s->T_K[4] << " "
                << s->T_K[5]);
    INFO("loads = " << s->load_W[1] << " " << s->load_W[2] << " " << s->load_W[3] << " "
                    << s->load_W[4] << " " << s->load_W[5]);
    REQUIRE(s->steady);
    REQUIRE(s->T_K[stageIndex(Stage::PT1)] > 30.0);
    REQUIRE(s->T_K[stageIndex(Stage::PT1)] < 60.0);
    REQUIRE(s->T_K[stageIndex(Stage::PT2)] > 2.8);
    REQUIRE(s->T_K[stageIndex(Stage::PT2)] < 4.5);
    REQUIRE(s->T_K[stageIndex(Stage::STILL)] < 1.2);
    REQUIRE(s->T_K[stageIndex(Stage::CP)] < 0.2);
    double Tmxc = s->T_K[stageIndex(Stage::MXC)];
    REQUIRE(Tmxc > 0.010);
    REQUIRE(Tmxc < 0.025);
    // Spec 11 worked example: PT2 load for ~40 coax lines + 4 HEMTs is a few hundred mW, below 1.5
    // W.
    ...
    Wiring big = f.wiring;
    for (auto l : f.wiring.lines) {
        l.id += "_b";
        l.channel += "_b";
        big.lines.push_back(l);
    }
...
TEST_CASE("applied RF power heats the attenuators at the mixing chamber") {
    ...
    double extraPT2 = s1->loads[stageIndex(Stage::PT2)].dissipation_W -
                      s0->loads[stageIndex(Stage::PT2)].dissipation_W;
    REQUIRE(extraPT2 > 0.3e-3);
    REQUIRE(extraPT2 < 1.0e-3);
    double extra = s1->loads[stageIndex(Stage::MXC)].dissipation_W -
                   s0->loads[stageIndex(Stage::MXC)].dissipation_W;
    REQUIRE(extra > 3e-6);
    REQUIRE(extra < 1e-5);
...
TEST_CASE("photon-number budget reproduces the T07 §9 tables") {
    ...
    NoiseBudgetOptions opt;
    opt.f_Hz = 5e9;
    opt.n_in = 1250.0;
    auto d = ChainCatalog::instantiate("drive_std", "d", "d[0]");
    REQUIRE(d);
    ...
    auto r = ChainCatalog::instantiate("readout_in_std", "r", "m[0]");
    REQUIRE(r);
    ...
    auto d60 = ChainCatalog::instantiate("drive_60dB", "d60", "d[1]");
    REQUIRE(d60);
...
TEST_CASE("transient cooldown follows the published timeline order of magnitude") {
    ...
    while (t < 72.0 * 3600.0) {
        ...
        if (tPT2cold < 0 && T[stageIndex(Stage::PT2)] < 4.5)
            tPT2cold = t;
        if (tBase < 0 && T[stageIndex(Stage::MXC)] < 0.030) {
            tBase = t;
            break;
        }
    }
