#pragma once
// Shared helpers for the graphics oracles: hidden window, screenshot → luminance image, cube readback.
...
inline std::unique_ptr<Window> makeHidden(int w = 320, int h = 240) {
    WindowDesc d; d.visible = false; d.width = w; d.height = h; d.vsync = false;
...
struct LumImage {
    ...
    float at(int x, int y) const { return lum[static_cast<std::size_t>(y) * static_cast<std::size_t>(w) + static_cast<std::size_t>(x)]; }
};
inline LumImage grab(const Renderer& r, const std::string& name) {
    const auto out = std::filesystem::temp_directory_path() / name;
    if (!r.screenshot(out)) return {};
    int w = 0, h = 0, n = 0;
    unsigned char* px = stbi_load(out.string().c_str(), &w, &h, &n, 4);
    if (!px) return {};
    LumImage img; img.w = w; img.h = h; img.lum.resize(static_cast<std::size_t>(w) * static_cast<std::size_t>(h));
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            const unsigned char* p = px + (static_cast<std::size_t>(y) * static_cast<std::size_t>(w) + static_cast<std::size_t>(x)) * 4;
            img.lum[static_cast<std::size_t>(y) * static_cast<std::size_t>(w) + static_cast<std::size_t>(x)] =
...
struct Stats { double mean = 0, stddev = 0; std::size_t n = 0; };
inline Stats stats(const std::vector<float>& v) {
    Stats s; s.n = v.size();
    if (v.empty()) return s;
    double sum = 0; for (float x : v) sum += x;
    s.mean = sum / static_cast<double>(v.size());
    double var = 0; for (float x : v) var += (x - s.mean) * (x - s.mean);
...
inline std::vector<float> cubeLuminance(const TextureCube& c, int level, int face = -1) {
    ...
    for (int f = 0; f < 6; ++f) {
        if (face >= 0 && f != face) continue;
        auto px = c.readFace(f, level);
        for (std::size_t i = 0; i + 3 < px.size(); i += 4) out.push_back(0.2126f * px[i] + 0.7152f * px[i + 1] + 0.0722f * px[i + 2]);
    }
    return out;
}
...
inline std::vector<float> readTexture(const Texture2D& t, GLenum format, int channels) {
    std::vector<float> px(static_cast<std::size_t>(t.width()) * static_cast<std::size_t>(t.height()) * static_cast<std::size_t>(channels));
