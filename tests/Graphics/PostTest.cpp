...
namespace {
glm::mat4 at(glm::vec3 p) {
    return glm::translate(glm::mat4(1.0f), p);
}
Material matte(glm::vec3 c) {
    Material m;
    m.baseColor = glm::vec4(c, 1.0f);
    m.metallic = 0.0f;
    m.roughness = 0.8f;
    return m;
}
...
TEST_CASE("SSAO: the crease where a box meets a plane is occluded, the open plane is not") {
    ...
    Camera cam;
    cam.lookAt({2.2, 1.3, 0.0}, {0.5, 0.05, 0.0});
    ...
    for (int i = -4; i <= 4; ++i) {
        ...
        crease.push_back(
            aoAt({0.505, 0.0, z})); // the crease pixel: on the plane, 5 mm from the box
...
TEST_CASE("soft shadow: the penumbra of a box on a plane spans at least 3 pixels on each edge") {
    ...
    Camera cam;
    cam.lookAt({0, 5, 0}, {0, 0, 0}, {0, 0, -1});
    cam.setOrtho(true);
    Bookmark b = cam.bookmark();
    b.ortho = true;
    b.orthoHeight = 3.0;
    cam.set(b);
    cam.setAspect(320.0 / 240.0);
    ...
    for (int x = 0; x < W; ++x)
        line.push_back(img.at(x, row));
    float lo = 1.0f, hi = 0.0f;
    int imin = 0;
    for (int x = 0; x < W; ++x) {
        if (line[static_cast<std::size_t>(x)] < lo) {
            lo = line[static_cast<std::size_t>(x)];
            imin = x;
        }
        hi = std::max(hi, line[static_cast<std::size_t>(x)]);
    }
    ...
    INFO("penumbra pixels left " << left << " right " << right << " (lit " << hi << ", shadow "
                                 << lo << ")");
    CHECK(left >= 3);
    CHECK(right >= 3);
}
...
TEST_CASE("bloom: an unlit emissive sphere brightens pixels 6–10 px outside its silhouette") {
    ...
    auto render = [&](bool bloom) {
        ...
        Material m;
        m.baseColor = {0.1f, 1.0f, 0.3f, 1};
        m.emissive = {0.1f, 1.0f, 0.3f};
        m.emissiveStrength = 6.0f;
        m.unlit = true;
        Camera cam;
    ...
    int n = 0;
    for (int d = 6; d <= 10; ++d) {
        halo += on.at(160 + radius + d, 120) - off.at(160 + radius + d, 120);
        ++n;
        halo += on.at(160 - radius - d, 120) - off.at(160 - radius - d, 120);
        ++n;
        halo += on.at(160, 120 + radius + d) - off.at(160, 120 + radius + d);
        ++n;
        halo += on.at(160, 120 - radius - d) - off.at(160, 120 - radius - d);
        ++n;
    }
...
TEST_CASE("post chain GPU time at 3200x2000 (SSAO + blur + bloom + outline + tonemap)") {
    ...
    for (int i = 0; i < 20; ++i)
        for (int j = 0; j < 20; ++j)
            inst.push_back({at({i * 0.35f - 3.5f, 0.1f, j * 0.35f - 3.5f}),
                            {1000u + static_cast<unsigned>(i * 20 + j), 0u},
                            {},
                            {1, 1, 1, 1}});
    Camera cam;
    cam.lookAt({5, 4, 5}, {0, 0, 0});
    cam.setAspect(1.6);
    cam.setClip(0.1, 50.0);
    const int W = 3200, H = 2000;
    double aoMs = 0, bloomMs = 0, postMs = 0, resMs = 0;
    int n = 0;
    for (int f = 0; f < 12; ++f) {
        r.beginFrame(W, H, cam, 0.0);
        ...
        if (f >= 4) {
            aoMs += r.stats().aoMs;
            bloomMs += r.stats().bloomMs;
            postMs += r.stats().postMs;
            resMs += r.stats().resolveMs;
            ++n;
        }
    }
    aoMs /= n;
    ...
    std::printf("[post-chain @ %dx%d, selection outline on] ssao+blur %.2f ms, bloom %.2f ms, "
                "resolve+composite+outline+tonemap %.2f ms, "
