...
std::unique_ptr<Window> makeHidden() {
    WindowDesc d; d.visible = false; d.width = 320; d.height = 240; d.vsync = false;
...
TEST_CASE("all shaders compile and link") {
    ...
    for (auto& e : std::filesystem::directory_iterator(shaderRoot())) {
        auto p = e.path();
        if (p.extension() != ".frag") continue;
        std::string stem = p.stem().string();
        ShaderDesc d; d.name = stem;
        d.vertex = std::filesystem::exists(shaderRoot() / (stem + ".vert")) ? stem + ".vert"
                 : stem == "id" ? "pbr.vert"
                 : stem.starts_with("env_") ? "cube_face.vert"   // IBL generation (spec 18 §5 EnvPrefilter)
                 : "fullscreen.vert";
...
TEST_CASE("render a box: pick returns its id, screenshot written") {
    auto win = makeHidden();
    if (!win) { SKIP("no GL context available"); }
    RendererDesc rd; rd.samples = 4; rd.shadowSize = 512;
    ...
    for (int i = 0; i < 5; ++i) inst.push_back({glm::translate(glm::mat4(1.0f), {i * 0.3f - 0.6f, 0, 0}) * glm::scale(glm::mat4(1.0f), glm::vec3(0.1f)), {100u + static_cast<unsigned>(i), 0u}, {}, {1, 1, 1, 1}});
    r.beginFrame(320, 240, cam, 0.0);
...
TEST_CASE("world-space overlay lines follow the camera (no displacement far from the origin)") {
    ...
    auto win = makeHidden();
    if (!win) { SKIP("no GL context available"); }
    RendererDesc rd; rd.samples = 4; rd.shadowSize = 256;
    rd.clearColor = {0.0f, 0.0f, 0.0f};
    auto rr = Renderer::create(rd);
    REQUIRE(rr.has_value());
    auto& r = **rr;
    const glm::dvec3 target{120.0, 45.0, -80.0};   // far from the world origin
    Camera cam; cam.lookAt(target + glm::dvec3(0, 0, 3), target); cam.setAspect(1.0);
    auto brightness = [&](bool withLine) {
        ...
        if (withLine) {
            ...
            r.linesNoDepth().segment(target - glm::dvec3(0.5, 0, 0), target + glm::dvec3(0.5, 0, 0), {1, 1, 1, 1}, 12.0f);
            r.linesNoDepth().segment(target - glm::dvec3(0, 0.5, 0), target + glm::dvec3(0, 0.5, 0), {1, 1, 1, 1}, 12.0f);
        }
        r.endFrame();
        ...
        for (int y = h / 2 - 4; y < h / 2 + 4; ++y)
            for (int x = w / 2 - 4; x < w / 2 + 4; ++x) sum += px[(y * w + x) * 4];
        stbi_image_free(px);
        return sum / 64;
    };
...
TEST_CASE("asynchronous pick returns the id and the surface point two frames later without a stall") {
    auto win = makeHidden();
    if (!win) { SKIP("no GL context available"); }
    RendererDesc rd; rd.samples = 4; rd.shadowSize = 256;
    auto rr = Renderer::create(rd);
