...
TEST_CASE("Camera: dollyToward keeps the anchor under the same pixel and shortens the distance by "
          "0.9 per step") {
    Camera cam;
...
TEST_CASE(
    "Camera: setPivot moves the orbit centre along the viewing line without moving the view") {
    Camera cam;
...
TEST_CASE("Camera: unproject inverts project at the stored depth") {
    ...
    for (const glm::dvec3 p :
         {glm::dvec3{0.3, 0.4, -0.2}, glm::dvec3{-1.0, 0.0, 1.0}, glm::dvec3{0.0, 2.0, -3.0}}) {
        glm::dvec2 px{};
    ...
    CHECK(glm::dot(far - cam.position(), cam.forward()) ==
          Approx(60.0).epsilon(1e-4)); // float projection
