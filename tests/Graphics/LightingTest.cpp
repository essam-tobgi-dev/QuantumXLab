...
TEST_CASE("BRDF LUT: A→1, B→0 at normal incidence and zero roughness; A+B bounded and monotone in "
          "roughness") {
    auto win = makeHidden();
    if (!win) {
    ...
    auto ab = [&](int x, int y) {
        const std::size_t i = (static_cast<std::size_t>(y) * 128 + static_cast<std::size_t>(x)) * 2;
        return glm::vec2(px[i], px[i + 1]);
    };
...
TEST_CASE("environment: prefiltered mip 0 is structured, mip 5 is smooth within every face, +Y "
          "outshines -Y by 2x") {
    auto win = makeHidden();
    if (!win) {
        SKIP("no GL context available");
    ...
    // 5.0–5.3 % on the +Y and +Z faces, which is what the bound accommodates. It is NOT uniform
    // across the cube — the ceiling/floor asymmetry that the +Y/−Y oracle below demands survives
    // any cosine convolution (measured ≈ 0.3 of the mean; see SPEC_DEVIATIONS).
...
TEST_CASE(
    "IBL on a sphere: a polished metal reflects the room with the sun off; black when IBL is off") {
    auto win = makeHidden();
    if (!win) {
        SKIP("no GL context available");
    }
    auto render = [&](bool ibl) {
        ...
        Material m;
        m.baseColor = {0.95f, 0.95f, 0.95f, 1};
        m.metallic = 1.0f;
        m.roughness = 0.1f;
        Camera cam;
    ...
    for (int y = 0; y < on.h; ++y)
        for (int x = 0; x < on.w; ++x) {
            ...
            if (dy < -rad / 2)
                top.push_back(on.at(x, y));
            if (dy > rad / 2)
                bottom.push_back(on.at(x, y));
        }
    auto sd = stats(disc);
...
TEST_CASE("Material presets carry measured F0 and the amended roughness table (spec 18 §4)") {
    ...
    CHECK(gold.baseColor.r == Approx(1.00f));
    CHECK(gold.baseColor.g == Approx(0.71f));
    CHECK(gold.baseColor.b == Approx(0.29f));
    CHECK(gold.roughness == Approx(0.28f));
    CHECK(gold.metallic == Approx(1.0f));
    auto cu = Material::preset("copper");
    CHECK(cu.baseColor.r == Approx(0.95f));
    CHECK(cu.baseColor.g == Approx(0.64f));
    CHECK(cu.baseColor.b == Approx(0.54f));
    CHECK(cu.roughness == Approx(0.38f));
    auto al = Material::preset("aluminium");
    CHECK(al.baseColor.r == Approx(0.91f));
    CHECK(al.baseColor.g == Approx(0.92f));
    CHECK(al.baseColor.b == Approx(0.92f));
    CHECK(al.roughness == Approx(0.5f));
    auto ss = Material::preset("stainless");
    CHECK(ss.baseColor.r == Approx(0.56f));
    CHECK(ss.baseColor.g == Approx(0.57f));
    CHECK(ss.baseColor.b == Approx(0.58f));
    CHECK(ss.roughness == Approx(0.35f));
    auto nb = Material::preset("niobium");
    CHECK(nb.baseColor.r == Approx(0.66f));
    CHECK(nb.baseColor.g == Approx(0.65f));
    CHECK(nb.baseColor.b == Approx(0.68f));
}
