...
namespace {
compiler::CompiledProgram compiled(const std::string& src, const std::string& id, compiler::CompileOptions o = {}) {
    o.pulseLevel = true;
...
std::vector<const pulse::Play*> playsOn(const pulse::Schedule& s, pulse::ChannelId ch) {
    ...
    for (const auto& i : s.instructions())
        if (const auto* p = std::get_if<pulse::Play>(&i); p && p->ch == ch) v.push_back(p);
    return v;
}
} // namespace

TEST_CASE("a Bell circuit on sc_fixed_5 lowers to a schedule that passes pulse::verify") {
    const auto out = compiled(program("qubit[2] q; bit[2] c; h q[0]; cx q[0], q[1]; c = measure q;"), "sc_fixed_5");
    ...
    for (const auto& i : s.instructions()) {   // every pulse lies inside the slot of some node
        ...
        const bool inside = std::any_of(out.pulses->windows.begin(), out.pulses->windows.end(), [&](const auto& w) { return w.start <= t0 && t1 <= w.end; });
        CHECK(inside);
    }
...
TEST_CASE("rz lowers to shift_phase(−θ) on the drive frame: the accumulated virtual-Z phase") {
    const auto out = compiled(program("pragma qlab.layout physical\npragma qlab.optimize 0\nrz(0.3) $1; sx $1; rz(0.5) $1; sx $1;"), "sc_fixed_5");
...
c = measure $0;
    ...
    CHECK(plays[2]->wf.kind == pulse::WaveformKind::Gaussian);               // the named waveform of the cal block
    ...
    CHECK(out.timing.nodes[1].duration.get() == 7 * granule + 14 * granule); // 24 ns + 48 ns on the grid
    ...
    CHECK(std::abs(frames.phaseOps(pulse::ChannelId::drive(0), pulse::secondsOf(out.timing.duration)) - 0.25) < 1e-12);
...
TEST_CASE("the shipped pulse examples lower and verify") {
    for (const char* name : {"defcal_x_custom", "drag_beta_scan", "cross_resonance_echo"}) {
        const auto path = core::assetDir() / "Programs" / "Examples" / "Pulse" / (std::string(name) + ".qasm");
    ...
    const auto echo = compiled(*core::readTextFile(core::assetDir() / "Programs" / "Examples" / "Pulse" / "cross_resonance_echo.qasm"), "sc_fixed_5");
...
kick $1;
    ...
    const auto ff = compiled(program("pragma qlab.layout physical\nbit c; x $0; c = measure $0; if (c == 1) { x $0; }"), "sc_fixed_5");
    std::size_t conditional = 0;
    for (const auto& w : ff.pulses->windows) {
        if (!w.conditional) { CHECK(w.parent == compiler::PulseWindow::kTopLevel); continue; }
        ++conditional;
        ...
        CHECK(w.start.get() == ff.timing.nodes[2].start.get() + pulse::quantise(200e-9, Picoseconds{222}, 16).get());
        CHECK(w.start >= ff.timing.nodes[1].end());
    }
    CHECK(conditional == 1);
    auto withElse = parse(program("pragma qlab.layout physical\nbit c; c = measure $0; if (c == 1) { x $0; } else { sx $0; }"));
...
TEST_CASE("port names map to channels; ions and tunable couplers lower and verify") {
    ...
    for (auto ch : ion.pulses->schedule.channels()) bichromatic = bichromatic || ch.kind == pulse::ChannelKind::Bichromatic;
    CHECK(bichromatic);
    const auto grid = compiled(bell, "sc_tunable_grid_54");
    CHECK(countGates(grid.circuit, "cz") == 1);
    bool flux = false;
    for (auto ch : grid.pulses->schedule.channels()) flux = flux || ch.kind == pulse::ChannelKind::Flux;
    CHECK(flux);
