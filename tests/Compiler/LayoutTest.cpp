...
ir::Circuit line(std::uint32_t n) {
    ...
    for (std::uint32_t q = 0; q + 1 < n; ++q)
        gs.push_back(G("cx", {q, q + 1}));
    return circuit(n, gs);
}
bool everyInteractionCoupled(const compiler::Layout& l, const compiler::InteractionGraph& ig,
                             const compiler::CouplingGraph& g) {
    return std::all_of(ig.edges.begin(), ig.edges.end(),
                       [&](const auto& e) { return g.adjacent(l.v2p[e.a], l.v2p[e.b]); });
}
} // namespace
...
TEST_CASE("coupling graph: hop distances, data qubits only, reliability distances") {
    ...
    const double want = -std::log1p(-cal.edge(0, 1)->gateError2q.value) -
                        std::log1p(-cal.edge(1, 3)->gateError2q.value) -
    ...
    // 54 data qubits on a grid, 93 couplers that never hold a program qubit: hops = Manhattan
    // distance.
    ...
    for (std::uint32_t a : grid.dataQubits()) {
        ...
        for (std::uint32_t b : grid.dataQubits()) {
            const double manhattan = std::abs(dev.qubits[a].pos[0] - dev.qubits[b].pos[0]) +
                                     std::abs(dev.qubits[a].pos[1] - dev.qubits[b].pos[1]);
...
TEST_CASE("VF2 embeds a line into heavy-hex exactly; trivial and dense layouts are valid") {
    for (const auto& [id, length] : std::vector<std::pair<std::string, std::uint32_t>>{
             {"sc_heavyhex_27", 12}, {"sc_heavyhex_127", 40}}) {
        ...
        for (std::uint32_t v = 0; v < length; ++v)
            CHECK(trivial->v2p[v] == g.dataQubits()[v]);
        auto dense = compiler::denseLayout(ig, g, 7);
        REQUIRE(dense.has_value());
        CHECK(compiler::validateLayout(*dense, length, g).has_value());
        // The dense subgraph is connected: a BFS from one member reaches all of them inside the
        // set.
        ...
        while (!stack.empty()) {
            ...
            for (auto q : g.neighbours(p))
                if (members.contains(q) && seen.insert(q).second)
                    stack.push_back(q);
        }
...
TEST_CASE(
    "VF2 proves that a degree-4 star does not embed in heavy-hex; a tiny budget reports QL4060") {
    const auto g = graphOf("sc_heavyhex_27");
    const ir::Circuit star =
        circuit(5, {G("cx", {0, 1}), G("cx", {0, 2}), G("cx", {0, 3}), G("cx", {0, 4})});
...
TEST_CASE(
    "noise-aware layout reaches the brute-force optimum of the spec 14 §7 score on sc_fixed_5") {
    ...
    const std::vector<ir::Circuit> programs = {
        ...
        build(program("qubit[3] q; bit[3] c; h q[0]; cx q[0], q[1]; cx q[1], q[2]; cx q[1], q[2]; "
                      "c = measure q;")),
    ...
    for (const auto& c : programs) {
        ...
        auto choice =
            compiler::chooseLayout(c, g, compiler::LayoutPolicy::NoiseAware, 5, 1'000'000, nullptr);
        REQUIRE(choice.has_value());
        INFO("k = " << k << ", layout " << choice->layout.text());
        CHECK(std::abs(choice->score - best) <= 1e-12 * std::max(1.0, best));
        auto dense =
            compiler::chooseLayout(c, g, compiler::LayoutPolicy::Dense, 5, 1'000'000, nullptr);
        REQUIRE(dense.has_value());
    ...
    for (std::uint32_t q : {0u, 1u})
        want += -std::log(cal.qubits[q].readoutFidelity()) + tCirc / cal.qubits[q].t1.value.si();
