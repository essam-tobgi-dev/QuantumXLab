...
ir::Circuit randomCircuit(core::Random& rng, std::uint32_t n, std::size_t twoQubitGates) {
    ...
    while (twoQubitGates > 0) {
        ...
        case 0: gs.push_back(G("h", {a})); break;
        case 1: gs.push_back(G("rz", {a}, {angle(rng)})); break;
        default: gs.push_back(G("cx", {a, b})); --twoQubitGates; break;
        }
    }
...
void requireOnCouplingMap(const ir::Circuit& c, const hw::Device& dev) {
    ...
    for (const ir::Gate* g : gatesOf(c)) {
        for (ir::Wire w : g->wires()) REQUIRE_FALSE(dev.isCoupler(w.index));
        if (g->width() == 2) {
...
TEST_CASE("routing respects the coupling map and preserves the circuit on the shipped devices") {
    core::Random rng(99);
    struct Case { std::string id; std::uint32_t qubits; std::size_t gates; };
    for (const Case& k : {Case{"sc_fixed_5", 5, 40}, Case{"sc_heavyhex_27", 9, 50}, Case{"sc_heavyhex_127", 10, 40},
        ...
        for (int trial = 0; trial < 3; ++trial) {
            ...
            auto layout = compiler::chooseLayout(source, g, trial == 0 ? compiler::LayoutPolicy::Trivial : compiler::LayoutPolicy::NoiseAware, 11, 100000, nullptr);
            ...
            CHECK(routed->circuit.meta()["final_layout"].get<std::vector<std::uint32_t>>() == routed->finalLayout.v2p);
            CHECK(routed->initialLayout.injective());
            CHECK(routed->finalLayout.injective());
            if (device(k.id).device.allToAll) CHECK(routed->swaps == 0);
            ...
            INFO(report->detail << " on " << report->wires << " wires by " << compiler::equivalenceMethodName(report->method));
        ...
        auto permutation = [](const compiler::Layout& l) {
            ...
            for (std::size_t x = 0; x < 32; ++x) {
                ...
                for (std::uint32_t v = 0; v < 5; ++v)
                    if ((x >> v) & 1u) y |= std::size_t{1} << l.v2p[v];
                p(y, x) = 1.0;
            }
            return p;
        };
        const num::Matrix want = num::matmul(num::matmul(permutation(routed->finalLayout), unitary(source)),
...
cx q[0], q[2]; out = measure q;
    ...
    for (auto id : routed->circuit.topologicalOrder())
        if (const auto* b = std::get_if<ir::Branch>(&routed->circuit.node(id))) branch = b;
    ...
    for (const ir::Circuit* body : {&*branch->thenBody, &*branch->elseBody}) {
        ...
        for (const ir::Gate* gt : gatesOf(*body))
            if (gt->width() == 2) CHECK(g.adjacent(gt->wires()[0].index, gt->wires()[1].index));
        CHECK(countGates(*body, "swap") % 2 == 0);   // every swap of a body is undone at its end
    }
    CHECK(countGates(*branch->thenBody, "swap") >= 2);   // q1–q3 are not coupled under any placement of the star
    ...
    for (const ir::Gate* gt : gatesOf(*branch->thenBody))
        if (gt->name == "swap") CHECK(gt->span.line == 5);
    for (const ir::Gate* gt : gatesOf(routed->circuit))
        if (gt->name == "swap") { CHECK(gt->span.line >= 4); CHECK(gt->span.file == "test.qasm"); }
...
TEST_CASE("routing none validates the coupling map (QL4030); physical programs are adopted as written") {
    const auto g = graphOf("sc_fixed_5");
    ...
    const ir::Circuit physical = build(program("pragma qlab.layout physical\nbit c; h $1; cx $1, $3; c = measure $3;"));
    ...
    auto wrong = compiler::adoptPhysical(build(program("pragma qlab.layout physical\ncx $0, $4;")), g);
    ...
    CHECK_FALSE(compiler::route(ok, compiler::Layout{{0, 1, 60}}, graphOf("sc_tunable_grid_54")).has_value());
}

TEST_CASE("performance: SABRE on sc_heavyhex_127 with 5000 two-qubit gates (spec 24 §6: 500 ms in Release)") {
    ...
    std::cout << "[perf] SABRE 127 qubits, 5000 two-qubit gates: " << ms << " ms, " << routed->swaps << " swaps (budget 500 ms in Release)\n";
#ifdef NDEBUG
    CHECK(ms < 500.0);
#else
    CHECK(ms < 30000.0);   // loose bound for the unoptimised Debug build on a shared machine (≈ 3 s alone)
#endif
}
