    ...
    for (std::size_t k = 0; k < length; ++k) {
        ...
        case Basis1q::ZSX:
            if (pick < 3)
                ...
                gs.push_back(G("cz", {a, b}));
            break;
        case Basis1q::ZYZ:
            if (pick < 3)
                gs.push_back(G("rz", {a}, {theta()}));
            else if (pick < 5)
                gs.push_back(G("rx", {a}, {theta()}));
            else if (pick < 7)
                gs.push_back(G("ry", {a}, {theta()}));
            else
                gs.push_back(G("ms", {a, b}, {theta() / 2}));
            break;
        case Basis1q::U:
            if (pick < 6)
                gs.push_back(G("U", {a}, {theta(), theta(), theta()}));
            else
                gs.push_back(G("cx", {a, b}));
            break;
        }
...
std::size_t pulses(const ir::Circuit& c) {
    ...
    for (const auto* g : gatesOf(c))
        if (g->targets.size() == 1 && !compiler::isFrameChange(g->name))
            ++p;
    return p;
}
} // namespace
...
TEST_CASE("optimizing a decomposed circuit keeps it native and equivalent to the source") {
    ...
    for (const auto& [label, target] : std::vector<std::pair<std::string, compiler::Target>>{
             {"{U,cx}", compiler::Target::universal()},
             {"sc cx", targetOf("sc_fixed_5")},
             {"sc ecr", targetOf("sc_fixed_5", "ecr")},
             {"grid cz", targetOf("sc_tunable_grid_54")},
             {"ion", targetOf("ion_chain_11")}})
        for (int trial = 0; trial < 6; ++trial) {
            ...
            for (int k = 0; k < 30; ++k) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    gs.push_back(G("cp", {a, b}, {angle(rng)}));
                    break;
                case 2:
                    gs.push_back(G("swap", {a, b}));
                    break;
                case 3:
                    gs.push_back(G("ry", {a}, {angle(rng)}));
                    break;
                default:
                    gs.push_back(G(names1[rng.uniformInt(7)], {a}));
                    break;
                }
            }
...
TEST_CASE(
    "§5.4 virtual Z: rz gates become zero-duration frame changes and the frame phase is recorded") {
    ir::Circuit c = circuit(2, {G("rz", {0}, {0.3}), G("sx", {0}), G("rz", {0}, {0.5}),
                                G("cx", {0, 1}), G("rz", {1}, {-kPi}), G("x", {1})});
    ...
    for (const auto* g : gatesOf(c)) {
        if (g->name == "rz") {
            ...
            CHECK(g->duration->get() == 0);
        } else
            CHECK_FALSE(g->duration.has_value());
    }
    ...
    CHECK(vz["phases"]["node"].get<std::vector<std::uint32_t>>() ==
          std::vector<std::uint32_t>{1, 3, 5});
    CHECK(vz["phases"]["wire"].get<std::vector<std::uint32_t>>() ==
