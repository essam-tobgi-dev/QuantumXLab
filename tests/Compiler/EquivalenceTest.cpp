...
namespace {
compiler::EquivalenceReport check(const ir::Circuit& a, const ir::Circuit& b,
...
ir::Circuit mapped(std::uint32_t wires, std::vector<ir::Gate> gates,
                   std::vector<std::uint32_t> initial, std::vector<std::uint32_t> last) {
...
TEST_CASE("global phase is ignored, a 1e-6 angle error or a relative phase is not") {
    const ir::Circuit ref = circuit(3, {G("h", {0}), G("t", {0}), G("cx", {0, 1}),
                                        G("ry", {2}, {0.4}), G("ccx", {0, 1, 2}), G("s", {1})});
    const ir::Circuit phased =
        circuit(3, {G("h", {0}), G("rz", {0}, {kPi / 4}), G("cx", {0, 1}), G("ry", {2}, {0.4}),
                    G("ccx", {0, 1, 2}), G("rz", {1}, {kPi / 2}), G("gphase", {}, {1.3})});
    const double delta = 2e-4;
    const ir::Circuit off =
        circuit(3, {G("h", {0}), G("t", {0}), G("cx", {0, 1}), G("ry", {2}, {0.4 + delta}),
                    G("ccx", {0, 1, 2}), G("s", {1})});
    const ir::Circuit relative =
        circuit(3, {G("h", {0}), G("t", {0}), G("cx", {0, 1}), G("ry", {2}, {0.4}),
                    G("ccx", {0, 1, 2}), G("sdg", {1})});
    for (EquivalenceMethod m :
         {EquivalenceMethod::Auto, EquivalenceMethod::Unitary, EquivalenceMethod::RandomStates}) {
...
TEST_CASE(
    "the recorded layouts are undone before comparing; routing ancillas start and end in |0>") {
    ...
    const std::vector<ir::Gate> gates = {G("h", {2}), G("t", {0}), G("swap", {2, 1}),
                                         G("cx", {1, 0})};
    for (EquivalenceMethod m : {EquivalenceMethod::Unitary, EquivalenceMethod::RandomStates}) {
        ...
        CHECK_FALSE(
            check(ref, mapped(5, gates, {2, 0}, {2, 0}), with(m)).equivalent); // wrong final layout
        CHECK_FALSE(check(ref, mapped(5, gates, {1, 0}, {1, 0}), with(m))
                        .equivalent); // wrong initial layout
...
TEST_CASE("Clifford circuits of any width are compared exactly through their Pauli tableaux") {
    ...
    std::vector<ir::Gate> a{G("h", {0})},
        b{G("rz", {0}, {kPi / 2}), G("sx", {0}), G("rz", {0}, {kPi / 2})};
    ...
    // The first generator whose image carries an X on qubit 30 is Z_0 → X_0 X_1 ⋯ X_59: its sign
    // flips.
    ...
    CHECK(t->imageZ(0).text() ==
          "+XY"); // Z0 → X0 → Y0 → Y0 X1 (X on a control spreads to the target)
    ...
    CHECK(compiler::cliffordTableauOf(
              circuit(2, {G("ecr", {0, 1}), G("iswap", {0, 1}), G("rxx", {0, 1}, {kPi / 2})}))
              .has_value());
    auto forced = compiler::checkEquivalence(circuit(1, {G("t", {0})}), circuit(1, {G("t", {0})}),
                                             with(EquivalenceMethod::Clifford));
    CHECK_FALSE(forced.has_value());
}

TEST_CASE("measurement and feed-forward: teleportation against its decomposition and against a "
          "wrong correction") {
