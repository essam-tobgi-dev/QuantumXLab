...
namespace {
compiler::CompileContext contextFor(const std::string& id, compiler::OptimizeLevel level, bool pulse = false) {
    compiler::CompileContext c;
    if (!id.empty()) { c.device = &device(id).device; c.calibration = &device(id).calibration; }
    c.level = level;
    c.pulseLevel = pulse;
    return c;
}
...
TEST_CASE("standard pipeline: the fixed pass order of spec 14 §1.1 for each context") {
    ...
    CHECK(compiler::PassManager::standard(contextFor("", L::O1)).passNames() == Names{"Verify", "Decompose", "Optimize", "Verify", "Equivalence"});
    CHECK(compiler::PassManager::standard(contextFor("", L::O0)).passNames() == Names{"Verify", "Decompose", "Verify", "Equivalence"});
    CHECK(compiler::PassManager::standard(contextFor("sc_fixed_5", L::O1)).passNames() ==
          Names{"Verify", "Decompose", "Optimize", "Layout", "Route", "Decompose", "Optimize", "VirtualZ", "Schedule", "Verify", "Equivalence"});
    CHECK(compiler::PassManager::standard(contextFor("sc_fixed_5", L::O0)).passNames() ==
          Names{"Verify", "Decompose", "Layout", "Route", "Decompose", "Schedule", "Verify", "Equivalence"});
    ...
    struct DropBarriers final : compiler::IPass {
        ...
        Result<compiler::PassMetrics> run(ir::Circuit& c, compiler::PassContext& ctx) override {
            ...
            for (ir::Node& n : compiler::takeNodes(c))
                if (!std::holds_alternative<ir::Barrier>(n)) kept.push_back(std::move(n));
            compiler::setNodes(c, std::move(kept));
            ctx.diagnostics.push_back(lang::Diagnostics::make("QL4060", SourceSpan{}));   // any finding travels with the pass
...
swap q[0], q[3]; swap q[1], q[2];
    ...
    REQUIRE(out.has_value());
    Names names;
    for (const auto& r : out->trace) names.push_back(r.pass);
    CHECK(names == Names{"Build", "Verify", "Decompose", "Optimize", "Layout", "Route", "Decompose", "Optimize", "VirtualZ", "Schedule", "Verify", "Equivalence"});
    ...
    CHECK(tr[6].after.twoQubitCount == tr[5].after.twoQubitCount + 2 * tr[5].after.swapCount);   // each swap → 3 cx
    ...
    for (const auto& r : tr) CHECK(r.wallTime.count() >= 0);
    ...
    for (const ir::Gate* g : gatesOf(out->circuit)) {
        ...
        if (g->width() == 2) CHECK(d.device.nativeDirection(g->targets[0].index, g->targets[1].index));
    }
}

TEST_CASE("GHZ-20 on sc_heavyhex_127: the line embeds without swaps and the Clifford check covers all 127 wires") {
    ...
    REQUIRE(out->equivalence.has_value());
    CHECK(out->equivalence->method == compiler::EquivalenceMethod::Skipped);   // more than ten program qubits: on demand
    ...
    auto stripped = [](const ir::Circuit& c) {
        ...
        for (auto id : c.topologicalOrder())
            if (std::holds_alternative<ir::Gate>(c.node(id))) u.add(c.node(id));
        return u;
    };
...
TEST_CASE("pragmas set the context unless an option overrides them") {
    ...
    auto out = compiler::compile(prog, d.device, d.calibration);
    REQUIRE(out.has_value());
    CHECK(out->initialLayout.v2p == std::vector<std::uint32_t>{0, 1, 2});   // trivial, 0-1 and 1-2 are edges
    for (const auto& r : out->trace) CHECK(r.pass != "Optimize");
    ...
    const auto sweep = parse(program("input float theta = 0.5;\nqubit q; bit c;\nry(theta) q; c = measure q;"));
    compiler::CompileOptions bound;
    bound.inputs = {{"theta", 0.0}};
    auto flat = compiler::compile(sweep, d.device, d.calibration, bound), dflt = compiler::compile(sweep, d.device, d.calibration);
    ...
    const auto far = parse(program("pragma qlab.layout trivial\npragma qlab.routing none\nqubit[3] q;\ncx q[0], q[2];"));
...
TEST_CASE("level 2 keeps the best of four routing seeds; cancellation stops the pipeline") {
    ...
    auto a = compiler::compile(prog, d.device, d.calibration, o1), b = compiler::compile(prog, d.device, d.calibration, o2);
    REQUIRE(a.has_value());
    REQUIRE(b.has_value());
    ...
    stop.request_stop();
    compiler::PassManager pm = compiler::PassManager::standard(compiler::resolveContext(prog, {}, &d.device, &d.calibration));
...
TEST_CASE("compile errors carry the QL4xxx id, the statement span, and print in the spec 14 §11 format") {
    const auto& d = device("sc_fixed_5");
    std::string many = "qubit[10] q;\nh q[0];\nctrl(9) @ x q[0], q[1], q[2], q[3], q[4], q[5], q[6], q[7], q[8], q[9];\n";
    ...
    CHECK(compiler::formatError(tooMany.error()) ==
          "big.qasm:5:1: error[QL4050]: 'x' has 9 controls; the ancilla-free decomposition is limited to 8\n"
          "  help: use ancilla qubits and ccx ladders");
    auto dt = compiler::compileSource(program("qubit q;\nx q;\ndelay[32dt] q;\n"), "dt.qasm", nullptr, nullptr);
    ...
    CHECK(compiler::formatError(dt.error()) ==
          ...
          "  help: select a device ('pragma qlab.device <id>') or write the duration in ns, us or ms");
    CHECK(compiler::compileSource(program("qubit q;\nx q;\ndelay[32dt] q;\n"), "dt.qasm", &d.device, &d.calibration).has_value());
    // A front-end error comes back unchanged.
    auto syntax = compiler::compileSource(program("qubit q;\nfoo q;\n"), "bad.qasm", nullptr, nullptr);
    ...
    o.vf2StateBudget = 3;
    auto ring = compiler::compileSource(program("qubit[12] q;\nfor int i in [0:10] { cz q[i], q[i+1]; }\ncz q[11], q[0];\n"), "ring.qasm",
                                        &device("sc_heavyhex_27").device, &device("sc_heavyhex_27").calibration, o);
    REQUIRE(ring.has_value());
    REQUIRE(ring->diagnostics.size() == 1);
    CHECK(compiler::formatDiagnostic(ring->diagnostics[0]) == "info[QL4060]: VF2 layout search exceeded its budget; falling back to the dense layout");
    CHECK(ring->programHash == compiler::programHash(program("qubit[12] q;\nfor int i in [0:10] { cz q[i], q[i+1]; }\ncz q[11], q[0];\n")));
}
...
TEST_CASE("performance: parse + compile of a 1000-gate program (spec 24 §6: 50 ms in Release)") {
    ...
    for (int k = 0; k < 1000; ++k) {
        ...
        if (b >= a) ++b;
        if (k % 3 == 0) body += "cx q[" + std::to_string(a) + "], q[" + std::to_string(b) + "];\n";
        else if (k % 3 == 1) body += "rz(" + std::to_string(rng.uniform(-3.0, 3.0)) + ") q[" + std::to_string(a) + "];\n";
        else body += std::string(one[rng.uniformInt(5)]) + " q[" + std::to_string(a) + "];\n";
    }
    body += "c = measure q;\n";
    const std::string text = program(body);
    const auto& d = device("sc_heavyhex_27");
    compiler::CompileOptions o;
    o.verifyEquivalence = false;   // the budget is for compilation; the checker is a verification tool
    core::Timer timer;
    auto out = compiler::compileSource(text, "k.qasm", &d.device, &d.calibration, o);
    const double ms = timer.ms();
    INFO((out ? std::string() : compiler::formatError(out.error())));
    REQUIRE(out.has_value());
    std::cout << "[perf] parse + compile, 1000 gates on sc_heavyhex_27: " << ms << " ms (budget 50 ms in Release); passes:";
    for (const auto& r : out->trace) std::cout << ' ' << r.pass << '=' << static_cast<double>(r.wallTime.count()) / 1000.0;
    std::cout << " ms\n";
#ifdef NDEBUG
    CHECK(ms < 50.0);
#else
