...
inline ir::Wire W(std::uint32_t i) {
    return ir::Wire{i};
}

inline ir::Gate G(std::string_view name, std::vector<std::uint32_t> wires,
                  std::vector<double> params = {}) {
    std::vector<ir::Wire> ws;
    for (auto w : wires)
        ws.push_back(ir::Wire{w});
...
inline ir::Gate controlled(ir::Gate g, std::vector<std::uint32_t> controls,
                           std::vector<std::uint8_t> negative = {}) {
    for (auto c : controls)
        g.controls.push_back(ir::Wire{c});
    g.negControl = std::move(negative);
    g.matrixCache.reset();
    return g;
}

inline ir::Circuit circuit(std::uint32_t nQubits, std::vector<ir::Gate> gates,
                           bool physical = false) {
    ir::Circuit c;
...
inline std::vector<const ir::Gate*> gatesOf(const ir::Circuit& c) {
    ...
    for (auto id : c.topologicalOrder())
        if (const auto* g = std::get_if<ir::Gate>(&c.node(id)))
            v.push_back(g);
    return v;
}
...
inline const hw::LoadedDevice& device(const std::string& id) {
    ...
    if (it == cache.end()) {
        ...
        if (!d)
            FAIL("loading " << id << ": " << d.error().format());
...
inline double angle(core::Random& rng) {
