...
std::int64_t onGrid(double seconds) {
    return pulse::quantise(seconds, Picoseconds{222}, 16).get();
}

compiler::ScheduleInfo scheduled(ir::Circuit& c, SchedulePolicy policy,
                                 const std::string& id = "sc_fixed_5") {
...
std::vector<std::vector<std::size_t>> predecessors(const ir::Circuit& c) {
    ...
    for (std::size_t i = 0; i < order.size(); ++i) {
        ...
        for (ir::Wire w : ir::nodeWiresIn(n, c.qubitCount())) {
            if (lastWire[w.index] >= 0)
                preds[i].push_back(static_cast<std::size_t>(lastWire[w.index]));
        ...
        for (auto b : ir::nodeReads(n))
            bits.push_back(b);
        for (auto b : bits) {
            if (lastBit[b.index] >= 0 && lastBit[b.index] != static_cast<long>(i))
                preds[i].push_back(static_cast<std::size_t>(lastBit[b.index]));
...
void requireWellPlaced(const ir::Circuit& c, const compiler::ScheduleInfo& s) {
    ...
    for (std::size_t i = 0; i < preds.size(); ++i) {
        ...
        if (s.policy == SchedulePolicy::Asap)
            REQUIRE(s.nodes[i].start.get() == ready);
    }
    if (s.policy == SchedulePolicy::Alap)
        for (std::size_t i = 0; i < preds.size(); ++i)
            REQUIRE(s.nodes[i].end().get() == earliestSuccessor[i]);
}
} // namespace

TEST_CASE(
    "durations are the calibrated values on the device grid; ASAP and ALAP start times by hand") {
    const std::string src = program("pragma qlab.layout physical\nbit[2] c;\nrz(0.3) $0; sx $0; cx "
                                    "$0, $1; x $1; c[0] = measure $0; c[1] = measure $1;");
    const auto& cal = device("sc_fixed_5").calibration;
    const std::int64_t sx = onGrid(cal.qubits[0].duration1q.value.si()),
                       cx = onGrid(cal.edge(0, 1)->duration.value.si()),
    ...
    const std::vector<std::int64_t> lengths{0, sx, cx, sx, ro, ro},
        starts{0, 0, sx, sx + cx, sx + cx, sx + cx + sx};
    ...
    CHECK(l.idle.at(0) ==
          std::vector<compiler::IdleInterval>{{Picoseconds{sx + cx}, Picoseconds{sx + cx + sx}}});
    CHECK(l.totalIdle().get() == sx);
    CHECK(alap.meta()["schedule"]["idle"]["0"][0][1] == sx + cx + sx);
    for (std::size_t i = 0; i < 6; ++i)
        CHECK(a.nodes[i].start <= l.nodes[i].start);
}

TEST_CASE(
    "random circuits: no misplaced node under either policy, equal critical paths, slack ≥ 0") {
    core::Random rng(17);
    ...
    for (int trial = 0; trial < 20; ++trial) {
        ...
        for (int k = 0; k < 60; ++k) {
            ...
            case 0:
                ...
                break;
            case 1:
                gs.push_back(G("rz", {q}, {angle(rng)}));
                break;
            case 2:
                gs.push_back(G("x", {q}));
                break;
            default:
                gs.push_back(G("cx", {e.first, e.second}));
                break;
            }
        }
...
box[200ns] { x $4; x $4; }
    ...
    CHECK(s.nodes[6].duration.get() ==
          onGrid(dev.timing.readout.si() + dev.control.feedbackLatency.si() +
                 cal.qubits[3].duration1q.value.si()));
...
TEST_CASE("QL4090 over the maximum program duration, QL4080 without calibration, QL4010 without a "
    ...
    CHECK(
        compiler::requireResolvedDurations(build(program("qubit q; delay[16ns] q;"))).has_value());
}
...
TEST_CASE("performance: scheduling 10 000 operations (spec 24 §6: 30 ms in Release)") {
    ...
    for (int k = 0; k < 10000; ++k) {
        ...
        if (k % 3 == 0)
            ...
            gs.push_back(G(k % 3 == 1 ? "sx" : "rz",
                           {static_cast<std::uint32_t>(rng.uniformInt(5))},
                           k % 3 == 1 ? std::vector<double>{} : std::vector<double>{0.1}));
    }
