...
std::vector<fs::path> examplePrograms() {
    ...
    for (const auto& e :
         fs::recursive_directory_iterator(core::assetDir() / "Programs" / "Examples"))
...
void requireNativeOnDevice(const compiler::CompiledProgram& out, const hw::LoadedDevice& d,
    ...
    while (!levels.empty()) {
        ...
        for (auto id : c->topologicalOrder()) {
            ...
            for (ir::Wire w : g->wires())
                REQUIRE_FALSE(d.device.isCoupler(w.index));
            if (g->width() == 2 && !g->opaque)
                REQUIRE(d.device.nativeDirection(g->targets[0].index, g->targets[1].index));
...
TEST_CASE(
    "every example program compiles for every device it fits and stays equivalent to its source") {
    ...
    for (const auto& file : files) {
        ...
        const bool pulseProgram =
            file.parent_path().filename() == "Pulse"; // physical qubits + defcals of sc_fixed_5
        std::vector<std::string> devices =
            pulseProgram ? std::vector<std::string>{"sc_fixed_5"}
                         : std::vector<std::string>{"", "sc_fixed_5", "sc_heavyhex_27",
                                                    "sc_tunable_grid_54", "ion_chain_11"};
        for (const std::string& id : devices) {
            const std::uint32_t capacity =
                id.empty() ? 1000 : static_cast<std::uint32_t>(device(id).device.dataQubitCount());
            if (prog.qubitCount > capacity)
                continue;
            INFO(file.filename().string() << " on " << (id.empty() ? "{U, cx}" : id));
            auto out = id.empty()
                           ? compiler::compile(prog)
                           : compiler::compile(prog, device(id).device, device(id).calibration);
            ...
            if (prog.qubitCount > 10 || pulseProgram)
                continue;
            // On demand, without the pipeline's work limit; fewer random states keep Debug runs
            // short.
            ...
            INFO(compiler::equivalenceMethodName(report->method)
                 << " on " << report->wires << " wires: " << report->detail);
...
TEST_CASE("every pipeline setting keeps the examples equivalent; equal seeds give equal output "
          "(spec 25 §4)") {
    ...
    struct Setting {
        OptimizeLevel level;
        LayoutPolicy layout;
        compiler::SchedulePolicy schedule;
        bool kak;
    ...
    for (const auto& file : examplePrograms()) {
        ...
        if (group != "Basics" && group != "Fourier" && group != "Protocols" &&
            group != "Variational")
            continue;
        ...
        for (const Setting& s : settings) {
            ...
            INFO(file.filename().string()
                 << " level " << static_cast<int>(s.level) << " layout "
                 << compiler::layoutPolicyName(s.layout)
                 << (out ? std::string() : " " + compiler::formatError(out.error())));
            REQUIRE(out.has_value());
            requireNativeOnDevice(*out, d, targetOf("sc_heavyhex_27", s.twoQubitBasis));
            if (!s.twoQubitBasis.empty())
                CHECK(countGates(out->circuit, "cx") == 0); // the chosen entangler only
            compiler::EquivalenceOptions eo;
            eo.states = 3;
            auto report = compiler::checkEquivalence(out->source, out->circuit, eo);
            REQUIRE(report.has_value());
            ...
            REQUIRE(report->equivalent);
            ++checked;
            // Determinism (DEVELOPMENT.md): the same seed gives the same circuit, layout and
            // timing.
...
TEST_CASE("export: compiled OpenQASM 3 re-imports and compiles to an equivalent circuit with the "
          "same timing") {
    const auto& d = device("sc_heavyhex_27");
...
c = measure q;
    ...
    CHECK(text->find(" for sc_heavyhex_27 (calibration " + d.calibration.timestamp + ")") !=
          std::string::npos);
    ...
    CHECK(text->find("q[") ==
          text->find("q[0]->$")); // operands are physical: `q[` appears in the mapping only
    ...
    // Round trip: the re-compiled circuit implements the ORIGINAL source under the original
    // mapping.
