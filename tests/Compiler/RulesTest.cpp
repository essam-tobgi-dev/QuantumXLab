...
TEST_CASE("every rule of the table reproduces its source gate exactly at 50 random parameter sets") {
    ...
    for (const compiler::Rule& rule : compiler::decompositionRules()) {
        ...
        for (int trial = 0; trial < 50; ++trial) {
            ...
            for (int k = 0; k < def->nParams; ++k) params.push_back(angle(rng));
            std::vector<std::uint32_t> wires;
            for (std::uint32_t q = 0; q < n; ++q) wires.push_back(q);
            ...
            const num::Matrix got = num::scale(unitary(circuit(n, expansion->gates)), std::polar(1.0, expansion->phase));
    ...
    for (const ir::GateDef& d : ir::gates::all()) {
        if (d.nQubits < 2 || d.name == "cx") continue;
        INFO("gate " << d.name);
    ...
    CHECK(compiler::findRule("cx", "cx") == nullptr);   // cx is the generic entangler: nothing to rewrite
}
...
TEST_CASE("rule table serialises as the decompositions.json document") {
    ...
    for (const auto& r : rules) {
        if (r["source"] != "cp" || r["target_set"] != "1q+cx") continue;
        sawCp = true;
...
TEST_CASE("euler angles: U(θ,φ,λ) and the phase are recovered from the matrix") {
    ...
    for (int trial = 0; trial < 200; ++trial) {
        const double theta = rng.uniform(0.01, kPi - 0.01), phi = angle(rng), lambda = angle(rng), gamma = angle(rng);
        ...
        const num::Matrix back = num::scale(ir::gates::u3(e.theta, e.phi, e.lambda), std::polar(1.0, e.phase));
...
TEST_CASE("single-qubit synthesis is exact in every basis for random unitaries and all library gates") {
    core::Random rng(11);
    std::vector<num::Matrix> cases;
    for (int trial = 0; trial < 300; ++trial)
        cases.push_back(num::scale(ir::gates::u3(angle(rng), angle(rng), angle(rng)), std::polar(1.0, angle(rng))));
    for (const ir::GateDef& d : ir::gates::all()) {
        if (d.nQubits != 1 || d.name == "unitary") continue;
        std::vector<double> params;
    ...
    for (const auto& m : cases)
        for (Basis1q basis : {Basis1q::U, Basis1q::ZSX, Basis1q::ZYZ}) {
            ...
            for (const auto& g : seq.gates) {
                if (basis == Basis1q::U) CHECK(g.name == "U");
                if (basis == Basis1q::ZSX) CHECK((g.name == "rz" || g.name == "sx" || g.name == "x"));
                if (basis == Basis1q::ZYZ) CHECK((g.name == "rz" || g.name == "ry" || g.name == "rx"));
                if (g.name == "rz") CHECK(std::abs(g.params[0]) <= kPi + 1e-15);   // angles are reduced to (−π, π]
            }
