...
namespace {
num::Matrix gateMatrix(std::string_view name, std::vector<double> p = {}) { return *ir::gates::matrix(name, p); }
...
num::Matrix randomUnitary(core::Random& rng) {
    ...
    for (int layer = 0; layer < 4; ++layer) {
        ...
        gs.push_back(G(layer % 2 ? "cx" : "rzz", {0, 1}, layer % 2 ? std::vector<double>{} : std::vector<double>{angle(rng)}));
    }
...
TEST_CASE("canonical gate: exp(i(aXX + bYY + cZZ)) = rxx(−2a)·ryy(−2b)·rzz(−2c)") {
    ...
    for (int trial = 0; trial < 20; ++trial) {
        ...
        const num::Matrix want = num::matmul(num::matmul(gateMatrix("rxx", {-2 * a}), gateMatrix("ryy", {-2 * b})), gateMatrix("rzz", {-2 * c}));
...
TEST_CASE("KAK reconstructs random two-qubit unitaries exactly, phase included") {
    ...
    for (int trial = 0; trial < 200; ++trial) {
        ...
        for (double v : {k->a, k->b, k->c}) { CHECK(v > -kPi / 4 - 1e-12); CHECK(v <= kPi / 4 + 1e-12); }
        for (const num::Matrix* m : {&k->before0, &k->before1, &k->after0, &k->after1}) {
            REQUIRE(num::isUnitary(m->view(), 1e-9));
            CHECK(std::abs((*m)(0, 0) * (*m)(1, 1) - (*m)(0, 1) * (*m)(1, 0) - 1.0) < 1e-9);   // SU(2)
        }
    }
...
TEST_CASE("canonical coordinates and minimal cx counts of the library gates (T02 §6)") {
    struct Row { std::string name; std::vector<double> params; std::array<double, 3> coords; std::uint32_t cx; };
    const double q = kPi / 4;
    const std::vector<Row> table = {
        {"cx", {}, {q, 0, 0}, 1},      {"cz", {}, {q, 0, 0}, 1},   {"ecr", {}, {q, 0, 0}, 1},       {"ch", {}, {q, 0, 0}, 1},
        {"iswap", {}, {q, q, 0}, 2},   {"siswap", {}, {q / 2, q / 2, 0}, 2},                          {"swap", {}, {q, q, q}, 3},
        {"rzz", {0.6}, {0.3, 0, 0}, 2}, {"cp", {1.0}, {0.25, 0, 0}, 2}, {"rxx", {kPi / 2}, {q, 0, 0}, 1}, {"fsim", {0.4, 0.0}, {0.2, 0.2, 0}, 2},
    };
    for (const Row& r : table) {
        ...
        for (std::size_t i = 0; i < 3; ++i) CHECK(std::abs(got[i] - r.coords[i]) < 1e-9);
...
TEST_CASE("two-qubit synthesis over {U, cx} uses the minimal cx count and matches the unitary") {
    ...
    for (int trial = 0; trial < 60; ++trial) cases.emplace_back(randomUnitary(rng), 3u);
    for (const char* name : {"cx", "cz", "ecr", "cy"}) cases.emplace_back(gateMatrix(name), 1u);
    ...
    for (const auto& [u, cx] : cases) {
        ...
        for (const ir::Gate* g : gatesOf(c)) CHECK((g->name == "U" || g->name == "cx"));
    ...
    for (const auto& target : {compiler::Target::universal(), targetOf("sc_fixed_5", "ecr"), targetOf("ion_chain_11")}) {
        ir::Circuit c = circuit(2, {*custom});
        const num::Matrix want = unitary(c);
        REQUIRE(compiler::decompose(c, target).has_value());
        for (const ir::Gate* g : gatesOf(c)) REQUIRE(target.accepts(*g));
...
TEST_CASE("block resynthesis lowers the two-qubit count and keeps the unitary") {
    ...
    for (bool generic : {true, false}) {
        ...
        for (int k = 0; k < 5; ++k) {
            ...
            gs.push_back(generic ? G("U", {0}, {angle(rng), angle(rng), angle(rng)}) : G("rz", {0}, {angle(rng)}));
            gs.push_back(generic ? G("U", {1}, {angle(rng), angle(rng), angle(rng)}) : G("sx", {1}));
        }
        const std::uint32_t minimal = compiler::kakDecompose(unitary(circuit(2, gs)).view())->cxCount();
        ...
        for (const ir::Gate* g : gatesOf(c)) CHECK(target.accepts(*g));
    ...
    for (int k = 0; k < 12; ++k) body += std::string(k % 3 == 0 ? "cz" : k % 3 == 1 ? "cx" : "swap") + " q[0], q[1];\nry(0." + std::to_string(k + 1) + ") q[1];\nh q[0];\n";
    body += "cx q[1], q[2];\n";
    const auto prog = parse(program(body));
    const auto& d = device("sc_heavyhex_27");
    compiler::CompileOptions plain, kak;
    kak.kak = true;
    auto a = compiler::compile(prog, d.device, d.calibration, plain), b = compiler::compile(prog, d.device, d.calibration, kak);
    REQUIRE(a.has_value());
    REQUIRE(b.has_value());
