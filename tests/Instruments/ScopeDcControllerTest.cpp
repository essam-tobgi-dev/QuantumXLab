...
// edge trigger), DC source (Φ = M I, f01(Φ)), controller (run timeline, trigger rate, memory fault).
...
struct ScopeBench {
    ...
    explicit ScopeBench(std::shared_ptr<const pulse::Schedule> schedule, std::vector<pulse::ChannelId> channels = {}) {
        RunView run;
        ...
        b.inputs = hub;
        b.routing = &graph;
        for (IInstrument* i : std::initializer_list<IInstrument*>{&lo, &mixer, &scope}) { i->bind(b); powerOn(*i); }
...
TEST_CASE("oscilloscope: the envelope view reproduces the schedule samples to 1e-12") {
    ...
    REQUIRE(bench.scope.set("ch[1].source", std::string("awg[0].ch[1]"))); // u[0,1]: the cross-resonance tone
    ...
    for (auto [channel, scheduleChannel, imag] : {std::tuple{"ch[0]", pulse::ChannelId::drive(0), false},
    ...
    REQUIRE(qPeak > 1e-3);
    for (double y : q1->y) REQUIRE(y == 0.0);
...
TEST_CASE("oscilloscope: front-end model, RF nodes and the edge trigger") {
    ...
    schedule->insert(pulse::Play{pulse::ChannelId::drive(0), pulse::Waveform::constant(160e-9, 0.4), Picoseconds{96000}}, Picoseconds{96000});
    ...
    REQUIRE(t->x.front() == Approx(96e-9 - 2e-9).margin(1e-15));         // shown one division into the record
    // 8-bit vertical resolution over 8 divisions.
    const double lsb = 8.0 * 0.05 / 256.0;
    REQUIRE(t->marker("lsb")->value == lsb);
    for (double y : t->y) REQUIRE(std::abs(y / lsb - std::round(y / lsb)) < 1e-9);
    ...
    for (std::size_t j = 1; j < t->size(); ++j) {
        if (t->y[j - 1] < 0.02 && t->y[j] >= 0.02) t10 = t->x[j];
        if (t->y[j - 1] < 0.18 && t->y[j] >= 0.18) t90 = t->x[j];
    }
    ...
    for (std::size_t j = 1; j < fast->size(); ++j) {
        if (fast->y[j - 1] < 0.02 && fast->y[j] >= 0.02) f10 = fast->x[j];
        if (fast->y[j - 1] < 0.18 && fast->y[j] >= 0.18) f90 = fast->x[j];
    }
    REQUIRE(f90 - f10 < t90 - t10);

    // The 5 GHz mixer output is far outside a 1 GHz scope: e^{−(ln 2/2)(f/B)²} = 1.7e−4 of it survives.
    ...
    REQUIRE(rf);
    double rfPeak = 0.0;
    for (double y : rf->y) rfPeak = std::max(rfPeak, std::abs(y));
...
TEST_CASE("DC source: flux bias and f01(flux) from the charge-basis spectrum") {
    ...
    REQUIRE(*dc.qubitFrequencyHz(0, -0.3 * perQuantum) == Approx(*dc.qubitFrequencyHz(0, 0.3 * perQuantum)).epsilon(1e-10));
    ...
    REQUIRE(*dc.theoryFrequencyHz(0, 0.25 * perQuantum) == Approx(std::sqrt(8.0 * ejQuarter.v * solved->EC.v) - solved->EC.v).epsilon(1e-12));
    ...
    REQUIRE(arc);
    REQUIRE(arc->cls == FidelityClass::Model); // Numerical eigenvalues over a Model M: the weaker class
    REQUIRE(arc->size() < 49);
    REQUIRE(arc->size() > 30);
    REQUIRE(arc->aux.at("theory").size() == arc->size());
    const auto top = static_cast<std::size_t>(std::max_element(arc->y.begin(), arc->y.end()) - arc->y.begin());
    REQUIRE(std::abs(arc->x[top] - std::round(arc->x[top])) < 0.03);
    REQUIRE(arc->y[top] == Approx(f0).epsilon(2e-3));
    for (double phi : arc->x) REQUIRE(std::abs(std::abs(phi - std::round(phi)) - 0.5) > 0.01);
...
TEST_CASE("controller: run timeline, executing instruction, trigger rate, sequencer memory") {
    ...
    for (std::size_t i = 1; i < entries.size(); ++i) REQUIRE(entries[i].t0S >= entries[i - 1].t0S);
    ...
    const auto firstPlay = std::find_if(entries.begin(), entries.end(), [](const TimelineEntry& e) { return e.kind == 0; });
    ...
    for (int k = 0; k < 100; ++k) big->append(pulse::FrameOp{pulse::ChannelId::drive(0), Picoseconds{0}, pulse::FrameOp::Op::ShiftPhase, 0.1});
    run.schedule = big;
    hub->publish(run);
