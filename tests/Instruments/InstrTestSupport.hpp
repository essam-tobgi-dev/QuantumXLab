...
inline instr::Environment deviceEnvironment(const std::string& id = "sc_fixed_5") {
    ...
    for (cryo::Stage s : cryo::kStages)
        env.thermal.T_K[static_cast<std::size_t>(cryo::stageIndex(s))] = cryo::nominalTemperature(s);
...
inline instr::Environment withPreamp(instr::Environment env, const std::string& preamp) {
    ...
    for (auto& line : wiring->lines) {
        if (line.kind != cryo::LineKind::ReadoutOut) continue;
        std::erase_if(line.elements, [](const cryo::Element& e) { return e.kind == cryo::ElementKind::Preamp; });
        line.preamp = preamp;
        if (preamp == "none") continue;
        cryo::Element p; // same insertion as cryo::parseWiring: after the first isolator
        p.kind = cryo::ElementKind::Preamp; p.stage = cryo::Stage::MXC; p.variant = preamp; p.id = preamp;
        p.gain_dB = preamp == "twpa" ? 20.0 : 18.0;
        p.noiseTemperature_K = preamp == "twpa" ? 0.35 : 0.25;
        p.dissipation_W = 0.5e-6;
        auto pos = std::find_if(line.elements.begin(), line.elements.end(),
                                [](const cryo::Element& e) { return e.kind == cryo::ElementKind::Isolator; });
...
inline std::shared_ptr<const pulse::Schedule> toneSchedule(double durationS, double amplitude = 1.0,
                                                           std::uint32_t qubit = 0, double phase = 0.0) {
    auto s = std::make_shared<pulse::Schedule>(Picoseconds{1000});
    s->insert(pulse::Play{pulse::ChannelId::drive(qubit), pulse::Waveform::constant(durationS, amplitude, phase), Picoseconds{0}},
              Picoseconds{0});
    return s;
}
...
inline void powerOn(instr::IInstrument& i) { REQUIRE(i.execute(instr::Command::of(instr::Command::Kind::PowerOn))); }
