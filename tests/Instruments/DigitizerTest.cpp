...
ReadoutParams textbook(double photons, double windowS, double acquireDelayS) {
    ...
    p.photons = photons;
    p.efficiency = 0.5;
    p.tSysK = units::consts::h.v * p.resonatorHz / (2.0 * units::consts::k_B.v) / p.efficiency; // η = T_q / T_sys
    p.gainDb = 86.0;
...
std::unique_ptr<Digitizer> bench(const std::optional<ReadoutParams>& fixed, std::shared_ptr<InputHub> hub = nullptr) {
    auto dig = std::make_unique<Digitizer>();
    Bindings b;
    b.inputs = hub ? hub : instrtest::makeHub(Environment{});
    dig->bind(b);
    powerOn(*dig);
    if (fixed) dig->setReadoutOverride(0, fixed);
    return dig;
}
} // namespace
...
TEST_CASE("readout chain: pointer states, SNR formula and the T05 worked numbers") {
    ...
    REQUIRE(std::arg(a0 / a1) == Approx(-2.0 * std::atan(-p.chiRadS / (p.kappaRadS / 2.0))).epsilon(1e-9));
    ...
    for (std::size_t k : {std::size_t{50}, std::size_t{200}, std::size_t{499}}) {
        ...
        const Complex want0 = a0 * (1.0 - std::exp(-p.kappaRadS * t / 2.0) * std::polar(1.0, p.chiRadS * t));
        const Complex want1 = a1 * (1.0 - std::exp(-p.kappaRadS * t / 2.0) * std::polar(1.0, -p.chiRadS * t));
        REQUIRE(std::abs(r.alpha[0][k] - want0) < 1e-9);
        REQUIRE(std::abs(r.alpha[1][k] - want1) < 1e-9);
    }
    // Boxcar on the steady state reproduces the formula; the matched filter is optimal during ring-up.
    ReadoutParams steady = textbook(5.0, 500e-9, 2e-6);
    const CavityResponse rs = cavityResponse(steady, 1e9);
    REQUIRE(weightedSnr(steady, rs, boxcarWeights(rs.samples())) == Approx(theorySnr(steady)).epsilon(1e-3));
    const double box = weightedSnr(p, r, boxcarWeights(r.samples())), matched = weightedSnr(p, r, matchedWeights(r));
    REQUIRE(matched > box);
    REQUIRE(box < theorySnr(p));
    REQUIRE(box / matched == Approx(0.85).margin(0.12)); // boxcar loses SNR while the cavity rings up (T07 §6)
}
...
TEST_CASE("digitizer: IQ cloud separation over sigma follows the T07 SNR formula") {
    ...
    REQUIRE(rep->snrTheory == Approx(std::abs(steadyStateField(rep->params, 1) - steadyStateField(rep->params, 0)) *
                                     std::sqrt(2.0 * 0.5 * rep->params.kappaRadS * 1e-6)).epsilon(1e-12));
    ...
    REQUIRE(1.0 - rep->assignment.fidelity() == Approx(overlap).margin(3.0 * std::sqrt(overlap / 10000.0)));
    REQUIRE(rep->assignment.matrix[0][0] + rep->assignment.matrix[0][1] == Approx(1.0).margin(1e-12)); // row-stochastic
    REQUIRE(rep->assignment.sigma[0][1] == Approx(std::sqrt(overlap * (1 - overlap) / 5000.0)).epsilon(0.2));
...
TEST_CASE("digitizer: the ADC record is quantised and carries the chain noise k_B T_sys G") {
    ...
    for (double v : rec->y) REQUIRE(std::abs(v / lsb - std::round(v / lsb)) < 1e-9);
    ...
    REQUIRE(sigma == Approx(std::sqrt(units::consts::k_B.v * p.tSysK * std::pow(10.0, 8.6) * 50.0 * 0.5e9)).epsilon(1e-12));
    REQUIRE(sigma > 8.0 * lsb); // T07 §7: the chain noise spans ≥ 8 LSB, so quantisation is negligible
    double ms = 0.0;
    for (double v : rec->y) ms += v * v / 1000.0;
    REQUIRE(std::sqrt(ms) == Approx(std::sqrt(amplitude * amplitude / 2.0 + sigma * sigma + lsb * lsb / 12.0)).epsilon(0.08));
...
TEST_CASE("digitizer: trained assignment matrix reproduces the calibration on the reference chain; a TWPA raises F_ro") {
    const Environment shipped = instrtest::deviceEnvironment("sc_fixed_5");
    const Environment reference = instrtest::withPreamp(shipped, "none"); // readout_out_std, no preamp (spec 12 §15)
    ...
    for (int i = 0; i < 2; ++i)
        for (int j = 0; j < 2; ++j) {
            ...
            INFO("M[" << i << "][" << j << "] = " << rep->assignment.matrix[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)] << ", calibration " << want << ", sigma " << sigma);
            REQUIRE(std::abs(rep->assignment.matrix[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)] - want) < 3.0 * sigma);
        }
    ...
    REQUIRE(quiet->classify(0, repq->discriminator.means[1][0], repq->discriminator.means[1][1]) == 1);
    REQUIRE(quiet->classify(0, repq->discriminator.means[0][0], repq->discriminator.means[0][1]) == 0);
    ...
    for (std::size_t b = 0; b < hist->size(); ++b) { total += hist->y[b]; if (hist->x[b] < 0.0) left += hist->y[b]; }
    REQUIRE(total == Approx(2000.0).margin(3.0));
    REQUIRE(left / total == Approx(0.5).margin(0.05)); // two equal blobs either side of the threshold
}
...
TEST_CASE("digitizer: three-state discrimination, live run source, and the window/memory fault") {
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        double row = 0.0;
        for (std::size_t j = 0; j < 3; ++j) row += rep->assignment.matrix[i][j];
