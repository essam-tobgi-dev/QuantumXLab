...
TEST_CASE("def subroutines inline: qubits by reference, classical arguments by value, returned values") {
...
c = bell(q[0], q[1]);
    ...
    CHECK((copy.dst.reg.first == 0 && copy.expr.op == ir::ClassOp::BitRef && copy.expr.bit.index == 1));
    REQUIRE(c.bitRegisters().size() == 2);
    CHECK(c.bitRegisters()[1].name == "__meas");
    // Inlined nodes keep the call site: `int next = layer(q, 3);` is line 15, `c = bell(…)` line 17.
...
if (k == 5) { x q[1]; }
    ...
    REQUIRE(c.bitRegisters().size() == 2);
    CHECK((c.bitRegisters()[0].name == "k" && c.bitRegisters()[0].kind == ir::RegKind::Int && c.bitRegisters()[0].size == 32));
    const auto ns = nodes(c);
    REQUIRE(ns.size() == 4);
    CHECK(std::get<ir::ClassicalOp>(*ns[0]).expr.value == 0);
    const auto& set = std::get<ir::Branch>(*ns[2]);
    const auto& inner = std::get<ir::ClassicalOp>((*set.thenBody).node((*set.thenBody).topologicalOrder()[0]));
    ...
    auto rejected = tryBuild(program("qubit q;\nbit b;\nfloat a = 0.5;\nb = measure q;\nif (b == 1) { a = 1.5; }\nrz(a) q;\n"));
...
TEST_CASE("classical expressions read registers little-endian and int registers as two's complement") {
    ...
    const auto e = ClassicalExpr::binary(ClassOp::LogicAnd,
                                         ClassicalExpr::binary(ClassOp::Eq, ClassicalExpr::regRef(ir::CregRef{0, 2}), ClassicalExpr::constant(1)),
    ...
    ir::applyClassicalOp(ir::ClassicalOp{ClassicalExpr::constant(6), ir::ClassicalTarget{ir::CregRef{1, 2}, std::nullopt}, {}}, mem);
