...
TEST_CASE("Result propagates errors with QXL_TRY") {
    auto f = [](int x) -> Result<int> { if (x < 0) return fail(ErrorCode::InvalidArgument, "neg"); return x * 2; };
    auto g = [&](int x) -> Result<int> { QXL_TRY_ASSIGN(auto v, f(x)); return v + 1; };
    REQUIRE(g(2).value() == 5);
    REQUIRE_FALSE(g(-1).has_value());
    REQUIRE(g(-1).error().code == ErrorCode::InvalidArgument);
    Error e(ErrorCode::Lang_ + 7, "bad token"); e.withSpan({3, 4, 3, 9, "a.qasm"}).withId("QL2007");
...
TEST_CASE("Random is deterministic and streams differ") {
    ...
    Random c(7); double sum = 0; const int N = 200000;
    for (int i = 0; i < N; ++i) sum += c.uniform();
    REQUIRE(sum / N == Catch::Approx(0.5).margin(0.005));
    double m = 0, v = 0; for (int i = 0; i < N; ++i) { double x = c.normal(); m += x; v += x * x; }
    REQUIRE(m / N == Catch::Approx(0.0).margin(0.01));
    REQUIRE(v / N == Catch::Approx(1.0).margin(0.02));
    Random j1(5), j2(5); j2.jump(); REQUIRE(j1.next() != j2.next());
}
TEST_CASE("EventBus immediate, queued, and RAII unsubscribe") {
    ...
    { auto sub = bus.subscribe<Ev>([&](const Ev& e) { hits += e.v; });
      bus.publish(Ev{1}); REQUIRE(hits == 1);
      bus.post(Ev{10}); REQUIRE(hits == 1); bus.drain(); REQUIRE(hits == 11); }
    bus.publish(Ev{100}); REQUIRE(hits == 11);
}
TEST_CASE("JobSystem parallelFor covers the range exactly once") {
    JobSystem js(4);
    std::vector<int> v(100000, 0);
    js.parallelFor(v.size(), 1000, [&](std::size_t b, std::size_t e) { for (auto i = b; i < e; ++i) v[i] += 1; });
