...
TEST_CASE("Bell on the state vector gives only 00 and 11, each 1/2 within 5 sigma") {
    ...
    options.seed = 12345;
    const RunResult r =
        lab("sc_heavyhex_27").run(readAsset("Programs/Examples/Basics/bell.qasm"), options);
    ...
    for (const Marginal& m : r.marginals)
        REQUIRE(std::abs(m.p1 - 0.5) < 5.0 * s);
}
...
TEST_CASE("GHZ-5 produces only 00000 and 11111") {
    ...
    options.seed = 7;
    const RunResult r =
        lab("sc_heavyhex_27").run(readAsset("Programs/Examples/Basics/ghz_5.qasm"), options);
    REQUIRE(r.counts.distinct() == 2);
...
TEST_CASE("teleportation runs per shot and reproduces the input state statistics") {
    ...
    const RunResult r =
        lab("sc_heavyhex_27")
            .run(readAsset("Programs/Examples/Protocols/teleportation.qasm"), options);
    ...
    REQUIRE(m != nullptr);
    for (const ShotRecord& s : r.memory)
        ++arms[s.bits[m->first] + 2u * s.bits[m->first + 1]];
    for (std::uint64_t n : arms)
        REQUIRE(std::abs(static_cast<double>(n) / 8192.0 - 0.25) < 5.0 * sigma(0.25, 8192));
}
...
TEST_CASE("the same seed reproduces bit-identical memory, a different seed does not") {
    ...
    for (std::size_t s = 0; s < a.memory.size(); ++s)
        REQUIRE(a.memory[s].bits == b.memory[s].bits);

    options.seed = 100;
    const RunResult c = sc.run(text, options);
    bool differs = false;
    for (std::size_t s = 0; s < a.memory.size() && !differs; ++s)
        differs = a.memory[s].bits != c.memory[s].bits;
    REQUIRE(differs);
    // Terminal-measurement runs are deterministic in the same way (model (a) materialises in key
    // order).
...
TEST_CASE("a runtime loop that hits its iteration bound truncates the shot with QL5020") {
    ...
    const RunResult r = lab("sc_fixed_5")
                            .run(source("qubit[1] q;\nbit c;\nbit d;\nx q[0];\nc = measure "
                                        "q[0];\nwhile (c == 1) { x q[0]; d = c; }\n"),
                                 options);
    ...
    for (const lang::Diagnostic& d : r.diagnostics)
        warned = warned || d.id() == "QL5020";
    REQUIRE(warned);
}

TEST_CASE(
    "snapshots are published at the configured cadence with the schedule time of the boundary") {
    RunOptions options;
    options.noise = NoiseSource::Ideal;
    options.shots = 16;
    options.seed = 2;
    ...
    const RunResult r =
        lab("sc_fixed_5").run(readAsset("Programs/Examples/Basics/bell.qasm"), options);
