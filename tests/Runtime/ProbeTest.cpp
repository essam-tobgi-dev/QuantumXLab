...
TEST_CASE("bloch and entanglement probes follow the state through a Bell circuit") {
    ...
    const RunResult r = l.run(source("pragma qlab.probe bloch q[0] q[1]\n"
    ...
    for (double v : last.values) REQUIRE(std::abs(v) < 1e-9);
...
TEST_CASE("a density probe returns the reduced density matrix, and $-operands name physical qubits") {
    Lab& l = lab("sc_fixed_5");
    RunOptions options;
    options.noise = NoiseSource::Ideal;
    options.shots = 4;
    ...
    for (std::size_t k = 1; k < d->values.size(); k += 2) REQUIRE(std::abs(d->values[k]) < 1e-9);
...
TEST_CASE("probe operands accept a whole register and unknown qubits are dropped") {
    Lab& l = lab("sc_fixed_5");
    const Lab::Job job = l.compile(source("pragma qlab.probe bloch q\nqubit[3] q;\nbit[3] c;\n"
                                          "h q[0];\ncx q[0], q[1];\ncx q[1], q[2];\nc = measure q;\n"));
    ...
    for (std::uint32_t q : probes[0].qubits) REQUIRE(q < l.session.device()->qubitCount());
