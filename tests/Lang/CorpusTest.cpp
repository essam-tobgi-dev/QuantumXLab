...
static std::string slurp(const fs::path& p) { std::ifstream f(p); std::stringstream ss; ss << f.rdbuf(); return ss.str(); }
static std::vector<fs::path> qasmFiles(const fs::path& dir) {
    std::vector<fs::path> v;
    if (!fs::exists(dir)) return v;
    for (auto& e : fs::recursive_directory_iterator(dir)) if (e.is_regular_file() && e.path().extension() == ".qasm") v.push_back(e.path());
...
TEST_CASE("every example and calibration program parses without error diagnostics") {
    ...
    for (auto& f : files) {
        ...
        std::string diag; for (auto& d : p.diagnostics) if (d.isError()) diag += d.error.format() + "\n";
        INFO(f.string() << "\n" << diag);
        CHECK(p.ok());
        CHECK((p.hasQuantumStatements || p.pragmas.rb.has_value() || !p.defcals.empty()));
        if (f.parent_path().filename() == "Examples" || f.parent_path().parent_path().filename() == "Examples") {
            fs::path exp = f; exp.replace_extension(".expected.json");
            CHECK(fs::exists(exp));
        }
    }
}
...
TEST_CASE("conformance corpus matches expected diagnostics") {
    ...
    int checked = 0;
    for (auto& f : files) {
        fs::path dj = f; dj.replace_extension(".diag.json");
        ...
        Program p = analyzeProgram(slurp(f), f.filename().string());
        std::vector<std::string> got; std::string text;
        for (auto& d : p.diagnostics) { got.push_back(d.id()); text += d.error.format() + "\n"; }
        std::sort(expected.begin(), expected.end()); std::sort(got.begin(), got.end());
        INFO(f.string() << "\n" << text);
        CHECK(got == expected);
        ++checked;
    }
...
TEST_CASE("dumpAst is deterministic and stdgates table is complete") {
    ...
    for (const char* g : {"p", "x", "y", "z", "h", "s", "sdg", "t", "tdg", "sx", "rx", "ry", "rz", "cx", "cy", "cz", "cp", "crx", "cry", "crz", "ch", "swap", "ccx", "cswap", "cu", "CX", "phase", "id", "u1", "u2", "u3", "U", "gphase"})
        REQUIRE(StdGates::find(g) != nullptr);
    REQUIRE(StdGates::doc("cx")->signature == "cx c, t");
    Program inc = analyzeProgram(std::string("OPENQASM 3.0;\nqubit q;\n") + std::string(StdGates::source()).substr(std::string(StdGates::source()).find('\n') + 1) + "x q;\n");
    // stdgates.inc source itself only redefines std names -> QL3061 for each definition is expected
    bool allShadow = true; for (auto& d : inc.diagnostics) if (d.id() != "QL3061" && d.isError()) allShadow = false;
    REQUIRE(allShadow);
