...
static std::string diagText(const Program& p) { std::string s; for (auto& d : p.diagnostics) s += d.error.format() + "\n"; return s; }
static bool has(const Program& p, const char* id) { for (auto& d : p.diagnostics) if (d.id() == id) return true; return false; }
...
TEST_CASE("lexer classifies tokens") {
    auto toks = tokenize("OPENQASM 3.0; qubit[2] q; rx(pi/2) q[0]; delay[100ns] q; $3 // c\n", "t.qasm").value();
    REQUIRE(toks[0].kind == TokenKind::Keyword);
    REQUIRE(toks[1].kind == TokenKind::Number); REQUIRE(toks[1].isFloat);
    REQUIRE(toks[3].kind == TokenKind::Type);
    bool sawDur = false, sawPhys = false, sawBuiltin = false;
    for (auto& t : toks) { if (t.kind == TokenKind::Duration) { sawDur = true; REQUIRE(t.unit == DurationUnit::Ns); REQUIRE(t.fval == 100.0); }
        if (t.kind == TokenKind::PhysicalQubit) { sawPhys = true; REQUIRE(t.ival == 3); } if (t.kind == TokenKind::Builtin && t.text == "pi") sawBuiltin = true; }
    REQUIRE(sawDur); REQUIRE(sawPhys); REQUIRE(sawBuiltin);
    auto lenient = tokenizeLenient("x q; // comment\n");
    bool comment = false; for (auto& t : lenient) if (t.kind == TokenKind::Comment) comment = true;
    REQUIRE(comment);
...
TEST_CASE("bell program parses and analyzes without diagnostics") {
    ...
    INFO(dumpAst(p.ast));
    for (auto& d : p.diagnostics) INFO(d.error.format());
    REQUIRE(p.diagnostics.empty());
    REQUIRE(p.qubitCount == 2); REQUIRE(p.bitCount == 2);
...
TEST_CASE("error recovery reports multiple independent errors") {
    Program p = analyzeProgram("OPENQASM 3.0;\ninclude \"stdgates.inc\";\nqubit[2] q;\nh q[0]\ncx q[0], q[1];\nx ;\nmeasure q[9];\n");
    int errs = 0; for (auto& d : p.diagnostics) if (d.isError()) ++errs;
    INFO(dumpAst(p.ast)); for (auto& d : p.diagnostics) INFO(d.error.format());
    REQUIRE(errs >= 2);
}

TEST_CASE("pragmas are typed") {
    Program p = analyzeProgram("OPENQASM 3.0;\ninclude \"stdgates.inc\";\npragma qlab.shots 2048\npragma qlab.device sc_heavyhex_27\npragma qlab.backend densitymatrix\npragma qlab.seed 7\ninput float theta;\npragma qlab.sweep theta from 0 to 1 step 0.25\npragma qlab.probe bloch q[0]\npragma qlab.layout physical\nx $0;\nrx(theta) $1;\n");
    INFO(diagText(p));
    REQUIRE(p.ok());
    REQUIRE(p.pragmas.shots == 2048); REQUIRE(p.pragmas.device == "sc_heavyhex_27");
    REQUIRE(p.pragmas.backend == BackendChoice::DensityMatrix); REQUIRE(p.pragmas.seed == 7);
    REQUIRE(p.pragmas.sweeps.size() == 1); REQUIRE(p.pragmas.sweeps[0].count() == 5);
    REQUIRE(p.pragmas.probes.size() == 1); REQUIRE(p.pragmas.probes[0].kind == "bloch");
    REQUIRE(p.usesPhysicalQubits); REQUIRE(p.inputs.size() == 1); REQUIRE(p.inputs[0].hasSweep);
}
...
delay[50ns] q[2];
    ...
    INFO(diagText(p));
    REQUIRE(p.ok());
    REQUIRE(p.userGates.contains("mygate")); REQUIRE(p.defs.contains("parity"));
    REQUIRE(p.hasFeedforward); REQUIRE(p.hasMidCircuitMeasurement);
    std::string d = dumpAst(p.ast);
...
x $0;
    ...
    Program bad = analyzeProgram("OPENQASM 3.0;\ncal { extern port d0; frame f = newframe(d0, 1e9, 0); play(f, constant(1.5, 10ns)); }\nqubit q;\n");
