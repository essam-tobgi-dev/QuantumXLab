...
bool corrects(IDecoder& decoder, const StabilizerCode& code, const PauliString& error) {
    ...
    for (const auto& g : code.stabilizers)
        if (!g.commutesWith(residual)) return false;
    for (std::uint32_t l = 0; l < code.k; ++l)
        if (!residual.commutesWith(code.logicalX[l]) || !residual.commutesWith(code.logicalZ[l])) return false;
    return true;
}

std::vector<PauliString> errorsOfWeight(const StabilizerCode& code, std::uint32_t weight, std::string_view letters) {
    std::vector<PauliString> out;
    std::vector<std::uint32_t> support(weight);
    std::function<void(std::uint32_t, std::uint32_t, PauliString)> place = [&](std::uint32_t depth, std::uint32_t from, PauliString p) {
        if (depth == weight) { out.push_back(std::move(p)); return; }
...
TEST_CASE("Decoders: d = 3 surface code corrects every single-qubit X, Y, Z error (union-find and lookup)") {
    ...
    for (const char* name : {"union_find", "lookup"}) {
        ...
        const auto none = (*decoder)->decode(SyndromeLattice::fromSyndrome(std::vector<std::uint8_t>(8, 0)));
...
TEST_CASE("Decoders: every shipped code corrects all single-qubit errors it is built against") {
    // Letters the code protects against: the repetition codes detect one Pauli type only (T09 §4.1).
    const std::vector<std::pair<std::string, std::string>> cases = {
        {"repetition_bitflip_3", "X"}, {"repetition_phaseflip_3", "Z"}, {"shor_9", "XYZ"},       {"steane_7", "XYZ"},
        {"five_qubit", "XYZ"},         {"surface_rot_3", "XYZ"},        {"surface_rot_5", "XYZ"}, {"surface_rot_7", "XYZ"}};
    for (const auto& [id, letters] : cases) {
        const StabilizerCode code = shipped(id);
        std::vector<std::string> names = {code.defaultDecoder};
        if (code.isMatchable()) names = {"union_find"};
        if (code.n <= 25) names.push_back("lookup");
    ...
    REQUIRE(makeCodeCapacityDecoder("union_find", shipped("steane_7")).error().code == err::NotMatchable);
    REQUIRE(makeCodeCapacityDecoder("union_find", shipped("five_qubit")).error().code == err::NotMatchable);
    REQUIRE(makeCodeCapacityDecoder("mwpm", shipped("surface_rot_3")).error().code == ErrorCode::Unsupported);
    REQUIRE(LookupDecoder::create(shipped("surface_rot_7")).error().code == err::TooLarge);   // 24 generators per table
}
...
TEST_CASE("Decoders: lookup tables hold minimum-weight corrections") {
    // Steane: the Z-check syndrome of X on qubit j is the binary representation of j + 1 (T09 §4.4),
    // so the 8-entry table is the identity plus the seven single-qubit corrections.
    ...
    // Five-qubit code: perfect, 2⁴ = 1 + 15 syndromes ↔ identity + 15 single-qubit errors (T09 §4.5).
    ...
    for (std::uint32_t index = 0; index < 16; ++index) {
        std::vector<std::uint8_t> s = {std::uint8_t(index & 1), std::uint8_t((index >> 1) & 1), std::uint8_t((index >> 2) & 1), std::uint8_t(index >> 3)};
    ...
    REQUIRE(lookup.decode(SyndromeLattice::fromSyndrome(std::vector<std::uint8_t>(3, 0))).error().code == err::BadSyndrome);
}
...
TEST_CASE("Decoders: union-find clusters, boundary matching and matched edges") {
    ...
    const auto c = uf.decode(SyndromeLattice::fromSyndrome(syndromeOf(code.stabilizers, bulk))).value();
    ...
    const auto cc = uf.decode(SyndromeLattice::fromSyndrome(syndromeOf(code.stabilizers, corner))).value();
    ...
    auto copy = uf.clone();
    REQUIRE(copy->decode(SyndromeLattice::fromSyndrome(syndromeOf(code.stabilizers, bulk))).value().pauli == bulk);
    REQUIRE(uf.decode(SyndromeLattice::fromSyndrome(std::vector<std::uint8_t>(7, 0))).error().code == err::BadSyndrome);
}
...
TEST_CASE("Decoders: every correction reproduces its syndrome, whatever the error weight") {
    ...
    for (int trial = 0; trial < 1500; ++trial) {
        ...
        for (std::uint32_t q = 0; q < code.n; ++q)
            if (rng.uniform() < 0.10) error.setLetter(q, "XYZ"[rng.uniformInt(3)]);
        const auto lattice = SyndromeLattice::fromSyndrome(syndromeOf(code.stabilizers, error));
        for (IDecoder* decoder : {static_cast<IDecoder*>(&standard), static_cast<IDecoder*>(&plain),
            ...
            for (const auto& g : code.stabilizers) REQUIRE(g.commutesWith(residual));
        }
        const PauliString residual = error * standard.decode(lattice).value().pauli;
        logicalFailures += (residual.commutesWith(code.logicalX[0]) && residual.commutesWith(code.logicalZ[0])) ? 0 : 1;
    }
