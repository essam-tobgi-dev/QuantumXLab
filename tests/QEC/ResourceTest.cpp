...
TEST_CASE("Resource: scaling law (7.1), its inversion (7.2) and the T09 §7 table") {
    ...
    struct Row { std::uint32_t d; double pL; double qubits; };
    const Row table[] = {{3, 1e-3, 17},   {5, 1e-4, 49},    {7, 1e-5, 97},    {9, 1e-6, 161},   {11, 1e-7, 241}, {13, 1e-8, 337},
                         {15, 1e-9, 449}, {17, 1e-10, 577}, {21, 1e-12, 881}, {25, 1e-14, 1249}, {27, 1e-15, 1457}};
    for (const Row& r : table) {
        ...
        // (7.2) inverts (7.1): the table's own p_L needs exactly its d, a slightly smaller target d + 2.
    ...
    REQUIRE(ResourceEstimator::tCountFromToffoli(2.7e9) == Approx(1.89e10));   // 7 T per Toffoli (spec 16 §7)
...
TEST_CASE("Resource: RSA-2048 worked example of T09 §9 and T12 §6") {
    ...
    in.cycleTimeS = 1e-3;
    in.factoryQubitsOverride = 5e6 * (13.0 * 13.0) / (27.0 * 27.0);    // reference footprint scaled with d²
...
TEST_CASE("Resource: cycle time comes from the device file") {
    ...
    REQUIRE(loadDeviceCycleTime(devices / "sc_tunable_grid_54" / "device.json").value() == Approx(0.8e-6));
    REQUIRE(loadDeviceCycleTime(devices / "sc_heavyhex_27" / "device.json").value() == Approx(1.0e-6));
    REQUIRE(loadDeviceCycleTime(devices / "ion_chain_11" / "device.json").value() == Approx(5.0e-3));
    const auto missing = cycleTimeFromDevice(core::Json::object());
    REQUIRE_FALSE(missing.has_value());
    REQUIRE(missing.error().message.find("data.qec") != std::string::npos);
    REQUIRE(cycleTimeFromDevice(core::Json{{"qec", {{"cycle_time_us", 2.5}}}}).value() == Approx(2.5e-6));
}
