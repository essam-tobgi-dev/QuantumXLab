...
TEST_CASE(
    "Extraction: surface_rot_3 has 8 ancillas, 24 CNOTs per round and the spec 16 §3 bit layout") {
    ...
    // One round alone: reset, h, four CNOT layers, h, measure (T09 §5.2 "depth 6" plus reset and
    // measure).
    ...
    for (const char* id : {"surface_rot_3", "surface_rot_5"}) {
        ...
        for (std::uint32_t j = 0; j < code.checkCount(); ++j) {
            ...
            const std::vector<std::string> pattern =
                xType ? std::vector<std::string>{"NW", "NE", "SW", "SE"}
...
TEST_CASE("Extraction: the four CNOT layers are parallel and hook pairs are perpendicular to the "
          "logicals") {
    ...
    for (const Op& op : ex->schedule.ops) {
        if (op.marker()) {
            if (!cnotMoments.back().empty())
                cnotMoments.emplace_back();
            continue;
        }
        if (op.kind == OpKind::CX)
            cnotMoments.back().push_back(op);
    }
    if (cnotMoments.back().empty())
        cnotMoments.pop_back();
    ...
    for (std::size_t i = 1; i < xbar.size(); ++i)
        REQUIRE(code.dataLayout[xbar[i]].x == code.dataLayout[xbar[0]].x);
    for (std::size_t i = 1; i < zbar.size(); ++i)
        REQUIRE(code.dataLayout[zbar[i]].y == code.dataLayout[zbar[0]].y);
    for (std::uint32_t j = 0; j < code.checkCount(); ++j) {
        const auto& order = code.ancillas[j].order;
        if (order.size() != 4)
            continue;
        const Coord2 a = code.dataLayout[order[2]], b = code.dataLayout[order[3]];
        if (code.checkType(j) == CheckType::X)
            REQUIRE(a.y == b.y);
        else
            REQUIRE(a.x == b.x);
    }
...
TEST_CASE("Extraction: detector lattice and observables") {
    ...
    const Row rows[] = {
        ...
        // Mixed checks: fixed from round 0 by the explicit encoder, never closed by a transversal
        // readout.
    ...
    for (const Row& r : rows) {
        ...
        for (const Detector& d : ex->detectors)
            REQUIRE(ex->detectorAt(d.check, d.layer) != kNoIndex);
    }
    // T09 (5.2): a bulk detector is the XOR of one check in consecutive rounds; the closing layer
    // adds the data readout of the check's support.
    ExtractionOptions opt;
    opt.rounds = 3;
    ...
    REQUIRE(last.bits == std::vector<std::uint32_t>{ex.syndromeBit(2, 2), ex.dataBit(1),
                                                    ex.dataBit(2), ex.dataBit(4), ex.dataBit(5)});
    ...
    REQUIRE(ex.observables[0] ==
            std::vector<std::uint32_t>{ex.dataBit(2), ex.dataBit(5), ex.dataBit(8)});
    ...
    REQUIRE(planMemoryExperiment(s3, ExtractionOptions{.rounds = 0}).error().code ==
            err::BadOptions);
}
