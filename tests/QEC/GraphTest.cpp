...
TEST_CASE(
    "Graph: phenomenological lattice of the repetition code has space, time and boundary edges") {
    ...
    for (const GraphEdge& e : g.edges) {
        if (e.boundary()) {
            ...
            ++space;
            REQUIRE(e.correction ==
                    std::vector<std::pair<std::uint32_t, char>>{{1u, 'X'}}); // the shared qubit
...
TEST_CASE("Graph: the circuit-level detector error model agrees with the stabilizer backend fault "
          "by fault") {
    const StabilizerCode code = shipped("surface_rot_3");
    ExtractionOptions x;
    x.rounds = 2;
    ...
    for (const FaultEvent& f : faults) {
        ...
        for (std::size_t d = 0; d < tableau.events.size(); ++d)
            if (tableau.events[d])
                (runner.experiment().detectors[d].type == CheckType::X ? xEvents : zEvents)++;
        REQUIRE(xEvents <= 2);
        REQUIRE(zEvents <= 2);
        REQUIRE_FALSE(tableau.decoderRejected);
        REQUIRE_FALSE(
            tableau.failure); // circuit-level distance 3: no single fault defeats the decoder
...
TEST_CASE(
    "Graph: hook faults give edges between checks two steps apart, perpendicular to the logical") {
    ...
    for (std::uint32_t i = 0; i < ex.schedule.ops.size() && afterSecond == kNoIndex; ++i)
        if (ex.schedule.ops[i].kind == OpKind::CX && ex.schedule.ops[i].a == ancilla && ++seen == 2)
            afterSecond = i;
    FaultEvent hook;
    ...
    const Coord2 a = code.ancillas[ex.detectors[fired[0]].check].coord,
                 b = code.ancillas[ex.detectors[fired[1]].check].coord;
