...
std::vector<double> encodedExpectations(qsim::IBackend& backend, const StabilizerCode& code,
    ...
    if (input == "1" || input == "-")
        prep.add(*ir::makeGate("x", {ir::Wire{0}}));
    if (input == "+" || input == "-")
        prep.add(*ir::makeGate("h", {ir::Wire{0}}));
    ...
    for (const PauliString& p : operators)
        out.push_back(backend.expectation(p.toQsim()).value());
    return out;
}
} // namespace
...
TEST_CASE("Encoder: every generator reads +1 and the logical operators read the input state") {
    for (const std::string& id : shippedCodeIds()) {
        ...
        for (const auto& [input, logical] : inputs) {
            ...
            if (code.n <= 9)
                backends.push_back(qsim::makeBackend(qsim::Kind::StateVector)); // exact amplitudes
            for (auto& backend : backends) {
                const auto values = encodedExpectations(*backend, code, input, ops);
                for (std::size_t j = 0; j < m; ++j)
                    REQUIRE(values[j] == Approx(1.0).margin(1e-12));
...
TEST_CASE("Encoder: the repetition encoder is the textbook pair of CNOTs") {
    ...
    REQUIRE(conjugated(PauliString::parse("XII").value(), s.ops).value() ==
            PauliString::parse("XXX").value());
    REQUIRE(conjugated(PauliString::parse("ZII").value(), s.ops).value() ==
            PauliString::parse("ZII").value());
    REQUIRE(conjugated(PauliString::parse("IZI").value(), s.ops).value() ==
            PauliString::parse("ZZI").value());
    // Signs are tracked: h maps Y to −Y, cx maps X⊗Z to −Y⊗Y.
    const std::vector<Op> h = {{OpKind::H, 0}}, cx = {{OpKind::CX, 0, 1}};
    REQUIRE(conjugated(PauliString::parse("Y").value(), h).value() ==
            PauliString::parse("-Y").value());
    REQUIRE(conjugated(PauliString::parse("XZ").value(), cx).value() ==
            PauliString::parse("-YY").value());
...
TEST_CASE(
    "Encoder: the five-qubit code corrects every single-qubit error on the stabilizer backend") {
    // Non-CSS: the memory experiment starts from the encoder instead of a projected product state,
    // so all four checks are deterministic in round 0 (spec 16 §3 "explicit encoder for small
    // codes").
    ...
    for (std::uint32_t q = 0; q < code.n; ++q)
        for (char letter : {'X', 'Y', 'Z'}) {
            ...
            REQUIRE(events ==
                    syndromeOf(code.stabilizers,
                               PauliString::single(5, q, letter))); // perfect code: all distinct
            ...
            REQUIRE((ex.observableValues(bits)[0] ^ predicted) ==
                    0); // corrected readout: still |0̄⟩
        }
}
...
TEST_CASE("Logical gates: transversal gates exist exactly where the theory says") {
    ...
    for (const std::string& id : shippedCodeIds()) {
        ...
        REQUIRE(conjugated(code.logicalZ[0], xbar.ops).value() ==
                PauliString::parse("-" + code.logicalZ[0].str()).value());
    ...
    for (const Op& op : s.ops)
        REQUIRE(op.kind == OpKind::Sdg);
    // S̄ X̄ S̄† = Ȳ = i X̄ Z̄ = −Y⊗7.
    REQUIRE(conjugated(steane.logicalX[0], s.ops).value() ==
            PauliString::parse("-YYYYYYY").value());
    ...
    const Schedule repetitionS =
        planLogicalGate(shipped("repetition_bitflip_3"), LogicalGate::S).value();
    REQUIRE(repetitionS.ops ==
            std::vector<Op>{{OpKind::Sdg, 0}, {OpKind::Sdg, 1}, {OpKind::Sdg, 2}});
    REQUIRE(planLogicalGate(shipped("repetition_bitflip_3"), LogicalGate::H).error().code ==
            err::NoGate);
