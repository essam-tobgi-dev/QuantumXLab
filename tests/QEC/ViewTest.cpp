...
TEST_CASE("View: roles, coordinates and gate arrows of the d = 3 surface code") {
    ...
    auto count = [&](QubitRole r) {
        return std::count_if(view.qubits.begin(), view.qubits.end(),
                             [r](const RoleEntry& e) { return e.role == r; });
    };
    ...
    for (const GateArrow& a : view.arrows) {
        ...
        REQUIRE(a.step < 4);
        REQUIRE(int(a.step) == tomitaSvoreLayer(code.checkType(a.check),
                                                code.ancillas[a.check].coord,
                                                code.dataLayout[a.dataQubit]));
    }
...
TEST_CASE("View: space-time lattice of a decoded shot") {
    ...
    for (std::uint32_t i = 0; i < runner.experiment().schedule.ops.size(); ++i)
        if (runner.experiment().schedule.ops[i].kind == OpKind::RoundStart &&
            runner.experiment().schedule.ops[i].a == 2)
            x4.afterOp = i;
    x4.qubitA = 4;
    x4.pauliA = 'X';
    ...
    // Both Z checks flip in round 2 relative to round 1 (the raw bits themselves are fixed at 0
    // here: Z checks of |0̄⟩ read 0 until the error arrives).
    ...
    const ShotResult timeShot =
        worker.shotWithFaults({flip}, rng, SampleEngine::Stabilizer).value();
    ...
    REQUIRE(timeLattice.points[timeLattice.matched[0].b].layer ==
            timeLattice.points[timeLattice.matched[0].a].layer + 1);
