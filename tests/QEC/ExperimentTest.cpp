...
double majorityVote(std::uint32_t d, double p) {
    ...
    for (std::uint32_t j = d / 2 + 1; j <= d; ++j) {
        double binom = 1.0;
        for (std::uint32_t i = 0; i < j; ++i) binom = binom * double(d - i) / double(i + 1);
...
TEST_CASE("Experiment: every single-qubit error is corrected on the stabilizer backend (d = 3 surface code)") {
    ...
    for (LogicalBasis basis : {LogicalBasis::Z, LogicalBasis::X})
        for (const char* decoder : {"union_find", "lookup"}) {
            ...
            x.basis = basis;
            NoiseParams n;
            n.setting = NoiseSetting::Phenomenological;   // data errors before each round, q = 0: reliable syndromes
            n.p = 0.01;
            n.q = 0.0;
            ...
            for (std::uint32_t round : {0u, 1u})
                for (std::uint32_t q = 0; q < code.n; ++q)
                    for (char letter : {'X', 'Y', 'Z'}) {
                        ...
                        f.qubitA = q;
                        f.pauliA = letter;
                        for (SampleEngine engine : {SampleEngine::Stabilizer, SampleEngine::PauliFrame}) {
                            const auto shot = worker.shotWithFaults({f}, rng, engine);
                            INFO(decoder << " basis " << basisName(basis) << " round " << round << ": " << letter << " on " << q);
                            ...
                            const bool visible = letter == 'Y' || (letter == 'X') == (basis == LogicalBasis::Z);
                            if (visible) REQUIRE(std::count(shot->events.begin(), shot->events.end(), 1) > 0);
                        }
                    }
        }
    ...
    REQUIRE(ExperimentRunner::create(shipped("steane_7"), {}, {}, "union_find").error().code == err::NotMatchable);
}
...
TEST_CASE("Experiment: surface code d = 5 beats d = 3 under circuit-level noise at p = 1e-3") {
    // Spec 25 §3.8: p_L(d = 5) < p_L(d = 3) < p. One logical cycle (d rounds) per shot, union-find on
    // the detector error model, 10^5 seeded shots per distance on the Pauli-frame engine.
    LogicalErrorExperiment e;
    e.noise = NoiseSetting::CircuitLevel;
    e.p = {1e-3};
    e.samples = 100000;
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        // The same seeded series on the stabilizer backend fails in exactly the same shots: checked on
        // its first 3000 shots, which keeps the run time bounded.
    ...
    INFO("d = 3: " << point[0].failures << " failures, d = 5: " << point[1].failures << " of " << e.samples);
    ...
    REQUIRE(perRoundErrorRate(point[0].pL_hi, 3) < 1e-3);  // per round, d = 3 already beats the physical rate
}
