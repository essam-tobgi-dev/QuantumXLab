...
TEST_CASE("resonator_notch recovers f_r, Q_i, Q_c at 40 dB SNR") {
    ...
    for (int i = 0; i < 400; ++i) {
        ...
        f.push_back(x);
        re.push_back(s.real() + rng.normal(0, noise));
        im.push_back(s.imag() + rng.normal(0, noise));
    }
    // Analytic Jacobian vs finite differences at the truth.
    {
        ...
        for (std::size_t j = 0; j < 7; ++j) {
            ...
            double h = j == 6
                           ? 1e-15
                           : std::max(1e-6 * std::abs(truth[j]), 1e-12); // tau: 2πf h must stay ≪ 1
            bp[j] += h;
            bm[j] -= h;
...
TEST_CASE("gmm2 separates two IQ blobs and estimates assignment error") {
    ...
    REQUIRE(g->assignmentError ==
            Approx(0.5 * std::erfc(expectedSnr / (2 * std::numbers::sqrt2))).epsilon(0.3));
    ...
    Gmm2Options o;
    o.labels = lab;
    o.sharedCovariance = false;
    auto g2 = fitGmm2(x, y, o);
    REQUIRE(g2);
    REQUIRE(g2->comp[1].mx == Approx(sep).margin(0.15));
