...
TEST_CASE("Q-sphere latitude is set by the Hamming weight, longitude by index order") {
    ...
    for (std::uint64_t i : {0b001u, 0b010u, 0b100u})
        CHECK(qspherePlacement(i, n).z == Approx(1.0 - 2.0 / 3.0));
    for (std::uint64_t i : {0b011u, 0b101u, 0b110u})
        CHECK(qspherePlacement(i, n).z == Approx(1.0 - 4.0 / 3.0));
    ...
    for (std::uint32_t w = 0; w <= big; ++w)
        seen[w].assign(binomial(big, w), false);
...
TEST_CASE("Q-sphere nodes: area proportional to probability, phase colour, top-k above 12 qubits") {
    ...
    CHECK(m.nodes[0].radius * m.nodes[0].radius / (o.maxRadius * o.maxRadius) ==
          Approx(0.5)); // area ∝ p
...
TEST_CASE("amplitude selection: threshold, top-k with probability mass, ordering, marginals") {
    // QFT|1⟩ on 3 qubits: uniform magnitudes, phases 0, π/4, …, 7π/4 in little-endian order (spec
    // 21 §5).
    std::vector<Complex> qft(8);
    for (std::size_t k = 0; k < 8; ++k)
        qft[k] = std::polar(1.0 / std::sqrt(8.0), 2 * kPi * static_cast<double>(k) / 8.0);
    ...
    for (std::size_t k = 0; k < 8; ++k) {
        ...
        CHECK(all.entries[k].phase ==
              Approx(wrapPhase(2 * kPi * static_cast<double>(k) / 8.0)).margin(1e-12));
    }
    ...
    CHECK_FALSE(
        marginalProbabilities(probs, 3, std::vector<QubitIndex>{QubitIndex{3}}).has_value());
}
...
TEST_CASE("city-plot bars and Hinton squares of a Bell state") {
    ...
    for (const auto& s : hinton->squares)
        CHECK(s.side == Approx(1.0));                                // all four have |ρ_ij| = max
...
TEST_CASE("display colours survive the renderer's ACES + gamma tone map exactly") {
    for (const glm::vec3 c :
         {glm::vec3(0.0f), glm::vec3(1.0f), glm::vec3(0.31f, 0.64f, 1.0f),
          glm::vec3(0.886f, 0.85f, 0.888f), glm::vec3(0.186f, 0.078f, 0.223f)}) {
    ...
    CHECK(readableOn(phaseColor(0.0), glm::vec3(1.0f), glm::vec3(0.0f)) ==
          glm::vec3(1.0f)); // dark hue → light text
