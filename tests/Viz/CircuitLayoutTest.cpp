...
ir::Circuit qft4() {
    ...
    for (int t = 3; t >= 0; --t) {
        ...
        for (int k = t - 1; k >= 0; --k)
            gate(c, "cp", {static_cast<std::uint32_t>(k), static_cast<std::uint32_t>(t)}, {kPi / static_cast<double>(1 << (t - k))});
    }
    gate(c, "swap", {0, 3});
    gate(c, "swap", {1, 2});
    return c;
...
const Glyph& byTopo(const CircuitLayout& lay, std::uint32_t topoIndex) {
    for (const Glyph& g : lay.glyphs)
        if (g.depth == 0 && g.topoIndex == topoIndex) return g;
...
TEST_CASE("QFT-4 lays out in 8 moments and 11 columns with no overlapping boxes") {
    ...
    for (const Glyph& g : lay->glyphs) {
        ...
        if (std::find(cols.begin(), cols.end(), g.column) == cols.end()) cols.push_back(g.column);
    }
    for (std::size_t m = 0; m < 8; ++m) CHECK(columnsOfMoment[m].size() == ((m == 2 || m == 3 || m == 4) ? 2u : 1u));
    ...
    for (const Glyph& a : lay->glyphs)
        for (const Glyph& b : lay->glyphs)
            if (a.moment < b.moment) CHECK(a.column < b.column);
    // Glyph content: cp is a controlled box labelled with its base gate and angle as a multiple of π.
...
TEST_CASE("classical control: a branch is a bracketed region around its body") {
    ...
    gate(c, "h", {0});
    c.add(ir::Measure{ir::Wire{0}, ir::ClassicalBit{0}, std::nullopt, {}});
    ir::Branch br{ir::ClassicalExpr::binary(ir::ClassOp::Eq, ir::ClassicalExpr::bitRef(ir::ClassicalBit{0}), ir::ClassicalExpr::constant(1)),
                  ir::SubCircuit(body), ir::SubCircuit(), {}};
    c.add(std::move(br));
    auto lay = layoutCircuit(c);
    REQUIRE(lay.has_value());
    CHECK_FALSE(lay->firstOverlap().has_value());
    ...
    for (const Glyph& g : lay->glyphs) {
        if (g.kind == GlyphKind::Region) region = &g;
        if (g.depth == 1) inside.push_back(&g);
    }
    REQUIRE(region != nullptr);
    CHECK(region->label.rfind("if (", 0) == 0);
    CHECK(region->columnSpan == 2);
    REQUIRE(inside.size() == 2);
    for (const Glyph* g : inside) { // the bracket encloses its body; body glyphs share the branch's playhead index
    ...
    CHECK(lay->glyphAt(inside[0]->bounds.cx(), inside[0]->bounds.cy()) == inside[0]); // the gate wins over its region
}
...
TEST_CASE("timed layout places boxes at t_start with widths proportional to duration") {
    ...
    c.meta()["schedule"] = {{"start_ps", {0, 40000, 0, 40000}}, {"length_ps", {40000, 0, 40000, 300000}}};
