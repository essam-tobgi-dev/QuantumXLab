...
int distance(const unsigned char* px, glm::vec3 c) {
    return std::abs(px[0] - static_cast<int>(c.r * 255)) +
           std::abs(px[1] - static_cast<int>(c.g * 255)) +
           std::abs(px[2] - static_cast<int>(c.b * 255));
}
} // namespace
...
TEST_CASE("Q-sphere view: GHZ sits on the poles; hover reads the state, click focuses it") {
    ...
    CHECK(selection.basisFocus() ==
          std::optional<std::uint64_t>{7}); // sets the amplitude-bar filter
    REQUIRE(view.click(south, &selection).has_value());
...
TEST_CASE("Q-sphere view renders to PNG with node colours from the phase LUT") {
    ...
    REQUIRE(px != nullptr);
    REQUIRE(w == 480);
    const auto at = [&](glm::vec2 p) {
        return px + (static_cast<std::size_t>(std::lround(p.y)) * 480 +
                     static_cast<std::size_t>(std::lround(p.x))) *
                        4;
    };
