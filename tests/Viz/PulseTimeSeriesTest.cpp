...
Picoseconds ns(double v) { return Picoseconds{static_cast<std::int64_t>(std::llround(v * 1000.0))}; }
...
pulse::Schedule makeSchedule() {
    ...
    s.insert(pulse::Play{pulse::ChannelId::drive(0), pulse::Waveform::constant(20e-9, 0.5), ns(0)}, ns(0));
    s.insert(pulse::FrameOp{pulse::ChannelId::drive(0), ns(10), pulse::FrameOp::Op::SetFrequency, 5.2e9}, ns(10));
    s.insert(pulse::FrameOp{pulse::ChannelId::drive(0), ns(20), pulse::FrameOp::Op::ShiftPhase, kPi / 2}, ns(20));
    s.insert(pulse::Play{pulse::ChannelId::drive(0), pulse::Waveform::gaussian(20e-9, 5e-9, 0.8), ns(20)}, ns(20));
    s.insert(pulse::Play{pulse::ChannelId::measure(0), pulse::Waveform::constant(40e-9, 0.3), ns(40)}, ns(40));
    s.insert(pulse::Acquire{pulse::ChannelId::acquire(0), ns(40), ns(30), "matched", pulse::AcquireKind::Integrate, 2},
             ns(40));
...
TEST_CASE("pulse viewer: one row per channel, the envelope on the device grid, frames and windows") {
    ...
    const std::vector<num::Complex> wave = pulse::Waveform::gaussian(20e-9, 5e-9, 0.8).sampled(kDtPs);
    REQUIRE(wave.size() == 40);
    for (std::size_t k = 0; k < wave.size(); ++k) CHECK(d.trace.re[40 + k] == Approx(wave[k].real()));
...
TEST_CASE("populations vs time: P0, P1 and the leakage curve of one site, with the drive underneath") {
    ...
    series->blochNorm = {{}, {}};
    for (int k = 0; k <= 40; ++k) series->blochNorm[0].push_back(1.0 - 0.001 * k);
    ...
    for (std::size_t k = 1; k < 41; ++k) CHECK(view.level(2)[k] >= view.level(2)[k - 1]);
