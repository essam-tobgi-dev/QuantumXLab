...
std::string verifyId(const Schedule& s, const hw::Device& dev, std::vector<Warning>* warnings = nullptr) {
...
TEST_CASE("durations quantise to whole granules of dt, never below the minimum pulse") {
    ...
    for (double s : {1.8e-9, 17.3e-9, 200e-9, 700e-9, 1e-6}) {       // ≥ half a granule: off by at most half a granule
...
TEST_CASE("schedule composition: append ASAP per channel, barrier, block alignment") {
    ...
    for (auto const* x : {&left, &right, &seq}) REQUIRE(x->verifyTiming(16, 16).has_value());
    REQUIRE(left.verifyTiming(16, 64).error().diagnosticId == "E_PULSE_MIN"); // the 16-sample d1 pulses
...
TEST_CASE("verify rejects the negative corpus with the spec 10 identifiers") {
    ...
    offGridLength.insert(Play{d0, Waveform::constant(10 * kDt * 1e-12 + 64 * kGrain * 1e-12, 0.2), {}}, Picoseconds{0});
    REQUIRE(verifyId(offGridLength, dev) == "E_PULSE_ALIGN");

    Schedule shortPulse(Picoseconds{kDt});
    shortPulse.insert(Play{d0, Waveform::constant(3 * kGrain * 1e-12, 0.2), {}}, Picoseconds{0}); // 48 < 64 samples
    ...
    auto onChannel = [&](ChannelId ch, const hw::Device& d, std::int64_t dt = kDt) {
        ...
        s.insert(Play{ch, Waveform::constant(static_cast<double>(64 * dt) * 1e-12, 0.1), {}}, Picoseconds{0});
        return verifyId(s, d);
    };
