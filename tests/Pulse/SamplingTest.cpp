...
double maxSlope(const Waveform& w) {
    ...
    double m = 0.0;
    for (int i = 1; i < n; ++i) m = std::max(m, std::abs(w.sample((i + 0.5) * h) - w.sample((i - 0.5) * h)) / h);
    return 1.01 * m;
}
...
TEST_CASE("device-rate sampling reproduces every envelope on whole 16-sample granules") {
    ...
    // Rz_t(π) precedes the target correction: the sx samples are the plain sx samples times e^{−iπ}.
    ...
    for (std::size_t k = 0; k < sxSpan.count; ++k)
        REQUIRE(std::abs(target->samples[sxSpan.start + k] + sxAlone->channels[0].samples[k]) <= 1e-12);
}

TEST_CASE("1 GS/s sampling: dt quantisation, granularity, and zero-order-hold interpolation tolerance") {
    const PulseLibrary& lib = library("sc_fixed_5");
    for (const char* gate : {"sx", "cx", "measure"}) {
        INFO(gate);
        const std::vector<std::uint32_t> qubits = std::string(gate) == "cx" ? std::vector<std::uint32_t>{0, 1}
        ...
        for (auto const& instr : s->instructions()) {
            auto* p = std::get_if<Play>(&instr);
            if (!p) continue;
            // |s_1GS(t_k) − s_dev(t_j)| ≤ max|ṡ|·222 ps, t_j = ⌊t_k/222 ps⌋·222 ps: frame ops and play
            // edges sit on 16·222 ps granules, so no jump falls between t_j and t_k.
            ...
            const std::size_t last = static_cast<std::size_t>((p->t0.value + p->duration().value + 999) / 1000);
            bool withinPlaySpans = std::any_of(a->plays.begin(), a->plays.end(), [&](const SampleSpan& sp) {
...
TEST_CASE("frame phase and frequency instructions shape the sampled baseband") {
    ...
    for (auto const& f : sx.frames()) withShift.addFrame(f);
    withShift.insert(FrameOp{ChannelId::drive(0), {}, FrameOp::Op::ShiftFrequency, shiftHz}, Picoseconds{0});
    ...
    for (std::size_t k = 0; k < p.size(); ++k) {
        ...
        REQUIRE(std::abs(z->find(ChannelId::drive(0))->samples[k] - p[k] * std::polar(1.0, -theta)) <= 1e-15);
        ...
        REQUIRE(std::abs(shifted->channels[0].samples[k] - p[k] * std::polar(1.0, -2.0 * kPi * shiftHz * tk)) <= 1e-12);
    }
...
TEST_CASE("invalid sample rates and granularities are rejected") {
    ...
    REQUIRE(sampleSchedule(cx, 1e9, 1)->length == static_cast<std::size_t>((cx.duration().value + 999) / 1000));
}
