...
TEST_CASE("sx drive: Ω = κ_d·A·e(t) and the two-level area theorem yields Rx(pi/2)") {
    ...
    REQUIRE(d.referenceFrequencyHz == d.channelFrequencyHz); // resonant: no rotation in the qubit frame
    ...
    for (std::size_t k = 0; k < d.held.size(); k += 17)
        REQUIRE(std::abs(d.held[k] - d.scale * play.wf.sample(static_cast<double>(k) * 222e-12)) <= 1e-15 * d.scale);
    ...
    // Σ Re Ω_k dt = π/2 up to the zero-order-hold (left Riemann) error (h²/6)·|Ω'(0)| of a lifted Gaussian.
    double sum = 0.0;
    for (auto const& w : d.held) sum += w.real() * d.dtS;
    const double slope0 = d.scale * play.wf.amplitude * gaussianShapeDerivative(0.0, play.wf.duration, play.wf.sigma);
    REQUIRE(std::abs(sum - kPi / 2.0) <= 2.0 * d.dtS * d.dtS / 6.0 * std::abs(slope0));

    // Spec 10 §11: the area theorem on the two-level RWA model gives Rx(π/2) to 1e−9 (in-phase envelope;
    // the DRAG quadrature is a three-level correction).
    Schedule plain(lib.dt());
    for (auto const& f : sx.frames()) plain.addFrame(f);
    Play noDrag = play;
    noDrag.wf.beta = 0.0;
...
TEST_CASE("virtual Z: shift_phase(-theta) between two sx pulses is Rz(theta) exactly (spec 10 §3)") {
    const PulseLibrary& lib = library("sc_fixed_5");
    const Schedule sx = *lib.scheduleFor("sx", {0});
...
TEST_CASE("CR tone: rotates at f_t - f_c in the control frame; the lab frame carries the full carrier") {
    const PulseLibrary& lib = library("sc_fixed_5");
    ...
    const double f0 = lib.calibration().qubit(0)->f01.value.v, f1 = lib.calibration().qubit(1)->f01.value.v;
    REQUIRE(cr.site == 0);
    ...
    for (double t : {1.0e-9, 37.3e-9, 150.0e-9, 300.1e-9}) {
        ...
        REQUIRE(std::abs(cr.at(t) - h * std::polar(1.0, -kTwoPi * (f1 - f0) * t)) <= 1e-12 * std::abs(h));
        ...
        // RWA: demodulating the lab signal over one carrier period inside the hold interval recovers Ω.
...
TEST_CASE("a drive envelope plugs straight into qsim::DriveTerm (spec 07 §5)") {
    ...
    for (auto const& d : drives->drives) {
        ...
        term.envelope = d.envelope(); // std::function<Complex(double)>, the type the backend expects
        terms.push_back(std::move(term));
    }
    REQUIRE(terms.size() == drives->drives.size());
    for (std::size_t i = 0; i < terms.size(); ++i)
        for (double t : {0.0, 12.5e-9, 260e-9}) REQUIRE(terms[i].envelope(t) == drives->drives[i].at(t));
}

TEST_CASE("toSystemDrives refuses a foreign dt and uncalibrated channels; loads from a device directory") {
    const PulseLibrary& lib = library("sc_fixed_5");
    Schedule foreign(Picoseconds{250});
    foreign.insert(Play{ChannelId::drive(0), Waveform::constant(64 * 250e-12 * 16, 0.1), {}}, Picoseconds{0});
    REQUIRE(toSystemDrives(foreign, lib).error().code == kErrDrive);
    const PulseLibrary& ions = library("ion_chain_11");
    Schedule global(ions.dt());
    global.insert(Play{ChannelId::globalRaman(), Waveform::constant(64e-9, 0.1), {}}, Picoseconds{0});
