...
inline const std::vector<std::string>& shippedDevices() {
    static const std::vector<std::string> ids = {"sc_fixed_5",         "sc_heavyhex_27", "sc_heavyhex_127",
...
inline const qlab::pulse::PulseLibrary& library(const std::string& id) {
    ...
    if (it == cache.end()) {
        ...
        if (!lib) FAIL("loading " << id << ": " << lib.error().format());
...
inline U2 rotationStep(double wx, double wy, double h) {
    ...
    if (w == 0.0) return {1.0, 0.0, 0.0, 1.0};
...
inline U2 propagate(const std::vector<Complex>& omega, double dt) {
    ...
    for (const Complex& w : omega) u = mul(rotationStep(w.real(), w.imag(), dt), u);
    return u;
}

inline U2 rx(double theta) { return rotationStep(1.0, 0.0, theta); }
...
inline double processFidelity(const U2& u, const U2& v) {
    const Complex tr = std::conj(u[0]) * v[0] + std::conj(u[2]) * v[2] + std::conj(u[1]) * v[1] + std::conj(u[3]) * v[3];
