...
TEST_CASE("channel names round-trip") {
    for (std::string s :
         {"d[0]", "u[3,7]", "f[12]", "m[1]", "a[1]", "g", "r[5]", "ms[0,1]", "detect", "pump"}) {
...
TEST_CASE("gaussian area matches the closed form") {
    ...
    const double expected =
        (sigma * std::sqrt(2 * std::numbers::pi) * std::erf(T / (2 * std::numbers::sqrt2 * sigma)) -
         c * T) /
        (1 - c);
...
TEST_CASE("waveform validation rejects bad parameters") {
    ...
    REQUIRE_FALSE(
        Waveform::gaussianSquare(32e-9, 8e-9, 20e-9).validate().has_value()); // 2·rise > T
