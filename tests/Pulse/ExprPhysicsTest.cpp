...
TEST_CASE("pulses.json expressions: parameters, pi, arithmetic, and diagnostics") {
    ...
    REQUIRE(*evalExpression("theta-1", theta, none) == Approx(kPi / 2.0 - 1.0).epsilon(1e-15)); // '-' subtracts
    ...
    const core::Json cal = core::Json::parse(R"({"qubits": {"0": {"f01_ghz": [4.8, 2e-5, "qubit_spectroscopy"]}}})");
...
TEST_CASE("cal.* and device.* paths resolve against the typed calibration in the field's unit") {
    ...
    REQUIRE(*cal("cal.qubits.0.anharmonicity_mhz") == Approx(c.qubit(0)->anharmonicity.value.v / 1e6).epsilon(1e-15));
    REQUIRE(*cal("cal.qubits.2.readout_f_ghz") == Approx(c.qubit(2)->readoutFrequency->value.v / 1e9).epsilon(1e-15));
    REQUIRE(*cal("cal.qubits.4.t1_us") == Approx(c.qubit(4)->t1.value.v * 1e6).epsilon(1e-15));
    REQUIRE(*cal("cal.edges.1-0.duration_ns") == Approx(440.0).epsilon(1e-15)); // either order of the key
    REQUIRE(*evalExpression("cal.edges.0-1.duration_ns*2", {}, cal) == Approx(880.0).epsilon(1e-15));
    REQUIRE(*evalExpression("cal.edges.0-1.duration_ns-40", {}, cal) == Approx(400.0).epsilon(1e-15));
    ...
    REQUIRE(*ion("cal.qubits.5.lamb_dicke") == Approx(ions.calibration().qubit(5)->lambDicke->value).epsilon(1e-15));
    REQUIRE(*ion("cal.motional.axial_modes_mhz.0") == Approx(0.30).epsilon(1e-9)); // COM mode at ω_z (T06 §1.3)
    REQUIRE(*ion("device.ion.f_qubit_ghz") == Approx(12.642812118).epsilon(1e-15));

    for (const char* bad : {"cal.qubits.99.f01_ghz", "cal.qubits.0.lamb_dicke", "cal.edges.0-4.duration_ns",
...
TEST_CASE("area theorem, DRAG and cross-resonance relations (spec 10 §7, T05 §7–§8)") {
    ...
    // T05 §8 numbers: J/2π = 3 MHz, Δ/2π = +150 MHz, α_c/2π = −330 MHz → ν_ZX = −0.0367 Ω, ν_IX = +0.0167 Ω
    const double J = kTwoPi * 3e6, delta = kTwoPi * 150e6, alpha = kTwoPi * -330e6;
    REQUIRE(physics::crZxRate(J, 1.0, delta, alpha) == Approx(-0.02 * 330.0 / 180.0).epsilon(1e-12));
    ...
    REQUIRE(omegaFor2MHz / kTwoPi == Approx(2e6 * 180.0 / (0.02 * 330.0)).epsilon(1e-12)); // 54.55 MHz
    ...
    REQUIRE(physics::crZxRate(J, kappa * amp, delta, alpha) * area == Approx(kPi / 4.0).epsilon(1e-12));
}
...
TEST_CASE("Mølmer–Sørensen square-pulse relations match T06 (6.5)-(6.6) and its worked numbers") {
    ...
    REQUIRE(physics::msAngle(eta, eta, omega, physics::msLoopDuration(d1, 1), -d1) == Approx(kPi / 2.0).epsilon(1e-12));
    REQUIRE(physics::msAngle(eta, eta, omega, physics::msLoopDuration(d2, 2), d2) == Approx(-kPi / 2.0).epsilon(1e-12));
    REQUIRE(physics::msAmplitudeForAngle(kPi / 2.0, eta, eta, physics::msLoopDuration(d1, 1), d1) ==
            Approx(omega).epsilon(1e-12));
    // Spec 10 §6.6 at the shipped 200 µs, K = 1: Ω = π√K/(ητ); the superseded line was smaller by √π.
    const double tau = 200e-6;
    const double om = physics::msAmplitudeForAngle(kPi / 2.0, eta, eta, tau, physics::msDetuningForDuration(tau, 1));
    REQUIRE(om == Approx(kPi / (eta * tau)).epsilon(1e-12));
}

TEST_CASE("MS propagator integrals are exact for piecewise-constant couplings (T06 (6.2), (11.1))") {
    // constant g split into segments, loop not closed: α = −(g/δ)(e^{iδτ} − 1), Φ = (g²/δ)(τ − sin δτ/δ)
