...
// mapped, the error budget p_dep = d r/(d−1) − p_relax (clamped at 0 with a warning), and the queries.
...
double krausRelaxation(const hw::Calibration& cal, std::span<const std::uint32_t> qs, double t) {
    ...
    for (std::uint32_t qb : qs) {
        ...
        auto k = channels::thermalRelaxation(c.t1.value.si(), std::min(c.t2echo.value.si(), 2.0 * c.t1.value.si()), t,
                                             c.thermalPopulation.value);
        NOISE_REQUIRE_OK(k);
        if (!joint) { joint = std::move(*k); continue; }
...
double weightOf(const AttachedChannel& a, std::size_t pauliIndex) {
    ...
    for (std::size_t n = 0; n < k->ops.size(); ++n)
        if (k->pauliIndices[n] == pauliIndex) return k->pauliWeights[n];
    return 0.0;
}
} // namespace
...
TEST_CASE("fromCalibration maps every sc_fixed_5 calibration field (spec 08 §4.1)") {
    ...
    for (std::uint32_t qb = 0; qb < 5; ++qb) {
        ...
        REQUIRE(qn->driftSigmaHz == Approx(driftSigmaFromT2Star(c.t2star.value.si())).epsilon(1e-14)); // T2* < T2
        ...
        REQUIRE(sx->pRelaxation == Approx(krausRelaxation(dev.calibration, target, 32e-9)).epsilon(1e-9));
        ...
        REQUIRE(ids(chans) == std::vector<std::string_view>{id::ThermalRelaxation, id::DetuningDrift, id::Depolarizing1q});
        ...
        // Total infidelity of the attached channels reproduces the calibrated r to first order (T04 (6.1)):
        // F_e(dep ∘ E) = (1 − p)F_e(E) + p/4 exactly, with 1 − F_e(E) = 3 p_relax/4.
        const double feTotal = (1.0 - sx->depolarizing) * (1.0 - 0.75 * sx->pRelaxation) + sx->depolarizing / 4.0;
    ...
    for (const auto& [key, e] : dev.calibration.edges) {
        ...
        for (const char* gate : {"cx", "ecr"}) {
            ...
            REQUIRE(g->pRelaxation == Approx(krausRelaxation(dev.calibration, pair, e.duration.value.si())).epsilon(1e-9));
            for (const auto& order : {q({e.a, e.b}), q({e.b, e.a})}) {
                const auto chans = m.channelsFor(gate, order);
                const auto names = ids(chans);
                REQUIRE(std::count(names.begin(), names.end(), id::Depolarizing2q) == 1); // flipped key too
                const auto dep = std::find_if(chans.begin(), chans.end(), [](const auto& a) { return a.channel->id() == id::Depolarizing2q; });
    ...
    REQUIRE(slot.size() == 3);
    for (const auto& a : slot) REQUIRE_FALSE((a.qubits[0].get() <= 1 && a.qubits[1].get() <= 1));
    ...
    REQUIRE(weightOf(m.channelsFor("reset", q({2}))[0], 1) == Approx(dev.calibration.qubits[2].resetError.value).epsilon(1e-15));
    REQUIRE(ids(m.preparationChannels(q({3}))) == std::vector<std::string_view>{id::ThermalPreparation});
    ...
    REQUIRE(m.idleChannels(QubitIndex{4}, Picoseconds{250000})[0].context.durationS == Approx(250e-9).epsilon(1e-15));
    ...
    REQUIRE((*full)(1, 2) == Approx(m2[1][0] * m0[0][1]).epsilon(1e-15)); // prepared q2=1,q0=0 → read q2=0,q0=1
}

TEST_CASE("fromCalibration on sc_heavyhex_27 clamps where relaxation exceeds the gate error, with a warning") {
    ...
    // Scaling every gate error down makes relaxation dominate almost everywhere: warnings, no error.
    Overrides o;
    o.scaleGateError = 0.05;
    NOISE_REQUIRE_OK(m.setOverrides(o));
    std::size_t clampedNow = 0;
    for (const GateNoise* g : m.gates()) clampedNow += g->clamped;
    REQUIRE(clampedNow > 50);
    REQUIRE(m.warnings().size() == clampedNow);
    REQUIRE(std::all_of(m.warnings().begin(), m.warnings().end(), [](const std::string& w) { return w.starts_with("warn: "); }));
}
...
TEST_CASE("fromCalibration loads every shipped device without error diagnostics (spec 08 §9)") {
    ...
    for (const auto& deviceId : all) {
        ...
        for (std::uint32_t levels : {2u, 3u}) {
            ...
            NOISE_REQUIRE_OK(m);
            for (const auto& w : m->warnings()) REQUIRE(w.starts_with("warn: "));
            ...
            for (std::uint32_t c = 0; c < dev.device.qubitCount(); ++c)
                if (dev.device.isCoupler(c)) REQUIRE(m->idleChannels(QubitIndex{c}, 1e-6).empty()); // noiseless slot
        }
    }
    // With three levels the leakage channel is attached per qubit of a calibrated gate (spec 08 §2.5).
    const auto dev = device("sc_fixed_5");
    NoiseOptions three;
    three.levels = 3;
    ...
    REQUIRE(m->gate("cx", q({1, 2}))->seepage == m->gate("cx", q({1, 2}))->leakage); // p_S = p_L unless given
}
