// Spec 08 §3, T10 §7 / spec 25 §3.4 — assignment matrices (row-stochastic, M_ij = P(read j |
// prepared i)), correlated groups, sampling statistics and constrained least-squares mitigation.
...
namespace {
// Independent dense construction: M = M_{n−1} ⊗ … ⊗ M_0 (little-endian), entries P(read j |
// prepared i).
...
TEST_CASE("probability vectors are mapped by q = M^T p, factor by factor") {
    ...
    const double rows[4][4] = {{0.93, 0.03, 0.03, 0.01},
                               {0.06, 0.88, 0.01, 0.05},
                               {0.05, 0.02, 0.90, 0.03},
                               {0.01, 0.07, 0.06, 0.86}};
    ...
    for (std::size_t j = 0; j < 8; ++j) {
        double acc =
            0.0; // bits 0 and 2 through `corr` (local index = b0 + 2 b2), bit 1 through M_1
        for (std::size_t i = 0; i < 8; ++i) {
            const std::size_t li = ((i >> 0) & 1u) | (((i >> 2) & 1u) << 1),
                              lj = ((j >> 0) & 1u) | (((j >> 2) & 1u) << 1);
            acc += p[i] * corr(li, lj) * kThree[1][(i >> 1) & 1u][(j >> 1) & 1u];
        }
        REQUIRE((*out)[j] == Approx(acc).margin(1e-15));
    }
    std::vector<ReadoutFactor> overlapping{
        {{0, 2}, corr}, {{2}, toRealMatrix(kThree[1])}, {{1}, toRealMatrix(kThree[1])}};
...
TEST_CASE("apply-then-mitigate recovers the ideal distribution") {
    ...
    for (int trial = 0; trial < 20; ++trial) {
        ...
        double total = 0.0;
        for (auto& x : ideal) {
            x = trial % 4 == 0 && rng.uniform() < 0.5 ? 0.0 : rng.uniform();
            total += x;
        }
        for (auto& x : ideal)
            x /= total;
...
TEST_CASE(
    "constrained mitigation satisfies the KKT conditions of min ||M^T p - q|| on the simplex") {
    auto model = ReadoutModel::independent(q({0, 1, 2}), kThree);
    NOISE_REQUIRE_OK(model);
    ...
    REQUIRE(std::any_of(mit->unconstrained.begin(), mit->unconstrained.end(),
                        [](double x) { return x < 0.0; }));
    double sum = 0.0;
    for (double x : mit->probabilities) {
        REQUIRE(x >= 0.0);
        sum += x;
    }
    REQUIRE(sum == Approx(1.0).margin(1e-12));
    // Gradient g = M(Mᵀp − q): equal on the support, not smaller off it (Lagrange multiplier of Σp
    // = 1).
    ...
    for (std::size_t i = 0; i < 8; ++i)
        for (std::size_t j = 0; j < 8; ++j)
            g[i] += (*full)(i, j) * ((*mt)[j] - measured[j]);
    double multiplier = 0.0;
    std::size_t support = 0;
    for (std::size_t i = 0; i < 8; ++i)
        if (mit->probabilities[i] > 1e-9) {
            multiplier += g[i];
            ++support;
        }
    REQUIRE(support > 0);
    ...
    for (std::size_t i = 0; i < 8; ++i) {
        if (mit->probabilities[i] > 1e-9)
            REQUIRE(g[i] == Approx(multiplier).margin(1e-9));
        else
            REQUIRE(g[i] >= multiplier - 1e-9);
    }
    ...
    for (int trial = 0; trial < 200; ++trial) {
        ...
        double res = 0.0;
        for (std::size_t j = 0; j < 8; ++j)
            res += ((*r)[j] - measured[j]) * ((*r)[j] - measured[j]);
...
TEST_CASE("sampled readout flips match the assignment matrix within 3 sigma (10^6 shots)") {
    ...
    for (std::size_t prepared = 0; prepared < 2; ++prepared) {
        ...
        INFO("prepared " << prepared << " flip rate " << static_cast<double>(flipped) / n
                         << " expected " << p);
    ...
    auto pair =
        ReadoutModel::independent(q({0, 1}), std::vector<Assignment2>{kThree[1], kThree[2]});
    ...
    REQUIRE(std::abs(static_cast<double>((*noisy)["10"]) / n - p10) <
            4.0 * std::sqrt(p10 * (1 - p10) / n));
    auto probs = probabilitiesFromCounts(*noisy, 2);
    NOISE_REQUIRE_OK(probs);
    REQUIRE((*probs)[2] ==
            Approx(static_cast<double>((*noisy)["10"]) / n).epsilon(1e-15)); // "10" = index 2
}
