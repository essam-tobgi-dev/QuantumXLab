...
// P(1) after sx – idle t – sx (Ramsey) or sx – idle t/2 – x – idle t/2 – sx (Hahn echo), T10
// §8.3–8.4.
double runSequence(qsim::IBackend& b, const NoiseModel& m, Sequence s, double t, core::Random& rng,
                   const ApplyOptions& o) {
    ApplyReport report;
    auto idle = [&](double dt) {
        NOISE_REQUIRE_OK(applyChannels(b, m.idleChannels(QubitIndex{0}, dt), rng, o, report));
    };
...
double ramseyExact(double t) {
    return 0.5 * (1.0 + std::exp(-t / kT2) * std::exp(-std::pow(t / kT2Star, 2)));
}
double echoExact(double t) {
    return 0.5 * (1.0 - std::exp(-t / kT2));
}
} // namespace
...
TEST_CASE("drift sigma follows (2.5) and Gauss-Hermite nodes reproduce the normal law") {
    ...
    REQUIRE(sigma ==
            Approx(std::numbers::sqrt2 / (2.0 * std::numbers::pi * kT2Star)).epsilon(1e-15));
    REQUIRE(t2StarFromDriftSigma(sigma) == Approx(kT2Star).epsilon(1e-15));
    REQUIRE(driftSigmaFromCalibration(kT2Star, kT2) == sigma);
    REQUIRE(driftSigmaFromCalibration(kT2, kT2) ==
            0.0); // T2* = T2: no quasi-static part (spec 08 §4.1)
    ...
    for (std::size_t n : {7u, 20u, 40u, 64u}) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            if (i > 0)
                REQUIRE(node.detuningHz > (*nodes)[i - 1].detuningHz);
            REQUIRE(node.detuningHz ==
                    Approx(-(*nodes)[n - 1 - i].detuningHz).margin(1e-9 * sigma));
        ...
        if (n >= 40)
            for (double t : {10e-6, 30e-6, 60e-6, 90e-6}) {
                ...
                for (const auto& node : *nodes)
                    c += node.weight * std::cos(2.0 * std::numbers::pi * node.detuningHz * t);
                REQUIRE(c ==
                        Approx(std::exp(-std::pow(t / kT2Star, 2))).margin(1e-12)); // T04 (8.2)
            }
    }
...
TEST_CASE(
    "Ramsey with quasi-static detuning decays as a Gaussian with T2* (DensityMatrix, fitted)") {
    const NoiseModel m = driftModel();
    REQUIRE(m.qubit(QubitIndex{0})->t1S == Approx(kT1).epsilon(1e-15));
    REQUIRE(m.qubit(QubitIndex{0})->driftSigmaHz ==
            Approx(driftSigmaFromT2Star(kT2Star)).epsilon(1e-15));
    core::Random rng(1);
    ...
    for (int i = 1; i <= 24; ++i) {
        ...
        REQUIRE(
            p1 ==
            Approx(ramseyExact(t)).margin(1e-12)); // exact Gaussian-averaged channel (spec 08 §2.6)
...
TEST_CASE("a Hahn echo refocuses the drift and reveals T2 (DensityMatrix, shot average over "
          "Gauss-Hermite nodes)") {
    const NoiseModel m = driftModel();
    ...
    for (double t : {5e-6, 30e-6, 60e-6, 90e-6}) {
        ...
        REQUIRE(
            echo ==
            Approx(echoExact(t)).margin(1e-12)); // exponential with T2: the Gaussian part is gone
        ...
        REQUIRE(p ==
                Approx(0.5 * (1.0 - std::exp(-t / kT2) * std::exp(-0.5 * std::pow(t / kT2Star, 2))))
                    .margin(1e-12));
    }
}

TEST_CASE("per-shot detuning on the StateVector backend: Ramsey shows T2*, echo shows T2 (4000 "
          "shots, 4 sigma)") {
    const NoiseModel m = driftModel();
    ...
    for (double t : {15e-6, 30e-6, 45e-6}) {
        for (Sequence s : {Sequence::Ramsey, Sequence::Echo}) {
            ...
            INFO((s == Sequence::Ramsey ? "Ramsey" : "echo")
                 << " t = " << t << " mean " << mean << " exact " << exact << " se " << se);
