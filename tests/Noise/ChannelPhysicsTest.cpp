// Spec 08 §2, §9 / spec 25 §3.4 — closed-form physics of the catalogue on the DensityMatrix backend.
...
TEST_CASE("depolarizing with p = 1 gives the maximally mixed state") {
    ...
    for (const auto& prep : preparations) {
        ...
        REQUIRE(num::approxEqual(dm.rho(), num::scale(Matrix::identity(2), Complex(0.5, 0)), 1e-15));
    }
    ...
    auto mixed = applyToDensity(*three, num::projector(std::vector<Complex>{0, 0, 0, 1, 0, 0, 0, 0}));
    ...
    const Matrix rho = num::projector(std::vector<Complex>{std::sqrt(0.2), Complex(0, std::sqrt(0.8))});
...
TEST_CASE("amplitude damping on |1> leaves exactly exp(-t/T1) in the excited state") {
    ...
    for (double t : {0.0, 1e-9, 35e-9, 12e-6, 87e-6, 300e-6}) {
        ...
        INFO("t = " << t);
        REQUIRE(dm.population(QubitIndex{1}, 1) == Approx(std::exp(-t / t1)).margin(1e-12)); // spec 25 §3.4
        REQUIRE(dm.population(QubitIndex{0}, 0) == Approx(1.0).margin(1e-15));               // spectator untouched
    }
    ...
    for (double t : {5e-6, 60e-6, 60.0 * t1}) {
        ...
        REQUIRE(dm.population(QubitIndex{0}, 1) == Approx(pth + (1.0 - pth) * std::exp(-t / t1)).margin(1e-12));
    }
}

TEST_CASE("thermal relaxation: <Z> decays with T1, <X> and <Y> with T2, off-diagonals exactly exp(-t/T2)") {
    core::Random rng(3);
    struct Params { double t1, t2, pth; };
    const Params sets[] = {{90.99e-6, 80.47e-6, 0.0073}, {50e-6, 100e-6, 0.0}, {120e-6, 30e-6, 0.02},
    ...
    for (const auto& s : sets) {
        ...
        for (double t : {0.0, 32e-9, 700e-9, 20e-6, 150e-6}) {
            ...
            const double e1 = std::isfinite(s.t1) ? std::exp(-t / s.t1) : 1.0, e2 = std::exp(-t / s.t2);
            ...
            REQUIRE(expectation(one, "Z") == Approx(1.0 - 2.0 * s.pth - 2.0 * (1.0 - s.pth) * e1).margin(1e-12));
    ...
    // The channel is the exact semigroup element: t then t' equals t + t' (no idle splitting needed, T04 §6).
...
TEST_CASE("generalized amplitude damping relaxes to n_th/(2 n_th + 1)") {
    for (double nth : {0.0, 2.5e-3, 0.019, 0.1, 1.7}) {
        ...
        REQUIRE(channels::photonsFromThermalPopulation(pth) == Approx(nth).epsilon(1e-12).margin(1e-15));
        auto full = channels::generalizedAmplitudeDamping(1.0, pth);
        NOISE_REQUIRE_OK(full);
        for (const auto& start : {Matrix::fromRows({{1, 0}, {0, 0}}), Matrix::fromRows({{0, 0}, {0, 1}})}) {
...
TEST_CASE("phase damping and measurement dephasing multiply coherences by the stated factors") {
    ...
    // spec 08 §3: at scale 1 an unmeasured neighbour loses coherence as e^{−t_ro/T2}; scale s mixes λ.
    ...
    for (double s : {0.0, 0.25, 1.0}) {
        ...
        if (s == 1.0) REQUIRE((*rho)(0, 1).real() == Approx(0.5 * std::exp(-tro / t2)).margin(1e-15));
    }
}
...
TEST_CASE("coherent errors: over-rotation, ZZ phase and detuning phase act as their unitaries") {
    ...
    REQUIRE(one.population(QubitIndex{0}, 1) == Approx(std::pow(std::sin(eps / 2), 2)).margin(1e-15));
    ...
    for (int control : {0, 1}) {
        ...
        if (control) NOISE_REQUIRE_OK(two.applyGate(X(), q({1})));
        apply(two, attach(channel(overRotationChannel("ZX", eps)), q({1, 0})), rng);
        REQUIRE(two.population(QubitIndex{0}, 1) == Approx(std::pow(std::sin(eps / 2), 2)).margin(1e-15));
        REQUIRE(expectation(two, "IY") == Approx((control ? 1.0 : -1.0) * std::sin(eps)).margin(1e-15));
    }
    ...
    REQUIRE(expectation(det, "X") == Approx(std::cos(2 * std::numbers::pi * df * tau)).margin(1e-14));
    REQUIRE(expectation(det, "Y") == Approx(std::sin(2 * std::numbers::pi * df * tau)).margin(1e-14));
}
