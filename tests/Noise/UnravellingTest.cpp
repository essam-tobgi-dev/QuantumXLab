...
ChannelPtr generalizedAmplitudeDampingProbe() {
    return channel(amplitudeDampingChannel(15e-6, 0.3));
}

// A two-qubit circuit exercising every unravelling path: Pauli weights (depolarizing, bit flip,
// Pauli), single unitaries (over-rotation, ZZ) and general Kraus sets (relaxation, damping,
// dephasing).
std::vector<Step> mixedCircuit() {
    std::vector<Step> c;
    c.push_back({H(), q({0}),
                 attach(channel(thermalRelaxationChannel(40e-6, 30e-6, 0.05)), q({0}), 12e-6)});
    ...
    c.push_back(
        {RY(0.9), q({1}), attach(channel(amplitudeDampingChannel(25e-6, 0.1)), q({1}), 10e-6)});
...
void run(qsim::IBackend& b, const std::vector<Step>& circuit, core::Random& rng,
         ApplyReport& report) {
    for (const auto& step : circuit) {
        if (!step.gate.empty())
            NOISE_REQUIRE_OK(b.applyGate(step.gate, step.gateTargets));
...
}; // P(00), P(01), P(10), P(11), <X0>, <Z1>, <Y1 X0>
...
Observables observe(const qsim::IBackend& b) {
    ...
    for (const char* label : {"IX", "ZI", "YX"})
        o.values.push_back(expectation(b, label));
    return o;
}
} // namespace

TEST_CASE(
    "StateVector unravelling over 4000 seeded shots matches the DensityMatrix within 4 sigma") {
    ...
    REQUIRE(exact.values[0] + exact.values[1] + exact.values[2] + exact.values[3] ==
    ...
    for (std::size_t i = 0; i < k; ++i) {
        const double mean = sum[i] / n,
                     se = std::sqrt(std::max(sumSq[i] / n - mean * mean, 1e-14) / n);
    ...
    for (const auto& step : circuit)
        if (!step.gate.empty())
            NOISE_REQUIRE_OK(ideal.applyGate(step.gate, step.gateTargets));
    const Observables clean = observe(ideal);
    double distance = 0.0;
    for (std::size_t i = 0; i < k; ++i)
        distance = std::max(distance, std::abs(clean.values[i] - exact.values[i]));
    REQUIRE(distance > 0.1);
}

TEST_CASE(
    "sampled counts follow the unravelled distribution, and readout errors apply per backend") {
    // Terminal sampling: DM applies Mᵀ to the exact distribution (§7.1); SV flips sampled bits
    // (§7.2).
    const std::vector<Assignment2> assign{{{{0.97, 0.03}, {0.08, 0.92}}},
                                          {{{0.94, 0.06}, {0.12, 0.88}}}};
    ...
    for (const qsim::IBackend* b : std::initializer_list<const qsim::IBackend*>{&dm, &sv}) {
        ...
        for (std::size_t i = 0; i < 4; ++i) {
            const double e = (*expected)[i],
                         sigma = std::sqrt(e * (1 - e) / static_cast<double>(shots));
            INFO(qsim::kindName(b->kind())
                 << " outcome " << i << ": " << (*freq)[i] << " vs " << e);
