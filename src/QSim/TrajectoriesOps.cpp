...
Result<Probabilities> TrajectoriesBackend::probabilities(std::span<const QubitIndex> qubits) const {
    ...
    if (qs.empty())
        for (std::uint32_t q = 0; q < n_; ++q)
            qs.push_back(q); // whole register, index order
...
Result<Outcome> TrajectoriesBackend::measure(std::span<const QubitIndex> qubits,
                                             core::Random& rng) {
    if (!allocated_)
        return fail(err::NotAllocated, "Trajectories backend has no model");
    if (!validTargets(qubits, n_))
...
Result<double> TrajectoriesBackend::expectation(const PauliString& p) const {
    ...
    for (std::size_t j = 0; j < dim; ++j) {
        ...
        norm2 += std::norm(amp);
        acc += std::conj(psi_[computationalIndex(model_.siteDims, j ^ p.xMask())]) *
               detail::pauliCoefficient(p, j) * amp;
    }
...
Result<Counts> TrajectoriesBackend::sample(std::span<const QubitIndex> qubits, std::uint64_t shots,
    ...
    if (!probs)
        return std::unexpected(probs.error());
    const auto hist =
        num::sampleCounts(*probs, shots, rng); // the sampler shared with SV/DM/Lindblad (T11 §2.7)
