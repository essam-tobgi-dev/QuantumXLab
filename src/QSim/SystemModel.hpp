...
// supplied by the caller. `driveFromLadder` builds inPhase = a + a† and quadrature = i(a† − a), which
// are σx and σy on the qubit levels, so Ω(t) = (Ωx(t) + iΩy(t))/2 reproduces T05 (7.1) and T07 (1.2):
// a resonant Ω(t) = Ω/2 is H = (Ω/2)σx and gives P₁ = sin²(Ωt/2); Ω(t) = iΩ/2 rotates about +y.
struct DriveTerm {
...
Matrix levelProjector(std::span<const std::uint32_t> siteDims, std::uint32_t site, std::uint32_t level);
// Builds the (A, B) quadrature pair for a drive that couples through the ladder operator of a site:
// A = a + a†, B = i(a† − a) (σx, σy on the qubit levels; see DriveTerm for the envelope convention).
std::pair<Matrix, Matrix> driveFromLadder(std::span<const std::uint32_t> siteDims, std::uint32_t site);
