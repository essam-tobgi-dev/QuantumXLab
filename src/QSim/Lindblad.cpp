...
Capabilities LindbladBackend::capabilities() const {
    return Capabilities{Kind::Lindblad, kMaxSites, /*exactNoise*/ true, /*stochastic*/ false,
                        /*nonClifford*/ true, /*midCircuit*/ true, /*multiLevel*/ true,
...
Status LindbladBackend::allocate(std::uint32_t nQubits, std::uint32_t levelsPerSite) {
    if (nQubits == 0 || nQubits > kMaxSites)
        return fail(err::TooLarge, std::format("Lindblad backend supports 1..{} sites, {} requested",
                                               kMaxSites, nQubits));
    ...
    if (m.dimension() > kMaxDim) // checked before H0 is sized
        return fail(err::TooLarge, std::format("Lindblad dimension {} exceeds the cap {} (3^5)", m.dimension(), kMaxDim));
...
Status LindbladBackend::setModel(SystemModel model) {
    ...
    if (model.siteDims.size() > kMaxSites)
        return fail(err::TooLarge, std::format("Lindblad backend supports at most {} sites", kMaxSites));
    ...
    if (D > kMaxDim)
        return fail(err::TooLarge, std::format("Lindblad dimension {} exceeds the cap {} (3^5)", D, kMaxDim));
    auto square = [D](const Matrix& m) { return m.rows == D && m.cols == D; };
    if (!square(model.h0))
        return fail(err::BadTargets, std::format("H0 is {}x{}, expected {}x{}", model.h0.rows, model.h0.cols, D, D));
    ...
    for (auto& c : model.collapse) {
        if (!square(c.op))
            return fail(err::BadTargets, std::format("collapse operator '{}' does not match the system dimension {}", c.name, D));
    ...
    QXL_LOG_INFO(Sim, "Lindblad allocated: {} sites, dim {}, {} drives, {} collapse ops, {} KiB",
                 n_, D, model_.drives.size(), model_.collapse.size(), (D * D * sizeof(Complex)) >> 10);
...
bool LindbladBackend::hasActiveDrives() const {
    return std::any_of(model_.drives.begin(), model_.drives.end(), [](const DriveTerm& d) { return bool(d.envelope); });
}

void LindbladBackend::enterSegment(double a, double b) {
    // Envelopes are sampled strictly inside the grid segment [a, b]: an AWG envelope that steps at a
    // sample boundary is seen as its one-sided limit, so no stage reads the neighbouring sample and
    // the integrators keep their order (a 1e-9 fraction of the segment is far below any envelope scale).
...
void LindbladBackend::derivative(double t, const Matrix& rho, Matrix& out) {
    // (5.1) with ħ = 1 as dρ/dt = Gρ + (Gρ)† + Σ_k L_k ρ L_k†, G = −iH − ½ Σ_k L_k†L_k (ρ Hermitian).
    hamiltonianAt(std::clamp(t, segLo_, segHi_), hCache_);
    const std::size_t n2 = D_ * D_;
    for (std::size_t i = 0; i < n2; ++i) hCache_.data[i] = Complex(0, -1) * hCache_.data[i] - 0.5 * lDagLSum_.data[i];
    num::matmulInto(hCache_, rho, work1_);
    const Complex* w = work1_.data.data();
    Complex* o = out.data.data();
    for (std::size_t i = 0; i < D_; ++i)
        for (std::size_t j = 0; j < D_; ++j) o[i * D_ + j] = w[i * D_ + j] + std::conj(w[j * D_ + i]);
    for (std::size_t k = 0; k < model_.collapse.size(); ++k) {
        ...
        for (std::size_t i = 0; i < n2; ++i) o[i] += l[i];
    }
}
...
double LindbladBackend::stateNorm() const {
    Complex tr = 0;
    for (std::size_t i = 0; i < D_; ++i) tr += rho_(i, i);
    return tr.real();
}

double LindbladBackend::population(std::uint32_t site, std::uint32_t level) const {
    if (site >= n_ || level >= model_.siteDims[site]) return 0.0;
    std::size_t stride = 1;
    ...
    for (std::size_t idx = 0; idx < D_; ++idx)
        if (static_cast<std::uint32_t>((idx / stride) % d) == level) p += rho_(idx, idx).real();
    return p;
}
...
double LindbladBackend::leakage() const {
    ...
    for (std::size_t idx = 0; idx < D_; ++idx) {
        ...
        for (std::uint32_t s = 0; s < n_; ++s) {
            if (rest % model_.siteDims[s] > 1) { computational = false; break; }
            rest /= model_.siteDims[s];
        }
        if (computational) inside += rho_(idx, idx).real();
    }
