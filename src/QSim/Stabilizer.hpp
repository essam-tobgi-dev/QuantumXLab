...
struct LocalPauli { std::uint32_t x = 0, z = 0; std::uint32_t phase = 0; /* 0..3, i^phase */ };
...
public:
    ...
    Status applyChannel(const Kraus& kraus, std::span<const QubitIndex> targets) override; // refused (Pauli frames are Runtime's)
    ...
    void h(std::uint32_t q); void s(std::uint32_t q); void sdg(std::uint32_t q);
    void x(std::uint32_t q); void y(std::uint32_t q); void z(std::uint32_t q); void sx(std::uint32_t q);
    void cnot(std::uint32_t c, std::uint32_t t); void cz(std::uint32_t a, std::uint32_t b); void swap(std::uint32_t a, std::uint32_t b);
...
private:
    bool getX(std::size_t row, std::uint32_t q) const { return (words_[row * W_ + (q >> 6)] >> (q & 63)) & 1; }
    bool getZ(std::size_t row, std::uint32_t q) const { return (words_[row * W_ + zOff_ + (q >> 6)] >> (q & 63)) & 1; }
