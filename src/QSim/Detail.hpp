...
// P|j⟩ = c_j |j ⊕ x⟩ with c_j = phase · i^{n_Y} · (−1)^{popcount(j & z)} (T11 (2.3), Y = iXZ).
// Valid for strings of at most 64 qubits (the amplitude-based backends).
...
// Re-index a reduced state computed over `sortedSites` (ascending; sortedSites[0] least
// significant) so that requestedSites[0] is the least significant index, as for probabilities().
// `sortedDims[k]` is the local dimension of sortedSites[k]; requestedSites is a permutation of
// sortedSites.
inline Matrix reorderSites(const Matrix& rho, std::span<const std::size_t> sortedSites,
                           std::span<const std::size_t> sortedDims,
                           std::span<const std::size_t> requestedSites) {
    const std::size_t k = sortedSites.size();
    std::vector<std::size_t> pos(k), sortedStride(k);
    for (std::size_t r = 0; r < k; ++r)
        pos[r] = static_cast<std::size_t>(
            std::find(sortedSites.begin(), sortedSites.end(), requestedSites[r]) -
            sortedSites.begin());
    for (std::size_t i = 0, s = 1; i < k; s *= sortedDims[i], ++i)
        sortedStride[i] = s;
