...
Status DensityMatrixBackend::applyOp(const Matrix& u, std::span<const QubitIndex> targets,
    ...
    if (k == 0 || k > 6)
        return fail(err::BadTargets, "gates act on 1–6 qubits");
    if (u.rows != u.cols)
        return fail(err::BadTargets, "matrix must be square");
    std::vector<std::uint32_t> sites(k), sd(k);
    std::size_t full = 1;
    for (std::size_t i = 0; i < k; ++i) {
        sites[i] = targets[i].value;
        sd[i] = dims_[sites[i]];
        full *= sd[i];
    }
    Matrix op;
    if (u.rows == (std::size_t{1} << k))
        op = expandToSites(u, sd);
    else if (u.rows == full)
        op = u;
    else
        return fail(
            err::BadTargets,
            "matrix dimension matches neither the qubit subspace nor the full site dimension");
#ifdef QXL_DEV
    if (!num::isUnitary(u, 1e-10))
        return fail(err::NotUnitary, "gate matrix is not unitary (tol 1e-10)"); // spec 07 §1
#endif
    std::vector<std::size_t> ctrl;
    for (auto c : controls) {
        if (c.value >= n_)
            return fail(err::BadTargets, "control out of range");
        for (auto t : targets)
            if (t == c)
...
Status DensityMatrixBackend::applyGate(const Matrix& u, std::span<const QubitIndex> targets) {
    return applyOp(u, targets, {});
}
Status DensityMatrixBackend::applyControlled(const Matrix& u, std::span<const QubitIndex> controls,
                                             std::span<const QubitIndex> targets) {
    return applyOp(u, targets, controls);
}
...
Status DensityMatrixBackend::applyChannel(const Kraus& kraus, std::span<const QubitIndex> targets) {
    ...
    if (!validTargets(targets, n_))
        return fail(err::BadTargets, "invalid targets");
    const std::size_t k = targets.size();
    if (k == 0 || k > 6)
        return fail(err::BadTargets, "channels act on 1–6 qubits");
    std::vector<std::uint32_t> sites(k), sd(k);
    std::size_t full = 1;
    for (std::size_t i = 0; i < k; ++i) {
        sites[i] = targets[i].value;
    ...
    for (auto& K : kraus)
        if (K.rows != K.cols || K.rows != kraus.front().rows || (K.rows != sub && K.rows != full))
            return fail(err::BadKraus, std::format("Kraus operator is {}x{}; {} target site(s) "
                                                   "need {}x{} (qubit subspace) or {}x{}",
                                                   K.rows, K.cols, k, sub, sub, full, full));
#ifdef QXL_DEV
    if (!num::isTracePreserving(kraus, 1e-10))
        return fail(err::BadKraus, "Kraus set is not trace preserving (tol 1e-10)"); // T11 §9
#endif
    ...
    for (std::size_t idx = 0; idx < kraus.size(); ++idx) {
        ...
        // A qubit-subspace channel is the identity on the leakage levels: K_0 ⊕ 1 and K_{k>0} ⊕ 0
        // keeps Σ K†K = 1 on the full site space (expandToSites alone would count |2⟩ once per
        // operator).
...
void DensityMatrixBackend::checkInvariants() {
    ...
    for (std::size_t i = 0; i < D_; ++i)
        for (std::size_t j = i + 1; j < D_; ++j)
            herm = std::max(herm, std::abs(rho_(i, j) - std::conj(rho_(j, i))));
    if (std::abs(tr - 1.0) > 1e-8) {
        QXL_LOG_WARN(Sim, "density matrix trace drift {:.3e}; renormalizing", tr - 1.0);
        rho_ *= 1.0 / tr;
    }
    if (herm > 1e-8)
        QXL_LOG_WARN(Sim, "density matrix Hermiticity drift {:.3e}", herm);
}

double DensityMatrixBackend::population(QubitIndex q, std::uint32_t level) const {
    double p = 0.0;
    for (std::size_t i = 0; i < D_; ++i)
        if ((i / strides_[q.value]) % dims_[q.value] == level)
            p += rho_(i, i).real();
    return p;
}

Result<Probabilities>
DensityMatrixBackend::probabilities(std::span<const QubitIndex> qubits) const {
    if (!allocated_)
        return fail(err::NotAllocated, "backend not allocated");
...
Result<Counts> DensityMatrixBackend::sample(std::span<const QubitIndex> qubits, std::uint64_t shots,
...
Result<Outcome> DensityMatrixBackend::measure(std::span<const QubitIndex> qubits,
                                              core::Random& rng) {
    if (!allocated_)
        return fail(err::NotAllocated, "backend not allocated");
    Outcome o;
    for (auto q : qubits) {
        ...
        double tot = p0 + p1;
        if (tot <= 0)
            return fail(ErrorCode::Internal, "no computational population left");
...
Status DensityMatrixBackend::reset(std::span<const QubitIndex> qubits, core::Random&) {
    ...
    for (auto q : qubits) {
        ...
        const std::size_t st = strides_[q.value];
        const std::uint32_t d = dims_[q.value];
        std::fill(scratch_.data.begin(), scratch_.data.end(), Complex{});
        for (std::size_t i = 0; i < D_; ++i) {
...
Result<double> DensityMatrixBackend::expectation(const PauliString& ps) const {
    ...
    // tr(Pρ) = Σ_i Σ_j P_ij ρ_ji ; P_ij ≠ 0 only for j = i with X-bits flipped (qubit subspace
    // only).
    Complex acc{};
    for (std::size_t i = 0; i < D_; ++i) {
        std::size_t j = i;
...
Result<Matrix> DensityMatrixBackend::reducedDensityMatrix(std::span<const QubitIndex> keep) const {
    if (!allocated_)
        return fail(err::NotAllocated, "backend not allocated");
    if (keep.empty())
        return fail(err::BadTargets, "a reduced state needs at least one site");
    if (keep.size() > 8)
...
Result<Snapshot> DensityMatrixBackend::snapshot(const SnapshotRequest& req) const {
    ...
    if (req.probabilities) {
        std::vector<QubitIndex> all(n_);
        for (std::uint32_t i = 0; i < n_; ++i)
            all[i] = QubitIndex{i};
        auto p = probabilities(all);
        if (!p)
            return std::unexpected(p.error());
