...
Matrix localMatrix(std::uint32_t x, std::uint32_t z, std::size_t k) {
    ...
    for (std::size_t j = 0; j < k; ++j) {
        ...
        const num::Mat2& m =
            bx ? (bz ? num::pauli::Y : num::pauli::X) : (bz ? num::pauli::Z : num::pauli::I);
...
Result<std::vector<LocalPauli>> StabilizerBackend::cliffordImages(const Matrix& u, std::size_t k) {
    ...
    if (u.rows != dim || u.cols != dim)
        return fail(err::BadTargets, "matrix dimension mismatch");
    if (k > 3)
        return fail(err::Unsupported, "Clifford recognition supports gates on ≤ 3 qubits");
    ...
    for (std::size_t j = 0; j < k; ++j) {
        for (int which = 0; which < 2; ++which) { // X_j then Z_j
            ...
            // x of the image: the single non-zero column of row 0 (a Pauli is a phased
            // permutation).
            std::uint32_t xi = 0;
            bool found = false;
            for (std::size_t c = 0; c < dim; ++c)
                if (std::abs(m(0, c)) > 1e-9) {
                    if (found)
                        return fail(err::NotClifford, "gate is not Clifford");
                    xi = static_cast<std::uint32_t>(c);
                    found = true;
                }
            if (!found)
                return fail(err::NotClifford, "gate is not Clifford");
            ...
            for (std::uint32_t zi = 0; zi < dim && !matched; ++zi) {
                ...
                if (std::abs(std::abs(alpha) - 1.0) > 1e-9)
                    continue;
                bool ok = true;
                for (std::size_t r = 0; r < dim && ok; ++r)
                    for (std::size_t c = 0; c < dim; ++c)
                        if (std::abs(m(r, c) - alpha * p(r, c)) > 1e-9) {
                            ok = false;
                            break;
                        }
                if (!ok)
                ...
                if (std::abs(alpha - 1.0) < 1e-9)
                    ph = 0;
                else if (std::abs(alpha + 1.0) < 1e-9)
                    ph = 2;
                else
                    return fail(err::NotClifford, "gate is not Clifford (non-Hermitian image)");
...
void StabilizerBackend::applyImages(std::span<const std::uint32_t> t,
                                    std::span<const LocalPauli> img) {
    ...
    for (std::size_t row = 0; row < 2 * std::size_t(n_); ++row) {
        ...
        for (std::size_t j = 0; j < k; ++j) {
            if ((x >> j) & 1)
                acc = mul(acc, img[2 * j]);
            if ((z >> j) & 1)
                acc = mul(acc, img[2 * j + 1]);
        }
        acc.phase = (acc.phase + std::popcount(x & z)) & 3;
        std::uint32_t rloc =
            (acc.phase + 4 - static_cast<std::uint32_t>(std::popcount(acc.x & acc.z))) &
            3; // 0 or 2
        if (rloc == 2)
            r_[row] ^= 1;
        for (std::size_t j = 0; j < k; ++j) {
            setX(row, t[j], (acc.x >> j) & 1);
            setZ(row, t[j], (acc.z >> j) & 1);
        }
    }
}
...
Status StabilizerBackend::applyControlled(const Matrix& u, std::span<const QubitIndex> controls,
                                          std::span<const QubitIndex> targets) {
    if (controls.size() + targets.size() > 3)
        return fail(err::Unsupported, "controlled Clifford recognition limited to 3 qubits total");
    ...
    for (std::size_t a = 0; a < dt; ++a)
        for (std::size_t b = 0; b < dt; ++b)
            cu(allOn | a, allOn | b) = u(a, b);
...
Status StabilizerBackend::apply(const GateOp& op) {
    ...
    // Class fast paths only when the op is shaped like its class; anything else is recognised by
    // matrix.
    if (op.controls.empty() && validTargets(op.targets, n_) &&
        ...
        case GateClass::PauliZ:
            ...
            return {};
        case GateClass::Cnot:
            cnot(t0, op.targets[1].value);
            ++ops_;
            return {};
        case GateClass::Cz:
            cz(t0, op.targets[1].value);
            ++ops_;
            return {};
        case GateClass::Swap:
            swap(t0, op.targets[1].value);
            ++ops_;
            return {};
        default:
            break;
    ...
    Status r = op.controls.empty() ? applyGate(op.matrix, op.targets)
                                   : applyControlled(op.matrix, op.controls, op.targets);
    if (!r)
