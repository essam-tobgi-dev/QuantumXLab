...
Result<Matrix> LindbladBackend::computationalSubspace() const {
    ...
    for (std::size_t a = 0; a < dim; ++a)
        for (std::size_t b = 0; b < dim; ++b)
            out(a, b) = rho_(computationalIndex(model_.siteDims, a), computationalIndex(model_.siteDims, b));
    Complex tr = 0;
    for (std::size_t a = 0; a < dim; ++a) tr += out(a, a);
    const double leak = leakage();
    if (leak > 1e-9)
        QXL_LOG_WARN(Sim, "projecting Lindblad state onto the computational subspace discards P_leak = {:.3e}", leak);
    if (tr.real() > 1e-12)
        for (auto& v : out.data) v /= tr.real();
    return out;
}

Status LindbladBackend::setRho(const Matrix& rho) {
    if (!allocated_) return fail(err::NotAllocated, "Lindblad backend has no model");
    if (rho.rows != D_ || rho.cols != D_) return fail(err::BadTargets, "density matrix dimension mismatch");
    rho_ = rho;
    if (settings_.recordTrajectory) { // the sample at the current time must show the installed state
        if (!samples_.empty() && samples_.back().timeS == t_) samples_.pop_back();
        recordSample();
    }
    return {};
}
...
Status LindbladBackend::setPure(std::span<const Complex> psi) {
    ...
    if (psi.size() == D_) {
        for (std::size_t i = 0; i < D_; ++i)
            for (std::size_t j = 0; j < D_; ++j) rho(i, j) = psi[i] * std::conj(psi[j]);
        return setRho(rho);
    }
    if (psi.size() == (std::size_t{1} << n_)) { // computational-subspace state
        for (std::size_t a = 0; a < psi.size(); ++a)
            for (std::size_t b = 0; b < psi.size(); ++b)
                rho(computationalIndex(model_.siteDims, a), computationalIndex(model_.siteDims, b)) =
                    psi[a] * std::conj(psi[b]);
        return setRho(rho);
    }
    return fail(err::BadTargets, "state length matches neither the full nor the computational dimension");
}
...
Status LindbladBackend::reset(std::span<const QubitIndex> qubits, core::Random&) {
    ...
    for (auto q : qubits) {
        ...
        std::size_t stride = 1;
        for (std::uint32_t k = 0; k < site; ++k) stride *= model_.siteDims[k];
        const std::uint32_t d = model_.siteDims[site];
        Matrix out(D_, D_);
        auto levelOf = [&](std::size_t idx) { return static_cast<std::uint32_t>((idx / stride) % d); };
        auto withLevel = [&](std::size_t idx, std::uint32_t lv) {
            return idx + (static_cast<std::size_t>(lv) - levelOf(idx)) * stride;
        };
        for (std::size_t i = 0; i < D_; ++i)
            for (std::size_t j = 0; j < D_; ++j) {
                if (levelOf(i) != levelOf(j)) continue; // off-diagonal in the traced site is discarded
...
Status LindbladBackend::applyChannel(const Kraus&, std::span<const QubitIndex>) {
    return fail(err::Unsupported,
                "noise enters the Lindblad backend as collapse operators in the SystemModel, not as Kraus maps");
}
...
Result<Probabilities> LindbladBackend::probabilities(std::span<const QubitIndex> qubits) const {
    ...
    if (qs.empty()) for (std::uint32_t q = 0; q < n_; ++q) qs.push_back(q);
    Probabilities p(std::size_t{1} << qs.size(), 0.0);
    const std::size_t dim = std::size_t{1} << n_;
    for (std::size_t idx = 0; idx < dim; ++idx) {
        std::size_t out = 0;
        for (std::size_t k = 0; k < qs.size(); ++k) out |= ((idx >> qs[k]) & 1) << k;
    ...
    for (std::size_t i = 0; i < D_; ++i) {
        for (std::size_t j = 0; j < D_; ++j) {
            ...
            for (std::size_t k = 0; k < qs.size() && keep; ++k) {
                ...
                if (li != want || lj != want) keep = false;
            }
            if (!keep) rho_(i, j) = 0;
        }
    }
    double tr = stateNorm();
    if (tr > 1e-15) for (auto& v : rho_.data) v /= tr;
    ++ops_;
    return o;
}
...
Result<double> LindbladBackend::expectation(const PauliString& p) const {
    ...
    const std::size_t dim = std::size_t{1} << n_;
    Complex acc = 0;
    // Tr(ρP) = Σ_j ⟨j|ρP|j⟩ = Σ_j c_j ρ_{j, j⊕x} with P|j⟩ = c_j |j ⊕ x⟩, c_j carrying i^{n_Y} (T11 (2.3)).
    for (std::size_t j = 0; j < dim; ++j) acc += detail::pauliCoefficient(p, j) * (*sub)(j, j ^ p.xMask());
    return acc.real();
}
...
Result<Snapshot> LindbladBackend::snapshot(const SnapshotRequest& req) const {
    ...
    if (req.reducedStates) {
        ...
        for (auto& set : req.subsystems) {
            if (set.size() > 8) return fail(err::TooLarge, "reduced states are limited to 8 qubits");
            if (!validTargets(set, n_)) return fail(err::BadTargets, "invalid or repeated qubits in the reduced-state request");
            std::vector<std::size_t> keep, sorted;
            for (auto q : set) keep.push_back(q.get());
            sorted = keep;
            std::sort(sorted.begin(), sorted.end());
            const Matrix r = num::partialTrace(*sub, dims, sorted);
            s.reduced.push_back({set, detail::reorderSites(r, sorted, std::vector<std::size_t>(sorted.size(), 2), keep)});
        }
    }
    return s;
}
