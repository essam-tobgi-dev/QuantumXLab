...
class StateVectorBackend final : public IBackend {
  public:
    ...
    // Stochastic Kraus unravelling for one shot (spec 08 §7): pick K_k with prob ‖K_k ψ‖²,
    // renormalize.
    Status applyChannelStochastic(const Kraus& kraus, std::span<const QubitIndex> targets,
                                  core::Random& rng);

    static std::uint32_t maxQubits();

    // Kernels (public for the density-matrix backend, which reuses them column/row-wise).
    static void kernel1(std::span<Complex> psi, std::uint32_t n, std::uint32_t t,
                        const num::Mat2& u, std::uint64_t ctrlMask);
    static void kernel2(std::span<Complex> psi, std::uint32_t n, std::uint32_t t0, std::uint32_t t1,
                        const num::Mat4& u, std::uint64_t ctrlMask);
    static void kernelK(std::span<Complex> psi, std::uint32_t n,
                        std::span<const std::uint32_t> targets, const Matrix& u,
                        std::uint64_t ctrlMask);
    static void kernelDiag1(std::span<Complex> psi, std::uint32_t t, Complex d0, Complex d1,
                            std::uint64_t ctrlMask);
    ...
    static void kernelSwap(std::span<Complex> psi, std::uint32_t t0, std::uint32_t t1,
                           std::uint64_t ctrlMask);
    static void kernelCnot(std::span<Complex> psi, std::uint32_t c, std::uint32_t t,
                           std::uint64_t ctrlMask);
    static void kernelCz(std::span<Complex> psi, std::uint32_t a, std::uint32_t b,
                         std::uint64_t ctrlMask);
    static void kernelDiag2(std::span<Complex> psi, std::uint32_t t0, std::uint32_t t1,
                            const num::Mat4& u, std::uint64_t ctrlMask);

  private:
    Status applyImpl(const Matrix& u, std::span<const QubitIndex> targets, std::uint64_t ctrlMask,
                     GateClass cls);
