...
namespace qlab::qsim {
using num::Mat2; using num::Mat4;

std::uint32_t StateVectorBackend::maxQubits() { return std::min<std::uint32_t>(maxQubitsFor(sizeof(Complex), false), 34); }
...
Status StateVectorBackend::allocate(std::uint32_t n, std::uint32_t levels) {
    if (levels != 2) return fail(err::Unsupported, "state-vector backend supports 2 levels per site");
    if (n > maxQubits())
        return fail(err::TooLarge, std::format("{} qubits need {} GiB; cap on this machine is {} qubits",
                                               n, double(sizeof(Complex)) * std::ldexp(1.0, int(n)) / (1ull << 30), maxQubits()));
    n_ = n;
...
Status StateVectorBackend::setAmplitudes(std::span<const Complex> psi) {
    if (psi.size() != psi_.size()) return fail(err::BadTargets, "amplitude count mismatch");
    std::copy(psi.begin(), psi.end(), psi_.begin());
    return {};
}

double StateVectorBackend::stateNorm() const { return std::sqrt(num::norm2Squared(psi_)); }

std::unique_ptr<IBackend> StateVectorBackend::clone() const { return std::make_unique<StateVectorBackend>(*this); }

Status StateVectorBackend::applyImpl(const Matrix& u, std::span<const QubitIndex> targets, std::uint64_t ctrl, GateClass cls) {
    ...
    if (k == 0 || k > 6) return fail(err::BadTargets, "gates act on 1–6 qubits; decompose larger gates");
    if (u.rows != (std::size_t{1} << k) || u.cols != u.rows) return fail(err::BadTargets, "matrix dimension does not match target count");
#ifdef QXL_DEV
    if (!num::isUnitary(u, 1e-10)) return fail(err::NotUnitary, "gate matrix is not unitary (tol 1e-10)");
#endif
    ++ops_;
    if (k == 1) {
        ...
        case GateClass::Diagonal: kernelDiag1(psi_, t, u(0, 0), u(1, 1), ctrl); return {};
        case GateClass::PauliX: kernelX(psi_, t, ctrl); return {};
        case GateClass::PauliZ: kernelZ(psi_, t, ctrl); return {};
        default: kernel1(psi_, n_, t, Mat2::fromMatrix(u), ctrl); return {};
        }
    }
    if (k == 2) {
        const std::uint32_t t0 = targets[0].value, t1 = targets[1].value;
        switch (cls) {
        case GateClass::Cnot: kernelCnot(psi_, t0, t1, ctrl); return {};
        case GateClass::Cz: kernelCz(psi_, t0, t1, ctrl); return {};
        case GateClass::Swap: kernelSwap(psi_, t0, t1, ctrl); return {};
        case GateClass::Diagonal2: kernelDiag2(psi_, t0, t1, Mat4::fromMatrix(u), ctrl); return {};
        default: kernel2(psi_, n_, t0, t1, Mat4::fromMatrix(u), ctrl); return {};
        }
    }
    std::vector<std::uint32_t> t(k);
    for (std::size_t i = 0; i < k; ++i) t[i] = targets[i].value;
...
Status StateVectorBackend::applyControlled(const Matrix& u, std::span<const QubitIndex> controls, std::span<const QubitIndex> targets) {
    std::uint64_t mask = 0;
    for (auto c : controls) {
        if (c.value >= n_) return fail(err::BadTargets, "control qubit out of range");
...
Status StateVectorBackend::applyChannel(const Kraus&, std::span<const QubitIndex>) {
    return fail(err::Unsupported, "state-vector backend cannot apply Kraus maps exactly; use applyChannelStochastic or the density-matrix backend");
}

Status StateVectorBackend::applyChannelStochastic(const Kraus& kraus, std::span<const QubitIndex> targets, core::Random& rng) {
    ...
    for (std::size_t k = 0; k < kraus.size(); ++k) {
        ...
        if (targets.size() == 1) kernel1(psi_, n_, t[0], Mat2::fromMatrix(kraus[k]), 0);
        else if (targets.size() == 2) kernel2(psi_, n_, t[0], t[1], Mat4::fromMatrix(kraus[k]), 0);
        else kernelK(psi_, n_, t, kraus[k], 0);
        double p = num::norm2Squared(psi_);
        acc += p;
        if (r < acc || k + 1 == kraus.size()) {
            if (p <= 0) { psi_ = saved; return {}; }
...
double StateVectorBackend::probOne(std::uint32_t q) const {
    ...
    double p = 0.0;
    for (std::size_t i = 0; i < psi_.size(); ++i) if (i & m) p += std::norm(psi_[i]);
    return p;
}
...
Status StateVectorBackend::reset(std::span<const QubitIndex> qubits, core::Random& rng) {
    for (auto q : qubits) {
        ...
        if (r->bits[0]) kernelX(psi_, q.value, 0);
    }
    return {};
}
...
Result<Probabilities> StateVectorBackend::probabilities(std::span<const QubitIndex> qubits) const {
    ...
    for (std::size_t i = 0; i < psi_.size(); ++i) {
        ...
        if (w == 0.0) continue;
        std::size_t idx = 0;
        for (std::size_t k = 0; k < qubits.size(); ++k) if (i & (std::uint64_t{1} << qubits[k].value)) idx |= std::size_t{1} << k;
        p[idx] += w;
    }
    return p;
}

Result<Counts> StateVectorBackend::sample(std::span<const QubitIndex> qubits, std::uint64_t shots, core::Random& rng) const {
...
Result<double> StateVectorBackend::expectation(const PauliString& ps) const {
    if (!allocated_) return fail(err::NotAllocated, "backend not allocated");
    if (ps.size() != n_) return fail(err::BadPauli, std::format("Pauli string has {} qubits, state has {}", ps.size(), n_));
    // ⟨ψ|P|ψ⟩ with P = phase · Π: P|i⟩ = phase · (−1)^{popcount(i & z)} · i^{#Y} … handled via per-qubit factors.
    ...
    for (std::size_t i = 0; i < psi_.size(); ++i) {
        ...
        for (std::uint32_t q = 0; q < n_; ++q) {
            ...
            if (bx && bz) f *= bit ? Complex(0, -1) : Complex(0, 1); // Y|0⟩ = i|1⟩, Y|1⟩ = −i|0⟩
            else if (bz && bit) f = -f;
        }
...
Result<Matrix> StateVectorBackend::reducedDensityMatrix(std::span<const QubitIndex> keep) const {
    if (!allocated_) return fail(err::NotAllocated, "backend not allocated");
    if (keep.empty()) return fail(err::BadTargets, "a reduced state needs at least one qubit");
...
Result<Snapshot> StateVectorBackend::snapshot(const SnapshotRequest& req) const {
    if (!allocated_) return fail(err::NotAllocated, "backend not allocated");
    Snapshot s; s.kind = Kind::StateVector; s.nQubits = n_; s.gateIndex = ops_; s.cls = FidelityClass::Exact;
    if (req.amplitudes) {
        if (n_ > 24) return fail(err::TooLarge, "amplitude snapshots are refused above 24 qubits");
        s.amplitudes = std::vector<Complex>(psi_.begin(), psi_.end());
    }
    if (req.probabilities) s.probabilities = num::probabilities(psi_);
