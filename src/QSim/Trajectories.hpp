...
struct TrajectorySettings {
    ...
    double sampleS =
        1e-9; // segment grid (AWG sample spacing); averages recorded at each segment end
...
class TrajectoriesBackend final : public IBackend {
  public:
    ...
    // Run `settings().trajectories` independent trajectories from |0…0⟩ over `durationS`,
    // trajectory i drawing from rng.stream(i). The stored state is the last trajectory's; averages
    // live in samples(), whose last entry is always the end of the run.
  ...
  private:
    Status oneTrajectory(std::span<const double> segmentEnds, const std::vector<bool>& recordAt,
