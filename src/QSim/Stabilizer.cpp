...
Capabilities StabilizerBackend::capabilities() const {
    return {Kind::Stabilizer, 10000, false, true, false, true, false, false, true};
}

Status StabilizerBackend::allocate(std::uint32_t n, std::uint32_t levels) {
    if (levels != 2)
        return fail(err::Unsupported, "stabilizer backend supports qubits only");
    if (n > 10000)
        return fail(err::TooLarge, "stabilizer backend cap is 10000 qubits");
    n_ = n;
    zOff_ = (n + 63) / 64;
    W_ = 2 * zOff_;
...
std::unique_ptr<IBackend> StabilizerBackend::clone() const {
    return std::make_unique<StabilizerBackend>(*this);
}

void StabilizerBackend::setX(std::size_t row, std::uint32_t q, bool v) {
    auto& w = words_[row * W_ + (q >> 6)];
    std::uint64_t m = std::uint64_t{1} << (q & 63);
    w = v ? (w | m) : (w & ~m);
}
void StabilizerBackend::setZ(std::size_t row, std::uint32_t q, bool v) {
    auto& w = words_[row * W_ + zOff_ + (q >> 6)];
    std::uint64_t m = std::uint64_t{1} << (q & 63);
    w = v ? (w | m) : (w & ~m);
}
...
int StabilizerBackend::rowsumPhase(std::size_t h, std::size_t i) const {
    ...
    const int sum =
        2 * int(r_[h]) + 2 * int(r_[i]) + productPhase(ii, ii + zOff_, hi, hi + zOff_, zOff_);
    return ((sum % 4) + 4) % 4;
}
void StabilizerBackend::rowsum(std::size_t h, std::size_t i) {
    // Destabilizer rows may anticommute with row i (odd phase); their sign is irrelevant (T09
    // §3.2).
    r_[h] = rowsumPhase(h, i) == 2 ? 1 : 0;
    for (std::size_t w = 0; w < W_; ++w)
        words_[h * W_ + w] ^= words_[i * W_ + w];
}
void StabilizerBackend::rowsumInto(std::vector<std::uint64_t>& acc, int& phase,
                                   std::size_t i) const {
    // acc is a scratch row (x words then z words); phase holds 2·r in units of i.
    const std::uint64_t* ii = &words_[i * W_];
    const int sum = phase + 2 * int(r_[i]) +
                    productPhase(ii, ii + zOff_, acc.data(), acc.data() + zOff_, zOff_);
    phase = ((sum % 4) + 4) % 4;
    for (std::size_t w = 0; w < W_; ++w)
        acc[w] ^= ii[w];
}
...
void StabilizerBackend::sdg(std::uint32_t q) {
    s(q);
    s(q);
    s(q);
}
// Paulis only flip signs (spec 07 §4): a row anticommutes with X_q iff it has Z or Y on q, etc.
void StabilizerBackend::x(std::uint32_t q) {
    for (std::size_t i = 0; i < 2 * std::size_t(n_); ++i)
        r_[i] ^= getZ(i, q);
}
void StabilizerBackend::z(std::uint32_t q) {
    for (std::size_t i = 0; i < 2 * std::size_t(n_); ++i)
        r_[i] ^= getX(i, q);
}
void StabilizerBackend::y(std::uint32_t q) {
    for (std::size_t i = 0; i < 2 * std::size_t(n_); ++i)
        r_[i] ^= (getX(i, q) != getZ(i, q));
}
void StabilizerBackend::sx(std::uint32_t q) {
    h(q);
    s(q);
    h(q);
}
...
void StabilizerBackend::cz(std::uint32_t a, std::uint32_t b) {
    h(b);
    cnot(a, b);
    h(b);
}
void StabilizerBackend::swap(std::uint32_t a, std::uint32_t b) {
    cnot(a, b);
    cnot(b, a);
    cnot(a, b);
}
...
TableauExport StabilizerBackend::exportTableau() const {
    ...
    for (std::size_t i = 0; i < n_; ++i)
        t.destabilizers.push_back(rowStr(i));
    for (std::size_t i = 0; i < n_; ++i)
        t.stabilizers.push_back(rowStr(n_ + i));
    return t;
}

double StabilizerBackend::entanglementEntropy(std::span<const QubitIndex> sub) const {
    // S = rank_F2(stabilizer rows restricted to A) − |A| (spec 07 §8). Invalid subsystems yield
    // NaN.
    if (!allocated_ || !validTargets(sub, n_))
        return std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<std::uint8_t>> m;
    for (std::size_t i = n_; i < 2 * std::size_t(n_); ++i) {
        std::vector<std::uint8_t> row;
        for (auto q : sub) {
            row.push_back(getX(i, q.value));
            row.push_back(getZ(i, q.value));
        }
        m.push_back(row);
    }
