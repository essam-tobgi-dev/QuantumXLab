...
public:
    ...
    Status applyControlled(const Matrix& u, std::span<const QubitIndex> controls, std::span<const QubitIndex> targets) override;
    ...
    // Apply a full-dimension operator O (dim m over the given sites, little-endian) as ρ ← O ρ (left)
    // or ρ ← ρ O† (right). Public for the Lindblad backend.
    static void applyLeft(Matrix& rho, std::span<const std::uint32_t> dims, std::span<const std::uint32_t> sites, const Matrix& op, std::span<const std::size_t> ctrlSites);
    static void applyRightAdjoint(Matrix& rho, std::span<const std::uint32_t> dims, std::span<const std::uint32_t> sites, const Matrix& op, std::span<const std::size_t> ctrlSites);
    // Expand a qubit-subspace operator (2^k) to the sites' full dimensions (identity on levels ≥ 2).
    static Matrix expandToSites(const Matrix& u, std::span<const std::uint32_t> siteDims);

private:
    Status applyOp(const Matrix& u, std::span<const QubitIndex> targets, std::span<const QubitIndex> controls);
