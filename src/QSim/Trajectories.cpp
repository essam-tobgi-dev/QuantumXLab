...
Status TrajectoriesBackend::allocate(std::uint32_t nQubits, std::uint32_t levelsPerSite) {
    if (nQubits == 0 || nQubits > kMaxSites)
        return fail(err::TooLarge,
                    std::format("Trajectories backend supports 1..{} sites, {} requested",
                                kMaxSites, nQubits));
    if (levelsPerSite < 2)
    ...
    if (m.dimension() > kMaxDim)
        return fail(err::TooLarge, std::format("trajectories dimension {} exceeds the cap {}",
                                               m.dimension(), kMaxDim));
...
Status TrajectoriesBackend::setModel(SystemModel model) {
    ...
    if (model.siteDims.size() > kMaxSites)
        return fail(err::TooLarge,
                    std::format("Trajectories backend supports at most {} sites", kMaxSites));
    ...
    if (D > kMaxDim)
        return fail(err::TooLarge,
                    std::format("trajectories dimension {} exceeds the cap {}", D, kMaxDim));
    auto square = [D](const Matrix& m) { return m.rows == D && m.cols == D; };
    if (!square(model.h0))
        return fail(err::BadTargets, "H0 dimension mismatch");
    ...
    for (auto& c : model.collapse) {
        if (!square(c.op))
            return fail(err::BadTargets,
                        std::format("collapse operator '{}' dimension mismatch", c.name));
...
void TrajectoriesBackend::applyHeff(double t, std::span<const Complex> in, std::span<Complex> out) {
    // dψ/dt = −i H_eff ψ = (−iH − ½ Σ L†L) ψ, with envelopes sampled inside the current grid
    // segment (one-sided at AWG sample boundaries, as in the Lindblad backend).
    ...
    for (std::uint32_t site = 0, stride = 1; site < n_; stride *= model_.siteDims[site], ++site)
        for (std::uint32_t lv = 0; lv < model_.siteDims[site]; ++lv, ++col) {
            ...
            for (std::size_t idx = 0; idx < D_; ++idx)
                if ((idx / stride) % model_.siteDims[site] == lv)
                    p += std::norm(psi_[idx]);
            row[col] = p / norm2;
        }
}

Status TrajectoriesBackend::oneTrajectory(std::span<const double> ends,
                                          const std::vector<bool>& recordAt, core::Random& rng,
                                          std::span<double> pops) {
    const std::size_t cols =
        pops.size() /
        (1 + static_cast<std::size_t>(std::count(recordAt.begin(), recordAt.end(), true)));
    ...
    for (std::size_t s = 0; s < ends.size(); ++s) {
        ...
        const auto steps = std::max<std::uint64_t>(
            1, static_cast<std::uint64_t>(std::ceil((b - a) / settings_.stepS - 1e-9)));
        ...
        for (std::uint64_t i = 0; i < steps; ++i) {
            ...
            for (std::size_t j = 0; j < D_; ++j)
                tmp[j] = psi_[j] + 0.5 * h * k1[j];
            applyHeff(ti + 0.5 * h, tmp, k2);
            for (std::size_t j = 0; j < D_; ++j)
                tmp[j] = psi_[j] + 0.5 * h * k2[j];
            applyHeff(ti + 0.5 * h, tmp, k3);
            for (std::size_t j = 0; j < D_; ++j)
                tmp[j] = psi_[j] + h * k3[j];
            applyHeff(ti + h, tmp, k4);
            for (std::size_t j = 0; j < D_; ++j)
                psi_[j] += (h / 6.0) * (k1[j] + 2.0 * k2[j] + 2.0 * k3[j] + k4[j]);
            double norm2 = 0;
            for (auto& c : psi_)
                norm2 += std::norm(c);
            if (norm2 > threshold)
            ...
            if (total > 1e-300) {
                ...
                for (std::size_t j = 0; j < D_; ++j)
                    psi_[j] = applied[pick][j] / nrm;
            }
            threshold = rng.uniform();
        }
        t = b;
        if (recordAt[s])
            writePopulations(pops.subspan(cols * rec++, cols));
    }
...
Status TrajectoriesBackend::runEnsemble(double durationS, core::Random& rng) {
    if (!allocated_)
        return fail(err::NotAllocated, "Trajectories backend has no model");
    if (!std::isfinite(durationS) || durationS < 0)
        return fail(ErrorCode::InvalidArgument, "run duration must be finite and non-negative");
    if (!(settings_.stepS > 0) || !(settings_.sampleS > 0))
        return fail(ErrorCode::InvalidArgument,
                    "trajectory step and sample spacing must be positive");
    // Segments on the sampleS grid, the last ending exactly at durationS, which is always recorded.
    std::vector<double> ends;
    if (durationS > 0) {
        const auto whole =
            static_cast<std::uint64_t>(std::floor(durationS / settings_.sampleS + 1e-9));
        double rest = durationS - static_cast<double>(whole) * settings_.sampleS;
        if (rest <= 1e-9 * settings_.sampleS)
            rest = 0.0;
        for (std::uint64_t k = 1; k <= whole; ++k)
            ends.push_back(k == whole && rest == 0.0 ? durationS
                                                     : static_cast<double>(k) * settings_.sampleS);
        if (rest > 0.0)
            ends.push_back(durationS);
    }
    ...
    for (std::size_t s = 0; s < ends.size(); ++s)
        if (recordAt[s])
            times.push_back(ends[s]);
    std::size_t cols = 0;
    for (auto d : model_.siteDims)
        cols += d;
    ...
    for (std::size_t s = 0; s < times.size(); ++s) {
        ...
        for (std::uint32_t site = 0; site < n_; ++site)
            for (std::uint32_t lv = 0; lv < model_.siteDims[site]; ++lv, ++col)
                if (lv >= 2)
                    as.leakage += as.populations[col];
        samples_.push_back(std::move(as));
    }
    QXL_LOG_INFO(Sim, "Trajectories: {} runs over {:.3f} ns, {} samples", N, durationS * 1e9,
                 samples_.size());
    return {};
}

double TrajectoriesBackend::population(std::uint32_t site, std::uint32_t level) const {
    if (samples_.empty() || site >= n_ || level >= model_.siteDims[site])
        return 0.0;
    std::size_t col = level;
    for (std::uint32_t s = 0; s < site; ++s)
        col += model_.siteDims[s];
...
double TrajectoriesBackend::stateNorm() const {
    return std::sqrt(num::norm2Squared(psi_));
}

std::unique_ptr<IBackend> TrajectoriesBackend::clone() const {
    return std::make_unique<TrajectoriesBackend>(*this);
}
