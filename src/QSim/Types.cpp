...
std::string_view kindName(Kind k) {
    switch (k) {
    case Kind::StateVector: return "StateVector";
    case Kind::DensityMatrix: return "DensityMatrix";
    case Kind::Stabilizer: return "Stabilizer";
    case Kind::Lindblad: return "Lindblad";
    case Kind::Trajectories: return "Trajectories";
    }
    return "?";
}
...
Result<PauliString> PauliString::parse(std::string_view label) {
    ...
    if (i < label.size() && (label[i] == '+' || label[i] == '-')) {
        if (label[i] == '-') p.phase_ = -1.0;
        ++i;
    }
    if (i < label.size() && label[i] == 'i') { p.phase_ *= Complex(0, 1); ++i; }
    std::string_view body = label.substr(i);
    if (body.empty()) return fail(err::BadPauli, "empty Pauli string");
...
PauliString PauliString::fromQubits(std::size_t n, std::span<const std::pair<QubitIndex, char>> terms, Complex phase) {
    PauliString p;
    p.ops_.assign(n, 'I');
    p.phase_ = phase;
    for (auto& [q, c] : terms)
        if (q.value < n) p.setLetter(q.value, c);
    return p;
}
...
std::string PauliString::label() const {
    ...
    for (std::size_t k = 0; k < ops_.size(); ++k) s[ops_.size() - 1 - k] = ops_[k];
    return s;
}

std::string bitsToKey(std::span<const std::uint8_t> bits) {
    std::string s(bits.size(), '0');
    for (std::size_t i = 0; i < bits.size(); ++i) s[bits.size() - 1 - i] = bits[i] ? '1' : '0';
    return s;
}
...
Counts countsFromIndices(std::span<const std::size_t> idx, std::size_t nBits) {
    ...
    for (auto i : idx) {
        ...
        for (std::size_t b = 0; b < nBits; ++b) if (i & (std::size_t{1} << b)) s[nBits - 1 - b] = '1';
        ++c[s];
    }
    return c;
}
...
bool validTargets(std::span<const QubitIndex> t, std::uint32_t n) {
    for (std::size_t i = 0; i < t.size(); ++i) {
        if (t[i].value >= n) return false;
        for (std::size_t j = 0; j < i; ++j) if (t[i] == t[j]) return false;
    }
    return true;
}
