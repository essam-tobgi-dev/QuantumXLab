...
template <int T, int L, int M, int I, int Th, int N, int J, int Ang = 0, int Log = 0> struct Dim {
...
inline constexpr bool isDimensionless =
    ...
    A::ang == 0 && A::log == 0;
template <class A>
inline constexpr bool isEvenDim =
    A::t % 2 == 0 && A::l % 2 == 0 && A::m % 2 == 0 && A::i % 2 == 0 && A::th % 2 == 0 &&
    A::n % 2 == 0 && A::j % 2 == 0 && A::ang % 2 == 0 && A::log % 2 == 0;
...
template <class D> constexpr Q<D> operator+(Q<D> a, Q<D> b) {
    return Q<D>(a.v + b.v);
}
template <class D> constexpr Q<D> operator-(Q<D> a, Q<D> b) {
    return Q<D>(a.v - b.v);
}
template <class D> constexpr Q<D> operator*(Q<D> a, double s) {
    return Q<D>(a.v * s);
}
template <class D> constexpr Q<D> operator*(double s, Q<D> a) {
    return Q<D>(a.v * s);
}
template <class D> constexpr Q<D> operator/(Q<D> a, double s) {
    return Q<D>(a.v / s);
...
template <class D> constexpr Q<D> abs(Q<D> a) {
    return Q<D>(a.v < 0 ? -a.v : a.v);
}
template <class D> constexpr Q<D> min(Q<D> a, Q<D> b) {
    return a.v < b.v ? a : b;
}
template <class D> constexpr Q<D> max(Q<D> a, Q<D> b) {
    return a.v > b.v ? a : b;
}
...
constexpr AngularFrequency omega(Frequency f) {
    return AngularFrequency(2.0 * std::numbers::pi * f.v);
}
constexpr Frequency freq(AngularFrequency w) {
    return Frequency(w.v / (2.0 * std::numbers::pi));
}
// Angle helpers: rad pseudo-dimension is stripped explicitly.
constexpr double radians(Angle a) {
    return a.v;
}
constexpr Angle fromRadians(double r) {
    return Angle(r);
}
constexpr Angle fromDegrees(double d) {
    return Angle(d * std::numbers::pi / 180.0);
}
constexpr double degrees(Angle a) {
    return a.v * 180.0 / std::numbers::pi;
}

// ---------------------------------------------------------------- literals (§2)
namespace literals {
#define QXL_UDL(name, Type, scale)                                                                 \
    constexpr Type operator""_##name(long double x) {                                              \
