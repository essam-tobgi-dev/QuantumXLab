#pragma once
// Spec 05 §3 — bounded dimensionless types: Probability, Fidelity, Population in [0,1]; Infidelity
// = 1 - F.
...
struct Probability : detail::Bounded01<Probability> {
    static constexpr const char* name() { return "Probability"; }
};
struct Fidelity : detail::Bounded01<Fidelity> {
    static constexpr const char* name() { return "Fidelity"; }
};
struct Population : detail::Bounded01<Population> {
    static constexpr const char* name() { return "Population"; }
};
...
struct Infidelity {
    ...
    static Result<Infidelity> make(double r) {
        if (!(r >= -detail::kBoundEps && r <= 1.0 + detail::kBoundEps))
            return fail(ErrorCode::OutOfRange,
                        std::format("Infidelity value {} outside [0, 1]", r));
...
constexpr Infidelity infidelityOf(Fidelity f) {
    return Infidelity(1.0 - f.v);
}
