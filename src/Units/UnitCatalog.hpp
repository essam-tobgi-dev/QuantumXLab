...
public:
    ...
    std::string format(double si, DimSig d, int digits = 4, FormatContext ctx = FormatContext::Default) const;
    template <class D>
    ...
    // Parse "4.5 GHz", "-20 dB", "320ns". Bare numbers take `defaultUnit` (may be empty → Parse error).
    struct Parsed { double si; std::string symbol; DimSig dim; };
    Result<Parsed> parse(std::string_view text, std::string_view defaultUnit = {}) const;
    template <class D> Result<Q<D>> parseAs(std::string_view text, std::string_view defaultUnit = {}) const {
        auto p = parse(text, defaultUnit);
        if (!p) return std::unexpected(p.error());
        if (p->dim != DimSig::of<D>())
            return fail(ErrorCode::InvalidArgument, "unit '" + p->symbol + "' has the wrong dimension");
        return Q<D>(p->si);
    }

    // JSON quantity { "value": 4.812, "unit": "GHz" }
    core::Json toJson(double si, std::string_view symbol, int digits = 17) const;
    template <class D> core::Json toJson(Q<D> q, std::string_view symbol) const { return toJson(q.v, symbol); }
...
private:
    ...
    static constexpr std::array<std::pair<const char*, double>, 12> kPrefixes{{
        {"a", 1e-18}, {"f", 1e-15}, {"p", 1e-12}, {"n", 1e-9}, {"µ", 1e-6}, {"m", 1e-3},
        {"", 1.0}, {"k", 1e3}, {"M", 1e6}, {"G", 1e9}, {"T", 1e12}, {"P", 1e15}}};
};

// Convenience wrappers on the global catalog.
template <class D> std::string fmt(Q<D> q, int digits = 4, FormatContext ctx = FormatContext::Default) {
    return UnitCatalog::global().format(q, digits, ctx);
}
inline std::string fmt(PowerDbm p, int digits = 4) { return UnitCatalog::global().format(p, digits); }
inline std::string fmt(GainDb g, int digits = 4) { return UnitCatalog::global().format(g, digits); }
