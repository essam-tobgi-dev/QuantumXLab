...
void observabilityBadge(const DrawContext& ctx, Observability o) {
    // Physical views are the default and carry no badge; Simulator-only is always announced (spec
    // 00 §6).
    if (o == Observability::SimulatorOnly)
        badge(ctx, observabilityName(o), ctx.theme->simOnly);
}

float headerHeight(const DrawContext& ctx) {
    return ctx.showHeader ? ImGui::GetTextLineHeight() + 4.0f + ImGui::GetStyle().ItemSpacing.y
                          : 0.0f;
}

void header(DrawContext& ctx, IStateView& view, data::FidelityClass fidelity, const Rect& body) {
    if (!ctx.showHeader || !ctx.theme)
        return;
    ...
    if (view.stale()) {
        ImGui::SameLine();
        badge(ctx, "stale",
              ctx.theme->warn); // spec 21 §1: previous reduction shown until the new one arrives
    }
    ...
    if (ImGui::SmallButton("Export") && ctx.requestExport) {
        ...
        req.screenRect = body;
        if (auto csv = view.exportCsv();
            csv && ImGui::GetIO().KeyShift) { // Shift+click: CSV where tabular
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip(view.exportCsv() ? "PNG (Shift: CSV)" : "PNG");
    ImGui::SameLine();
    if (ImGui::SmallButton("?") && ctx.openTheory)
        ctx.openTheory(view.theoryAnchor());
    if (ImGui::IsItemHovered()) {
...
void readoutCard(const DrawContext& ctx, const HitResult& hit) {
    if (!ctx.theme || (hit.title.empty() && hit.readout.empty()))
        return;
    ...
    if (ImGui::BeginTooltip()) {
        if (!hit.title.empty())
            text(ctx.theme->textPrimary, hit.title);
        for (const ReadoutRow& row : hit.readout) {
            // Class dot, label, value: colour never carries the class alone — the header badge
            // names it.
...
void placeholder(const DrawContext& ctx, std::string_view message) {
    ...
    drawText(ImGui::GetWindowDrawList(),
             ImVec2(origin.x + 0.5f * std::max(0.0f, avail.x - size.x),
                    origin.y + 0.5f * std::max(0.0f, avail.y - size.y)),
             color, message);
}
...
void phaseWheel(const DrawContext& ctx, glm::vec2 center, float radius) {
    ...
    for (int k = 0; k < kSegments; ++k) {
        const double a0 = 2.0 * std::numbers::pi * k / kSegments,
                     a1 = 2.0 * std::numbers::pi * (k + 1) / kSegments;
        const glm::vec3 col = math::phaseColor(0.5 * (a0 + a1));
        const ImVec2 q[4] = {ImVec2(center.x + inner * static_cast<float>(std::cos(a0)),
                                    center.y - inner * static_cast<float>(std::sin(a0))),
                             ImVec2(center.x + radius * static_cast<float>(std::cos(a0)),
                                    center.y - radius * static_cast<float>(std::sin(a0))),
                             ImVec2(center.x + radius * static_cast<float>(std::cos(a1)),
                                    center.y - radius * static_cast<float>(std::sin(a1))),
                             ImVec2(center.x + inner * static_cast<float>(std::cos(a1)),
                                    center.y - inner * static_cast<float>(std::sin(a1)))};
    ...
    for (const math::PhaseTick& tick : math::phaseLegend(4)) {
        const float c = static_cast<float>(std::cos(tick.phi)),
                    s = static_cast<float>(std::sin(tick.phi));
        dl->AddLine(ImVec2(center.x + radius * c, center.y - radius * s),
                    ImVec2(center.x + (radius + 4.0f) * c, center.y - (radius + 4.0f) * s),
                    toU32(ctx.theme->textSecondary));
        const ImVec2 sz = textSize(tick.label);
        drawText(dl,
                 ImVec2(center.x + (radius + 8.0f + 0.5f * sz.x) * c - 0.5f * sz.x,
                        center.y - (radius + 8.0f + 0.5f * sz.y) * s - 0.5f * sz.y),
                 ctx.theme->textSecondary, tick.label);
    }
}

void colorBar(const DrawContext& ctx, const Rect& r,
              const std::function<glm::vec3(double)>& colorAt, std::string_view minLabel,
              std::string_view maxLabel, std::string_view title) {
    if (!ctx.theme || !colorAt)
        return;
    ImDrawList* dl = ImGui::GetWindowDrawList();
    ...
    for (int k = 0; k < kSteps; ++k) {
        ...
        const float ya = static_cast<float>(r.y1 - t0 * r.height()),
                    yb = static_cast<float>(r.y1 - t1 * r.height());
        dl->AddRectFilled(ImVec2(x0, yb), ImVec2(x1, ya), toU32(colorAt(0.5 * (t0 + t1))));
    }
    dl->AddRect(ImVec2(x0, static_cast<float>(r.y0)), ImVec2(x1, static_cast<float>(r.y1)),
                toU32(ctx.theme->border));
    const float lh = ImGui::GetTextLineHeight();
    drawText(dl, ImVec2(x1 + 4.0f, static_cast<float>(r.y0) - 0.5f * lh), ctx.theme->textSecondary,
             maxLabel);
    drawText(dl, ImVec2(x1 + 4.0f, static_cast<float>(r.y1) - 0.5f * lh), ctx.theme->textSecondary,
             minLabel);
