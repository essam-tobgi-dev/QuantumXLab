...
Status checkState(std::span<const Complex> psi, std::uint32_t n) {
    if (n == 0 || n > 40)
        return fail(ErrorCode::InvalidArgument, "reduced state: qubit count out of range");
    if (psi.size() != (std::size_t{1} << n))
        return fail(ErrorCode::InvalidArgument, "reduced state: the state has " +
    ...
    for (QubitIndex q : keep) {
        if (q.get() >= n)
            return fail(ErrorCode::OutOfRange,
                        "reduced state: qubit " + std::to_string(q.get()) + " out of range");
        if (std::find(out.begin(), out.end(), std::size_t{q.get()}) != out.end())
            return fail(ErrorCode::InvalidArgument,
                        "reduced state: qubit " + std::to_string(q.get()) + " listed twice");
        out.push_back(q.get());
    }
    if (out.empty())
        return fail(ErrorCode::InvalidArgument, "reduced state: empty qubit subset");
    return out;
}
...
Result<std::uint32_t> qubitCountOf(std::size_t amplitudes) {
    if (amplitudes < 2 || !std::has_single_bit(amplitudes))
        return fail(ErrorCode::InvalidArgument,
                    "state length " + std::to_string(amplitudes) + " is not a power of two");
...
Result<Matrix> reducedSingle(std::span<const Complex> psi, std::uint32_t n, std::uint32_t k) {
    ...
    if (k >= n)
        return fail(ErrorCode::OutOfRange,
                    "reduced state: qubit " + std::to_string(k) + " out of range");
...
Result<Matrix> reducedPair(std::span<const Complex> psi, std::uint32_t n, std::uint32_t i,
                           std::uint32_t j) {
    QXL_TRY(checkState(psi, n));
    if (i >= n || j >= n)
        return fail(ErrorCode::OutOfRange, "reduced pair: qubit out of range");
    if (i == j)
        return fail(ErrorCode::InvalidArgument, "reduced pair: the two qubits must differ");
    ...
    for (std::size_t t = 0; t < quarter; ++t) {
        ...
        const std::array<Complex, 4> a{psi[base], psi[base | off[1]], psi[base | off[2]],
                                       psi[base | off[3]]};
        for (std::size_t r = 0; r < 4; ++r)
            for (std::size_t c = r; c < 4; ++c)
                acc[r * 4 + c] += a[r] * std::conj(a[c]);
    }
    Matrix rho(4, 4);
    for (std::size_t r = 0; r < 4; ++r)
        for (std::size_t c = r; c < 4; ++c) {
            rho(r, c) = acc[r * 4 + c];
            if (c != r)
                rho(c, r) = std::conj(acc[r * 4 + c]);
        }
    return rho;
}

Result<Matrix> reducedSubset(std::span<const Complex> psi, std::uint32_t n,
                             std::span<const QubitIndex> keep) {
    QXL_TRY(checkState(psi, n));
    QXL_TRY_ASSIGN(auto sites, checkedSites(keep, n));
    if (sites.size() == 1)
        return reducedSingle(psi, n, static_cast<std::uint32_t>(sites[0]));
    if (sites.size() == 2)
        return reducedPair(psi, n, static_cast<std::uint32_t>(sites[0]),
                           static_cast<std::uint32_t>(sites[1]));
    if (sites.size() > 14)
        return fail(ErrorCode::OutOfRange, "reduced state: at most 14 qubits may be kept");
...
Result<Matrix> reducedFromDensity(const Matrix& rho, std::uint32_t nSites, std::uint32_t levels,
                                  std::span<const QubitIndex> keep) {
    if (levels < 2 || nSites == 0)
        return fail(ErrorCode::InvalidArgument, "reduced state: bad site description");
    const std::size_t dim = num::ipow(levels, nSites);
    if (rho.rows != dim || rho.cols != dim)
        return fail(ErrorCode::InvalidArgument,
                    "reduced state: density matrix is " + std::to_string(rho.rows) + "x" +
...
Result<Matrix> computationalBlock(const Matrix& rho, std::uint32_t nSites, std::uint32_t levels) {
    if (levels < 2 || nSites == 0)
        return fail(ErrorCode::InvalidArgument, "computational block: bad site description");
    const std::size_t dim = num::ipow(levels, nSites);
    if (rho.rows != dim || rho.cols != dim)
        return fail(ErrorCode::InvalidArgument, "computational block: dimension mismatch");
    if (levels == 2)
        return rho;
    const std::size_t qdim = std::size_t{1} << nSites;
    std::vector<std::size_t> map(
        qdim); // qubit basis index → mixed-radix index with every digit ∈ {0, 1}
    ...
    for (std::size_t a = 0; a < qdim; ++a)
        for (std::size_t b = 0; b < qdim; ++b)
            out(a, b) = rho(map[a], map[b]);
    return out;
}

double BlochVector::norm() const {
...
double purity(const Matrix& rho) {
    return qsim::measures::purity(rho);
}
...
Result<PairMeasures> pairMeasures(const Matrix& rhoIJ) {
    if (rhoIJ.rows != 4 || rhoIJ.cols != 4)
        return fail(ErrorCode::InvalidArgument, "pair measures need a 4x4 density matrix");
...
Result<SchmidtSpectrum> schmidtSpectrum(std::span<const Complex> psi, std::uint32_t n,
    ...
    QXL_TRY(checkState(psi, n));
    if (partition.size() > 14)
        return fail(ErrorCode::OutOfRange,
                    "Schmidt spectrum: |A| must be at most 14 (spec 21 §3.9)");
