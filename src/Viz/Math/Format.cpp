// Spec 21 §4 — readout text (see Format.hpp). Unit prefixes come from the units catalog (spec 05
// §4).
...
std::string formatTime(double seconds, int sig) {
    return units::fmt(units::Time{seconds}, sig);
}
std::string formatFrequency(double hertz, int sig) {
    return units::fmt(units::Frequency{hertz}, sig);
}
...
std::string bitString(std::uint64_t index, std::size_t nbits) {
    ...
    for (std::size_t k = 0; k < nbits && k < 64; ++k)
        if ((index >> k) & 1u)
            s[nbits - 1 - k] = '1'; // qubit k is character n−1−k
    return s;
}
...
std::string braLabel(std::uint64_t index, std::size_t nbits, bool decimal, bool ascii) {
    return (ascii ? std::string("<") : std::string("\xE2\x9F\xA8")) + body(index, nbits, decimal) +
           "|"; // U+27E8
}
