...
double wrapPhase(double phi) {
    ...
    if (w > kPi)
        ...
        w += kTwoPi; // −π itself folds to +π: the range is (−π, π]
    return w;
}
...
double phaseHue(double phi) {
    return (wrapPhase(phi) + kPi) / kTwoPi;
}

std::span<const glm::vec3> phaseLut() {
    static const std::vector<glm::vec3> lut =
        gfx::Colormap::get(gfx::ColormapId::Twilight).lut(kPhaseLutEntries);
    return lut;
}
...
std::string formatAngle(double radians) {
    if (!std::isfinite(radians))
        return formatSig(radians);
    if (std::abs(radians) < 1e-9)
        return "0";
    ...
    for (int q = 1; q <= 16; ++q) {
        ...
        if (p == 0.0 || std::abs(p) > 1e6)
            continue;
        if (std::abs(radians - p * kPi / static_cast<double>(q)) >= 1e-9)
            continue;
        const long long ip = static_cast<long long>(p);
        std::string s = ip < 0 ? "-" : "";
        if (std::llabs(ip) != 1)
            s += std::to_string(std::llabs(ip));
        s += "\xCF\x80"; // π
        if (q != 1)
            s += "/" + std::to_string(q);
        return s;
