...
double acesForward(double x) { return std::clamp(x * (kA * x + kB) / (x * (kC * x + kD) + kE), 0.0, 1.0); }
...
double srgbChannelToLinear(double c) { return c <= 0.04045 ? c / 12.92 : std::pow((c + 0.055) / 1.055, 2.4); }

} // namespace

std::optional<glm::vec4> colorFromHex(std::string_view hex) {
    if (hex.empty() || hex.front() != '#' || (hex.size() != 7 && hex.size() != 9)) return std::nullopt;
...
glm::vec3 sceneToDisplay(glm::vec3 scene, float exposure) {
    ...
    for (glm::length_t k = 0; k < 3; ++k)
        out[k] = static_cast<float>(std::pow(acesForward(static_cast<double>(scene[k]) * exposure), 1.0 / 2.2));
    return out;
}
...
double relativeLuminance(glm::vec3 c) {
    return 0.2126 * srgbChannelToLinear(c.r) + 0.7152 * srgbChannelToLinear(c.g) + 0.0722 * srgbChannelToLinear(c.b);
}
...
glm::vec3 sequentialColor(double t) {
    return gfx::Colormap::get(gfx::ColormapId::Viridis).sample(static_cast<float>(std::clamp(t, 0.0, 1.0)));
}

glm::vec3 divergingColor(double s) {
    return gfx::Colormap::get(gfx::ColormapId::Coolwarm).sample(static_cast<float>(0.5 + 0.5 * std::clamp(s, -1.0, 1.0)));
}
