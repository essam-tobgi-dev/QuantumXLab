...
std::uint64_t binomial(std::uint32_t n, std::uint32_t k) {
    ...
    k = std::min(k, n - k);
    std::uint64_t c = 1;
    for (std::uint32_t i = 1; i <= k; ++i) c = c * (n - k + i) / i; // exact: c·(n−k+i) is divisible by i
    return c;
}
...
std::uint64_t rankAmongEqualWeight(std::uint64_t index) {
    ...
    for (std::uint32_t pos = 0; pos < 64; ++pos)
        if ((index >> pos) & 1u) rank += binomial(pos, ++k);
    return rank;
}
...
QSpherePlacement qspherePlacement(std::uint64_t index, std::uint32_t n) {
    ...
    p.longitude = p.count > 0 ? 2.0 * std::numbers::pi * static_cast<double>(p.rank) / static_cast<double>(p.count) : 0.0;
...
QSphereModel buildQSphere(const AmplitudeSelection& selection, const QSphereOptions& options) {
    ...
    std::sort(m.nodes.begin(), m.nodes.end(),
              [](const QSphereNode& a, const QSphereNode& b) { return a.state.index < b.state.index; });
    return m;
}
...
QSphereModel buildQSphere(std::span<const Complex> psi, const QSphereOptions& options) {
    ...
    const std::uint32_t n = psi.empty() ? 0u : static_cast<std::uint32_t>(std::bit_width(psi.size()) - 1);
