...
Status checkMode(const num::Matrix& rho) {
    if (rho.rows == 0 || rho.rows != rho.cols) return fail(ErrorCode::InvalidArgument, "Wigner: the mode density matrix must be square");
    if (rho.rows > kWignerMaxFock + 1)
        return fail(ErrorCode::OutOfRange, "Wigner: Fock truncation above N_max = " + std::to_string(kWignerMaxFock));
    return {};
}
...
std::vector<std::size_t> activeDiagonals(const num::Matrix& rho) {
    ...
    for (std::size_t k = 0; k < rho.rows; ++k) {
        double w = 0.0;
        for (std::size_t n = 0; n + k < rho.rows; ++n) w = std::max(w, std::abs(rho(n + k, n)));
        if (k == 0 || w > 1e-15) ks.push_back(k);
    }
    return ks;
}

// W at α = r e^{iθ} with x = 4r². `scratch` holds l_n^{(k)}(x) for the current k.
double evaluate(const num::Matrix& rho, std::span<const std::size_t> diagonals, double r, double theta,
    ...
    for (std::size_t k : diagonals) {
        ...
        double l0;
        if (x <= 0.0) l0 = k == 0 ? 1.0 : 0.0;
        else l0 = std::exp(0.5 * kd * std::log(x) - 0.5 * std::lgamma(kd + 1.0) - 0.5 * x);
        scratch[0] = l0;
        for (std::size_t n = 0; n + 1 < count; ++n) {
            const double nd = static_cast<double>(n);
            const double prev = n == 0 ? 0.0 : scratch[n - 1];
            scratch[n + 1] = ((2.0 * nd + 1.0 + kd - x) * scratch[n] - std::sqrt(nd * (nd + kd)) * prev) /
...
double WignerGrid::x(std::size_t ix) const {
    return axes.nx < 2 ? axes.xMin : axes.xMin + (axes.xMax - axes.xMin) * static_cast<double>(ix) / static_cast<double>(axes.nx - 1);
}
double WignerGrid::p(std::size_t iy) const {
    return axes.ny < 2 ? axes.pMin : axes.pMin + (axes.pMax - axes.pMin) * static_cast<double>(iy) / static_cast<double>(axes.ny - 1);
}
double WignerGrid::symmetricLimit() const { return std::max({std::abs(minValue), std::abs(maxValue), 1e-12}); }
...
Result<WignerGrid> wignerGrid(const num::Matrix& rho, const WignerOptions& o) {
    ...
    if (!(o.xMax > o.xMin) || !(o.pMax > o.pMin)) return fail(ErrorCode::InvalidArgument, "Wigner: empty phase-space range");
    WignerGrid g;
    g.axes = o;
    ...
    const double cell = 0.5 * (o.xMax - o.xMin) / static_cast<double>(o.nx - 1) * (o.pMax - o.pMin) / static_cast<double>(o.ny - 1);
...
std::vector<double> photonNumberDistribution(const num::Matrix& rho) {
    ...
    for (std::size_t n = 0; n < p.size(); ++n) p[n] = rho(n, n).real();
    return p;
}
