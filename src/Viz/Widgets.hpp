...
// Small rounded label: the text in `color` on a faint fill of the same hue. Same line as what
// follows.
...
void header(DrawContext& ctx, IStateView& view, data::FidelityClass fidelity,
            const Rect& bodyScreenRect);
...
void colorBar(const DrawContext& ctx, const Rect& screenRect,
