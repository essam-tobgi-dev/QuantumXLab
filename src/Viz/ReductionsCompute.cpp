...
struct Source {
    ...
    Result<Matrix> reduced(std::span<const QubitIndex> keep) const {
        if (pure()) return math::reducedSubset(psi, n, keep);
        if (mixed()) return math::reducedFromDensity(*rho, n, levels, keep);
        for (const auto& r : snap->reduced) // what the backend attached (tableau-only snapshots)
            if (r.qubits.size() == keep.size() && std::equal(r.qubits.begin(), r.qubits.end(), keep.begin())) return r.rho;
...
Result<PairReduction> reducePair(const Source& src, QubitIndex i, QubitIndex j) {
    ...
    const auto siteLevels = static_cast<std::uint32_t>(std::lround(std::sqrt(static_cast<double>(rho.rows))));
...
Status checkQubits(std::span<const QubitIndex> qs, std::uint32_t n, const char* what) {
    for (QubitIndex q : qs)
        if (q.get() >= n)
            return fail(ErrorCode::OutOfRange, std::string(what) + ": qubit " + std::to_string(q.get()) + " is outside the " +
...
Result<Reductions> computeReductions(const qsim::Snapshot& snap, const ReductionRequest& req, std::stop_token stop) {
    Reductions out;
    ...
    if (snap.amplitudes && !snap.amplitudes->empty()) src.psi = *snap.amplitudes;
    else if (snap.densityMatrix && !snap.densityMatrix->empty()) src.rho = &*snap.densityMatrix;
    ...
    if (qubits.empty())
        for (std::uint32_t q = 0; q < src.n; ++q) qubits.emplace_back(q);
    std::sort(qubits.begin(), qubits.end());
    qubits.erase(std::unique(qubits.begin(), qubits.end()), qubits.end());

    if (req.singles)
        for (QubitIndex q : qubits) {
            if (cancelled()) return fail(ErrorCode::Cancelled, "reductions cancelled");
            auto s = reduceSingle(src, q);
            if (s) out.singles.push_back(std::move(*s));
            else out.notes.push_back("rho of q" + std::to_string(q.get()) + ": " + s.error().message);
        }
        ...
        if (qubits.size() > kPairReductionMaxQubits) {
            ...
            for (std::size_t a = 0; a < qubits.size(); ++a)
                for (std::size_t b = a + 1; b < qubits.size(); ++b) {
                    ...
                    if (p) out.pairs.push_back(*p);
                    else out.notes.push_back("pair (" + std::to_string(qubits[a].get()) + "," + std::to_string(qubits[b].get()) +
                                             "): " + p.error().message);
                }
        }
    }
    ...
    for (const auto& subset : req.subsets) {
        ...
        if (rho) rho = math::computationalBlock(*rho, static_cast<std::uint32_t>(subset.size()),
                                                src.pure() ? 2u : src.levels);
        if (rho) out.subsets.push_back({subset, std::move(*rho)});
        else out.notes.push_back("reduced state: " + rho.error().message);
    }

    if (req.schmidtPartition) {
        QXL_TRY(checkQubits(*req.schmidtPartition, src.n, "Schmidt partition"));
        if (!src.pure()) out.notes.push_back("the Schmidt spectrum needs a pure state (state-vector snapshot)");
        else if (auto s = math::schmidtSpectrum(src.psi, src.n, *req.schmidtPartition)) out.schmidt = std::move(*s);
        else out.notes.push_back("Schmidt spectrum: " + s.error().message);
    }

    if (req.singleQubitPaulis || !req.pauliStrings.empty()) {
        std::optional<Matrix> qubitRho; // computational block of a multi-level density matrix, built once
        if (!src.pure() && src.mixed()) {
            auto block = math::computationalBlock(*src.rho, src.n, src.levels);
            if (block) qubitRho = std::move(*block);
        }
        const auto expect = [&](const qsim::PauliString& p) -> Result<double> {
            if (src.pure()) return math::pauliExpectation(src.psi, p);
            if (qubitRho) return math::pauliExpectation(*qubitRho, p);
            return fail(ErrorCode::NotFound, "the snapshot carries no state");
        };
        const auto add = [&](const qsim::PauliString& p, bool user) {
            auto v = expect(p);
            if (v) out.paulis.push_back({math::pauliRowLabel(p), p.label(), *v, user});
            else out.notes.push_back("<" + p.label() + ">: " + v.error().message);
        };
        ...
        for (const auto& text : req.pauliStrings) {
            ...
            if (p) add(*p, true);
            else out.notes.push_back(p.error().message);
        }
    }
    ...
    if (req.wigner) {
        if (!req.modeState) out.notes.push_back("Wigner function: no oscillator mode state was supplied");
        else if (auto g = math::wignerGrid(*req.modeState, *req.wigner)) out.wigner = std::move(*g);
        else out.notes.push_back("Wigner function: " + g.error().message);
    }
    return out;
}
