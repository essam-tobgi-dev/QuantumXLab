...
struct Pen {
    ...
    glm::dvec3 at(double x, double y, double z = 0.0) const { return {x - center.x, center.y - y, z}; }
    void line(glm::dvec2 a, glm::dvec2 b, const glm::vec4& c, float widthPx, float dashPx = 0.0f) const {
    ...
    void fill(const Rect& b, const glm::vec4& c) const {
        if (b.height() <= 0.0 || b.width() <= 0.0) return;
    ...
    // Angles are layout angles: the layout's y grows down the page, so t ∈ (π, 2π) is the upper half.
    void arc(glm::dvec2 p, double radius, double a0, double a1, const glm::vec4& c, float widthPx, int seg) const {
        for (int k = 0; k < seg; ++k) {
    ...
    void label(glm::dvec2 p, std::string_view s, float sizePx, const glm::vec4& c,
        ...
        if (sizePx >= 6.0f && !s.empty()) r->text().label3D(at(p.x, p.y), s, sizePx, c, a);
    }
};
...
void boxLabel(const Pen& pen, const Rect& box, const Glyph& g, const glm::vec4& text, float fontPx) {
    const std::string full = g.params.empty() ? g.label : g.label + "(" + g.params + ")";
    if (box.width() < 0.34) return pen.label({box.cx(), box.y0 - 0.05}, full, 0.8f * fontPx, text,
                                             gfx::TextAnchor::BottomCenter);
    if (g.params.empty() || box.height() < 0.55) return pen.label({box.cx(), box.cy()}, g.label, fontPx, text);
...
void drawGlyph(const Pen& pen, const VizTheme& theme, const layout::CircuitLayout& lay, const Glyph& g, float alpha,
               bool outline, float fontPx) {
    ...
    const auto span = [&] {
        if (g.rowMax > g.rowMin && !lay.timed) pen.line({cx, rowY(g.rowMin)}, {cx, rowY(g.rowMax)}, mark, 1.6f);
    };
    const auto controls = [&] {
    ...
    // than a box (CX, CZ, SWAP) would otherwise hide its duration — and a line joins the wires, so a
    // concurrent gate between them stays visible (spec 21 §3.13).
    if (lay.timed && g.kind != GlyphKind::Region) {
        ...
        for (std::size_t k = 1; k < g.parts.size(); ++k)
            pen.line({g.parts[k - 1].cx(), g.parts[k - 1].cy()}, {g.parts[k].cx(), g.parts[k].cy()}, mark, 1.4f);
    }
    ...
    case GlyphKind::Cx:
        ...
        if (outline) pen.frame(g.bounds.inset(-0.06), hilite, 2.0f);
        return;
    case GlyphKind::Cz:
        span();
        controls();
        for (std::uint32_t t : g.targetRows) pen.dot({cx, rowY(t)}, kControlDot, mark);
        if (outline) pen.frame(g.bounds.inset(-0.06), hilite, 2.0f);
        return;
    case GlyphKind::Swap:
        span();
        ...
        if (g.routingSwap) pen.label({cx, rowY(g.rowMin) - 0.4}, "routed", 0.72f * fontPx, ink(theme.warn, alpha));
        if (outline) pen.frame(g.bounds.inset(-0.06), hilite, 2.0f);
        return;
    case GlyphKind::Region: // bracketed block with its branch / loop header
        ...
        pen.label({g.bounds.x0 + 0.06, g.bounds.y0 - 0.05}, g.label, 0.9f * fontPx, ink(theme.accent, alpha),
                  gfx::TextAnchor::BottomLeft);
        if (g.elseColumn && *g.elseColumn < lay.columnX.size()) {
            const double x = lay.columnX[*g.elseColumn] - 0.15;
            pen.line({x, g.bounds.y0}, {x, g.bounds.y1}, ink(theme.accent, 0.55f * alpha), 1.4f, 5.0f);
    ...
    if (g.kind == GlyphKind::Delay) // hatched, so an idle window never reads as a gate
        for (double x = box.x0 + 0.08; x < box.x1; x += 0.11)
            pen.line({x, box.y1}, {std::min(x + 0.16, box.x1), box.y0}, ink(theme.textDisabled, alpha), 1.0f);
    if (g.kind == GlyphKind::Measure) {
        const glm::dvec2 m{box.cx(), box.cy() + 0.12}; // meter face: an arc with its needle (T01 §7)
        ...
        if (g.classicalRow) { // double line down to the register, with the bit name beside it
            ...
            pen.line({cx + 0.1, y1 - 0.1}, {cx, y1}, faint, 1.3f);
            pen.label({cx + 0.14, y1 - 0.1}, g.params, 0.8f * fontPx, faint, gfx::TextAnchor::BottomLeft);
        }
        return;
    }
    ...
    return [this, &theme, pxScale](gfx::Renderer& r, GlBackend&) {
        ...
        for (std::uint32_t row = 0; row < layout_.rows.size(); ++row) {
            ...
            pen.label({-0.5 * detail::kLabelGutter, y}, detail::wireLabelText(w), fontPx,
                      GlBackend::exact(w.classical ? theme.textSecondary : theme.textPrimary), gfx::TextAnchor::Center);
        }
        ...
        if (layout_.timed && layout_.nsPerUnit > 0.0) {
            ...
            for (double x = 0.0; x + 0.35 <= layout_.width; x += step) {
                ...
                pen.label({x + 0.35, y + 0.2}, math::formatSig(x * layout_.nsPerUnit, 3) + " ns", 0.8f * fontPx,
                          GlBackend::exact(theme.textSecondary), gfx::TextAnchor::TopCenter);
            }
        }

        // Regions first (they sit behind their bodies), then every other glyph in layout order.
        for (const Glyph& g : layout_.glyphs)
            if (g.kind == GlyphKind::Region) drawGlyph(pen, theme, layout_, g, alphaOf(g), false, fontPx);
        for (const Glyph& g : layout_.glyphs) {
            if (g.kind == GlyphKind::Region) continue;
            ...
            if (current)
                pen.line({g.bounds.cx(), 0.0}, {g.bounds.cx(), layout_.height},
                         GlBackend::exact(math::mixColor(glm::vec3(theme.bgPanel), glm::vec3(theme.accent), 0.5f), 1.0f),
                         1.8f, 8.0f);
