...
void WignerView::rebuild(const ViewInput& in) {
    ...
    if (in.reductions && in.reductions->wigner) {
        ...
        note_ = "Bind an oscillator mode (resonator, cavity or motional mode) to see its Wigner function";
    }
    if (mode_) photons_ = math::photonNumberDistribution(*mode_);
    if (!grid_) return;
    ...
    for (std::size_t iy = 0; iy < g.axes.ny; ++iy)
        for (std::size_t ix = 0; ix < g.axes.nx; ++ix) heat_[(g.axes.ny - 1 - iy) * g.axes.nx + ix] = g.at(ix, iy);
}

std::optional<HitResult> WignerView::hitTest(glm::vec2 local) const {
    if (!grid_ || plot_.width() <= 0.0 || plot_.height() <= 0.0) return std::nullopt;
    if (!plot_.contains(local.x, local.y)) return std::nullopt;
    const math::WignerGrid& g = *grid_;
    const double u = (local.x - plot_.x0) / plot_.width(), v = (local.y - plot_.y0) / plot_.height();
    const auto ix = std::min<std::size_t>(g.axes.nx - 1, static_cast<std::size_t>(u * static_cast<double>(g.axes.nx)));
    const auto iyTop = std::min<std::size_t>(g.axes.ny - 1, static_cast<std::size_t>(v * static_cast<double>(g.axes.ny)));
    const std::size_t iy = g.axes.ny - 1 - iyTop; // screen y grows downward, p grows upward
    HitResult h;
    h.kind = HitKind::MatrixElement;
    h.element = std::pair<std::uint32_t, std::uint32_t>{static_cast<std::uint32_t>(ix), static_cast<std::uint32_t>(iy)};
    ...
    h.readout.push_back({"α", math::formatCartesian(num::Complex((g.x(ix)) / std::sqrt(2.0), g.p(iy) / std::sqrt(2.0))), cls});
    return h;
}
...
std::string WignerView::statusLine() const {
    ...
    if (mode_) s += "N_max = " + std::to_string(mode_->rows > 0 ? mode_->rows - 1 : 0) + "   ";
    // Spec 21 §3.17: the negativity volume is the headline number of this view.
    return s + "negativity = " + math::formatSig(grid_->negativity) + "   ∫W = " + math::formatSig(grid_->integral);
}

void WignerView::drawBody(DrawContext& ctx) {
    if (!grid_ || heat_.empty()) return widgets::placeholder(ctx, note_.empty() ? "No Wigner grid" : note_);
    ...
    const float photonW = body.x > 420.0f ? 0.3f * body.x : 0.0f; // P(n) beside the map when there is room
    const float mapW = std::max(80.0f, body.x - barW - photonW);

    if (ImPlot::BeginPlot("##wigner", ImVec2(mapW, body.y), ImPlotFlags_NoLegend | ImPlotFlags_NoTitle)) {
        ...
        ImPlot::PlotHeatmap("W", heat_.data(), static_cast<int>(g.axes.ny), static_cast<int>(g.axes.nx), -limit, limit,
                            nullptr, ImPlotPoint(g.axes.xMin, g.axes.pMin), ImPlotPoint(g.axes.xMax, g.axes.pMax));
    ...
    // Spec 22 §4 / 21 §4: the diverging scale is always shown with its range, W = 0 at the neutral colour.
    ImGui::SameLine();
    ImPlot::ColormapScale("W", -limit, limit, ImVec2(barW - 10.0f, body.y), "%.3f");
    if (photonW > 0.0f && !photons_.empty()) {
        ImGui::SameLine();
        if (ImPlot::BeginPlot("##photons", ImVec2(photonW - 8.0f, body.y), ImPlotFlags_NoLegend | ImPlotFlags_NoTitle)) {
