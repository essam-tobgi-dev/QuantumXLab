...
std::optional<HitResult> ReducedView::hitTest(glm::vec2 local) const {
    for (const Card& c : cards_) {
        if (!c.rect.contains(local.x, local.y))
            continue;
        HitResult h;
        ...
        if (c.valid) {
            ...
            if (c.leakage > 1e-9)
                h.readout.push_back({"leakage", math::formatSig(c.leakage), cls});
        }
        if (c.t1S)
            h.readout.push_back({"T₁", math::formatTime(*c.t1S), data::FidelityClass::Model});
        if (c.t2S)
            h.readout.push_back({"T₂", math::formatTime(*c.t2S), data::FidelityClass::Model});
        if (c.f01Hz)
            h.readout.push_back(
                {"f₀₁", math::formatFrequency(*c.f01Hz), data::FidelityClass::Model});
        if (c.readoutError)
            h.readout.push_back({"readout error", math::formatProbability(*c.readoutError),
...
std::optional<std::string> ReducedView::exportCsv() const {
    if (cards_.empty())
        return std::nullopt;
    std::string csv =
        "qubit,rx,ry,rz,purity,entropy_bits,p1,leakage,t1_s,t2_s,f01_hz,readout_error\n";
    for (const Card& c : cards_) {
        csv += std::to_string(c.qubit.get()) + "," + math::formatSig(c.r.x, 12) + "," +
               math::formatSig(c.r.y, 12) + "," + math::formatSig(c.r.z, 12) + "," +
               math::formatSig(c.purity, 12) + "," + math::formatSig(c.entropyBits, 12) + "," +
               math::formatSig(c.r.p1(), 12) + "," + math::formatSig(c.leakage, 12) + ",";
...
void ReducedView::drawBody(DrawContext& ctx) {
    if (cards_.empty())
        return widgets::placeholder(ctx, note_.empty() ? "No qubit selected" : note_);
    ...
    for (const Card& c : cards_) {
        ...
        const ImVec2 p0(origin.x + static_cast<float>(c.rect.x0) + 3.0f,
                        origin.y + static_cast<float>(c.rect.y0) + 3.0f);
        const ImVec2 p1(origin.x + static_cast<float>(c.rect.x1) - 3.0f,
                        origin.y + static_cast<float>(c.rect.y1) - 3.0f);
        dl->AddRectFilled(p0, p1, toU32(theme.bgRaised), theme.radiusMd);
        dl->AddRect(p0, p1, toU32(selected ? theme.accent : theme.border), theme.radiusMd, 0,
                    selected ? 2.0f : 1.0f);
        float y = p0.y + kCardPadding;
        drawText(dl, ImVec2(p0.x + kCardPadding, y), theme.qubitColor(c.qubit.get()),
                 "q" + std::to_string(c.qubit.get()));
        y += lh + 2.0f;
        if (!c.valid) {
        ...
        for (std::size_t i = 0; i < 2 && c.rho.rows >= 2; ++i) {
            ...
            for (std::size_t j = 0; j < 2; ++j)
                line += (j ? "   " : "") + math::formatCartesian(c.rho(i, j), 3);
        ...
        dl->AddLine(ImVec2(mx - 5.0f, bracketTop), ImVec2(mx - 5.0f, bracketBot),
                    toU32(theme.textSecondary));
        y = bracketBot + 4.0f;
        ...
        line("r",
             "(" + math::formatSig(c.r.x, 3) + ", " + math::formatSig(c.r.y, 3) + ", " +
                 math::formatSig(c.r.z, 3) + ")",
             theme.textPrimary);
        ...
        if (c.t1S)
            line("T₁", math::formatTime(*c.t1S), theme.textPrimary);
        if (c.t2S)
            line("T₂", math::formatTime(*c.t2S), theme.textPrimary);
        if (c.f01Hz)
            line("f₀₁", math::formatFrequency(*c.f01Hz), theme.textPrimary);
        if (c.readoutError)
            line("readout err", math::formatProbability(*c.readoutError), theme.textPrimary);
    }
    ImGui::Dummy(ImVec2(bodySize().x,
                        cardH_ * static_cast<float>((cards_.size() + columns_ - 1) / columns_)));
}
