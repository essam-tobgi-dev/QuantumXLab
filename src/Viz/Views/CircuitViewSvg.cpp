...
struct Svg {
    ...
    void line(double x0, double y0, double x1, double y1, const char* stroke, double w, const char* dash = nullptr) {
        out += std::format(R"(<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}")", num(x(x0)),
                           num(y(y0)), num(x(x1)), num(y(y1)), stroke, num(w));
        if (dash) out += std::format(R"( stroke-dasharray="{}")", dash);
        out += "/>\n";
    }
    void rect(const Rect& r, const char* fill, const char* stroke, double w) {
        out += std::format(R"(<rect x="{}" y="{}" width="{}" height="{}" rx="3" fill="{}" stroke="{}" stroke-width="{}"/>)"
                           "\n",
                           num(x(r.x0)), num(y(r.y0)), num(r.width() * kPx), num(r.height() * kPx), fill, stroke, num(w));
    }
    void circle(double cxL, double cyL, double rL, const char* fill, const char* stroke, double w) {
        out += std::format(R"(<circle cx="{}" cy="{}" r="{}" fill="{}" stroke="{}" stroke-width="{}"/>)"
                           "\n",
    ...
    void arcUp(double cxL, double cyL, double rL, const char* stroke, double w) {
        out += std::format(R"(<path d="M {} {} A {} {} 0 0 1 {} {}" fill="none" stroke="{}" stroke-width="{}"/>)"
                           "\n",
                           num(x(cxL - rL)), num(y(cyL)), num(rL * kPx), num(rL * kPx), num(x(cxL + rL)), num(y(cyL)),
                           stroke, num(w));
    }
    void text(double lx, double ly, std::string_view s, double sizePx, const char* fill, const char* anchor = "middle") {
        if (s.empty()) return;
...
void glyphSvg(Svg& s, const layout::CircuitLayout& lay, const layout::Glyph& g) {
    ...
    if (g.rowMax > g.rowMin && !lay.timed && g.kind != GlyphKind::Barrier && g.kind != GlyphKind::Region)
    ...
    if (lay.timed) {
        if (g.kind == GlyphKind::Cx || g.kind == GlyphKind::Cz || g.kind == GlyphKind::Swap)
            for (const Rect& p : g.parts)
                if (p.width() > 0.03) s.rect(p, kPaper, "#c8ccd2", 1.1); // the gate's duration band
        for (std::size_t k = 1; k < g.parts.size(); ++k)
            s.line(g.parts[k - 1].cx(), g.parts[k - 1].cy(), g.parts[k].cx(), g.parts[k].cy(), mark, 1.3);
    }
    ...
    case GlyphKind::Cx:
        ...
        return;
    case GlyphKind::Cz:
        for (std::uint32_t t : g.targetRows) s.circle(cx, rowY(t), detail::kControlDot, mark, mark, 1.4);
        return;
    case GlyphKind::Swap:
        for (std::uint32_t t : g.targetRows) {
        ...
        if (g.routingSwap) s.text(cx, rowY(g.rowMin) - 0.42, "routed", 9.0, kWarn);
        return;
    ...
    if (g.params.empty() || box.height() < 0.55) {
        s.text(box.cx(), box.cy(), g.params.empty() ? g.label : g.label + "(" + g.params + ")", 12.0, kInk);
    } else {
...
std::string CircuitView::exportSvg() const {
    ...
    s.out = std::format(R"(<svg xmlns="http://www.w3.org/2000/svg" width="{}" height="{}" viewBox="0 0 {} {}" )"
    ...
    for (std::uint32_t row = 0; row < layout_.rows.size(); ++row) {
        ...
        if (wl.classical) {
            s.line(0.0, y - detail::kClassicalGap, layout_.width, y - detail::kClassicalGap, kFaint, 1.1);
            s.line(0.0, y + detail::kClassicalGap, layout_.width, y + detail::kClassicalGap, kFaint, 1.1);
        } else {
    ...
    for (const layout::Glyph& g : layout_.glyphs)
        if (g.kind == layout::GlyphKind::Region) glyphSvg(s, layout_, g);
    for (const layout::Glyph& g : layout_.glyphs)
        if (g.kind != layout::GlyphKind::Region) glyphSvg(s, layout_, g);
    s.out += "</svg>\n";
    return s.out;
