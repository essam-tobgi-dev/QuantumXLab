...
Status PauliView::addPauli(std::string_view text) {
    ...
    if (std::find(user_.begin(), user_.end(), label) == user_.end())
        user_.push_back(label);
    markDirty();
    return {};
}
...
void PauliView::push(Row& row, std::optional<double> value) {
    ...
    while (row.history.size() > kHistory)
...
void PauliView::rebuild(const ViewInput& in) {
    ...
    for (const std::string& s : user_)
        if (auto p = math::parseUserPauli(s, nQubits_))
    ...
    for (std::size_t k = 0; k < strings.size(); ++k) {
        ...
        if (const Row* old = row(r.data.label))
            r.history = old->history;

        if (in.reductions)
            if (const PauliValue* v = in.reductions->pauli(strings[k].label()))
                r.data.exact = v->value;
        if (!r.data.exact && in.snapshot && inlineOk) {
            if (in.snapshot->amplitudes) {
                if (auto e = math::pauliExpectation(*in.snapshot->amplitudes, strings[k]))
                    r.data.exact = *e;
            } else if (in.snapshot->densityMatrix && in.snapshot->levels == 2) {
                if (auto e = math::pauliExpectation(*in.snapshot->densityMatrix, strings[k]))
                    r.data.exact = *e;
            }
        }
        ...
        push(r, r.data.exact ? r.data.exact
                             : (r.data.estimate ? std::optional<double>{r.data.estimate->value}
                                                : std::nullopt));
...
std::optional<HitResult> PauliView::hitTest(glm::vec2 local) const {
    if (rows_.empty() || rowHeight_ <= 0.0f)
        return std::nullopt;
    if (local.y < headerHeight_ || local.x < 0.0f || local.x > bodySize().x)
        return std::nullopt;
    ...
    if (r.data.exact)
        h.readout.push_back({"exact", math::formatSig(*r.data.exact), input().stateClass()});
    if (r.data.estimate)
        h.readout.push_back({"shots",
                             math::formatSig(r.data.estimate->value) + " ± " +
                                 math::formatSig(r.data.estimate->standardError, 2) +
                                 "  (N = " + std::to_string(r.data.estimate->shots) + ")",
                             data::FidelityClass::Statistical});
    else if (input().counts)
        h.readout.push_back(
            {"shots", "not measured in this basis", data::FidelityClass::Statistical});
...
std::optional<std::string> PauliView::exportCsv() const {
    ...
    for (const Row& r : rows_) {
        csv += r.data.label + "," +
               (r.data.exact ? math::formatSig(*r.data.exact, 12) : std::string()) + ",";
        if (r.data.estimate)
            csv += math::formatSig(r.data.estimate->value, 12) + "," +
                   math::formatSig(r.data.estimate->standardError, 12) + "," +
...
void PauliView::drawBody(DrawContext& ctx) {
    if (rows_.empty())
        return widgets::placeholder(ctx, note_.empty() ? "No Pauli rows" : note_);
    ...
    drawText(dl, ImVec2(origin.x + colLabel + colValue, origin.y + 2.0f), theme.textSecondary,
             "shots");
    drawText(dl, ImVec2(origin.x + colLabel + colValue + colErr, origin.y + 2.0f),
             theme.textSecondary, "history");

    for (std::size_t k = 0; k < rows_.size(); ++k) {
        const Row& r = rows_[k];
        const float y = origin.y + headerHeight_ + static_cast<float>(k) * rowHeight_;
        if (y > origin.y + bodySize().y)
            break;
        if (k % 2 == 1)
            dl->AddRectFilled(ImVec2(origin.x, y), ImVec2(origin.x + bodySize().x, y + rowHeight_),
                              toU32(glm::vec3(theme.bgRaised), 0.5f));
        drawText(dl, ImVec2(origin.x, y + 2.0f),
                 r.data.userAdded ? theme.accent : theme.textPrimary, r.data.label);
        ...
        if (r.history.size() > 1 && sparkW > 20.0f) {
            ...
            dl->AddLine(ImVec2(x0, mid), ImVec2(x0 + sparkW, mid),
                        toU32(glm::vec3(theme.border), 0.6f));
            const float step = sparkW / static_cast<float>(r.history.size() - 1);
            for (std::size_t i = 1; i < r.history.size(); ++i)
                dl->AddLine(ImVec2(x0 + step * static_cast<float>(i - 1),
                                   mid - half * static_cast<float>(r.history[i - 1])),
                            ImVec2(x0 + step * static_cast<float>(i),
                                   mid - half * static_cast<float>(r.history[i])),
                            toU32(theme.accent), 1.4f);
        }
    }
    ImGui::Dummy(
        ImVec2(bodySize().x, headerHeight_ + static_cast<float>(rows_.size()) * rowHeight_));
}
