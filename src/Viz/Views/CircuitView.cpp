...
void CircuitView::rebuild(const ViewInput& in) {
    ...
    for (std::size_t k = static_cast<std::size_t>(stage_) + 1; !circuit_ && k < kCircuitStageCount;
         ++k)
        if (in.circuits.stages[k]) {
            circuit_ = in.circuits.stages[k];
            shown_ = static_cast<CircuitStage>(k);
    ...
    if (shown_ == CircuitStage::Routed || shown_ == CircuitStage::Scheduled) {
        ...
        for (std::size_t k = 0; k < static_cast<std::size_t>(CircuitStage::Routed); ++k)
            if (in.circuits.stages[k])
                before = std::max(before, layout::countSwaps(*in.circuits.stages[k]));
...
float CircuitView::alphaOf(const layout::Glyph& g) const {
    if (!hasPlayhead_)
        return 1.0f;
    if (static_cast<std::uint64_t>(g.topoIndex) < playhead_)
        return 0.34f; // executed: dimmed
...
std::optional<HitResult> CircuitView::hitTest(glm::vec2 local) const {
    if (layout_.rows.empty() || zoom_ <= 0.0)
        return std::nullopt;
    ...
    if (const layout::Glyph* g = layout_.glyphAt(p.x, p.y)) {
        ...
        std::string wires;
        for (std::uint32_t r : g->controlRows)
            wires += (wires.empty() ? "" : ", ") + detail::wireLabelText(layout_.rows[r]);
        for (std::uint32_t r : g->targetRows)
            if (r < layout_.rows.size())
                wires += (wires.empty() ? "" : ", ") + detail::wireLabelText(layout_.rows[r]);
        if (!wires.empty())
            h.readout.push_back({g->controlRows.empty() ? "wires" : "control, target", wires,
                                 data::FidelityClass::Exact});
        if (g->clbit)
            h.readout.push_back({"bit", g->params, data::FidelityClass::Exact});
        if (!layout_.timed)
            h.readout.push_back({"moment", std::to_string(g->moment), data::FidelityClass::Exact});
        if (g->startNs) {
            h.readout.push_back(
                {"t start", math::formatWithUnit(*g->startNs, "ns"), data::FidelityClass::Model});
            h.readout.push_back({"duration",
                                 math::formatWithUnit(g->durationNs.value_or(0.0), "ns"),
                                 data::FidelityClass::Model});
        }
        if (g->routingSwap)
            h.readout.push_back(
                {"routing", "SWAP inserted by the router", data::FidelityClass::Exact});
        if (hasPlayhead_)
            h.readout.push_back({"playhead",
                                 isCurrent(*g) ? "current gate"
    ...
    if (p.x < detail::kLabelGutter * -1.0 || p.x > layout_.width)
        return std::nullopt;
    const double rowF = std::floor(p.y);
    if (rowF < 0.0 || rowF >= static_cast<double>(layout_.rows.size()))
        return std::nullopt;
    const auto row = static_cast<std::uint32_t>(rowF);
    if (std::abs(p.y - layout_.rowY(row)) > kWireHitBand)
        return std::nullopt;
    ...
    if (w.classical) {
        ...
        h.row = row;
        h.readout.push_back(
            {"classical register", std::to_string(w.bits) + " bits", data::FidelityClass::Exact});
        return h;
    }
    ...
    if (w.physical && w.virtualQubit)
        h.readout.push_back(
            {"program qubit", "q" + std::to_string(*w.virtualQubit), data::FidelityClass::Exact});
    else if (!w.physical && row < layoutMap_.size())
        h.readout.push_back(
            {"physical qubit", "$" + std::to_string(layoutMap_[row]), data::FidelityClass::Exact});
    return h;
}
...
std::optional<std::string> CircuitView::exportCsv() const {
    ...
    const auto rows = [](const std::vector<std::uint32_t>& v) {
        ...
        for (std::uint32_t r : v)
            s += (s.empty() ? "" : " ") + std::to_string(r);
        return s;
    };
    for (const layout::Glyph& g : layout_.glyphs) {
        csv += std::to_string(g.topoIndex) + "," + std::to_string(static_cast<int>(g.kind)) + "," +
               g.label + ",\"" + g.params + "\"," + rows(g.targetRows) + "," + rows(g.controlRows) +
               "," + std::to_string(g.moment) + "," + std::to_string(g.column) + "," +
               std::to_string(g.depth) + "," +
...
std::string CircuitView::statusLine() const {
    ...
    s += "   " + std::to_string(layout_.qubitRows) + " qubits   " +
         std::to_string(layout_.glyphs.size()) + " ops";
    if (layout_.timed)
        s += "   " + math::formatWithUnit(layout_.durationNs, "ns");
    else
        s += "   " + std::to_string(layout_.moments) + " moments   " +
             std::to_string(layout_.columns) + " columns";
    if (layout_.routingSwaps > 0)
        s += "   " + std::to_string(layout_.routingSwaps) + " routing SWAPs";
    if (hasPlayhead_)
        s += "   gate " + std::to_string(playhead_);
    return s;
}
