...
void PhaseDiskView::layout() {
    ...
    for (std::size_t c = 1; c <= n; ++c) {
        ...
        const float side =
            std::min(body.x / static_cast<float>(c), (body.y - kLabelH) / static_cast<float>(r));
        if (side > cell_) {
            cell_ = side;
            columns_ = c;
        }
    ...
    grid_ = {0.5 * (body.x - w), 0.5 * (body.y - h - kLabelH), 0.5 * (body.x - w) + w,
             0.5 * (body.y - h - kLabelH) + h};
}

glm::vec2 PhaseDiskView::discCenter(std::size_t k) const {
    const std::size_t row = k / std::max<std::size_t>(1, columns_),
                      col = k % std::max<std::size_t>(1, columns_);
    return {static_cast<float>(grid_.x0) + (static_cast<float>(col) + 0.5f) * cell_,
            static_cast<float>(grid_.y0) + (static_cast<float>(row) + 0.5f) * cell_ -
                0.5f * kLabelH};
}

float PhaseDiskView::discRadius(std::size_t k) const {
    if (k >= selection_.entries.size())
        return 0.0f;
    // Radius ∝ |a_i| with the largest amplitude of the shown set filling the cell.
    double maxAbs = 0.0;
    for (const math::BasisEntry& e : selection_.entries)
        maxAbs = std::max(maxAbs, std::sqrt(e.probability));
...
std::optional<HitResult> PhaseDiskView::hitTest(glm::vec2 local) const {
    if (selection_.entries.empty() || cell_ <= 0.0f)
        return std::nullopt;
...
std::optional<std::string> PhaseDiskView::exportCsv() const {
    ...
    for (const math::BasisEntry& e : selection_.entries)
        csv += math::bitString(e.index, nQubits_) + "," + math::formatSig(e.amplitude.real(), 12) +
               ...
               math::formatSig(std::sqrt(e.probability), 12) + "," + math::formatSig(e.phase, 12) +
               "\n";
    return csv;
}
...
std::string PhaseDiskView::statusLine() const {
    ...
    if (!s.empty())
        s += "  ";
    return s + std::to_string(selection_.entries.size()) + " of " +
...
void PhaseDiskView::drawBody(DrawContext& ctx) {
    ...
    for (std::size_t k = 0; k < selection_.entries.size(); ++k) {
        ...
        dl->AddCircle(screen(c), 0.5f * cell_ - kPad, toU32(theme.border), 0,
                      1.0f); // the unit-magnitude ring
        if (r > 0.5f) {
            dl->AddCircleFilled(screen(c), r, toU32(hue, 0.9f));
            // Radial line at arg a (screen y grows downward, so the angle is negated).
            const auto dx = static_cast<float>(std::cos(e.phase)),
                       dy = static_cast<float>(-std::sin(e.phase));
        ...
        if (!small) {
            ...
            drawTextCentered(dl, screen({c.x, c.y + 0.5f * cell_ - 5.0f}), theme.textSecondary,
                             lbl);
        }
    }
