...
std::vector<QubitIndex> SchmidtView::defaultPartition(std::uint32_t n) const {
    ...
    for (std::uint32_t q = 0; q < std::min(n / 2 == 0 ? 1u : n / 2, kMaxPartition); ++q) a.emplace_back(q);
    return a;
}
...
ReductionRequest SchmidtView::wants(const ViewInput& in) const {
    ...
    if (n > kInlineQubits) {
        std::vector<QubitIndex> a = qubitSubset().empty() ? defaultPartition(n)
                                                          : std::vector<QubitIndex>(qubitSubset().begin(), qubitSubset().end());
        if (!a.empty() && a.size() <= kMaxPartition) r.schmidtPartition = std::move(a);
    }
    return r;
}
...
void SchmidtView::rebuild(const ViewInput& in) {
    ...
    if (in.reductions && in.reductions->schmidt && in.reductions->schmidt->partition == partition_) {
        ...
        if (s) spectrum_ = std::move(*s);
        else note_ = std::string(s.error().message);
    } else if (in.snapshot) {
        note_ = in.snapshot->amplitudes ? "Waiting for the Schmidt spectrum from the run"
...
std::optional<HitResult> SchmidtView::hitTest(glm::vec2 local) const {
    if (!spectrum_ || x_.empty() || plot_.width() <= 0.0) return std::nullopt;
    ...
    if (t < 0.0 || k >= x_.size()) return std::nullopt;
...
std::optional<std::string> SchmidtView::exportCsv() const {
    if (!spectrum_) return std::nullopt;
...
std::string SchmidtView::statusLine() const {
    if (!spectrum_) return StateView::statusLine();
...
void SchmidtView::drawBody(DrawContext& ctx) {
    if (!spectrum_ || x_.empty()) return widgets::placeholder(ctx, note_.empty() ? "No Schmidt spectrum" : note_);
    ...
    if (ImPlot::BeginPlot("##schmidt", ImVec2(body.x, body.y), ImPlotFlags_NoLegend | ImPlotFlags_NoTitle)) {
        ImPlot::SetupAxes("k", "λ_k", kCategoryAxis,
                          log_ ? ImPlotAxisFlags_AutoFit : ImPlotAxisFlags_AutoFit);
        if (log_) ImPlot::SetupAxisScale(ImAxis_Y1, ImPlotScale_Log10);
        ImPlot::SetupAxisLimits(ImAxis_X1, -0.75, static_cast<double>(x_.size()) - 0.25, ImPlotCond_Always);
