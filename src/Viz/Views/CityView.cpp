...
ReductionRequest CityView::wants(const ViewInput& in) const {
    ...
    if (!qubitSubset().empty() && subset.size() <= math::kDensityPlotMaxQubits && subset.size() < in.qubitCount())
        r.subsets.push_back(subset);
    return r;
}

void CityView::setQuantity(math::CityQuantity q) {
    if (quantity_ == q) return;
    quantity_ = q;
    markDirty();
}
...
void CityView::rebuild(const ViewInput& in) {
    ...
    if (source_.needsReduction) setStale(true);
    if (!source_.rho) return;
    if (auto built = math::buildCity(*source_.rho, quantity_)) model_ = std::move(*built);
}
...
glm::dvec3 CityView::barBase(std::uint32_t row, std::uint32_t col) const {
    ...
    return {(static_cast<double>(row) - 0.5 * (d - 1.0)) * cell_, 0.0, (static_cast<double>(col) - 0.5 * (d - 1.0)) * cell_};
}
...
std::optional<glm::vec2> CityView::projectBar(std::uint32_t row, std::uint32_t col) const {
    ...
    if (body.x <= 0.0f || body.y <= 0.0f || model_.dim == 0) return std::nullopt;
    double height = 0.0;
    for (const math::CityBar& b : model_.bars)
        if (b.row == row && b.col == col) height = b.height * heightScale_;
    glm::dvec2 px;
    const glm::dvec3 top = barBase(row, col) + glm::dvec3(0.0, std::max(0.0, height), 0.0);
    if (!canvas_.camera().project(top, static_cast<int>(body.x), static_cast<int>(body.y), px)) return std::nullopt;
...
std::optional<HitResult> CityView::hitTest(glm::vec2 local) const {
    if (model_.bars.empty()) return std::nullopt;
    // Nearest projected bar top within a radius that shrinks as the grid grows.
    const float radius = std::max(6.0f, 0.5f * bodySize().x / static_cast<float>(std::max<std::size_t>(1, model_.dim)));
    ...
    h.title = "rho[" + math::bitString(best->row, nbits) + ", " + math::bitString(best->col, nbits) + "]";
...
std::string CityView::statusLine() const {
    if (model_.dim == 0) return StateView::statusLine();
    std::string s = std::to_string(model_.dim) + " × " + std::to_string(model_.dim);
    if (source_.reduced) s += " reduced (" + subsetCaption(source_.qubits) + ")";
...
GlCanvas::SceneFn CityView::scene(const VizTheme& theme, float pxScale) const {
    return [this, &theme, pxScale](gfx::Renderer& r, GlBackend& gl) {
        const double d = static_cast<double>(model_.dim);
        const float labelPx = std::clamp(220.0f / static_cast<float>(std::max<std::size_t>(4, model_.dim)), 8.0f, 14.0f) * pxScale;
        ...
        for (int k = 0; k <= static_cast<int>(d); ++k) {
            ...
            const float a = 0.22f;
            r.linesNoDepth().segment({-half, 0.0, t}, {half, 0.0, t}, GlBackend::exact(glm::vec3(theme.border), a), pxScale);
            r.linesNoDepth().segment({t, 0.0, -half}, {t, 0.0, half}, GlBackend::exact(glm::vec3(theme.border), a), pxScale);
        }
        r.linesNoDepth().segment({-half, 0.001, -half}, {half, 0.001, half}, GlBackend::exact(glm::vec3(theme.accent), 0.55f),
                                 1.6f * pxScale);
        for (const math::CityBar& b : model_.bars) {
            ...
            const glm::mat4 model = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(base)),
            ...
            if (b.diagonal && d <= 16.0)
                r.text().label3D(base + glm::dvec3(0.0, std::max(0.0, h) + 0.12, 0.0),
                                 math::formatSig(std::abs(b.value), 2), labelPx, GlBackend::exact(theme.textSecondary),
...
void CityView::drawBody(DrawContext& ctx) {
    ...
    if (!ctx.gl) return widgets::placeholder(ctx, "The city plot needs the GL canvas (no GL context)");
    const glm::vec2 body = bodySize();
    aimCamera(body);
    ...
    if (ImGui::IsItemHovered() && ImGui::IsMouseDragging(ImGuiMouseButton_Left, 4.0f)) {
        ...
        setRotation(yaw_ - 0.008 * static_cast<double>(d.x), pitch_ + 0.008 * static_cast<double>(d.y));
    }
...
Status CityView::renderPng(GlBackend& gl, const VizTheme& theme, int widthPx, int heightPx, const std::filesystem::path& png) {
