...
inline constexpr double kClassicalGap = 0.035; // half separation of the two lines of a classical wire

inline constexpr double kBoxHalf = 0.35;      // half height of a gate box (CircuitLayout.cpp)
inline constexpr double kTimeAxisBand = 0.8;  // band under the wires holding the ns axis (timed layout)
...
inline Rect contentExtent(const layout::CircuitLayout& lay) {
    return {-kLabelGutter, lay.timed ? -0.55 : 0.0, lay.width, lay.height + (lay.timed ? kTimeAxisBand : 0.0)};
}
...
inline Rect targetBox(const layout::CircuitLayout& lay, const layout::Glyph& g) {
    if (g.targetRows.empty()) return g.bounds;
...
inline std::string wireLabelText(const layout::WireLabel& w) {
    ...
    if (w.classical) return w.bits > 0 ? s + " /" + std::to_string(w.bits) : s;
    if (w.physical && w.virtualQubit) return "q" + std::to_string(*w.virtualQubit) + " \xE2\x86\x92 " + s;
    return s;
}
