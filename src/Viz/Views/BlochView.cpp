...
void BlochView::rebuild(const ViewInput& in) {
    ...
    for (Cell& c : cells_) trails[c.qubit.get()] = std::move(c.trail);
    cells_.clear();
    canvas_.invalidate();
    if (!in.snapshot) {
        haveLast_ = false;
    ...
    const bool advanced = !haveLast_ || snap.gateIndex != lastGate_ || snap.simTimePs != lastTimePs_;
    for (QubitIndex q : shownQubits(snap.nQubits)) {
        Cell c;
        c.qubit = q;
        if (!rewound) c.trail = std::move(trails[q.get()]);
        if (const auto s = reductionFor(in, q)) {
            ...
            if (advanced && !in.reductionsStale()) {
                ...
                while (c.trail.size() > trailLength_) c.trail.pop_front();
            }
        }
...
void BlochView::layout() {
    ...
    for (Cell& c : cells_) c.onPage = false;
    ...
    const std::size_t rowsFit = std::max<std::size_t>(1, static_cast<std::size_t>(body.y / kMinCellPx));
    ...
    const float cellPx = std::min(body.x / static_cast<float>(cols), body.y / static_cast<float>(rows));
    ...
    for (std::size_t k = first; k < last; ++k) {
        ...
        c.rect = {x0 + cellPx * static_cast<float>(col), y0 + cellPx * static_cast<float>(row),
                  x0 + cellPx * static_cast<float>(col + 1), y0 + cellPx * static_cast<float>(row + 1)};
        c.centerPx = {static_cast<float>(c.rect.cx()), static_cast<float>(c.rect.cy()) - 0.04f * cellPx};
...
void BlochView::resetCamera() { setRotation(-0.45, 0.32); }

void BlochView::onSelectionChanged(const SelectionModel& sel) {
    canvas_.invalidate();
    const auto q = sel.primaryQubit(); // spec 21 §1.1: the Bloch panel scrolls to the selected qubit
    if (!q) return;
    for (std::size_t k = 0; k < cells_.size(); ++k)
        if (cells_[k].qubit == *q && perPage_ > 0 && k / perPage_ != page_) setPage(k / perPage_);
}
...
glm::vec2 BlochView::projectPoint(const Cell& c, const glm::dvec3& p, double* depth) const {
    ...
    if (depth) *depth = w.z; // the orthographic camera looks down −z: larger z is nearer
    return {c.centerPx.x + c.radiusPx * static_cast<float>(w.x), c.centerPx.y - c.radiusPx * static_cast<float>(w.y)};
}

std::optional<HitResult> BlochView::hitTest(glm::vec2 local) const {
    for (const Cell& c : cells_) {
        if (!c.onPage || !c.rect.contains(local.x, local.y)) continue;
        HitResult hit;
        ...
        hit.readout.push_back({"r", "(" + math::formatSig(c.r.x) + ", " + math::formatSig(c.r.y) + ", " + math::formatSig(c.r.z) + ")", cls});
        ...
        if (c.leakage > 1e-9) hit.readout.push_back({"leakage", math::formatSig(c.leakage), cls});
        return hit;
    }
    return std::nullopt;
}
