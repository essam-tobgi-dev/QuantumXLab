...
void HistogramView::rebuild(const ViewInput& in) {
    ...
    const std::span<const double> ideal =
        in.idealProbabilities ? std::span<const double>(*in.idealProbabilities) : std::span<const double>{};
    ...
    for (std::size_t k = 0; k < model_.bars.size(); ++k) {
        ...
        y_.push_back(value);
        // Wilson bounds as distances from p̂ (ImPlot takes ± lengths); the cumulative curve has none.
...
std::optional<HitResult> HistogramView::hitTest(glm::vec2 local) const {
    if (model_.bars.empty() || plot_.width() <= 0.0) return std::nullopt;
    ...
    if (t < 0.0 || k >= model_.bars.size()) return std::nullopt;
    const math::HistogramBar& b = model_.bars[k];
    HitResult h;
    ...
    h.readout.push_back({"counts", std::to_string(b.count) + " / " + std::to_string(model_.shots), data::FidelityClass::Statistical});
    h.readout.push_back({"p̂", math::formatSig(b.estimate.pHat), data::FidelityClass::Statistical});
    h.readout.push_back({"68.3 % Wilson",
                         "[" + math::formatSig(b.estimate.lo) + ", " + math::formatSig(b.estimate.hi) + "]",
                         data::FidelityClass::Statistical});
    if (b.ideal) h.readout.push_back({"exact p", math::formatSig(*b.ideal), input().idealClass});
    return h;
}
...
std::optional<std::string> HistogramView::exportCsv() const {
    ...
    for (const math::HistogramBar& b : model_.bars)
        csv += b.label + "," + std::to_string(b.count) + "," + math::formatSig(b.estimate.pHat, 12) + "," +
               math::formatSig(b.estimate.lo, 12) + "," + math::formatSig(b.estimate.hi, 12) + "," +
               (b.ideal ? math::formatSig(*b.ideal, 12) : std::string()) + "\n";
    return csv;
}

std::string HistogramView::statusLine() const {
    if (model_.bars.empty()) return {};
    std::string s = std::to_string(model_.shots) + " shots  " + std::to_string(model_.distinct) + " outcomes";
    if (model_.hellinger) s += "  H = " + math::formatSig(*model_.hellinger);
    if (model_.totalVariation) s += "  TVD = " + math::formatSig(*model_.totalVariation);
    return s;
}

void HistogramView::drawBody(DrawContext& ctx) {
    if (model_.bars.empty()) return widgets::placeholder(ctx, note_.empty() ? "No outcomes" : note_);
    ...
    if (ImPlot::BeginPlot("##histogram", ImVec2(body.x, body.y), ImPlotFlags_NoTitle)) {
        ...
        ImPlot::SetupAxisLimits(ImAxis_X1, -0.75, static_cast<double>(model_.bars.size()) - 0.25, ImPlotCond_Always);
        TickLabels ticks;
        const std::size_t stride = std::max<std::size_t>(1, model_.bars.size() / 24);
        for (std::size_t k = 0; k < model_.bars.size(); k += stride) ticks.add(static_cast<double>(k), model_.bars[k].label);
        ...
        if (!cumulative_) {
            ...
            ImPlot::PlotErrorBars("shots", x_.data(), y_.data(), neg_.data(), pos_.data(), static_cast<int>(x_.size()));
        }
        if (!ideal_.empty()) {
            // Theory overlay: markers in the secondary ink (spec 22 §3), never a filled bar.
            ImPlot::SetNextMarkerStyle(ImPlotMarker_Diamond, 4.0f, toImVec4(theme.theory()), 1.0f, toImVec4(theme.theory()));
            ImPlot::SetNextLineStyle(toImVec4(theme.theory()), 0.0f);
            ImPlot::PlotScatter("exact", idealX_.data(), ideal_.data(), static_cast<int>(ideal_.size()));
        }
