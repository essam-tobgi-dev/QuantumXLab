...
void PulseView::frameContent() { setWindow(0.0, 0.0); }
...
void PulseView::rebuild(const ViewInput& in) {
    ...
    if (model_.rows.empty()) {
        note_ = qubitSubset().empty() ? "The schedule has no channels" : "No channel of the selected qubits";
        return;
    }
    ...
    for (std::size_t k = 0; k < model_.rows.size(); ++k) {
        ...
        for (const layout::FrameMark& m : model_.rows[k].phaseJumps) p.phaseX.push_back(m.tNs);
    }
}

Rect PulseView::rowRect(std::size_t k) const {
    if (k >= model_.rows.size() || rowH_ <= 0.0f) return {};
    return {plot_.x0, plot_.y0 + static_cast<double>(k) * rowH_, plot_.x1, plot_.y0 + static_cast<double>(k + 1) * rowH_};
}

std::optional<HitResult> PulseView::hitTest(glm::vec2 local) const {
    if (model_.rows.empty() || plot_.width() <= 0.0 || rowH_ <= 0.0f) return std::nullopt;
    if (!plot_.contains(local.x, local.y)) return std::nullopt;
    const auto k = static_cast<std::size_t>((local.y - plot_.y0) / rowH_);
    if (k >= model_.rows.size()) return std::nullopt;
    const double tNs = model_.t0Ns + (model_.t1Ns - model_.t0Ns) * (local.x - plot_.x0) / plot_.width();
    ...
    if (r->frequencyHz != 0.0) h.readout.push_back({"frequency", math::formatFrequency(r->frequencyHz), kExact});
    if (r->inAcquisition) h.readout.push_back({"acquisition", "window open", kExact});
    return h;
}

std::optional<std::string> PulseView::exportCsv() const {
    if (model_.rows.empty()) return std::nullopt;
    std::string csv = "channel,t_ns,re,im\n";
    for (const layout::PulseRow& row : model_.rows)
        for (std::size_t k = 0; k < row.trace.tNs.size(); ++k)
            csv += row.label + "," + math::formatSig(row.trace.tNs[k], 12) + "," + math::formatSig(row.trace.re[k], 12) +
                   "," + math::formatSig(row.trace.im[k], 12) + "\n";
    return csv;
}

std::string PulseView::statusLine() const {
    if (model_.rows.empty()) return {};
    std::string s = std::to_string(model_.rows.size()) + " channels   " + math::formatTime(model_.durationNs * 1e-9) +
                    "   dt = " + math::formatTime(model_.dtNs * 1e-9);
    for (const layout::PulseRow& row : model_.rows)
        if (row.trace.decimated) { // spec 22 §2: say so, the line is an envelope and not the samples
            s += "   decimated";
            break;
        }
    if (hasPlayhead_) s += "   playhead " + math::formatTime(playheadNs_ * 1e-9);
    return s;
}

void PulseView::drawBody(DrawContext& ctx) {
    if (model_.rows.empty()) return widgets::placeholder(ctx, note_.empty() ? "No pulse channels" : note_);
    ...
    if (!ImPlot::BeginSubplots("##pulses", rows, 1, ImVec2(body.x, rowH_ * static_cast<float>(rows)),
                               ImPlotSubplotFlags_LinkAllX | ImPlotSubplotFlags_NoTitle))
        return;
    for (std::size_t k = 0; k < model_.rows.size(); ++k) {
        const layout::PulseRow& row = model_.rows[k];
        const RowPlot& p = plots_[k];
        if (!ImPlot::BeginPlot(row.label.c_str(), ImVec2(), ImPlotFlags_NoTitle)) continue;
        ...
        if (!p.a.empty()) {
            if (row.trace.decimated) { // min/max band under the mean line (spec 22 §2)
                ...
                ImPlot::PlotShaded("##band", row.trace.tNs.data(), row.trace.reLo.data(), row.trace.reHi.data(),
                                   static_cast<int>(row.trace.tNs.size()));
            }
            ImPlot::SetNextLineStyle(toImVec4(glm::vec3(theme.accent), 1.0f), 1.6f);
            ImPlot::PlotLine(polar_ ? "|e|" : "Re", row.trace.tNs.data(), p.a.data(), static_cast<int>(p.a.size()));
            ImPlot::SetNextLineStyle(toImVec4(glm::vec3(theme.accentSoft), 1.0f), 1.2f);
            ImPlot::PlotLine(polar_ ? "arg e" : "Im", row.trace.tNs.data(), p.b.data(), static_cast<int>(p.b.size()));
        }
        ...
        for (const layout::FrequencyMark& f : row.frequencyChanges)
            ImPlot::Annotation(f.tNs, 0.0, toImVec4(theme.textSecondary), ImVec2(4.0f, -4.0f), true, "%s",
                               math::formatFrequency(f.frequencyHz).c_str());
