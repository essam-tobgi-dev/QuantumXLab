...
void AmplitudeView::rebuild(const ViewInput& in) {
    ...
    const std::vector<QubitIndex> subset =
        qubitSubset().empty() ? std::vector<QubitIndex>{} : shownQubits(selection_.nQubits);
    if (!subset.empty() && subset.size() < selection_.nQubits) {
        ...
        for (const math::BasisEntry& e : selection_.entries)
            if (e.index < full.size())
                full[e.index] = e.probability;
        if (auto marg = math::marginalProbabilities(full, selection_.nQubits, subset)) {
            ...
            for (std::size_t k = 0; k < marg->size(); ++k) {
                if (filter_.threshold > 0.0 && (*marg)[k] <= filter_.threshold)
                    continue;
                sel.entries.push_back(
                    {k, num::Complex{std::sqrt((*marg)[k]), 0.0}, (*marg)[k], 0.0});
    ...
    if (filter_.order == math::AmplitudeOrder::ByMagnitude)
        std::stable_sort(selection_.entries.begin(), selection_.entries.end(),
                         [](const math::BasisEntry& a, const math::BasisEntry& b) {
...
std::optional<HitResult> AmplitudeView::hitTest(glm::vec2 local) const {
    if (selection_.entries.empty() || plot_.width() <= 0.0)
        return std::nullopt;
    ...
    if (t < 0.0 || k >= selection_.entries.size())
        return std::nullopt;
    const math::BasisEntry& e = selection_.entries[k];
    HitResult h;
    ...
    if (marginal_) {
        ...
        h.readout.push_back(
            {"marginal over", subsetCaption(shownQubits(input().qubitCount())), cls});
        return h;
    }
...
std::string AmplitudeView::statusLine() const {
    ...
    if (!s.empty())
        s += "  ";
    s += std::to_string(selection_.entries.size()) + " of " +
         std::to_string(selection_.totalStates) + " states";
    if (selection_.truncated || fromRun_)
        s += "  mass " +
             math::formatSig(selection_.shownProbability, 4); // spec 21 §3.2: top-k mass shown
    return s;
}
...
void AmplitudeView::drawBody(DrawContext& ctx) {
    if (selection_.entries.empty())
        return widgets::placeholder(ctx,
                                    note_.empty() ? "No basis state above the threshold" : note_);
    const VizTheme& theme = *ctx.theme;
    const PlotStyle style(theme);
    const glm::vec2 body = bodySize();
    const ImVec2 bodyOrigin =
        ImGui::GetCursorScreenPos();              // body-local (0,0): what hitTest works in
    const float wheel = marginal_ ? 0.0f : 54.0f; // room for the phase legend (spec 21 §2.4)
    if (ImPlot::BeginPlot("##amplitudes", ImVec2(body.x - wheel, body.y),
                          ImPlotFlags_NoLegend | ImPlotFlags_NoTitle)) {
        const char* yLabel = marginal_ ? "p" : probability_ ? "|a|²" : "|a|";
        ImPlot::SetupAxes("basis state  (bit 0 is rightmost)", yLabel, kCategoryAxis,
        ...
        for (std::size_t k = 0; k < heights_.size(); ++k) {
            const glm::vec3 fill = marginal_ ? glm::vec3(theme.neutral())
                                             : math::phaseColor(selection_.entries[k].phase);
