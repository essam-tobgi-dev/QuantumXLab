...
ReductionRequest QSphereView::wants(const ViewInput& in) const {
    ReductionRequest r;
    if (in.snapshot && in.snapshot->nQubits > math::kFullAmplitudeQubits) r.topAmplitudes = options_.topK;
    return r;
}
...
void QSphereView::rebuild(const ViewInput& in) {
    ...
    if (in.snapshot->amplitudes && !in.snapshot->amplitudes->empty())
        ...
        model_ = math::buildQSphere(*in.reductions->topAmplitudes, options_); // the run's top-k above 20 qubits
}
...
void QSphereView::resetCamera() { setRotation(-0.45, 0.32); }
...
glm::vec2 QSphereView::projectPoint(const glm::dvec3& p, double* depth) const {
    ...
    if (depth) *depth = w.z;
    return {center_.x + radius_ * static_cast<float>(w.x), center_.y - radius_ * static_cast<float>(w.y)};
}
...
std::string QSphereView::statusLine() const {
    ...
    if (model_.topKOnly) // spec 21 §3.4: "shows the top-k nodes only and says so"
        s += "  top " + std::to_string(model_.nodes.size()) + " of " + std::to_string(model_.totalStates) + " states, p = " +
             math::formatSig(model_.shownProbability);
    return s;
}
...
std::optional<HitResult> QSphereView::hitTest(glm::vec2 local) const {
    ...
    hit.readout.push_back({"phase", math::formatAngle(best->state.phase), cls}); // hue is always paired with a label (§4)
    hit.readout.push_back({"Hamming weight", std::to_string(best->place.weight), cls});
    return hit;
}

GlCanvas::SceneFn QSphereView::scene(const VizTheme& theme, const SelectionModel* selection, float pxScale) const {
    return [this, &theme, selection, pxScale](gfx::Renderer& r, GlBackend& gl) {
        ...
        const glm::dvec3 center((center_.x - 0.5 * body.x) * worldPerPx, -(center_.y - 0.5 * body.y) * worldPerPx, 0.0);
        ...
        for (std::uint32_t w = 1; w < n; ++w) {
            ...
            for (int k = 1; k <= 60; ++k) {
                ...
                r.linesNoDepth().segment(prev, cur, ink(theme.textSecondary, front ? 0.45f : 0.16f), 1.0f * pxScale);
                prev = cur;
            }
        }
        r.linesNoDepth().segment(world({0, 0, -1}), world({0, 0, 1}), ink(theme.textSecondary, 0.35f), 1.0f * pxScale);
        ...
        for (const auto& node : model_.nodes) {
            ...
            if (node.spoke) r.linesNoDepth().segment(center, p, ink(glm::vec4(node.color, 1.0f), 0.75f), 1.4f * pxScale);
            gfx::Material m;
            m.unlit = true; // baked shading × exact phase hue: the lit side matches the legend exactly
            m.baseColor = GlBackend::exact(node.color);
            const float rad = static_cast<float>(std::max(node.radius, 0.012));
            r.submit(gl.sphereLow(), m, glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(p)), glm::vec3(rad)), ComponentId{0}, flat());
            // Outline facing the viewer: with H = (φ + π)/2π the φ = 0 hue is the darkest entry of the
            // twilight LUT, which all but vanishes on the dark panel without a rim.
            r.linesNoDepth().circle(glm::vec3(p), glm::vec3(0.0f, 0.0f, 1.0f), rad, ink(theme.textSecondary, 0.9f), 32, 1.2f * pxScale);
            ...
            if (focused || node.state.probability > 0.02)
                r.text().label3D(p, math::ketLabel(node.state.index, n, n > 6, true), labelPx,
                                 GlBackend::exact(focused ? theme.accent : theme.textPrimary), gfx::TextAnchor::BottomCenter,
...
void QSphereView::aimCamera() {
    ...
    b.ortho = true;
    b.orthoHeight = radius_ > 0.0f ? static_cast<double>(bodySize().y) / static_cast<double>(radius_) : 2.0;
    cam.set(b);
}

void QSphereView::drawBody(DrawContext& ctx) {
    if (model_.nodes.empty()) return widgets::placeholder(ctx, "Run a program on a state-vector backend to see the Q-sphere");
    if (!ctx.gl) return widgets::placeholder(ctx, "The Q-sphere needs the GL canvas (no GL context)");
    const glm::vec2 body = bodySize();
    aimCamera();
...
Status QSphereView::renderPng(GlBackend& gl, const VizTheme& theme, int widthPx, int heightPx, const std::filesystem::path& png) {
