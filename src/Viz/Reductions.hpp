...
inline constexpr std::uint32_t kPairReductionMaxQubits =
    20;                                              // spec 21 §3.8: all pairs only for n ≤ 20
...
struct ReductionRequest {
    ...
    std::vector<std::vector<QubitIndex>>
        subsets; // reduced ρ of each subset (≤ 8 qubits, first = LSB)
...
struct Reductions {
    ...
    std::vector<std::string>
        notes; // why a requested item is absent ("Schmidt spectrum needs a pure state")
...
Result<Reductions> computeReductions(const qsim::Snapshot& snapshot,
                                     const ReductionRequest& request, std::stop_token stop = {});
