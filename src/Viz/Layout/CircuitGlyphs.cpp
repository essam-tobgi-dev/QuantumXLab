...
std::string gateParamText(const ir::Gate& g) {
    ...
    for (std::size_t k = 0; k < g.params.size(); ++k) {
        if (k)
            s += ", ";
        std::string angle =
            math::formatAngle(g.params[k]); // multiples of π up to q = 16, else 4 s.f. radians
        if (const auto pos = angle.find(" rad"); pos != std::string::npos)
            angle.erase(pos); // boxes print bare radians
        s += angle;
    }
    return s;
}
...
std::vector<NodeTiming> scheduleTimes(const ir::Circuit& c) {
    ...
    if (start == it->end() || length == it->end() || !start->is_array() || !length->is_array())
        return out;
    const std::size_t n = std::min(start->size(), length->size());
    if (n != c.topologicalOrder().size())
        return {}; // stale record: the circuit changed after scheduling
    out.reserve(n);
    for (std::size_t k = 0; k < n; ++k) {
        if (!(*start)[k].is_number() || !(*length)[k].is_number())
            return {};
        out.push_back(
            {(*start)[k].get<double>() * 1e-3, (*length)[k].get<double>() * 1e-3}); // ps → ns
    }
    return out;
}
...
std::optional<std::uint32_t> classicalRowOf(const ir::Circuit& top, std::uint32_t bit) {
    ...
    for (std::size_t r = 0; r < regs.size(); ++r)
        if (bit >= regs[r].first && bit < regs[r].first + regs[r].size)
            return top.qubitCount() + static_cast<std::uint32_t>(r);
    return std::nullopt;
}
...
std::vector<Body> bodiesOf(const ir::Node& node) {
    ...
    if (const auto* b = std::get_if<ir::Branch>(&node)) {
        if (b->thenBody.present())
            out.push_back({&*b->thenBody, ""});
        if (b->elseBody.present() && b->elseBody->nodeCount() > 0)
            out.push_back({&*b->elseBody, "else"});
    } else if (const auto* l = std::get_if<ir::Loop>(&node)) {
        if (l->body.present())
            out.push_back({&*l->body, ""});
    } else if (const auto* x = std::get_if<ir::Box>(&node)) {
        if (x->body.present())
            out.push_back({&*x->body, ""});
    }
    return out;
}
...
std::string durationText(const ir::Duration& d) {
    std::string s;
    if (d.ps.get() != 0 || d.dt == 0)
        s = math::formatTime(static_cast<double>(d.ps.get()) * 1e-12);
    if (d.dt != 0)
        s += (s.empty() ? "" : " + ") + std::to_string(d.dt) + " dt";
    return s;
}

void setRows(Glyph& g, std::uint32_t extraMax = 0) {
    std::uint32_t lo = 0xFFFFFFFFu, hi = 0;
    for (auto r : g.targetRows)
        lo = std::min(lo, r), hi = std::max(hi, r);
    for (auto r : g.controlRows)
        lo = std::min(lo, r), hi = std::max(hi, r);
...
Glyph gateGlyph(const ir::Gate& gate) {
    Glyph g;
    // A named controlled gate is its base gate plus leading control operands (T02 §3): cx = ctrl @
    // x.
    ...
    if (base == "x" && controlled)
        g.kind = GlyphKind::Cx;
    else if (base == "z" && controlled)
        g.kind = GlyphKind::Cz;
    else if (base == "swap")
        g.kind = GlyphKind::Swap;
    else
        g.kind = GlyphKind::Box;
    g.label = base;
    if (gate.adjoint)
        g.label += "\xE2\x80\xA0"; // †
    // The base gate keeps its own parameters (cp(λ) → p(λ)); a split that changes them (cu) keeps
    // the node's.
...
std::optional<Glyph> describeNode(const ir::Circuit& top, const ir::Node& node,
    ...
    if (const auto* gate = std::get_if<ir::Gate>(&node)) {
        if (gate->targets.empty() && gate->controls.empty())
            return std::nullopt; // gphase: nothing to draw on a wire
    ...
    if (std::holds_alternative<ir::Barrier>(node)) {
        ...
        for (ir::Wire w : ir::nodeWiresIn(node, qubitRows))
            g.targetRows.push_back(w.index); // empty = every wire
    ...
    if (const auto* d = std::get_if<ir::Delay>(&node)) {
        ...
        for (ir::Wire w : ir::nodeWiresIn(node, qubitRows))
            g.targetRows.push_back(w.index);
        if (g.targetRows.empty())
            return std::nullopt;
        setRows(g);
        return g;
    ...
    if (const auto* br = std::get_if<ir::Branch>(&node))
        g.label = "if (" + br->cond.text() + ")";
    else if (const auto* lp = std::get_if<ir::Loop>(&node))
        g.label = "while (" + lp->cond.text() + ")";
    else if (const auto* bx = std::get_if<ir::Box>(&node))
        g.label = bx->duration ? "box[" + durationText(*bx->duration) + "]" : "box";
    for (ir::Wire w : ir::nodeWires(node))
        g.targetRows.push_back(w.index);
    if (g.targetRows.empty())
        return std::nullopt;
    setRows(g);
...
double glyphWidth(const Glyph& g) {
    ...
    case GlyphKind::Region:
        return 0.0; // sized by its contents
    default:
        break;
    }
    // ~0.13 units per character at the diagram's font size, one line for the name, one for
    // parameters.
