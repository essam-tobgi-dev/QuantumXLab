...
enum class NodeField : std::uint8_t { None, T1, T2, F01, Anharmonicity, Error1q, ReadoutError, QecRole };
...
struct GraphNode {
    ...
    std::optional<std::uint32_t> virtualQubit; // program qubit mapped here (printed inside the node)
...
struct CouplingOptions {
    ...
    const qec::CodeView* code = nullptr;    // role colouring + live syndrome bits when bound to the chip
};
DeviceGraph buildCouplingGraph(const hw::Device& device, const hw::Calibration* calibration, const CouplingOptions& options = {});

// Spec 21 §3.8: the same nodes, edges for every reduced pair, colours from the entropies.
DeviceGraph buildEntanglementGraph(const hw::Device* device, std::uint32_t nQubits, const Reductions& reductions);
...
constexpr std::size_t paletteIndexOf(qec::QubitRole r) {
    switch (r) {
    case qec::QubitRole::Data: return 1;         // sky blue
    case qec::QubitRole::AncillaX: return 0;     // orange
    case qec::QubitRole::AncillaZ: return 2;     // bluish green
    case qec::QubitRole::AncillaMixed: return 6; // reddish purple
    }
    return 7;
}
