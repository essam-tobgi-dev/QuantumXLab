...
Level layoutLevel(const ir::Circuit& top, const ir::Circuit& c, std::uint32_t depth, std::uint32_t qubitRows,
                  std::uint32_t totalRows, std::uint32_t inheritedTopo) {
    Level level;
    ...
    if (depth == 0) {
        ...
        for (ir::NodeId id : c.topologicalOrder()) {
            if (orderOf.size() <= id.get()) orderOf.resize(id.get() + 1, 0);
            orderOf[id.get()] = k++;
        }
    }
    for (std::size_t m = 0; m < layers.size(); ++m) {
        ...
        const auto place = [&](std::uint32_t lo, std::uint32_t hi, std::size_t span, bool exclusive) {
            std::size_t col = exclusive ? busy.size() : 0;
            for (; col < busy.size(); ++col) {
                bool free = true;
                for (std::uint32_t r = lo; r <= hi && free; ++r) free = !busy[col][r];
                if (free) break;
            }
            while (busy.size() < col + span) busy.emplace_back(totalRows, false);
            for (std::size_t s = 0; s < span; ++s)
                for (std::uint32_t r = lo; r <= hi; ++r) busy[col + s][r] = true;
            return col;
        };
        for (ir::NodeId id : layers[m]) {
            ...
            if (g.kind != GlyphKind::Region) {
                ...
                if (level.widths.size() <= base + col) level.widths.resize(base + col + 1, 0.0);
                level.widths[base + col] = std::max(level.widths[base + col], detail::glyphWidth(g));
                level.glyphs.push_back(std::move(g));
                continue;
            }
            // A region owns fresh columns: its bodies side by side, an "else" separator between them.
            std::vector<Level> bodies;
            std::vector<std::string> separators;
            std::size_t span = 0;
            for (const auto& body : detail::bodiesOf(node)) {
                bodies.push_back(layoutLevel(top, *body.circuit, depth + 1, qubitRows, totalRows, g.topoIndex));
            ...
            if (level.widths.size() < base + col + span) level.widths.resize(base + col + span, 0.0);
            ...
            if (bodies.size() > 1 && !separators[1].empty())
                g.elseColumn = static_cast<std::uint32_t>(base + col + std::max<std::size_t>(1, bodies[0].widths.size()));
            ...
            for (std::size_t b = 0; b < bodies.size(); ++b) {
                ...
                for (std::size_t k = 0; k < cols; ++k) {
                    const double w = k < bodies[b].widths.size() ? std::max(bodies[b].widths[k], 0.7) : 0.7;
                ...
                for (Glyph child : bodies[b].glyphs) {
                    ...
                    if (child.elseColumn) *child.elseColumn += static_cast<std::uint32_t>(offset);
                    level.glyphs.push_back(std::move(child));
                }
                offset += cols;
            }
            if (headerWidth > bodyWidth) level.widths[base + col + span - 1] += headerWidth - bodyWidth;
        }
    }
    return level;
}
...
void assignBounds(CircuitLayout& out, Level& level) {
    ...
    for (Glyph& g : level.glyphs) {
        const std::size_t c0 = g.column, c1 = std::min<std::size_t>(level.widths.size() - 1, g.column + g.columnSpan - 1);
        if (g.kind == GlyphKind::Region) {
            const double inset = 0.04 * static_cast<double>(g.depth);
            g.bounds = {out.columnX[c0] - 0.12 + inset, out.rowY(g.rowMin) - 0.48 + inset,
                        out.columnX[c1] + out.columnWidth[c1] + 0.12 - inset, out.rowY(g.rowMax) + 0.48 - inset};
        } else {
            const double w = detail::glyphWidth(g);
            const double cx = out.columnX[c0] + 0.5 * out.columnWidth[c0];
            g.bounds = {cx - 0.5 * w, out.rowY(g.rowMin) - kBoxHalf, cx + 0.5 * w, out.rowY(g.rowMax) + kBoxHalf};
            g.parts = {g.bounds};
        }
    }
}
...
void assignTimedBounds(CircuitLayout& out, Level& level, const std::vector<NodeTiming>& times) {
    ...
    for (const auto& t : times)
        if (t.durationNs > 0.0 && (shortest == 0.0 || t.durationNs < shortest)) shortest = t.durationNs;
    for (const auto& t : times) out.durationNs = std::max(out.durationNs, t.startNs + t.durationNs);
    out.nsPerUnit = shortest > 0.0 ? shortest : 1.0;
    if (out.durationNs / out.nsPerUnit > 20000.0) out.nsPerUnit = out.durationNs / 20000.0; // keep the axis drawable
    std::vector<double> tickEnd(out.rows.size(), -1.0); // zero-duration ticks on a wire are fanned out, never stacked
    for (Glyph& g : level.glyphs) {
        if (g.depth != 0 || g.topoIndex >= times.size()) continue; // nested bodies have their own time axis
        ...
        if (g.kind == GlyphKind::Classical) rows = {g.rowMin};
        g.parts.clear();
        if (w <= 0.0) { // virtual gates, barriers: a tick above the wire, clear of the box band
            for (auto r : rows) x0 = std::max(x0, tickEnd[r] + 0.04);
            for (auto r : rows) {
                g.parts.push_back({x0, out.rowY(r) - 0.49, x0 + 0.16, out.rowY(r) - kBoxHalf - 0.02});
                tickEnd[r] = x0 + 0.16;
            }
        } else {
            for (auto r : rows) g.parts.push_back({x0, out.rowY(r) - kBoxHalf, x0 + w, out.rowY(r) + kBoxHalf});
        }
        if (g.parts.empty()) continue;
        g.bounds = g.parts.front();
        for (const Rect& p : g.parts) g.bounds = g.bounds.unite(p);
    ...
    for (Glyph& g : level.glyphs)
        if (g.kind == GlyphKind::Region && g.startNs)
            g.bounds = {kMargin + *g.startNs / out.nsPerUnit, out.rowY(g.rowMin) - 0.48,
                        kMargin + (*g.startNs + g.durationNs.value_or(0.0)) / out.nsPerUnit, out.rowY(g.rowMax) + 0.48};
}
...
Result<CircuitLayout> layoutCircuit(const ir::Circuit& circuit, const CircuitLayoutOptions& options) {
    ...
    for (std::uint32_t q = 0; q < circuit.qubitCount(); ++q) {
        ...
        if (circuit.isPhysical()) {
            ...
            for (std::size_t v = 0; v < options.layout.size(); ++v)
                if (options.layout[v] == q) w.virtualQubit = static_cast<std::uint32_t>(v);
        }
    ...
    if (options.timed && times.empty())
        return fail(ErrorCode::InvalidArgument, "timed layout needs a scheduled circuit (no meta.schedule record)");
    if (options.timed) {
        out.timed = true;
        assignTimedBounds(out, level, times);
    } else {
        if (level.widths.empty()) level.widths.push_back(0.7);
...
const Glyph* CircuitLayout::glyphAt(double x, double y) const {
    ...
    for (const Glyph& g : glyphs) {
        bool inside = false;
        if (g.kind == GlyphKind::Region) inside = g.bounds.contains(x, y);
        else
            for (const Rect& p : g.parts) inside = inside || p.contains(x, y);
        if (!inside) continue;
        // Prefer a gate over the region around it, and the deeper of two regions.
        if (!best || (best->kind == GlyphKind::Region && (g.kind != GlyphKind::Region || g.depth > best->depth))) best = &g;
    }
    return best;
}
...
std::optional<std::pair<std::size_t, std::size_t>> CircuitLayout::firstOverlap() const {
    for (std::size_t a = 0; a < glyphs.size(); ++a) {
        if (glyphs[a].kind == GlyphKind::Region) continue;
        for (std::size_t b = a + 1; b < glyphs.size(); ++b) {
            if (glyphs[b].kind == GlyphKind::Region) continue;
            for (const Rect& pa : glyphs[a].parts)
                for (const Rect& pb : glyphs[b].parts)
                    if (pa.overlaps(pb)) return std::pair{a, b};
        }
    }
    return std::nullopt;
}
