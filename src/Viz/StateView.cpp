...
constexpr float kClickSlopPx =
    4.0f; // a drag that moved further orbits the camera, it does not select
} // namespace

std::string hitKey(const HitResult& h) {
    std::string k = std::to_string(static_cast<int>(h.kind));
    if (h.qubit)
        k += "q" + std::to_string(h.qubit->get());
    if (h.edge)
        k += "e" + std::to_string(h.edge->first.get()) + "-" + std::to_string(h.edge->second.get());
    if (h.basisIndex)
        k += "b" + std::to_string(*h.basisIndex);
    if (h.element)
        k += "m" + std::to_string(h.element->first) + "," + std::to_string(h.element->second);
    if (h.gate)
        k += "g" + std::to_string(*h.gate);
    if (h.row)
        k += "r" + std::to_string(*h.row);
    return k;
}
...
StateView::Stamp StateView::stampOf(const ViewInput& in) {
    ...
    for (const auto& c : in.circuits.stages)
        s.pointers[k++] = c.get();
...
void StateView::update(const ViewInput& in) {
    ...
    if (haveInput_ && !dirty_ && stamp == stamp_)
        return; // nothing changed: O(1)
    input_ = in;
    stamp_ = stamp;
    haveInput_ = true;
    dirty_ = false;
...
std::string StateView::statusLine() const {
    ...
    if (input_.snapshot->simTimePs > 0.0)
        s += "  t = " + math::formatTime(input_.snapshot->simTimePs * 1e-12);
    return s;
}

std::optional<HitResult> StateView::click(glm::vec2 local, SelectionModel* selection,
                                          bool additive) {
    ...
    if (selection) {
        ...
        case HitKind::Qubit:
            if (hit->qubit)
                selection->selectQubit(*hit->qubit, additive);
            break;
        case HitKind::Edge:
            if (hit->edge)
                selection->selectEdge(hit->edge->first, hit->edge->second);
            break;
        case HitKind::BasisState: // a second click on the focused state releases the filter (spec
                                  // 21 §3.4)
            if (hit->basisIndex)
                selection->setBasisFocus(
                    selection->basisFocus() == hit->basisIndex ? std::nullopt : hit->basisIndex);
            break;
        default:
            break;
        }
...
void StateView::draw(DrawContext& ctx) {
    ...
    const bool inside = local.x >= 0.0f && local.y >= 0.0f && local.x <= avail.x &&
                        local.y <= avail.y &&
    ...
    if (hit && *hit) {
        ...
        if (ctx.selection)
            ctx.selection->setHoveredQubit(hit->qubit);
        if (ctx.timeS - hoverSince_ >= kHoverDelayS)
            widgets::readoutCard(ctx, *hit);
    } else {
        hoverKey_.clear();
        if (ctx.selection)
            ctx.selection->setHoveredQubit(std::nullopt);
    }
    const bool released = ImGui::IsMouseReleased(ImGuiMouseButton_Left);
    if (released &&
        io.MouseDragMaxDistanceSqr[ImGuiMouseButton_Left] <= kClickSlopPx * kClickSlopPx)
        ...
        if (ctx.selection)
            for (int d = 0; d < 9; ++d) // keys 1–9 select qubits 0–8
                if (ImGui::IsKeyPressed(static_cast<ImGuiKey>(ImGuiKey_1 + d), false) &&
                    ...
                    ctx.selection->selectQubit(QubitIndex{static_cast<std::uint32_t>(d)},
                                               io.KeyShift);
    }
}
