...
template <class T> void appendUnique(std::vector<T>& into, const std::vector<T>& from) {
    for (const T& v : from)
        if (std::find(into.begin(), into.end(), v) == into.end()) into.push_back(v);
}
} // namespace

bool ReductionRequest::empty() const {
    return !singles && !pairs && subsets.empty() && !schmidtPartition && !singleQubitPaulis && pauliStrings.empty() &&
           topAmplitudes == 0 && !wigner;
}
...
void ReductionRequest::merge(const ReductionRequest& o) {
    ...
    if (hadWork && otherHasWork) {
        if (qubits.empty() || o.qubits.empty()) qubits.clear();
        else appendUnique(qubits, o.qubits);
    ...
    if (!schmidtPartition) schmidtPartition = o.schmidtPartition; // one bipartition per snapshot: first asker wins
...
bool ViewInput::reductionsStale() const {
    if (!snapshot || !reductions) return false;
    return reductions->gateIndex != snapshot->gateIndex || reductions->simTimePs != snapshot->simTimePs;
}

std::uint32_t ViewInput::qubitCount() const {
    if (snapshot && snapshot->nQubits > 0) return snapshot->nQubits;
    if (device) return static_cast<std::uint32_t>(device->qubitCount());
    if (const auto c = circuits.latest()) return c->qubitCount();
    return 0;
}
...
TrajectoryEnsemble ensembleFromAverages(std::span<const qsim::AveragedSample> samples, std::uint32_t site,
    ...
    for (const auto& s : samples) {
        ...
        if (base + 1 >= s.populations.size()) continue;
