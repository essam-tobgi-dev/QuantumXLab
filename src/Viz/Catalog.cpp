...
template <class V> std::unique_ptr<IStateView> make() { return std::make_unique<V>(); }
...
constexpr std::array<std::string_view, kCatalog.size()> kIds = [] {
    ...
    for (std::size_t k = 0; k < kCatalog.size(); ++k) ids[k] = kCatalog[k].id;
    return ids;
}();
...
std::vector<std::unique_ptr<IStateView>> makeAllViews() {
    ...
    for (const Entry& e : kCatalog) out.push_back(e.factory());
    return out;
}
...
std::span<const std::string_view> viewIds() { return kIds; }
