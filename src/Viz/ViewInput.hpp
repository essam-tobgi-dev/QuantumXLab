...
constexpr std::string_view circuitStageName(CircuitStage s) {
    switch (s) {
    case CircuitStage::Source:
        return "Source";
    case CircuitStage::Decomposed:
        return "Decomposed";
    case CircuitStage::Routed:
        return "Routed";
...
struct CircuitSet {
    std::array<std::shared_ptr<const ir::Circuit>, kCircuitStageCount>
        stages;                        // null = stage not produced
    ...
    const std::shared_ptr<const ir::Circuit>& at(CircuitStage s) const {
...
// [site][level]. `purity` and `blochNorm[site]` are optional second-axis curves, parallel to
// samples.
...
struct ViewInput {
    ...
    std::shared_ptr<const std::vector<double>>
        idealProbabilities; // exact Born distribution over the classical bits
    data::FidelityClass idealClass =
        data::FidelityClass::Exact; // Exact (state vector) or Numerical (density matrix)
    std::shared_ptr<const math::MeasurementMap>
        measurement; // which qubit was read in which basis into which bit
    // ---- program
    CircuitSet circuits;
    ...
    data::FidelityClass stateClass() const {
        return snapshot ? snapshot->cls : data::FidelityClass::Exact;
    }
