...
// layout is serialised through `ImGui::SaveIniSettingsToMemory` into this record, one ini string per
// workspace, together with the visibility set and each panel's own `serialize()` slice. The record
// goes into `<project>.qxlab` (spec 23 §2) and into the user config for the default project.
//
