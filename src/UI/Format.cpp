...
const units::UnitCatalog& cat() { return units::UnitCatalog::global(); }

// "dB", "dBm", "dBc", "dBc/Hz" are log ratios: the stored number IS the displayed number.
bool isLogSymbol(std::string_view u) { return u.size() >= 2 && (u[0] == 'd' && u[1] == 'B'); }
...
units::FormatContext contextFor(std::string_view unit) {
    if (unit == "s") return units::FormatContext::GateDuration;
    if (unit == "K") return units::FormatContext::Cryogenic;
    if (unit == "dBm") return units::FormatContext::RfPower;
    return units::FormatContext::Default;
}

std::string value(double si, std::string_view unit, int digits, units::FormatContext ctx) {
    if (!std::isfinite(si)) return "—";
    if (unit.empty()) return number(si, digits);
    if (isLogSymbol(unit)) return number(si, digits) + " " + std::string(unit);
    const units::UnitDef* def = cat().find(unit);
    if (def == nullptr) return number(si, digits) + " " + std::string(unit);
...
std::string withSigma(double si, double sigma, std::string_view unit, int digits) {
    ...
    if (!std::isfinite(sigma) || sigma <= 0.0) return full;
    ...
    if (symbol.empty()) return head + " ± " + number(sigma, 2);
    const auto scaled = cat().toUnit(si, symbol);
    const auto scaledSigma = cat().toUnit(sigma, symbol);
    if (!scaled || !scaledSigma) return head + " ± " + number(sigma, 2) + " " + symbol;
...
std::string integer(std::uint64_t v) {
    ...
    for (std::size_t i = 0; i < digits.size(); ++i) {
        if (i == lead || (i > lead && (i - lead) % 3 == 0)) out += " "; // narrow no-break space
        out += digits[i];
    }
    return out;
}
...
std::string duration(double seconds) {
    ...
    if (a < 60.0) return value(seconds, "s");
    ...
    if (h > 0) return std::format("{}{} h {:02} min", sign, h, m);
    return std::format("{}{} min {:02} s", sign, m, s);
}

std::string bytes(double count) {
    if (!std::isfinite(count)) return "—";
...
Result<double> parse(std::string_view text, std::string_view defaultUnit) {
    if (defaultUnit.empty()) {
        ...
        if (ec == std::errc{} && ptr == trimmed.data() + trimmed.size()) return v;
    }
    const auto parsed = cat().parse(text, defaultUnit);
    if (!parsed)
        return fail(err::BadToken, std::format("'{}' is not a number{}", text,
    ...
    if (!defaultUnit.empty()) {
        ...
        if (want != nullptr && parsed->dim != want->dim)
            return fail(err::BadToken,
                        std::format("unit '{}' has the wrong dimension; this field is in {}", parsed->symbol, defaultUnit));
    }
    return parsed->si;
}
