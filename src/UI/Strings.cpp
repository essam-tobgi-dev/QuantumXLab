...
void flatten(const core::Json& node, const std::string& prefix,
...
Result<Strings> Strings::fromJson(const core::Json& data) {
    if (!data.is_object())
        return fail(err::BadAsset, "strings: `data` is not an object");
    Strings s;
    flatten(data, {}, s.table_);
    if (s.table_.empty())
        return fail(err::BadAsset, "strings: no entries");
    return s;
}
...
const Strings& Strings::global() {
    return mutableGlobal();
}
void Strings::setGlobal(Strings s) {
    mutableGlobal() = std::move(s);
}
...
std::string_view Strings::get(std::string_view key) const {
    ...
    if (it != table_.end())
        return it->second;
    if (std::find(missing_.begin(), missing_.end(), key) == missing_.end())
        missing_.emplace_back(key);
    return key;
}
