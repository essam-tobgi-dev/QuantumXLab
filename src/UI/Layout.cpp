...
core::Json LayoutState::toJson() const {
    ...
    if (!shortcuts.is_null() && !shortcuts.empty()) j["shortcuts"] = shortcuts;
    ...
    for (const auto& [key, value] : panels) ps[key] = value;
    j["panels"] = std::move(ps);
    return j;
}

Result<LayoutState> LayoutState::fromJson(const core::Json& j) {
    if (!j.is_object()) return fail(err::BadLayout, "layout: `data` is not an object");
    LayoutState s;
    if (const auto it = j.find("active"); it != j.end() && it->is_string()) {
        const auto w = workspaceFromName(it->get<std::string>());
        if (!w) return fail(err::BadLayout, "layout: unknown workspace '" + it->get<std::string>() + "'");
        s.active = *w;
    }
    if (const auto it = j.find("physical_lab"); it != j.end() && it->is_boolean()) s.physicalLab = it->get<bool>();
    if (const auto it = j.find("palette"); it != j.end() && it->is_string()) s.palette = it->get<std::string>();
    if (const auto it = j.find("font_scale"); it != j.end() && it->is_number())
        s.fontScale = std::clamp(it->get<float>(), 0.8f, 1.6f); // spec 19 §7
    if (const auto it = j.find("shortcuts"); it != j.end()) s.shortcuts = *it;

    if (const auto ws = j.find("workspaces"); ws != j.end()) {
        if (!ws->is_object()) return fail(err::BadLayout, "layout: `workspaces` is not an object");
        for (const auto& [name, entry] : ws->items()) {
            const auto w = workspaceFromName(name);
            if (!w) return fail(err::BadLayout, "layout: unknown workspace '" + name + "'");
            WorkspaceLayout& into = s.of(*w);
            if (!entry.is_object()) return fail(err::BadLayout, "layout: workspace '" + name + "' is not an object");
            if (const auto i = entry.find("ini"); i != entry.end() && i->is_string()) into.ini = i->get<std::string>();
            ...
            if (const auto i = entry.find("open"); i != entry.end() && i->is_array())
                for (const core::Json& p : *i) {
                    if (!p.is_string()) return fail(err::BadLayout, "layout: `open` entry is not a string");
                    const std::string key = p.get<std::string>();
                    if (!panelFromKey(key)) return fail(err::UnknownPanel, "layout: unknown panel '" + key + "'");
                    into.open.push_back(key);
                }
        }
    }
    if (const auto ps = j.find("panels"); ps != j.end()) {
        if (!ps->is_object()) return fail(err::BadLayout, "layout: `panels` is not an object");
