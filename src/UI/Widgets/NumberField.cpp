...
double autoStep(double value, const FieldSpec& spec) {
    if (spec.step > 0.0)
        return spec.step;
    if (std::isfinite(spec.lo) && std::isfinite(spec.hi) && spec.hi > spec.lo)
        return (spec.hi - spec.lo) / 200.0;
...
void commit(const UiContext& ctx, std::string_view label, const FieldSpec& spec, double* value,
            double from, double to) {
    if (from == to || value == nullptr)
        return;
    *value = to;
    if (ctx.undo == nullptr)
        return;
...
bool body(const UiContext& ctx, std::string_view label, double* value, const FieldSpec& spec,
          float width) {
    const Metrics m = ctx.metrics_px();
    const ImGuiID id = ImGui::GetID(label.data(), label.data() + label.size());
    FieldState& st = stateOf(id);
    const std::string text =
        format::value(*value, spec.unit, spec.digits, format::contextFor(spec.unit));
    bool changed = false;
    ...
    dl->AddRect(p, ImVec2(p.x + size.x, p.y + size.y),
                u32(hovered ? ctx.th()[Token::Accent] : ctx.th()[Token::Border]), m.radiusSm, 0,
                m.border);
    // Value in the primary tone (Simulator-only values in `sim_only`), unit suffix in the
    // secondary.
    ...
    dl->AddText(at, u32(Theme::readableText(valueColor, fill)), head.c_str(),
                head.c_str() + head.size());
    if (!unit.empty())
    ...
    if (active) {
        ...
        if (dx != 0.0f) {
            *value =
                format::applyDrag(*value, dx, autoStep(*value, spec), mods(), spec.lo, spec.hi);
            changed = true;
        }
    }
    ...
    if (hovered) {
        ...
        if (wheel != 0.0f) {
            ...
            const double to = std::clamp(
                from + wheel * autoStep(from, spec) * format::dragScale(mods()), spec.lo, spec.hi);
            commit(ctx, label, spec, value, from, to);
            changed = true;
        }
    }
    if (hovered)
        classTooltip(ctx, spec.cls);
    return changed;
}
...
bool numberField(const UiContext& ctx, std::string_view label, double* value,
    ...
    ImGui::SameLine();
    const bool changed =
        body(ctx, label, value, spec, spec.width > 0.0f ? spec.width : fieldWidth());
...
bool intField(const UiContext& ctx, std::string_view label, std::int64_t* value,
    ...
    if (changed)
        *value = static_cast<std::int64_t>(std::llround(v));
    return changed;
}
        ...
        for (int i = 0; i < static_cast<int>(items.size()); ++i) {
            ...
            if (ImGui::Selectable(item.c_str(), i == current) && i != current) {
                const std::string name =
                    undoLabel.empty() ? std::string(label) : std::string(undoLabel);
                *index = i;
                if (ctx.undo != nullptr)
                    ctx.undo->pushValue<int>(name, index, current, i);
                changed = true;
            }
        }
        ImGui::EndCombo();
