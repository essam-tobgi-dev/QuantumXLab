...
Result<TheoryAssets> TheoryAssets::fromJson(const core::Json& equations, const core::Json& gates,
    ...
    if (eqs == equations.end() || !eqs->is_array())
        return fail(err::BadAsset, "equations.json: missing `data.equations`");
    for (const core::Json& e : *eqs) {
        EquationDoc d;
        d.id = str(e, "id");
        if (d.id.empty())
            return fail(err::BadAsset, "equations.json: an entry has no `id`");
        d.latex = str(e, "latex");
        d.plain = str(e, "plain");
    ...
    if (gs == gates.end() || !gs->is_array())
        return fail(err::BadAsset, "gates.json: missing `data.gates`");
    for (const core::Json& g : *gs) {
        GateDocEntry d;
        d.name = str(g, "name");
        if (d.name.empty())
            return fail(err::BadAsset, "gates.json: an entry has no `name`");
        ...
        if (const auto q = g.find("qubits"); q != g.end() && q->is_number_integer())
            d.qubits = q->get<int>();
        if (const auto c = g.find("clifford"); c != g.end() && c->is_boolean())
            d.clifford = c->get<bool>();
    ...
    for (const auto& [key, value] : as->items())
        if (value.is_string())
            a.assumptions_[key] = value.get<std::string>();
    return a;
}

Result<TheoryAssets> TheoryAssets::load() {
    const std::filesystem::path dir = core::assetDir() / "Theory";
    QXL_TRY_ASSIGN(const core::Envelope eq,
                   core::JsonEnvelope::load(dir / "equations.json", "theory.equations"));
    QXL_TRY_ASSIGN(const core::Envelope ga,
                   core::JsonEnvelope::load(dir / "gates.json", "theory.gates"));
    QXL_TRY_ASSIGN(const core::Envelope au,
                   core::JsonEnvelope::load(dir / "assumptions.json", "theory.assumptions"));
