...
bool buttonWith(const UiContext& ctx, std::string_view label, ImVec2 size, const Color& fill,
                const Color& border, const Color& label_color) {
    const Metrics m = ctx.metrics_px();
    ImGui::PushStyleColor(ImGuiCol_Button, iv(fill));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          iv(Color(glm::vec3(fill) * 1.12f, std::max(fill.a, 0.25f))));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                          iv(Color(glm::vec3(fill) * 0.88f, std::max(fill.a, 0.4f))));
...
void badge(const UiContext& ctx, std::string_view label, const Color& color) {
    ...
    dl->AddText(ImVec2(p0.x + pad.x, p0.y + pad.y),
                u32(Theme::readableText(color, ctx.th()[Token::BgPanel])), label.data(),
...
void classTooltip(const UiContext& ctx, data::FidelityClass cls) {
    ...
    const std::string key =
        std::string("fidelity_class.tooltip.") + std::string(data::fidelityName(cls));
...
bool primaryButton(const UiContext& ctx, std::string_view label, ImVec2 size) {
    ...
    return buttonWith(ctx, label, size, accent, accent,
                      Theme::readableText(ctx.th()[Token::TextPrimary], accent));
}

bool secondaryButton(const UiContext& ctx, std::string_view label, ImVec2 size) {
    return buttonWith(ctx, label, size, Color(glm::vec3(ctx.th()[Token::BgRaised]), 0.0f),
                      ctx.th()[Token::Border], readable(ctx, ctx.th()[Token::TextPrimary]));
}
...
bool toggleChip(const UiContext& ctx, std::string_view label, bool* value) {
    ...
    const bool pressed = *value ? buttonWith(ctx, label, ImVec2(0, 0), Color(glm::vec3(on), 0.25f),
                                             on, readable(ctx, on))
...
void sectionHeader(const UiContext& ctx, std::string_view label) {
    ...
    dl->AddLine(ImVec2(p.x, p.y), ImVec2(p.x + ImGui::GetContentRegionAvail().x, p.y),
                u32(ctx.th()[Token::Border]), m.border);
    ImGui::Dummy(ImVec2(0.0f, m.spacing(1)));
}
...
void specRow(const UiContext& ctx, std::string_view field, std::string_view value,
    ...
    if (typicalHi > typicalLo && std::isfinite(si)) {
        ...
        dl->AddRectFilled(a, ImVec2(a.x + w, a.y + h),
                          u32(Color(glm::vec3(ctx.th()[Token::Border]), 0.6f)), m.radiusSm);
        const auto t =
            static_cast<float>(std::clamp((si - typicalLo) / (typicalHi - typicalLo), 0.0, 1.0));
        const float x = a.x + t * w;
        dl->AddRectFilled(ImVec2(x - m.border, a.y - h * 0.5f),
                          ImVec2(x + m.border, a.y + h * 1.5f), u32(ctx.th()[Token::Accent]));
...
void progressBar(const UiContext& ctx, double fraction, std::string_view overlay, float width) {
    ...
    if (fraction >= 0.0) {
        ...
        dl->AddRectFilled(a, ImVec2(a.x + w * f, a.y + h), u32(ctx.th()[Token::Accent]),
                          m.radiusSm);
    } else {
        // Indeterminate: a short sweep, held still under the reduced-motion setting (spec 19 §8).
        const float span = w * 0.25f;
        const auto phase =
            ctx.reducedMotion ? 0.0f : static_cast<float>(std::fmod(ctx.timeS * 0.6, 1.0));
        const float x0 = a.x + (w + span) * phase - span;
        dl->AddRectFilled(ImVec2(std::max(a.x, x0), a.y),
                          ImVec2(std::min(a.x + w, x0 + span), a.y + h),
...
void placeholder(const UiContext& ctx, std::string_view message) {
    ...
    ImGui::SetCursorPos(
        ImVec2(ImGui::GetCursorPos().x + std::max(0.0f, (avail.x - size.x) * 0.5f),
...
ImGuiTableFlags tableFlags(bool sortable) {
    ImGuiTableFlags f = ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerV |
                        ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollY |
                        ImGuiTableFlags_SizingStretchProp;
    if (sortable)
        f |= ImGuiTableFlags_Sortable;
    return f;
}
