...
math::MathStyle styleFor(const UiContext& ctx, bool display, float scale) {
    math::MathStyle s;
    s.display = display;
    s.sizePx = std::max(8.0f, ctx.metrics_px().bodyPx * std::max(0.5f, scale) * (display ? 1.15f : 1.0f));
    return s;
}
...
void EquationView::setDocument(const EquationDoc* doc) {
    doc_ = doc;
    if (doc != nullptr) setLatex(doc->latex);
}

void EquationView::setValues(std::vector<TermValue> values) { values_ = std::move(values); }
...
ImVec2 EquationView::size(const UiContext& ctx) const {
    ...
    if (lr == nullptr) return ImVec2(0.0f, 0.0f);
...
std::string EquationView::draw(const UiContext& ctx) {
    ...
    if (lr == nullptr) {
        ...
        widgets::text(ctx, Token::TextSecondary, latex_.empty() ? std::string_view{} : std::string_view(latex_));
        return {};
    }
    ...
    if (hovered) {
        ...
        const auto hit = math::hitTestMath(*lr, 0.0, lr->height, mouse.x - origin.x, mouse.y - origin.y);
        if (hit) {
            ...
            dl->AddLine(ImVec2(origin.x + static_cast<float>(hit->x), y),
                        ImVec2(origin.x + static_cast<float>(hit->x + hit->w), y), u32(ctx.th()[Token::Accent]),
    ...
    if (hovered && !hoverSymbol.empty()) {
        ...
        if (ImGui::BeginTooltip()) {
            ...
            widgets::text(ctx, Token::TextPrimary, term != nullptr ? std::string_view(term->name) : std::string_view(hoverSymbol));
            ...
            if (doc_ != nullptr && ctx.assets != nullptr && !doc_->assumptions.empty()) {
                ...
                for (const std::string& key : doc_->assumptions) {
                    ...
                    widgets::text(ctx, Token::TextSecondary, sentence.empty() ? std::string_view(key) : sentence);
                }
            }
    ...
    if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) {
        if (ImGui::GetIO().KeyCtrl || ImGui::GetIO().KeySuper) ImGui::SetClipboardText(latex_.c_str());
        else if (doc_ != nullptr && !doc_->theory.empty() && ctx.cmd.openTheory) ctx.cmd.openTheory(doc_->theory);
        clicked = hoverSymbol;
    }
    if (ImGui::BeginPopupContextItem("##equation_menu")) {
        if (ImGui::MenuItem("Copy LaTeX")) ImGui::SetClipboardText(latex_.c_str());
        if (doc_ != nullptr && ImGui::MenuItem("Copy plain")) ImGui::SetClipboardText(doc_->plain.c_str());
        const bool hasTheory = doc_ != nullptr && !doc_->theory.empty();
        if (ImGui::MenuItem("Open theory", nullptr, false, hasTheory) && ctx.cmd.openTheory) ctx.cmd.openTheory(doc_->theory);
        ImGui::EndPopup();
    }
...
void eduCard(const UiContext& ctx, std::string_view id, std::string_view title, std::string_view meaning,
    ...
    if (!meaning.empty()) widgets::textWrapped(ctx, Token::TextSecondary, meaning);
    ...
    for (const TermValue& v : values) {
        ...
        const std::string unit = v.unit.empty() && term != nullptr ? term->unit : v.unit;
        widgets::labelled(ctx, term != nullptr ? std::string_view(term->name) : std::string_view(v.symbol),
    ...
    if (doc != nullptr && !doc->theory.empty()) {
        ...
        if (ImGui::IsItemClicked() && ctx.cmd.openTheory) ctx.cmd.openTheory(doc->theory);
    }
    ImGui::PopTextWrapPos();
}
