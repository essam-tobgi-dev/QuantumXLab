...
Box Layouter::wrapDelims(Box content, std::string_view left, std::string_view right, Ctx c) const {
    ...
    double x = 0;
    if (!left.empty()) {
        Box l = scaledGlyph(left, size, h, d);
        l.x = 0;
        x = l.width + 0.03 * size;
    ...
    if (!right.empty()) {
        Box r = scaledGlyph(right, size, h, d);
        r.x = x + 0.03 * size;
        x = r.x + r.width;
...
Box Layouter::layoutAccent(const MathNode& n, Ctx c) {
    ...
    if (n.flag && (n.text == "¯" || n.text == "_")) {
    ...
    } else if (n.flag) { // wide glyph accents (braces, arrows): stretch horizontally by drawing at
                         // base width
        acc = glyph(n.text, size, GlyphStyle::Upright, std::nullopt);
        acc.kind = BoxKind::Scaled;
        acc.width = base.width;
        acc.x = 0;
        acc.y = below ? base.depth + acc.height + 0.05 * size
                      : -(base.height + acc.depth + 0.05 * size);
    } else {
        ...
        if (n.text == "⃗") {
            acc.y = -(std::max(base.height, xh) + 0.02 * size);
        } // combining arrow sits high
    }
...
Box Layouter::layoutBigOp(const MathNode& n, Ctx c) {
    ...
    Box op =
        textOp ? textRun(n.text, size, GlyphStyle::Upright, n.src)
    ...
    if (sub) {
        subB = layout(*sub, sc);
        w = std::max(w, subB->width);
    }
    if (sup) {
        supB = layout(*sup, sc);
        w = std::max(w, supB->width);
    }
    double gap = 0.1 * size;
    centerHoriz(op, w);
    ...
    if (supB) {
        centerHoriz(*supB, w);
        supB->y = -(opTop + gap + supB->depth);
    }
    if (subB) {
        centerHoriz(*subB, w);
        subB->y = opBottom + gap + subB->height;
    }
...
Box Layouter::layoutMatrix(const MathNode& n, Ctx c) {
    ...
    Ctx cc =
        c.with(small ? MathStyleLevel::Script
                     : (c.display() && (env == "aligned" || env == "align" || env == "align*" ||
                                        env == "gathered" || env == "gather" || env == "split" ||
                                        env == "equation" || env == "equation*" || env == "cases")
    ...
    for (const auto& ch : n.children)
        cells.push_back(ch ? layout(*ch, cc) : space(0));
    std::vector<double> colW(static_cast<std::size_t>(cols), 0.0),
        rowH(static_cast<std::size_t>(rows), 0.0), rowD(static_cast<std::size_t>(rows), 0.0);
    for (int r = 0; r < rows; ++r)
        for (int k = 0; k < cols; ++k) {
            std::size_t i = static_cast<std::size_t>(r * cols + k);
            if (i >= cells.size())
                break;
            colW[static_cast<std::size_t>(k)] =
                std::max(colW[static_cast<std::size_t>(k)], cells[i].width);
            rowH[static_cast<std::size_t>(r)] =
                std::max(rowH[static_cast<std::size_t>(r)], cells[i].height);
            rowD[static_cast<std::size_t>(r)] =
                std::max(rowD[static_cast<std::size_t>(r)], cells[i].depth);
        }
    bool alignEnv =
        env == "aligned" || env == "align" || env == "align*" || env == "split" || env == "alignat";
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int k = 0; k < cols; ++k) {
            ...
            if (alignEnv)
                b.x = x + ((k % 2 == 0) ? (cw - b.width) : 0.0);
            else if (leftAlign)
                b.x = x;
            else
                b.x = x + (cw - b.width) / 2.0;
            b.y = y;
...
void paintBox(const Box& b, MathCanvas& cv, double ox, double oy, const MathFont* font) {
    ...
    switch (b.kind) {
    case BoxKind::Glyph:
        cv.drawGlyph(b.text, ax, ay, b.size, b.style);
        break;
    case BoxKind::Scaled: {
        // Stretch the glyph vertically so its natural extent matches height+depth.
        double natural = b.size, scale = 1.0;
        if (font) {
            GlyphMetrics m = font->metrics(b.text, b.size, GlyphStyle::Upright);
            natural = m.ascent + m.descent;
            if (natural > 1e-9)
                scale = (b.height + b.depth) / natural;
        }
        double sz = b.size * std::max(1.0, scale);
        double descent = 0;
        if (font)
            descent = font->metrics(b.text, sz, GlyphStyle::Upright).descent;
        cv.drawGlyph(b.text, ax, ay + b.depth - descent, sz, GlyphStyle::Upright);
        break;
    }
    case BoxKind::Rule:
        cv.drawLine(ax, ay - b.thickness / 2, ax + b.width, ay - b.thickness / 2, b.thickness);
        break;
    case BoxKind::Space:
        break;
    case BoxKind::HBox:
        for (const auto& c : b.children)
            paintBox(c, cv, ax, ay, font);
        break;
    }
}
...
std::optional<SymbolHit> hitTestMath(const LayoutResult& lr, double ox, double oy, double px,
                                     double py) {
