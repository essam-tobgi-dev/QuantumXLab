...
std::vector<MathToken> tokenizeMath(std::string_view s) {
    ...
    auto push = [&](TokKind k, std::string text, std::size_t b, std::size_t e) {
        out.push_back({k, std::move(text), {static_cast<std::uint32_t>(b), static_cast<std::uint32_t>(e)}});
    };
    while (i < s.size()) {
        char c = s[i];
        ...
        std::size_t b = i;
        if (c == '\\') {
            if (i + 1 >= s.size()) { push(TokKind::Char, "\\", b, i + 1); ++i; continue; }
            char n = s[i + 1];
            if (n == '\\') { push(TokKind::NewRow, "\\\\", b, i + 2); i += 2; continue; }
            if (std::isalpha(static_cast<unsigned char>(n))) {
                ...
                if (i < s.size() && s[i] == '*' && out.back().text == "operatorname") { ++i; }
                continue;
            }
        ...
        switch (c) {
        case '{': push(TokKind::LBrace, "{", b, i + 1); ++i; continue;
        case '}': push(TokKind::RBrace, "}", b, i + 1); ++i; continue;
        case '^': push(TokKind::Caret, "^", b, i + 1); ++i; continue;
        case '_': push(TokKind::Underscore, "_", b, i + 1); ++i; continue;
        case '&': push(TokKind::Amp, "&", b, i + 1); ++i; continue;
        default: break;
        }
        std::size_t j = i;
        utf8Decode(s, j);
