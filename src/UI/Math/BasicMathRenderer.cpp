...
Result<std::shared_ptr<const math::LayoutResult>>
BasicMathRenderer::render(std::string_view latex, const math::MathStyle& style) {
    ...
    if (!po.root)
        return fail(ErrorCode::Ui_ + 1, "math parse produced no tree");
...
void BasicMathRenderer::paint(const math::LayoutResult& lr, math::MathCanvas& canvas, double x,
                              double baselineY) const {
    math::paintMath(lr, canvas, x, baselineY, font_);
}
std::optional<math::SymbolHit> BasicMathRenderer::hitTest(const math::LayoutResult& lr, double ox,
                                                          double oy, double px, double py) const {
    return math::hitTestMath(lr, ox, oy, px, py);
}
std::size_t BasicMathRenderer::cacheSize() const {
    std::lock_guard lk(mu_);
    return cache_.size();
}
void BasicMathRenderer::clearCache() {
    std::lock_guard lk(mu_);
    cache_.clear();
    order_.clear();
}
