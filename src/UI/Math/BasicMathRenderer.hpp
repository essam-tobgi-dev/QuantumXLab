...
public:
    ...
    Result<std::shared_ptr<const math::LayoutResult>> render(std::string_view latex, const math::MathStyle& style);
    void paint(const math::LayoutResult& lr, math::MathCanvas& canvas, double x, double baselineY) const;
    std::optional<math::SymbolHit> hitTest(const math::LayoutResult& lr, double originX, double baselineY, double px, double py) const;
...
private:
    ...
    std::unordered_map<std::string, std::pair<std::shared_ptr<const math::LayoutResult>, std::list<std::string>::iterator>> cache_;
    std::size_t hits_ = 0;
};
