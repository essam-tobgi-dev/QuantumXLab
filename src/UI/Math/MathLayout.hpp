...
struct GlyphMetrics {
    double advance = 0, ascent = 0, descent = 0;
};

// Font metrics provider. Sizes are pixels. Implementations: ImGui font atlas (UI), monospace stub
// (tests).
class MathFont {
  public:
    virtual ~MathFont() = default;
    virtual GlyphMetrics metrics(std::string_view utf8Glyph, double sizePx,
                                 GlyphStyle style) const = 0;
...
class MathCanvas {
  public:
    ...
    virtual void drawGlyph(std::string_view utf8, double x, double baselineY, double sizePx,
                           GlyphStyle style) = 0;
...
enum class BoxKind : std::uint8_t {
    ...
    Rule,
    Space,
    Scaled /*glyph stretched to height+depth*/
};
...
std::optional<SymbolHit> hitTestMath(const LayoutResult& lr, double originX, double baselineY,
                                     double px, double py);
