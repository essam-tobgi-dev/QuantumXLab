...
bool dashAt(std::string_view s, std::size_t i, std::size_t& len) {
    ...
    if (i + 2 < s.size() && b(i) == 0xE2 && b(i + 1) == 0x80 && b(i + 2) >= 0x90 && b(i + 2) <= 0x95) { len = 3; return true; }
    if (i + 2 < s.size() && b(i) == 0xE2 && b(i + 1) == 0x88 && b(i + 2) == 0x92) { len = 3; return true; }
    return false;
}
...
std::string slugify(std::string_view h) {
    ...
    for (std::size_t i = 0; i < h.size();) {
        ...
        std::size_t dlen = 0;
        if (dashAt(h, i, dlen)) { out.push_back('-'); i += dlen; continue; }
        if (c == ' ' || c == '\t' || c == '-' || c == '_' || c == '/') { out.push_back('-'); ++i; continue; }
        if (std::isalnum(c)) {
        ...
        if (c == '.') {
            ...
            if (prevDigit && nextDigit) out.push_back('.');
            ++i;
            continue;
        }
        if (c >= 0x80) {
...
LinkRef classifyLink(std::string_view href) {
    ...
    if (href.empty()) return r;
    if (startsWith(href, "http://") || startsWith(href, "https://") || startsWith(href, "mailto:")) {
    ...
    if (startsWith(href, "spec:")) {
        ...
        if (auto h = rest.find('#'); h != std::string::npos) { r.anchor = rest.substr(h + 1); rest = rest.substr(0, h); }
        r.id = rest;
        return r;
    }
    if (startsWith(href, "component:")) { r.kind = LinkKind::Component; r.id = std::string(href.substr(10)); return r; }
    if (startsWith(href, "eq:")) { r.kind = LinkKind::Equation; r.id = std::string(href.substr(3)); return r; }
    if (href[0] == '#') { r.kind = LinkKind::Internal; r.anchor = std::string(href.substr(1)); return r; }

    std::string path(href), anchor;
    if (auto h = path.find('#'); h != std::string::npos) { anchor = path.substr(h + 1); path = path.substr(0, h); }
    ...
    if (base.size() >= 3 && (base[0] == 'T' || base[0] == 't') &&
        std::isdigit(static_cast<unsigned char>(base[1])) && std::isdigit(static_cast<unsigned char>(base[2]))) {
...
std::vector<InlineSpan> parseInline(std::string_view t) {
    ...
    for (std::size_t i = 0; i < t.size();) {
        ...
        if (c == '`') { // inline code
            ...
            if (e == std::string_view::npos) { buf.push_back(c); ++i; continue; }
            flush();
            InlineSpan s;
        ...
        if (c == '$') { // inline math (a lone '$' stays literal)
            std::size_t e = i + 1;
            while (e < t.size() && !(t[e] == '$' && t[e - 1] != '\\')) ++e;
            if (e >= t.size()) { buf.push_back(c); ++i; continue; }
            flush();
            InlineSpan s;
            s.text = std::string(t.substr(i + 1, e - i - 1));
            s.math = true;
        ...
        if (c == '*' || c == '_') {
            ...
            bool wordInner = c == '_' && !buf.empty() && std::isalnum(static_cast<unsigned char>(buf.back())) &&
                             i + 1 < t.size() && std::isalnum(static_cast<unsigned char>(t[i + 1]));
            if (!wordInner) {
                flush();
                if (dbl) { bold = !bold; i += 2; } else { italic = !italic; ++i; }
                continue;
            }
        }
        if (c == '[') { // [text](href)
