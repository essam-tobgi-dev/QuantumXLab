...
std::string snippetAround(const std::string& text, const std::string& needleLower,
                          std::size_t width = 120) {
    std::string lo = lower(text);
    std::size_t p = lo.find(needleLower);
    if (p == std::string::npos)
        return text.substr(0, std::min(text.size(), width));
...
std::filesystem::path TheoryIndex::defaultDir() {
    if (const char* e = std::getenv("QXL_THEORY_DIR"))
        return e;
...
Result<TheoryIndex> TheoryIndex::load(const std::filesystem::path& dir) {
    ...
    if (idx.docs_.empty())
        return fail(ErrorCode::Ui_ + 17, "no theory documents in " + dir.string());
    return idx;
}
...
std::vector<SearchHit> TheoryIndex::search(std::string_view query, std::size_t maxHits) const {
    ...
    for (const auto& [id, doc] : docs_) {
        ...
        for (std::size_t bi = 0; bi < doc.blocks.size(); ++bi) {
            ...
            if (hay.empty())
                continue;
            if (lower(hay).find(q) == std::string::npos)
                continue;
            SearchHit h;
            h.docId = id;
