...
std::string runStatusText(const UiContext& ctx) {
    ...
    case SessionView::Status::Failed: {
        const std::array<std::pair<std::string_view, std::string>, 1> args{{{"n", std::to_string(s.errors)}}};
        return str.format("diagnostics.errors", args);
    }
    case SessionView::Status::Compiling:
    case SessionView::Status::Idle: break;
    }
...
Token runStatusToken(SessionView::Status s) {
    switch (s) {
    case SessionView::Status::Running: return Token::Ok;
    case SessionView::Status::Compiling: return Token::Accent;
    case SessionView::Status::Failed: return Token::Err;
    case SessionView::Status::Done: return Token::Ok;
    case SessionView::Status::Idle: break;
    }
...
void Shell::drawMenus(UiContext& ctx) {
    ...
    const auto item = [&](std::string_view labelKey, Action a, const std::function<void()>& fn, bool enabled = true) {
        ...
        if (ImGui::MenuItem(label.c_str(), shortcut.empty() ? nullptr : shortcut.c_str(), false, enabled && fn != nullptr) && fn)
            fn();
    };
    ...
    if (ImGui::BeginMenu(std::string(ctx.text("menu.edit")).c_str())) {
        ...
        if (ImGui::MenuItem(std::string(ctx.text("menu.undo")).c_str(), keys_.text(Action::Undo).c_str(), false, canUndo))
            ctx.undo->undo();
        if (ImGui::MenuItem(std::string(ctx.text("menu.redo")).c_str(), keys_.text(Action::Redo).c_str(), false, canRedo))
            ctx.undo->redo();
        ImGui::EndMenu();
    }
    if (ImGui::BeginMenu(std::string(ctx.text("menu.view")).c_str())) {
        for (const PanelPtr& p : panels_) {
            if (!preset(workspace_).contains(p->id())) continue;
            bool on = isOpen(p->id());
            const bool blocked = hiddenByPhysicalLab(*p);
            ImGui::BeginDisabled(blocked);
            if (ImGui::MenuItem(p->title().c_str(), nullptr, &on) && !blocked) setOpen(p->id(), on);
            ImGui::EndDisabled();
            // Spec 19 §2: greyed entries explain themselves.
            if (blocked) widgets::simOnlyDisabledTooltip(ctx);
        }
        ImGui::Separator();
        ...
        if (ImGui::BeginMenu("Text size")) {
            for (const auto& [label, value] : {std::pair<const char*, float>{"Smaller", 0.8f},
                ...
                if (ImGui::MenuItem(label, nullptr, on) && !on) setFontScale(value);
            }
            ImGui::EndMenu();
        }
        if (ImGui::MenuItem("Reset layout")) resetLayout(workspace_);
        ImGui::EndMenu();
    }
    ...
    if (ImGui::BeginMenu(std::string(ctx.text("menu.help")).c_str())) {
        if (ImGui::MenuItem(std::string(ctx.text("menu.theory")).c_str()) && ctx.cmd.openTheory) ctx.cmd.openTheory("T01");
        ImGui::EndMenu();
    }
    ImGui::EndMenuBar();
}
...
void Shell::drawTopBar(UiContext& ctx, float& heightOut) {
    ...
    if (ImGui::Begin("##qxl_topbar", nullptr, kFlags)) {
        ...
        for (std::size_t i = 0; i < kWorkspaceCount; ++i) {
            ...
            const std::string label = std::string(ctx.text(workspaceLabelKey(w))) + "  " + keys_.text(
                                          w == Workspace::Lab       ? Action::WorkspaceLab
                                          : w == Workspace::Program ? Action::WorkspaceProgram
                                                                    : Action::WorkspaceAnalysis);
            bool selected = w == workspace_;
            if (widgets::toggleChip(ctx, label, &selected) && selected) setWorkspace(w);
        }
        ...
        widgets::labelled(ctx, ctx.text("run.backend"),
                          ctx.session_view.backend.empty() ? ctx.text("run.backend_auto") : ctx.session_view.backend);
        ImGui::SameLine(0.0f, m.spacing(3));
        widgets::text(ctx, runStatusToken(ctx.session_view.status), runStatusText(ctx));
        if (ctx.session_view.status == SessionView::Status::Running && ctx.session_view.shotsTotal > 0) {
            ImGui::SameLine();
            widgets::progressBar(ctx, static_cast<double>(ctx.session_view.shotsDone) /
        ...
        const std::string project = ctx.session_view.project.empty() ? std::string(ctx.text("app.project_untitled"))
                                                                     : ctx.session_view.project;
        const float rightWidth = widgets::textSize(physical).x + widgets::textSize(project).x + m.spacing(6) * 2.0f;
        ImGui::SameLine(std::max(ImGui::GetCursorPosX(), ImGui::GetWindowWidth() - rightWidth));
        bool lab = physicalLab_;
        if (widgets::toggleChip(ctx, physical, &lab)) setPhysicalLab(lab);
...
void Shell::handleShortcuts(UiContext& ctx) {
    ...
    for (Action a : fired_) {
        switch (a) {
        case Action::WorkspaceLab: setWorkspace(Workspace::Lab); break;
        case Action::WorkspaceProgram: setWorkspace(Workspace::Program); break;
        case Action::WorkspaceAnalysis: setWorkspace(Workspace::Analysis); break;
        case Action::TogglePhysicalLab: setPhysicalLab(!physicalLab_); break;
        case Action::Undo:
            if (ctx.undo != nullptr) ctx.undo->undo();
            break;
        case Action::Redo:
            if (ctx.undo != nullptr) ctx.undo->redo();
            break;
        case Action::Run:
            if (ctx.cmd.run) ctx.cmd.run();
            break;
        case Action::Stop:
            if (ctx.cmd.stop) ctx.cmd.stop();
            break;
        case Action::Compile:
            if (ctx.cmd.compile) ctx.cmd.compile();
            break;
        case Action::StepGate:
            if (ctx.cmd.stepGate) ctx.cmd.stepGate();
            break;
        case Action::StepShot:
            if (ctx.cmd.stepShot) ctx.cmd.stepShot();
            break;
        case Action::SaveProject:
            if (ctx.cmd.saveProject) ctx.cmd.saveProject();
            break;
        case Action::ExportResults:
            if (ctx.cmd.exportResults) ctx.cmd.exportResults();
            break;
        case Action::Settings:
            if (ctx.cmd.settings) ctx.cmd.settings();
            break;
        default: break; // the panels handle the rest (focus, X-ray, autocomplete, …)
        }
    }
}

void Shell::draw(UiContext& ctx) {
    if (ImGui::GetCurrentContext() == nullptr || ctx.theme == nullptr || ctx.fonts == nullptr) return;
    configureImGui();
    applyPending();
    ...
    // Simulator-only panels — the Bloch markers above the transmon pads are the probe of spec 00 §6.
    if (ctx.overlays != nullptr) ctx.overlays->setEnabled(ctx.overlays->temperatureTint(), true, !physicalLab_, true);
    if (!ctx.cmd.setBusy) ctx.cmd.setBusy = [this](std::string_view msg, double p) { setBusy(msg, p); };
