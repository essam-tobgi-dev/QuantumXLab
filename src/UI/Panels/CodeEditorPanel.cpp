...
public:
    CodeEditorPanel()
        : BasicPanel(PanelId::CodeEditor, "code_editor", "panels.code_editor", "¶", Workspace::Program) {}
...
void CodeEditorPanel::draw(UiContext& ctx) {
    ...
    if (ctx.diagnostics != nullptr) editor_.model().setDiagnostics(*ctx.diagnostics);
    ...
    if (!ctx.cmd.openProgram)
        ctx.cmd.openProgram = [this](std::string source, std::string) { editor_.model().setSource(std::move(source)); };
    ...
    if (editor_.model().errorCount() > 0)
        widgets::badge(ctx, std::to_string(editor_.model().errorCount()) + " errors", ctx.th()[Token::Err]);
    else if (editor_.model().warningCount() > 0)
        widgets::badge(ctx, std::to_string(editor_.model().warningCount()) + " warnings", ctx.th()[Token::Warn]);
    else
        widgets::badge(ctx, "OK", ctx.th()[Token::Ok]);
    ImGui::SameLine(0.0f, m.spacing(3));
    widgets::text(ctx, Token::TextSecondary,
                  "Ln " + std::to_string(editor_.caret().line) + ", Col " + std::to_string(editor_.caret().column));
    ImGui::Separator();
    ...
    if (changed && ctx.incremental != nullptr && editor_.model().doc().revision() != submittedRevision_) {
...
public:
    DiagnosticsPanel()
        : BasicPanel(PanelId::Diagnostics, "diagnostics", "panels.diagnostics", "⚠", Workspace::Program) {}
    ...
    void deserialize(const core::Json& j) override {
        if (j.is_object())
            if (const auto it = j.find("show_info"); it != j.end() && it->is_boolean()) showInfo_ = it->get<bool>();
    }
...
void DiagnosticsPanel::draw(UiContext& ctx) {
    ...
    for (const lang::Diagnostic& d : *ctx.diagnostics) {
        if (d.severity == lang::Severity::Error) ++errors;
        else if (d.severity == lang::Severity::Warning) ++warnings;
    }
    ...
    if (!ImGui::BeginTable("##diags", 4, widgets::tableFlags(false))) return;
    ...
    int row = 0;
    for (const lang::Diagnostic& d : *ctx.diagnostics) {
        if (!showInfo_ && d.severity == lang::Severity::Info) continue;
        ImGui::PushID(row++);
        ImGui::TableNextRow();
        ...
        if (ImGui::IsItemClicked() && d.error.span && ctx.cmd.gotoSource) ctx.cmd.gotoSource(*d.error.span);
        ImGui::TableNextColumn();
        widgets::text(ctx, Token::TextPrimary, d.error.message);
        if (ImGui::IsItemClicked() && d.error.span && ctx.cmd.gotoSource) ctx.cmd.gotoSource(*d.error.span);
        ImGui::TableNextColumn();
        if (!d.fix.empty()) widgets::text(ctx, Token::Ok, d.fix);
        else if (!d.error.notes.empty()) widgets::text(ctx, Token::TextSecondary, d.error.notes.front());
        ImGui::PopID();
    }
    ImGui::EndTable();
}
...
PanelPtr makeCodeEditorPanel() { return std::make_unique<CodeEditorPanel>(); }
PanelPtr makeDiagnosticsPanel() { return std::make_unique<DiagnosticsPanel>(); }
