...
public:
    CircuitPanel() : BasicPanel(PanelId::Circuit, "circuit", "panels.circuit", "◇", Workspace::Program) {}
    ...
    void deserialize(const core::Json& j) override {
        ...
        if (const auto it = j.find("stage"); it != j.end() && it->is_string())
            for (std::size_t s = 0; s < viz::kCircuitStageCount; ++s)
                if (viz::circuitStageName(static_cast<viz::CircuitStage>(s)) == it->get<std::string>())
                    view_.setStage(static_cast<viz::CircuitStage>(s));
        if (const auto it = j.find("timed"); it != j.end() && it->is_boolean()) view_.setTimed(it->get<bool>());
        if (const auto it = j.find("zoom"); it != j.end() && it->is_number()) view_.setZoom(it->get<double>());
    }
...
void CircuitPanel::draw(UiContext& ctx) {
    ...
    for (std::size_t s = 0; s < viz::kCircuitStageCount; ++s) {
        ...
        if (widgets::toggleChip(ctx, viz::circuitStageName(stage), &on) && on) view_.setStage(stage);
    }
    ImGui::SameLine(0.0f, ctx.metrics_px().spacing(4));
    bool timed = view_.timed();
    if (widgets::toggleChip(ctx, "Timed", &timed)) view_.setTimed(timed);
    // `shownStage` reports what was actually drawn when the compiler produced no such stage.
    if (view_.shownStage() != view_.stage()) {
        ImGui::SameLine();
        widgets::badge(ctx, std::string("showing ") + std::string(viz::circuitStageName(view_.shownStage())),
                       ctx.th()[Token::Warn]);
    }
    if (view_.minimap()) {
        ImGui::SameLine();
    ...
    if (widgets::secondaryButton(ctx, "SVG") && ctx.cmd.requestExport) {
        ...
        request.format = viz::ExportRequest::Format::Csv; // the SVG travels in `csv` as text (spec 23 §8)
    ...
    if (ctx.assets == nullptr || !ImGui::IsWindowHovered(ImGuiHoveredFlags_ChildWindows)) return;
    const ImVec2 mouse = ImGui::GetIO().MousePos;
    const ImVec2 origin = ImGui::GetItemRectMin();
    const auto hit = view_.hitTest(glm::vec2(mouse.x - origin.x, mouse.y - origin.y));
    if (!hit || hit->kind != viz::HitKind::Gate || !ImGui::BeginTooltip()) return;
...
public:
    PulsePanel() : BasicPanel(PanelId::Pulses, "pulses", "panels.pulses", "⬒", Workspace::Program) {}
    ...
    void deserialize(const core::Json& j) override {
        if (!j.is_object()) return;
        if (const auto it = j.find("polar"); it != j.end() && it->is_boolean()) view_.setPolar(it->get<bool>());
...
void PulsePanel::draw(UiContext& ctx) {
    ...
    if (widgets::toggleChip(ctx, "Amplitude / phase", &polar)) view_.setPolar(polar);
    ImGui::SameLine();
    if (widgets::secondaryButton(ctx, "Whole schedule")) view_.setWindow(0.0, 0.0);
    ImGui::SameLine();
...
PanelPtr makeCircuitPanel() { return std::make_unique<CircuitPanel>(); }
PanelPtr makePulsePanel() { return std::make_unique<PulsePanel>(); }
