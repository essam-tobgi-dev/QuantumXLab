...
class InspectorPanel final : public BasicPanel {
  public:
    InspectorPanel()
        : BasicPanel(PanelId::Inspector, "inspector", "panels.inspector", "◆", Workspace::Lab) {}
...
void InspectorPanel::drawPhysics(UiContext& ctx, const lab::Inspectable& what,
                                 const lab::Node& node) {
    if (what.equationIds.empty() || ctx.assets == nullptr)
        return;
    widgets::sectionHeader(ctx, ctx.text("inspector.physics"));
    if (!what.physicsSummary.empty())
        widgets::textWrapped(ctx, Token::TextSecondary, what.physicsSummary);
        ...
        if (ctx.bindings != nullptr) {
            for (const EquationTerm& term : doc->terms) {
                for (const lab::SpecRow& row : what.specSheet) {
                    ...
                    v.available =
                        resolved.has_value() && resolved->available() && resolved->isNumber();
                    if (v.available) {
    ...
    // Spec 19 §5.4: every displayed quantity carries a fidelity class; the equation carries its
    // own.
    ImGui::SameLine();
...
void InspectorPanel::drawSpecSheet(UiContext& ctx, const lab::Inspectable& what,
                                   const lab::Node& node) {
    if (what.specSheet.empty())
        return;
    widgets::sectionHeader(ctx, ctx.text("inspector.spec_sheet"));
    if (!ImGui::BeginTable("##spec", 4, widgets::tableFlags(false), ImVec2(0.0f, 0.0f)))
        return;
    ...
    for (const lab::SpecRow& row : what.specSheet) {
        ...
        if (ctx.physicalLab && row.simulatorOnly)
            continue;
        std::optional<lab::BindingValue> value;
        if (ctx.bindings != nullptr)
            value = ctx.bindings->resolve(node, row);
        const std::string shown = lab::formatBindingValue(value, row.unit);
        const double si = value && value->isNumber() ? value->asNumber()
...
void InspectorPanel::draw(UiContext& ctx) {
    ...
    widgets::text(ctx, Token::TextSecondary, ctx.scene->breadcrumbText(id));
    {
        ...
        widgets::text(ctx, Token::TextPrimary,
                      what.displayName.empty() ? what.name : what.displayName);
    }
    ...
    if (node->can) {
        ...
        if (open ? widgets::secondaryButton(ctx, "Close the cans")
                 : widgets::primaryButton(ctx, "Open — show the interior"))
            ctx.interaction->setCansVisible(open);
        ImGui::SameLine();
        bool cut = ctx.interaction->cutaway();
        if (widgets::toggleChip(ctx, "Cutaway", &cut))
            ctx.interaction->setCutaway(cut, ctx.interaction->cutawayAngleDeg());
        ImGui::SameLine();
        bool xray = ctx.interaction->xray();
        if (widgets::toggleChip(ctx, "X-ray", &xray))
            ctx.interaction->setXray(xray);
    }
    ...
    if (!what.children.empty()) {
        ...
        for (ComponentId child : what.children) {
            ...
            widgets::text(ctx, Token::TextSecondary,
                          c->displayName.empty() ? c->instanceName : c->displayName);
            if (ImGui::IsItemClicked()) {
                ctx.interaction->select(child);
                if (ctx.selection != nullptr)
                    ctx.selection->selectComponent(child);
            }
        }
    }
}
