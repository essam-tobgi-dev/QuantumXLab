...
class ViewportPanel final : public BasicPanel {
  public:
    ...
    void deserialize(const core::Json& j) override {
        ...
        if (const auto it = j.find("show_legend"); it != j.end() && it->is_boolean())
            showLegend_ = it->get<bool>();
        if (const auto it = j.find("show_help"); it != j.end() && it->is_boolean())
            showHelp_ = it->get<bool>();
        if (const auto it = j.find("invert_orbit"); it != j.end() && it->is_boolean())
            invertOrbit_ = it->get<bool>();
...
void ViewportPanel::drawToolbar(UiContext& ctx) {
    ...
    auto buttonWidth = [&](std::string_view label) {
        return widgets::textSize(label).x + 2.0f * pad;
    };
    ...
    for (const lab::LayoutBookmark& b : ui->bookmarks()) {
        if (shown >= 9)
            break; // digits 1–9 recall them (spec 19 §5)
        const std::string label = std::to_string(shown + 1) + " " + b.name;
        place(buttonWidth(label));
        if (widgets::secondaryButton(ctx, label) && ctx.camera != nullptr)
            ui->applyBookmark(b.name, *ctx.camera);
        ++shown;
    }
    ...
    if (ctx.overlays != nullptr) { // spec 17 §8: the stage-temperature tint is opt-in
        ...
        if (widgets::toggleChip(ctx, "Temp", &temp))
            ctx.overlays->setTemperatureTint(temp);
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip(
                "Colour the stages by their live temperature (inferno, 10 mK – 300 K)");
    }
    bool xray = ui->xray();
    place(buttonWidth("X-ray"), ctx.overlays != nullptr ? -1.0f : m.spacing(4));
    if (widgets::toggleChip(ctx, "X-ray", &xray))
        ui->setXray(xray);
    ...
    if (ui->cutaway()) {
        ...
        double offset =
            glm::radians(cutawayOffsetDeg_); // the field works in SI (rad), shows degrees
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Mouse and keyboard controls");
    if (ctx.devMode) {
...
void ViewportPanel::drawImage(UiContext& ctx) {
    ...
    if (std::fabs(avail.x - lastImageSize_.x) > 0.5f ||
        std::fabs(avail.y - lastImageSize_.y) > 0.5f) {
        lastImageSize_ = avail;
        stableFrames_ = 0;
    } else if (++stableFrames_ == 1 && ctx.cmd.resizeViewport) {
        ctx.cmd.resizeViewport(static_cast<int>(avail.x * ctx.dpiScale),
                               static_cast<int>(avail.y * ctx.dpiScale));
    }
    ...
    if (ctx.renderer != nullptr) // origin bottom-left in GL, top-left in ImGui
        dl->AddImage(static_cast<ImTextureID>(ctx.renderer->colorTexture().id()), origin, imageMax,
    ...
    if (ctx.interaction != nullptr)
        ctx.interaction->hover(hovered ? hoverId_ : ComponentId{0}, ctx.timeS);
    ...
    if (gfx::Camera* cam = ctx.camera; cam != nullptr) {
        ...
        for (int b = 0; b < 3; ++b) {
            ...
            if (hoverHit_)
                cam->setPivot(hoverWorld_);
            if (b == ImGuiMouseButton_Left)
                pressedInside_ = true;
        }
        if (active) {
            ...
            const bool pan = ImGui::IsMouseDown(ImGuiMouseButton_Right) ||
                             ImGui::IsMouseDown(ImGuiMouseButton_Middle) || (left && io.KeyShift);
            if (pan)
                cam->pan(d.x, d.y, static_cast<int>(avail.y));
            else if (left && io.KeyAlt)
                cam->rotateInPlace(d.x, d.y);
            ...
            // mouse (drag down → what is under the cursor moves down). "Invert orbit" flips both.
                ...
                cam->orbit(invertOrbit_ ? -d.x : d.x, invertOrbit_ ? d.y : -d.y);
        }
        ...
        if (hovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) &&
            ...
            if (hoverId_.value != 0)
                ctx.interaction->focus(hoverId_, *cam);
    ...
    if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
        ...
        if (pressedInside_ && hovered && travel < ctx.ui(4.0f) && ctx.interaction != nullptr) {
            ...
            if (ctx.cmd.selectComponent)
                ctx.cmd.selectComponent(hoverId_);
        }
        pressedInside_ = false;
    }
    ...
    if (hovered && !active)
        drawHoverCard(ctx);
    if (showLegend_ && ctx.overlays != nullptr && ctx.overlays->temperatureTint())
        drawLegend(ctx, origin, imageMax);
    if (showHelp_)
        drawHelp(ctx, origin, imageMax);
    if (ctx.tour != nullptr)
        drawTourOverlay(ctx, origin, imageMax);
}

void ViewportPanel::handleKeys(UiContext& ctx, bool pointerInside) {
    if (ctx.interaction == nullptr || ctx.camera == nullptr)
        return;
    if (!ImGui::IsWindowFocused(ImGuiFocusedFlags_ChildWindows) && !pointerInside)
        return;
    const ImGuiIO& io = ImGui::GetIO();
    if (io.WantTextInput)
        return;
    ...
    if (const int digit = pollBookmarkDigit(io.WantTextInput); digit != 0) {
        ...
        if (digit < 0)
            ctx.interaction->storeBookmark("user " + std::to_string(n), cam);
        else if (n <= marks.size())
            ctx.interaction->applyBookmark(marks[n - 1].name, cam);
    }
    if (ImGui::IsKeyPressed(ImGuiKey_F, false))
        ctx.interaction->focus(ctx.interaction->selected(), cam);
    if (ImGui::IsKeyPressed(ImGuiKey_X, false))
        ctx.interaction->setXray(!ctx.interaction->xray());
    ...
    if (ImGui::IsKeyPressed(ImGuiKey_E, false) && !io.KeyShift) // Shift+E flies up
        ctx.interaction->setExplode(
            lab::Assembly::FridgeStages,
            ctx.interaction->explode(lab::Assembly::FridgeStages) > 0.5 ? 0.0 : 1.0);
    if (ImGui::IsKeyPressed(ImGuiKey_T, false) && ctx.cmd.openTheory) {
        if (const auto tip = ctx.interaction->tooltip(1e300); tip && !tip->theory.empty())
            ctx.cmd.openTheory(tip->theory);
    }
    ...
    if (ImGui::IsKeyDown(ImGuiKey_A))
        move.x -= 1.0;
    if (ImGui::IsKeyDown(ImGuiKey_E) && io.KeyShift)
        move.y += 1.0;
...
void ViewportPanel::drawContextMenu(UiContext& ctx) {
    ...
    lab::Interaction& ui = *ctx.interaction;
    const lab::Node* node =
        ctx.scene != nullptr && contextId_.value != 0 ? ctx.scene->node(contextId_) : nullptr;
    if (node != nullptr) {
        widgets::text(ctx, Token::TextSecondary, node->displayName);
        ImGui::Separator();
        if (node->can) {
            const bool open = !ui.cansVisible();
            if (ImGui::MenuItem(open ? "Close the cans" : "Open — show the interior"))
                ui.setCansVisible(open);
        }
        if (ImGui::MenuItem("Inspect")) {
            ui.select(contextId_);
            if (ctx.cmd.selectComponent)
                ctx.cmd.selectComponent(contextId_);
        }
        if (ImGui::MenuItem("Fly to") && ctx.camera != nullptr)
            ui.focus(contextId_, *ctx.camera);
        if (const auto tip = ui.tooltip(1e300);
            tip && tip->id == contextId_ && !tip->theory.empty())
            if (ImGui::MenuItem(("Theory " + anchorLabel(tip->theory)).c_str()) &&
                ctx.cmd.openTheory)
                ctx.cmd.openTheory(tip->theory);
        ImGui::Separator();
    }
    bool cans = ui.cansVisible();
    if (ImGui::MenuItem("Cans visible", nullptr, &cans))
        ui.setCansVisible(cans);
    bool cut = ui.cutaway();
    if (ImGui::MenuItem("Cutaway", "C", &cut) && ctx.camera != nullptr)
        ui.setCutaway(cut, cameraAzimuthDeg(*ctx.camera) + cutawayOffsetDeg_);
    bool xray = ui.xray();
    if (ImGui::MenuItem("X-ray", "X", &xray))
        ui.setXray(xray);
    bool exploded = ui.explode(lab::Assembly::FridgeStages) > 0.5;
    if (ImGui::MenuItem("Exploded stages", "E", &exploded))
        ui.setExplode(lab::Assembly::FridgeStages, exploded ? 1.0 : 0.0);
    ImGui::Separator();
...
void ViewportPanel::drawHoverCard(UiContext& ctx) {
    ...
    ImGui::SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f),
                                        ImVec2(width + 2.0f * m.spacing(3), FLT_MAX));
    ...
    ImGui::PushTextWrapPos(width);
    {
        ...
        widgets::text(ctx, Token::TextPrimary,
                      tip->displayName.empty() ? tip->name : tip->displayName);
    }
    ...
    if (!tip->category.empty())
        widgets::badge(ctx, tip->category, ctx.th()[Token::AccentSoft]);
    if (tip->simulatorOnly) {
        ImGui::SameLine();
        widgets::simOnlyBadge(ctx);
    }
    if (!tip->text.empty())
        widgets::textWrapped(ctx, Token::TextPrimary, tip->text);
    ...
    if (const lab::Node* n = ctx.scene != nullptr ? ctx.scene->node(tip->id) : nullptr;
        n != nullptr && n->can)
        footer = "Right-click: open the can  ·  " + footer;
    if (!tip->theory.empty())
        footer += "  ·  T: theory " + anchorLabel(tip->theory);
...
void ViewportPanel::drawHelp(UiContext& ctx, ImVec2 imageMin, ImVec2 imageMax) {
    ...
    for (const auto& [key, what] : kRows)
        keyW = std::max(keyW, widgets::textSize(key).x);
...
void ViewportPanel::drawLegend(UiContext& ctx, ImVec2 imageMin, ImVec2 imageMax) {
    ...
    const ImU32 label =
        u32(Theme::readableText(ctx.th()[Token::TextSecondary], ctx.th()[Token::BgViewport]));
    dl->AddText(ImVec2(a.x - widgets::textSize(hi).x - 4.0f, a.y), label, hi.c_str());
    dl->AddText(ImVec2(a.x - widgets::textSize(lo).x - 4.0f, a.y + h - ImGui::GetTextLineHeight()),
                label, lo.c_str());
}
