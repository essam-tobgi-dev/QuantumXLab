...
class UndoStack {
  public:
    ...
    // Pushes an already-applied edit (the caller changed the model, then records how to reverse
    // it). Clears the redo branch. A command without both closures is ignored.
    void push(Command c);
    // Convenience for the common "one value changed" case.
    template <class T>
    void pushValue(std::string label, T* target, T oldValue, T newValue,
                   std::string mergeKey = {}) {
        if (target == nullptr || oldValue == newValue)
            return;
