...
constexpr std::array<NameEntry, kTokenCount> kNames{
...
constexpr std::array<Token, 11> kTextTokens{Token::TextPrimary,
                                            Token::TextSecondary,
                                            Token::Accent,
                                            Token::Ok,
                                            Token::Warn,
                                            ...
                                            Token::SimOnly,
                                            Token::ClassExact,
                                            Token::ClassNumerical,
                                            Token::ClassStatistical,
                                            Token::ClassIllustrative};
...
constexpr std::array<std::string_view, kTokenCount> kDarkHex{
    "#0F1115", "#161A20", "#1E232B",   "#0A0C10", "#2A3039", "#E6E9EF", "#9AA3B2",
    "#5F6875", "#4FA3FF", "#4FA3FF33", "#3DD68C", "#F5B841", "#FF5D5D", "#C084FC",
    "#3DD68C", "#4FA3FF", "#F5B841",   "#FF9F43", "#9AA3B2"};
constexpr std::array<std::string_view, kTokenCount> kLightHex{
    "#F4F5F7", "#FFFFFF", "#EEF0F3",   "#DDE1E6", "#C9CED6", "#1A1D22", "#5B6472",
    "#9AA3B2", "#1F6FEB", "#1F6FEB22", "#1B8F5A", "#B7791F", "#C62828", "#7C3AED",
    "#1B8F5A", "#1F6FEB", "#B7791F",   "#C2410C", "#5B6472"};
...
double Theme::relativeLuminance(const Color& c) {
    return viz::math::relativeLuminance(glm::vec3(c));
}
...
Result<Color> Theme::parseHex(std::string_view hex) {
    if (auto c = viz::math::colorFromHex(hex))
        return *c;
    return fail(err::BadToken,
                std::string("malformed colour '") + std::string(hex) + "' (expected #RRGGBB[AA])");
}
...
Color Theme::readableText(const Color& fg, const Color& bg, double minRatio) {
    ...
    if (contrastRatio(Color(target, fg.a), bg) < minRatio)
        return Color(target, fg.a); // unreachable: best effort
...
Theme Theme::fallback(bool dark) {
    ...
    for (std::size_t i = 0; i < kTokenCount; ++i)
        t.colors_[i] = hexOrMagenta(table[i]);
    for (std::size_t i = 0; i < kOkabeIto.size(); ++i)
        t.qubits_[i] = hexOrMagenta(kOkabeIto[i]);
    return t;
}
...
Result<Theme> Theme::fromJson(const core::Json& themeData, std::string_view palette) {
    ...
    if (!themeData.is_object())
        return fail(err::BadAsset, "theme: `data` is not an object");
    ...
    if (entry == palettes->end() || !entry->is_object())
        return fail(err::BadAsset,
                    std::string("theme: missing `data.palettes.") + std::string(palette) + "`");
    for (const NameEntry& n : kNames) {
        const auto it = entry->find(n.name);
        if (it == entry->end())
            continue; // a missing token keeps its §1 value
        if (!it->is_string())
            return fail(err::BadToken,
                        std::string("theme: token `") + std::string(n.name) + "` is not a string");
    ...
    if (const auto s = themeData.find("space_px"); s != themeData.end() && s->is_array()) {
        for (std::size_t i = 0; i < t.metrics_.space.size() && i < s->size(); ++i)
            if ((*s)[i].is_number())
                t.metrics_.space[i] = (*s)[i].get<float>();
    }
    if (const auto ty = themeData.find("typography"); ty != themeData.end() && ty->is_object()) {
        if (const auto u = ty->find("ui"); u != ty->end()) {
            ...
            t.metrics_.workspaceTitlePx =
                jsonFloat(*u, "workspace_title_px", t.metrics_.workspaceTitlePx);
        }
