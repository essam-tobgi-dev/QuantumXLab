...
constexpr std::array<std::string_view, 17> kPragmaKeys{
    "qlab.device",  "qlab.backend", "qlab.shots",       "qlab.seed",     "qlab.noise",  "qlab.layout",
    "qlab.routing", "qlab.optimize", "qlab.sweep",      "qlab.probe",    "qlab.rb",     "qlab.pulse_level",
    "qlab.snapshot", "qlab.alignment", "qlab.assert",   "qlab.noise_file", "qlab.compare_devices"};
...
std::span<const std::string_view> pragmaKeys() { return kPragmaKeys; }
...
std::string_view completionKindName(CompletionKind k) {
    ...
    case CompletionKind::Gate: return "gate";
    case CompletionKind::Builtin: return "builtin";
    case CompletionKind::UserGate: return "gate (user)";
    case CompletionKind::Defcal: return "defcal";
    case CompletionKind::Qubit: return "qubit";
    case CompletionKind::Register: return "register";
    case CompletionKind::Pragma: return "pragma";
    }
    return "?";
}
...
void EditorModel::setDiagnostics(std::vector<lang::Diagnostic> diagnostics) {
    ...
    for (const lang::Diagnostic& d : diagnostics_) {
        if (d.severity == lang::Severity::Error) ++errors_;
        else if (d.severity == lang::Severity::Warning) ++warnings_;
        Marker m;
    ...
    std::stable_sort(markers_.begin(), markers_.end(),
                     [](const Marker& a, const Marker& b) { return std::tie(a.line, a.begin) < std::tie(b.line, b.begin); });
    markerRevision_ = doc_.revision();
}

void EditorModel::analyse() const {
    if (analysedRevision_ == doc_.revision() && !highlights_.empty()) return;
    ...
    for (std::size_t i = 0; i < tokens_.size(); ++i) {
        ...
        if ((isGate || isDefcal) && i + 1 < tokens_.size()) {
            ...
            if (name.kind == lang::TokenKind::Identifier || name.kind == lang::TokenKind::Gate) {
                Definition d;
                d.name = name.text;
                d.at = Position{std::max<std::uint32_t>(1, name.span.line), std::max<std::uint32_t>(1, name.span.column)};
                d.defcal = isDefcal;
                d.signature = std::string(isDefcal ? "defcal " : "gate ") + name.text;
                definitions_.push_back(d);
                declared_.push_back(Completion{d.name, isDefcal ? CompletionKind::Defcal : CompletionKind::UserGate,
                                               d.signature});
            }
        }
        // `qubit[n] q;` / `bit[n] c;` / `qubit q;` declare completion candidates.
        if (t.kind == lang::TokenKind::Type && (t.text == "qubit" || t.text == "bit" || t.text == "int" ||
                                                t.text == "float" || t.text == "angle" || t.text == "complex")) {
            std::size_t j = i + 1;
            if (j < tokens_.size() && tokens_[j].isText("[")) { // skip the [size] part
                int depth = 0;
                while (j < tokens_.size()) {
                    if (tokens_[j].isText("[")) ++depth;
            ...
            if (j < tokens_.size() && tokens_[j].kind == lang::TokenKind::Identifier)
                declared_.push_back(Completion{tokens_[j].text,
                                               t.text == "qubit" ? CompletionKind::Qubit : CompletionKind::Register,
                                               t.text});
        }
    }
    ...
    for (const lang::Token& t : tokens_) {
        if (t.kind == lang::TokenKind::Eof) continue;
        HighlightSpan h;
        ...
        if (h.kind == lang::TokenKind::Identifier) {
            if (lang::StdGates::isStd(t.text)) h.kind = lang::TokenKind::Gate;
            else
    ...
    for (std::size_t i = highlights_.size(); i-- > 0;) {
        ...
        if (l < lineIndex_.size()) lineIndex_[l] = i;
    }
    for (std::size_t l = lineIndex_.size(); l-- > 1;)
        if (lineIndex_[l - 1] > lineIndex_[l]) lineIndex_[l - 1] = lineIndex_[l];
}
...
std::span<const HighlightSpan> EditorModel::highlightsOn(std::uint32_t line) const {
    analyse();
    if (line == 0 || line + 1 >= lineIndex_.size()) return {};
    const std::size_t begin = lineIndex_[line];
    std::size_t end = begin;
    while (end < highlights_.size() && highlights_[end].line == line) ++end;
    return std::span<const HighlightSpan>(highlights_).subspan(begin, end - begin);
}

std::span<const Marker> EditorModel::markers() const { return markers_; }
...
std::vector<const Marker*> EditorModel::markersOn(std::uint32_t line) const {
    ...
    for (const Marker& m : markers_)
        if (m.line == line) out.push_back(&m);
    return out;
}
...
std::optional<Position> EditorModel::matchingBracket(Position p) const {
    ...
    if (kOpenBrackets.find(here) == std::string_view::npos && kCloseBrackets.find(here) == std::string_view::npos) {
    ...
    if (forward) {
        for (std::size_t i = at; i < text.size(); ++i) {
            if (text[i] == here) ++depth;
            else if (text[i] == want && --depth == 0) return doc_.positionOf(i);
        }
    } else {
...
std::string EditorModel::indentAfter(std::uint32_t line) const {
    ...
    std::size_t spaces = 0;
    while (spaces < s.size() && (s[spaces] == ' ' || s[spaces] == '\t')) ++spaces;
...
std::vector<Completion> EditorModel::completions(Position p) const {
    ...
    const auto add = [&](std::string name, CompletionKind kind, std::string detail) {
        if (!prefix.empty() && !std::string_view(name).starts_with(prefix)) return;
        if (std::find_if(out.begin(), out.end(), [&](const Completion& e) { return e.text == name; }) != out.end())
            return;
    ...
    if (head.find("pragma") != std::string_view::npos) {
        for (std::string_view key : kPragmaKeys) add(std::string(key), CompletionKind::Pragma, "pragma");
        return out;
    }
    for (const Completion& d : declared_) add(d.text, d.kind, d.detail);
    for (const lang::GateInfo& g : lang::StdGates::all())
        add(std::string(g.name), CompletionKind::Gate, std::string(g.signature));
    for (std::string_view kw : {"OPENQASM", "include", "gate", "def", "defcal", "cal", "if", "else", "for", "while",
                                "return", "measure", "reset", "barrier", "box", "delay", "input", "output", "const",
                                "let", "extern", "pragma"})
        add(std::string(kw), CompletionKind::Keyword, "keyword");
    for (std::string_view ty : {"int", "uint", "float", "angle", "bool", "bit", "duration", "stretch", "complex",
                                "qubit", "array"})
        add(std::string(ty), CompletionKind::Type, "type");
    for (std::string_view b : {"pi", "euler", "tau", "sin", "cos", "tan", "exp", "ln", "sqrt", "popcount", "rotl",
                               "rotr", "durationof", "sizeof"})
    ...
    std::stable_sort(out.begin(), out.end(), [](const Completion& a, const Completion& b) {
        const auto rank = [](CompletionKind k) {
            ...
            case CompletionKind::Register: return 0;
            case CompletionKind::UserGate:
            case CompletionKind::Defcal: return 1;
            case CompletionKind::Gate: return 2;
            case CompletionKind::Pragma: return 3;
            case CompletionKind::Builtin: return 4;
            case CompletionKind::Type: return 5;
