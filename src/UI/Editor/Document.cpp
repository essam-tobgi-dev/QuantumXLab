...
std::uint32_t Document::lineLength(std::uint32_t l) const {
    return codePoints(line(l));
}
...
void Document::record(Edit e) {
    ...
    if (mergeable_ && !done_.empty() && e.removed.empty() && e.inserted.size() == 1 &&
        e.inserted[0] != '\n') {
