...
void UndoStack::commit(Entry e) {
    ...
    if (!e.mergeKey.empty() && !done_.empty() && done_.back().mergeKey == e.mergeKey) {
        ...
        for (Command& c : e.members)
            into.members.push_back(std::move(c));
        return;
    }
    done_.push_back(std::move(e));
    if (done_.size() > capacity_)
        done_.erase(done_.begin(),
                    done_.begin() + static_cast<std::ptrdiff_t>(done_.size() - capacity_));
}
...
std::string_view UndoStack::undoLabel() const {
    return done_.empty() ? std::string_view{} : done_.back().label;
}
std::string_view UndoStack::redoLabel() const {
...
bool UndoStack::undo() {
    ...
    done_.pop_back();
    for (auto it = e.members.rbegin(); it != e.members.rend(); ++it)
        it->undo();
