...
std::size_t index(PanelId id) { return static_cast<std::size_t>(id); }
std::size_t index(Workspace w) { return static_cast<std::size_t>(w); }
...
Shell::Shell(std::vector<PanelPtr> panels) : panels_(std::move(panels)) {
    for (std::size_t w = 0; w < kWorkspaceCount; ++w) {
        ...
        for (PanelId id : p.panels) open_[w][index(id)] = true;
    }
...
void Shell::setWorkspace(Workspace w) {
    if (w == workspace_ || w >= Workspace::Count) return;
    captureIni();
    workspace_ = w;
    // Spec 19 §4: a workspace the user has arranged is restored from its ini; otherwise the
    // preset's DockBuilder script runs again.
    if (saved_[index(w)].customised && !saved_[index(w)].ini.empty()) applyIni_ = true;
    else rebuildDock_ = true;
}

void Shell::setPhysicalLab(bool on) { physicalLab_ = on; }

bool Shell::visible(const Panel& p) const {
    if (hiddenByPhysicalLab(p)) return false;
    return open_[index(workspace_)][index(p.id())];
}

void Shell::setOpen(PanelId id, bool on) { open_[index(workspace_)][index(id)] = on; }
bool Shell::isOpen(PanelId id) const { return open_[index(workspace_)][index(id)]; }
...
void Shell::captureIni() {
    ...
    into.open.clear();
    for (const PanelPtr& p : panels_)
        if (open_[index(workspace_)][index(p->id())]) into.open.emplace_back(p->key());
}
...
void Shell::resetLayout(Workspace w) {
    ...
    for (PanelId id : p.panels) open_[index(w)][index(id)] = true;
    if (w == workspace_) rebuildDock_ = true;
}

void Shell::setFontScale(float s) { fontScale_ = std::clamp(s, 0.8f, 1.6f); }   // spec 19 §7
...
LayoutState Shell::saveLayout() {
    ...
    for (std::size_t w = 0; w < kWorkspaceCount; ++w) {
        ...
        into.open.clear();
        for (const PanelPtr& p : panels_)
            if (open_[w][index(p->id())]) into.open.emplace_back(p->key());
    }
    for (const PanelPtr& p : panels_) {
        core::Json j = p->serialize();
        if (!j.is_null() && !j.empty()) s.panels[std::string(p->key())] = std::move(j);
    }
    return s;
}
...
Status Shell::loadLayout(LayoutState state) {
    ...
    for (std::size_t w = 0; w < kWorkspaceCount; ++w) {
        ...
        if (from.open.empty() && !from.customised) continue; // keep the preset's set
        open_[w].assign(kPanelCount, false);
        for (const std::string& key : from.open) {
            const auto id = panelFromKey(key);
            if (!id) return fail(err::UnknownPanel, "layout: unknown panel '" + key + "'");
            open_[w][index(*id)] = true;
        }
    }
    for (const PanelPtr& p : panels_) {
        const auto it = state.panels.find(p->key());
        if (it != state.panels.end()) p->deserialize(it->second);
    }
...
void Shell::buildDock(unsigned dockspaceId) {
    ...
    const auto dock = [&](PanelId id, ImGuiID node) {
        if (const Panel* p = panel(id); p != nullptr) ImGui::DockBuilderDockWindow(p->windowTitle().c_str(), node);
    };
    ...
    case Workspace::Count:
        ...
        dock(PanelId::Theory, rightBottom); // spec 19 §2 lists it in Analysis; without this it floats
