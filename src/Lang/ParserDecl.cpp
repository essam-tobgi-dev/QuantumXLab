...
StmtPtr Parser::parseVersion() {
    ...
    if (check(TokenKind::Number)) {
        ...
        if (v.version != "3.0" && v.version != "3") error("QL2003", sp, v.version);
    } else { expected("version number"); }
    expectPunct(';');
    sawVersion_ = true;
...
bool isUnterminatedString(const Token& t) {
    if (t.kind != TokenKind::Invalid || t.span.line != t.span.endLine) return false;
    std::uint32_t codePoints = 0;
    for (char c : t.text) if ((static_cast<unsigned char>(c) & 0xC0) != 0x80) ++codePoints;
...
StmtPtr Parser::parseInclude() {
    ...
    if (check(TokenKind::String)) {
        ...
        while (!path.empty() && (path.back() == ';' || path.back() == ' ' || path.back() == '\t' || path.back() == '\r'))
            path.pop_back();
...
StmtPtr Parser::parsePragma() {
    ...
    if (p.isQlab) p.name = p.name.substr(5);
    // args: tokens separated by whitespace, keeping {…} groups together
    std::string cur; int brace = 0;
    for (std::size_t k = j; k < s.size(); ++k) {
        char c = s[k];
        if (c == '{') ++brace; if (c == '}') --brace;
        if (std::isspace(static_cast<unsigned char>(c)) && brace == 0) { if (!cur.empty()) { p.args.push_back(cur); cur.clear(); } }
        else cur += c;
    }
    if (!cur.empty()) p.args.push_back(cur);
    return mk(std::move(p), t.span);
}

std::optional<ExprPtr> Parser::parseDesignator() {
    if (!checkPunct('[')) return std::nullopt;
    advance();
...
bool Parser::parseTypeSpec(TypeSpec& out) {
    if (!check(TokenKind::Type)) { expected("type"); return false; }
    const std::string& n = peek().text;
    static const std::pair<const char*, BaseType> map[] = {
        {"bit", BaseType::Bit}, {"bool", BaseType::Bool}, {"int", BaseType::Int}, {"uint", BaseType::Uint},
        {"float", BaseType::Float}, {"angle", BaseType::Angle}, {"duration", BaseType::Duration},
        {"stretch", BaseType::Stretch}, {"complex", BaseType::Complex}, {"qubit", BaseType::Qubit}};
    bool found = false;
    for (auto& [k, v] : map) if (n == k) { out.base = v; found = true; break; }
    if (!found) { expected("type"); return false; }
    advance();
    if (out.base == BaseType::Complex && checkPunct('[')) { // complex[float[64]]
        advance(); TypeSpec inner; parseTypeSpec(inner); expectPunct(']');
        return true;
    }
...
StmtPtr Parser::parseQubitDecl() {
    ...
    expectPunct(';');
    if (inGateBody_) error("QL2012", sp, "qubit declaration");
    return mk(std::move(d), spanFrom(sp));
}

StmtPtr Parser::parseClassicalDecl(IoKind io) {
    SourceSpan sp = here();
    ClassicalDecl d; d.io = io;
    if (!parseTypeSpec(d.type)) { synchronize(); return nullptr; }
    auto name = expectIdent("variable name");
    if (!name) { synchronize(); return nullptr; }
    d.name = *name;
    if (matchOp("=")) d.init = parseExprOrError();
    expectPunct(';');
    if (inGateBody_) error("QL2012", sp, "classical declaration");
    return mk(std::move(d), spanFrom(sp));
}
...
StmtPtr Parser::parseGateDef() {
    ...
    if (matchPunct('(')) { if (!checkPunct(')')) g.params = parseIdList(); expectPunct(')'); }
    g.qubits = parseIdList();
    bool saved = inGateBody_; inGateBody_ = true;
    g.body = parseBlock();
    inGateBody_ = saved;
...
StmtPtr Parser::parseDef() {
    ...
    if (expectPunct('(')) {
        while (!checkPunct(')') && !atEnd()) {
            DefParam p;
            if (checkText("qubit")) { advance(); p.isQubit = true; p.type.base = BaseType::Qubit; p.qubitSize = parseDesignator(); }
            else if (!parseTypeSpec(p.type)) { synchronize(); return nullptr; }
...
StmtPtr Parser::parseExtern() {
    ...
    if (checkText("port")) { // OpenPulse `extern port` outside cal: still an extern
        advance(); auto n = expectIdent("port name"); e.name = n.value_or("port"); expectPunct(';');
    ...
    synchronize();
    if (!detail::isPunct(toks_[pos_ > 0 ? pos_ - 1 : 0], ';')) expectPunct(';');
    return mk(std::move(e), spanFrom(sp));
}
