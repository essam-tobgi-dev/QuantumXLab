...
ParseResult parse(std::string_view text, std::string filename) {
    ...
    out.diagnostics.insert(out.diagnostics.end(), std::make_move_iterator(pd.begin()),
                           std::make_move_iterator(pd.end()));
    return out;
}
...
bool Parser::checkText(std::string_view t) const {
    return peek().kind != TokenKind::Eof && peek().isText(t);
}
...
bool Parser::matchText(std::string_view t) {
    if (checkText(t)) {
        advance();
        return true;
    }
...
bool Parser::matchOp(std::string_view t) {
    if (checkOp(t)) {
        advance();
        return true;
    }
...
bool Parser::expectPunct(char c, std::string_view what) {
    ...
    if (c == ';' && pos_ > 0 && !check(TokenKind::Invalid)) {
        ...
        const bool lineBreak =
            peek().span.line > prev.span.endLine && detail::startsStatement(peek());
        if (closes || lineBreak) {
            SourceSpan at = prev.span;
            at.line = at.endLine;
            at.column = at.endColumn;
            const bool wasPanic = panic_;
            error("QL2010", std::move(at), label,
                  atEnd()  ? "end of file"
                  : closes ? "'}'"
                           : "end of line");
            panic_ = wasPanic;
    ...
    for (std::size_t k = 0;; ++k) {
        ...
        if (t.kind == TokenKind::Eof || t.kind == TokenKind::Pragma)
            return false;
...
bool Parser::expectOp(std::string_view t) {
    if (matchOp(t))
        return true;
    expected(std::format("'{}'", t));
    return false;
}
bool Parser::expectKw(std::string_view t) {
    if (matchText(t))
        return true;
    expected(std::format("'{}'", t));
    return false;
...
SourceSpan Parser::spanFrom(const SourceSpan& start) const {
    ...
    s.endLine = prev.span.endLine;
    s.endColumn = prev.span.endColumn;
    return s;
}
...
void Parser::synchronize() {
    ...
    while (!atEnd()) {
        const Token& t = peek();
        if (depth == 0) {
            if (t.kind == TokenKind::Pragma || checkPunct('}'))
                return;
            if (pos_ > 0 && t.span.line > toks_[pos_ - 1].span.endLine &&
                detail::startsStatement(t))
                return;
        }
        if (t.kind == TokenKind::Punct && t.text.size() == 1) {
            const char p = t.text[0];
...
Ast Parser::parseProgram() {
    ...
    if (checkKw("OPENQASM")) {
        if (auto s = parseVersion())
            ast.statements.push_back(std::move(s));
    } else {
        error("QL2001", here(), "");
    }
    while (!atEnd()) {
        ...
        if (pos_ == before && !atEnd())
            advance(); // no progress: force one token
    }
    return ast;
}
