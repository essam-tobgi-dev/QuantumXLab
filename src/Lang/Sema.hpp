...
struct Symbol {
    ...
    std::optional<ConstValue>
        value; // for Const / folded Input defaults / loop vars during unroll (not here)
    SourceSpan span;
...
struct InputVar {
    std::string name;
    SemaType type;
    std::optional<ConstValue> defaultValue;
    bool hasSweep;
    SourceSpan span;
};
...
struct Program {
    ...
    bool hasMidCircuitMeasurement =
        false;                   // measurement followed by more quantum ops or feedforward
...
class Sema {
  ...
  private:
    struct Scope {
        ...
        bool isBlock = false;
        bool isGateBody = false;
        bool isDef = false;
    };
    template <class... A> void diag(std::string_view id, SourceSpan sp, A&&... a) {
        p_.diagnostics.push_back(Diagnostics::make(id, std::move(sp), std::forward<A>(a)...));
    }
    void pushScope(bool block = true, bool gate = false, bool def = false) {
        scopes_.push_back(Scope{{}, block, gate, def});
    }
    ...
    std::size_t checkQubitOperand(const Expr& e,
    ...
    std::map<std::string, std::vector<std::string>>
        gateCalls_; // gate/def -> callees (recursion check)
