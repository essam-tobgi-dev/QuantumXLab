...
}; // measure q  (as an expression)
struct DurationOfExpr {
    StmtList body;
};
...
struct SwitchStmt {
    ExprPtr subject;
}; // parsed for the diagnostic only (QL3095)
struct DefParam {
    TypeSpec type;
    std::string name;
    bool isQubit = false;
    std::optional<ExprPtr> qubitSize;
};
struct DefStmt {
    std::string name;
...
struct PulseWaveform {
    std::string kind;
    std::vector<ExprPtr> args;
}; // gaussian(amp, dur, sigma) | constant | drag | gaussian_square
struct PulsePortDecl {
    std::string name;
};
struct PulseFrameDecl {
    std::string name;
    std::string port;
    ExprPtr frequency;
    ExprPtr phase;
};
struct PulseWaveformDecl {
    std::string name;
    PulseWaveform waveform;
};
struct PulsePlay {
    std::string frame;
    std::variant<std::string, PulseWaveform> waveform;
};
struct PulseFrameOp {
    std::string op;
    std::string frame;
    ExprPtr value;
}; // set_frequency/shift_frequency/set_phase/shift_phase
struct PulseDelay {
    ExprPtr duration;
...
struct PulseStmt {
    std::variant<PulsePortDecl, PulseFrameDecl, PulseWaveformDecl, PulsePlay, PulseFrameOp,
                 PulseDelay, PulseCapture, PulseBarrier>
        node;
    SourceSpan span;
};
struct CalStmt {
...
struct Stmt {
    std::variant<VersionStmt, IncludeStmt, QubitDecl, ClassicalDecl, GateDecl, GateCall,
                 MeasureStmt, ResetStmt, BarrierStmt, DelayStmt, BoxStmt, AssignStmt, ExprStmt,
                 IfStmt, ForStmt, WhileStmt, SwitchStmt, DefStmt, ExternStmt, ReturnStmt, BreakStmt,
                 ContinueStmt, EndStmt, CalStmt, DefcalStmt, PragmaStmt>
        node;
    SourceSpan span;
    template <class T> const T* as() const { return std::get_if<T>(&node); }
    template <class T> T* as() { return std::get_if<T>(&node); }
