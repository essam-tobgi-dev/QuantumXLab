...
int exprDepth(const Expr& e, int cap) {
    ...
    auto sub = [&](const ExprPtr& c) {
        if (c && child < cap)
            child = std::max(child, exprDepth(*c, cap - 1));
    };
    std::visit(
        [&](const auto& n) {
            ...
            if constexpr (std::is_same_v<T, UnaryExpr>)
                sub(n.operand);
            else if constexpr (std::is_same_v<T, BinaryExpr> || std::is_same_v<T, ConcatExpr>) {
                sub(n.lhs);
                sub(n.rhs);
...
const OpInfo kOps[] = {
    {"||", 1, BinaryOp::Or, false},    {"&&", 2, BinaryOp::And, false},
    {"|", 3, BinaryOp::BitOr, false},  {"^", 4, BinaryOp::BitXor, false},
    {"&", 5, BinaryOp::BitAnd, false}, {"==", 6, BinaryOp::Eq, false},
    {"!=", 6, BinaryOp::Ne, false},    {"<", 7, BinaryOp::Lt, false},
    {"<=", 7, BinaryOp::Le, false},    {">", 7, BinaryOp::Gt, false},
    {">=", 7, BinaryOp::Ge, false},    {"<<", 8, BinaryOp::Shl, false},
    {">>", 8, BinaryOp::Shr, false},   {"+", 9, BinaryOp::Add, false},
    {"-", 9, BinaryOp::Sub, false},    {"*", 10, BinaryOp::Mul, false},
    {"/", 10, BinaryOp::Div, false},   {"%", 10, BinaryOp::Mod, false},
...
ExprPtr Parser::parseBinary(int minPrec) {
    ...
    for (;;) {
        ...
        if (concat ? minPrec > 1 : (!op || op->prec < minPrec))
            break;
        if (nesting_ + depth >= detail::kMaxDepth) {
            if (detail::lastDiagnosticIs(diags_, "QL2015"))
                panic_ = true;
            else
                error("QL2015", here(), detail::kMaxDepth);
            break;
        }
        SourceSpan sp = lhs->span;
        SourceSpan opSpan = advance().span;
        ExprPtr rhs =
            concat ? parseBinary(2) : parseBinary(op->rightAssoc ? op->prec : op->prec + 1);
        if (!rhs)
            rhs = mkE(ErrorExpr{}, opSpan); // keep the partial tree instead of discarding it
...
ExprPtr Parser::parsePrimary() {
    ...
    case TokenKind::Number: {
        advance();
        if (t.isImaginary)
            return mkE(ImagLit{t.fval}, sp);
        if (t.isFloat)
            return mkE(FloatLit{t.fval}, sp);
        return mkE(IntLit{t.ival}, sp);
    }
    case TokenKind::Duration:
        advance();
        return mkE(DurationLit{t.fval, t.unit}, sp);
    case TokenKind::BitString:
        advance();
        return mkE(BitStringLit{t.text}, sp);
    case TokenKind::String:
        advance();
        return mkE(BitStringLit{t.text}, sp);
    case TokenKind::PhysicalQubit:
        advance();
        return mkE(PhysicalQubitRef{static_cast<std::uint32_t>(t.ival)}, sp);
    case TokenKind::Keyword:
        if (t.text == "measure") {
            advance();
            MeasureExpr m;
            m.qubits = parseOperandList();
            return mkE(std::move(m), spanFrom(sp));
        }
        break;
    case TokenKind::Identifier:
        if (t.text == "true" || t.text == "false") {
            advance();
            return mkE(BoolLit{t.text == "true"}, sp);
        }
        [[fallthrough]];
    case TokenKind::Gate:
    ...
    case TokenKind::Builtin: {
        advance();
        if (detail::isBuiltinConstant(t.text))
            return mkE(ConstantRef{std::string(detail::canonicalConstant(t.text))}, sp);
