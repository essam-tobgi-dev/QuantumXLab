...
bool containsErrorExpr(const Expr& e) {
    ...
    return std::visit(
        [&](const auto& n) -> bool {
            ...
            if constexpr (std::is_same_v<T, ErrorExpr>)
                return true;
            else if constexpr (std::is_same_v<T, UnaryExpr>)
                return one(n.operand);
            else if constexpr (std::is_same_v<T, BinaryExpr> || std::is_same_v<T, ConcatExpr>)
                return one(n.lhs) || one(n.rhs);
            else if constexpr (std::is_same_v<T, CallExpr>)
                return any(n.args);
            else if constexpr (std::is_same_v<T, CastExpr>)
                return (n.type.width && one(*n.type.width)) || one(n.operand);
            else if constexpr (std::is_same_v<T, IndexExpr>)
                return one(n.base) || one(n.index);
            else if constexpr (std::is_same_v<T, RangeExpr>)
                return one(n.start) || (n.step && one(*n.step)) || one(n.stop);
            else if constexpr (std::is_same_v<T, SliceExpr>)
                return one(n.base) || one(n.range);
    ...
    return std::visit(
        [&](const auto& n) -> std::string {
            ...
            if constexpr (std::is_same_v<T, IntLit>)
                ...
                return std::string(unaryOpName(n.op)) + text(n.operand);
            else if constexpr (std::is_same_v<T, BinaryExpr>)
                return text(n.lhs) + " " + binaryOpName(n.op) + " " + text(n.rhs);
            else if constexpr (std::is_same_v<T, IndexExpr>)
                return text(n.base) + "[" + text(n.index) + "]";
            else if constexpr (std::is_same_v<T, RangeExpr>)
                return text(n.start) + ":" + (n.step ? text(*n.step) + ":" : "") + text(n.stop);
            else if constexpr (std::is_same_v<T, SliceExpr>)
                return text(n.base) + "[" + text(n.range) + "]";
            else if constexpr (std::is_same_v<T, MeasureExpr>) {
                std::string s = "measure";
                for (std::size_t i = 0; i < n.qubits.size(); ++i)
                    s += (i ? ", " : " ") + text(n.qubits[i]);
                return s;
            } else
                return dumpExpr(e);
