...
const std::vector<DiagInfo>& table() {
    static const std::vector<DiagInfo> t = {
        // lexical
        {"QL1001", S::Error, "invalid character '{}'",
         "remove the character or place it inside a comment"},
        ...
        // syntax
        {"QL2001", S::Error, "missing or misplaced 'OPENQASM 3.0;' header{}",
        ...
        {"QL2012", S::Error, "invalid statement inside gate body: {}",
         "gate bodies may contain only gate calls and barriers"},
        {"QL2013", S::Error, "invalid pulse statement '{}' inside cal/defcal block", ""},
        {"QL2014", S::Error, "malformed {} pragma: {}", ""},
        {"QL2015", S::Error, "nesting deeper than {} levels",
         "split the expression or block into smaller parts"},
        {"QL2050", S::Error, "unknown pragma 'qlab.{}'",
         "see spec 13 §7 for the list of qlab pragmas"},
        {"QL2051", S::Warning, "non-qlab pragma '{}' ignored", ""},
        // semantic
        {"QL3001", S::Error, "qubit '{}' declared after the first quantum statement",
         "move all qubit declarations before the first gate, measure or reset"},
        ...
        {"QL3012", S::Error, "'{}' shadows a global declaration inside a block",
         "rename the local variable"},
        {"QL3020", S::Error, "type mismatch: {}", ""},
        {"QL3021", S::Error, "physical qubit '{}' mixed with virtual qubits",
         "use 'pragma qlab.layout physical' and only $k operands, or only declared qubits"},
        {"QL3022", S::Error, "index {} out of range for '{}' of size {}", ""},
        {"QL3030", S::Warning, "measurement result of '{}' is discarded",
         "assign it to a bit: 'c = measure q;'"},
        ...
        {"QL3080", S::Error, "bit register '{}' used directly as a condition",
         "compare it: 'if (c == 3)'"},
        {"QL3090", S::Error,
         "unbounded 'while' loop: condition depends on no measurement and is not foldable", ""},
        ...
        {"QL3110", S::Warning,
         "defcal for '{}' defined after first use; earlier calls use the device calibration", ""},
        ...
        {"QL3178", S::Warning, "input '{}' has no sweep or default",
         "add 'pragma qlab.sweep' or provide a value at run time"},
        ...
        {"QL4020", S::Error, "loop expands to {} statements; bound is {}",
         "raise qlab.unroll_bound or reduce the loop"},
        ...
        {"QL4040", S::Error, "pulse-level execution requested above the Lindblad qubit cap ({})",
         ""},
        {"QL4050", S::Error, "'{}' has {} controls; the ancilla-free decomposition is limited to 8",
         "use ancilla qubits and ccx ladders"},
        {"QL4060", S::Info,
         "VF2 layout search exceeded its budget; falling back to the dense layout", ""},
        {"QL4070", S::Error, "gate '{}' cannot be decomposed to the native set of device '{}'", ""},
        {"QL4080", S::Error, "no calibration for '{}' on {}",
         "recalibrate the device or select another qubit/edge"},
        ...
        {"QL5011", S::Error, "circuit needs {} qubits; the largest available backend supports {}",
         ""},
        {"QL5012", S::Error, "memory budget exceeded: {}",
         "reduce shots, qubits or the backend's precision"},
...
const std::vector<DiagInfo>& Diagnostics::all() {
    return table();
}
...
ErrorCode Diagnostics::codeFor(std::string_view id) {
    unsigned n = 0;
    if (id.size() == 6 && id.starts_with("QL"))
        std::from_chars(id.data() + 2, id.data() + 6, n);
    switch (n / 1000) {
    case 1:
    case 2:
    case 3:
        return ErrorCode::Lang_ + (n % 1000);
    case 4:
        return ErrorCode::Compiler_ + (n % 1000);
    case 5:
        return ErrorCode::Runtime_ + (n % 1000);
    default:
        return ErrorCode::Parse;
    }
}

std::string Diagnostics::formatMessage(std::string_view tmpl,
