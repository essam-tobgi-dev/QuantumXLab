...
struct Cursor {
    ...
    char advance() {
        char c = src[pos++];
        if (c == '\n') { ++line; col = 1; }
        else if ((static_cast<unsigned char>(c) & 0xC0) != 0x80) ++col;
        return c;
    }
};

bool isDigit(char c) { return c >= '0' && c <= '9'; }
bool isNonAscii(char c) { return static_cast<unsigned char>(c) >= 0x80; }
...
std::optional<DurationUnit> unitOf(std::string_view s) {
    if (s == "ns") return DurationUnit::Ns;
    if (s == "us" || s == "\xC2\xB5s" || s == "\xCE\xBCs") return DurationUnit::Us; // us, µs, μs
    if (s == "ms") return DurationUnit::Ms;
    if (s == "s") return DurationUnit::S;
    if (s == "dt") return DurationUnit::Dt;
    return std::nullopt;
}
...
TokenKind classifyWord(std::string_view id) {
    if (isTypeKeyword(id)) return TokenKind::Type;
    if (isKeyword(id)) return TokenKind::Keyword;
    if (id == "U" || id == "gphase" || id == "CX") return TokenKind::Gate;
    if (isBuiltinName(id)) return TokenKind::Builtin;
    if (isPulseKeyword(id)) return TokenKind::CalBlock;
...
public:
    ...
    LexResult run() {
        while (!c_.eof()) {
            char ch = c_.peek();
            if (isSpace(ch)) { c_.advance(); continue; }
            start_ = c_.pos; line_ = c_.line; col_ = c_.col;
            if (ch == '/' && c_.peek(1) == '/') lineComment();
            else if (ch == '/' && c_.peek(1) == '*') blockComment();
            else if (text_.substr(c_.pos).starts_with("pragma") && !isIdentChar(c_.peek(6))) pragma();
            else if (ch == '"' || ch == '\'') stringLiteral();
            else if (ch == '$' && isDigit(c_.peek(1))) physicalQubit();
            else if (isDigit(ch) || (ch == '.' && isDigit(c_.peek(1)))) number();
            else if (isIdentStart(ch)) word();
            else symbol();
        }
        start_ = c_.pos; line_ = c_.line; col_ = c_.col;
        Token eof; eof.kind = TokenKind::Eof; eof.span = span();
...
private:
    SourceSpan span() const {
        SourceSpan s; s.line = line_; s.column = col_; s.endLine = c_.line; s.endColumn = c_.col;
    ...
    void push(Token t) { t.span = span(); out_.tokens.push_back(std::move(t)); }
    void comment() {
        if (!opts_.keepComments) return;
        Token t; t.kind = TokenKind::Comment; t.text = lexeme(); push(std::move(t));
    }
    ...
    void blockComment() {
        c_.advance(); c_.advance();
        bool closed = false;
        while (!c_.eof()) {
            if (c_.peek() == '*' && c_.peek(1) == '/') { c_.advance(); c_.advance(); closed = true; break; }
            c_.advance();
        }
        if (!closed) report("QL1002", "block comment");
        comment();
    }
    // `pragma` swallows the rest of the line; the token text is what follows the word (spec 13 §7).
    void pragma() {
        for (int i = 0; i < 6; ++i) c_.advance();
        while (!c_.eof() && (c_.peek() == ' ' || c_.peek() == '\t')) c_.advance();
        std::size_t ps = c_.pos;
        while (!c_.eof() && c_.peek() != '\n') c_.advance();
        Token t; t.kind = TokenKind::Pragma; t.text = std::string(text_.substr(ps, c_.pos - ps));
        while (!t.text.empty() && (t.text.back() == ';' || isSpace(t.text.back()))) t.text.pop_back();
        push(std::move(t));
    }
    ...
    void physicalQubit() {
        c_.advance(); // '$'
        while (isIdentChar(c_.peek())) c_.advance();
        Token t; t.text = lexeme(); t.kind = TokenKind::PhysicalQubit;
...
void Scanner::number() {
    ...
    bool isFloat = false;
    auto take = [&] {
        while (isDigit(c_.peek()) || c_.peek() == '_') { char d = c_.advance(); if (d != '_') digits += d; }
    };
    if (c_.peek() == '0' && std::string_view("xXbBoO").find(c_.peek(1)) != std::string_view::npos)
        return radixNumber();
    take();
    if (c_.peek() == '.' && isDigit(c_.peek(1))) { isFloat = true; digits += c_.advance(); take(); }
    else if (c_.peek() == '.' && !isIdentStart(c_.peek(1))) { isFloat = true; c_.advance(); digits += ".0"; }
    if ((c_.peek() == 'e' || c_.peek() == 'E') &&
        (isDigit(c_.peek(1)) || ((c_.peek(1) == '+' || c_.peek(1) == '-') && isDigit(c_.peek(2))))) {
        isFloat = true;
        digits += c_.advance();
        if (c_.peek() == '+' || c_.peek() == '-') digits += c_.advance();
        while (isDigit(c_.peek())) digits += c_.advance();
    }
    std::size_t sufStart = c_.pos;
    while (isIdentChar(c_.peek())) c_.advance();
    std::string_view suffix = text_.substr(sufStart, c_.pos - sufStart);

    Token t; t.text = lexeme(); t.kind = TokenKind::Number; t.isFloat = isFloat;
    std::string_view err;
    std::string arg = t.text;
    t.fval = std::strtod(digits.c_str(), nullptr);
    if (!std::isfinite(t.fval)) err = "QL1003";
    if (!isFloat) {
        auto r = std::from_chars(digits.data(), digits.data() + digits.size(), t.ival);
        if (r.ec != std::errc{} || r.ptr != digits.data() + digits.size()) err = "QL1003";
    }
    if (suffix.empty()) {
    } else if (suffix == "im") {
        t.isImaginary = true;
    } else if (auto unit = unitOf(suffix)) {
        t.kind = TokenKind::Duration; t.unit = *unit;
    } else if (suffix.back() == 's') {
        err = "QL1004"; arg = std::string(suffix);
    } else {
        err = "QL1003";
    }
    if (!err.empty()) { t.kind = TokenKind::Invalid; report(err, arg); }
    push(std::move(t));
}
...
void Scanner::radixNumber() {
    ...
    std::string digits;
    while (isIdentChar(c_.peek())) { char d = c_.advance(); if (d != '_') digits += d; }
    int radix = (base == 'x' || base == 'X') ? 16 : (base == 'b' || base == 'B') ? 2 : 8;
    Token t; t.text = lexeme(); t.kind = TokenKind::Number;
    std::uint64_t v = 0;
...
void Scanner::word() {
    while (isIdentChar(c_.peek())) c_.advance();
    Token t; t.text = lexeme();
    bool ascii = true;
    for (char ch : t.text) if (isNonAscii(ch)) ascii = false;
...
void Scanner::symbol() {
    static constexpr std::array<std::string_view, 30> ops = {"<<=", ">>=", "**=", "==", "!=", "<=",
        ...
        "+", "-", "*", "/", "%", "&", "|", "^"};
    for (auto o : ops) {
        if (!text_.substr(c_.pos).starts_with(o)) continue;
        for (std::size_t i = 0; i < o.size(); ++i) c_.advance();
        Token t; t.kind = TokenKind::Operator; t.text = std::string(o); push(std::move(t));
        return;
    }
    char ch = c_.advance();
    Token t; t.text = std::string(1, ch);
    if (std::string_view("~!<>=").find(ch) != std::string_view::npos) t.kind = TokenKind::Operator;
    else if (std::string_view("(){}[];,:@.?").find(ch) != std::string_view::npos) t.kind = TokenKind::Punct;
    else { t.kind = TokenKind::Invalid; report("QL1001", t.text); }
    push(std::move(t));
}

} // namespace

LexResult lex(std::string_view text, const LexOptions& opts) { return Scanner(text, opts).run(); }
