...
Status writeBinaryAtomic(const std::filesystem::path& path, std::span<const std::byte> bytes) {
    ...
    tmp += ".tmp";
    {
        ...
        if (!f)
            return fail(ErrorCode::Io, "write failed: " + tmp.string());
    }
...
std::string shapeTuple(std::span<const std::size_t> shape) {
    ...
    if (shape.size() == 1)
        s += ","; // NumPy writes 1-D shapes as `(N,)`
    s += ")";
    return s;
}
...
Status writeNpy(const std::filesystem::path& path, std::string_view descr,
    ...
    if (!data.empty())
        std::memcpy(bytes.data() + header.size(), data.data(), data.size());
...
Status writeNpyComplex(const std::filesystem::path& path, std::span<const num::Complex> values,
    ...
    if (expect != values.size())
        return fail(ErrorCode::InvalidArgument,
                    std::format("shape {} holds {} elements, the state has {}", shapeTuple(shape),
                                expect, values.size()));
...
static Status writeSidecar(const std::filesystem::path& base, const Json& data) {
    ensureKinds();
    if (!allFinite(data))
        return fail(ErrorCode::InvalidArgument, "state sidecar holds a non-finite number");
    std::filesystem::path p = base;
    p.replace_extension(".json");
    return core::writeTextFileAtomic(p,
                                     core::JsonEnvelope::serialize(kStateKind, data, kStateSchema));
}
...
Status exportDensityMatrix(const std::filesystem::path& base, const qsim::Snapshot& s,
    ...
    QXL_TRY(writeNpyComplex(npy, std::span<const num::Complex>(rho.data.data(), rho.data.size()),
                            shape));
    return writeSidecar(base, stateSidecar(s, ctx, npy.filename().generic_string(), "<c16", shape));
}
...
std::string tableauText(const qsim::TableauExport& t) {
    ...
    out += "# stabilizers\n";
    for (const auto& g : t.stabilizers)
        out += g + "\n";
    if (!t.destabilizers.empty()) {
        out += "# destabilizers\n";
        for (const auto& g : t.destabilizers)
            out += g + "\n";
    }
    return out;
}
...
Status exportTableau(const std::filesystem::path& base, const qsim::Snapshot& s,
    ...
    return writeSidecar(
        base, stateSidecar(s, ctx, txt.filename().generic_string(), "pauli_strings", shape));
}
