...
static Json programJson(const ProgramRef& p) {
    ...
    if (!p.path.empty())
        j["path"] = p.path;
    if (!p.inlineSource.empty())
        j["inline"] = p.inlineSource;
    j["isMain"] = p.isMain;
...
core::Json Project::toJson() const {
    ...
    for (const auto& p : programs)
        j["programs"].push_back(programJson(p));
    ...
    for (const auto& r : runs)
        j["runs"].push_back(runJson(r));
    j["recipes"] = Json::array();
    for (const auto& r : recipes)
        j["recipes"].push_back(recipeJson(r));
    return withExtra(extra, std::move(j));
}

Result<Project> Project::fromJson(const core::Json& data) {
    if (!data.is_object())
        return fail(ErrorCode::Parse, "project: 'data' must be an object");
    Json rest = data;
    Project p;
    p.name = takeString(rest, "name");
    if (auto it = rest.find("programs"); it != rest.end()) {
        if (!it->is_array())
            return fail(ErrorCode::Parse, "project: 'data.programs' must be an array");
        for (const auto& e : *it)
            if (e.is_object())
                p.programs.push_back(programFrom(e));
        rest.erase(it);
    }
    ...
    if (auto it = rest.find("runs"); it != rest.end()) {
        if (!it->is_array())
            return fail(ErrorCode::Parse, "project: 'data.runs' must be an array");
        for (const auto& e : *it)
            if (e.is_object())
                p.runs.push_back(runFrom(e));
        rest.erase(it);
    }
    if (auto it = rest.find("recipes"); it != rest.end()) {
        if (!it->is_array())
            return fail(ErrorCode::Parse, "project: 'data.recipes' must be an array");
        for (const auto& e : *it)
            if (e.is_object())
                p.recipes.push_back(recipeFrom(e));
        rest.erase(it);
    }
    p.extra = std::move(rest);
    return p;
...
std::string Project::addRun(RunSummary summary) {
    if (summary.id.empty()) {
        std::uint64_t next = 1;
        for (const auto& r : runs) {
            if (!r.id.starts_with("run-"))
                continue;
            // `from_chars` never throws: an id too long to be a number simply does not raise
            // `next`.
            std::uint64_t n = 0;
            const char* first = r.id.data() + 4;
            const char* last = r.id.data() + r.id.size();
            const auto parsed = std::from_chars(first, last, n);
            if (parsed.ec == std::errc{} && parsed.ptr == last && n < kMaxRunNumber)
                next = std::max(next, n + 1);
        }
        summary.id = std::format("run-{:06}", next);
    }
    if (summary.resultPath.empty())
        summary.resultPath = "runs/" + summary.id + ".json";
