...
std::string diagnosticsHtml(const RunReportInput& in) {
    if (!in.run || in.run->diagnostics.empty()) {
        if (!in.compiled || in.compiled->diagnostics.empty()) return "<p class=\"muted\">none</p>";
    }
        ...
        for (const auto& d : ds) {
            const char* sev = d.severity == lang::Severity::Error     ? "error"
                              : d.severity == lang::Severity::Warning ? "warning"
                                                                      : "info";
            rows.push_back({std::string(origin), d.id().empty() ? "-" : d.id(), sev, d.error.message});
        }
    };
    if (in.compiled) add(in.compiled->diagnostics, "compile");
    if (in.run) add(in.run->diagnostics, "run");
...
std::string subtitle(const RunReportInput& in) {
    std::string s;
    if (!in.project.empty()) s = in.project;
    if (in.run) {
        if (!s.empty()) s += " — ";
...
Result<std::string> loadReportTemplate(const std::filesystem::path& templatePath) {
    const std::filesystem::path p = templatePath.empty() ? core::assetDir() / "Report" / "run.html.tmpl" : templatePath;
    auto text = core::readTextFile(p);
    if (!text) {
        Error e = text.error();
        e.notes.push_back("the run report template of spec 23 §10 lives in Assets/Report/run.html.tmpl");
...
Result<std::string> renderRunReport(const RunReportInput& in) {
    ...
    values["estimate"] = in.run ? estimateHtml(in.run->estimate) : std::string("<p class=\"muted\">no estimate</p>");
    ...
    if (!missing.empty()) {
        ...
        return fail(ErrorCode::InvalidArgument, "report template has unknown placeholders: " + list);
    }
    std::vector<std::string> offenders;
    if (!isSelfContained(html, &offenders)) {
        std::string list;
        for (const auto& o : offenders) list += (list.empty() ? "" : " | ") + o;
        return fail(ErrorCode::InvalidArgument, "report would not be self-contained (spec 23 §10): " + list);
    }
    return html;
}
