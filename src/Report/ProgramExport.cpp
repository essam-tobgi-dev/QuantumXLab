...
std::string estimatePragma(const runtime::Estimate& e) {
    ...
    return std::format("pragma qlab.estimate wall_s={:.6g} fidelity={:.6g}", e.wallTime.valueS, e.fidelity.fast);
}
...
Result<std::string> compiledQasm(const compiler::CompiledProgram& program, const ProgramExportOptions& options) {
    ...
    if (!options.estimate) return text;
    ...
    for (std::size_t at = text.find("pragma qlab."); at != std::string::npos; at = text.find("pragma qlab.", at + 1)) {
...
std::string scheduleSamplesCsv(const pulse::Schedule& schedule) {
    ...
    const std::size_t n = totalPs > 0 ? static_cast<std::size_t>((totalPs + dtPs - 1) / dtPs) + 1 : 0;

    std::vector<pulse::ChannelId> channels;
    for (auto ch : schedule.channels())
        if (ch.isDrivelike()) channels.push_back(ch);
    ...
    for (auto ch : channels) waves[ch].assign(n, num::Complex{0.0, 0.0});
    for (const auto& instr : schedule.instructions()) {
        const auto* play = std::get_if<pulse::Play>(&instr);
        if (!play || !play->ch.isDrivelike()) continue;
        ...
        for (std::size_t k = 0; k < s.size() && first + k < n; ++k) it->second[first + k] += s[k];
    }

    std::vector<double> time(n);
    for (std::size_t i = 0; i < n; ++i) time[i] = static_cast<double>(static_cast<std::int64_t>(i) * dtPs) * 1e-12;
