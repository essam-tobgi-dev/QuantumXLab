...
const char* regKindName(ir::RegKind k) {
    ...
    case ir::RegKind::Bit:
        return "bit";
    case ir::RegKind::Bool:
        return "bool";
    case ir::RegKind::Int:
        return "int";
    case ir::RegKind::Uint:
        return "uint";
    }
    return "bit";
}
ir::RegKind regKindFrom(std::string_view s) {
    if (s == "bool")
        return ir::RegKind::Bool;
    if (s == "int")
        return ir::RegKind::Int;
    if (s == "uint")
        return ir::RegKind::Uint;
    return ir::RegKind::Bit;
}

std::size_t bytesPerColumn(std::uint64_t shots) {
    return static_cast<std::size_t>((shots + 7) / 8);
}
...
std::vector<std::uint8_t> packMemory(const runtime::RunResult& r) {
    ...
    for (std::uint64_t s = 0; s < shots; ++s) {
        ...
        for (std::uint32_t b = 0; b < bits; ++b) {
            if (b >= shot.bits.size() || shot.bits[b] == 0)
                continue;
...
Result<data::Histogram> decodeMemory(std::span<const std::uint8_t> blob, const core::Json& header) {
    ...
    if (!header.contains("shots"))
        return fail(ErrorCode::Parse, "memory header: missing 'shots'");
    ...
    if (blob.size() != static_cast<std::size_t>(layout.bits) * stride)
        return fail(ErrorCode::Parse,
                    "memory file is " + std::to_string(blob.size()) + " bytes, expected " +
    ...
    for (std::uint64_t s = 0; s < shots; ++s) {
        for (std::uint32_t b = 0; b < layout.bits; ++b) {
            const std::uint8_t byte =
                blob[static_cast<std::size_t>(b) * stride + static_cast<std::size_t>(s / 8)];
...
core::Json runResultJson(const runtime::RunResult& r, const ResultExportOptions& options) {
    ...
    if (r.circuit)
        for (const auto& [name, n] : r.circuit->gateCounts())
            gates[name] = n;
    ...
    if (!options.channelsFile.empty())
        j["channels"] = "path: " + options.channelsFile;
    j["timing"] = {
        {"compile_ms", options.compileTime.count()},
...
std::string serializeRunResult(const runtime::RunResult& r, const ResultExportOptions& options) {
    ensureKinds();
    return core::JsonEnvelope::serialize(kRunResultKind, runResultJson(r, options),
                                         kRunResultSchema);
}
...
Status writeRunResult(const std::filesystem::path& path, const runtime::RunResult& r,
    ...
    if (!allFinite(doc))
        return fail(ErrorCode::InvalidArgument, "run result holds a non-finite number");
    QXL_TRY(core::writeTextFileAtomic(
        path, core::JsonEnvelope::serialize(kRunResultKind, doc, kRunResultSchema)));
    ...
    if (bin.has_parent_path())
        std::filesystem::create_directories(bin.parent_path(), ec);
    std::filesystem::path tmp = bin;
    tmp += ".tmp";
    {
        std::ofstream f(tmp, std::ios::binary | std::ios::trunc);
        if (!f)
            return fail(ErrorCode::Io, "cannot write " + tmp.string());
        if (!blob.empty())
...
Status verifyRunResult(const std::filesystem::path& path) {
    ...
    if (!d.contains("memoryHeader"))
        return fail(ErrorCode::Parse, "run_result: 'data.memoryHeader' is missing");
    ...
    if (!f)
        return fail(ErrorCode::Io, "cannot open " + bin.string());
    ...
    if (decoded.raw() != stored.raw())
        return fail(ErrorCode::Parse,
                    "memory file does not decode to the counts of " + path.string());
    return {};
}
...
std::string sweepToCsv(const runtime::SweepResult& sweep) {
    ...
    for (const auto& p : sweep.points) {
        for (std::size_t a = 0; a < axes; ++a)
            columns[a].push_back(a < p.coords.size() ? p.coords[a] : 0.0);
        columns[axes].push_back(p.p1);
        columns[axes + 1].push_back(p.p1Stderr);
    }
    std::vector<data::CsvColumn> cols;
    for (std::size_t a = 0; a < axes; ++a)
        cols.push_back({sweep.axes[a].input, sweep.axes[a].unit, columns[a]});
