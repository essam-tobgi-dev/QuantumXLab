...
const std::set<std::string, std::less<>>& qasmKeywords() {
    static const std::set<std::string, std::less<>> kw{
        "OPENQASM", "include", "pragma", "qubit",    "qreg",    "bit",     "creg",
        "int",      "uint",    "float",  "angle",    "bool",    "complex", "duration",
        "stretch",  "const",   "input",  "output",   "gate",    "def",     "defcal",
        "cal",      "extern",  "let",    "if",       "else",    "for",     "while",
        "in",       "return",  "break",  "continue", "end",     "measure", "reset",
        "barrier",  "delay",   "box",    "ctrl",     "negctrl", "inv",     "pow",
        "array",    "sizeof",  "true",   "false",    "switch",  "case",    "default"};
    return kw;
}
...
std::string svgHead(int width, int height) {
    return std::format(R"(<svg class="fig" viewBox="0 0 {} {}" width="100%" role="img">)", width,
                       height);
}
...
std::string substitute(std::string_view tmpl, const Placeholders& values,
...
std::string highlightQasm(std::string_view source) {
    ...
    while (i < n) {
        ...
        if (c == '/' && i + 1 < n && source[i + 1] == '/') {
            ...
            std::size_t end = i + 1;
            while (end < n && std::isdigit(static_cast<unsigned char>(source[end])))
                ++end;
            ...
            std::size_t end = i;
            while (end < n &&
                   (std::isalnum(static_cast<unsigned char>(source[end])) || source[end] == '.'))
                ++end;
            out += "<span class=\"n\">" + htmlEscape(source.substr(i, end - i)) + "</span>";
            i = end;
        } else if (identStart(c)) {
            std::size_t end = i;
            while (end < n && identChar(source[end]))
                ++end;
...
std::string tableHtml(std::span<const std::string> headers,
                      std::span<const std::vector<std::string>> rows, std::string_view cssClass) {
...
std::string histogramSvg(const data::Histogram& h, std::size_t topK, int width, int height) {
    ...
    for (const auto& [label, count] : top)
        maxP = std::max(maxP, static_cast<double>(count) / total);
    if (maxP <= 0.0)
        maxP = 1.0;

    std::string svg = svgHead(width, height);
    svg += std::format(R"(<line class="ax" x1="{}" y1="{}" x2="{}" y2="{}"/>)", left, top_ + plotH,
                       left + plotW, top_ + plotH);
    svg += std::format(R"(<line class="ax" x1="{}" y1="{}" x2="{}" y2="{}"/>)", left, top_, left,
                       top_ + plotH);
    ...
    for (std::size_t i = 0; i < top.size(); ++i) {
        ...
        svg += std::format(
            R"(<rect class="bar" x="{:.2f}" y="{:.2f}" width="{:.2f}" height="{:.2f}"><title>{} : {} ({:.4g})</title></rect>)",
            x, y, barW, bh, htmlEscape(top[i].first), top[i].second, p);
        svg += std::format(
            R"SVG(<text class="tick" x="{:.2f}" y="{}" transform="rotate(-60 {:.2f} {})">{}</text>)SVG",
...
std::string seriesSvg(std::span<const double> x, std::span<const double> y,
                      std::span<const double> sigma, std::string_view xLabel,
                      std::string_view yLabel, int width, int height) {
    ...
    if (!sigma.empty() && sigma.size() == y.size()) {
        ...
        for (std::size_t i = 0; i < x.size(); ++i)
            band += std::format("{:.2f},{:.2f} ", px(x[i]), py(y[i] + sigma[i]));
    ...
    for (std::size_t i = 0; i < x.size(); ++i)
        path += std::format("{:.2f},{:.2f} ", px(x[i]), py(y[i]));
    path += "\"/>";
    svg += path;
    svg += std::format(R"(<text class="tick" x="{}" y="{}">{}</text>)", left, height - 8,
                       htmlEscape(xLabel));
    svg +=
        std::format(R"(<text class="tick" x="4" y="{}">{}</text>)", top_ + 8, htmlEscape(yLabel));
...
bool isSelfContained(std::string_view html, std::vector<std::string>* offenders) {
    static constexpr std::array<std::string_view, 6> kBad{"http://", "https://", "//cdn",
                                                          "<script", "@import",  "url(http"};
    bool ok = true;
    for (std::string_view bad : kBad) {
        for (std::size_t at = html.find(bad); at != std::string_view::npos;
             at = html.find(bad, at + 1)) {
            ok = false;
            if (!offenders)
                return false;
