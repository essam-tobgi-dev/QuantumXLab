...
namespace { constexpr double kTwoPi = 2.0 * std::numbers::pi; }

Result<std::vector<double>> equilibriumPositions(int n) {
    if (n < 1) return fail(ErrorCode::InvalidArgument, "equilibriumPositions: n < 1");
    std::vector<double> u(static_cast<std::size_t>(n));
    if (n == 1) { u[0] = 0.0; return u; }
    const double spacing = 2.018 * std::pow(double(n), -0.559);
    for (int i = 0; i < n; ++i) u[static_cast<std::size_t>(i)] = spacing * (i - 0.5 * (n - 1));
    ...
    for (int it = 0; it < 100; ++it) {
        ...
        for (std::size_t m = 0; m < N; ++m) {
            ...
            for (std::size_t k = 0; k < N; ++k) {
                if (k == m) continue;
                double d = u[m] - u[k];
                double s = d > 0 ? 1.0 : -1.0;         // sign(m−k) ordering equals sign of separation for sorted u
                F[m] -= s / (d * d);
        ...
        for (std::size_t c = 0; c < N; ++c) {
            std::size_t piv = c;
            for (std::size_t r = c + 1; r < N; ++r) if (std::abs(J(r, c)) > std::abs(J(piv, c))) piv = r;
            if (piv != c) { for (std::size_t k = 0; k < N; ++k) std::swap(J(c, k), J(piv, k)); std::swap(rhs[c], rhs[piv]); }
            if (std::abs(J(c, c)) < 1e-14) return fail(ErrorCode::Internal, "equilibriumPositions: singular Jacobian");
            for (std::size_t r = c + 1; r < N; ++r) {
                double f = J(r, c) / J(c, c);
                for (std::size_t k = c; k < N; ++k) J(r, k) -= f * J(c, k);
                rhs[r] -= f * rhs[c];
            }
        }
        std::vector<double> delta(N);
        for (std::size_t i = N; i-- > 0;) { double s = rhs[i]; for (std::size_t k = i + 1; k < N; ++k) s -= J(i, k) * delta[k]; delta[i] = s / J(i, i); }
        double maxStep = 0.0;
        for (std::size_t i = 0; i < N; ++i) { u[i] += delta[i]; maxStep = std::max(maxStep, std::abs(delta[i])); }
...
double lambDicke(double deltaK, double b, units::Mass m, units::Frequency fMode, double cosTheta) {
    return deltaK * cosTheta * b * std::sqrt(units::consts::hbar.v / (2.0 * m.v * kTwoPi * fMode.v));
}
...
Result<std::pair<RealVector, RealMatrix>> realSymEig(const RealMatrix& A) {
    ...
    Matrix H(N, N);
    for (std::size_t i = 0; i < N; ++i) for (std::size_t j = 0; j < N; ++j) H(i, j) = A(i, j);
    auto e = eigh(H, true);
    if (!e) return std::unexpected(e.error());
    RealMatrix V(N, N);
    for (std::size_t i = 0; i < N; ++i) for (std::size_t j = 0; j < N; ++j) V(i, j) = e->vectors(i, j).real();
    // Fix sign so the largest component of each column is positive (deterministic participation vectors).
    for (std::size_t j = 0; j < N; ++j) {
        std::size_t im = 0; for (std::size_t i = 1; i < N; ++i) if (std::abs(V(i, j)) > std::abs(V(im, j))) im = i;
        if (V(im, j) < 0) for (std::size_t i = 0; i < N; ++i) V(i, j) = -V(i, j);
    }
...
Result<Modes> normalModes(const ChainParams& p) {
    ...
    for (std::size_t k = 0; k < N; ++k) m.axial.push_back(units::Frequency(p.omegaZ.v * std::sqrt(std::max(0.0, ax->first[k]))));
    // Radial: B = (ωr/ωz)² δ − ½(A − δ); eigenvalues descending → COM highest.
    RealMatrix B(N, N);
    const double ratio2 = (p.omegaR.v / p.omegaZ.v) * (p.omegaR.v / p.omegaZ.v);
    for (std::size_t i = 0; i < N; ++i) for (std::size_t j = 0; j < N; ++j) B(i, j) = (i == j ? ratio2 : 0.0) - 0.5 * (A(i, j) - (i == j ? 1.0 : 0.0));
    ...
    for (std::size_t k = 0; k < N; ++k) {
        ...
        for (std::size_t i = 0; i < N; ++i) m.radialVectors(i, k) = rd->second(i, src);
    }
    const double deltaK = 2.0 * kTwoPi / p.ramanWavelength.v; // counter-propagating Raman pair
    m.etaAxial.assign(N, std::vector<double>(N)); m.etaRadial.assign(N, std::vector<double>(N));
    for (std::size_t i = 0; i < N; ++i) for (std::size_t k = 0; k < N; ++k) {
        m.etaAxial[i][k] = lambDicke(deltaK, m.axialVectors(i, k), p.mass, m.axial[k], p.beamAngleCos);
        m.etaRadial[i][k] = lambDicke(deltaK, m.radialVectors(i, k), p.mass, m.radial[k], p.beamAngleCos);
    }
    return m;
}

SpinMotionOps spinMotionOperators(std::size_t nQubits, units::Frequency fMode, int fock) {
    SpinMotionOps ops; ops.nQubits = nQubits; ops.fock = fock;
    const std::size_t F = static_cast<std::size_t>(fock), Q = ipow(2, nQubits);
    SparseMatrix a(F, F), n(F, F);
    std::vector<Triplet> ta, tn;
    for (std::size_t k = 1; k < F; ++k) ta.push_back({k - 1, k, std::sqrt(double(k))});
    for (std::size_t k = 0; k < F; ++k) tn.push_back({k, k, double(k)});
    a = SparseMatrix::fromTriplets(F, F, ta); n = SparseMatrix::fromTriplets(F, F, tn);
    SparseMatrix IQ = SparseMatrix::identity(Q), IF = SparseMatrix::identity(F);
    ops.a = kron(a, IQ); ops.adag = ops.a.adjoint();                 // mode is the high subsystem
    ops.H0 = kron(n, IQ); ops.H0.scale(kTwoPi * fMode.v);
