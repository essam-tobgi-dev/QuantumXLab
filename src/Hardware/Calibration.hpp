...
using Matrix2d = std::array<std::array<double, 2>, 2>; // row-stochastic M[i][j] = P(read j | prepared i)
...
struct QubitCal {
    ...
    double readoutFidelity() const { return 1.0 - 0.5 * (readoutAssignment[0][1] + readoutAssignment[1][0]); }
...
struct EdgeCal {
    ...
    // Optional per-gate overrides on the same edge (e.g. siswap on a cz-native edge): gate → (error, duration).
...
struct MotionalCal {
    ...
    std::optional<units::Frequency> gateModeFrequency() const {
        ...
        if (gateModeIndex < list.size()) return list[gateModeIndex];
        return std::nullopt;
    }
};
...
struct Calibration {
    ...
    const QubitCal* qubit(std::uint32_t q) const { return q < qubits.size() ? &qubits[q] : nullptr; }
    ...
    Result<units::Time> gateDuration(std::string_view gate, std::span<const std::uint32_t> qubits) const;
    Result<double> gateError(std::string_view gate, std::span<const std::uint32_t> qubits) const;
    struct IdleParams { units::Time t1, t2, t2star, tphi; double pThermal; };
    Result<IdleParams> idleParams(std::uint32_t q) const;
    units::Time maxT1() const;

    // Validation rules of spec 09 §3 (T2* ≤ T2echo ≤ 2T1, row-stochastic readout, ranges). Collects all violations.
    Result<void> validate() const;

    // Deterministic perturbation with the spec 09 §8 spreads (used by tests and the drift tutorial).
    struct Spreads { double t1Ln = 0.30, t2Ln = 0.35, errLn = 0.25, fSigmaHz = 15e6, alphaSigmaHz = 6e6, readoutRel = 0.3, pThermalLn = 0.3; };
