...
enum class CollisionKind { Degenerate, Straddle, TwoPhoton, CrTooSlow, CrUnstable, Spectator, NeighbourDetuning };
...
public:
    ...
    // Frequencies (Hz) per qubit index from the calibration; NaN for couplers without a calibration entry.
    std::vector<double> frequencies() const;
private:
    const Device& dev_;
    const Calibration& cal_;
    CollisionThresholds th_;
    void checkPair(std::uint32_t i, std::uint32_t j, bool coupled, std::vector<Collision>& out) const;
};
