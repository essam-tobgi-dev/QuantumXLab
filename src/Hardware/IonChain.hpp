#pragma once
// Spec 09 §5.4 — linear ion chain: equilibrium positions, normal modes, Lamb–Dicke parameters (T06
// §1, §3).
...
struct ChainParams {
    ...
    double beamAngleCos =
        1.0; // cos θ of Δk with the mode axis (counter-propagating Raman: |Δk| = 2k)
};
...
double lambDicke(double deltaK, double participation, units::Mass m, units::Frequency fMode,
                 double cosTheta = 1.0);
...
// Spin–motion operators for N qubits ⊗ one motional mode with Fock cutoff (little-endian: qubits
// low, mode high).
