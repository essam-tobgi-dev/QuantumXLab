...
num::SparseMatrix siteProjector(std::span<const std::uint32_t> dims, std::uint32_t site, std::uint32_t level);
num::SparseMatrix siteOperator(std::span<const std::uint32_t> dims, std::uint32_t site, num::ConstMatrixView local);
