...
Result<Spectrum> chargeBasisSpectrum(units::Frequency EJ, units::Frequency EC, double ng = 0.0, int d = 3, int N = 30);
...
struct EjEc { units::Frequency EJ, EC; int iterations = 0; };
...
units::Frequency josephsonEnergy(units::Frequency EJsum, double phiOverPhi0, double asymmetry = 0.0);
// Duffing/Kerr operators truncated to d levels: a, a†, n̂ = a†a, and H/ħ = ω a†a + (α/2) a†a†aa (rad/s units).
struct KerrOps { num::Matrix a, adag, n, H; };
KerrOps kerrOperators(units::Frequency f01, units::Frequency alpha, int d);
// Static ZZ (T05 (5.3)): ζ/2π for coupling g (ordinary freq), detuning Δ12 = f1 − f2, anharmonicities α1, α2.
units::Frequency staticZZ(units::Frequency g, units::Frequency f1, units::Frequency f2, units::Frequency alpha1, units::Frequency alpha2);
