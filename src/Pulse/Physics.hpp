...
inline double rabiRate(double kappaD, double amplitude) {
    return kappaD * amplitude;
}
...
inline double dragBeta(double alphaAngular) {
    return alphaAngular == 0.0 ? 0.0 : -1.0 / alphaAngular;
}
...
MsIntegrals msIntegrals(std::span<const double> coupling, double dt, double deltaAngular,
                        double t0 = 0.0);
