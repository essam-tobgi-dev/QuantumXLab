...
struct DriveEnvelope {
    ...
    //  Rabi, rwa:  Ω(t) = held_k·e^{−i2π∫₀ᵗ(f_ch − f_ref)dt'};  H = (Re Ω·X + Im Ω·Y)/2 on the
    //  site, in the
    //              frame at f_ref (T05 (7.1), T07 (1.2)). A CR tone thus oscillates at f_t − f_c
    //              (spec 10 §8.2).
    //  Rabi, lab:  Ω(t) = 2·Re[held_k·e^{−i2π∫₀ᵗ f_ch dt'}] (real);  H = Ω(t)·X/2.
    //  MS, rwa:    F(t) = 2·held_k·cos(2π∫₀ᵗ μ dt'), the bichromatic field on each ion (spins in
    //  their
    //              rotating frames, the mode in its lab frame as hw::buildIonModel keeps it):
    //              H = Σ_{n∈{i,j}} (η_n/2)(Re F σ_x⁽ⁿ⁾ + Im F σ_y⁽ⁿ⁾)(a + a†). Its interaction
    //              picture is T06 (6.1) with g = η|held|/2, S_φ at φ = arg held, detuning δ =
    //              ω_mode − μ.
...
struct DriveOptions {
    ...
    std::optional<double>
        sharedFrameHz; // one rotating frame for every site (hw::SystemModelOptions)
};
...
struct SystemDrives {
    ...
    std::vector<ScheduleEvent>
        events; // readout tones, acquisition windows, detection, pumping; by time
...
Result<SystemDrives> toSystemDrives(const Schedule& schedule, const hw::Device& device,
                                    bool rwa = true);
