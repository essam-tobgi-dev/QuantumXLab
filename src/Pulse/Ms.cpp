...
std::vector<double> heldEnvelope(double T, double edge, double dt) {
    ...
    for (std::size_t k = 0; k < n; ++k)
        e[k] = gaussianSquareShape(static_cast<double>(k) * dt, T, edge / 2.0, edge);
    return e;
}
...
Result<PulseLibrary::MsShape> PulseLibrary::msShape(double durationS) const {
    ...
    if (auto it = msShapes_.find(key); it != msShapes_.end())
        return it->second;

    const core::Json& tpl =
        templates_.contains("ms_bichromatic") ? templates_["ms_bichromatic"] : core::Json::object();
    MsShape shape;
    ...
    if (shape.loops < 1 || 2.0 * shape.edgeS >= durationS)
        return fail(kErrMsClosure,
                    std::format("ms_bichromatic: K = {} and {} us edges do not fit a {} us gate",
    ...
    if (!(rLo * rHi < 0.0))
        return fail(
            kErrMsClosure,
            std::format("ms_bichromatic: no loop-closing detuning between {:.6g} and {:.6g} "
                        "rad/s for a {} us gate",
                        lo, hi, durationS * 1e6));
    ...
    if (std::abs(integrals.alpha) > 1e-9 * area || !(shape.phaseIntegral > 0.0))
        return fail(kErrMsClosure, std::format("ms_bichromatic: the loop does not close at δ = "
                                               "{:.6g} rad/s (residual |α|/g = {:.3g} s "
                                               "against an envelope area of {:.3g} s)",
                                               shape.delta, std::abs(integrals.alpha), area));
    return shape;
}

Result<void> PulseLibrary::prepareMsShapes() {
    msShapes_.clear();
    if (!isIonDevice() || !templates_.contains("ms_bichromatic"))
        return {};
...
Result<MsParams> PulseLibrary::msParams(std::uint32_t i, std::uint32_t j, double theta) const {
    ...
    if (!d)
        return fail(kErrNoDefcal, std::format("no 'ms' defcal on ions {}-{}", i, j));
    if (!templates_.contains("ms_bichromatic"))
        return fail(kErrLibrary, "pulses.json has no 'ms_bichromatic' template");
    ...
    if (!(eta2 > 0.0))
        return fail(kErrLibrary,
                    std::format("ms({},{}): Lamb–Dicke factors must be positive", i, j));
    ...
    p.omegaSquareRadPerS = physics::msAmplitudeForAngle(kPi / 2.0, p.etaI, p.etaJ, p.durationS,
                                                        p.squareDetuningRadPerS);
...
// the superseded spec 10 §6.6 line is smaller by √π and is refused (regenerate with
// tools/gencal.py).
Result<void> PulseLibrary::checkMsTable() const {
    for (auto const& [key, d] : defcals_) {
        if (d.ref != "ms_bichromatic" || key.qubits.size() != 2)
            continue;
        ...
        if (std::abs(ratio - 1.0) > 1e-6)
            return fail(
                kErrLibrary,
                std::format("{}: stored omega_rad_s_at_pi_2 = {:.7g} rad/s but T06 (6.6) gives "
...
Result<Schedule> PulseLibrary::buildMs(const Defcal& d, const ParamMap& params) const {
    if (d.key.qubits.size() != 2)
        return fail(kErrLibrary, std::format("defcal {} needs two ions", d.key.toString()));
    ...
    if (p.amplitude > 1.0 + 1e-12)
        return fail(
            kErrWaveform,
            std::format("ms({},{}) at θ = {:.4f} needs {:.3f}× the full-scale drive; |θ| ≤ π/2 "
                        "is the calibrated range",
                        d.key.qubits[0], d.key.qubits[1], theta, p.amplitude));
