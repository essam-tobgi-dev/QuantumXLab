...
const ChannelSamples* SampledSchedule::find(ChannelId ch) const {
    auto it =
        std::lower_bound(channels.begin(), channels.end(), ch,
                         [](const ChannelSamples& c, ChannelId id) { return c.channel < id; });
    return it != channels.end() && it->channel == ch ? &*it : nullptr;
}

Result<SampledSchedule> sampleSchedule(const Schedule& schedule, double sampleRateHz,
                                       int granularity) {
    if (!(sampleRateHz > 0.0) || !std::isfinite(sampleRateHz))
        return fail(kErrSampling,
                    std::format("sample rate {} Hz must be positive and finite", sampleRateHz));
    if (granularity < 1)
        return fail(kErrSampling, std::format("granularity {} must be at least 1", granularity));
    const std::int64_t dtPs = std::llround(1e12 / sampleRateHz);
    if (dtPs < 1)
        return fail(
            kErrSampling,
            std::format("sample rate {:.3g} Hz is finer than the 1 ps time base", sampleRateHz));
    ...
    const std::int64_t n =
        ceilDiv(ceilDiv(std::max<std::int64_t>(schedule.duration().value, 0), dtPs), g) * g;
    if (n > kMaxSamples)
        return fail(
            kErrSampling,
            std::format("the record needs {} samples per channel at {:.3g} Hz", n, sampleRateHz));
        ...
        if (auto* p = std::get_if<Play>(&instr)) {
            ...
            for (std::int64_t k = first; k < last; ++k) {
                ...
                cs.samples[static_cast<std::size_t>(k)] +=
                    e * std::polar(1.0, frames.basebandPhase(p->ch, tk));
            }
            if (last > first)
                cs.plays.push_back(
            ...
            const std::int64_t last =
                std::min<std::int64_t>(ceilDiv(a->t0.value + a->length.value, dtPs), n);
            AcquireWindow w;
            w.channel = a->ch;
            w.span = {static_cast<std::size_t>(first),
                      static_cast<std::size_t>(std::max<std::int64_t>(last - first, 0))};
            w.weights = a->weights;
            w.kind = a->kind;
    ...
    for (auto& [ch, cs] : byChannel) {
        (void)ch;
        std::sort(cs.plays.begin(), cs.plays.end(),
                  [](const SampleSpan& x, const SampleSpan& y) { return x.start < y.start; });
        out.channels.push_back(std::move(cs));
    }
    std::stable_sort(
        out.acquisitions.begin(), out.acquisitions.end(),
