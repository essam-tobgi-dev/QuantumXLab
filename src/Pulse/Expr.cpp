...
struct Parser {
    ...
    void setErr(std::string m) {
        if (!err) err = Error(kErrLibrary, std::format("{} in expression '{}'", m, s));
    }
    ...
    double parseTerm() {
        ...
        for (;;) {
            ...
            if (eat('*')) v *= parseUnary();
                ...
                double d = parseUnary();
                if (d == 0.0) { setErr("division by zero"); return 0.0; }
                v /= d;
            } else break;
        }
        return v;
    ...
    double parseAtom() {
        skip();
        if (i >= s.size()) { setErr("unexpected end"); return 0.0; }
        if (eat('(')) {
            double v = parseExpr();
            if (!eat(')')) setErr("missing ')'");
            return v;
        }
        const char c = s[i];
        if (std::isdigit(static_cast<unsigned char>(c)) || c == '.') {
            ...
            if (ec != std::errc{}) { setErr("bad number"); return 0.0; }
        ...
        if (std::isalpha(static_cast<unsigned char>(c)) || c == '_') {
            ...
            for (;;) {
                ...
                const bool digits = !seg.empty() && std::all_of(seg.begin(), seg.end(), [](char ch) {
            ...
            if (id == "pi") return std::numbers::pi;
            if (id == "e") return std::numbers::e;
            if (auto it = params.find(id); it != params.end()) return it->second;
            if (paths) {
                auto r = paths(id);
                if (r) return *r;
                if (!err) err = r.error();
...
Result<double> evalJsonValue(const core::Json& j, const ParamMap& params, const PathResolver& paths) {
    ...
    if (j.is_string()) return evalExpression(j.get<std::string>(), params, paths);
...
PathResolver makeJsonResolver(const core::Json& calibration, const core::Json& device) {
    return [&calibration, &device](std::string_view path) -> Result<double> {
        ...
        if (rest.starts_with("cal.")) { node = &calibration; rest.remove_prefix(4); }
        else if (rest.starts_with("device.")) { node = &device; rest.remove_prefix(7); }
        else return fail(kErrLibrary, std::format("unknown symbol '{}'", path));
        ...
        while (!rest.empty()) {
            ...
            if (node->is_array()) {
                ...
                if (ec != std::errc{} || idx >= node->size())
                    return fail(kErrLibrary, std::format("'{}' is not an index of '{}'", key, path));
                node = &(*node)[idx];
        ...
        if (node->is_array() && !node->empty() && (*node)[0].is_number()) return (*node)[0].get<double>();
