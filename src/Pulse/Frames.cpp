...
FrameTimeline::FrameTimeline(const Schedule& schedule) {
    for (auto const& f : schedule.frames()) {
        if (tracks_.contains(f.channel)) continue; // the first declaration of a channel wins
        Track t;
    ...
    for (auto const& i : schedule.instructions())
        if (auto* fo = std::get_if<FrameOp>(&i)) ops.push_back(fo);
    std::stable_sort(ops.begin(), ops.end(), [](const FrameOp* a, const FrameOp* b) { return a->t0 < b->t0; });

    for (const FrameOp* fo : ops) {
        Track& t = tracks_[fo->ch];
        if (t.segments.empty()) t.segments.push_back({0.0, 0.0, 0.0, 0.0}); // undeclared frame: 0 Hz, 0 rad
        ...
        next.t0 = tOp;
        switch (fo->op) {
        case FrameOp::Op::ShiftPhase: next.phaseOps += fo->value; break;
        case FrameOp::Op::SetPhase: next.phaseOps = fo->value; break;
        case FrameOp::Op::ShiftFrequency: next.frequencyHz += fo->value; break;
        case FrameOp::Op::SetFrequency: next.frequencyHz = fo->value; break;
        }
...
std::vector<ChannelId> FrameTimeline::channels() const {
    ...
    for (auto const& [ch, t] : tracks_) { (void)t; out.push_back(ch); }
    return out;
}
