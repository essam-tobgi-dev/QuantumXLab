...
Result<std::vector<std::uint32_t>> parseQubits(const core::Json& j) {
    if (!j.is_array())
        return fail(kErrLibrary, "defcal 'qubits' must be an array");
    std::vector<std::uint32_t> out;
    for (auto const& q : j) {
        if (!q.is_number_unsigned())
            return fail(kErrLibrary, "defcal qubit index must be an integer");
        out.push_back(q.get<std::uint32_t>());
    }
    return out;
}

bool isSymmetricTwoQubitGate(std::string_view g) {
    return g == "ms" || g == "cz" || g == "siswap";
}
} // namespace
...
Result<PulseLibrary> PulseLibrary::fromJson(core::Json pulses, hw::Device device,
    ...
    if (lib.deviceId_.empty())
        return fail(kErrLibrary, "pulses.json: missing field 'device'");
    if (lib.deviceId_ != lib.device_.id)
        return fail(kErrLibrary, std::format("pulses.json is for '{}' but the device is '{}'",
                                             lib.deviceId_, lib.device_.id));
    ...
    if (lib.dt_.value <= 0)
        return fail(kErrLibrary, "device timing.dt_ps must be positive");
    if (pulses.contains("templates") && pulses["templates"].is_object())
        lib.templates_ = pulses["templates"];
        ...
        for (auto const& [name, f] : pulses["frames"].items()) {
            ...
            if (chName.empty())
                return fail(kErrLibrary,
                            std::format("pulses.json: missing field frames.{}.channel", name));
            QXL_TRY_ASSIGN(d.channel, parseChannel(chName));
            if (f.contains("frequency_ghz")) {
                auto ghz = evalJsonValue(f["frequency_ghz"], {}, res);
                if (!ghz)
                    return fail(
                        ghz.error().withNote(std::format("in frames.{}.frequency_ghz", name)));
    ...
    for (auto const& dj : pulses["defcals"]) {
        ...
        if (d.key.gate.empty())
            return fail(kErrLibrary, "pulses.json: defcal without field 'gate'");
        if (!dj.contains("qubits"))
            return fail(kErrLibrary,
                        std::format("pulses.json: defcal '{}' has no field 'qubits'", d.key.gate));
        QXL_TRY_ASSIGN(d.key.qubits, parseQubits(dj["qubits"]));
        if (dj.contains("params"))
            for (auto const& p : dj["params"])
                d.params.push_back(p.get<std::string>());
        if (dj.contains("instructions"))
            d.instructions = dj["instructions"];
        d.ref = dj.value("ref", std::string{});
        d.paramsFrom = dj.value("params_from", std::string{});
        d.extra = dj;
        if (d.instructions.is_null() && d.ref.empty())
            return fail(kErrLibrary, std::format("defcal {} has neither 'instructions' nor 'ref'",
                                                 d.key.toString()));
...
Result<PulseLibrary> PulseLibrary::load(const std::filesystem::path& dir) {
    ...
    QXL_TRY_ASSIGN(auto lib, fromJson(std::move(pulses.data), std::move(loaded.device),
                                      std::move(loaded.calibration)));
...
Result<PulseLibrary> loadPulses(const std::filesystem::path& deviceDir) {
    return PulseLibrary::load(deviceDir);
}
...
const Defcal* PulseLibrary::find(std::string_view gate,
                                 std::span<const std::uint32_t> qubits) const {
    DefcalKey key{std::string(gate), {qubits.begin(), qubits.end()}};
    if (auto it = defcals_.find(key); it != defcals_.end())
        return &it->second;
...
PathResolver PulseLibrary::resolver() const {
    return makeCalibrationResolver(device_, calibration_);
}

Result<double> PulseLibrary::path(std::string_view p) const {
    return resolver()(p);
}
...
Schedule PulseLibrary::emptySchedule() const {
    return Schedule(dt_);
}
...
void PulseLibrary::attachFrames(Schedule& s) const {
    ...
    for (auto const& [name, f] : frames_) {
        ...
        if (used.contains(f.channel) && !present)
            s.addFrame(f);
    }
}
...
Result<double> PulseLibrary::driveCoupling(std::uint32_t qubit) const {
    ...
    static constexpr Known kKnown[] = {
        {"sx", kPi / 2.0}, {"x", kPi}, {"rx", kPi / 2.0}, {"ry", kPi / 2.0}};
    ...
    return fail(kErrNoDefcal,
                std::format("no defcal on qubit {} fixes the drive coupling (need sx, x, rx or ry)",
                            qubit));
}
...
Result<double> PulseLibrary::rabiScale(ChannelId ch) const {
    ...
    default:
        return fail(kErrDrive,
                    std::format("channel {} has no calibrated Rabi scale", ch.toString()));
    }
}

// ---- dispatch -------------------------------------------------------------------------

Result<Schedule> PulseLibrary::scheduleFor(std::string_view gate,
                                           std::span<const std::uint32_t> qubits,
                                           const ParamMap& params) const {
    const Defcal* d = find(gate, qubits);
    if (!d) {
        DefcalKey key{std::string(gate), {qubits.begin(), qubits.end()}};
        return fail(Error(kErrNoDefcal, std::format("device '{}' has no defcal for {}", deviceId_,
                                                    key.toString()))
                        .withId("E_NO_DEFCAL"));
    }
    for (auto const& p : d->params)
        if (!params.contains(p))
            return fail(kErrNoDefcal,
                        std::format("defcal {} needs parameter '{}'", d->key.toString(), p));

    Result<Schedule> s =
        d->ref.empty() ? buildFromInstructions(*d, params) : buildFromTemplate(*d, params);
    if (!s)
        return s;
    s->sort();
    attachFrames(*s);
...
Result<Schedule> PulseLibrary::scheduleFor(std::string_view gate,
                                           std::initializer_list<std::uint32_t> qubits,
                                           const ParamMap& params) const {
