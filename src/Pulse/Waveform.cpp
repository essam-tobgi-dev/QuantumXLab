...
std::string_view waveformKindName(WaveformKind k) {
    switch (k) {
    case WaveformKind::Constant: return "constant";
    case WaveformKind::Gaussian: return "gaussian";
    case WaveformKind::GaussianSquare: return "gaussian_square";
    case WaveformKind::Drag: return "drag";
    case WaveformKind::Cosine: return "cosine";
    case WaveformKind::Sech: return "sech";
    case WaveformKind::Slepian: return "slepian";
    case WaveformKind::Samples: return "samples";
    }
    return "?";
}

Result<WaveformKind> waveformKindFromName(std::string_view s) {
    if (s == "constant") return WaveformKind::Constant;
    if (s == "gaussian") return WaveformKind::Gaussian;
    if (s == "gaussian_square") return WaveformKind::GaussianSquare;
    if (s == "drag") return WaveformKind::Drag;
    if (s == "cosine") return WaveformKind::Cosine;
    if (s == "sech") return WaveformKind::Sech;
    if (s == "slepian") return WaveformKind::Slepian;
    if (s == "samples") return WaveformKind::Samples;
...
double gaussianShapeArea(double T, double sigma) {
    ...
    const double c = std::exp(-(edge * edge) / (2.0 * sigma * sigma));
    const double integral = sigma * std::sqrt(2.0 * kPi) * std::erf(T / (2.0 * std::numbers::sqrt2 * sigma));
...
double gaussianSquareShape(double t, double T, double sigma, double rise) {
    ...
    if (t < rise) return liftedGauss(t, rise, sigma, rise);
    if (t > T - rise) return liftedGauss(t, T - rise, sigma, rise);
    return 1.0;
}
double gaussianSquareShapeDerivative(double t, double T, double sigma, double rise) {
    if (t < 0.0 || t > T || rise <= 0.0) return 0.0;
    if (t < rise) return liftedGaussDeriv(t, rise, sigma, rise);
    if (t > T - rise) return liftedGaussDeriv(t, T - rise, sigma, rise);
    return 0.0;
}
double gaussianSquareShapeArea(double T, double sigma, double rise) {
    if (rise <= 0.0) return T;
    const double c = std::exp(-(rise * rise) / (2.0 * sigma * sigma));
    // ∫₀^rise exp(-(t-rise)²/2σ²) dt = σ√(π/2) erf(rise/(σ√2))
    const double half = sigma * std::sqrt(kPi / 2.0) * std::erf(rise / (sigma * std::numbers::sqrt2));
...
double slepianShape(double t, double T, double lambda1) {
    ...
    double peak = 2.0;
    if (std::abs(4.0 * lambda1) >= 1.0) peak = std::max(peak, std::abs(1.0 + 2.0 * lambda1 + 1.0 / (8.0 * lambda1)));
    return raw / peak;
}

Complex Waveform::sample(double t) const {
    if (kind == WaveformKind::Samples) {
        if (samples.empty() || duration <= 0.0) return {};
    ...
    switch (kind) {
    case WaveformKind::Constant: return scale;
    case WaveformKind::Gaussian: return scale * gaussianShape(t, duration, sigma);
    case WaveformKind::GaussianSquare: return scale * gaussianSquareShape(t, duration, sigma, rise);
    ...
    case WaveformKind::Cosine: return scale * 0.5 * (1.0 - std::cos(2.0 * kPi * t / duration));
    ...
    case WaveformKind::Slepian: return scale * slepianShape(t, duration, lambda1);
    case WaveformKind::Samples: break;
    }
    return {};
}
...
Complex Waveform::area() const {
    const Complex scale = envelopeScale(amplitude, phase);
    switch (kind) {
    case WaveformKind::Constant: return scale * duration;
    case WaveformKind::Gaussian: return scale * gaussianShapeArea(duration, sigma);
    case WaveformKind::GaussianSquare: return scale * gaussianSquareShapeArea(duration, sigma, rise);
    case WaveformKind::Drag:
    ...
    case WaveformKind::Cosine: return scale * duration / 2.0;
    ...
    for (int i = 1; i < n; ++i) acc += sample(i * h) * ((i % 2) ? 4.0 : 2.0);
    return acc * (h / 3.0);
}

// A non-finite sample makes the result NaN (std::max alone would skip it), so validate() refuses it.
double Waveform::maxAbs() const {
    ...
    const int n = 2048;
    for (int i = 0; i <= n; ++i)
        if (!take(sample(duration * i / n))) break;
    return m;
}
...
Waveform::Spectrum Waveform::spectrum(std::int64_t dtPs) const {
    ...
    for (std::size_t i = 0; i < n; ++i) out.magnitude[i] = std::abs(v[i]);
    return out;
}

Result<void> Waveform::validate() const {
    if (kind == WaveformKind::Samples) {
        if (samples.empty()) return fail(kErrWaveform, "samples waveform is empty");
    } else if (duration <= 0.0) {
        return fail(kErrWaveform, std::format("{} waveform needs T > 0", waveformKindName(kind)));
    }
    const bool needsSigma = kind == WaveformKind::Gaussian || kind == WaveformKind::GaussianSquare ||
                            kind == WaveformKind::Drag;
    if (needsSigma && sigma <= 0.0)
        return fail(kErrWaveform, std::format("{} waveform needs sigma > 0", waveformKindName(kind)));
    ...
    if (!std::isfinite(amplitude) || !std::isfinite(phase) || !std::isfinite(duration))
        return fail(kErrWaveform, std::format("{} waveform has a non-finite parameter", waveformKindName(kind)));
    const double peak = maxAbs();
    if (!std::isfinite(peak))
        return fail(kErrWaveform, std::format("{} envelope evaluates to a non-finite sample", waveformKindName(kind)));
    if (peak > 1.0 + 1e-9)
        return fail(kErrWaveform, std::format("envelope exceeds unit amplitude (|e|max = {:.4f})", peak));
    return {};
}
