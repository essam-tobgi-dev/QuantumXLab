...
public:
    ...
    // the twirl changed the channel. nullopt: no twirl exists (d = 3) or the parameters are invalid.
...
namespace id {
inline constexpr std::string_view BitFlip = "bit_flip", PhaseFlip = "phase_flip", BitPhaseFlip = "bit_phase_flip",
    Pauli = "pauli", Depolarizing1q = "depolarizing_1q", Depolarizing2q = "depolarizing_2q",
    DepolarizingNq = "depolarizing_nq", AmplitudeDamping = "amplitude_damping", PhaseDamping = "phase_damping",
    ThermalRelaxation = "thermal_relaxation", OverRotation = "over_rotation", DetuningPhase = "detuning_phase",
    ZzCrosstalk = "zz_crosstalk", Leakage = "leakage", ResetError = "reset_error",
    MeasurementDephasing = "measurement_dephasing", DetuningDrift = "detuning_drift",
...
Result<ChannelPtr> depolarizingChannel(std::uint32_t nQubits, double p); // id by arity: _1q, _2q, _nq
...
Result<ChannelPtr> thermalPreparationChannel(double pThermal); // SPAM: bit flip at shot start (T04 §10.3)

// ---- duration-dependent channels: evaluated at Context::durationS.
Result<ChannelPtr> thermalRelaxationChannel(double t1S, double t2S, double pThermal); // rejects T2 > 2T1
...
// R_Z(2π (fixedHz + Context::detuningHz) · duration): the per-shot drift sample comes through the context.
...
Result<std::vector<LindbladTerm>> thermalRelaxationLindblad(double t1S, double t2S, double pThermal, std::uint32_t levels = 2);
