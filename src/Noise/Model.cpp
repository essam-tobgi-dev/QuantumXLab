...
bool has(const Replacements* rep, const char* key) { return rep && rep->contains(key); }
...
Status checkLifetime(double v, const std::string& path) { // +inf = absent process
    if (!(v > 0.0)) return fail(err::InvalidParameter, std::format("{} = {} must be positive", path, v));
    return {};
}
Status checkScale(double v, bool allowZero, const char* name) {
    if (!std::isfinite(v) || v < 0.0 || (!allowZero && v == 0.0))
        return fail(err::InvalidParameter, std::format("overrides.{} = {} must be finite and {}", name, v, allowZero ? ">= 0" : "> 0"));
    return {};
}
} // namespace
...
void Overrides::setEnabled(std::string_view channelId, bool on) {
    ...
    if (on && it != disable.end()) disable.erase(it);
    if (!on && it == disable.end()) disable.emplace_back(channelId);
}
...
const QubitNoise* NoiseModel::qubit(QubitIndex q) const { return q.get() < qubits_.size() ? &qubits_[q.get()] : nullptr; }

const GateNoise* NoiseModel::gate(std::string_view gateName, std::span<const QubitIndex> qubits) const {
...
std::vector<const GateNoise*> NoiseModel::gates() const {
    ...
    for (const auto& [name, byKey] : gates_)
        for (const auto& [key, entry] : byKey) out.push_back(&entry.noise);
    return out;
}

const EdgeNoise* NoiseModel::edge(std::uint32_t a, std::uint32_t b) const {
    if (auto it = edges_.find(std::format("{}-{}", a, b)); it != edges_.end()) return &it->second.noise;
    if (auto it = edges_.find(std::format("{}-{}", b, a)); it != edges_.end()) return &it->second.noise;
    return nullptr;
}

const NoiseModel::GateEntry* NoiseModel::findGate(std::string_view gateName, std::span<const QubitIndex> qubits) const {
    std::vector<std::uint32_t> ids;
    for (auto q : qubits) ids.push_back(q.get());
    const std::string key = targetKey(ids);
    auto lookup = [&](std::string_view name) -> const GateEntry* {
        auto g = gates_.find(std::string(name));
        if (g == gates_.end()) return nullptr;
        if (auto e = g->second.find(key); e != g->second.end()) return &e->second;
        if (ids.size() == 2) { // a two-qubit entry is stored in calibration order
            const std::uint32_t flipped[2] = {ids[1], ids[0]};
            if (auto e = g->second.find(targetKey(flipped)); e != g->second.end()) return &e->second;
        }
        return nullptr;
    };
    if (const GateEntry* e = lookup(gateName)) return e;
    const std::string& fallback = ids.size() == 1 ? fallback1q_ : ids.size() == 2 ? fallback2q_ : std::string{};
    if (!fallback.empty() && fallback != gateName) return lookup(fallback); // spec 08 §8 arity default
    return nullptr;
}
...
Status NoiseModel::setLinePhotonNumbers(std::span<const double> photons) {
    if (photons.size() > rawQubits_.size())
        return fail(err::UnknownTarget, std::format("{} line photon numbers for {} qubits", photons.size(), rawQubits_.size()));
    for (std::size_t q = 0; q < photons.size(); ++q)
        if (!std::isfinite(photons[q]) || photons[q] < 0.0)
            return fail(err::InvalidParameter, std::format("line photon number {} of qubit {} must be finite and >= 0", photons[q], q));
    ...
    if (!d) {
        for (std::size_t q = 0; q < rawQubits_.size(); ++q) rawQubits_[q].linePhotons = previous[q];
...
Status NoiseModel::deriveQubits(const Overrides& o, Derived& out) const {
    static constexpr const char* kKnown[] = {"t1_us", "t2_us", "t2_star_us", "p_thermal", "reset_error", "readout_e01",
                                             "readout_e10", "readout_duration_ns", "readout_crosstalk_dephasing",
                                             "gate_error_1q", "duration_1q_ns", "coherent_fraction_1q", "leakage_1q"};
    for (const auto& [q, fields] : o.replaceQubit) {
        if (q >= rawQubits_.size())
            out.warnings.push_back(std::format("warn: overrides.replace.qubits.{} names a qubit outside the model; ignored", q));
        for (const auto& [key, v] : fields)
            if (std::find(std::begin(kKnown), std::end(kKnown), key) == std::end(kKnown))
                out.warnings.push_back(std::format("warn: overrides.replace.qubits.{}.{} is not a known field; ignored", q, key));
    }
    ...
    for (std::size_t q = 0; q < rawQubits_.size(); ++q) {
        ...
        if (qn.t2S > 2.0 * qn.t1S) { // spec 08 §2.3: clamp and warn, never an error
            if (std::isfinite(qn.t2S) && qn.t2S > 2.0 * qn.t1S * (1.0 + 1e-9))
                out.warnings.push_back(std::format("warn: T2 = {:.6g} us exceeds 2 T1 = {:.6g} us on qubit {}; clamped to 2 T1",
                                                   t2Us, 2.0 * t1Us, q));
        ...
        if (qn.t2StarS > qn.t2S) { // T2* <= T2 (spec 09 §3)
            if (std::isfinite(qn.t2StarS) && qn.t2StarS > qn.t2S * (1.0 + 1e-9))
                out.warnings.push_back(std::format("warn: T2* = {:.6g} us exceeds T2 on qubit {}; clamped to T2", t2StarUs, q));
            qn.t2StarS = qn.t2S;
        }
        ...
        if (!std::isfinite(rec.linePhotons) || rec.linePhotons < 0.0)
            return fail(err::InvalidParameter, std::format("{}.line_photon_number = {} must be finite and >= 0", path, rec.linePhotons));
        ...
        if (has(rep, "readout_e01")) { const double e = pick(rep, "readout_e01", 0.0); qn.readoutAssignment[0] = {1.0 - e, e}; }
        if (has(rep, "readout_e10")) { const double e = pick(rep, "readout_e10", 0.0); qn.readoutAssignment[1] = {e, 1.0 - e}; }
        if (o.scaleReadoutError != 1.0)
            for (std::size_t r = 0; r < 2; ++r) { // scale the error (off-diagonal) of each row
                const double e = std::clamp(qn.readoutAssignment[r][1 - r] * o.scaleReadoutError, 0.0, 1.0);
                qn.readoutAssignment[r][1 - r] = e;
                qn.readoutAssignment[r][r] = 1.0 - e;
            }
        QXL_TRY(validateAssignment(toRealMatrix(qn.readoutAssignment), path + ".readout.assignment"));
        const double roNs = pick(rep, "readout_duration_ns", rec.readoutDurationNs);
        if (!std::isfinite(roNs) || roNs < 0.0)
            return fail(err::InvalidParameter, std::format("{}.readout.duration_ns = {} must be finite and >= 0", path, roNs));
