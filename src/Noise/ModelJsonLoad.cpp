...
// No exception crosses the module boundary (spec 02 §1): a stray nlohmann exception becomes
// BadJson.
...
Status NoiseModel::readReadout(const Json& data) {
    if (auto groups = data.find("readout_groups"); groups != data.end()) {
        if (!groups->is_array())
            return fail(err::BadJson, "noise.readout_groups: expected an array");
        for (std::size_t i = 0; i < groups->size(); ++i) {
            const std::string path = std::format("noise.readout_groups[{}]", i);
            const Json& g = (*groups)[i];
            QXL_TRY(objectAt(g, path));
            ReadoutGroup group;
            if (!g.contains("qubits"))
                return fail(err::BadJson, path + ".qubits: missing field");
            QXL_TRY_ASSIGN(group.qubits, indexList(g["qubits"], path + ".qubits"));
            const std::size_t k = group.qubits.size();
            if (k == 0 || k > 4)
                return fail(
                    err::BadReadout,
                    std::format("{}.qubits: a readout group holds 1..4 qubits (spec 08 §3)", path));
            for (std::size_t a = 0; a < k; ++a)
                for (std::size_t b = a + 1; b < k; ++b)
                    if (group.qubits[a] == group.qubits[b])
                        return fail(err::BadJson, std::format("{}.qubits: repeated qubit", path));
            if (auto a = g.find("assignment"); a != g.end() && !a->is_null()) {
                ...
                if (m.rows != (std::size_t{1} << k))
                    return fail(err::BadReadout,
                                std::format("{}.assignment: {} qubits need a {}x{} matrix", path, k,
                                            1u << k, 1u << k));
    ...
    if (auto lines = data.find("feedlines"); lines != data.end()) {
        if (!lines->is_array())
            return fail(err::BadJson, "noise.feedlines: expected an array");
        for (std::size_t i = 0; i < lines->size(); ++i) {
            QXL_TRY_ASSIGN(auto line,
                           indexList((*lines)[i], std::format("noise.feedlines[{}]", i)));
...
Status NoiseModel::readOverrides(const Json& data) {
    ...
    overridesExtra_ = unknownFields(o, {"scale_t1", "scale_t2", "scale_gate_error",
                                        "scale_readout_error", "disable", "replace"});
    ...
    if (auto d = o.find("disable"); d != o.end()) {
        if (!d->is_array())
            return fail(err::BadJson, path + ".disable: expected an array of channel ids");
        for (std::size_t i = 0; i < d->size(); ++i) {
            if (!(*d)[i].is_string())
                return fail(err::BadJson,
                            std::format("{}.disable[{}]: expected a channel id", path, i));
    ...
    if (auto rep = o.find("replace"); rep != o.end()) {
        ...
        if (!unknown.empty())
            overridesExtra_["replace"] = std::move(unknown);
        if (auto qs = rep->find("qubits"); qs != rep->end()) {
            QXL_TRY(objectAt(*qs, path + ".replace.qubits"));
            for (auto q = qs->begin(); q != qs->end(); ++q) {
                QXL_TRY_ASSIGN(const std::uint32_t index,
                               indexFrom(q.key(), path + ".replace.qubits"));
                QXL_TRY_ASSIGN(overrides_.replaceQubit[index],
                               numberMap(q.value(), path + ".replace.qubits." + q.key()));
            }
        }
        if (auto es = rep->find("edges"); es != rep->end()) {
            ...
            for (auto e = es->begin(); e != es->end(); ++e) {
                ...
                if (ends.size() != 2)
                    return fail(err::BadJson, epath + ": an edge key names two qubits 'a-b'");
                QXL_TRY_ASSIGN(overrides_.replaceEdge[targetKey(ends)],
                               numberMap(e.value(), epath));
            }
        }
    }
    return {};
...
Result<NoiseModel> NoiseModel::fromJson(const Json& data) {
    try {
        ...
        NoiseModel m;
        m.extra_ =
            unknownFields(data, {"schema", "source", "levels", "qubits", "gates", "edges",
                                 "readout_groups", "feedlines", "fallback_gates", "overrides"});
        QXL_TRY_ASSIGN(const std::string schema, text(data, "noise", "schema"));
        if (!schema.empty() && schema != kSchema)
            return fail(err::BadJson,
                        std::format("noise.schema: expected '{}', found '{}'", kSchema, schema));
        ...
        if (auto l = data.find("levels"); l != data.end()) {
            if (!l->is_number_unsigned() ||
                (l->get<std::uint64_t>() != 2 && l->get<std::uint64_t>() != 3))
                return fail(err::LevelsMismatch,
                            std::format("noise.levels: 2 or 3 expected, found {}", l->dump()));
        ...
        // Qubits referenced by gates, edges, groups or feedlines but not listed are noiseless
        // slots.
        std::size_t needed = m.rawQubits_.size();
        auto cover = [&](std::uint32_t q) {
            needed = std::max(needed, static_cast<std::size_t>(q) + 1);
        };
        for (const auto& [name, byKey] : m.rawGates_)
            for (const auto& [key, g] : byKey)
                for (auto q : g.qubits)
                    cover(q);
        for (const auto& [key, e] : m.rawEdges_) {
            cover(e.a);
            cover(e.b);
        }
