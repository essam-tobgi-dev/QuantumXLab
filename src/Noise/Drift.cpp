...
double driftSigmaFromT2Star(double t2StarS) {
    if (!(t2StarS > 0.0) || !std::isfinite(t2StarS))
        return 0.0;
...
Result<std::vector<DetuningNode>> gaussHermiteDetunings(double sigmaHz, std::size_t nodes) {
    if (nodes == 0 || nodes > 64)
        return fail(err::InvalidParameter,
                    std::format("Gauss-Hermite rule needs 1..64 nodes, {} requested", nodes));
    if (!(sigmaHz >= 0.0) || !std::isfinite(sigmaHz))
        return fail(err::InvalidParameter, "detuning sigma must be finite and >= 0");
    if (sigmaHz == 0.0)
        return std::vector<DetuningNode>{{0.0, 1.0}};
    ...
    for (std::size_t i = 0; i < half; ++i) {
        if (i == 0)
            z = std::sqrt(2.0 * nn + 1.0) - 1.85575 * std::pow(2.0 * nn + 1.0, -1.0 / 6.0);
        else if (i == 1)
            z -= 1.14 * std::pow(nn, 0.426) / z;
        else if (i == 2)
            z = 1.86 * z - 0.86 * x[0];
        else if (i == 3)
            z = 1.91 * z - 0.91 * x[1];
        ...
        for (int iter = 0; iter < 100 && !converged; ++iter) {
            ...
            for (std::size_t j = 0; j < n; ++j) {
                ...
                const double next =
                    z * std::sqrt(2.0 / (jj + 1.0)) * h - std::sqrt(jj / (jj + 1.0)) * hPrev;
                hPrev = h;
                h = next;
            }
        ...
        if (!converged)
            return fail(ErrorCode::Internal, "Gauss-Hermite Newton iteration did not converge");
        x[i] = z;
        x[n - 1 - i] = -z;
        w[i] = w[n - 1 - i] = 2.0 / (derivative * derivative);
    }
    if (n % 2 == 1)
        x[half - 1] = 0.0; // the middle root is exactly zero
    std::vector<DetuningNode> out(n);
    double total = 0.0;
    for (std::size_t i = 0; i < n; ++i)
        total += w[i];
    for (std::size_t i = 0; i < n; ++i)
        out[i] = {std::numbers::sqrt2 * sigmaHz * x[n - 1 - i],
                  w[n - 1 - i] / total}; // ascending detuning
    return out;
}
