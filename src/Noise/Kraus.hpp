...
struct Kraus {
    ...
    std::vector<std::uint32_t> pauliIndices; // if isPauli: Pauli-string index of ops[k] (see PauliTwirl)
    ...
    static Result<Kraus> make(std::vector<num::Matrix> ops, std::uint32_t arity, std::uint32_t levels = 2);
...
num::Matrix pauliStringMatrix(std::uint32_t index, std::uint32_t arity);     // ⊗ with targets[0] least significant
std::string pauliStringLabel(std::uint32_t index, std::uint32_t arity);      // MSB-first, like qsim::PauliString
Result<std::uint32_t> pauliStringIndex(std::string_view targetOrderLetters); // "ZX": Z on targets[0], X on targets[1]
...
// Pauli twirl p_P = Σ_k |Tr(P K_k)|²/d² — the diagonal of the χ matrix (T04 §5.3). Qubit channels only.
Result<PauliTwirl> pauliTwirl(const Kraus& k);
// Entanglement fidelity to a target unitary, F_e = Σ_k |Tr(U†K_k)|²/d² (T10 (1.2)); identity if empty.
...
// Depolarizing-equivalent strength p = d/(d−1) · (1 − F_avg) of a channel (T10 (1.5), spec 08 §4.1).
