...
// (spec 04 §8: every read returns Result and names the field path; spec 23 §1: unknown fields
// kept).
...
inline Status objectAt(const Json& j, const std::string& path) {
    if (!j.is_object())
        return fail(err::BadJson, std::format("{}: expected an object", path));
    return {};
}
...
inline Result<double> number(const Json& o, const std::string& path, const char* key,
                             double fallback) {
    auto it = o.find(key);
    if (it == o.end())
        return fallback;
    if (!it->is_number())
        return fail(err::BadJson, std::format("{}.{}: expected a number", path, key));
...
inline Result<std::string> text(const Json& o, const std::string& path, const char* key) {
    ...
    if (it == o.end())
        return std::string{};
    if (!it->is_string())
        return fail(err::BadJson, std::format("{}.{}: expected a string", path, key));
...
inline Result<std::vector<std::uint32_t>> targetsFrom(std::string_view key, const std::string& path,
                                                      std::size_t maxCount) {
    ...
    while (true) {
        ...
        if (std::find(out.begin(), out.end(), q) != out.end())
            return fail(err::BadJson, std::format("{}: repeated qubit {}", path, q));
        out.push_back(q);
    ...
    if (out.size() > maxCount)
        return fail(err::BadJson, std::format("{}: at most {} targets expected", path, maxCount));
    return out;
}
inline Result<std::vector<std::uint32_t>> indexList(const Json& j, const std::string& path) {
    if (!j.is_array())
        return fail(err::BadJson, std::format("{}: expected an array of qubit indices", path));
    std::vector<std::uint32_t> out;
...
inline Result<num::RealMatrix> realMatrix(const Json& j, const std::string& path) {
    if (!j.is_array() || j.empty())
        return fail(err::BadJson, std::format("{}: expected a square matrix", path));
    const std::size_t n = j.size();
    num::RealMatrix m(n, n);
    for (std::size_t r = 0; r < n; ++r) {
        if (!j[r].is_array() || j[r].size() != n)
            return fail(err::BadJson, std::format("{}[{}]: expected {} numbers", path, r, n));
        for (std::size_t c = 0; c < n; ++c) {
            if (!j[r][c].is_number())
                return fail(err::BadJson, std::format("{}[{}][{}]: expected a number", path, r, c));
...
inline Result<std::map<std::string, double>> numberMap(const Json& j, const std::string& path) {
    ...
    for (auto it = j.begin(); it != j.end(); ++it) {
        if (!it->is_number())
            return fail(err::BadJson, std::format("{}.{}: expected a number", path, it.key()));
