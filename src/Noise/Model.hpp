...
public:
    ...
    static Result<NoiseModel> fromCalibration(const hw::Device& device, const hw::Calibration& calibration,
    ...
    std::vector<AttachedChannel> channelsFor(std::string_view gate, std::span<const QubitIndex> qubits) const;
    ...
    std::vector<AttachedChannel> crosstalkChannels(double durationS, std::span<const QubitIndex> inTwoQubitGates = {}) const;
    // Photon shot-noise dephasing of the unmeasured qubits on the feedlines being read (spec 08 §3).
    ...
    Result<std::vector<qsim::CollapseOp>> lindbladOperators(std::span<const QubitIndex> qubits,
                                                            std::span<const std::uint32_t> siteDims) const;
    Result<std::vector<qsim::CollapseOp>> lindbladOperators(std::span<const std::uint32_t> siteDims) const; // site k = qubit k
...
private:
    ...
    struct QubitChannels { ChannelPtr relaxation, drift, preparation, reset, measurement; };
    struct GateEntry {
        GateNoise noise;
    ...
    std::map<std::string, std::map<std::string, GateRecord>> rawGates_; // gate → target key "0" / "0-1"
    ...
    std::string fallback1q_, fallback2q_; // default gate per arity for uncalibrated names (spec 08 §8)
    Overrides overrides_;
    core::Json extra_ = core::Json::object(), sourceExtra_ = core::Json::object(), overridesExtra_ = core::Json::object();
