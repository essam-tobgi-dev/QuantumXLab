...
Status checkProbability(double p, std::string_view name) {
    if (!(p >= 0.0 && p <= 1.0)) return fail(err::InvalidParameter, std::format("{} = {} is outside [0, 1]", name, p));
    return {};
}
Status checkDuration(double tS) {
    if (!(tS >= 0.0) || !std::isfinite(tS)) return fail(err::InvalidParameter, std::format("duration {} s must be finite and >= 0", tS));
    return {};
}
Status checkLifetime(double tS, std::string_view name) { // +inf = process absent
    if (!(tS > 0.0)) return fail(err::InvalidParameter, std::format("{} = {} s must be positive", name, tS));
    return {};
}
// (2.2) with s = √(1−γ) supplied separately so the duration form keeps e^{−t/2T1} to full precision.
...
Result<Kraus> amplitudeDamping(double gamma) { return generalizedAmplitudeDamping(gamma, 0.0); }
...
double thermalPopulationFromPhotons(double n) { return n > 0.0 ? n / (1.0 + 2.0 * n) : 0.0; }
double photonsFromThermalPopulation(double p) {
    if (p <= 0.0) return 0.0;
    if (p >= 0.5) return std::numeric_limits<double>::infinity();
    return p / (1.0 - 2.0 * p);
}
...
Result<double> pureDephasingTime(double t1S, double t2S) {
    ...
    if (t2S > 2.0 * t1S * (1.0 + 1e-9))
        return fail(err::Unphysical, std::format("T2 = {} s exceeds 2 T1 = {} s (T04 (4.4))", t2S, 2.0 * t1S));
    const double rate = 1.0 / t2S - 0.5 / t1S;        // (2.4)
    if (rate <= 1e-12 / t2S) return std::numeric_limits<double>::infinity();
    return 1.0 / rate;
}
...
Result<Kraus> measurementDephasing(double readoutDurationS, double t2S, double scale) {
    ...
    // spec 08 §3 with the (2.3) convention: at scale 1 the neighbour's coherence falls by e^{−t_ro/T2}.
...
Result<Kraus> gaussianDephasing(double sigmaHz, double tS) {
    if (!(sigmaHz >= 0.0) || !std::isfinite(sigmaHz))
        return fail(err::InvalidParameter, std::format("detuning sigma {} Hz must be finite and >= 0", sigmaHz));
    QXL_TRY(checkDuration(tS));
