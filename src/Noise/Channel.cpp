...
class BuiltChannel final : public IChannel {
  public:
    ...
    BuiltChannel(std::string_view cid, std::uint32_t arity, std::uint32_t levels, Builder build,
        ...
        : id_(cid), arity_(arity), levels_(levels), build_(std::move(build)),
          terms_(std::move(terms)), description_(std::move(description)) {}
...
Result<ChannelPtr> built(std::string_view cid, std::uint32_t arity, BuiltChannel::Builder build,
    ...
    return ChannelPtr(std::make_shared<BuiltChannel>(cid, arity, 2u, std::move(build),
                                                     std::move(terms), std::move(description)));
}
} // namespace
...
Result<ChannelPtr> fixedChannel(std::string_view channelId, Result<Kraus> kraus,
    ...
    if (!kraus)
        return std::unexpected(std::move(kraus.error()));
...
Result<ChannelPtr> bitFlipChannel(double p) {
    return fixedChannel(id::BitFlip, channels::bitFlip(p), std::format("p={:.6g}", p));
}
Result<ChannelPtr> phaseFlipChannel(double p) {
    return fixedChannel(id::PhaseFlip, channels::phaseFlip(p), std::format("p={:.6g}", p));
}
Result<ChannelPtr> bitPhaseFlipChannel(double p) {
    return fixedChannel(id::BitPhaseFlip, channels::bitPhaseFlip(p), std::format("p={:.6g}", p));
}
Result<ChannelPtr> pauliChannel(double px, double py, double pz) {
    return fixedChannel(id::Pauli, channels::pauli(px, py, pz),
                        std::format("px={:.6g} py={:.6g} pz={:.6g}", px, py, pz));
}
Result<ChannelPtr> depolarizingChannel(std::uint32_t n, double p) {
    const std::string_view cid = n == 1   ? id::Depolarizing1q
                                 : n == 2 ? id::Depolarizing2q
                                          : id::DepolarizingNq;
...
Result<ChannelPtr> overRotationChannel(std::string_view axis, double eps) {
    return fixedChannel(id::OverRotation, channels::overRotation(axis, eps),
                        std::format("axis={} epsilon={:.6g}rad", axis, eps));
}
Result<ChannelPtr> leakageChannel(double pLeak, double pSeep) {
    return fixedChannel(id::Leakage, channels::leakage(pLeak, pSeep),
                        std::format("p_L={:.6g} p_S={:.6g}", pLeak, pSeep));
}
Result<ChannelPtr> resetErrorChannel(double p) {
    return fixedChannel(id::ResetError, channels::resetError(p), std::format("p={:.6g}", p));
}
Result<ChannelPtr> thermalPreparationChannel(double pTh) {
    return fixedChannel(id::ThermalPreparation, channels::bitFlip(pTh),
                        std::format("p_th={:.6g}", pTh));
}

Result<std::vector<LindbladTerm>> thermalRelaxationLindblad(double t1S, double t2S, double pTh,
                                                            std::uint32_t levels) {
    if (levels < 2)
        return fail(err::BadDimensions, "a site needs at least 2 levels");
    if (!(pTh >= 0.0 && pTh <= 1.0))
        return fail(err::InvalidParameter, std::format("p_th = {} is outside [0, 1]", pTh));
    ...
    for (std::uint32_t n = 1; n < levels; ++n)
        lower(n - 1, n) = std::sqrt(static_cast<double>(n));
    ...
    if (gamma1 > 0.0) {
        ...
        if (pTh > 0.0)
            terms.push_back({"Tth", num::scale(num::adjoint(lower), std::sqrt(gamma1 * pTh))});
    }
    if (std::isfinite(tPhi)) {
        ...
        if (levels == 2) { // √(γφ/2)·Z, T04 (4.1)
            ...
            for (std::uint32_t n = 0; n < levels; ++n)
                op(n, n) = std::sqrt(2.0 * gphi) * static_cast<double>(n);
        }
...
Result<ChannelPtr> thermalRelaxationChannel(double t1S, double t2S, double pTh) {
    ...
    return built(
        id::ThermalRelaxation, 1,
        [=](const Context& c) { return channels::thermalRelaxation(t1S, t2S, c.durationS, pTh); },
        std::move(terms),
...
Result<ChannelPtr> amplitudeDampingChannel(double t1S, double pTh) {
    QXL_TRY_ASSIGN(auto terms,
                   thermalRelaxationLindblad(t1S, 2.0 * t1S, pTh, 2)); // T2 = 2T1: no Tφ term
    return built(
        id::AmplitudeDamping, 1,
        [=](const Context& c) { return channels::amplitudeDampingOver(t1S, c.durationS, pTh); },
...
Result<ChannelPtr> phaseDampingChannel(double tPhiS) {
    ...
    return built(
        id::PhaseDamping, 1,
        [=](const Context& c) { return channels::phaseDampingOver(tPhiS, c.durationS); },
        std::move(terms), std::format("T_phi={:.6g}us", tPhiS * 1e6));
}
Result<ChannelPtr> zzCrosstalkChannel(double zetaHz) {
    return built(
        id::ZzCrosstalk, 2,
        [=](const Context& c) { return channels::zzCrosstalk(zetaHz, c.durationS); }, {},
        std::format("zeta={:.6g}Hz", zetaHz));
}
Result<ChannelPtr> measurementDephasingChannel(double t2S, double scale) {
    return built(
        id::MeasurementDephasing, 1,
        [=](const Context& c) { return channels::measurementDephasing(c.durationS, t2S, scale); },
...
Result<ChannelPtr> detuningPhaseChannel(double fixedHz) {
    return built(
        id::DetuningPhase, 1,
        [=](const Context& c) {
            return channels::detuningPhase(fixedHz + c.detuningHz, c.durationS);
        },
...
Result<ChannelPtr> gaussianDephasingChannel(double sigmaHz) {
    return built(
        id::DetuningDrift, 1,
        [=](const Context& c) { return channels::gaussianDephasing(sigmaHz, c.durationS); }, {},
...
Result<ChannelPtr> driftChannel(double sigmaHz) {
    ...
    auto build = [=](const Context& c) -> Result<Kraus> {
        if (c.perShotDrift)
            return channels::detuningPhase(c.detuningHz, c.durationS);
        return channels::gaussianDephasing(sigmaHz, c.durationS);
    };
    return built(id::DetuningDrift, 1, std::move(build), {},
                 std::format("sigma_f={:.6g}Hz (T2*={:.6g}us)", sigmaHz,
                             sigmaHz > 0.0
                                 ? 1e6 * std::numbers::sqrt2 / (2.0 * std::numbers::pi * sigmaHz)
                                 : 0.0));
}
