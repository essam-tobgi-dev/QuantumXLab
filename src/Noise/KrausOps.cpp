...
Result<Kraus> compose(const Kraus& first, const Kraus& second) {
    if (first.arity != second.arity || first.levels != second.levels)
        return fail(err::BadDimensions,
                    "composed channels must act on the same targets and levels");
    if (first.isPauli && second.isPauli) {
        ...
        for (std::size_t a = 0; a < first.ops.size(); ++a)
            for (std::size_t b = 0; b < second.ops.size(); ++b)
                w[first.pauliIndices[a] ^ second.pauliIndices[b]] +=
                    first.pauliWeights[a] * second.pauliWeights[b];
    ...
    for (const auto& B : second.ops)
        for (const auto& A : first.ops) {
            ...
            if (num::maxAbsNorm(prod) > 0.0)
                ops.push_back(std::move(prod));
        }
    if (ops.empty())
        return fail(err::NotTracePreserving, "composition produced no operators");
    return Kraus::make(std::move(ops), first.arity, first.levels);
}

Result<Kraus> tensor(const Kraus& low, const Kraus& high) {
    if (low.levels != high.levels)
        return fail(err::BadDimensions, "tensored channels must share the site dimension");
    ...
    if (low.isPauli && high.isPauli && arity <= 4) {
        ...
        for (std::size_t a = 0; a < low.ops.size(); ++a)
            for (std::size_t b = 0; b < high.ops.size(); ++b)
                w[low.pauliIndices[a] | (high.pauliIndices[b] << (2 * low.arity))] +=
                    low.pauliWeights[a] * high.pauliWeights[b];
        return Kraus::fromPauliWeights(arity, w);
    }
    std::vector<Matrix> ops;
    for (const auto& H : high.ops)
        for (const auto& L : low.ops)
            ops.push_back(num::kron(H, L)); // left factor = more significant
...
Result<Kraus> expandLevels(const Kraus& k, std::uint32_t levels) {
    ...
    if (k.levels != 2 || levels < 2)
        return fail(
            err::LevelsMismatch,
            std::format("cannot map a {}-level channel onto {}-level sites", k.levels, levels));
    ...
    for (std::size_t b = 0; b < dq; ++b) {
        ...
        for (std::uint32_t s = 0; s < k.arity; ++s) {
            idx += ((b >> s) & 1u) * stride;
            stride *= levels;
        }
        siteIndex[b] = idx;
    ...
    for (auto i : siteIndex)
        computational[i] = true;
    std::vector<Matrix> ops;
    for (std::size_t n = 0; n < k.ops.size(); ++n) {
        Matrix big(D, D);
        for (std::size_t a = 0; a < dq; ++a)
            for (std::size_t b = 0; b < dq; ++b)
                big(siteIndex[a], siteIndex[b]) = k.ops[n](a, b);
...
Result<PauliTwirl> pauliTwirl(const Kraus& k) {
    if (k.levels != 2)
        return fail(err::NotPauli, "a Pauli twirl exists only for qubit (d = 2) channels");
    if (k.arity == 0 || k.arity > 4)
        return fail(err::InvalidParameter, "Pauli twirl supports 1..4 qubits");
    ...
    if (k.isPauli) {
        for (std::size_t n = 0; n < k.ops.size(); ++n)
            t.probs[k.pauliIndices[n]] += k.pauliWeights[n];
        return t;
    }
    ...
    for (std::size_t p = 0; p < count; ++p) {
        ...
        for (std::size_t n = 0; n < k.ops.size(); ++n)
            c[n][p] = num::trace(num::matmul(P, k.ops[n])) / d;
    }
    ...
    for (std::size_t p = 0; p < count; ++p)
        for (std::size_t q = 0; q < count; ++q) {
            Complex chi{};
            for (std::size_t n = 0; n < k.ops.size(); ++n)
                chi += c[n][p] * std::conj(c[n][q]);
            if (p == q)
                t.probs[p] = chi.real();
            else
                offDiagonal = std::max(offDiagonal, std::abs(chi));
        }
...
double processFidelity(const Kraus& k, num::ConstMatrixView target) {
    ...
    if (target.rows == 0) {
        for (const auto& K : k.ops)
            sum += std::norm(num::trace(K));
    } else {
        const Matrix ud = num::adjoint(target);
        for (const auto& K : k.ops)
            sum += std::norm(num::trace(num::matmul(ud, K)));
    }
    return sum / (d * d);
}
...
Result<Matrix> applyToDensity(const Kraus& k, const Matrix& rho) {
    ...
    if (rho.rows != d || rho.cols != d)
        return fail(err::BadDimensions,
                    std::format("density matrix is {}x{}, channel acts on {}x{}", rho.rows,
                                rho.cols, d, d));
    Matrix out(d, d);
    for (const auto& K : k.ops)
        out += num::matmul(num::matmul(K, rho), num::adjoint(K));
    return out;
}
