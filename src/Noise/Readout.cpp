...
std::size_t localIndex(std::span<const std::uint8_t> bits, const ReadoutFactor& f) {
    ...
    for (std::size_t k = 0; k < f.positions.size(); ++k)
        i |= static_cast<std::size_t>(bits[f.positions[k]] & 1u) << k;
    return i;
}
} // namespace
...
Status validateAssignment(const num::RealMatrix& m, std::string_view what) {
    if (m.rows == 0 || m.rows != m.cols)
        return fail(err::BadReadout, std::format("{}: assignment matrix must be square", what));
    for (std::size_t i = 0; i < m.rows; ++i) {
        ...
        for (std::size_t j = 0; j < m.cols; ++j) {
            ...
            if (!(v >= 0.0 && v <= 1.0))
                return fail(
                    err::BadReadout,
                    std::format("{}: entry ({}, {}) = {} is not a probability", what, i, j, v));
            sum += v;
        }
        if (std::abs(sum - 1.0) > 1e-6)
            return fail(err::BadReadout,
                        std::format("{}: row {} sums to {:.9g}, not 1 (row-stochastic, spec 08 §3)",
                                    what, i, sum));
    }
    return {};
}
...
double readoutFidelity(const Assignment2& a) {
    return 1.0 - 0.5 * (a[0][1] + a[1][0]);
}

Result<ReadoutModel> ReadoutModel::make(std::vector<QubitIndex> qubits,
    ...
    for (std::size_t n = 0; n < factors.size(); ++n) {
        ...
        if (k == 0 || k > 4)
            return fail(
                err::BadReadout,
                std::format("readout factor {} spans {} qubits; groups hold 1..4 (spec 08 §3)", n,
                            k));
        if (f.matrix.rows != (std::size_t{1} << k))
            return fail(err::BadReadout,
                        std::format("readout factor {} needs a {}x{} matrix", n, 1u << k, 1u << k));
        QXL_TRY(validateAssignment(f.matrix, std::format("readout factor {}", n)));
        for (auto pos : f.positions) {
            if (pos >= qubits.size())
                return fail(
                    err::BadReadout,
                    std::format("readout factor {} refers to position {} of {} measured qubits", n,
                                pos, qubits.size()));
            ++covered[pos];
        }
    }
    for (std::size_t k = 0; k < covered.size(); ++k)
        if (covered[k] != 1)
            return fail(
                err::BadReadout,
                std::format("measured qubit {} is covered by {} readout factors, expected 1",
                            qubits[k].get(), covered[k]));
    ReadoutModel m;
...
Result<ReadoutModel> ReadoutModel::independent(std::vector<QubitIndex> qubits,
                                               std::span<const Assignment2> assignment) {
    if (assignment.size() != qubits.size())
        return fail(err::BadReadout, std::format("{} assignment matrices for {} qubits",
                                                 assignment.size(), qubits.size()));
    std::vector<ReadoutFactor> factors;
    for (std::size_t k = 0; k < qubits.size(); ++k)
        factors.push_back({{k}, toRealMatrix(assignment[k])});
...
ReadoutModel ReadoutModel::ideal(std::vector<QubitIndex> qubits) {
    ReadoutModel m;
    for (std::size_t k = 0; k < qubits.size(); ++k)
        m.factors_.push_back({{k}, num::RealMatrix::identity(2)});
    m.qubits_ = std::move(qubits);
    return m;
}
...
Status ReadoutModel::applyToBits(std::span<std::uint8_t> bits, core::Random& rng) const {
    if (bits.size() != qubits_.size())
        return fail(err::BadReadout,
                    std::format("{} bits for {} measured qubits", bits.size(), qubits_.size()));
    for (const auto& f : factors_) {
        ...
        for (std::size_t k = 0; k < f.positions.size(); ++k)
            bits[f.positions[k]] = static_cast<std::uint8_t>((j >> k) & 1u);
    }
    return {};
}
...
Result<std::size_t> ReadoutModel::applyToIndex(std::size_t prepared, core::Random& rng) const {
    ...
    for (std::size_t k = 0; k < bits.size(); ++k)
        bits[k] = static_cast<std::uint8_t>((prepared >> k) & 1u);
    QXL_TRY(applyToBits(bits, rng));
    std::size_t out = 0;
    for (std::size_t k = 0; k < bits.size(); ++k)
        out |= static_cast<std::size_t>(bits[k]) << k;
    return out;
}

Result<qsim::Counts> ReadoutModel::applyToCounts(const qsim::Counts& ideal,
    ...
    for (const auto& [key, count] : sorted) {
        if (key.size() != n)
            return fail(err::BadReadout,
                        std::format("count key '{}' has {} bits, {} qubits are measured", key,
                                    key.size(), n));
        for (std::uint64_t s = 0; s < count; ++s) {
            for (std::size_t k = 0; k < n; ++k)
                bits[k] = key[n - 1 - k] == '1' ? 1 : 0; // MSB-first key
            QXL_TRY(applyToBits(bits, rng));
...
Status ReadoutModel::checkLength(std::size_t n) const {
    if (qubits_.size() > kMaxProbabilityBits)
        return fail(err::InvalidParameter,
                    std::format("probability vectors are limited to {} measured qubits",
                                kMaxProbabilityBits));
    if (n != (std::size_t{1} << qubits_.size()))
        return fail(err::BadReadout,
                    std::format("distribution has {} entries, 2^{} expected", n, qubits_.size()));
    return {};
}

void ReadoutModel::applyAlong(std::vector<double>& v, const ReadoutFactor& f,
                              const num::RealMatrix& b) const {
    const std::size_t dim = b.rows;
    std::size_t mask = 0;
    std::vector<std::size_t> offset(dim, 0);
    for (std::size_t k = 0; k < f.positions.size(); ++k)
        mask |= std::size_t{1} << f.positions[k];
    for (std::size_t l = 0; l < dim; ++l)
        for (std::size_t k = 0; k < f.positions.size(); ++k)
            if ((l >> k) & 1u)
                offset[l] |= std::size_t{1} << f.positions[k];
    std::vector<double> in(dim);
    for (std::size_t base = 0; base < v.size(); ++base) {
        if (base & mask)
            continue;
        for (std::size_t l = 0; l < dim; ++l)
            in[l] = v[base | offset[l]];
...
ReadoutModel::applyToProbabilities(std::span<const double> ideal) const {
    ...
    for (const auto& f : factors_) {
        ...
        for (std::size_t i = 0; i < mt.rows; ++i)
            for (std::size_t j = 0; j < mt.cols; ++j)
                mt(j, i) = f.matrix(i, j);
        applyAlong(q, f, mt);
    }
    return q;
...
Result<num::RealMatrix> ReadoutModel::fullMatrix() const {
    const std::size_t n = qubits_.size();
    if (n > 10)
        return fail(err::InvalidParameter, "dense assignment matrices are limited to 10 qubits");
