    ...
    for (std::size_t c = 0; c < n; ++c) {
        ...
        for (std::size_t r = c + 1; r < n; ++r)
            if (std::abs(m(r, c)) > std::abs(m(pivot, c)))
                pivot = r;
        if (std::abs(m(pivot, c)) < 1e-14)
            return false;
        if (pivot != c)
            for (std::size_t j = 0; j < n; ++j) {
                std::swap(m(c, j), m(pivot, j));
                std::swap(inv(c, j), inv(pivot, j));
            }
...
void projectSimplex(std::vector<double>& v) {
    ...
    if (rho == 0)
        theta = (std::accumulate(v.begin(), v.end(), 0.0) - 1.0) / static_cast<double>(n);
    for (auto& x : v)
...
Result<std::vector<double>> probabilitiesFromCounts(const qsim::Counts& counts, std::size_t nBits) {
    if (nBits > 24)
        return fail(err::InvalidParameter,
                    std::format("count histograms are limited to 24 bits, {} requested", nBits));
    std::vector<double> p(std::size_t{1} << nBits, 0.0);
    std::uint64_t total = 0;
    for (const auto& [key, n] : counts) {
        if (key.size() != nBits)
            return fail(err::BadReadout, std::format("count key '{}' has {} bits, {} expected", key,
                                                     key.size(), nBits));
        std::size_t index = 0;
        for (std::size_t k = 0; k < nBits; ++k) { // MSB-first key, little-endian index
            const char ch = key[nBits - 1 - k];
            if (ch != '0' && ch != '1')
                return fail(err::BadReadout, std::format("count key '{}' is not binary", key));
            if (ch == '1')
    ...
    if (total == 0)
        return fail(err::InvalidParameter, "count histogram is empty");
    for (auto& x : p)
...
Result<Mitigation> ReadoutModel::mitigate(std::span<const double> measured) const {
    ...
    // M^{-T} q: invert each factor's transpose and apply it along that factor (T10 §7.2 linear
    // inversion).
    ...
    if (invertible) {
        ...
        for (std::size_t n = 0; n < factors_.size(); ++n)
            applyAlong(out.unconstrained, factors_[n], inverseT[n]);
    }

    auto misfit = [&](const std::vector<double>& p) { // Mᵀp − q
        std::vector<double> r = p;
        for (std::size_t n = 0; n < factors_.size(); ++n)
            applyAlong(r, factors_[n], forwardT[n]);
    ...
    if (invertible && std::all_of(out.unconstrained.begin(), out.unconstrained.end(),
                                  [](double x) { return x >= -1e-12; })) {
        out.probabilities = out.unconstrained;
        for (auto& x : out.probabilities)
            x = std::max(0.0, x);
    ...
    for (const auto& f : factors_) {
        ...
        for (std::size_t i = 0; i < f.matrix.rows; ++i) {
            ...
            for (std::size_t j = 0; j < f.matrix.cols; ++j) {
                row += std::abs(f.matrix(i, j));
                col += std::abs(f.matrix(j, i));
            }
    ...
    for (std::size_t it = 0; it < 5000 && !out.converged; ++it) {
        ...
        for (std::size_t n = 0; n < factors_.size(); ++n)
            applyAlong(g, factors_[n], forward[n]);
        std::vector<double> next(y.size());
        for (std::size_t i = 0; i < y.size(); ++i)
            next[i] = y[i] - 2.0 * g[i] / lipschitz;
        projectSimplex(next);
        const double tNext = 0.5 * (1.0 + std::sqrt(1.0 + 4.0 * t * t));
        for (std::size_t i = 0; i < y.size(); ++i)
            y[i] = next[i] + (t - 1.0) / tNext * (next[i] - x[i]);
