...
SparseMatrix SparseMatrix::fromDense(ConstMatrixView d, double dropTol) {
    ...
    for (std::size_t i = 0; i < d.rows; ++i) {
        for (std::size_t j = 0; j < d.cols; ++j)
            if (std::abs(d(i, j)) > dropTol) {
                m.colIdx.push_back(j);
                m.values.push_back(d(i, j));
            }
...
SparseMatrix SparseMatrix::identity(std::size_t n) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        m.colIdx.push_back(i);
        m.values.push_back(Complex(1, 0));
        m.rowPtr[i + 1] = i + 1;
    }
    return m;
}
...
Vector SparseMatrix::matvec(std::span<const Complex> x) const {
    Vector y(rows);
    matvecInto(x, y);
    return y;
}
SparseMatrix& SparseMatrix::scale(Complex s) {
    for (auto& v : values)
        v *= s;
    return *this;
}
SparseMatrix SparseMatrix::adjoint() const {
    ...
    for (std::size_t i = 0; i < rows; ++i)
        for (std::size_t k = rowPtr[i]; k < rowPtr[i + 1]; ++k)
            t.push_back({colIdx[k], i, std::conj(values[k])});
...
SparseMatrix add(const SparseMatrix& a, const SparseMatrix& b, Complex alpha, Complex beta) {
    std::vector<Triplet> t;
    t.reserve(a.nnz() + b.nnz());
    for (std::size_t i = 0; i < a.rows; ++i)
        for (std::size_t k = a.rowPtr[i]; k < a.rowPtr[i + 1]; ++k)
            t.push_back({i, a.colIdx[k], alpha * a.values[k]});
    for (std::size_t i = 0; i < b.rows; ++i)
        for (std::size_t k = b.rowPtr[i]; k < b.rowPtr[i + 1]; ++k)
            t.push_back({i, b.colIdx[k], beta * b.values[k]});
        ...
        for (std::size_t ka = a.rowPtr[i]; ka < a.rowPtr[i + 1]; ++ka)
            for (std::size_t p = 0; p < b.rows; ++p)
                for (std::size_t kb = b.rowPtr[p]; kb < b.rowPtr[p + 1]; ++kb)
                    t.push_back({i * b.rows + p, a.colIdx[ka] * b.cols + b.colIdx[kb],
                                 a.values[ka] * b.values[kb]});
...
SparseMatrix matmul(const SparseMatrix& a, const SparseMatrix& b) {
    ...
    for (std::size_t i = 0; i < a.rows; ++i) {
        ...
        for (std::size_t ka = a.rowPtr[i]; ka < a.rowPtr[i + 1]; ++ka) {
            ...
            for (std::size_t kb = b.rowPtr[j]; kb < b.rowPtr[j + 1]; ++kb)
                row[b.colIdx[kb]] += a.values[ka] * b.values[kb];
        }
        for (auto& [c, v] : row)
            t.push_back({i, c, v});
    }
