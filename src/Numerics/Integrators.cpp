...
AdaptiveResult dopri5(const OdeRhs& f, Vector& y, double t0, double t1, double rtol, double atol,
...
void magnus2Step(
    ...
    y = expmTimesVector(
        dim,
        [&](std::span<const Complex> x, std::span<Complex> out) {
            applyL(tm, x, out);
            for (auto& v : out)
                v *= h;
        },
