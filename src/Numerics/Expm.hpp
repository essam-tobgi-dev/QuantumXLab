...
// Hermitian H: V diag(e^{-i t λ}) V†  — i.e. exp(-i H t). For exp(+H t) pass t as complex via expmHermitianGeneral.
...
Vector expmTimesVector(std::size_t dim, const std::function<void(std::span<const Complex>, std::span<Complex>)>& applyA,
