...
struct SparseMatrix {
    ...
    static SparseMatrix fromTriplets(std::size_t r, std::size_t c,
                                     std::vector<Triplet> t); // sums duplicates
...
SparseMatrix add(const SparseMatrix& a, const SparseMatrix& b, Complex alpha = 1.0,
                 Complex beta = 1.0); // αA + βB
SparseMatrix kron(const SparseMatrix& a,
                  const SparseMatrix& b); // left factor more significant (same rule as dense)
