...
bool isHermitian(ConstMatrixView H, double tolAbs) {
    ...
    for (std::size_t i = 0; i < H.rows; ++i)
        for (std::size_t j = i; j < H.cols; ++j)
            if (std::abs(H(i, j) - std::conj(H(j, i))) > tolAbs)
                return false;
    return true;
}
bool isPositiveSemidefinite(ConstMatrixView rho, double tolAbs) {
    if (!isHermitian(rho, 1e-9))
        return false;
    auto e = eigh(rho);
    if (!e)
        return false;
...
bool isTracePreserving(std::span<const Matrix> kraus, double tolAbs) {
    ...
    for (std::size_t i = 0; i < d; ++i)
        for (std::size_t j = 0; j < d; ++j) {
            Complex expect = (i == j) ? Complex(1, 0) : Complex(0, 0);
            if (std::abs(sum(i, j) - expect) > tolAbs)
                return false;
        }
    return true;
}
bool isDensityMatrix(ConstMatrixView rho, double tolAbs) {
    return isHermitian(rho, 1e-9) && std::abs(trace(rho) - Complex(1, 0)) <= tolAbs &&
           isPositiveSemidefinite(rho, 1e-8);
}
...
double purity(ConstMatrixView rho) {
    return std::real(trace(matmul(rho, rho)));
}
...
double globalPhaseBetween(ConstMatrixView U, ConstMatrixView V) {
    ...
    for (std::size_t j = 0; j < U.cols; ++j)
        for (std::size_t i = 0; i < U.rows; ++i)
            if (std::abs(V(i, j)) > 1e-9 && std::abs(U(i, j)) > 1e-9)
                return std::arg(U(i, j) / V(i, j));
    return 0.0;
}
bool equalUpToGlobalPhase(ConstMatrixView U, ConstMatrixView V, double tolAbs) {
    ...
    for (std::size_t i = 0; i < U.rows; ++i)
        for (std::size_t j = 0; j < U.cols; ++j)
            if (std::abs(U(i, j) - ph * V(i, j)) > tolAbs)
                return false;
    return true;
}
