...
void matmulInto(ConstMatrixView a, ConstMatrixView b, MatrixView c, Complex alpha = 1.0,
                Complex beta = 0.0);
...
inline Matrix operator*(Complex s, const Matrix& a) {
    return scale(a, s);
}
inline Matrix operator*(const Matrix& a, Complex s) {
    return scale(a, s);
}
inline Matrix operator*(double s, const Matrix& a) {
    return scale(a, Complex(s, 0));
}
