...
EigResult jacobiEigh(ConstMatrixView Hin) {
    ...
    for (int sweep = 0; sweep < 100; ++sweep) {
        ...
        for (std::size_t p = 0; p < n; ++p)
            for (std::size_t q = p + 1; q < n; ++q) {
                ...
                // G columns: G(:,p) = c e_p - s conj(ph)? Derive: D = diag(1, conj(ph)) on (p,q); P
                // = [[c, s],[-s, c]]. G = D P: G_pp = c, G_pq = s, G_qp = -s conj(ph), G_qq = c
                // conj(ph).
    ...
    for (std::size_t i = 0; i < n; ++i)
        r.values[i] = H(i, i).real();
    std::sort(order.begin(), order.end(),
              [&](std::size_t x, std::size_t y) { return r.values[x] < r.values[y]; });
    RealVector sv(n);
    Matrix SV(n, n);
    for (std::size_t k = 0; k < n; ++k) {
        sv[k] = r.values[order[k]];
        for (std::size_t i = 0; i < n; ++i)
            SV(i, k) = V(i, order[k]);
    }
    r.values = std::move(sv);
    r.vectors = std::move(SV);
    return r;
}
} // namespace
...
Result<EigResult> eigh(ConstMatrixView H, bool forceOwn) {
    if (!H.square())
        return fail(ErrorCode::InvalidArgument, "eigh: matrix not square");
...
Result<SvdResult> svd(ConstMatrixView Ain) {
        ...
        for (std::size_t i = 0; i < n; ++i)
            for (std::size_t j = i + 1; j < n; ++j) {
                ...
                for (std::size_t r = 0; r < m; ++r) {
                    alpha += std::norm(A(r, i));
                    beta += std::norm(A(r, j));
                    gamma += std::conj(A(r, i)) * A(r, j);
                }
                double g = std::abs(gamma);
                if (g < 1e-300 || g <= 1e-15 * std::sqrt(alpha * beta))
                    continue;
                rot += g * g;
                Complex ph = gamma / g;
                double zeta = (beta - alpha) / (2.0 * g);
                double t =
                    (zeta >= 0 ? 1.0 : -1.0) / (std::abs(zeta) + std::sqrt(1.0 + zeta * zeta));
                double c = 1.0 / std::sqrt(1.0 + t * t), s = c * t;
                // columns i,j of A and V: [ai aj] ← [ai aj] G with G = [[c, s],[-s conj(ph), c
                // conj(ph)]]
                for (std::size_t r = 0; r < m; ++r) {
    ...
    for (std::size_t j = 0; j < n; ++j) {
        ...
        sing[j] = std::sqrt(nn);
        for (std::size_t r = 0; r < m; ++r)
            U(r, j) = sing[j] > 1e-300 ? A(r, j) / sing[j] : Complex{};
    }
    std::vector<std::size_t> order(n);
    std::iota(order.begin(), order.end(), 0);
    std::sort(order.begin(), order.end(),
              [&](std::size_t x, std::size_t y) { return sing[x] > sing[y]; });
    SvdResult r;
    r.singular.resize(n);
    r.U = Matrix(m, n);
    r.V = Matrix(n, n);
...
Result<SchmidtResult> schmidt(std::span<const Complex> psi, std::size_t dimLow,
                              std::size_t dimHigh) {
    if (psi.size() != dimLow * dimHigh)
        return fail(ErrorCode::InvalidArgument, "schmidt: dimension mismatch");
    // C(low, high) = psi[high*dimLow + low]
    Matrix C(dimLow, dimHigh);
    for (std::size_t h = 0; h < dimHigh; ++h)
        for (std::size_t l = 0; l < dimLow; ++l)
            C(l, h) = psi[h * dimLow + l];
    auto s = svd(C);
    if (!s)
        return std::unexpected(s.error());
    SchmidtResult r;
    r.coefficients = s->singular;
    r.low = s->U;
    r.high = conj(s->V); // C = U S V† → ψ = Σ s_k u_k ⊗ conj(v_k)
    return r;
}
