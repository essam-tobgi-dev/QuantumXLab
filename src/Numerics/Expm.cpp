...
constexpr double kB[14] = {64764752532480000., 32382376266240000., 7771770303897600., 1187353796428800.,
...
Result<Matrix> expm(ConstMatrixView Ain) {
    if (!Ain.square()) return fail(ErrorCode::InvalidArgument, "expm: matrix must be square");
    std::size_t n = Ain.rows;
    if (n == 0) return Matrix();
    double nrm = norm1(Ain);
    int s = 0;
    if (nrm > kTheta13) s = static_cast<int>(std::ceil(std::log2(nrm / kTheta13)));
    Matrix A(Ain);
    if (s > 0) A *= Complex(std::ldexp(1.0, -s), 0.0);
    Matrix I = Matrix::identity(n);
    Matrix A2 = matmul(A, A), A4 = matmul(A2, A2), A6 = matmul(A4, A2);
    auto lin = [&](double c0, double c2, double c4, double c6) {
        Matrix r = scale(I, c0); r += scale(A2, c2); r += scale(A4, c4); r += scale(A6, c6); return r;
    };
    Matrix inner = lin(kB[7], kB[9], kB[11], kB[13]);
    Matrix U = matmul(A6, inner); U += lin(kB[1], kB[3], kB[5], 0.0);
    U = matmul(A, U);
    Matrix innerV = lin(kB[6], kB[8], kB[10], kB[12]);
    Matrix V = matmul(A6, innerV); V += lin(kB[0], kB[2], kB[4], 0.0);
...
Result<Matrix> expmHermitianGeneral(ConstMatrixView H, Complex sc) {
    ...
    std::size_t n = H.rows;
    Matrix D(n, n);
    for (std::size_t i = 0; i < n; ++i) D(i, i) = std::exp(sc * e->values[i]);
    return matmul(matmul(e->vectors, D), adjoint(e->vectors));
}
Result<Matrix> expmHermitian(ConstMatrixView H, double t) { return expmHermitianGeneral(H, Complex(0.0, -t)); }
...
Mat2 expPauli(double nx, double ny, double nz, double theta) {
    ...
    Complex mi(0, -1);
    Mat2 r;
    r(0, 0) = c + mi * s * nz;          r(0, 1) = mi * s * Complex(nx, -ny);
    r(1, 0) = mi * s * Complex(nx, ny); r(1, 1) = c - mi * s * nz;
    return r;
}
Mat4 expPauliPair(const Mat2& P, const Mat2& Q, double theta) {
    ...
    for (auto& v : r.a) v *= c;
    // P⊗Q with P on the high (second) qubit and Q on the low (first) — callers use pauli pairs symmetrically.
...
Vector expmTimesVector(std::size_t dim,
    ...
    if (beta == 0.0 || dim == 0) return result;
    std::size_t m = std::min(maxKrylov, dim);
    std::vector<Vector> V; V.reserve(m + 1);
    V.emplace_back(v.begin(), v.end());
    for (auto& x : V[0]) x /= beta;
    Matrix Hm(m + 1, m + 1);
    Vector w(dim);
    std::size_t k = 0;
    ...
    for (; k < m; ++k) {
        ...
        for (std::size_t j = 0; j <= k; ++j) {            // modified Gram–Schmidt (twice for stability)
            Complex h = dot(V[j], w);
            for (std::size_t i = 0; i < dim; ++i) w[i] -= h * V[j][i];
            Hm(j, k) += h;
        }
        ...
        Hm(k + 1, k) = hn;
        if (hn < tolAbs) { breakdown = true; ++k; break; }
        Vector next(w); for (auto& x : next) x /= hn;
        V.push_back(std::move(next));
    }
    std::size_t kk = breakdown ? k : m;
    Matrix Hk(kk, kk);
    for (std::size_t i = 0; i < kk; ++i) for (std::size_t j = 0; j < kk; ++j) Hk(i, j) = Hm(i, j);
    auto E = expm(Hk);
    if (!E) return result;
    for (std::size_t j = 0; j < kk; ++j) {
        Complex c = beta * (*E)(j, 0);
        for (std::size_t i = 0; i < dim; ++i) result[i] += c * V[j][i];
    }
    return result;
}
Vector expmTimesVector(ConstMatrixView A, std::span<const Complex> v, double tolAbs) {
    return expmTimesVector(A.rows, [&](std::span<const Complex> x, std::span<Complex> y) { matvecInto(A, x, y); }, v, tolAbs);
}
} // namespace qlab::num
