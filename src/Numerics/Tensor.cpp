...
std::size_t ipow(std::size_t b, std::size_t e) {
    std::size_t r = 1;
    while (e--)
        r *= b;
    return r;
...
Matrix partialTrace(ConstMatrixView rho, std::span<const std::size_t> dims,
    ...
    auto fullIndex = [&](std::size_t ik, std::size_t it) {
        std::size_t idx = 0;
        for (auto k : keep) {
            idx += (ik % dims[k]) * st.stride[k];
            ik /= dims[k];
        }
        for (auto k : traced) {
            idx += (it % dims[k]) * st.stride[k];
            it /= dims[k];
        }
        return idx;
    };
    for (std::size_t i = 0; i < dimKeep; ++i)
        for (std::size_t j = 0; j < dimKeep; ++j) {
            ...
            for (std::size_t t = 0; t < dimTr; ++t)
                s += rho(fullIndex(i, t), fullIndex(j, t));
            out(i, j) = s;
        }
    return out;
}
...
ConstMatrixView reshape(std::span<const Complex> v, std::size_t rows, std::size_t cols) {
    ...
    return ConstMatrixView(v.data(), rows, cols);
}
MatrixView reshape(std::span<Complex> v, std::size_t rows, std::size_t cols) {
    assert(v.size() == rows * cols);
    return MatrixView(v.data(), rows, cols);
}

Matrix reducedState(std::span<const Complex> psi, std::size_t n, std::span<const std::size_t> keep,
                    std::size_t d) {
    std::vector<std::size_t> traced;
    ...
    for (std::size_t t = 0; t < dimTr; ++t)
        for (std::size_t i = 0; i < dimKeep; ++i) {
            ...
            if (a == 0.0)
                continue;
            for (std::size_t j = 0; j < dimKeep; ++j)
                rho(i, j) += a * std::conj(psi[fullIndex(j, t)]);
        }
    return rho;
}
} // namespace qlab::num
