...
struct EigResult {
    RealVector values;
    Matrix vectors;
}; // values ascending; vectors as COLUMNS
// Hermitian eigendecomposition. n ≤ 16: cyclic complex Jacobi. Larger: LAPACK (Accelerate) when
// available, else Jacobi.
Result<EigResult> eigh(ConstMatrixView H, bool forceOwn = false);
struct SvdResult {
    RealVector singular;
    Matrix U;
    Matrix V;
}; // A = U diag(s) V†, singular descending
Result<SvdResult> svd(ConstMatrixView A);
struct SchmidtResult {
    RealVector coefficients;
    Matrix low;
    Matrix high;
}; // |ψ⟩ = Σ λ_k |low_k⟩ ⊗ |high_k⟩
