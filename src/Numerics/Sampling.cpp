...
AliasTable::AliasTable(std::span<const double> probs) {
    ...
    if (n == 0) return;
    double total = 0; for (double p : probs) total += std::max(0.0, p);
    if (total <= 0) { for (std::size_t i = 0; i < n; ++i) { prob_[i] = 1.0; alias_[i] = i; } return; }
    std::vector<double> scaled(n);
    for (std::size_t i = 0; i < n; ++i) scaled[i] = std::max(0.0, probs[i]) / total * static_cast<double>(n);
    ...
    for (std::size_t i : large) { prob_[i] = 1.0; alias_[i] = i; }
    for (std::size_t i : small) { prob_[i] = 1.0; alias_[i] = i; }
}
...
CumulativeSampler::CumulativeSampler(std::span<const double> probs) {
    ...
    double acc = 0;
    for (std::size_t i = 0; i < probs.size(); ++i) { acc += std::max(0.0, probs[i]); cdf_[i] = acc; }
    if (acc > 0) for (auto& c : cdf_) c /= acc;
}
...
std::vector<std::size_t> sampleCounts(std::span<const double> probs, std::size_t shots, core::Random& rng) {
    std::vector<std::size_t> counts(probs.size(), 0);
    if (probs.empty()) return counts;
    if (shots > 64) { AliasTable t(probs); for (std::size_t s = 0; s < shots; ++s) ++counts[t.sample(rng)]; }
    else { CumulativeSampler c(probs); for (std::size_t s = 0; s < shots; ++s) ++counts[c.sample(rng)]; }
    return counts;
}
double chiSquare(std::span<const std::size_t> counts, std::span<const double> probs) {
    ...
    for (std::size_t i = 0; i < counts.size() && i < probs.size(); ++i) {
        ...
        if (e > 0) { double d = static_cast<double>(counts[i]) - e; chi += d * d / e; }
    }
    return chi;
}
} // namespace qlab::num
