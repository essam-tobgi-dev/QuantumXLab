...
bool isPositiveSemidefinite(ConstMatrixView rho,
                            double tolAbs = tol::kPsdTol); // min eigenvalue ≥ −tol
...
double fidelity(ConstMatrixView rho,
                ConstMatrixView sigma); // (Tr √(√ρ σ √ρ))² — squared convention
...
// U ≈ e^{iφ} V. φ = arg Tr(U†V); if |Tr(U†V)| < 1e-6, falls back to the first non-zero column
// ratio.
