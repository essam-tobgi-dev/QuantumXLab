...
class Emitter {
  public:
    ...
    Result<std::string> run() {
        ...
        const bool calibrated = meta.contains("calibrations") && meta["calibrations"].is_array() &&
                                !meta["calibrations"].empty();
        if (calibrated)
            out += "defcalgrammar \"openpulse\";\n";
        if (meta.contains("device") && meta["device"].is_string())
            out += std::format("pragma qlab.device {}\n", meta["device"].get<std::string>());
        if (meta.contains("shots") && meta["shots"].is_number_unsigned())
            out += std::format("pragma qlab.shots {}\n", meta["shots"].get<std::uint64_t>());
        if (meta.contains("seed") && meta["seed"].is_number_unsigned())
            out += std::format("pragma qlab.seed {}\n", meta["seed"].get<std::uint64_t>());
        if (meta.contains("optimize") && meta["optimize"].is_number_integer())
            out += std::format("pragma qlab.optimize {}\n", meta["optimize"].get<int>());
        if (c_.isPhysical())
            out += "pragma qlab.layout physical\n";
        if (meta.contains("pulse_level") && meta["pulse_level"].is_boolean())
            out += std::format("pragma qlab.pulse_level {}\n",
                               meta["pulse_level"].get<bool>() ? "on" : "off");
        out += declarations_;
        if (calibrated)
            for (const auto& cal : meta["calibrations"])
                if (cal.is_string())
                    out += cal.get<std::string>() + "\n";
  ...
  private:
        ...
        for (std::uint32_t q = 0; q < n; ++q)
            if (wire_[q].empty())
                return fail(err::Unsupported,
                            std::format("toQasm: wire {} belongs to no qubit register", q));
        return {};
    }
    ...
    Status nameBits() {
        ...
        bit_.assign(n, {});
        regs_.clear();
        for (const auto& r : c_.bitRegisters())
            regs_.push_back(&r);
        if (regs_.empty() && n > 0) {
        ...
        std::sort(regs_.begin(), regs_.end(),
                  [](const BitRegister* a, const BitRegister* b) { return a->first < b->first; });
        for (const BitRegister* r : regs_) {
            // `bit c;` is one bit, `int k;` is int[32] (spec 13 §3 default widths), `bool` has
            // none.
            std::string type;
            switch (r->kind) {
            case RegKind::Bit:
                type = r->scalar ? "bit" : std::format("bit[{}]", r->size);
                break;
            case RegKind::Bool:
                type = "bool";
                break;
        ...
        for (std::uint32_t b = 0; b < n; ++b)
            if (bit_[b].empty())
                return fail(err::Unsupported,
                            std::format("toQasm: bit {} belongs to no register", b));
        return {};
    }

    std::string freeName(std::string base) const {
        auto used = [&](const std::string& s) {
            return std::any_of(c_.bitRegisters().begin(), c_.bitRegisters().end(),
                               [&](const auto& r) { return r.name == s; }) ||
                   std::any_of(c_.qubitRegisters().begin(), c_.qubitRegisters().end(),
                               [&](const auto& r) { return r.name == s; });
        };
        while (used(base))
            base += "_";
    ...
    Result<std::string> wire(Wire w) const {
        if (w.index >= wire_.size())
            return fail(err::BadWire,
                        std::format("toQasm: wire {} is outside the circuit", w.index));
        return wire_[w.index];
    }
    Result<std::string> bit(ClassicalBit b) const {
        if (b.index >= bit_.size())
            return fail(err::BadBit, std::format("toQasm: bit {} is outside the circuit", b.index));
        return bit_[b.index];
    }
    ...
    Result<std::string> span(CregRef r) const {
        for (const BitRegister* reg : regs_) {
            if (r.first < reg->first || r.first >= reg->first + reg->size)
                continue;
            if (r.first + r.size > reg->first + reg->size)
                break;
            if (r.first == reg->first && r.size == reg->size)
                return reg->name;
            if (reg->scalar)
                break;
        ...
        return fail(err::Unsupported,
                    std::format("toQasm: bits {}..{} do not lie inside one register", r.first,
                                r.first + r.size - 1));
    }
    Result<std::string> expr(const ClassicalExpr& e) const {
        ...
        case ClassOp::RegRef:
            return span(e.reg);
        case ClassOp::LogicNot:
        case ClassOp::BitNot:
        case ClassOp::Neg: {
            if (e.args.size() != 1)
                return fail(err::BadNode, "toQasm: malformed unary classical expression");
            QXL_TRY_ASSIGN(std::string a, expr(e.args[0]));
            return std::format("{}({})", classOpName(e.op), a);
        }
        default: {
            if (e.args.size() != 2)
                return fail(err::BadNode, "toQasm: malformed binary classical expression");
            QXL_TRY_ASSIGN(std::string a, expr(e.args[0]));
    ...
    Result<std::string> topExpr(const ClassicalExpr& e) const {
        ...
        const bool binary = e.args.size() == 2 && e.op != ClassOp::Const &&
                            e.op != ClassOp::BitRef && e.op != ClassOp::RegRef;
    ...
    Result<std::string> gate(const Gate& g) const {
        if (g.custom)
            return fail(err::Unsupported,
                        "toQasm: a gate defined by an explicit matrix has no OpenQASM 3 spelling");
        ...
        if (!g.opaque && !lang::StdGates::find(name))
            return fail(err::Unsupported,
                        std::format("toQasm: gate '{}' is not in stdgates.inc", g.name));
        std::string s;
        for (std::size_t j = 0; j < g.controls.size(); ++j)
            s += g.isNegControl(j) ? "negctrl @ " : "ctrl @ ";
        if (inverted)
            s += "inv @ ";
        s += name;
        if (!g.params.empty()) {
            ...
            for (std::size_t k = 0; k < g.params.size(); ++k) {
                if (!std::isfinite(g.params[k]))
                    return fail(
                        err::Unsupported,
                        std::format("toQasm: gate '{}' has a non-finite parameter", g.name));
    ...
    Status body(const Circuit& c, int depth, std::string& out) const {
        ...
        for (NodeId id : c.topologicalOrder()) {
            ...
            if (const auto* g = std::get_if<Gate>(&n)) {
                ...
                out += std::format("{}delay[{}]{};\n", pad, detail::qasmDuration(d->duration),
                                   ws.empty() ? "" : " " + ws);
            } else if (const auto* co = std::get_if<ClassicalOp>(&n)) {
                std::string dst;
                if (co->dst.element) {
                    QXL_TRY_ASSIGN(dst, bit(co->dst.reg.bit(*co->dst.element)));
                } else {
                    QXL_TRY_ASSIGN(dst, span(co->dst.reg));
                }
                ...
                out += pad + (bx->duration
                                  ? std::format("box[{}] {{\n", detail::qasmDuration(*bx->duration))
                                  : std::string("box {\n"));
