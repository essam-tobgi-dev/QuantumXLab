...
std::string formatReal(double v) {
    if (v == 0.0)
        return "0"; // also −0: the lexer has no signed zero
    return std::format("{}", v);
}
...
struct Printer {
    ...
    std::string expr(const lang::Expr& e) const {
        ...
        if (const auto* n = e.as<ImagLit>())
            return formatFloatLiteral(n->value) + "im";
        if (const auto* n = e.as<BoolLit>())
            return n->value ? "true" : "false";
        if (const auto* n = e.as<BitStringLit>())
            return "\"" + n->bits + "\"";
        if (const auto* n = e.as<DurationLit>())
            return formatReal(n->value) + durationUnitName(n->unit);
        if (const auto* n = e.as<ConstantRef>())
            return n->name;
        if (const auto* n = e.as<Ident>()) {
            if (!locals.contains(n->name) && resolve)
                if (auto lit = resolve(n->name))
                    return *lit;
            return n->name;
        }
        if (const auto* n = e.as<PhysicalQubitRef>())
            return std::format("${}", n->index);
        ...
        if (const auto* n = e.as<BinaryExpr>())
            return std::format("({} {} {})", expr(n->lhs), binaryOpName(n->op), expr(n->rhs));
        if (const auto* n = e.as<CallExpr>())
            return std::format("{}({})", n->callee, list(n->args));
        if (const auto* n = e.as<CastExpr>())
            return std::format("{}{}({})", baseTypeName(n->type.base),
                               n->type.width ? "[" + expr(*n->type.width) + "]" : "",
                               expr(n->operand));
        if (const auto* n = e.as<IndexExpr>())
            return std::format("{}[{}]", expr(n->base), expr(n->index));
        ...
        if (const auto* n = e.as<SliceExpr>())
            return std::format("{}[{}]", expr(n->base), expr(n->range));
        if (const auto* n = e.as<SetExpr>())
            return "{" + list(n->items) + "}";
        if (const auto* n = e.as<ConcatExpr>())
            return std::format("({} ++ {})", expr(n->lhs), expr(n->rhs));
    ...
    std::string waveform(const lang::PulseWaveform& w) const {
        ...
        if (w.kind == "ref") {
            const auto* id =
                w.args.empty() || !w.args.front() ? nullptr : w.args.front()->as<lang::Ident>();
            ...
            out += std::visit(
                Overloaded{
                    ...
                    [&](const PulseFrameDecl& f) {
                        return std::format("frame {} = newframe({}, {}, {});", f.name, f.port,
                                           expr(f.frequency), expr(f.phase));
                    },
                    [&](const PulseWaveformDecl& w) {
                        return std::format("waveform {} = {};", w.name, waveform(w.waveform));
                    },
                    [&](const PulsePlay& p) {
                        const auto* named = std::get_if<std::string>(&p.waveform);
                        return std::format("play({}, {});", p.frame,
                                           named ? *named
                                                 : waveform(std::get<PulseWaveform>(p.waveform)));
                    },
                    [&](const PulseFrameOp& o) {
                        return std::format("{}({}, {});", o.op, o.frame, expr(o.value));
                    },
                    [&](const PulseDelay& d) {
                        return std::format("delay[{}]{};", expr(d.duration), names(d.frames));
                    },
                    [&](const PulseCapture& c) {
                        std::string call =
                            std::format("capture_v2({}, {});", c.frame, expr(c.duration));
                        if (!c.target)
                            return call;
                        return *c.target == "return" ? "return " + call : *c.target + " = " + call;
                    },
                    [&](const PulseBarrier& b) {
                        return std::format("barrier{};", names(b.frames));
                    },
                },
                ps.node);
            out += "\n";
...
std::string qasmDuration(const Duration& d) {
    ...
    if (d.ps.get() != 0 || d.dt == 0)
        out = nanoseconds(d.ps.get());
    if (d.dt != 0) {
        if (out.empty())
            out = std::format("{}{}dt", d.dt < 0 ? "-" : "", magnitude(d.dt));
        else
            out += std::format(" {} {}dt", d.dt < 0 ? "-" : "+", magnitude(d.dt));
    }
    return out;
...
std::string calibrationText(const lang::Stmt& s, const NameResolver& resolve) {
    Printer pr{resolve, {}};
    if (const auto* c = s.as<lang::CalStmt>())
        return "cal {\n" + pr.body(c->body) + "}";
