...
const char* classOpName(ClassOp op) {
    ...
    case ClassOp::Ge:
        return ">=";
    case ClassOp::LogicAnd:
        return "&&";
    case ClassOp::LogicOr:
        return "||";
...
std::string ClassicalExpr::text() const {
    ...
    case ClassOp::BitRef:
        return std::format("c[{}]", bit.index);
    case ClassOp::RegRef:
        return std::format("{}c[{}:{}]", isSigned ? "int " : "", reg.first,
                           reg.first + reg.size - 1);
    case ClassOp::LogicNot:
    case ClassOp::BitNot:
    case ClassOp::Neg:
        return std::format("({}{})", classOpName(op),
                           args.empty() ? "<malformed>" : args[0].text());
    default:
        if (args.size() != 2)
            return "<malformed>";
...
std::int64_t ClassicalExpr::eval(std::span<const std::uint8_t> bits) const {
    auto get = [&](std::uint32_t i) -> std::int64_t {
        return i < bits.size() && bits[i] != 0 ? 1 : 0;
    };
    switch (op) {
    case ClassOp::Const:
        return value;
    case ClassOp::BitRef:
        return get(bit.index);
    case ClassOp::RegRef: {
        const std::uint32_t n = std::min<std::uint32_t>(reg.size, 63);
        std::int64_t v = 0;
        for (std::uint32_t i = 0; i < n; ++i)
            v |= get(reg.first + i) << i; // little-endian
        if (isSigned && n > 0 && get(reg.first + n - 1) != 0)
            v -= (std::int64_t{1} << n); // two's complement
        return v;
    }
    case ClassOp::LogicNot:
        return !args.empty() && args[0].eval(bits) == 0 ? 1 : 0;
    case ClassOp::BitNot:
        return args.empty() ? 0 : ~args[0].eval(bits);
    case ClassOp::Neg:
        return args.empty() ? 0 : -args[0].eval(bits);
    default:
        break;
    ...
    if (op == ClassOp::LogicAnd)
        return (a != 0 && args[1].eval(bits) != 0) ? 1 : 0;
    if (op == ClassOp::LogicOr)
        return (a != 0 || args[1].eval(bits) != 0) ? 1 : 0;
    ...
    case ClassOp::Mod:
        return b == 0 ? 0 : a % b;
    case ClassOp::Shl:
        return shiftOk ? a << b : 0;
    case ClassOp::Shr:
        return shiftOk ? a >> b : (a < 0 ? -1 : 0);
    default:
        return 0;
    }
}
...
bool ClassicalExpr::operator==(const ClassicalExpr& o) const {
    return op == o.op && value == o.value && bit == o.bit && reg == o.reg &&
           isSigned == o.isSigned && args == o.args;
}
...
void applyClassicalOp(const ClassicalOp& op, std::span<std::uint8_t> bits) {
    ...
    auto put = [&](std::uint32_t i, std::uint64_t b) {
        if (i < bits.size())
            bits[i] = static_cast<std::uint8_t>(b & 1u);
    };
    if (op.dst.element) {
        put(op.dst.reg.first + *op.dst.element, v);
        return;
    }
    for (std::uint32_t i = 0; i < op.dst.reg.size; ++i)
        put(op.dst.reg.first + i, i < 64 ? (v >> i) : 0);
}
