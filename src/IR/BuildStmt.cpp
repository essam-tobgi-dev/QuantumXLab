...
RegKind kindOf(BaseType b) {
    switch (b) {
    case BaseType::Bool: return RegKind::Bool;
    case BaseType::Int: return RegKind::Int;
    case BaseType::Uint: return RegKind::Uint;
    default: return RegKind::Bit;
    }
}
std::optional<lang::BinaryOp> binaryOf(lang::AssignOp op) {
    ...
    switch (op) {
    case A::Add: return B::Add;
    case A::Sub: return B::Sub;
    case A::Mul: return B::Mul;
    case A::Div: return B::Div;
    case A::Mod: return B::Mod;
    case A::Pow: return B::Pow;
    case A::BitAnd: return B::BitAnd;
    case A::BitOr: return B::BitOr;
    case A::BitXor: return B::BitXor;
    case A::Shl: return B::Shl;
    case A::Shr: return B::Shr;
    case A::Set: return std::nullopt;
    }
    return std::nullopt;
}
// OpenQASM 3 casts on declaration: int/uint truncate, bool normalises, float/angle promote.
Value coerce(Value v, BaseType t) {
    if (!v.constant() || v.kind == Value::Kind::Dur) return v;
    switch (t) {
    case BaseType::Int: case BaseType::Uint: return Value::ofInt(v.kind == Value::Kind::Float ? static_cast<std::int64_t>(v.f) : v.i);
    case BaseType::Bool: return Value::ofInt(v.asDouble() != 0.0 ? 1 : 0);
...
Result<Value> Builder::inputValue(const lang::ClassicalDecl& d) {
    if (auto it = inputs_.find(d.name); it != inputs_.end()) {
        ...
        if (d.type.base == BaseType::Int || d.type.base == BaseType::Uint || d.type.base == BaseType::Bit)
            return Value::ofInt(std::llround(it->second));
        return coerce(Value::ofFloat(it->second), d.type.base);
    }
    if (d.init) { QXL_TRY_ASSIGN(Value v, eval(**d.init)); return coerce(std::move(v), d.type.base); }
...
Status Builder::lowerDecl(const lang::ClassicalDecl& d) {
    ...
    if (d.type.base != BaseType::Bit && !runtime_.contains(d.name)) {
        Binding b;
        if (d.init) { QXL_TRY_ASSIGN(Value v, eval(**d.init)); b.value = coerce(std::move(v), d.type.base); }
        bind(d.name, std::move(b));
        return {};
    }
    // Per-shot storage (spec 14 §2): bit registers always, other scalars when a measurement reaches them.
    std::uint32_t width = 1;
    switch (d.type.base) {
    case BaseType::Bit: case BaseType::Bool: case BaseType::Int: case BaseType::Uint:
        if (d.type.base == BaseType::Int || d.type.base == BaseType::Uint) width = 32;
        if (d.type.width && d.type.base != BaseType::Bool) {
            QXL_TRY_ASSIGN(Value w, eval(**d.type.width));
            if (!w.constant() || w.asInt() < 1) return fail(err::NotConstant, std::format("'{}' needs a constant positive width", d.name));
            width = static_cast<std::uint32_t>(w.asInt());
        }
        break;
    default:
        return fail(err::Unsupported, std::format("'{}' depends on a measurement but {} has no bit representation",
...
Status Builder::lowerAssign(const lang::AssignStmt& a) {
    if (const auto* m = a.value->as<lang::MeasureExpr>()) {
        if (a.op != lang::AssignOp::Set) return fail(err::Unsupported, "a measurement can only be assigned with '='");
    ...
    if (pl.binding->kind == Binding::Kind::Qubits) return fail(err::BadBit, "a qubit cannot be assigned");
    if (pl.binding->kind == Binding::Kind::Value) {
        if (conditionScope_ > 0 && pl.scope < conditionScope_)
            return fail(err::Unsupported, "a compile-time classical variable is assigned inside a run-time branch or loop; "
                                          "declare it as a bit register so the IR can carry it");
        if (!rhs.constant())
            return fail(err::Unsupported, "a compile-time classical variable is assigned a value that is only known per "
                                          "shot; declare it as a bit register so the IR can carry it");
        if (!op) { pl.binding->value = std::move(rhs); return {}; }
    ...
    if (op) {
        static constexpr std::pair<lang::BinaryOp, ClassOp> kOps[] = {
            {lang::BinaryOp::Add, ClassOp::Add}, {lang::BinaryOp::Sub, ClassOp::Sub}, {lang::BinaryOp::Mul, ClassOp::Mul},
            {lang::BinaryOp::Div, ClassOp::Div}, {lang::BinaryOp::Mod, ClassOp::Mod}, {lang::BinaryOp::BitAnd, ClassOp::BitAnd},
        ...
        auto it = std::find_if(std::begin(kOps), std::end(kOps), [&](const auto& e) { return e.first == *op; });
        if (it == std::end(kOps)) return fail(err::Unsupported, "'**=' is not available on per-shot classical values");
        ClassicalExpr cur = pl.element ? ClassicalExpr::bitRef(pl.reg.bit(*pl.element))
...
Status Builder::lowerMeasureInto(const std::vector<lang::ExprPtr>& qubits, const Expr* target) {
    ...
    if (!target) {
        for (Wire w : ws) emit(Measure{w, kNoBit, std::nullopt, {}});
        return {};
    }
    ...
    if (pl.element) bits.push_back(pl.reg.bit(*pl.element));
    else for (std::uint32_t i = 0; i < pl.reg.size; ++i) bits.push_back(pl.reg.bit(i));
    if (bits.size() != ws.size())
        return fail(err::BadArity, std::format("measure writes {} qubit(s) into {} bit(s)", ws.size(), bits.size()));
    for (std::size_t i = 0; i < ws.size(); ++i) emit(Measure{ws[i], bits[i], std::nullopt, {}});
    return {};
}
