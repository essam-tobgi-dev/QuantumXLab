...
std::optional<GateRewrite> inverseOf(std::string_view name, std::span<const double> params) {
    ...
    if (d->rotation) {
        ...
        for (auto& v : p)
            v = -v;
        return GateRewrite{std::string(d->inverseName.empty() ? name : d->inverseName),
                           std::move(p)};
    }
    if (!d->inverseName.empty())
        return GateRewrite{std::string(d->inverseName),
...
std::optional<GateRewrite> powerOf(std::string_view name, std::span<const double> params,
                                   double k) {
    const GateDef* d = find(name);
    if (!d || !d->rotation)
        return std::nullopt;
...
std::optional<GateRewrite> namedPower(std::string_view name, std::span<const double> params,
                                      double k) {
    const GateDef* d = find(name);
    if (!d || !params.empty())
        return std::nullopt;
    ...
    if (base != 0) {
        ...
        if (std::abs(r) < eps)
            return GateRewrite{"", {}};
        if (std::abs(std::abs(r) - kPi) < eps)
            return GateRewrite{"z", {}};
        if (std::abs(r - kPi / 2) < eps)
            return GateRewrite{"s", {}};
        if (std::abs(r + kPi / 2) < eps)
            return GateRewrite{"sdg", {}};
        if (std::abs(r - kPi / 4) < eps)
            return GateRewrite{"t", {}};
        if (std::abs(r + kPi / 4) < eps)
            return GateRewrite{"tdg", {}};
    ...
    if (d->selfInverse)
        return GateRewrite{odd() ? std::string(name) : std::string(), {}};
    return std::nullopt;
}

namespace {
struct ControlledEntry {
    std::string_view name, base;
    std::size_t controls;
};
// First entry per (base, controls) is the canonical name; `CX` is the legacy spelling of `cx`.
constexpr ControlledEntry kControlled[] = {
    {"cx", "x", 1},   {"CX", "x", 1},   {"ccx", "x", 2},      {"cy", "y", 1},
    {"cz", "z", 1},   {"ch", "h", 1},   {"cp", "p", 1},       {"crx", "rx", 1},
    {"cry", "ry", 1}, {"crz", "rz", 1}, {"cswap", "swap", 1},
};
} // namespace

std::optional<ControlledRewrite> splitControlled(std::string_view name,
                                                 std::span<const double> params) {
    // cu(θ,φ,λ,γ) = ctrl @ (e^{iγ} U): a plain controlled U only when γ is exactly zero.
    if (name == "cu") {
        if (params.size() != 4 || params[3] != 0.0)
            return std::nullopt;
        return ControlledRewrite{GateRewrite{"U", {params[0], params[1], params[2]}}, 1};
    }
    for (const auto& e : kControlled)
        if (e.name == name)
            return ControlledRewrite{
                GateRewrite{std::string(e.base), std::vector<double>(params.begin(), params.end())},
                e.controls};
    return std::nullopt;
}
...
GateClass classify(std::string_view name, std::span<const double> params) {
    const GateDef* d = find(name);
    if (!d)
        return GateClass::Generic;
    if (d->rotation && d->nQubits == 1 && !params.empty()) {
        bool allZero = true;
        for (double v : params)
            allZero = allZero && std::abs(v) < 1e-15;
        if (allZero)
            return GateClass::Identity;
    }
    return d->cls;
}
