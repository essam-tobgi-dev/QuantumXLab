...
Result<Builder::Range> Builder::range(const lang::RangeExpr& r) {
    ...
    if (out.step == 0)
        return fail(err::BadArity, "range step must not be zero");
    // OpenQASM ranges include both bounds (spec 13 §3): [4:-1:0] is 4, 3, 2, 1, 0.
    if (out.step > 0)
        out.count =
            stop < out.start ? 0 : static_cast<std::uint64_t>((stop - out.start) / out.step) + 1;
    else
        out.count =
            stop > out.start ? 0 : static_cast<std::uint64_t>((out.start - stop) / -out.step) + 1;
    return out;
}
...
Result<std::vector<Wire>> Builder::qubitOperand(const Expr& e) {
    ...
    if (const auto* n = e.as<PhysicalQubitRef>())
        return std::vector<Wire>{Wire{n->index}};
    ...
    if (const auto* n = e.as<IndexExpr>()) {
        ...
        if (!ix.constant())
            return fail(err::NotConstant, "a qubit index must be known at build time");
        const std::int64_t k = ix.asInt();
        if (k < 0 || static_cast<std::size_t>(k) >= base.size())
            return fail(err::BadWire,
                        std::format("qubit index {} is out of range for an operand of {} qubit(s)",
                                    k, base.size()));
    ...
    if (const auto* n = e.as<SliceExpr>()) {
        ...
        if (!r)
            return fail(err::Unsupported, "a qubit slice must be a range");
...
Result<Builder::Place> Builder::place(const Expr& e) {
    ...
    if (const auto* n = e.as<Ident>()) {
        ...
        if (!f.binding)
            return fail(err::BadBit, std::format("'{}' is not declared", n->name));
    ...
    if (const auto* n = e.as<IndexExpr>()) {
        QXL_TRY_ASSIGN(Value ix, eval(*n->index));
        if (!ix.constant())
            return fail(err::NotConstant, "a register index must be known at build time");
        const std::int64_t k = ix.asInt();
        if (k < 0 || k >= static_cast<std::int64_t>(reg.size))
            return fail(err::BadBit,
                        std::format("bit index {} is out of range for '{}'", k, id->name));
        return Place{reg, static_cast<std::uint32_t>(k), f.binding, f.scope};
    }
    const auto* r = e.as<SliceExpr>()->range->as<RangeExpr>();
    if (!r)
        return fail(err::Unsupported, "a register slice must be a range");
    QXL_TRY_ASSIGN(Range rg, range(*r));
    const std::int64_t last = rg.start + static_cast<std::int64_t>(rg.count) - 1;
    if (rg.step != 1 || rg.count == 0 || rg.start < 0 ||
        last >= static_cast<std::int64_t>(reg.size))
        return fail(
            err::BadBit,
            std::format("slice of '{}' must be a non-empty contiguous range inside the register",
                        id->name));
    return Place{CregRef{reg.first + static_cast<std::uint32_t>(rg.start),
                         static_cast<std::uint32_t>(rg.count)},
...
Result<Value> Builder::measureValue(const lang::MeasureExpr& m) {
    ...
    if (ws.empty())
        return fail(err::BadWire, "measurement of no qubits");
    const auto n = static_cast<std::uint32_t>(ws.size());
    const CregRef reg = allocRegister("__meas", n, RegKind::Bit, n == 1, false);
    for (std::uint32_t i = 0; i < n; ++i)
        emit(Measure{ws[i], reg.bit(i), std::nullopt, {}});
    return n == 1 ? Value::ofSym(ClassicalExpr::bitRef(reg.bit(0)))
                  : Value::ofSym(ClassicalExpr::regRef(reg));
}
