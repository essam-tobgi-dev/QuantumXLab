...
template <> struct std::hash<qlab::ir::Wire> {
    std::size_t operator()(qlab::ir::Wire w) const noexcept { return std::hash<std::uint32_t>{}(w.index); }
};
template <> struct std::hash<qlab::ir::ClassicalBit> {
    std::size_t operator()(qlab::ir::ClassicalBit b) const noexcept { return std::hash<std::uint32_t>{}(b.index); }
};
