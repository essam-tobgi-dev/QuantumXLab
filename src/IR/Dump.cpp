...
const char* kindName(RegKind k) {
    switch (k) {
    case RegKind::Bit: return "bit";
    case RegKind::Bool: return "bool";
    case RegKind::Int: return "int";
    case RegKind::Uint: return "uint";
    }
    return "?";
}

std::string durationText(const Duration& d) {
    if (d.dt == 0) return std::format("{}ps", d.ps.get());
    if (d.ps.get() == 0) return std::format("{}dt", d.dt);
...
std::string wireList(const std::vector<Wire>& ws) {
    if (ws.empty()) return "*";
    std::string s;
    for (std::size_t i = 0; i < ws.size(); ++i) s += std::format("{}q[{}]", i ? ", " : "", ws[i].index);
    return s;
}
...
std::string gateText(const Gate& g) {
    ...
    if (g.adjoint) s += "inv ";
    if (g.opaque) s += "opaque ";
    s += g.name;
    if (!g.params.empty()) {
        s += "(";
        for (std::size_t k = 0; k < g.params.size(); ++k) s += (k ? ", " : "") + detail::formatReal(g.params[k]);
        s += ")";
    }
    if (g.custom) {
        s += "[";
        for (std::size_t r = 0; r < g.custom->rows; ++r)
            for (std::size_t c = 0; c < g.custom->cols; ++c) {
                ...
                s += std::format("{}{}{:+}i", c ? " " : (r ? "; " : ""), detail::formatReal(z.real()), z.imag());
            }
        s += "]";
    }
    if (!g.targets.empty()) s += " " + wireList(g.targets);
    return s;
}

std::string timed(const std::optional<Picoseconds>& d) { return d ? std::format(" @{}ps", d->get()) : std::string(); }
...
void dumpBody(const Circuit& c, int depth, std::string& out) {
    ...
    for (NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<Gate>(&n)) {
            ...
            out += std::format("measure q[{}] -> {}", m->qubit.index, m->discards() ? "discard" : std::format("c[{}]", m->bit.index));
            ...
            out += bx->duration ? std::format("box[{}] {{\n", durationText(*bx->duration)) : std::string("box {\n");
