...
struct Context {
    ...
    bool inDeclaredRegister(ClassicalBit b) const {
        ...
        if (regs.empty())
            return true; // a register-less circuit (built through the API) has a flat bit space
        return std::any_of(regs.begin(), regs.end(), [&](const BitRegister& r) {
            return b.index >= r.first && b.index < r.first + r.size;
        });
    }
};
...
Status checkWires(std::span<const Wire> ws, const Circuit& c, std::string_view what) {
    ...
    for (Wire w : ws) {
        if (w.index >= c.qubitCount())
            return fail(err::BadWire, std::format("{} uses wire {} outside the circuit ({} qubits)",
                                                  what, w.index, c.qubitCount()));
        if (!seen.insert(w.index).second)
            return fail(err::BadWire, std::format("{} lists wire {} twice", what, w.index));
    }
    return {};
}
...
Status checkBit(ClassicalBit b, const Circuit& c, const Context& ctx, std::string_view what) {
    if (b.index >= c.clbitCount())
        return fail(err::BadBit, std::format("{} uses bit {} outside the circuit ({} bits)", what,
                                             b.index, c.clbitCount()));
    if (!ctx.inDeclaredRegister(b))
        return fail(
            err::BadBit,
            std::format("{} uses bit {}, which belongs to no declared register", what, b.index));
    return {};
}
...
Status checkMatrix(const num::Matrix& m, std::size_t width, std::string_view what) {
    ...
    if (m.rows != dim || m.cols != dim)
        return fail(err::BadArity, std::format("{} carries a {}x{} matrix, expected {}x{}", what,
                                               m.rows, m.cols, dim, dim));
    if (!num::isUnitary(m.view(), num::tol::kUnitaryTol))
        return fail(err::NotUnitary, std::format("{} carries a matrix that is not unitary to {}",
                                                 what, num::tol::kUnitaryTol));
    return {};
}

Status checkGate(const Gate& g, const Circuit& c) {
    const std::string what = std::format("gate '{}'", g.name);
    if (g.opaque && g.custom)
        return fail(err::BadNode, what + " is both defcal-only and matrix-defined");
    if (g.custom) {
        QXL_TRY(checkMatrix(*g.custom, g.targets.size(), what));
    } else if (!g.opaque) {
        const GateDef* d = gates::find(g.name);
        if (!d || g.name == "unitary")
            return fail(err::UnknownGate, std::format("unknown gate '{}'", g.name));
        if (static_cast<int>(g.params.size()) != d->nParams)
            return fail(err::BadArity, std::format("{} takes {} parameter(s), got {}", what,
                                                   d->nParams, g.params.size()));
        if (static_cast<int>(g.targets.size()) != d->nQubits)
            return fail(err::BadArity, std::format("{} acts on {} qubit(s), got {}", what,
                                                   d->nQubits, g.targets.size()));
    }
    ...
    for (Wire t : g.targets)
        if (std::find(g.controls.begin(), g.controls.end(), t) != g.controls.end())
            return fail(err::BadWire,
                        std::format("{} uses wire {} as control and target (they must be disjoint)",
                                    what, t.index));
    const auto ws = g.wires();
    QXL_TRY(checkWires(ws, c, what));
    if (g.matrixCache)
        QXL_TRY(checkMatrix(*g.matrixCache, g.width(), what + " (cached matrix)"));
    return {};
}

Status verifyIn(const Circuit& c, const Context& ctx);

Status checkBody(const Circuit& body, const Circuit& parent, const Context& ctx,
    ...
    if (body.qubitCount() != parent.qubitCount() || body.clbitCount() != parent.clbitCount() ||
        ...
        if (body.qubitCount() != parent.qubitCount() || body.clbitCount() != parent.clbitCount())
            ...
            detail =
                std::format("{} but its parent is {}", body.isPhysical() ? "physical" : "virtual",
...
Status checkCondition(const ClassicalExpr& e, const Circuit& c, const Context& ctx,
                      std::string_view what) {
    for (auto b : e.reads())
        QXL_TRY(checkBit(b, c, ctx, what));
    return {};
}
...
Status verifyIn(const Circuit& c, const Context& ctx) {
    for (NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<Gate>(&n)) {
            ...
            if (!m->discards())
                QXL_TRY(checkBit(m->bit, c, ctx, "measure"));
            ...
            if (co->dst.reg.size == 0 || (co->dst.element && *co->dst.element >= co->dst.reg.size))
                return fail(err::BadBit,
                            "classical assignment targets an empty span or an element outside it");
            for (auto bit : nodeWrites(n))
                QXL_TRY(checkBit(bit, c, ctx, "classical assignment"));
            ...
            if (br->thenBody.present())
                QXL_TRY(checkBody(*br->thenBody, c, ctx, "branch"));
            if (br->elseBody.present())
                QXL_TRY(checkBody(*br->elseBody, c, ctx, "else"));
        } else if (const auto* l = std::get_if<Loop>(&n)) {
            QXL_TRY(checkCondition(l->cond, c, ctx, "loop condition"));
            if (l->maxIterations == 0)
                return fail(err::BadNode, "loop has no iteration bound");
            if (l->body.present())
                QXL_TRY(checkBody(*l->body, c, ctx, "loop"));
        } else if (const auto* bx = std::get_if<Box>(&n)) {
            if (bx->body.present())
                QXL_TRY(checkBody(*bx->body, c, ctx, "box"));
        }
    }
    ...
    for (std::uint32_t q = 0; q < c.qubitCount(); ++q)
        for (NodeId id : c.onWire(Wire{q})) {
            ...
            if (std::find(ws.begin(), ws.end(), Wire{q}) == ws.end())
                return fail(
                    err::DanglingWire,
                    std::format("node {} is listed on wire {} but does not touch it", id.get(), q));
        }
    return {};
}
...
Status checkRegisters(const std::vector<R>& regs, std::uint32_t space, std::string_view kind) {
    ...
    for (const auto& r : regs) {
        if (r.size == 0 || r.first + r.size > space)
            return fail(err::BadNode,
                        std::format("{} register '{}' does not fit the {} space of size {}", kind,
                                    r.name, kind, space));
        for (std::uint32_t i = r.first; i < r.first + r.size; ++i)
            if (used[i]++)
                return fail(err::BadNode, std::format("{} register '{}' overlaps another register",
                                                      kind, r.name));
    }
    return {};
}
