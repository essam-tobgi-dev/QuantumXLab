...
Result<Circuit> Circuit::inverse() const {
    if (!isPureUnitary())
        return fail(err::NotPure, "only a circuit without measurement, reset or classical control can be inverted");
    Circuit out;
    ...
    for (auto it = topo.rbegin(); it != topo.rend(); ++it) {
        ...
        if (const auto* g = std::get_if<Gate>(&n)) {
            ...
            if (!g->opaque && !g->custom && !g->adjoint) rw = gates::inverseOf(g->name, g->params);
            if (rw) {
                inv.name = rw->name;
...
bool closeMatrix(const std::optional<num::Matrix>& a, const std::optional<num::Matrix>& b, double tol) {
...
bool sameBody(const SubCircuit& x, const SubCircuit& y, double tol) {
    if (x.present() && y.present()) return (*x).structurallyEqual(*y, tol);
...
bool sameNode(const Node& x, const Node& y, double tol) {
    ...
    if (const auto* gx = std::get_if<Gate>(&x)) {
        ...
        return gx->name == gy.name && gx->targets == gy.targets && gx->controls == gy.controls &&
               gx->negControl == gy.negControl && gx->adjoint == gy.adjoint && gx->opaque == gy.opaque &&
               closeParams(gx->params, gy.params, tol) && closeMatrix(gx->custom, gy.custom, tol);
    }
    if (const auto* m = std::get_if<Measure>(&x)) return m->qubit == std::get<Measure>(y).qubit && m->bit == std::get<Measure>(y).bit;
    if (const auto* r = std::get_if<Reset>(&x)) return r->qubit == std::get<Reset>(y).qubit;
    if (const auto* b = std::get_if<Barrier>(&x)) return b->wires == std::get<Barrier>(y).wires;
    if (const auto* d = std::get_if<Delay>(&x)) return d->duration == std::get<Delay>(y).duration && d->wires == std::get<Delay>(y).wires;
    if (const auto* c = std::get_if<ClassicalOp>(&x)) return c->expr == std::get<ClassicalOp>(y).expr && c->dst == std::get<ClassicalOp>(y).dst;
    ...
    if (const auto* l = std::get_if<Loop>(&x)) {
        ...
        return l->cond == o.cond && l->maxIterations == o.maxIterations && sameBody(l->body, o.body, tol);
    }
...
bool Circuit::structurallyEqual(const Circuit& o, double tol) const {
    if (qubits_ != o.qubits_ || clbits_ != o.clbits_ || physical_ != o.physical_) return false;
    auto a = topologicalOrder(), b = o.topologicalOrder();
    if (a.size() != b.size()) return false;
    for (std::size_t i = 0; i < a.size(); ++i)
        if (!sameNode(nodes_[a[i].get()], o.nodes_[b[i].get()], tol)) return false;
    return true;
}
...
// basis states whose control bits equal `pattern` (T11 §2.3 index permutation, no 2^n × 2^n product).
void applyLeft(num::Matrix& acc, const num::Matrix& u, std::span<const std::size_t> targets,
    ...
    for (std::size_t t = 0; t < targets.size(); ++t) targetMask |= std::size_t{1} << targets[t];
    for (std::size_t m = 0; m < k; ++m)
        for (std::size_t t = 0; t < targets.size(); ++t)
            if ((m >> t) & 1u) offset[m] |= std::size_t{1} << targets[t];
    std::vector<num::Complex> in(k);
    for (std::size_t base = 0; base < dim; ++base) {
        if ((base & targetMask) != 0 || (base & ctrlMask) != pattern) continue;
        for (std::size_t col = 0; col < dim; ++col) {
            for (std::size_t m = 0; m < k; ++m) in[m] = acc(base + offset[m], col);
            for (std::size_t r = 0; r < k; ++r) {
                num::Complex s{};
                for (std::size_t m = 0; m < k; ++m) s += u(r, m) * in[m];
        ...
        if (const auto* g = std::get_if<Gate>(&node)) {
            ...
            for (Wire w : g->wires()) {
                if (w.index >= n || (used >> w.index) & 1u)
                    return fail(err::BadWire, std::format("gate '{}' uses wire {} outside the {}-qubit space or twice", g->name, w.index, n));
                used |= std::size_t{1} << w.index;
            }
            for (Wire w : g->targets) targets.push_back(w.index);
            for (std::size_t j = 0; j < g->controls.size(); ++j) {
                const std::size_t bit = std::size_t{1} << g->controls[j].index;
                mask |= bit;
                if (!g->isNegControl(j)) pattern |= bit;
            }
            QXL_TRY_ASSIGN(num::Matrix base, baseMatrixOf(*g));
            if (base.rows != (std::size_t{1} << targets.size()) || !base.square())
                return fail(err::BadArity, std::format("gate '{}' matrix does not match its {} target(s)", g->name, targets.size()));
            ...
            return fail(err::NotPure, std::format("toUnitary needs a circuit without measurement, reset or "
                                                  "classical control; found a {} node", nodeKindName(node)));
        }
    }
    return {};
}
...
Result<num::Matrix> toUnitary(const Circuit& c, std::uint32_t nQubits) {
    ...
    if (n > 12) return fail(err::TooLarge, std::format("toUnitary is limited to 12 qubits, got {}", n));
    if (!c.isPureUnitary())
        return fail(err::NotPure, "toUnitary needs a circuit without measurement, reset or classical control");
