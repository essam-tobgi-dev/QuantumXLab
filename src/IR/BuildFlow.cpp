...
Result<Circuit> Builder::subCircuit(const StmtList& body, bool conditional) {
    ...
    if (conditional)
        conditionScope_ = scopes_.size() - 1;
    ...
    if (conditional && flow_ != Flow::Normal)
        return fail(
            err::Unsupported,
            "break, continue, return and end are not supported inside a run-time branch or loop");
    return sub;
}
...
Status Builder::lowerFor(const lang::ForStmt& f) {
    ...
    if (const auto* r = f.iterable->as<lang::RangeExpr>()) {
        ...
        for (const auto& item : s->items) {
            ...
            if (!v.constant())
                return fail(err::NotConstant, "for-loop set members must be known at build time");
    ...
    if (rg.count > opts_.loopUnrollBound / perIteration) {
        const std::uint64_t stmts =
            rg.count > UINT64_MAX / perIteration ? UINT64_MAX : rg.count * perIteration;
    ...
    for (std::uint64_t i = 0; i < rg.count; ++i) {
        ...
        Binding b;
        b.value =
            set.empty() ? Value::ofInt(rg.start + static_cast<std::int64_t>(i) * rg.step) : set[i];
        ...
        if (flow_ == Flow::Continue)
            flow_ = Flow::Normal;
        if (flow_ == Flow::Return || flow_ == Flow::End)
            break;
    }
    --loopDepth_;
    return st;
...
Status Builder::lowerWhile(const lang::WhileStmt& w) {
    ...
    for (std::uint64_t iterations = 0;; ++iterations) {
        ...
        if (!cur) {
            st = std::unexpected(cur.error());
            break;
        }
        if (!cur->constant()) {
            st =
                fail(err::NotConstant, "while condition became run-time dependent while unrolling");
            break;
        }
        if (cur->asDouble() == 0.0)
            break;
        if (iterations >= opts_.loopUnrollBound) {
            const std::uint64_t stmts =
                (iterations + 1) * std::max<std::uint64_t>(w.body.size(), 1);
...
Status Builder::lowerBox(const lang::BoxStmt& b) {
    Box box;
    if (b.duration) {
        QXL_TRY_ASSIGN(Duration d, evalDuration(**b.duration));
        box.duration = d;
    }
