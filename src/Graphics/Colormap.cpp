...
const Colormap& Colormap::get(ColormapId id) {
    static const Colormap maps[4] = {Colormap(k_viridis, 33, false), Colormap(k_inferno, 33, false),
                                     Colormap(k_coolwarm, 33, false),
                                     Colormap(k_twilight, 33, true)};
    return maps[static_cast<int>(id)];
}
ColormapId Colormap::byName(std::string_view n) {
    if (n == "inferno")
        return ColormapId::Inferno;
    if (n == "coolwarm")
        return ColormapId::Coolwarm;
    if (n == "twilight")
        return ColormapId::Twilight;
...
std::vector<glm::vec3> Colormap::lut(int n) const {
    ...
    for (int i = 0; i < n; ++i)
        out[static_cast<std::size_t>(i)] =
            sample(static_cast<float>(i) / static_cast<float>(cyclic_ ? n : n - 1));
    return out;
}
