...
namespace {
glm::vec3 catmull(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2,
...
std::vector<glm::vec3> smoothPath(const std::vector<glm::vec3>& p, int sub) {
    ...
    for (std::size_t i = 0; i + 1 < p.size(); ++i) {
        ...
        for (int s = 0; s < sub; ++s)
            out.push_back(catmull(p0, p[i], p[i + 1], p3, static_cast<float>(s) / sub));
    }
...
MeshData tube(const std::vector<glm::vec3>& inPath, float r, int seg, bool smooth) {
    ...
    for (std::size_t i = 0; i < path.size(); ++i) {
        ...
        if (i > 0)
            arc += glm::length(path[i] - path[i - 1]);
        glm::vec3 n = prevN - t * glm::dot(prevN, t);
        if (glm::length(n) < 1e-4f)
            n = std::abs(t.y) < 0.9f ? glm::vec3(0, 1, 0) : glm::vec3(1, 0, 0),
            n -= t * glm::dot(n, t);
        n = glm::normalize(n);
    ...
    for (int end = 0; end < 2; ++end) {
        ...
        glm::vec3 t = end == 0 ? glm::normalize(path[0] - path[1])
                               : glm::normalize(path.back() - path[path.size() - 2]);
        ...
        for (int j = 0; j <= seg; ++j)
            m.vertices.push_back({m.vertices[ring + j].position, t, {0, 0}});
        for (int j = 0; j < seg; ++j) {
            if (end == 0)
                m.indices.insert(m.indices.end(), {c, c + 1 + j, c + 2 + j});
            else
                m.indices.insert(m.indices.end(), {c, c + 2 + j, c + 1 + j});
        }
    }
    m.orientToNormals();
    return m;
...
MeshData extrudePolygon(const std::vector<glm::vec2>& poly, float h) {
    ...
    auto cross2 = [](glm::vec2 a, glm::vec2 b, glm::vec2 c) {
    ...
    while (idx.size() > 3 && guard++ < 10000) {
        ...
        for (std::size_t i = 0; i < idx.size(); ++i) {
            std::uint32_t ia = idx[(i + idx.size() - 1) % idx.size()], ib = idx[i],
                          ic = idx[(i + 1) % idx.size()];
            if (cross2(poly[ia], poly[ib], poly[ic]) <= 0)
                continue;
            bool ok = true;
            for (auto j : idx)
                if (j != ia && j != ib && j != ic &&
                    inside(poly[j], poly[ia], poly[ib], poly[ic])) {
                    ok = false;
                    break;
                }
            if (!ok)
        ...
        for (std::size_t t = 0; t + 2 < tris.size(); t += 3) {
            if (side)
                m.indices.insert(m.indices.end(),
                                 {base + tris[t], base + tris[t + 2], base + tris[t + 1]});
            else
                m.indices.insert(m.indices.end(),
                                 {base + tris[t], base + tris[t + 1], base + tris[t + 2]});
        }
    }
    m.orientToNormals();
    return m;
...
MeshData extrudePath(const std::vector<glm::vec2>& path, float w, float h) {
    ...
    for (std::size_t i = 0; i < path.size(); ++i) {
        glm::vec2 d0 =
            i > 0 ? glm::normalize(path[i] - path[i - 1]) : glm::normalize(path[1] - path[0]);
...
MeshData plateWithHoles(glm::vec2 size, float t, const std::vector<glm::vec3>& holes) {
    ...
    for (auto& hcyl : holes) {
        ...
        for (std::size_t i = 0; i + 2 < c.indices.size(); i += 3)
            std::swap(c.indices[i + 1], c.indices[i + 2]);
