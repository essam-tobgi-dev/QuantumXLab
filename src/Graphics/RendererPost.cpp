...
void Renderer::ssaoPass() {
    ...
    ssaoUpsample_.use();
    ssaoH_.bind(1);
    ssaoUpsample_.set("uAo", 1); ssaoUpsample_.set("uDepth", 0);
...
void Renderer::bloomPass() {
    ...
    const glm::vec2 dx(1.0f / static_cast<float>(bw), 0.0f), dy(0.0f, 1.0f / static_cast<float>(bh));
    ...
    bloomFboA_.bind(); bloomB_.bind(0); bloomBlur_.set("uDir", dy); screenQuad_.draw();
    bloomFboB_.bind(); bloomA_.bind(0); bloomBlur_.set("uDir", dx); screenQuad_.draw();
    bloomFboA_.bind(); bloomB_.bind(0); bloomBlur_.set("uDir", dy); screenQuad_.draw();
    stats_.drawCalls += 3;
}
