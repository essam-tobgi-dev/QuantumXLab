...
void Buffer::setData(std::span<const std::byte> bytes, BufferUsage usage) {
    GLenum u = usage == BufferUsage::Static    ? GL_STATIC_DRAW
...
void Buffer::update(std::size_t offset, std::span<const std::byte> bytes) {
    ...
    if (offset + bytes.size() > size_) {
        glBufferData(target_, static_cast<GLsizeiptr>(offset + bytes.size()), nullptr,
                     GL_DYNAMIC_DRAW);
        size_ = offset + bytes.size();
    }
    glBufferSubData(target_, static_cast<GLintptr>(offset), static_cast<GLsizeiptr>(bytes.size()),
                    bytes.data());
}
...
void VertexArray::setAttribs(const Buffer& buf, std::span<const VertexAttrib> attribs) {
    ...
    for (const auto& a : attribs) {
        ...
        if (a.type == GL_UNSIGNED_INT || a.type == GL_INT)
            ...
            glVertexAttribPointer(a.index, a.components, a.type, a.normalized ? GL_TRUE : GL_FALSE,
                                  a.stride, off);
...
void Texture2D::allocate(const void* pixels) {
    ...
    if (desc_.samples > 0) {
        ...
        glTexImage2D(tgt, 0, static_cast<GLint>(internal), desc_.width, desc_.height, 0, format,
                     type, pixels);
        ...
        glTexParameteri(tgt, GL_TEXTURE_MIN_FILTER,
                        desc_.mipmaps && !integer ? GL_LINEAR_MIPMAP_LINEAR : filt);
...
TextureCube::TextureCube(const CubeDesc& d) : desc_(d) {
    ...
    for (int level = 0; level < desc_.levels; ++level) {
        ...
        for (int face = 0; face < 6; ++face)
            glTexImage2D(faceTarget(face), level, static_cast<GLint>(internal), sz, sz, 0, format,
                         type, nullptr);
    }
    const GLint filt = desc_.linear ? GL_LINEAR : GL_NEAREST;
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER,
                    desc_.levels > 1
                        ? (desc_.linear ? GL_LINEAR_MIPMAP_LINEAR : GL_NEAREST_MIPMAP_NEAREST)
                        : filt);
...
// ---------------------------------------------------------------- TimerQuery
TimerQuery::TimerQuery(bool create) {
    if (create)
        glGenQueries(1, &id_);
}
...
void Framebuffer::setDrawBuffers(int count) {
    ...
    for (int i = 0; i < count && i < 8; ++i)
        bufs[i] = GL_COLOR_ATTACHMENT0 + static_cast<GLenum>(i);
    if (count == 0)
...
Status Framebuffer::check() const {
    ...
    if (s != GL_FRAMEBUFFER_COMPLETE)
        return fail(ErrorCode::Gfx_ + 10, "framebuffer incomplete: 0x" + std::to_string(s));
    return {};
}
void Framebuffer::blitTo(const Framebuffer& dst, int srcIndex, int dstIndex, int w, int h,
                         GLenum filter, bool depth) const {
