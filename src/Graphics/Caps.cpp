...
Caps Caps::query() {
    ...
    for (GLint i = 0; i < nExt; ++i) {
        ...
        if (!e)
            continue;
        if (std::strcmp(e, "GL_EXT_texture_filter_anisotropic") == 0 ||
            std::strcmp(e, "GL_ARB_texture_filter_anisotropic") == 0)
            c.anisotropic = true;
        if (std::strcmp(e, "GL_KHR_debug") == 0)
            c.debugOutput = true;
    }
    if (c.anisotropic) {
...
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT 0x84FF
    ...
    c.renderer = str(GL_RENDERER);
    c.vendor = str(GL_VENDOR);
    c.version = str(GL_VERSION);
    c.glsl = str(GL_SHADING_LANGUAGE_VERSION);
    QXL_LOG_INFO(Gfx, "OpenGL {}.{} {} | {} | GLSL {} | samples {} | compute {}", c.major, c.minor,
                 c.renderer, c.vendor, c.glsl, c.maxSamples, c.computeShaders);
    return c;
}
} // namespace qlab::gfx
