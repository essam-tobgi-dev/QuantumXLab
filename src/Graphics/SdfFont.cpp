...
std::vector<unsigned> charset() {
    ...
    for (unsigned c = 32; c < 127; ++c) cs.push_back(c);
    for (unsigned c = 0x391; c <= 0x3C9; ++c) cs.push_back(c);            // Greek
    for (unsigned c : {0x00B0u, 0x00B1u, 0x00B5u, 0x00D7u, 0x2013u, 0x2014u, 0x2019u, 0x2022u, 0x2026u,
...
void makeSdf(const std::vector<std::uint8_t>& bin, int w, int h, float spread, std::vector<std::uint8_t>& out) {
    ...
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            ...
            for (int dy = -R; dy <= R; ++dy) {
                int yy = y + dy;
                if (yy < 0 || yy >= h) { if (inside) { float d = static_cast<float>(dy * dy); best = std::min(best, d); } continue; }
                for (int dx = -R; dx <= R; ++dx) {
                    int xx = x + dx;
                    bool other = (xx < 0 || xx >= w) ? false : bin[static_cast<std::size_t>(yy * w + xx)] > 127;
                    if (other != inside) best = std::min(best, static_cast<float>(dx * dx + dy * dy));
                }
            }
            float d = std::sqrt(best);
            float v = inside ? d : -d;
            out[static_cast<std::size_t>(y * w + x)] = static_cast<std::uint8_t>(std::clamp(128.0f + v / spread * 127.0f, 0.0f, 255.0f));
        }
}
} // namespace
...
Result<SdfAtlasData> buildSdfAtlas(const std::filesystem::path& ttf, int px, float spread) {
    FT_Library lib;
    if (FT_Init_FreeType(&lib)) return fail(ErrorCode::Gfx_ + 20, "FreeType init failed");
    FT_Face face;
    if (FT_New_Face(lib, ttf.string().c_str(), 0, &face)) { FT_Done_FreeType(lib); return fail(ErrorCode::Gfx_ + 21, "cannot load font " + ttf.string()); }
    FT_Set_Pixel_Sizes(face, 0, static_cast<FT_UInt>(px));
    SdfAtlasData a;
    a.nominalPx = px; a.spread = spread;
    ...
    struct Cell { unsigned cp; int w, h; std::vector<std::uint8_t> sdf; float bx, by, adv; };
    ...
    for (unsigned cp : charset()) {
        ...
        for (unsigned y = 0; y < bm.rows; ++y)
            for (unsigned x = 0; x < bm.width; ++x)
                bin[static_cast<std::size_t>((static_cast<int>(y) + pad) * w + static_cast<int>(x) + pad)] = bm.buffer[y * static_cast<unsigned>(bm.pitch) + x];
        Cell c{cp, w, h, {}, static_cast<float>(face->glyph->bitmap_left) - static_cast<float>(pad),
               static_cast<float>(face->glyph->bitmap_top) + static_cast<float>(pad), static_cast<float>(face->glyph->advance.x) / 64.0f};
    ...
    for (auto& c : cells) {
        if (x + c.w > W) { x = 0; y += rowH; rowH = 0; }
        pos.emplace_back(x, y);
        x += c.w; rowH = std::max(rowH, c.h);
    }
    int H = y + rowH;
    ...
    for (std::size_t i = 0; i < cells.size(); ++i) {
        ...
        for (int yy = 0; yy < c.h; ++yy)
            std::memcpy(&a.pixels[static_cast<std::size_t>((py0 + yy) * W + px0)], &c.sdf[static_cast<std::size_t>(yy * c.w)], static_cast<std::size_t>(c.w));
        Glyph g;
...
Status saveAtlasCache(const SdfAtlasData& a, const std::filesystem::path& p) {
    ...
    if (!f) return fail(ErrorCode::Io, "cannot write " + p.string());
    const char magic[8] = {'Q', 'X', 'L', 'S', 'D', 'F', '0', '2'};
    f.write(magic, 8);
    auto wr = [&](const auto& v) { f.write(reinterpret_cast<const char*>(&v), sizeof v); };
    wr(a.width); wr(a.height); wr(a.nominalPx); wr(a.spread); wr(a.ascent); wr(a.descent); wr(a.lineHeight);
    std::uint32_t n = static_cast<std::uint32_t>(a.glyphs.size()); wr(n);
    for (auto& [cp, g] : a.glyphs) { wr(cp); wr(g); }
    f.write(reinterpret_cast<const char*>(a.pixels.data()), static_cast<std::streamsize>(a.pixels.size()));
    return {};
}
Result<SdfAtlasData> loadAtlasCache(const std::filesystem::path& p) {
    std::ifstream f(p, std::ios::binary);
    if (!f) return fail(ErrorCode::Io, "no cache");
    char magic[8]; f.read(magic, 8);
    if (std::memcmp(magic, "QXLSDF02", 8) != 0) return fail(ErrorCode::Parse, "bad cache magic");
    SdfAtlasData a;
    auto rd = [&](auto& v) { f.read(reinterpret_cast<char*>(&v), sizeof v); };
    rd(a.width); rd(a.height); rd(a.nominalPx); rd(a.spread); rd(a.ascent); rd(a.descent); rd(a.lineHeight);
    std::uint32_t n = 0; rd(n);
    for (std::uint32_t i = 0; i < n; ++i) { unsigned cp; Glyph g; rd(cp); rd(g); a.glyphs[cp] = g; }
    ...
    if (!f) return fail(ErrorCode::Parse, "truncated cache");
    return a;
}

Result<std::unique_ptr<SdfFont>> SdfFont::load(const std::filesystem::path& ttf, int px) {
    auto cache = core::userDataDir() / "fontcache" / (ttf.stem().string() + "_" + std::to_string(px) + ".sdf");
    Result<SdfAtlasData> data = loadAtlasCache(cache);
    if (!data) {
        data = buildSdfAtlas(ttf, px);
        if (!data) return std::unexpected(data.error());
        if (auto s = saveAtlasCache(*data, cache); !s) QXL_LOG_WARN(Gfx, "font cache not written: {}", s.error().message);
    }
    auto font = std::unique_ptr<SdfFont>(new SdfFont());
    font->data_ = std::move(*data);
    TexDesc d; d.width = font->data_.width; d.height = font->data_.height; d.format = TexFormat::R8; d.linear = true;
    font->tex_ = Texture2D(d, font->data_.pixels.data());
    return font;
}
const Glyph* SdfFont::glyph(unsigned cp) const {
    auto it = data_.glyphs.find(cp);
    if (it == data_.glyphs.end()) it = data_.glyphs.find('?');
...
glm::vec2 SdfFont::measure(std::string_view s, float sizePx) const {
    ...
    for (unsigned cp : decodeUtf8(s)) if (auto* g = glyph(cp)) w += g->advance * scale;
