...
Status Renderer::init(const RendererDesc& d) {
    desc_ = d;
    auto load = [&](ShaderProgram& p, const char* name, const char* vs, const char* fs,
                    std::vector<std::string> defs = {}) -> Status {
        ShaderDesc sd;
        sd.name = name;
        sd.vertex = vs;
        sd.fragment = fs;
        sd.defines = std::move(defs);
    ...
    QXL_TRY(load(pbrTexInst_, "pbr_textured_instanced", "pbr.vert", "pbr.frag",
                 {"INSTANCED", "TEXTURED"}));
    ...
    QXL_TRY(load(ssaoUpsample_, "ssao_blur_upsample", "fullscreen.vert", "ssao_blur.frag",
                 {"UPSAMPLE"}));
    ...
    // shadow map
    TexDesc sd;
    sd.width = sd.height = d.shadowSize;
    sd.format = TexFormat::Depth32F;
    sd.linear = true;
    ...
    if (!f)
        QXL_LOG_WARN(Gfx, "font not loaded ({}); text disabled", f.error().message);
    else {
        font_ = std::move(*f);
...
void Renderer::resize(int w, int h) {
    ...
    c.width = w_;
    c.height = h_;
    c.format = TexFormat::RGBA16F;
    c.samples = s;
    TexDesc dd = c;
    dd.format = TexFormat::Depth24Stencil8;
    msaaColor_ = Texture2D(c);
    msaaDepth_ = Texture2D(dd);
    msaaFbo_ = Framebuffer();
    msaaFbo_.attachColor(0, msaaColor_);
    msaaFbo_.attachDepth(msaaDepth_, true);
    msaaFbo_.setDrawBuffers(1);
    if (auto st = msaaFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "msaa fbo: {}", st.error().message);
    c.samples = 0;
    dd.samples = 0;
    ...
    if (auto st = resolveFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "resolve fbo: {}", st.error().message);
    TexDesc i = c;
    i.format = TexFormat::R32UI;
    i.linear = false;
    resId_ = Texture2D(i);
    idDepth_ = Texture2D(dd);
    TexDesc ld = c;
    ld.format = TexFormat::R32F;
    ld.linear = false;
    idLinDepth_ = Texture2D(ld);
    idFbo_ = Framebuffer();
    idFbo_.attachColor(0, resId_);
    idFbo_.attachColor(1, idLinDepth_);
    idFbo_.attachDepth(idDepth_, true);
    idFbo_.setDrawBuffers(2);
    if (auto st = idFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "id fbo: {}", st.error().message);
    TexDesc p;
    p.width = w_;
    p.height = h_;
    p.format = TexFormat::RGBA8;
    postTex_ = Texture2D(p);
    postFbo_ = Framebuffer();
    postFbo_.attachColor(0, postTex_);
    postFbo_.setDrawBuffers(1);
    if (auto st = postFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "post fbo: {}", st.error().message);
    ...
    ao.width = hw;
    ao.height = hh;
    ao.format = TexFormat::RG16F;
    ao.linear = true;
    ssaoRaw_ = Texture2D(ao);
    ssaoH_ = Texture2D(ao);
    ssaoRawFbo_ = Framebuffer();
    ssaoRawFbo_.attachColor(0, ssaoRaw_);
    ssaoRawFbo_.setDrawBuffers(1);
    if (auto st = ssaoRawFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "ssao raw fbo: {}", st.error().message);
    ssaoHFbo_ = Framebuffer();
    ssaoHFbo_.attachColor(0, ssaoH_);
    ssaoHFbo_.setDrawBuffers(1);
    if (auto st = ssaoHFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "ssao h fbo: {}", st.error().message);
    TexDesc aoFull;
    aoFull.width = w_;
    aoFull.height = h_;
    aoFull.format = TexFormat::R8;
    aoFull.linear = true;
    ssaoTex_ = Texture2D(aoFull);
    ssaoFbo_ = Framebuffer();
    ssaoFbo_.attachColor(0, ssaoTex_);
    ssaoFbo_.setDrawBuffers(1);
    if (auto st = ssaoFbo_.check(); !st)
        QXL_LOG_ERROR(Gfx, "ssao fbo: {}", st.error().message);
    ...
    bl.width = hw;
    bl.height = hh;
    bl.format = TexFormat::R11G11B10F;
    bl.linear = true;
    bloomA_ = Texture2D(bl);
    bloomB_ = Texture2D(bl);
    bloomFboA_ = Framebuffer();
    bloomFboA_.attachColor(0, bloomA_);
    bloomFboA_.setDrawBuffers(1);
    if (auto st = bloomFboA_.check(); !st)
        QXL_LOG_ERROR(Gfx, "bloom fbo A: {}", st.error().message);
    bloomFboB_ = Framebuffer();
    bloomFboB_.attachColor(0, bloomB_);
    bloomFboB_.setDrawBuffers(1);
    if (auto st = bloomFboB_.check(); !st)
        QXL_LOG_ERROR(Gfx, "bloom fbo B: {}", st.error().message);
    Framebuffer::bindDefault();
