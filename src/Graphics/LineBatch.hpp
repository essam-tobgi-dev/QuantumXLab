...
struct LineVertex {
    glm::vec3
        position;    // relative to the batch origin (the camera), subtracted in double on the CPU
...
class LineBatch {
  public:
    ...
    // so micrometre-scale overlays on the chip stay exact next to the metre-scale room (spec 18
    // §6).
    ...
    void segment(const glm::dvec3& a, const glm::dvec3& b, const glm::vec4& color,
                 float widthPx = 1.5f, float dashPx = 0);
    void polyline(const std::vector<glm::vec3>& pts, const glm::vec4& color, float widthPx = 1.5f,
                  bool closed = false);
    void circle(const glm::vec3& center, const glm::vec3& normal, float radius,
                const glm::vec4& color, int seg = 64, float widthPx = 1.5f);
    void arrow(const glm::dvec3& from, const glm::dvec3& to, const glm::vec4& color,
               float widthPx = 2.0f, float headFrac = 0.15f);
