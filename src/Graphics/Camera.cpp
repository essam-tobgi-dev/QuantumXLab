...
bool Frustum::intersects(const Aabb& box) const {
    for (const auto& p : planes) {
        ...
        if (glm::dot(glm::dvec3(p), v) + p.w < 0) return false;
    }
    return true;
}
...
void Camera::lookAt(const glm::dvec3& pos, const glm::dvec3& tgt, const glm::dvec3& up) {
    pos_ = pos; target_ = tgt; up_ = glm::normalize(up);
}
void Camera::set(const Bookmark& b) {
    lookAt(b.position, b.target, b.up);
    fovDeg_ = b.fovDeg; ortho_ = b.ortho; orthoHeight_ = b.orthoHeight;
    animT_ = animDur_ = 0.0; animT_ = 1.0;
}
...
glm::dvec3 Camera::right() const {
    ...
    if (l < 1e-12) r = glm::cross(f, glm::dvec3(1, 0, 0)), l = glm::length(r);
    return r / l;
}
...
void Camera::pan(double dxPx, double dyPx, int viewportH) {
    ...
    double worldPerPx;
    if (ortho_) worldPerPx = orthoHeight_ / h;
    else worldPerPx = 2.0 * distance() * std::tan(glm::radians(fovDeg_) * 0.5) / h;
    glm::dvec3 r = right();
...
void Camera::dolly(double scrollSteps) {
    ...
    if (ortho_) { orthoHeight_ = std::max(1e-9, orthoHeight_ * f); return; }
...
void Camera::dollyToward(const glm::dvec3& anchor, double scrollSteps) {
    ...
    if (ortho_) {
        // Keep `anchor` fixed on screen: the view-plane offset from the target scales with the height.
    ...
    if (glm::length(toEye) * f < 1e-7) return;   // never fold the camera onto the anchor
    pos_ = anchor + toEye * f;
...
void Camera::setPivot(const glm::dvec3& point) {
    ...
    if (depth <= 1e-9) return;                   // behind the eye: keep the current pivot
    target_ = pos_ + fwd * depth;
}
void Camera::cancelTransition() {
    animDur_ = 0.0; animT_ = 1.0;
}
...
void Camera::rotateInPlace(double dxPx, double dyPx) {
    ...
    if (std::abs(glm::dot(glm::normalize(f2), up_)) < 0.995) f = f2;
    target_ = pos_ + f * dist;
}

void Camera::transitionTo(const Bookmark& b, double durationS) {
    from_ = bookmark(); to_ = b;
    animDur_ = std::max(0.0, durationS); animT_ = 0.0;
    if (animDur_ == 0.0) set(b);
}
void Camera::update(double dt) {
    ...
    if (animT_ >= animDur_) { ortho_ = to_.ortho; animT_ = animDur_ = 0.0; animT_ = 1.0; }
}
...
void Camera::frame(const Aabb& box, double margin) {
    ...
    if (ortho_) {
        ...
        target_ = c; pos_ = c - f * d;
    }
    near_ = std::max(1e-4 * r, distance() * 1e-4);
    far_ = std::max(far_, distance() + 4.0 * r);
}

glm::dmat4 Camera::viewMatrix() const { return glm::lookAt(pos_, target_, up_); }
...
Ray Camera::ray(double px, double py, int w, int h) const {
    ...
    if (ortho_) return {pa, glm::normalize(pb - pa)};
