...
int g_glfwRefs = 0;
std::mutex g_glfwMu;
void glfwErrorCb(int code, const char* msg) { QXL_LOG_ERROR(Gfx, "GLFW error {}: {}", code, msg ? msg : ""); }
bool glfwAcquire() {
...
Result<std::unique_ptr<Window>> Window::create(const WindowDesc& desc) {
    if (!glfwAcquire()) return fail(ErrorCode::Gfx_ + 1, "glfwInit failed");
...
#endif
    if (!win) win = tryCreate(4, 1);
    if (!win) { glfwRelease(); return fail(ErrorCode::Gfx_ + 2, "could not create an OpenGL 4.1+ core context"); }
    ...
    if (!loadGl(reinterpret_cast<void* (*)(const char*)>(glfwGetProcAddress))) {
        glfwDestroyWindow(win); glfwRelease();
    ...
    float sx = 1, sy = 1; glfwGetWindowContentScale(win, &sx, &sy); w->scale_ = sx;
    w->caps_ = Caps::query();
    installCallbacks(w.get());
    if (desc.srgb) glEnable(GL_FRAMEBUFFER_SRGB);
    drainGlErrors("Window::create");
    return w;
}

Window::~Window() {
    if (win_) { glfwDestroyWindow(win_); glfwRelease(); }
}
...
void Window::installCallbacks(Window* w) {
    glfwSetFramebufferSizeCallback(w->win_, [](GLFWwindow* g, int fw, int fh) {
        ...
        self->fbW_ = fw; self->fbH_ = fh;
        glfwGetWindowSize(g, &self->winW_, &self->winH_);
        if (self->onFramebufferResize) self->onFramebufferResize(fw, fh);
    });
    glfwSetCursorPosCallback(w->win_, [](GLFWwindow* g, double x, double y) {
        ...
        self->input_.mouseX = nx; self->input_.mouseY = ny;
    });
    glfwSetMouseButtonCallback(w->win_, [](GLFWwindow* g, int b, int action, int mods) {
        auto* self = static_cast<Window*>(glfwGetWindowUserPointer(g));
        if (b >= 0 && b < 8) self->input_.mouseDown[static_cast<std::size_t>(b)] = (action != GLFW_RELEASE);
        self->input_.shift = mods & GLFW_MOD_SHIFT; self->input_.ctrl = mods & GLFW_MOD_CONTROL;
        self->input_.alt = mods & GLFW_MOD_ALT; self->input_.super = mods & GLFW_MOD_SUPER;
    });
    glfwSetScrollCallback(w->win_, [](GLFWwindow* g, double x, double y) {
        auto* self = static_cast<Window*>(glfwGetWindowUserPointer(g));
        self->input_.scrollX += x; self->input_.scrollY += y;
    });
    glfwSetKeyCallback(w->win_, [](GLFWwindow* g, int key, int, int action, int mods) {
        auto* self = static_cast<Window*>(glfwGetWindowUserPointer(g));
        if (key >= 0 && key < 512) self->input_.keyDown[static_cast<std::size_t>(key)] = (action != GLFW_RELEASE);
        self->input_.shift = mods & GLFW_MOD_SHIFT; self->input_.ctrl = mods & GLFW_MOD_CONTROL;
        self->input_.alt = mods & GLFW_MOD_ALT; self->input_.super = mods & GLFW_MOD_SUPER;
    });
    ...
    glfwSetDropCallback(w->win_, [](GLFWwindow* g, int count, const char** paths) {
        auto* self = static_cast<Window*>(glfwGetWindowUserPointer(g));
        if (!self->onDrop) return;
        std::vector<std::string> v; for (int i = 0; i < count; ++i) v.emplace_back(paths[i]);
        self->onDrop(v);
    });
}

void Window::pollEvents() { input_.beginPoll(); glfwPollEvents(); }
void Window::swapBuffers() { glfwSwapBuffers(win_); }
bool Window::shouldClose() const { return glfwWindowShouldClose(win_); }
void Window::requestClose() { glfwSetWindowShouldClose(win_, GLFW_TRUE); }
void Window::makeCurrent() { glfwMakeContextCurrent(win_); }
void Window::setVsync(bool on) { glfwSwapInterval(on ? 1 : 0); }
void Window::setTitle(const std::string& t) { glfwSetWindowTitle(win_, t.c_str()); }
