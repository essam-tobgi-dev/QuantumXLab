...
void Renderer::beginFrame(int w, int h, const Camera& cam, double timeS) {
    ...
    time_ = timeS;
    opaque_.clear(); transparent_.clear(); instanced_.clear();
    lines_.clear(); linesNd_.clear(); text_.clear();
    // Overlay batches take world-space positions and make them camera-relative in double precision.
    lines_.setOrigin(cam_.position()); linesNd_.setOrigin(cam_.position()); text_.setOrigin(cam_.position());
    stats_ = FrameStats{};
}

void Renderer::submit(const Mesh& m, const Material& mat, const glm::mat4& model, ComponentId id, SubmitFlags f) {
    if (!m.valid()) return;
    // frustum cull on the transformed AABB (camera-relative)
    Aabb box = m.bounds().transformed(glm::dmat4(model));
    if (!cam_.frustum().intersects(box)) return;
...
void Renderer::submitInstanced(const Mesh& m, const Material& mat, std::span<const InstanceData> inst, SubmitFlags f) {
    if (!m.valid() || inst.empty()) return;
    instanced_.push_back({&m, mat, std::vector<InstanceData>(inst.begin(), inst.end()), f});
}
void Renderer::setSelection(ComponentId s, ComponentId h) { sel_.selected = glm::uvec4(s.value, h.value, 0, 0); }
...
void Renderer::setPointLights(std::span<const PointLight> pts) {
    ...
    for (int i = 0; i < n; ++i) lights_.points[i] = pts[static_cast<std::size_t>(i)];
...
void Renderer::shadowPass() {
    ...
    bool any = false;
    for (auto& it : opaque_) if (it.flags.castShadow) { scene.expand(it.mesh->bounds().transformed(glm::dmat4(it.model)).min); scene.expand(it.mesh->bounds().transformed(glm::dmat4(it.model)).max); any = true; }
    for (auto& ii : instanced_) if (ii.flags.castShadow) for (auto& d : ii.data) { Aabb b = ii.mesh->bounds().transformed(glm::dmat4(d.model)); scene.expand(b.min); scene.expand(b.max); any = true; }
    glm::mat4 lvp(1.0f);
    double r = 1.0;
    if (any) {
    ...
    f.params = glm::vec4(desc_.exposure, depthBias, 1.0f / desc_.shadowSize, cam_.ortho() ? 1.f : 0.f);
    ...
    if (any) {
        shadow_.use();
        for (auto& it : opaque_) {
            if (!it.flags.castShadow) continue;
            ObjectUbo o{}; o.model = glm::translate(glm::mat4(1.0f), -glm::vec3(cam_.position())) * it.model; o.normal = glm::mat4(1.0f); o.id = glm::uvec4(0);
            updateUbo(uboObject_, o);
            it.mesh->draw(); ++stats_.drawCalls;
        }
        shadowInst_.use();
        for (auto& ii : instanced_) {
            if (!ii.flags.castShadow) continue;
            ObjectUbo o{}; o.model = glm::translate(glm::mat4(1.0f), -glm::vec3(cam_.position())); o.normal = glm::mat4(1.0f);
            updateUbo(uboObject_, o);
            instBuf_.setData(ii.data, BufferUsage::Stream);
            const_cast<Mesh*>(ii.mesh)->bindInstanceBuffer(instBuf_);
            ii.mesh->drawInstanced(static_cast<int>(ii.data.size())); ++stats_.drawCalls;
        }
    }
    glCullFace(GL_BACK);
...
void Renderer::mainPass() {
    ...
    shadowDepth_.bind(5); shadowSampler_.bind(5);
    ...
    auto drawItem = [&](const Item& it) {
        ...
        updateUbo(uboObject_, o);
        if (!it.flags.depthTest) glDisable(GL_DEPTH_TEST);
    ...
    line_.use(); linesNd_.upload(); linesNd_.draw();
    if (font_) {
        textSdf_.use(); textSdf_.set("uAtlas", 0); font_->texture().bind(0);
        text_.upload(); text_.draw();
    }
    glDepthMask(GL_TRUE); glEnable(GL_DEPTH_TEST); glEnable(GL_CULL_FACE); glDisable(GL_BLEND);
    stats_.mainMs = t.ms();
}
...
void Renderer::idPass() {
    idFbo_.bind();
    glViewport(0, 0, w_, h_);
    glEnable(GL_DEPTH_TEST); glDepthFunc(GL_LEQUAL); glEnable(GL_CULL_FACE); glDisable(GL_BLEND);
    ...
    for (auto& it : opaque_) {
        ...
        if (unpickable && !desc_.ssao) continue;
        ObjectUbo o{}; o.model = camOff * it.model; o.normal = glm::mat4(1.0f); o.id = glm::uvec4(unpickable ? 0u : it.id.value, 0, 0, 0);
        updateUbo(uboObject_, o);
        if (!it.flags.depthTest) glDisable(GL_DEPTH_TEST);
        it.mesh->draw(); ++stats_.drawCalls;
        if (!it.flags.depthTest) glEnable(GL_DEPTH_TEST);
    ...
    idInst_.use();
    for (auto& ii : instanced_) {
        if (ii.flags.noPick && !desc_.ssao) continue;
        // bit1 of uObjectId.y tells id.frag to write 0 instead of the per-instance id
        ObjectUbo o{}; o.model = camOff; o.normal = glm::mat4(1.0f); o.id = glm::uvec4(0, ii.flags.noPick ? 2u : 0u, 0, 0);
        updateUbo(uboObject_, o);
        instBuf_.setData(ii.data, BufferUsage::Stream);
        const_cast<Mesh*>(ii.mesh)->bindInstanceBuffer(instBuf_);
        ii.mesh->drawInstanced(static_cast<int>(ii.data.size())); ++stats_.drawCalls;
...
void Renderer::resolveAndPost() {
    ...
    {
        const double seg[4] = {postQuery_[0].resultMs(), postQuery_[1].resultMs(), postQuery_[2].resultMs(), postQuery_[3].resultMs()};
        stats_.resolveMs = seg[0]; stats_.aoMs = seg[1]; stats_.bloomMs = seg[2]; stats_.postMs = seg[0] + seg[1] + seg[2] + seg[3];
    }
    postQuery_[0].begin();
    msaaFbo_.blitTo(resolveFbo_, 0, 0, w_, h_, GL_LINEAR, false);
    postQuery_[0].end();
    postQuery_[1].begin();
    if (desc_.ssao) ssaoPass();
    postQuery_[1].end();
    postQuery_[2].begin();
    if (desc_.bloom) bloomPass();
    postQuery_[2].end();
    postQuery_[3].begin();
    ...
    postProg_.use();
    resColor_.bind(0); resId_.bind(1); ssaoTex_.bind(2); bloomA_.bind(3);
    postProg_.set("uColor", 0); postProg_.set("uId", 1); postProg_.set("uAo", 2); postProg_.set("uBloom", 3);
    screenQuad_.draw(); ++stats_.drawCalls;
...
void Renderer::queuePick(int x, int y) { pickReq_ = PickRequest{true, x, y}; }

void Renderer::issuePick() {
    PickSlot& slot = pickSlot_[pickWrite_];
    slot.inFlight = false;
    if (pickReq_.queued && pickReq_.x >= 0 && pickReq_.y >= 0 && pickReq_.x < w_ && pickReq_.y < h_) {
        ...
        // With a pack buffer bound the pointer is an offset: id at 0, depth at 4. Neither call waits.
        glReadPixels(pickReq_.x, h_ - 1 - pickReq_.y, 1, 1, GL_RED_INTEGER, GL_UNSIGNED_INT, nullptr);
...
std::optional<Renderer::PickResult> Renderer::pollPick() {
    ...
    PickSlot& slot = pickSlot_[pickWrite_];
    if (!slot.inFlight) return std::nullopt;
    slot.inFlight = false;
    std::uint32_t id = 0;
    float depth = 1.0f;
    pickPbo_[pickWrite_].bind();
    ...
    r.x = slot.x;
    r.y = slot.y;
    if (r.hit) r.world = slot.cam.unproject(slot.x + 0.5, slot.y + 0.5, depth, slot.w, slot.h);
    return r;
}
...
ComponentId Renderer::pick(int x, int y) const {
    if (x < 0 || y < 0 || x >= w_ || y >= h_) return ComponentId{0};
    glBindFramebuffer(GL_READ_FRAMEBUFFER, idFbo_.id());
    glReadBuffer(GL_COLOR_ATTACHMENT0);
    std::uint32_t v = 0;
...
Status Renderer::screenshot(const std::filesystem::path& png) const {
    ...
    if (!ok) return fail(ErrorCode::Io, "cannot write " + png.string());
    return {};
}
} // namespace qlab::gfx
