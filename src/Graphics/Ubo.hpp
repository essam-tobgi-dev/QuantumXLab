...
constexpr unsigned kUboFrame = 0, kUboLights = 1, kUboMaterial = 2, kUboObject = 3,
                   kUboSelection = 4;
...
struct alignas(16) FrameUbo {
    ...
    glm::vec4 params; // x = exposure, y = shadow depth bias (NDC z), z = shadow texel (uv), w =
                      // ortho flag
    glm::vec4 shadow; // x = normal-offset (world m, 1.5 texels), y = Poisson radius (texels), z =
                      // texel world size, w unused
    glm::vec4
        post; // x = environment intensity, y = IBL flag, z = SSAO strength, w = bloom strength
};
