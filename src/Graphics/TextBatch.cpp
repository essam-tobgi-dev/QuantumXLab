...
namespace qlab::gfx {
void TextBatch::label3D(const glm::dvec3& w, std::string_view s, float px, const glm::vec4& c,
                        TextAnchor a, glm::vec2 off) {
    emit(glm::vec3(w - origin_), s, px, c, a, off, false); // world → origin-relative in double
}
void TextBatch::label2D(glm::vec2 p, std::string_view s, float px, const glm::vec4& c,
                        TextAnchor a) {
    emit(glm::vec3(p, 0), s, px, c, a, {0, 0}, true);
}
void TextBatch::emit(const glm::vec3& anchor, std::string_view s, float px, const glm::vec4& c,
                     TextAnchor a, glm::vec2 off, bool screen) {
    if (!font_)
        return;
    ...
    case TextAnchor::TopLeft:
        origin += glm::vec2(0, asc);
        break;
    case TextAnchor::Center:
        origin += glm::vec2(-size.x * 0.5f, (asc - desc) * 0.5f);
        break;
    case TextAnchor::BottomLeft:
        origin += glm::vec2(0, -desc);
        break;
    case TextAnchor::TopCenter:
        origin += glm::vec2(-size.x * 0.5f, asc);
        break;
    case TextAnchor::BottomCenter:
        origin += glm::vec2(-size.x * 0.5f, -desc);
        break;
    }
    float pen = 0;
