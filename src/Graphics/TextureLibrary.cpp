...
ImageRgba8 packOrm(const ImageRgba8& rough, const ImageRgba8& metal, const ImageRgba8& ao) {
    ...
    auto fetch = [](const ImageRgba8& m, int x, int y, int w2, int h2) -> std::uint8_t {
        ...
        return m.rgba[(static_cast<std::size_t>(my) * static_cast<std::size_t>(m.width) +
                       static_cast<std::size_t>(mx)) *
                      4];
    };
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            std::uint8_t* p = &out.rgba[(static_cast<std::size_t>(y) * static_cast<std::size_t>(w) +
...
TextureSet TextureSet::build(std::string name, const TextureSetSource& src, float maxAnisotropy) {
    ...
    s.orm =
        makeTexture(packOrm(src.roughness, src.metallic, src.ao), TexFormat::RGBA8, maxAnisotropy);
    if (s.hasAlbedo) {
        ...
        glm::dvec3 sum(0.0);
        const std::size_t n = static_cast<std::size_t>(src.albedo.width) *
                              static_cast<std::size_t>(src.albedo.height);
    ...
    if (s.hasRoughness) {
        double sum = 0.0;
        const std::size_t n = static_cast<std::size_t>(src.roughness.width) *
                              static_cast<std::size_t>(src.roughness.height);
        for (std::size_t i = 0; i < n; ++i)
            sum += src.roughness.rgba[i * 4] / 255.0;
...
Result<ImageRgba8> TextureLibrary::loadImage(const std::filesystem::path& file) {
    ...
    if (!px)
        return fail(ErrorCode::Io, "cannot decode " + file.string() + ": " +
    ...
    auto read = [&](const char* map, ImageRgba8& into) {
        for (const char* ext : {".jpg", ".png", ".jpeg"}) {
            ...
            if (img)
                ...
                QXL_LOG_WARN(Gfx, "texture {}: {}", file.string(), img.error().message);
            return;
        }
    };
...
bool TextureLibrary::existsOnDisk(std::string_view set) const {
    ...
    return std::filesystem::exists(dir / "albedo.jpg") ||
           std::filesystem::exists(dir / "albedo.png");
}
...
const TextureSet& TextureLibrary::get(std::string_view set) {
    ...
    if (std::filesystem::is_directory(dir))
        ...
        QXL_LOG_WARN(Gfx, "texture set '{}' not found under {}; using neutral maps", set,
                     root_.string());
    const TextureSet& ref = add(std::string(set), src);
    QXL_LOG_INFO(Gfx,
                 "texture set '{}': albedo {} normal {} roughness {} metallic {} ao {} ({}x{})",
                 set, ref.hasAlbedo, ref.hasNormal, ref.hasRoughness, ref.hasMetallic, ref.hasAo,
                 ref.albedo.width(), ref.albedo.height());
    return ref;
}
