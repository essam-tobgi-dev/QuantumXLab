...
// endFrame runs: shadow → PBR (MSAA RGBA16F) → transparent → resolve → id pass (R32UI,
// single-sample) → post (outline+tonemap).
...
struct FrameStats {
    ...
    double shadowMs = 0, mainMs = 0, postMs = 0, aoMs = 0, bloomMs = 0, resolveMs = 0;
};
...
class Renderer {
  public:
    ...
    void submit(const Mesh& m, const Material& mat, const glm::mat4& model,
                ComponentId id = ComponentId{0}, SubmitFlags f = {});
    void submitInstanced(const Mesh& m, const Material& mat, std::span<const InstanceData> inst,
                         SubmitFlags f = {});
    LineBatch& lines() {
        return lines_;
    } // WORLD-space overlay lines (depth tested); call after beginFrame
    ...
    void setEnvironmentIntensity(float k) {
        envIntensity_ = k;
    } // multiplier on the IBL (default 1)
    ...
    const Texture2D& hdrTexture() const {
        return resColor_;
    } // resolved linear RGBA16F, before post
    ...
    // IBL resources (read-only; tests read them back). Generated once in init (spec 18 §5
    // EnvPrefilter).
  ...
  private:
    ...
    void shadowPass();
    void mainPass();
    void idPass(); // GL_MAX_INTEGER_SAMPLES is 1 on macOS: ids are drawn in a separate
                   // single-sample pass
    void resolveAndPost();
    ...
    Status
    buildEnvironment(); // RendererEnv.cpp: env cubemap, prefiltered mips, irradiance, BRDF LUT
    ...
    double time_ = 0;
    // GL resources
    ShaderProgram pbr_, pbrInst_, pbrTex_, pbrTexInst_, shadow_, shadowInst_, idProg_, idInst_,
        line_, textSdf_, postProg_, outline_;
    ShaderProgram envLab_, envPrefilter_, envIrrProg_, brdfLutProg_, ssaoProg_, ssaoBlur_,
        ssaoUpsample_, bloomBright_, bloomBlur_;
    ...
    TimerQuery
        postQuery_[4]; // consecutive GL_TIME_ELAPSED segments: resolve, SSAO, bloom, composite
