...
std::optional<BindingValue> stateValue(const LiveState& s, std::uint32_t qubit,
    ...
    if (head.name == "bloch") {
        ...
        if (!head.index)
            return simOnly(r->bloch.norm(), {}, cls);
        if (*head.index > 2)
            return std::nullopt;
        return simOnly(v[*head.index], {}, cls);
    }
    if (rest == "purity")
        return simOnly(r->purity, {}, cls);
    if (rest == "leakage")
        return simOnly(r->leakage, {}, cls);
    if (rest == "entropy")
        return simOnly(r->entropyBits, "bit", cls);
    if (rest == "pop_e")
        return simOnly(r->bloch.p1(), {}, cls);
    return std::nullopt;
}
...
std::optional<BindingValue> qubitValue(const LiveState& s, std::uint32_t q, std::string_view rest) {
    ...
    if (auto v = stateValue(s, q, rest))
        return v;
    if (cal == nullptr)
        return std::nullopt;
    if (rest == "f01")
        return num(cal->f01.value.v, "Hz", FidelityClass::Numerical);
    if (rest == "alpha")
        return num(cal->anharmonicity.value.v, "Hz", FidelityClass::Numerical);
    if (rest == "T1")
        return num(cal->t1.value.v, "s", FidelityClass::Numerical);
    if (rest == "T2")
        return num(cal->t2echo.value.v, "s", FidelityClass::Numerical);
    if (rest == "T2star")
        return num(cal->t2star.value.v, "s", FidelityClass::Numerical);
    // Spec 00 §6: without the state, the excited population is the calibrated thermal one.
    if (rest == "pop_e")
        return num(cal->thermalPopulation.value, {}, FidelityClass::Numerical);
    if (rest == "EC" || rest == "EJ") {
    ...
    if (rest == "readout_fidelity")
        return num(cal->readoutFidelity(), {}, FidelityClass::Numerical);
...
std::optional<BindingValue> resonatorValue(const LiveState& s, std::uint32_t q,
                                           std::string_view rest) {
    const hw::QubitCal* cal = s.calibration ? s.calibration->qubit(q) : nullptr;
    if (rest == "f_r") {
    ...
    if (cal == nullptr)
        return std::nullopt;
    if (rest == "chi" && cal->readoutChi)
        return num(cal->readoutChi->value.v, "Hz", FidelityClass::Numerical);
    if (rest == "kappa" && cal->readoutKappa)
        return num(cal->readoutKappa->value.v, "Hz", FidelityClass::Numerical);
    if (rest == "n_photons") {
        // T05 (6.5): the steady-state occupation of the calibrated tone, zero while no tone is on.
        if (!s.readoutActive || !s.environment)
            return num(0.0, {}, FidelityClass::Model);
...
std::optional<BindingValue> edgeValue(const LiveState& s, std::uint32_t e, std::string_view rest) {
    if (!s.device || !s.calibration || e >= s.device->edges.size())
        return std::nullopt;
    ...
    if (cal == nullptr)
        return std::nullopt;
    if (rest == "zz")
        return num(cal->zz.value.v, "Hz", FidelityClass::Numerical);
    ...
    if (rest == "J")
        return num(cal->couplingG.value.v, "Hz", FidelityClass::Numerical);
    if (rest == "g_eff")
        return num(cal->couplingG.value.v, "Hz", FidelityClass::Model);
    if (rest == "error")
        return num(cal->gateError2q.value, {}, FidelityClass::Numerical);
    if (rest == "duration")
        return num(cal->duration.value.v, "s", FidelityClass::Numerical);
    if (rest == "f_c") {
        if (!edge.coupler)
            return std::nullopt;
        const hw::QubitCal* c = s.calibration->qubit(*edge.coupler);
        return c ? std::optional(num(c->f01.value.v, "Hz", FidelityClass::Numerical))
                 : std::nullopt;
    }
    return std::nullopt;
}
...
lab::BindingRegistry::Provider deviceProvider(std::shared_ptr<const LiveState> state) {
    return [state](std::string_view path) -> std::optional<BindingValue> {
        ...
        if (path == "T_sample") {
            ...
            return num(T[static_cast<std::size_t>(cryo::stageIndex(cryo::Stage::MXC))], "K",
                       FidelityClass::Numerical);
        }
        if (head.name == "qubit" && head.index)
            return qubitValue(s, *head.index, head.rest);
        if (head.name == "res" && head.index)
            return resonatorValue(s, *head.index, head.rest);
        if (head.name == "edge" && head.index)
            return edgeValue(s, *head.index, head.rest);
        if (head.name == "pf" &&
            head.index) { // Purcell filter, one per feedline, at the band centre
            auto f = s.resonatorHz(*head.index);
            if (head.rest == "f" && f)
                return num(*f, "Hz", FidelityClass::Model);
            return std::nullopt;
        }
        ...
        if (head.name == "trap" && head.rest == "omega_z") {
            if (!s.device || !s.device->motionalModes)
                return std::nullopt;
...
std::optional<lab::BindingValue> packetBinding(const LiveState& state, std::string_view path) {
    const PathHead head = splitPath(path);
    if (head.name != "line" || !head.index)
        return std::nullopt;
    const cryo::WiringLine* line = state.line(*head.index);
    if (line == nullptr || !state.running)
        return std::nullopt;
    ...
    if (path.ends_with(".t_s"))
        return BindingValue::number(t, "s", FidelityClass::Illustrative);
    ...
    if (path.ends_with(".sigma_s"))
        return BindingValue::number(0.2 * period, "s", FidelityClass::Illustrative);
    return std::nullopt;
}
