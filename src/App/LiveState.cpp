...
PathHead splitPath(std::string_view path) {
    ...
    if (bracket != std::string_view::npos && (dot == std::string_view::npos || bracket < dot)) {
        ...
        std::uint32_t v = 0;
        if (std::from_chars(path.data() + bracket + 1, path.data() + close, v).ec == std::errc{})
            h.index = v;
...
bool stageFromAnyName(std::string_view name, cryo::Stage& out) {
    ...
    if (cryo::stageFromName(upper, out))
        return true;
...
const cryo::WiringLine* LiveState::line(std::size_t index) const {
    if (wiring == nullptr || index >= wiring->lines.size())
        return nullptr;
...
lab::BindingRegistry::Provider instrProvider(std::shared_ptr<const LiveState> state) {
    return [state](std::string_view path) -> std::optional<lab::BindingValue> {
        if (state->registry == nullptr)
            return std::nullopt;
        // Spec 12 §12: the probes are Simulator-only and the registry hides them in Physical-lab
        // mode.
...
lab::BindingRegistry::Provider runProvider(std::shared_ptr<const LiveState> state) {
    return [state](std::string_view path) -> std::optional<lab::BindingValue> {
        ...
        if (path == "running")
            return number(state->running ? 1.0 : 0.0, {}, FidelityClass::Exact);
        if (path == "shot")
            return number(static_cast<double>(state->shotsDone), {}, FidelityClass::Exact);
        if (path == "shots")
            return number(static_cast<double>(state->shotsTotal), {}, FidelityClass::Exact);
        if (path == "progress")
            return number(state->shotsTotal ? static_cast<double>(state->shotsDone) /
                          ...
                          {}, FidelityClass::Exact);
        if (path == "gate_cursor")
            return number(static_cast<double>(state->playheadGate), {}, FidelityClass::Exact);
        if (path == "playhead")
            return number(state->playheadS, "s", FidelityClass::Exact);
        if (path == "wall_time")
            return number(state->runWallTimeS, "s", FidelityClass::Model);
        if (path == "seed")
            return number(static_cast<double>(state->seed), {}, FidelityClass::Exact);
        ...
        if (path.starts_with("schedule.line["))
            return detail::packetBinding(*state, path.substr(9));
        return std::nullopt;
    };
}
