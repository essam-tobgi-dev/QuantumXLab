...
runtime::BackendChoice fromPragma(lang::BackendChoice c) {
    switch (c) {
    case lang::BackendChoice::StateVector: return runtime::BackendChoice::StateVector;
    case lang::BackendChoice::DensityMatrix: return runtime::BackendChoice::DensityMatrix;
    case lang::BackendChoice::Stabilizer: return runtime::BackendChoice::Stabilizer;
    case lang::BackendChoice::Lindblad: return runtime::BackendChoice::Lindblad;
    case lang::BackendChoice::Auto: break;
    }
...
runtime::NoiseSource fromPragma(lang::NoiseChoice c) {
    switch (c) {
    case lang::NoiseChoice::Ideal: return runtime::NoiseSource::Ideal;
    case lang::NoiseChoice::Custom: return runtime::NoiseSource::Custom;
    case lang::NoiseChoice::Calibrated: break;
    }
    return runtime::NoiseSource::Calibrated;
}

runtime::SnapshotCadence cadenceFromName(std::string_view name) {
    if (name == "none") return runtime::SnapshotCadence::None;
    if (name == "layer") return runtime::SnapshotCadence::Layer;
    if (name == "barrier") return runtime::SnapshotCadence::Barrier;
    if (name == "end") return runtime::SnapshotCadence::End;
...
const lang::Diagnostic* firstError(const std::vector<lang::Diagnostic>& diagnostics) {
    for (const lang::Diagnostic& d : diagnostics)
        if (d.severity == lang::Severity::Error) return &d;
    return nullptr;
}
...
runtime::RunOptions runOptionsFor(const lang::Program& program, const Options& o) {
    ...
    options.shots = o.shotsGiven ? o.shots
                                 : (p.shots ? static_cast<std::uint32_t>(*p.shots) : o.shots);
    ...
    if (o.backendGiven && o.backend != runtime::BackendChoice::Auto) options.backend = o.backend;
    else if (pragmaBackend != runtime::BackendChoice::Auto) options.backend = pragmaBackend;
...
Result<ProgramRun> compileAndRun(runtime::Session& session, std::string source, std::filesystem::path origin,
                                 const Options& o) {
    if (session.device() == nullptr) return fail(ErrorCode::NotFound, "no device is selected");
    QXL_TRY_ASSIGN(const runtime::ProgramId id, session.loadProgram(std::move(source), std::move(origin)));
    const lang::Program* program = session.program(id);
    if (program == nullptr) return fail(ErrorCode::Internal, "the session lost the program it just loaded");
    ...
    if (result == nullptr) return fail(ErrorCode::Internal, "the run produced no result");
    out.result = *result;
    for (const lang::Diagnostic& d : out.result.diagnostics) out.diagnostics.push_back(d);
    return out;
}
