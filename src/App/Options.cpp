...
std::string versionText() {
    return std::format("quantumxlab {}", core::version());
}
    ...
    for (std::size_t i = 0; i < args.size(); ++i) {
        ...
        auto value = [&](std::string_view& out) -> Status {
            if (inlineValue) {
                out = a.substr(eq + 1);
                if (out.empty())
                    return badArg(name, "empty value");
                return {};
            }
            if (i + 1 >= args.size())
                return badArg(name, "expects a value");
            out = args[++i];
            return {};
        };
        ...
        if (name == "--run") {
            ...
            std::string_view v;
            QXL_TRY(value(v));
            if (!parseInteger(v, o.shots) || o.shots == 0)
                return badArg(name, std::format("'{}' is not a shot count", v));
            o.shotsGiven = true;
        } else if (name == "--seed") {
            std::string_view v;
            QXL_TRY(value(v));
            if (!parseInteger(v, o.seed))
                return badArg(name, std::format("'{}' is not a seed", v));
            o.seedGiven = true;
            ...
            if (sawPositional)
                return badArg(a, "only one project file may be given");
            sawPositional = true;
            o.project = a;
        }
    }
    if (o.mode == Mode::Run && o.program.empty())
        return badArg("--run", "expects a program path");
    if (o.mode == Mode::SelfTest && o.outDir.empty())
        return badArg("--selftest", "expects an output directory");
    if (o.layout.empty())
        o.layout = defaultLayoutFor(o.device);
    return o;
}
