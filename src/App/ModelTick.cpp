...
const runtime::RunSnapshot* snapshotAt(const runtime::RunResult& r, std::uint64_t gate) {
    ...
    for (const runtime::RunSnapshot& s : r.snapshots)
        if (s.gateIndex <= gate && (best == nullptr || s.gateIndex >= best->gateIndex)) best = &s;
    if (best == nullptr && !r.snapshots.empty()) best = &r.snapshots.front();
    return best;
}
...
std::uint64_t openingGate(const runtime::RunResult& r) {
    ...
    for (const runtime::RunSnapshot& s : r.snapshots)
        if (unmeasured(s) && (!best || s.gateIndex > *best)) best = s.gateIndex;
    if (best) return *best;
...
viz::math::MeasurementMap measurementMap(const runtime::RunResult& r) {
    ...
    for (const runtime::MeasuredBit& m : r.measurements)
        if (m.qubit < n && m.bit != ir::kNoBit) map.bitOfQubit[m.qubit] = static_cast<std::int32_t>(m.bit.index);
    return map;
}

std::shared_ptr<const viz::LindbladSeries> lindbladSeries(const std::shared_ptr<const runtime::RunResult>& r) {
    if (!r || r->lindblad.empty()) return nullptr;
    auto series = std::make_shared<viz::LindbladSeries>();
    series->levels = r->levels;
    series->sites = r->levels > 0 && !r->lindblad.front().populations.empty()
...
// Defined here because `ReductionJob` must be complete wherever a `LabModel` is created or destroyed.
LabModel::LabModel()
...
void LabModel::requestReductions(const viz::ReductionRequest& request) {
    if (request.empty() || !live_state_->snapshot) return;
    if (reduction_ != nullptr) return;   // one reduction at a time; the next frame asks again
    ...
    if (!live_state_->physicalLab && lab_ != nullptr && !lab_->scene.qubitNodes().empty()) merged.singles = true;
...
void LabModel::collectReductions() {
    if (reduction_ == nullptr || !reduction_->payload->done.load(std::memory_order_acquire)) return;
    if (reduction_->payload->result) live_state_->reductions = reduction_->payload->result;
    reduction_.reset();
}

void LabModel::waitReductions() {
    if (reduction_ == nullptr) return;
    if (reduction_->future.valid()) reduction_->future.wait();
    collectReductions();
}

void LabModel::tickLab(double dtSeconds, gfx::Camera& camera) {
    if (tour_ != nullptr && tour_->playing()) tour_->update(dtSeconds, camera);
    else if (lab_ != nullptr) lab_->interaction.update(dtSeconds, camera);
}
...
void LabModel::setResult(std::shared_ptr<const runtime::RunResult> r) {
    ...
    if (!live_state_->snapshot) live_state_->snapshot = run->finalState;
}
...
void LabModel::applyLinePowers(const cryo::LinePowers& powers) {
    ...
    if (cryo_ != nullptr) cryo_->network.linePowers = powers;
}
...
void LabModel::publishInputs() {
    ...
    if (run != nullptr) {
        ...
        if (view.nQubits == 0) view.nQubits = static_cast<std::uint32_t>(run->qubits.size());
...
void LabModel::refreshViewInput() {
    ...
    if (const std::shared_ptr<const compiler::CompiledProgram>& c = s.compiled; c) {
        ...
        in.circuits.stages[static_cast<std::size_t>(c->timing.empty() ? viz::CircuitStage::Routed
                                                                      : viz::CircuitStage::Scheduled)] =
    ...
    if (const std::shared_ptr<const runtime::RunResult>& r = s.result; r) {
        ...
        if (r->exact) in.idealProbabilities = std::shared_ptr<const std::vector<double>>(r, &*r->exact);
        in.idealClass = r->exactClass;
        in.measurement = std::make_shared<const viz::math::MeasurementMap>(measurementMap(*r));
        if (r->circuit) in.circuits.stages[static_cast<std::size_t>(viz::CircuitStage::Scheduled)] = r->circuit;
        if (r->source) in.circuits.stages[static_cast<std::size_t>(viz::CircuitStage::Source)] = r->source;
        if (!r->initialLayout.empty()) in.circuits.layout = r->initialLayout.v2p;
        if (!r->finalLayout.empty()) in.circuits.finalLayout = r->finalLayout.v2p;
        in.lindblad = lindbladSeries(r);
        if (!in.schedule) in.schedule = r->schedule;
    }
...
void LabModel::recordSamples() {
    ...
    (void)recorder_.push(mxcLoadChannel_, s.thermal.time_s,
...
void LabModel::tick(double dtSeconds) {
    ...
    if (cryo_ != nullptr) {
        ...
        if (fridgeSteps > 0) recordSamples();
    }
    collectReductions();
    publishInputs();
    if (s.registry != nullptr) live_.tick(s.labTimeS);
    refreshViewInput();
    if (lab_ != nullptr) lab_->overlays.update(s.labTimeS);
}
