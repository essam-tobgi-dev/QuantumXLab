...
double linear(double dB) { return std::pow(10.0, dB / 10.0); }
...
double elementLossDb(const LiveState& s, const cryo::Element& e, double fHz, double T_K) {
    ...
    case cryo::ElementKind::RcFilter: return e.attenuation_dB;
    case cryo::ElementKind::CoaxSegment:
        if (s.coax != nullptr)
            if (const cryo::CoaxSpec* c = s.coax->find(e.coax)) return cryo::coaxLoss_dB(*c, e.length_m, fHz, T_K);
        return 0.0;
    case cryo::ElementKind::Amplifier:
    case cryo::ElementKind::Preamp: return -e.gain_dB;
    default: return 0.0;
    }
}
...
Found findElement(const LiveState& s, const cryo::WiringLine& line, cryo::ElementKind kind, std::uint32_t nth,
                  const std::string* variant, double fHz) {
...
std::optional<double> attenuatorPhotons(const LiveState& s, std::size_t lineIndex, std::uint32_t nth, double fHz) {
    ...
    for (const cryo::PhotonStep& step : noise->steps) {
        if (step.A_dB <= 0.0) continue;  // a clamp applies no attenuation
        if (seen++ == nth) return step.n_out;
    }
    return std::nullopt;
}

std::optional<BindingValue> lineValue(const LiveState& s, std::size_t lineIndex, std::string_view path) {
    ...
    if (head.name == "attn") {
        const Found f = findElement(s, *line, cryo::ElementKind::Attenuator, head.index.value_or(0), nullptr, fHz);
        if (f.element == nullptr) return std::nullopt;
        if (head.rest == "P_diss") // spec 11 §2.3: P_in (1 − 10^(−A/10)) is dissipated at the stage
            return num(f.powerIn_W * (1.0 - linear(-f.element->attenuation_dB)), "W", FidelityClass::Model);
        if (head.rest == "n_th") {
        ...
        if (head.rest == "T") return num(stageOf(*f.element), "K", FidelityClass::Numerical);
        return std::nullopt;
    }
    if (head.name == "irf" && head.rest == "P_diss") {
        const Found f = findElement(s, *line, cryo::ElementKind::IrFilter, head.index.value_or(0), nullptr, fHz);
        if (f.element == nullptr) return std::nullopt;
        return num(f.powerIn_W * (1.0 - linear(-f.element->attenuation_dB)), "W", FidelityClass::Model);
    }
    if (head.name == "iso") {
        const Found f = findElement(s, *line, cryo::ElementKind::Isolator, head.index.value_or(0), nullptr, fHz);
        if (f.element == nullptr) return std::nullopt;
        if (head.rest == "T") return num(stageOf(*f.element), "K", FidelityClass::Numerical);
        if (head.rest == "isolation") return num(f.element->isolation_dB, "dB", FidelityClass::Model);
        return std::nullopt;
    }
    if (head.name == "clamp" && head.rest == "T") {
        cryo::Stage stage = cryo::Stage::MXC;
        if (!head.key.empty() && !stageFromAnyName(head.key, stage)) return std::nullopt;
        return num(T[static_cast<std::size_t>(cryo::stageIndex(stage))], "K", FidelityClass::Numerical);
    }
    if (head.name == "seg") {
        const Found f = findElement(s, *line, cryo::ElementKind::CoaxSegment, head.index.value_or(0), nullptr, fHz);
        if (f.element == nullptr) return std::nullopt;
        const double cold = stageOf(*f.element);
        const double hot = T[static_cast<std::size_t>(cryo::stageIndex(cryo::warmerStage(f.element->stage)))];
        if (head.rest == "T_cold") return num(cold, "K", FidelityClass::Numerical);
        if (head.rest == "T_hot") return num(hot, "K", FidelityClass::Numerical);
        if (head.rest == "P_cond") {
            if (s.coax == nullptr || s.materials == nullptr) return std::nullopt;
    ...
    if (head.name == "hemt" || head.name == "rtamp") {
        ...
        std::uint32_t nth = 0;
        for (const cryo::Element& e : line->elements) {
            if (e.kind != cryo::ElementKind::Amplifier) continue;
            const bool isWarm = e.stage == cryo::Stage::RT;
            if (isWarm != warm) continue;
            (void)nth;
            if (head.rest == "P_diss") return num(e.dissipation_W, "W", FidelityClass::Model);
            if (head.rest == "T_N") return num(e.noiseTemperature_K, "K", FidelityClass::Model);
            if (head.rest == "gain") return num(e.gain_dB, "dB", FidelityClass::Model);
            if (head.rest == "T")
                return num(temps[static_cast<std::size_t>(cryo::stageIndex(e.stage))], "K", FidelityClass::Numerical);
            return std::nullopt;
        }
        return std::nullopt;
    }
    if (head.name == "jpa" || head.name == "twpa") {
        ...
        if (f.element == nullptr) return std::nullopt;
        if (head.rest == "gain") return num(f.element->gain_dB, "dB", FidelityClass::Model);
        if (head.rest == "T_N") return num(f.element->noiseTemperature_K, "K", FidelityClass::Model);
        if (head.rest == "pump_on") return num(f.element->dissipation_W > 0.0 ? 1.0 : 0.0, {}, FidelityClass::Model);
        if (head.rest == "P_diss") return num(f.element->dissipation_W, "W", FidelityClass::Model);
        return std::nullopt;
    }
    return std::nullopt;
}
...
std::optional<BindingValue> wiringTotal(const LiveState& s, std::string_view path) {
    if (s.wiring == nullptr) return std::nullopt;
    double sum = 0.0;
    bool any = false;
    if (path == "hemt.P_total" || path == "pump.P") {
        const cryo::ElementKind kind = path[0] == 'h' ? cryo::ElementKind::Amplifier : cryo::ElementKind::Preamp;
    ...
    if (path == "dc.P_cond") { // conduction the DC looms bring to the mixing chamber
        if (s.heat == nullptr) return std::nullopt;
        const cryo::StageArray T = s.stageTemperatures();
        for (const cryo::WiringLine& l : s.wiring->lines) {
            if (l.kind != cryo::LineKind::DC) continue;
...
double lineInputPower(const LiveState& state, const cryo::WiringLine& line) {
    ...
    if (it != state.linePowers.end()) return it->second;
...
lab::BindingRegistry::Provider wiringProvider(std::shared_ptr<const LiveState> state) {
    return [state](std::string_view path) -> std::optional<BindingValue> {
        const PathHead head = splitPath(path);
        if (head.name == "line") {
            if (!head.index) return std::nullopt;
