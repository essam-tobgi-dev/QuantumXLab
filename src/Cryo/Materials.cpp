...
double debyeD(double x) {
    if (x <= 0) return 1.0;
    if (x > 200.0) return 4.0 * std::pow(std::numbers::pi, 4) / (5.0 * x * x * x); // T^3 law
    const int n = 200;
    double h = x / n, s = 0.0;
    auto f = [](double t) {
        if (t < 1e-6) return t * t;
        double e = std::exp(t);
        double d = e - 1.0;
...
double Material::specificHeat(double T) const {
    if (cp.model == "table" && !cp.table.empty()) {
        if (T <= cp.table.front().first) return cp.table.front().second * std::pow(T / cp.table.front().first, 3);
        if (T >= cp.table.back().first) return cp.table.back().second;
...
double Material::enthalpy(double Tlo, double Thi) const {
    if (Thi <= Tlo) return 0.0;
    const int n = 400;
    double h = (Thi - Tlo) / n, s = 0.0;
    for (int i = 0; i < n; ++i) s += specificHeat(Tlo + (i + 0.5) * h) * h;
    return s;
}
...
Result<Material> MaterialCatalog::parse(const std::string& text) {
    ...
    Material m;
    m.id = d.value("id", "");
    if (m.id.empty()) return fail(ErrorCode::Cryo_ + 1, "material without id");
    ...
    if (d.contains("superconducting")) {
        ...
        if (sc.is_boolean()) { m.superconducting = sc.get<bool>(); m.Tc_K = 9.2; }
        else if (sc.is_object()) { m.superconducting = true; m.Tc_K = sc.value("Tc_K", 9.2); }
        else if (sc.is_number()) { m.superconducting = true; m.Tc_K = sc.get<double>(); }
    }
    return m;
}
...
Result<MaterialCatalog> MaterialCatalog::load(const std::filesystem::path& dirIn) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(dir, ec)) {
        ...
        auto m = parse(*text);
        if (!m) { m.error().notes.push_back("file: " + entry.path().string()); return std::unexpected(m.error()); }
        cat.add(std::move(*m));
    }
    if (cat.size() == 0) return fail(ErrorCode::Cryo_ + 3, "no materials in " + dir.string());
    return cat;
}
