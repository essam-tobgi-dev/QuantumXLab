...
template <std::size_t N> double interpolate(const std::array<CapacityPoint, N>& tab, double T) {
    if (T <= tab.front().first)
        return tab.front().second;
    if (T >= tab.back().first)
...
template <std::size_t N> double slope(const std::array<CapacityPoint, N>& tab, double T) {
    if (T >= tab.back().first)
        return 0.0;
    for (std::size_t i = 1; i < N; ++i)
        if (T <= tab[i].first)
...
template <std::size_t N> double invert(const std::array<CapacityPoint, N>& tab, double load) {
    if (load <= tab.front().second)
        return tab.front().first;
    for (std::size_t i = 1; i < N; ++i)
...
std::string_view stageLongName(Stage s) {
    switch (s) {
    case Stage::RT:
        return "Room temperature (top plate)";
    case Stage::PT1:
        return "50 K plate (pulse tube 1st stage)";
    case Stage::PT2:
        return "4 K plate (pulse tube 2nd stage)";
    case Stage::STILL:
        return "Still";
    case Stage::CP:
        return "Cold plate";
    case Stage::MXC:
        return "Mixing chamber";
    }
    return "?";
}
...
double coolingPower(Stage s, double T, const CoolingParams& p) {
    ...
    case Stage::PT1:
        return p.pulseTubeOn ? (T <= 45.0 ? std::max(0.0, 40.0 * (T - 30.0) / 15.0)
                                          : interpolate(kPt1Cooldown, T))
                             : 0.0;
    case Stage::PT2:
        return p.pulseTubeOn ? (T <= 4.2 ? std::max(0.0, 1.5 * (T - 2.8) / 1.4)
                                         : interpolate(kPt2Cooldown, T))
                             : 0.0;
    case Stage::STILL: {
        if (!p.circulating)
            return 0.0;
    ...
    case Stage::CP:
        return p.circulating ? 0.35 * p.n3_mol_s * 84.0 * T * T : 0.0;
    case Stage::MXC:
        return p.circulating ? 84.0 * p.n3_mol_s * T * T - p.Q0_W : 0.0;
    }
    return 0.0;
}
...
double coolingPowerDerivative(Stage s, double T, const CoolingParams& p) {
    ...
    case Stage::RT:
        return 0.0;
    case Stage::PT1:
        return p.pulseTubeOn ? (T <= 45.0 ? (T > 30.0 ? 40.0 / 15.0 : 0.0) : slope(kPt1Cooldown, T))
                             : 0.0;
    case Stage::PT2:
        return p.pulseTubeOn ? (T <= 4.2 ? (T > 2.8 ? 1.5 / 1.4 : 0.0) : slope(kPt2Cooldown, T))
                             : 0.0;
    case Stage::STILL:
        return p.circulating
                   ? std::max(0.0, (p.n3_mol_s * phys::kHe3LatentHeat_Jmol - p.filmBurden_W) / 0.85)
                   : 0.0;
    case Stage::CP:
        return p.circulating ? 2.0 * 0.35 * p.n3_mol_s * 84.0 * T : 0.0;
    case Stage::MXC:
        return p.circulating ? 2.0 * 84.0 * p.n3_mol_s * T : 0.0;
    }
    return 0.0;
}
...
double temperatureForLoad(Stage s, double load, const CoolingParams& p) {
    ...
    switch (s) {
    case Stage::RT:
        return nominalTemperature(Stage::RT);
    case Stage::PT1:
        return p.pulseTubeOn
                   ? (load <= 40.0 ? 30.0 + 15.0 * load / 40.0 : invert(kPt1Cooldown, load))
                   : 293.0;
    case Stage::PT2:
        return p.pulseTubeOn ? (load <= 1.5 ? 2.8 + 1.4 * load / 1.5 : invert(kPt2Cooldown, load))
                             : 293.0;
    case Stage::STILL: {
        double base = p.n3_mol_s * phys::kHe3LatentHeat_Jmol - p.filmBurden_W;
        if (!p.circulating || base <= 0)
            return 4.0;
