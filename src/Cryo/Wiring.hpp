...
struct ChainCatalog {
    ...
    static Result<WiringLine>
    instantiate(std::string_view chainId, std::string lineId, std::string channel,
...
// JSON (kind "wiring"; the older "qlab.wiring" is still read): {device, layout, lengths_m?,
// lines:[{id, channel, chain, attenuation_db?: {STAGE: dB}, preamp?, hemt?: {gain_db, noise_k,
// dissipation_mw}}]}
