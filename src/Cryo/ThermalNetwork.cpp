...
ThermalNetwork::ThermalNetwork(const Wiring& w, const HeatLoadModel& l, const MaterialCatalog& m)
    ...
    for (Stage s : kStages) T_[stageIndex(s)] = nominalTemperature(s);
}

void ThermalNetwork::warmToRoomTemperature() {
    for (auto& t : T_) t = 293.0;
    time_s_ = 0;
}
...
double ThermalNetwork::heatCapacity(Stage s, double T) const {
    int i = stageIndex(s);
    double C = 0;
    if (const Material* cu = mats_.find("Cu_OFHC_RRR100")) C += masses.copper_kg[i] * cu->specificHeat(T);
    else C += masses.copper_kg[i] * 385.0 * std::min(1.0, std::pow(T / 300.0, 3.0)); // fallback
    if (const Material* ss = mats_.find("stainless_304")) C += masses.steel_kg[i] * ss->specificHeat(T);
    else C += masses.steel_kg[i] * 480.0 * std::min(1.0, std::pow(T / 300.0, 3.0));
...
Result<ThermalSnapshot> ThermalNetwork::evaluate(const StageArray& T, bool steady) const {
    ...
    if (cooling.pulseTubeOn) {
        if (s.load_W[1] > 40.0) s.warnings.push_back(std::format("PT1 load {:.1f} W exceeds 40 W capacity", s.load_W[1]));
        if (s.load_W[2] > 1.5) s.warnings.push_back(std::format("PT2 load {:.2f} W exceeds 1.5 W at 4.2 K", s.load_W[2]));
    }
    if (T[5] > 0.05 && cooling.circulating) s.warnings.push_back(std::format("MXC at {:.1f} mK: load {:.1f} µW", T[5] * 1e3, s.load_W[5] * 1e6));
    return s;
}
...
Result<ThermalSnapshot> ThermalNetwork::steadyState(int maxIter, double tol) const {
    ...
    if (!cooling.pulseTubeOn) { for (auto& t : T) t = 293.0; return evaluate(T, true); }
    for (int it = 0; it < maxIter; ++it) {
        ...
        for (int i = 1; i < kStageCount; ++i) {
            ...
            if (!cooling.circulating && i >= 3) Tnew = std::max(Tnew, T[2]); // no dilution: hangs at PT2
        ...
        if (maxDelta < tol) return evaluate(T, true);
    }
    auto s = evaluate(T, false);
    if (s) s->warnings.push_back("steady-state iteration did not converge");
    return s;
}
    ...
    while (remaining > 0) {
        ...
        for (int i = 1; i < kStageCount; ++i) {
            ...
            for (int it = 0; it < 30; ++it) {
                ...
                if (std::isinf(q)) { T = Told; break; }
                double g = C * (T - Told) / h - (load - q);
                double dg = C / h + coolingPowerDerivative(st, T, cooling);
                double Tn = T - g / dg;
                if (Tn < 0.5 * T) Tn = 0.5 * T;
                if (Tn > 2.0 * T) Tn = 2.0 * T;
                if (std::abs(Tn - T) < 1e-9 * std::max(T, 1e-3)) { T = Tn; break; }
                T = Tn;
            }
            // A stage cannot cool below the temperature its law reaches at this load.
            double floorT = temperatureForLoad(st, load, cooling);
            if (T < Told && T < floorT) T = std::min(Told, floorT);
            T = std::clamp(T, 0.005, 300.0);
            if (!cooling.circulating && i >= 3) {
                // No dilution cooling: the still, cold plate and mixing chamber are pre-cooled through
                // the heat switches / exchanger gas toward the PT2 plate with a ~2 h time constant.
