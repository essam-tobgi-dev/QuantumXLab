...
GasHandlingSystem::GasHandlingSystem() {
    ...
    snap_.valves = {{"v_ovc_pump", false},
                    {"v_still_pump", false},
                    {"v_condense", false},
                    {"v_dump", false},
                    {"v_bypass", false}};
}
Pump* GasHandlingSystem::pump(const std::string& id) {
    for (auto& p : snap_.pumps)
        if (p.id == id)
            return &p;
    return nullptr;
}
Valve* GasHandlingSystem::valve(const std::string& id) {
    for (auto& v : snap_.valves)
        if (v.id == id)
            return &v;
    return nullptr;
}
void GasHandlingSystem::setPump(const std::string& id, bool on) {
    if (auto* p = pump(id))
        p->on = on;
}
void GasHandlingSystem::setValve(const std::string& id, bool open) {
    if (auto* v = valve(id))
        v->open = open;
}

bool GasHandlingSystem::circulationReady() const {
    auto pc = std::find_if(snap_.pumps.begin(), snap_.pumps.end(),
                           [](const Pump& p) { return p.id == "compressor_3he"; });
    auto ps = std::find_if(snap_.pumps.begin(), snap_.pumps.end(),
                           [](const Pump& p) { return p.id == "turbo_still"; });
    auto vc = std::find_if(snap_.valves.begin(), snap_.valves.end(),
                           [](const Valve& v) { return v.id == "v_condense"; });
    auto vs = std::find_if(snap_.valves.begin(), snap_.valves.end(),
                           [](const Valve& v) { return v.id == "v_still_pump"; });
    return pc->speed_frac > 0.9 && ps->speed_frac > 0.9 && vc->open && vs->open &&
           snap_.mixtureCondensed_frac > 0.5;
}
...
void GasHandlingSystem::step(double dt, double stillPower_W, double T_still) {
    ...
    auto scroll = pump("scroll_backing"), turboOvc = pump("turbo_ovc"),
         turboStill = pump("turbo_still"), comp = pump("compressor_3he");
    // OVC pressure: exponential pump-down to the ultimate of the active pump when the valve is
    // open; slow leak-up (1e-6 mbar/s) otherwise.
    double ultimate = 1000.0;
    if (valve("v_ovc_pump")->open) {
        if (turboOvc->speed_frac > 0.5 && scroll->speed_frac > 0.5)
            ultimate = turboOvc->ultimate_mbar;
        else if (scroll->speed_frac > 0.5)
            ultimate = scroll->ultimate_mbar;
    }
    if (ultimate < snap_.p_ovc_mbar) {
        double rate =
            (turboOvc->speed_frac > 0.5 ? 1.0 / 600.0 : 1.0 / 900.0); // pump-down time constants
        snap_.p_ovc_mbar = ultimate + (snap_.p_ovc_mbar - ultimate) * std::exp(-dt * rate);
    } else
        snap_.p_ovc_mbar = std::min(1000.0, snap_.p_ovc_mbar + 1e-6 * dt);
    ...
    if (condensing)
        snap_.mixtureCondensed_frac =
            std::min(1.0, snap_.mixtureCondensed_frac + dt / (4.0 * 3600.0));
    if (valve("v_dump")->open && comp->speed_frac > 0.5)
        snap_.mixtureCondensed_frac =
            std::max(0.0, snap_.mixtureCondensed_frac - dt / (6.0 * 3600.0));
