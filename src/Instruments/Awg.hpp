...
// to `resolution_bits` (Δ = 2 V_fs / 2^N). A record longer than `memory_samples` is a Fault
// carrying the sample count — the reason long pulse-level programs must be looped (spec 14 §7).
//
...
struct IqCorrection {
    ...
    bool identity() const {
        return offsetI == 0.0 && offsetQ == 0.0 && gainRatio == 1.0 && phaseSkewRad == 0.0;
    }
};
...
class Awg final : public InstrumentBase, public ISignalSource {
  public:
    ...
    std::optional<pulse::ChannelId> channelOfPort(std::uint32_t port,
    ...
    // The schedule envelope after N-bit quantisation, s_q = Q(V_fs s)/V_fs, for the Lindblad
    // backend when `apply_quantization` is on (spec 12 §3).
    ...
    std::optional<double>
    query(std::string_view path) const override; // running, ch[k].waveform, seq.shot
  ...
  private:
    ...
    sampled(const std::shared_ptr<const pulse::Schedule>& s, double rateHz) const;
    Result<AwgStreams> renderWith(std::uint32_t port, const SettingValues& v,
...
inline double lsbVolts(double fullScaleV, int bits) {
    return 2.0 * fullScaleV / static_cast<double>(1u << bits);
}
