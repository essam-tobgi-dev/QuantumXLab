...
constexpr double kNominalLoDbm =
    7.0; // below this LO drive the conversion loss grows dB for dB (Model)

Complex coefficient(double dbc, double angleDeg) {
    return std::polar(std::pow(10.0, dbc / 20.0), angleDeg * kPi / 180.0);
}
} // namespace
...
SettingSchema IqMixer::makeSchema() {
    ...
    s.settings = {
        SettingSpec::real("lo_leakage_dbc", "dBc", -80.0, -20.0, 0.1, -40.0,
                          "Uncalibrated LO leakage relative to a full-scale carrier"),
        SettingSpec::real("image_rejection_dbc", "dBc", -80.0, -20.0, 0.1, -35.0,
                          "Uncalibrated image sideband relative to the wanted sideband"),
        SettingSpec::real(
            "leakage_phase", "deg", -180.0, 180.0, 0.0, 30.0,
            "Phase of the leakage vector (how it splits between the I and Q offsets)"),
        SettingSpec::real("imbalance_angle", "deg", -180.0, 180.0, 0.0, 45.0,
                          "0°: pure amplitude imbalance, 90°: pure phase skew"),
        SettingSpec::real("conversion_loss_db", "dB", 0.0, 15.0, 0.1, 6.0,
                          "IF → RF conversion loss"),
        SettingSpec::real("rf_min", "Hz", 2e9, 2e9, 0.0, 2e9, "Lower edge of the RF band")
            .constant(),
        SettingSpec::real("rf_max", "Hz", 12e9, 12e9, 0.0, 12e9, "Upper edge of the RF band")
            .constant(),
        refreshRateSetting(),
    };
    return s;
...
IqMixer::IqMixer(std::uint32_t index) : InstrumentBase({"iq_mixer", index}, makeSchema()) {
    setChannels({
        {{},
         ...
         false,
         false,
         "Residual LO leakage with the present AWG offsets"},
        {{},
         "image_rej",
         "s",
         "dBc",
         ...
         false,
         false,
         "Residual image sideband with the present AWG skew"},
        {{},
         "rf",
         "s",
         "V",
         ...
         true,
         false,
         "Complex envelope of the RF output around f_LO"},
    });
}
...
Result<Signal> IqMixer::signal(std::string_view, const SignalRequest& request) const {
    ...
    for (auto const& z : lo.samples)
        loAmplitude = std::max(loAmplitude, std::abs(z));
    const double ifHz = awg_ ? awg_->intermediateFrequencyHz() : 0.0;
    out.landmarks = {{"carrier", lo.referenceHz + ifHz},
                     {"lo", lo.referenceHz},
                     {"image", lo.referenceHz - ifHz}};
    if (state() == State::Off || loAmplitude <= 0.0) { // no LO drive: nothing is converted
        out.connected = lo.connected;
        out.note =
            state() == State::Off ? "mixer switched off" : (lo.connected ? "LO off" : lo.note);
        return out;
    }
    double lossDb = v.real("conversion_loss_db");
    const double loDbm = dbmFromWatts(tonePowerWatts(loAmplitude));
    if (loDbm < kNominalLoDbm)
        lossDb += kNominalLoDbm - loDbm; // starved LO (Model)
    ...
    if (!s.connected)
        out.note = s.note; // the leakage term survives a missing IF cable
    return out;
}
...
Result<Trace> IqMixer::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    if (channel.name == "lo_leak")
        return scalarTrace(channel, ctx, loLeakageDbc());
    if (channel.name == "image_rej")
        return scalarTrace(channel, ctx, imageDbc());
    ...
    for (auto const& [name, f] : s.landmarks)
        t.markers.push_back({0.0, 0.0, name, f, 0.0, "Hz"});
    return t;
}
