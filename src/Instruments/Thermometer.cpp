...
double SensorCurve::minK() const {
    return kind == SensorKind::RuO2 ? 0.010 : 1.0;
}
double SensorCurve::maxK() const {
    return kind == SensorKind::RuO2 ? 40.0 : 325.0;
}

double SensorCurve::resistanceOhm(double T) const {
    if (kind == SensorKind::RuO2)
        return kRuO2R0 * std::exp(std::pow(kRuO2T0 / T, 0.25));
...
double SensorCurve::temperatureK(double R) const {
    if (kind == SensorKind::RuO2)
        return kRuO2T0 / std::pow(std::log(R / kRuO2R0), 4.0);
...
double SensorCurve::sensitivity(double T) const {
    if (kind == SensorKind::RuO2)
        return 0.25 * std::pow(kRuO2T0 / T, 0.25);
...
SettingSchema Thermometer::makeSchema(SensorKind kind) {
    ...
    s.settings = {
        ruo2 ? SettingSpec::choice("stage", {"MXC", "CP", "STILL", "PT2"}, "MXC",
                                   "Stage the sensor is mounted on")
             : SettingSpec::choice("stage", {"PT1", "PT2", "STILL"}, "PT2",
                                   "Stage the sensor is mounted on"),
        SettingSpec::real("excitation", "W", 1e-12, 1e-7, 0.0, ruo2 ? 1e-10 : 1e-9,
                          "Bridge excitation power dissipated in the sensor"),
        // k of T_s⁴ = T⁴ + 4P/k. RuO₂ default: 10 nW reads 2.5 mK high at 10 mK (spec 12 §8, §15).
        SettingSpec::real("kapitza", "", 0.01, 1e4, 0.0, ruo2 ? 2.77 : 50.0,
                          "Boundary conductance coefficient k in W/K⁴ (Model)"),
        SettingSpec::real("noise_fraction", "", ruo2 ? kNoiseRuO2 : kNoiseCernox,
                          ruo2 ? kNoiseRuO2 : kNoiseCernox, 0.0, ruo2 ? kNoiseRuO2 : kNoiseCernox,
                          "Reading noise as a fraction of the reading")
            .constant(),
        SettingSpec::real("range_min", "K", ruo2 ? 0.010 : 1.0, ruo2 ? 0.010 : 1.0, 0.0,
                          ruo2 ? 0.010 : 1.0, "Lowest calibrated temperature")
            .constant(),
        SettingSpec::real("range_max", "K", ruo2 ? 40.0 : 325.0, ruo2 ? 40.0 : 325.0, 0.0,
                          ruo2 ? 40.0 : 325.0, "Highest calibrated temperature")
            .constant(),
...
Thermometer::Thermometer(SensorKind kind, std::uint32_t index)
    : InstrumentBase({kind == SensorKind::RuO2 ? "thermometer_ruo2" : "thermometer_cernox", index},
                     makeSchema(kind)) {
    curve_.kind = kind;
    setChannels({
        {{},
         ...
         false,
         false,
         "Temperature reading versus lab time"},
        {{},
         "R",
         "s",
         "Ω",
         ...
         false,
         false,
         "Sensor resistance versus lab time"},
    });
}
...
std::optional<double> Thermometer::query(std::string_view path) const {
    ...
    if (path == "T")
        return last ? std::optional(last->temperatureK) : std::nullopt;
    if (path == "R")
        return last ? std::optional(last->resistanceOhm) : std::nullopt;
...
Result<Trace> Thermometer::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    r.resistanceOhm = curve_.resistanceOhm(inRange) *
                      (1.0 + noise * curve_.sensitivity(inRange) * ctx.rng.normal());
    ...
    for (auto const& h : history) {
        ...
        if (channel.name == "T") {
            t.y.push_back(h.temperatureK);
            t.sigma->y.push_back(h.sigmaK);
        } else {
            t.y.push_back(h.resistanceOhm);
            t.sigma->y.push_back(noise * curve_.sensitivity(h.temperatureK) * h.resistanceOhm);
        }
    }
    t.markers.push_back(
        {r.timeS, t.y.back(), "reading", t.y.back(), t.sigma->y.back(), channel.yUnit});
    if (r.outOfRange)
        t.markers.push_back({r.timeS, t.y.back(), "out_of_range", r.temperatureK, 0.0, "K"});
    return t;
}
