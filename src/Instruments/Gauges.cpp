...
Trace StripChartInstrument::chart(const ChannelDesc& channel, const AcquireContext& ctx, const ScalarReading& reading) {
    std::vector<ScalarReading> history;
    {
        std::lock_guard lk(historyMu_);
        history_.push_back(reading);
        const auto keep = static_cast<std::size_t>(std::max<std::int64_t>(1, ctx.settings.integer("history_points")));
    ...
    t.markers.push_back({reading.timeS, reading.value, "reading", reading.value, reading.sigma, channel.yUnit});
    if (reading.outOfRange) t.markers.push_back({reading.timeS, reading.value, "out_of_range", reading.value, 0.0, channel.yUnit});
    return t;
}
...
SettingSchema PressureGauge::makeSchema() {
    ...
    s.settings = {
        SettingSpec::choice("node", {"ovc", "still", "condense", "dump"}, "ovc", "Node of the gas circuit the gauge sits on"),
        SettingSpec::real("range_min", "mbar", 1e-8, 1e-8, 0.0, 1e-8, "Lowest readable pressure").constant(),
        SettingSpec::real("range_max", "mbar", 3000.0, 3000.0, 0.0, 3000.0, "Highest readable pressure").constant(),
...
PressureGauge::PressureGauge(std::uint32_t index) : StripChartInstrument({"pressure_gauge", index}, makeSchema()) {
    setChannels({{{}, "p", "s", "mbar", FidelityClass::Model, false, false, "Pressure reading versus lab time"}});
}

std::string_view PressureGauge::technology(std::string_view node, double mbar) {
    if (node != "ovc") return "capacitance"; // Baratron on the still, condensing line and dump
    return mbar < 1e-3 ? "cold_cathode" : "pirani";
}

std::optional<double> PressureGauge::query(std::string_view path) const {
    if (path == "p") { const auto r = lastReading(); return r ? std::optional(r->value) : std::nullopt; }
    return InstrumentBase::query(path);
}

Result<Trace> PressureGauge::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    if (!ctx.env) return fail(err::NotBound, id().toString() + ": no environment (gas-handling snapshot) is bound");
    const std::string node = ctx.settings.text("node");
    const cryo::GhsSnapshot& g = ctx.env->ghs;
    const double truth = node == "still" ? g.p_still_mbar : node == "condense" ? g.p_condense_mbar : node == "dump" ? g.p_dump_mbar : g.p_ovc_mbar;
    // Repeatability of the technology in use: capacitance 0.25 %, Pirani 5 %, cold cathode 10 % of reading.
...
FlowMeter::FlowMeter(std::uint32_t index) : StripChartInstrument({"flow_meter", index}, makeSchema()) {
    setChannels({{{}, "n3", "s", "mmol/s", FidelityClass::Model, false, false, "³He circulation rate versus lab time"}});
}

std::optional<double> FlowMeter::query(std::string_view path) const {
    if (path == "n3") { const auto r = lastReading(); return r ? std::optional(r->value) : std::nullopt; }
    return InstrumentBase::query(path);
}

Result<Trace> FlowMeter::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    if (!ctx.env) return fail(err::NotBound, id().toString() + ": no environment (gas-handling snapshot) is bound");
    // ṅ₃ of the GHS; the thermal snapshot carries the same value once the network has taken it over.
    const double molPerS = ctx.env->ghs.n3_mol_s > 0.0 ? ctx.env->ghs.n3_mol_s : ctx.env->thermal.n3_mol_s;
    ScalarReading r;
    r.timeS = ctx.stamp.labTimeS;
...
SettingSchema PowerMeter::makeSchema() {
    ...
    s.settings = {
        ...
        SettingSpec::real("range_min", "dBm", -70.0, -70.0, 0.0, -70.0, "Sensor noise floor").constant(),
        ...
        SettingSpec::real("freq_min", "Hz", 10e6, 10e6, 0.0, 10e6, "Lowest calibrated frequency").constant(),
        SettingSpec::real("freq_max", "Hz", 18e9, 18e9, 0.0, 18e9, "Highest calibrated frequency").constant(),
        ...
        SettingSpec::real("uncertainty_db", "dB", 0.1, 0.5, 0.01, 0.1, "Absolute accuracy of the calibration factor"),
...
PowerMeter::PowerMeter(std::uint32_t index) : StripChartInstrument({"power_meter", index}, makeSchema()) {
    setChannels({{{}, "p", "s", "dBm", FidelityClass::Model, false, false, "Mean RF power at the node versus lab time"}});
}

double PowerMeter::repeatabilityDb(double averagingMs) {
    return kPowerRepeatabilityDb * std::sqrt(kPowerReferenceAveragingMs / std::max(averagingMs, 1e-9));
}

std::optional<double> PowerMeter::query(std::string_view path) const {
    if (path == "p" || path == "P_dBm") { const auto r = lastReading(); return r ? std::optional(r->value) : std::nullopt; }
    return InstrumentBase::query(path);
}
...
Result<Trace> PowerMeter::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    r.sigma = repeatability; // point-to-point scatter; the calibration factor's accuracy is a marker
    Trace t = chart(channel, ctx, r);
    t.markers.push_back({r.timeS, r.value, "uncertainty", ctx.settings.real("uncertainty_db"), 0.0, "dB"});
    // Out of the head's calibrated band the reading is uncalibrated, not wrong by a known factor.
    if (s.referenceHz > 0.0 && (s.referenceHz < ctx.settings.real("freq_min") || s.referenceHz > ctx.settings.real("freq_max")))
        t.markers.push_back({r.timeS, r.value, "out_of_band", s.referenceHz, 0.0, "Hz"});
    if (!s.connected) t.markers.push_back({r.timeS, r.value, "no_signal", r.value, 0.0, "dBm"});
    return t;
}
