...
Result<num::Matrix> reducedDensity(const qsim::Snapshot& s, std::span<const std::uint32_t> qubits) {
    ...
    if (keep.empty())
        return fail(err::BadInput, "state: no qubits selected");
    if (keep.back() >= s.nQubits)
        return fail(err::BadInput, std::format("state: qubit {} is outside the {}-qubit register",
                                               keep.back(), s.nQubits));
    for (auto const& r : s.reduced) { // already reduced by the runtime (large registers)
        std::vector<std::size_t> have;
        for (auto q : r.qubits)
            have.push_back(q.get());
        if (std::is_sorted(have.begin(), have.end()) && have == keep)
            return r.rho;
    }
    ...
    if (s.densityMatrix) {
        if (s.densityMatrix->rows != dim || s.densityMatrix->cols != dim)
            return fail(err::BadInput,
                        std::format("state: density matrix is {}x{}, expected {}",
                                    s.densityMatrix->rows, s.densityMatrix->cols, dim));
        if (keep.size() == s.nQubits)
            return *s.densityMatrix;
    ...
    if (s.amplitudes) {
        if (s.amplitudes->size() != dim)
            return fail(err::BadInput, std::format("state: {} amplitudes, expected {}",
                                                   s.amplitudes->size(), dim));
        return num::reducedState(*s.amplitudes, s.nQubits, keep, levels);
    }
    return fail(
        err::BadInput,
        "state: the snapshot holds neither amplitudes, a density matrix nor this reduced state");
}
...
Result<std::vector<double>> levelPopulations(const qsim::Snapshot& s, std::uint32_t qubit) {
    ...
    for (std::size_t l = 0; l < rho.rows; ++l)
        p[l] = rho(l, l).real();
    return p;
}
...
num::Matrix qubitBlock(const num::Matrix& site) {
    ...
    for (std::size_t i = 0; i < 2 && i < site.rows; ++i)
        for (std::size_t j = 0; j < 2 && j < site.cols; ++j)
            b(i, j) = site(i, j);
    return b;
}
...
Result<num::Matrix> fullDensity(const qsim::Snapshot& s) {
    ...
    if (!s.amplitudes)
        return fail(err::BadInput,
                    "state: the snapshot holds neither amplitudes nor a density matrix");
    if (s.nQubits > 12)
        return fail(err::BadInput,
                    std::format("state: a {}-qubit pure state is too large to expand", s.nQubits));
