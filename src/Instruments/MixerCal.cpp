...
SimplexResult nelderMead(const std::function<double(const std::vector<double>&)>& f,
                         std::vector<double> x0, const std::vector<double>& steps, double tolX,
                         int maxEvaluations) {
    ...
    while (out.evaluations < maxEvaluations) {
        ...
        for (std::size_t i = 0; i < n; ++i)
            for (std::size_t k = 1; k <= n; ++k)
                size = std::max(size,
                                std::abs(simplex[k].x[i] - simplex[0].x[i]) / std::abs(steps[i]));
        if (size < tolX) {
            out.converged = true;
            break;
        }
        ...
        for (std::size_t k = 0; k < n; ++k)
            for (std::size_t i = 0; i < n; ++i)
                centroid[i] += simplex[k].x[i] / static_cast<double>(n);
        auto along = [&](double t) { // centroid + t (centroid − worst)
            std::vector<double> x(n);
            for (std::size_t i = 0; i < n; ++i)
                x[i] = centroid[i] + t * (centroid[i] - simplex[n].x[i]);
            return x;
        };
        ...
        if (vr < simplex[0].v) {
            ...
            if (vc < std::min(vr, simplex[n].v))
                simplex[n] = {xc, vc};
            else
                for (std::size_t k = 1; k <= n; ++k) { // shrink toward the best vertex
                    for (std::size_t i = 0; i < n; ++i)
                        simplex[k].x[i] =
                            simplex[0].x[i] + 0.5 * (simplex[k].x[i] - simplex[0].x[i]);
...
Result<MixerCalReport> MixerCalibration::run(const MixerCalOptions& options) {
    if (!mixer_.lo())
        return fail(err::NotBound, mixer_.id().toString() + ": no LO generator is attached");
    MixerCalReport rep;
    rep.loHz = mixer_.lo()->carrierHz();
    rep.ifHz = awg_.intermediateFrequencyHz();
    if (rep.ifHz == 0.0)
        return fail(err::BadInput,
                    "mixer calibration needs a non-zero IF: the image coincides with the carrier");
    ...
    if (rbw > std::abs(rep.ifHz) / 8.0)
        QXL_TRY(analyzer_.set("rbw", std::abs(rep.ifHz) / 8.0));
    ...
    auto floorTrace =
        analyzer_.markerPowerDbm(rep.loHz + 1.7 * rep.ifHz); // between the tones: the floor
    if (floorTrace && rep.carrierDbm < *floorTrace + options.minCarrierAboveFloorDb)
        return restore(
            Error(err::NoSignal, std::format("mixer calibration: no carrier at {:.6g} Hz — play a "
                                             "calibration tone on the channel",
                                             carrierHz)));
    rep.loBeforeDbc = loBefore - rep.carrierDbm;
    rep.imageBeforeDbc = imageBefore - rep.carrierDbm;
    rep.log.push_back(
        std::format("carrier {:.2f} dBm at {:.6g} Hz; LO leakage {:.1f} dBc, image {:.1f} dBc",
    ...
    auto offsets = nelderMead(
        [&](const std::vector<double>& x) {
            c.offsetI = x[0];
            c.offsetQ = x[1];
            if (auto ok = awg_.setCorrection(port_, c); !ok && !failure)
                failure = ok.error();
            return read(rep.loHz);
        },
        {before.offsetI, before.offsetQ}, {options.offsetStepV, options.offsetStepV}, 1e-4,
    ...
    rep.log.push_back(std::format("offsets I {:+.3f} mV, Q {:+.3f} mV after {} readings",
                                  c.offsetI * 1e3, c.offsetQ * 1e3, offsets.evaluations));
    ...
    auto skew = nelderMead(
        ...
        {before.gainRatio, before.phaseSkewRad / kDeg}, {options.ratioStep, options.skewStepDeg},
        1e-5, options.maxEvaluations);
    if (failure)
        return restore(*failure);
    ...
    rep.log.push_back(
        std::format("amplitude ratio {:.5f}, phase skew {:+.4f} deg after {} readings", c.gainRatio,
                    skew.x[1], skew.evaluations));
    ...
    if (failure)
        return restore(*failure);
    rep.log.push_back(std::format("after calibration: LO leakage {:.1f} dBc, image {:.1f} dBc",
                                  rep.loAfterDbc, rep.imageAfterDbc));
    return rep;
}
