...
// with n(t) white of one-sided density k_B T_sys G Z0 (T_sys referred to the chip, vacuum
// included). Boxcar integration of the steady state over T then separates the two clouds by
//   SNR = |μ_1 − μ_0|/σ = |α_1 − α_0| √(2ηκT),   η = T_q/T_sys, T_q = ħω/2k_B   (T05 (6.7), T07
//   (8.2)),
...
double weightedSnr(const ReadoutParams& p, const CavityResponse& r,
                   std::span<const Complex> weights, int s0 = 0, int s1 = 1);
// Ṽ per √photon at the ADC, √(Z0 G ħω_r κ), and the rms noise per ADC sample, √(k_B T_sys G Z0
// f_s/2).
