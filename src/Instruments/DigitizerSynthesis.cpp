...
std::vector<double> Digitizer::Synth::meanOf(std::span<const Complex> field) const {
    ...
    for (std::size_t k = 0; k < field.size(); ++k)
        m[k] = (voltsPerRootPhoton * field[k] * std::polar(1.0, kTwoPi * ifHz * static_cast<double>(k) / sampleRateHz)).real();
    return m;
}

std::vector<double> Digitizer::Synth::record(std::span<const Complex> field, core::Random& rng) const {
...
Complex Digitizer::Synth::demodulate(std::span<const double> rec) const {
    Complex acc{};
    for (std::size_t k = 0; k < rec.size() && k < demod.size(); ++k) acc += demod[k] * rec[k];
    return acc;
}
...
Result<Digitizer::Synth> Digitizer::prepare(std::uint32_t k, const SettingValues& v) const {
    ...
    if (n < 8) return fail(err::BadInput, std::format("{}: acquisition window of {} samples is too short", id().toString(), n));
    if (n > static_cast<std::size_t>(v.integer("memory_samples")))
        return fail(err::MemoryOverflow, std::format("acquisition window of {} samples exceeds the {} samples of record memory",
    ...
    if (kind == "boxcar") s.weights = boxcarWeights(n);
    else if (kind == "matched") s.weights = matchedWeights(s.response);
    else {
        std::lock_guard lk(demodMu_);
        const auto& custom = demod_[k].customWeights;
        if (custom.empty()) return fail(err::BadInput, std::format("{}: integration = custom but no weights are loaded", id().toString()));
        s.weights.resize(n);
        for (std::size_t j = 0; j < n; ++j) s.weights[j] = custom[std::min(custom.size() - 1, j * custom.size() / n)];
    }
    s.demod.resize(n);
    double energy = 0.0;
    for (std::size_t j = 0; j < n; ++j) {
        s.demod[j] = 2.0 / static_cast<double>(n) * s.weights[j] * std::polar(1.0, -kTwoPi * s.ifHz * static_cast<double>(j) / s.sampleRateHz);
    ...
    // Complex variance of the point is Σ|c_k|² σ²; half of it per quadrature. Quantisation adds Δ²/12.
...
Result<std::vector<IqPoint>> Digitizer::measure(std::uint32_t k, std::span<const std::uint8_t> states, core::Random& rng) {
    ...
    for (std::uint8_t prepared : states) {
        if (prepared >= s.states)
            return fail(err::BadInput, std::format("{}: state |{}> needs `states` = {}", id().toString(), static_cast<int>(prepared), prepared + 1));
        Complex iq;
        if (s.averages == 1) { // one shot in the time domain
            int effective = prepared;
            std::optional<double> decayAt;
            const double u = rng.uniform();
            if (prepared == 0 && u < p.flip01) effective = 1;
            else if (prepared == 1 && u < p.flip10) effective = 0;
            else if (prepared == 1 && u < p.flip10 + p.decay10 && p.t1S > 0.0) // T1 decay inside the window
                decayAt = -p.t1S * std::log1p(-rng.uniform() * -std::expm1(-p.windowS / p.t1S));
            if (decayAt) iq = s.demodulate(s.record(decayedField(p, s.sampleRateHz, *decayAt), rng));
            else iq = s.demodulate(s.record(s.response.alpha[static_cast<std::size_t>(effective)], rng));
        } else { // mean of `averages` shots: exact Gaussian of the integrated noise around the mixed mean
            const double a = static_cast<double>(s.averages);
            Complex mean = s.meanIq[prepared];
            if (prepared <= 1) {
                const Complex other = s.meanIq[1 - prepared];
                const double flips = std::min(a, static_cast<double>(rng.poisson(a * (prepared == 0 ? p.flip01 : p.flip10))));
                const double decays = prepared == 1 ? std::min(a - flips, static_cast<double>(rng.poisson(a * p.decay10))) : 0.0;
                mean = ((a - flips - decays) * mean + flips * other + decays * 0.5 * (mean + other)) / a;
            }
        ...
        if (disc) pt.assigned = static_cast<std::int8_t>(disc->classify(pt.i, pt.q));
        out.push_back(pt);
    }
    ...
    while (cloud.size() > keep) cloud.pop_front();
    return out;
}
...
Result<TrainingReport> Digitizer::train(std::uint32_t k, std::span<const IqPoint> labelled) {
    ...
    for (auto& pt : demod_[k].cloud) pt.assigned = static_cast<std::int8_t>(rep.discriminator.classify(pt.i, pt.q));
    return rep;
}

Result<TrainingReport> Digitizer::calibrate(std::uint32_t k, std::size_t shotsPerState, std::uint64_t seed) {
    ...
    for (std::size_t shot = 0; shot < shotsPerState; ++shot)
        for (int st = 0; st < states; ++st) prepared.push_back(static_cast<std::uint8_t>(st));
