...
class Controller final : public InstrumentBase {
  public:
    ...
    static std::optional<std::size_t> executingAt(const std::vector<TimelineEntry>& entries,
                                                  double timeS);
    ...
    // over one period. Unlocked (`clock_ref = internal`) the free-running oscillator's 1e-8
    // applies.
    double periodSigmaS() const;
    std::optional<double>
    query(std::string_view path) const override; // locked, rate, shot, instruction, playhead
