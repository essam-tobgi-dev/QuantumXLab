...
bool emShared(std::span<const double> x, std::span<const double> y,
              std::vector<std::array<double, 2>>& mu, Cov& cov, int& iterations) {
    ...
    for (iterations = 0; iterations < 300; ++iterations) {
        ...
        for (std::size_t i = 0; i < n;
             ++i) { // E step (the common normalisation 2π√det cancels in the ratios)
            double total = 0.0;
            for (std::size_t k = 0; k < K; ++k) {
                resp[i * K + k] = weight[k] * std::exp(-0.5 * mahalanobis2(x[i] - mu[k][0],
                                                                           y[i] - mu[k][1], cov));
                total += resp[i * K + k];
            }
            if (total <= 0.0) {
                for (std::size_t k = 0; k < K; ++k)
                    resp[i * K + k] = 1.0 / static_cast<double>(K);
                total = 1.0;
            } else
                for (std::size_t k = 0; k < K; ++k)
                    resp[i * K + k] /= total;
        ...
        cov = {next.xx / static_cast<double>(n) + 1e-12, next.xy / static_cast<double>(n),
               next.yy / static_cast<double>(n) + 1e-12};
        if (std::abs(logLik - previous) < 1e-9 * std::abs(logLik))
            return true;
        previous = logLik;
    }
    return false;
...
double Discriminator::project(double i, double q) const {
    ...
    const double wx = (syy * dx - sxy * dy) / det,
                 wy = (-sxy * dx + sxx * dy) / det;                     // w = Σ⁻¹(μ1 − μ0)
    const double norm = std::sqrt(std::max(wx * dx + wy * dy, 1e-300)); // √(wᵀΣw) = √(wᵀΔμ)
    return (wx * (i - 0.5 * (means[0][0] + means[1][0])) +
            wy * (q - 0.5 * (means[0][1] + means[1][1]))) /
           norm;
}
...
Result<Discriminator> trainDiscriminator(std::span<const IqPoint> labelled, int states) {
    if (states != 2 && states != 3)
        return fail(err::BadInput,
                    std::format("discriminator: states must be 2 or 3, got {}", states));
    ...
    for (auto const& p : labelled) {
        if (p.prepared < 0 || p.prepared >= states)
            return fail(err::BadInput,
                        std::format("discriminator: a training shot carries the label {}",
                                    static_cast<int>(p.prepared)));
    ...
    for (std::size_t s = 0; s < K; ++s)
        if (count[s] < 2)
            return fail(err::BadInput,
                        std::format("discriminator: no training shots prepared in |{}>", s));
    // Work in centred, unit-rms coordinates: the fit is then independent of the ADC's volt scale.
    double spread = 0.0;
    for (auto const& p : labelled)
        spread += ((p.i - cx) * (p.i - cx) + (p.q - cy) * (p.q - cy)) / static_cast<double>(n);
    ...
    if (states == 2) {
        ...
        if (!g)
            return fail(err::FitFailed, "discriminator: " + g.error().message);
        // fitGmm2 orders its components by I; the prepared labels say which one is |1⟩.
        std::size_t agree = 0;
        for (std::size_t i = 0; i < n; ++i)
            agree += g->classify(x[i], y[i]) == labels[i] ? 1u : 0u;
    ...
    for (std::size_t s = 0; s < K; ++s)
        d.means.push_back({cx + spread * mu[s][0], cy + spread * mu[s][1]});
    d.sxx = cov.xx * spread * spread;
    d.sxy = cov.xy * spread * spread;
    d.syy = cov.yy * spread * spread;
    d.snr = std::sqrt(mahalanobis2(d.means[1][0] - d.means[0][0], d.means[1][1] - d.means[0][1],
                                   {d.sxx, d.sxy, d.syy}));
    return d;
}
...
AssignmentEstimate estimateAssignment(std::span<const IqPoint> labelled, const Discriminator& d) {
    ...
    a.shots.assign(K, 0);
    for (auto const& p : labelled) {
        if (p.prepared < 0 || p.prepared >= d.states)
            continue;
        ++counts[static_cast<std::size_t>(p.prepared)]
                [static_cast<std::size_t>(d.classify(p.i, p.q))];
