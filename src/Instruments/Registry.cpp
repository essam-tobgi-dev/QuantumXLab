...
template <class T> InstrumentRegistry::Factory make() {
    return [](std::uint32_t index) -> std::unique_ptr<IInstrument> { return std::make_unique<T>(index); };
}
} // namespace
...
InstrumentRegistry::InstrumentRegistry() {
    ...
    registerKind("thermometer_ruo2", [](std::uint32_t i) -> std::unique_ptr<IInstrument> { return std::make_unique<Thermometer>(SensorKind::RuO2, i); });
    registerKind("thermometer_cernox", [](std::uint32_t i) -> std::unique_ptr<IInstrument> { return std::make_unique<Thermometer>(SensorKind::Cernox, i); });
...
void InstrumentRegistry::registerKind(std::string kind, Factory factory, bool simulatorOnly) {
    if (kinds_.find(kind) == kinds_.end()) kindOrder_.push_back(kind);
    kinds_[std::move(kind)] = KindInfo{std::move(factory), simulatorOnly};
}

bool InstrumentRegistry::hasKind(std::string_view kind) const { return kinds_.find(kind) != kinds_.end(); }

std::vector<std::string> InstrumentRegistry::kinds(bool physicalLab) const {
    std::vector<std::string> out;
    for (auto const& k : kindOrder_)
        if (!physicalLab || !kinds_.find(k)->second.simulatorOnly) out.push_back(k);
    return out;
}
...
core::Json InstrumentRegistry::schemas() const {
    ...
    for (auto const& k : kindOrder_) {
        ...
        if (out.contains(s.id)) out[s.id]["x-variants"][s.instrument] = s.toJson();
        else out[s.id] = s.toJson();
    }
    return out;
}
...
Result<IInstrument*> InstrumentRegistry::create(std::string_view kind) {
    ...
    if (it == kinds_.end()) return fail(err::UnknownInstrument, std::format("no instrument kind '{}'", kind));
    ...
    if (auto* awg = dynamic_cast<Awg*>(raw))
        for (std::uint32_t k = 0; k < Awg::kChannels; ++k) graph_.addNode(std::format("{}.ch[{}]", name, k), awg, std::format("ch[{}]", k));
    if (auto* gen = dynamic_cast<Generator*>(raw)) graph_.addNode(name + ".rf", gen, "rf");
    ...
    if (dynamic_cast<Thermometer*>(raw) || dynamic_cast<ThermalTruthProbe*>(raw)) attachThermometers();
    return raw;
}
...
void InstrumentRegistry::attachThermometers() {
    ...
    for (auto const& i : instruments_)
        if (auto* t = dynamic_cast<const Thermometer*>(i.get())) thermometers.push_back(t);
    for (auto const& i : instruments_)
        if (auto* p = dynamic_cast<ThermalTruthProbe*>(i.get())) p->attach(thermometers);
}

IInstrument* InstrumentRegistry::find(std::string_view kind, std::uint32_t index) const {
    for (auto const& i : instruments_)
        if (i->id().kind == kind && i->id().index == index) return i.get();
    return nullptr;
}
...
std::vector<IInstrument*> InstrumentRegistry::all(bool physicalLab) const {
    ...
    for (auto const& i : instruments_)
        if (!physicalLab || !i->simulatorOnly()) out.push_back(i.get());
    return out;
}
...
std::vector<IInstrument*> InstrumentRegistry::ofKind(std::string_view kind) const {
    std::vector<IInstrument*> out;
    for (auto const& i : instruments_)
        if (i->id().kind == kind) out.push_back(i.get());
    return out;
}

Result<void> InstrumentRegistry::bindComponent(const InstrumentId& id, ComponentId component, std::vector<std::string> lines,
...
IInstrument* InstrumentRegistry::findByComponent(ComponentId component) const {
    for (auto const& i : instruments_)
        if (i->bindings().component == component && component.get() != 0) return i.get();
    return nullptr;
}

void InstrumentRegistry::setEventBus(core::EventBus* bus) {
    bus_ = bus;
    for (auto const& i : instruments_) i->bind(bindingsFor(i.get()));
}

Result<void> InstrumentRegistry::connectUpconversion(std::uint32_t awg, std::uint32_t port, std::uint32_t generator, std::uint32_t mixer) {
    auto* a = get<Awg>("awg", awg);
    auto* g = get<Generator>("sg_mw", generator);
    auto* m = get<IqMixer>("iq_mixer", mixer);
    if (!a || !g || !m) return fail(err::UnknownInstrument, std::format("up-conversion needs awg[{}], sg_mw[{}] and iq_mixer[{}]", awg, generator, mixer));
    if (port >= Awg::kChannels) return fail(err::UnknownChannel, std::format("awg[{}] has no output ch[{}]", awg, port));
...
Result<void> InstrumentRegistry::attachLine(std::string upstreamNode, const std::string& lineId) {
    if (!graph_.hasNode(upstreamNode)) return fail(err::BadRouting, std::format("routing: unknown node '{}'", upstreamNode));
    for (cryo::Stage stage : {cryo::Stage::PT1, cryo::Stage::PT2, cryo::Stage::STILL, cryo::Stage::CP, cryo::Stage::MXC}) {
        taps_.push_back(std::make_unique<LineTap>(graph_, upstreamNode, hub_, lineId, stage));
        graph_.addNode(std::format("line.{}.{}", lineId, cryo::stageName(stage)), taps_.back().get());
    }
    return {};
}
...
Result<void> InstrumentRegistry::createStandardSet(const StandardSetOptions& o) {
    auto many = [&](std::string_view kind, std::uint32_t count) -> Result<void> {
        for (std::uint32_t k = 0; k < count; ++k) QXL_TRY(create(kind));
        return {};
    };
    ...
    for (std::string_view kind : {"digitizer", "vna", "spectrum_analyzer", "oscilloscope", "dc_source", "flow_meter", "power_meter"}) QXL_TRY(many(kind, 1));
    // RuO₂ on the mixing chamber and cold plate, Cernox on the pulse-tube stages and the still (spec 12 §8).
    ...
    if (o.probes)
        for (auto const& kind : kinds())
            if (kinds_.find(kind)->second.simulatorOnly) QXL_TRY(many(kind, 1));
    ...
    if (o.powerOn) powerAll(true);
    return {};
}

void InstrumentRegistry::powerAll(bool on) {
    for (auto const& i : instruments_) (void)i->execute(Command::of(on ? Command::Kind::PowerOn : Command::Kind::PowerOff));
}
...
core::Json InstrumentRegistry::saveState() const {
    ...
    for (auto const& i : instruments_)
        instruments.push_back({{"kind", i->id().kind}, {"index", i->id().index}, {"on", i->state() != State::Off}, {"settings", i->saveSettings()}});
...
Result<void> InstrumentRegistry::loadState(const core::Json& state) {
    ...
    for (auto const& j : state["instruments"]) {
        ...
        if (!j.is_object() || !j.contains("kind") || !j["kind"].is_string()) return fail(err::BadSchema, std::format("instrument state: '{}.kind' missing", path));
        IInstrument* instrument = find(j["kind"].get<std::string>(), j.value("index", 0u));
        if (!instrument) continue; // an instrument this rack does not have: tolerated
        if (j.contains("settings")) QXL_TRY(instrument->loadSettings(j["settings"]));
        (void)instrument->execute(Command::of(j.value("on", true) ? Command::Kind::PowerOn : Command::Kind::PowerOff));
    }
    if (state.contains("routing") && state["routing"].contains("edges"))
        for (auto const& e : state["routing"]["edges"]) // cables are restored by id; sources stay attached
            if (e.contains("id") && e["id"].is_string()) (void)graph_.setConnected(e["id"].get<std::string>(), e.value("connected", true));
    return {};
}
