...
data::Trace2D Trace::toTrace2D() const {
    ...
    if (sigma)
        out.sigma = sigma->y;
    for (auto const& m : markers)
        out.markers.push_back({m.x, m.label});
    out.cls = cls;
...
std::string traceToCsv(const Trace& t, std::string_view settingsComment) {
    ...
    for (std::size_t i = 0; i < t.x.size(); ++i) {
        ...
        if (cplx)
            out += std::format(",{:.17g}", t.y_im[i]);
        if (withSigma)
            out += std::format(",{:.9g}", t.sigma->y[i]);
        for (auto const& [name, col] : t.aux) {
            (void)name;
            if (col.size() == t.x.size())
                out += std::format(",{:.12g}", col[i]);
        }
        out += "\n";
    }
    return out;
