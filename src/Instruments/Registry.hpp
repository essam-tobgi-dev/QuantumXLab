...
class LineTap final : public ISignalSource {
  public:
    LineTap(const SignalGraph& graph, std::string upstream, std::shared_ptr<const InputHub> inputs,
            std::string lineId, cryo::Stage stage);
...
class InstrumentRegistry {
  public:
    ...
    template <class T> T* get(std::string_view kind, std::uint32_t index = 0) const {
        return dynamic_cast<T*>(find(kind, index));
    }
    ...
    Result<void> bindComponent(const InstrumentId& id, ComponentId component,
    ...
    Result<void> connectUpconversion(std::uint32_t awg, std::uint32_t port, std::uint32_t generator,
                                     std::uint32_t mixer);
    // Adds the nodes "line.<lineId>.<STAGE>" (PT1 … MXC) fed by `upstreamNode` (a mixer's RF
    // output).
    Result<void> attachLine(std::string upstreamNode, const std::string& lineId);

    // One of every instrument of the lab rack (layout sc_lab_standard), thermometers on every
    // stage, gauges on every gas node, the probes, and the standard up-conversion routing.
    ...
    // "run.shot", "run.gate_cursor", "run.wall_time", "run.playhead", "run.running",
    // "run.progress", "probe.state.qubit[0].bloch[2]" (Simulator-only: nullopt when `physicalLab`),
    // and the generic
