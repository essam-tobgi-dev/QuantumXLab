...
//   v(t) = Re[Ṽ_s(t) e^{i2π f_IF t}] + n(t)        (ReadoutChain.hpp: cavity field, chain gain, T_sys)
// which the ADC samples at `sample_rate` and quantises to `resolution_bits`; the demodulator forms
//   I + iQ = (2/N) Σ_k w_k v_k e^{−i2π f_IF t_k}     (T07 (6.1); the 2 restores the envelope amplitude)
...
public:
    ...
    // resonators), else derived from the Environment; `window_ns`, the schedule's acquire window and
    // the played tone amplitude are applied on top.
    ...
    // One IQ point per entry of `states` — the true state of the qubit in each shot (0, 1, or 2 with
    // `states: 3`). Points are labelled, classified when a discriminator exists, and appended to
    // the cloud. This is what the runtime calls per shot of a pulse-level run.
    Result<std::vector<IqPoint>> measure(std::uint32_t k, std::span<const std::uint8_t> states, core::Random& rng);
    ...
    Result<TrainingReport> calibrate(std::uint32_t k, std::size_t shotsPerState, std::uint64_t seed);
    ...
    std::optional<double> query(std::string_view path) const override; // ch[k].iq(.i|.q), trigger, snr
