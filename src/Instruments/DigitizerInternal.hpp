...
struct Digitizer::Synth {
    ...
    std::vector<std::vector<double>> meanRecord; // [state][k]: Re[Ṽ_s e^{i2π f_IF t_k}] volts at the ADC
    ...
    // ADC record of a cavity field: carrier at f_IF, additive noise, N-bit quantisation with clipping.
