...
SettingSchema FidelityProbe::makeSchema() {
    ...
    s.settings = {SettingSpec::integer("history_points", "", 1, 100000, 4096, "Snapshots kept"), refreshRateSetting()};
    return s;
}

FidelityProbe::FidelityProbe(std::uint32_t index) : ProbeBase({"probe_fidelity", index}, makeSchema()) {
    setProbeChannels({
        {{}, "state_fidelity", "", "", FidelityClass::Exact, false, true, "F = <psi_ideal|rho|psi_ideal> versus gate index"},
        {{}, "process_fidelity", "", "", FidelityClass::Exact, false, true, "Process fidelity of the selected gate; marker `F_avg`"},
    });
}

Result<double> FidelityProbe::stateFidelity(const qsim::Snapshot& state, const qsim::Snapshot& ideal) {
    if (state.nQubits != ideal.nQubits || state.levels != ideal.levels)
        return fail(err::BadInput, "probe_fidelity: the run and its ideal reference describe different registers");
    if (ideal.amplitudes) { // squared convention (DEVELOPMENT.md): |⟨φ|ψ⟩|² or ⟨φ|ρ|φ⟩
        if (state.amplitudes) {
            if (state.amplitudes->size() != ideal.amplitudes->size()) return fail(err::BadInput, "probe_fidelity: state sizes differ");
            return num::fidelity(std::span<const num::Complex>(*ideal.amplitudes), std::span<const num::Complex>(*state.amplitudes));
        }
        if (state.densityMatrix) {
            if (state.densityMatrix->rows != ideal.amplitudes->size()) return fail(err::BadInput, "probe_fidelity: state sizes differ");
            return num::fidelity(std::span<const num::Complex>(*ideal.amplitudes), *state.densityMatrix);
        }
    }
    QXL_TRY_ASSIGN(num::Matrix rho, fullDensity(state));
    QXL_TRY_ASSIGN(num::Matrix sigma, fullDensity(ideal));
    if (rho.rows != sigma.rows) return fail(err::BadInput, "probe_fidelity: state sizes differ");
...
Result<double> FidelityProbe::processFidelity(const GateComparison& gate) {
    ...
    if (d == 0 || gate.ideal.cols != d || gate.kraus.empty()) return fail(err::BadInput, "probe_fidelity: gate comparison is empty");
    double f = 0.0;
    for (auto const& k : gate.kraus) {
        if (k.rows != d || k.cols != d) return fail(err::BadInput, "probe_fidelity: Kraus operator and ideal matrix differ in size");
...
Result<Trace> FidelityProbe::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    if (channel.name == "process_fidelity") {
        if (!ctx.run || !ctx.run->gate) return fail(err::NotBound, id().toString() + ": no gate is selected for comparison");
    ...
    if (!ctx.run->ideal) return fail(err::NotBound, id().toString() + ": the run view holds no ideal reference state");
    QXL_TRY_ASSIGN(double f, stateFidelity(*state, *ctx.run->ideal));
    if (!history_.empty() && state->gateIndex < history_.back().first) history_.clear(); // a new run
    if (history_.empty() || history_.back().first != state->gateIndex) history_.push_back({state->gateIndex, f});
    else history_.back().second = f;
...
TrajectoryProbe::TrajectoryProbe(std::uint32_t index) : ProbeBase({"probe_trajectory", index}, makeSchema()) {
    setProbeChannels({{{}, "jumps", "s", "", FidelityClass::Statistical, false, true,
...
Result<Trace> TrajectoryProbe::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    for (auto const& j : ctx.run->jumps) {
        ...
        t.markers.push_back({j.timeS, static_cast<double>(j.op), j.name, static_cast<double>(j.op), 0.0, ""});
    }
...
ThermalTruthProbe::ThermalTruthProbe(std::uint32_t index) : ProbeBase({"probe_thermal_truth", index}, makeSchema()) {
    setProbeChannels({{{}, "stages", "", "K", FidelityClass::Numerical, false, true,
                       "True stage temperatures: x = stage index (RT … MXC); aux `reading` = thermometer on that stage"}});
}

std::optional<double> ThermalTruthProbe::query(std::string_view path) const {
    auto env = environment();
    if (!env || !path.starts_with("stage.") || !path.ends_with(".T")) return InstrumentBase::query(path);
    cryo::Stage stage = cryo::Stage::MXC;
    if (!cryo::stageFromName(path.substr(6, path.size() - 8), stage)) return std::nullopt;
    ...
    for (cryo::Stage stage : cryo::kStages) {
        ...
        for (const Thermometer* th : thermometers_)
            if (th && th->stage() == stage)
                if (auto last = th->lastReading()) shown = last->temperatureK;
        reading.push_back(shown);
        t.markers.push_back({static_cast<double>(i), truth[i], std::string(cryo::stageName(stage)), truth[i], 0.0, "K"});
    }
    return t;
}
...
LeakageProbe::LeakageProbe(std::uint32_t index) : ProbeBase({"probe_leakage", index}, makeSchema()) {
    setProbeChannels({{{}, "leakage", "s", "", FidelityClass::Numerical, false, true,
                       "Population outside the computational subspace versus time: y = all qubits, aux `q<k>` per qubit"}});
}
...
Result<Trace> LeakageProbe::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    Trace t = makeTrace(channel, ctx);
    if (run.backend == qsim::Kind::Trajectories) t.cls = FidelityClass::Statistical;
    ...
    for (auto const& sample : run.populations) {
        if (sample.populations.size() != levels * sites)
            return fail(err::BadInput, std::format("{}: a population sample has {} entries, expected {} sites x {} levels", id().toString(),
                                                   sample.populations.size(), sites, levels));
        t.x.push_back(sample.timeS);
        double total = 0.0;
        for (std::size_t q = 0; q < sites; ++q) {
            double leaked = 0.0;
            for (std::size_t l = 2; l < levels; ++l) leaked += sample.populations[q * levels + l];
            t.aux[std::format("q{}", q)].push_back(leaked);
            total += leaked;
        }
        t.y.push_back(total);
    }
    if (levels < 3) t.markers.push_back({0.0, 0.0, "two-level run: no leakage levels", 0.0, 0.0, ""});
    return t;
}
