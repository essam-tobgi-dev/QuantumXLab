...
std::vector<double> PhaseNoiseMask::synthesize(std::size_t n, double sampleRateHz, core::Random& rng) const {
    std::vector<double> phi(n, 0.0);
    if (n == 0 || !(sampleRateHz > 0.0)) return phi;
    const double l10k = std::pow(10.0, at10kHzDbc / 10.0);
    const double floor = std::pow(10.0, floorDbc / 10.0);
...
SettingSchema Generator::makeSchema() {
    ...
    s.instrument = "sg_mw";
    s.settings = {
        SettingSpec::real("frequency", "Hz", 10e6, 20e9, 1.0, 5.0e9, "Carrier frequency (10 MHz – 20 GHz, 1 Hz steps)"),
        SettingSpec::real("power", "dBm", -20.0, 20.0, 0.01, 10.0, "Output power into 50 Ω"),
        SettingSpec::boolean("rf_on", false, "RF output enabled"),
        // Spec 12 §2 gives −110…−140 dBc/Hz, the descriptor's spec sheet −120…−100: the union is accepted.
        SettingSpec::real("phase_noise_dbc", "dBc/Hz", -140.0, -100.0, 0.1, -125.0, "SSB phase noise at 10 kHz offset"),
        SettingSpec::real("phase_noise_floor_dbc", "dBc/Hz", -175.0, -130.0, 0.1, -150.0, "Far-from-carrier phase-noise floor"),
        SettingSpec::choice("ref", {"internal", "ext_10MHz"}, "ext_10MHz", "Frequency reference"),
        SettingSpec::boolean("apply_phase_noise", false,
...
Generator::Generator(std::uint32_t index) : InstrumentBase({"sg_mw", index}, makeSchema()) {
    setChannels({
        ...
        {{}, "phase_noise", "Hz", "dBc/Hz", FidelityClass::Model, false, false, "SSB phase-noise mask L(f) versus offset"},
    });
}
...
bool Generator::outputOn() const { return snapshot().flag("rf_on"); }
double Generator::powerDbm() const { return snapshot().real("power"); }
...
Result<Signal> Generator::signal(std::string_view, const SignalRequest& request) const {
    ...
    for (std::size_t k = 0; k < n; ++k) s.samples[k] = std::polar(amplitude, phi[k]);
    return s;
}

std::optional<double> Generator::query(std::string_view path) const {
    if (path == "f") return carrierHz();
    if (path == "P_dBm") return powerDbm();
    if (path == "on") return outputOn() && state() != State::Off ? 1.0 : 0.0;
...
Result<Trace> Generator::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    if (channel.name == "f") return scalarTrace(channel, ctx, carrierHz());
    if (channel.name == "P_dBm") return scalarTrace(channel, ctx, v.real("power"));
    if (channel.name == "on") return scalarTrace(channel, ctx, v.flag("rf_on") ? 1.0 : 0.0);
