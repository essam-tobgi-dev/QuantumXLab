...
SettingSchema SpectrumAnalyzer::makeSchema() {
    ...
    s.settings = {
        SettingSpec::real("center", "Hz", 1e6, 26e9, 1.0, 5.1e9,
                          "Centre frequency (1 MHz – 26 GHz)"),
        SettingSpec::real("span", "Hz", 1e3, 5e9, 1.0, 500e6, "Frequency span"),
        SettingSpec::real("rbw", "Hz", 1.0, 3e6, 0.0, 1e6,
                          "Resolution bandwidth (noise bandwidth of the Hann window)"),
        SettingSpec::real("ref_level", "dBm", -100.0, 30.0, 0.1, 0.0,
                          "Reference level (top of the screen)"),
        SettingSpec::choice("detector", {"peak", "sample", "rms"}, "peak",
                            "Detector applied to each display bucket"),
        SettingSpec::integer("points", "", 101, 10001, 1001, "Display points"),
        SettingSpec::integer("averages", "", 1, 64, 1, "Power averages per trace"),
        SettingSpec::real("danl_dbm_hz", "dBm/Hz", -175.0, -120.0, 0.1, -150.0,
                          "Displayed average noise level of the analyzer"),
...
SpectrumAnalyzer::SpectrumAnalyzer(std::uint32_t index)
    : InstrumentBase({"spectrum_analyzer", index}, makeSchema()) {
    setChannels({
        {{},
         "trace",
         "Hz",
         ...
         false,
         false,
         "Power per resolution bandwidth"},
        {{},
         "markers",
         "Hz",
         "dBm",
         ...
         false,
         false,
         "Peak table, strongest first"},
    });
}

Result<SpectrumAnalyzer::Record> SpectrumAnalyzer::record(const SettingValues& v,
    ...
    const double fs =
        native > 0.0 ? native : std::max(2.5 * v.real("span"), 1e6); // CW source: we choose
    const double enbw = HannWindow(4096).enbwBins;
    const double wanted = std::ceil(enbw * fs / v.real("rbw"));
    const auto n =
        static_cast<std::size_t>(std::clamp(wanted, 64.0, static_cast<double>(kMaxRecord)));
    ...
    r.rbwHz = HannWindow(r.signal.samples.size()).enbwBins * r.signal.sampleRateHz /
              static_cast<double>(r.signal.samples.size());
...
Result<double> SpectrumAnalyzer::markerPowerDbm(double frequencyHz) const {
    ...
    for (std::size_t k = 0; k < z.size(); ++k)
        acc += win.w[k] * z[k] * std::polar(1.0, step * static_cast<double>(k));
...
Result<Trace> SpectrumAnalyzer::sweep(const ChannelDesc& channel, AcquireContext& ctx) const {
    ...
    const std::size_t pad =
        n <= (std::size_t{1} << 16) ? 8 : 4; // peak-detector scalloping < 0.03 dB / 0.09 dB
    ...
    for (std::size_t a = 0; a < averages; ++a) {
        ...
        for (std::size_t k = 0; k < n; ++k)
            buffer[k] =
                win.w[k] * (z[k] + Complex{ctx.rng.normal(0.0, sigma), ctx.rng.normal(0.0, sigma)});
        num::fftInPlace(buffer);
        for (std::size_t k = 0; k < m; ++k)
            power[k] += tonePowerWatts(std::abs(buffer[k]) * norm);
    }
    for (double& p : power)
        p /= static_cast<double>(averages);
    ...
    for (std::size_t j = 0; j < points; ++j) {
        ...
        t.x[j] = f;
        const double lo = f - bucket / 2.0 - r.signal.referenceHz,
                     hi = f + bucket / 2.0 - r.signal.referenceHz;
        if (hi <= -fs / 2.0 ||
            lo >= fs / 2.0) { // outside the band the node's record covers: floor only
            t.y[j] = dbmFromWatts(r.floorWatts);
            continue;
        }
        auto binAt = [&](std::int64_t k) {
            return power[static_cast<std::size_t>(
                ((k % static_cast<std::int64_t>(m)) + static_cast<std::int64_t>(m)) %
                static_cast<std::int64_t>(m))];
        };
        // Signed bin indices run from −m/2 to m/2 − 1. A display bucket that straddles the band
        // edge is clipped to that range: without it binAt's modulo wrap would fold bins in from the
        // opposite edge of the record's band and show them as power at this frequency.
        const auto half = static_cast<std::int64_t>(m / 2);
        const auto centre =
            std::clamp(static_cast<std::int64_t>(std::llround((f - r.signal.referenceHz) / binHz)),
                       -half, half - 1);
        std::int64_t first = std::max(-half, static_cast<std::int64_t>(std::ceil(lo / binHz)));
        std::int64_t last =
            std::min(half - 1, static_cast<std::int64_t>(std::ceil(hi / binHz)) - 1);
        if (last < first || detector == "sample")
            first = last = centre; // bucket narrower than a bin
        double value = 0.0;
        if (detector == "rms") {
    ...
    t.markers.push_back({t.x.front(), dbmFromWatts(r.floorWatts), "noise_floor",
                         dbmFromWatts(r.floorWatts), 0.0, "dBm"});
    ...
    if (*peakIt > v.real("ref_level"))
        t.markers.push_back(
            {t.x[peakIdx], *peakIt, "overload", *peakIt - v.real("ref_level"), 0.0, "dB"});
    ...
    for (auto const& [name, f] : r.signal.landmarks) {
        ...
        for (std::size_t j = 0; j < points; ++j)
            if (std::abs(t.x[j] - f) <= reach && t.y[j] > best) {
                best = t.y[j];
                at = t.x[j];
            }
        if (best <= -400.0)
    ...
    if (carrierDbm)
        for (const char* name : {"lo", "image"})
            if (const Marker* mk = t.marker(name)) {
                const Marker rel{mk->x, mk->y, std::string(name) + "_dbc", mk->value - *carrierDbm,
                                 0.0,   "dBc"};
                t.markers.push_back(rel);
            }
    if (!r.signal.connected)
        t.markers.push_back({t.x.front(), 0.0, "no_signal", 0.0, 0.0, ""});
    return t;
}

std::optional<double> SpectrumAnalyzer::query(std::string_view path) const {
    if (path == "trace" || path == "peak") {
        const double p = lastPeakDbm_.load();
        return std::isfinite(p) ? std::optional(p) : std::nullopt;
    }
    if (path == "peak_frequency") {
        const double f = lastPeakHz_.load();
        return std::isfinite(f) ? std::optional(f) : std::nullopt;
    }
    return InstrumentBase::query(path);
}

std::vector<SpectrumPeak> SpectrumAnalyzer::findPeaks(const Trace& trace, std::size_t maxPeaks,
                                                      double prominenceDb) {
    ...
    for (std::size_t i = 1; i + 1 < y.size(); ++i) {
        ...
        for (std::size_t k = i; k-- > 0 && y[k] <= y[i];)
            left = std::min(left, y[k]);
        for (std::size_t k = i + 1; k < y.size() && y[k] <= y[i]; ++k)
            right = std::min(right, y[k]);
        if (y[i] - std::max(left, right) >= prominenceDb)
            peaks.push_back({trace.x[i], y[i]});
    }
    std::sort(peaks.begin(), peaks.end(),
              [](const SpectrumPeak& a, const SpectrumPeak& b) { return a.powerDbm > b.powerDbm; });
...
Result<Trace> SpectrumAnalyzer::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    if (channel.name == "trace")
        return sweep(channel, ctx);
