...
std::size_t Signal::requestedSamples(const SignalRequest& r, double sampleRateHz,
                                     std::size_t whole) {
    if (r.samples)
        return r.samples;
    if (r.durationS > 0.0 && sampleRateHz > 0.0)
        return static_cast<std::size_t>(std::ceil(r.durationS * sampleRateHz));
    return whole;
}
...
Result<SignalGraph::Path> SignalGraph::resolve(std::string_view node) const {
    ...
    for (std::size_t hops = 0;; ++hops) {
        ...
        if (it == nodes_.end())
            return fail(err::BadRouting, std::format("routing: unknown node '{}'", at));
        if (it->second.source) {
        ...
        if (hops > nodes_.size())
            return fail(err::BadRouting, std::format("routing: cycle through '{}'", at));
        const RouteEdge* in = nullptr;
        for (auto const& e : edges_)
            if (e.to == at)
                in = &e;
        if (!in)
            return fail(err::BadRouting,
                        std::format("routing: node '{}' has no source upstream", at));
...
Result<Signal> SignalGraph::signalAt(std::string_view node, const SignalRequest& request) const {
    // The path is resolved under the lock, the source evaluated without it (a mixer asks the graph
    // again).
    ...
    if (!path.connected) { // spec 12 §11: the affected instrument shows no signal
        for (auto& z : sig->samples)
            z = Complex{};
        sig->connected = false;
        sig->note = std::format("no signal: a cable between '{}' and '{}' is disconnected",
...
Result<SignalGraph> SignalGraph::fromJson(const core::Json& j) {
    ...
    for (auto const& e : j["edges"]) {
        ...
        if (id.empty())
            id = e["from"].get<std::string>() + "->" + e["to"].get<std::string>();
        g.addEdge(e["from"].get<std::string>(), e["to"].get<std::string>(), e.value("gain_db", 0.0),
                  id);
        if (!e.value("connected", true))
            QXL_TRY(g.setConnected(id, false));
    }
    return g;
}
