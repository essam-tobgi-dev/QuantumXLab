...
// simplex is smaller than `tolX` in every coordinate (relative to `steps`) or after `maxEvaluations`.
...
SimplexResult nelderMead(const std::function<double(const std::vector<double>&)>& f, std::vector<double> x0,
                         const std::vector<double>& steps, double tolX = 1e-4, int maxEvaluations = 400);
