...
Result<void> LiveRunner::enable(const InstrumentId& id, std::string_view channel, bool on) {
    ...
    if (!ch)
        return fail(err::UnknownChannel,
                    std::format("{}: no channel '{}'", id.toString(), channel));
    const auto it = std::find_if(entries_.begin(), entries_.end(), [&](const Entry& e) {
        return e.instrument == instrument && e.channel == *ch;
    });
    if (!on) {
        if (it != entries_.end())
            entries_.erase(it);
    ...
    for (auto const& other :
         entries_) // one acquisition at a time per instrument: share its busy flag
        if (other.instrument == instrument)
            e.busy = other.busy;
...
bool LiveRunner::enabled(const InstrumentId& id, std::string_view channel) const {
    return std::any_of(entries_.begin(), entries_.end(), [&](const Entry& e) {
        return e.instrument->id() == id && e.channelName == channel;
    });
}
...
std::size_t LiveRunner::tick(double labTimeS) {
    ...
    for (auto& e : entries_) {
        ...
        if (e.instrument->state() == State::Off || e.busy->load())
            continue;
        if (labTimeS - e.lastS + 1e-12 < 1.0 / rate)
            continue;
            ...
            [target, work = std::move(work), busy, bus](std::stop_token stop) {
                for (auto const& [channel, name] : work) {
                    if (stop.stop_requested())
                        break;
                    // A trigger source that is still set re-arms the instrument for this
                    // acquisition.
                    const SettingValue trigger = target->get("trigger");
                    if (const auto* s = std::get_if<std::string>(&trigger);
                        s && *s != "free_run" && target->state() == State::Idle)
                        (void)target->execute(Command::of(Command::Kind::Arm));
                    auto trace = target->acquire(channel);
                    if (trace)
                        bus->post(TraceReady{std::make_shared<const Trace>(std::move(*trace))});
                    else
                        bus->post(AcquireFailed{target->id(), name, trace.error()});
                }
                busy->store(false);
            },
