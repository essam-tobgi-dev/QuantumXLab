...
double detuning(const ReadoutParams& p, int state) { return (2.0 * state - 1.0) * p.chiRadS; }

double driveAmplitude(const ReadoutParams& p) { // ε with |α_ss|² = photons at toneAmplitude 1
    return p.toneAmplitude * std::sqrt(p.photons * (p.kappaRadS * p.kappaRadS / 4.0 + p.chiRadS * p.chiRadS));
}
...
template <class StateAt> std::vector<Complex> integrate(const ReadoutParams& p, double fs, StateAt stateAt) {
    ...
    const pulse::Waveform tone = pulse::Waveform::gaussianSquare(p.toneS, p.toneSigmaS, p.toneRiseS);
    const double eps = driveAmplitude(p);
    std::vector<Complex> out(n);
    Complex alpha{};
    for (std::size_t j = 0; j < lead + n; ++j) {
        const double t = tStart + static_cast<double>(j) * h;
        if (j >= lead) out[j - lead] = alpha;
...
double decayFlipProbability(const ReadoutParams& p, const CavityResponse& r, std::span<const Complex> w, double snr) {
    Complex s0{}, s1{};
    for (std::size_t k = 0; k < w.size(); ++k) { s0 += w[k] * r.alpha[0][k]; s1 += w[k] * r.alpha[1][k]; }
    const Complex axis = s1 - s0;
    if (std::norm(axis) <= 0.0) return 0.5;
    constexpr int kSteps = 32;
    double acc = 0.0;
    for (int j = 0; j < kSteps; ++j) {
        const std::vector<Complex> field = decayedField(p, r.sampleRateHz, (j + 0.5) * p.windowS / kSteps);
        Complex sd{};
        for (std::size_t k = 0; k < w.size(); ++k) sd += w[k] * field[k];
        const double u = ((sd - 0.5 * (s0 + s1)) * std::conj(axis)).real() / std::norm(axis); // ±½ at the blobs
...
CavityResponse cavityResponse(const ReadoutParams& p, double sampleRateHz, int states) {
    ...
    for (int s = 0; s < states; ++s) r.alpha.push_back(integrate(p, sampleRateHz, [s](double) { return s; }));
    return r;
}
...
std::vector<Complex> matchedWeights(const CavityResponse& r) {
    ...
    if (peak > 0.0) for (auto& x : w) x /= peak;
    else std::fill(w.begin(), w.end(), Complex{1.0, 0.0});
    return w;
}

std::vector<Complex> boxcarWeights(std::size_t n) { return std::vector<Complex>(n, Complex{1.0, 0.0}); }
...
double weightedSnr(const ReadoutParams& p, const CavityResponse& r, std::span<const Complex> weights, int s0, int s1) {
    Complex signal{};
    double energy = 0.0;
    for (std::size_t k = 0; k < weights.size() && k < r.samples(); ++k) {
        signal += weights[k] * (r.alpha[static_cast<std::size_t>(s1)][k] - r.alpha[static_cast<std::size_t>(s0)][k]);
        energy += std::norm(weights[k]);
    }
    return energy > 0.0 ? std::abs(signal) * std::sqrt(2.0 * p.efficiency * p.kappaRadS / (r.sampleRateHz * energy)) : 0.0;
}
...
double overlapError(double snr) { return 0.5 * std::erfc(snr / (2.0 * std::numbers::sqrt2)); }
...
Result<ReadoutParams> readoutFromEnvironment(const Environment& env, std::uint32_t qubit, const ReadoutDefaults& d) {
    if (!env.calibration) return fail(err::NotBound, "readout: no calibration is bound");
    const hw::QubitCal* qc = env.calibration->qubit(qubit);
    if (!qc) return fail(err::BadInput, std::format("readout: calibration has no qubit {}", qubit));
    if (!qc->readoutChi || !qc->readoutKappa)
        return fail(err::BadInput, std::format("readout: qubit {} has no dispersive readout parameters (χ, κ)", qubit));
    ReadoutParams p;
    p.qubit = qubit;
    if (qc->readoutFrequency) p.resonatorHz = qc->readoutFrequency->value.v;
    ...
    else return fail(err::BadInput, std::format("readout: qubit {} has no resonator frequency", qubit));
    p.kappaRadS = kTwoPi * qc->readoutKappa->value.v;
    p.chiRadS = kTwoPi * qc->readoutChi->value.v;
    const double calibratedWindow = qc->readoutDuration.value.v > 0.0 ? qc->readoutDuration.value.v : 700e-9;
    ...
    p.photons = snrPerRootPhoton > 0.0 ? (snrReference / snrPerRootPhoton) * (snrReference / snrPerRootPhoton) : 1.0;
    ...
    p.flip10 = std::max(0.0, q10 - p.decay10 * flipGivenDecay); // … the rest is measurement-induced (Model)
    p.flip01 = q01;
    ...
    if (d.windowS > 0.0) p.windowS = d.windowS;
