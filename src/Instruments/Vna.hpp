...
// saturation Q_i(n̄) and the punch-out of χ above n_crit. The built-in fit is the diameter-correction
// circle fit refined by data::fit's `resonator_notch` model (spec 22 §5).
...
Result<ResonatorFit> fitResonatorNotch(std::span<const double> fHz, std::span<const double> re, std::span<const double> im,
                                       double sigma = 0.0, std::optional<double> nearHz = std::nullopt);
...
public:
    ...
    // span (of the sweep actually taken), P, trace (min |S21| of the last sweep, dB), f_r, kappa, Q_i.
