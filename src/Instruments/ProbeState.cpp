...
void ProbeBase::setProbeChannels(std::vector<ChannelDesc> channels) {
    for (auto& c : channels) c.simulatorOnly = true;
    setChannels(std::move(channels));
}

Result<std::shared_ptr<const qsim::Snapshot>> ProbeBase::stateOf(const AcquireContext& ctx, const InstrumentId& id) {
    if (!ctx.run || !ctx.run->state) return fail(err::NotBound, id.toString() + ": the run view holds no state snapshot");
...
Result<std::vector<std::uint32_t>> parseQubitList(std::string_view text, std::uint32_t nQubits) {
    ...
    while (!text.empty()) {
        ...
        if (!item.empty()) {
            ...
            if (q >= nQubits) return fail(err::BadInput, std::format("qubit {} is outside the {}-qubit register", q, nQubits));
            out.push_back(q);
        }
    ...
    if (out.empty())
        for (std::uint32_t q = 0; q < nQubits; ++q) out.push_back(q);
    return out;
}
...
SettingSchema StateProbe::makeSchema() {
    ...
    s.settings = {
        SettingSpec::text("qubits", "", "Qubits of the `reduced` channel, e.g. \"0,1\" (empty: all)"),
        refreshRateSetting(),
    };
    return s;
}
...
StateProbe::StateProbe(std::uint32_t index) : ProbeBase({"probe_state", index}, makeSchema()) {
    setProbeChannels({
        {{}, "amplitudes", "", "", FidelityClass::Exact, true, true, "State amplitudes: x = basis index (little-endian), y = Re, y_im = Im"},
        {{}, "probabilities", "", "", FidelityClass::Exact, false, true, "Born probabilities per basis index"},
        {{}, "reduced", "", "", FidelityClass::Exact, true, true, "Reduced density matrix of `qubits`: x = row·d + col"},
        {{}, "bloch_x", "", "", FidelityClass::Exact, false, true, "Tr(ρ_q X) per qubit"},
        {{}, "bloch_y", "", "", FidelityClass::Exact, false, true, "Tr(ρ_q Y) per qubit"},
        {{}, "bloch_z", "", "", FidelityClass::Exact, false, true, "Tr(ρ_q Z) per qubit; |0> is +1"},
        {{}, "purity", "", "", FidelityClass::Exact, false, true, "Tr ρ_q² per qubit; marker `global` = Tr ρ²"},
    });
}
...
double globalPurity(const qsim::Snapshot& s) {
    if (s.densityMatrix) return qsim::measures::purity(*s.densityMatrix);
...
std::optional<double> StateProbe::query(std::string_view path) const {
    auto run = runView();
    if (!run || !run->state) return std::nullopt;
    if (path == "purity") return globalPurity(*run->state);
    // qubit[i].bloch[k] / qubit[i].purity — consecutive scalar paths of spec 17 §5.
    if (!path.starts_with("qubit[")) return InstrumentBase::query(path);
    const auto close = path.find(']');
    if (close == std::string_view::npos) return std::nullopt;
    std::uint32_t q = 0;
    if (std::from_chars(path.data() + 6, path.data() + close, q).ec != std::errc{}) return std::nullopt;
...
Result<Trace> StateProbe::doAcquire(const ChannelDesc& channel, AcquireContext& ctx) {
    ...
    t.cls = s.cls; // Exact for state-vector/density-matrix runs, Numerical for Lindblad (spec 12 §12)
    t.markers.push_back({0.0, 0.0, "gate_index", static_cast<double>(s.gateIndex), 0.0, ""});
    if (channel.name == "amplitudes") {
        if (!s.amplitudes) return fail(err::BadInput, id().toString() + ": this backend exposes no amplitudes (use `reduced`)");
    ...
    if (channel.name == "probabilities") {
        if (s.probabilities) t.y = *s.probabilities;
        else if (s.amplitudes) for (auto const& a : *s.amplitudes) t.y.push_back(std::norm(a));
        else if (s.densityMatrix) for (std::size_t i = 0; i < s.densityMatrix->rows; ++i) t.y.push_back((*s.densityMatrix)(i, i).real());
        else return fail(err::BadInput, id().toString() + ": the snapshot holds no probabilities");
        for (std::size_t i = 0; i < t.y.size(); ++i) t.x.push_back(static_cast<double>(i));
        return t;
    }
    if (channel.name == "reduced") {
        auto qubits = parseQubitList(ctx.settings.text("qubits"), s.nQubits);
        if (!qubits) return fail(err::BadInput, id().toString() + ": " + qubits.error().message);
    ...
    for (std::uint32_t q = 0; q < s.nQubits; ++q) {
        ...
        if (channel.name == "purity") { t.y.push_back(qsim::measures::purity(rho)); continue; }
    ...
    if (channel.name == "purity") t.markers.push_back({0.0, 0.0, "global", globalPurity(s), 0.0, ""});
    return t;
}
