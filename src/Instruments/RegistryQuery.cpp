...
LineTap::LineTap(const SignalGraph& graph, std::string upstream,
                 std::shared_ptr<const InputHub> inputs, std::string lineId, cryo::Stage stage)
    : graph_(graph), upstream_(std::move(upstream)), inputs_(std::move(inputs)),
      lineId_(std::move(lineId)), stage_(stage) {}

double LineTap::sampleRateHz(std::string_view) const {
    return graph_.sampleRateAt(upstream_).value_or(0.0);
}
...
Result<Signal> LineTap::signal(std::string_view, const SignalRequest& request) const {
    ...
    if (!env || !env->wiring)
        return fail(err::NotBound, std::format("line tap {}: no wiring is bound", lineId_));
    const cryo::WiringLine* line = env->wiring->find(lineId_);
    if (!line)
        return fail(err::BadRouting, std::format("line tap: the wiring has no line '{}'", lineId_));
    ...
    for (auto const& e : line->elements) {
        if (cryo::stageIndex(e.stage) > cryo::stageIndex(stage_))
            break;
        if (e.kind == cryo::ElementKind::Attenuator)
            lossDb += e.attenuation_dB;
        if (e.kind == cryo::ElementKind::CoaxSegment)
            if (const cryo::CoaxSpec* c = catalogs.coax.find(e.coax))
                lossDb += cryo::coaxLoss_dB(
                    *c, e.length_m, f,
                    temperatures[static_cast<std::size_t>(cryo::stageIndex(e.stage))]);
    }
    ...
    for (auto const& step : budget.steps)
        if (cryo::stageIndex(step.stage) <= cryo::stageIndex(stage_))
            photons = step.n_out;
...
Head splitHead(std::string_view path) {
    ...
    if (bracket != std::string_view::npos && (dot == std::string_view::npos || bracket < dot)) {
        ...
        if (close != std::string_view::npos) {
            std::uint32_t v = 0;
            if (std::from_chars(path.data() + bracket + 1, path.data() + close, v).ec ==
                std::errc{})
                h.index = v;
            h.rest = close + 1 < path.size() && path[close + 1] == '.'
                         ? path.substr(close + 2)
...
std::optional<double> InstrumentRegistry::query(std::string_view path, bool physicalLab) const {
    ...
    if (path.starts_with("run.")) {
        ...
        if (key == "shot")
            return static_cast<double>(run->shot);
        if (key == "shots")
            return static_cast<double>(run->shots);
        if (key == "progress")
            return run->shots ? static_cast<double>(run->shot) / static_cast<double>(run->shots)
                              : 0.0;
        if (key == "gate_cursor")
            return static_cast<double>(run->gateCursor);
        if (key == "wall_time")
            return run->wallTimeS;
        if (key == "playhead")
            return run->playheadS;
        if (key == "running")
            return run->running ? 1.0 : 0.0;
        return std::nullopt;
    }
    const Head h = splitHead(path);
    auto ask = [&](std::string_view kind, std::uint32_t index,
                   std::string_view rest) -> std::optional<double> {
        const IInstrument* i = find(kind, index);
        if (!i || (physicalLab && i->simulatorOnly()))
            return std::nullopt;
        return i->query(rest);
    };
    const std::uint32_t idx = h.index.value_or(0);
    if (h.name == "gen")
        return ask("sg_mw", idx, h.rest);
    if (h.name == "mixer")
        return ask("iq_mixer", idx, h.rest);
    if (h.name == "dig")
        return ask("digitizer", idx, h.rest);
    if (h.name == "dc")
        return ask("dc_source", idx, h.rest);
    if (h.name == "sa")
        return ask("spectrum_analyzer", idx, h.rest);
    if (h.name == "scope")
        return ask("oscilloscope", idx, h.rest);
    if (h.name == "seq")
        return ask("awg", idx, "seq." + std::string(h.rest));
    if (h.name == "ref" || h.name == "trig")
        return ask("controller", idx, h.rest);
    if (h.name == "gauge")
        return ask("pressure_gauge", idx, h.rest);
    if (h.name == "flow")
        return ask("flow_meter", idx, h.rest);
    if (h.name == "pm")
        return ask("power_meter", idx, h.rest);
    if (h.name == "thermo") { // every thermometer in creation order, RuO₂ and Cernox alike
        std::uint32_t k = 0;
        for (auto const& i : instruments_)
            if (i->id().kind.starts_with("thermometer_") && k++ == idx)
                return i->query(h.rest);
        return std::nullopt;
    }
