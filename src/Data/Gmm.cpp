...
Result<Gmm2Result> fitGmm2(std::span<const double> x, std::span<const double> y,
                           const Gmm2Options& opt) {
    const std::size_t n = x.size();
    if (n < 4 || y.size() != n)
        return fail(ErrorCode::Data_ + 20, "gmm2 needs >= 4 (x,y) points");
    Gmm2Result r;
    ...
    if (!opt.labels.empty() && opt.labels.size() == n) {
        ...
        if (std::abs(vx) + std::abs(vy) < 1e-300) {
            vx = 1;
            vy = 0;
        }
        for (std::size_t i = 0; i < n; ++i)
            resp[i] = ((x[i] - mx) * vx + (y[i] - my) * vy) > 0 ? 1.0 : 0.0;
    }
    ...
    for (int it = 0; it < opt.maxIterations; ++it) {
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            c.weight = w / static_cast<double>(n);
            c.mx = sx / w;
            c.my = sy / w;
            double sxx = 0, sxy = 0, syy = 0;
            for (std::size_t i = 0; i < n; ++i) {
                double g = k ? resp[i] : 1 - resp[i];
                double dx = x[i] - c.mx, dy = y[i] - c.my;
                sxx += g * dx * dx;
                sxy += g * dx * dy;
                syy += g * dy * dy;
            }
        ...
        if (opt.sharedCovariance) {
            ...
            double sxx = w0 * r.comp[0].sxx + w1 * r.comp[1].sxx,
                   sxy = w0 * r.comp[0].sxy + w1 * r.comp[1].sxy,
                   syy = w0 * r.comp[0].syy + w1 * r.comp[1].syy;
            for (auto& c : r.comp) {
                c.sxx = sxx;
                c.sxy = sxy;
                c.syy = syy;
        ...
        r.iterations = it + 1;
        r.logLikelihood = ll;
        if (std::abs(ll - prevLL) < opt.tol * std::abs(ll)) {
            r.converged = true;
            break;
        }
        prevLL = ll;
    ...
    if (r.comp[0].mx > r.comp[1].mx)
        std::swap(r.comp[0], r.comp[1]);
    // Fisher LDA: w = Σ^{-1} (μ1 - μ0) with pooled covariance.
    double sxx = (r.comp[0].sxx + r.comp[1].sxx) / 2, sxy = (r.comp[0].sxy + r.comp[1].sxy) / 2,
           syy = (r.comp[0].syy + r.comp[1].syy) / 2;
    double det = sxx * syy - sxy * sxy;
...
std::array<std::array<double, 2>, 2> assignmentMatrix(std::span<const double> x,
                                                      std::span<const double> y,
                                                      std::span<const int> prepared,
                                                      const Gmm2Result& g) {
    std::array<std::array<double, 2>, 2> m{{{0, 0}, {0, 0}}};
    std::array<double, 2> tot{0, 0};
    for (std::size_t i = 0; i < x.size(); ++i) {
        int p = prepared[i] ? 1 : 0;
        int r = g.classify(x[i], y[i]);
        m[static_cast<std::size_t>(p)][static_cast<std::size_t>(r)] += 1;
        tot[static_cast<std::size_t>(p)] += 1;
    }
