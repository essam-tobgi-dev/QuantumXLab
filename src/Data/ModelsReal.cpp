...
struct Stats {
    double ymin, ymax, ymean, xmin, xmax, yFirst, yLast;
...
Stats stats(std::span<const double> x, std::span<const double> y) {
    ...
    s.ymin = *mn;
    s.ymax = *mx;
    s.imin = static_cast<std::size_t>(mn - y.begin());
    s.imax = static_cast<std::size_t>(mx - y.begin());
    ...
    for (double v : y)
        s.ymean += v;
    s.ymean /= static_cast<double>(y.size());
    s.yFirst = y.front();
    s.yLast = y.back();
    return s;
}
...
double ExpDecayModel::eval(std::span<const double> b, double x) const {
    return b[0] * std::exp(-x / b[1]) + b[2];
}
...
std::vector<double> ExpDecayModel::initialGuess(std::span<const double> x,
...
bool RamseyModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    double e = std::exp(-x / b[1]), arg = 2 * kPi * b[2] * x + b[3], cs = std::cos(arg),
           sn = std::sin(arg);
    o[0] = e * cs;
    o[1] = b[0] * e * cs * x / (b[1] * b[1]);
    o[2] = -b[0] * e * sn * 2 * kPi * x;
    o[3] = -b[0] * e * sn;
    o[4] = 1.0;
    return true;
}
std::vector<double> RamseyModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                              std::span<const double>) const {
    Stats s = stats(x, y);
    double ph = 0, amp = 0;
    ...
    for (std::size_t i = 1; i + 1 < y.size(); ++i) {
        ...
        if (d >= std::abs(y[i - 1] - s.ymean) && d >= std::abs(y[i + 1] - s.ymean)) {
            ex.push_back(x[i]);
            ey.push_back(d + s.ymean);
        }
    }
...
// ---------------------------------------------------------------- echo
std::vector<ParamInfo> EchoModel::params() const {
    return {{"A", "", {}}, {"T", "", {0.0, kInf}}, {"c", "", {}}};
}
double EchoModel::eval(std::span<const double> b, double x) const {
    return b[0] * std::exp(-std::pow(x / b[1], n_)) + b[2];
}
bool EchoModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    double u = x / b[1], e = std::exp(-std::pow(u, n_));
    o[0] = e;
    o[1] = b[0] * e * n_ * std::pow(u, n_) / b[1];
    o[2] = 1.0;
    return true;
}
std::vector<double> EchoModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                            std::span<const double>) const {
    Stats s = stats(x, y);
    double c = s.yLast, A = s.yFirst - s.yLast;
    if (std::abs(A) < 1e-12)
    ...
    if (n_ == 2.0)
        T *= std::sqrt(kPi) / 2.0; // e^{-t/T} vs e^{-(t/T)^2}: match areas
    return {A, T, c};
}

// ---------------------------------------------------------------- rabi_amp
std::vector<ParamInfo> RabiAmpModel::params() const {
    return {{"A", "", {}}, {"a_pi", "", {0.0, kInf}}, {"c", "", {}}};
}
double RabiAmpModel::eval(std::span<const double> b, double x) const {
    return b[0] * std::cos(kPi * x / b[1]) + b[2];
}
bool RabiAmpModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    double arg = kPi * x / b[1];
    o[0] = std::cos(arg);
    o[1] = b[0] * std::sin(arg) * kPi * x / (b[1] * b[1]);
    o[2] = 1.0;
    return true;
}
std::vector<double> RabiAmpModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                               std::span<const double>) const {
    Stats s = stats(x, y);
...
bool RabiTimeModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    double arg = 2 * kPi * b[1] * x + b[2], cs = std::cos(arg), sn = std::sin(arg),
           e = std::exp(-x / b[3]);
    o[0] = cs * e;
    o[1] = -b[0] * sn * 2 * kPi * x * e;
    o[2] = -b[0] * sn * e;
    o[3] = b[0] * cs * e * x / (b[3] * b[3]);
    o[4] = 1.0;
    return true;
}
std::vector<double> RabiTimeModel::initialGuess(std::span<const double> x,
                                                std::span<const double> y,
                                                std::span<const double>) const {
    RamseyModel r;
    auto g = r.initialGuess(x, y, {});
    return {g[0], g[2], g[3], g[1], g[4]};
}

// ---------------------------------------------------------------- lorentzian
std::vector<ParamInfo> LorentzianModel::params() const {
    return {{"A", "", {}}, {"x0", "", {}}, {"gamma", "", {0.0, kInf}}, {"c", "", {}}};
}
...
bool LorentzianModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    ...
    o[0] = L;
    o[1] = b[0] * 2 * d * h * h / (den * den);
    o[2] = b[0] * 0.5 * (2 * h * den - h * h * 2 * h) / (den * den);
    o[3] = 1.0;
    return true;
}
std::vector<double> LorentzianModel::initialGuess(std::span<const double> x,
                                                  std::span<const double> y,
                                                  std::span<const double>) const {
    Stats s = stats(x, y);
    // Decide peak vs dip by which extreme is farther from the median.
    std::vector<double> ys(y.begin(), y.end());
    std::nth_element(ys.begin(), ys.begin() + static_cast<std::ptrdiff_t>(ys.size() / 2), ys.end());
    ...
    while (lo > 0 && (peak ? y[lo] > half : y[lo] < half))
        --lo;
    while (hi + 1 < y.size() && (peak ? y[hi] > half : y[hi] < half))
        ++hi;
    double gamma = std::abs(x[hi] - x[lo]);
    if (gamma <= 0)
        gamma = (s.xmax - s.xmin) / 10;
    return {A, x[ip], gamma, med};
}

// ---------------------------------------------------------------- gaussian
std::vector<ParamInfo> GaussianModel::params() const {
    return {{"A", "", {}}, {"x0", "", {}}, {"sigma", "", {0.0, kInf}}, {"c", "", {}}};
}
...
std::vector<double> GaussianModel::initialGuess(std::span<const double> x,
                                                std::span<const double> y,
                                                std::span<const double> yi) const {
    LorentzianModel l;
    auto g = l.initialGuess(x, y, yi);
    return {g[0], g[1], g[2] / 2.3548, g[3]};
}

// ---------------------------------------------------------------- rb_decay
std::vector<ParamInfo> RbDecayModel::params() const {
    return {{"A", "", {}}, {"p", "", {0.0, 1.0}}, {"B", "", {}}};
}
double RbDecayModel::eval(std::span<const double> b, double x) const {
    return b[0] * std::pow(b[1], x) + b[2];
}
bool RbDecayModel::jacobian(std::span<const double> b, double x, std::span<double> o) const {
    double pm = std::pow(b[1], x);
    o[0] = pm;
    o[1] = b[1] > 0 ? b[0] * x * pm / b[1] : 0.0;
    o[2] = 1.0;
    return true;
}
std::vector<double> RbDecayModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                               std::span<const double>) const {
    Stats s = stats(x, y);
...
std::vector<std::pair<std::string, double>> RbDecayModel::derived(std::span<const double> b) const {
    std::vector<std::pair<std::string, double>> out{{"r", rbErrorFromP(b[1], nq_)},
                                                    {"F_avg", 1.0 - rbErrorFromP(b[1], nq_)}};
    if (il_) {
        double d = static_cast<double>(1u << nq_);
...
// ---------------------------------------------------------------- linear
std::vector<ParamInfo> LinearModel::params() const {
    return {{"a", "", {}}, {"b", "", {}}};
}
double LinearModel::eval(std::span<const double> b, double x) const {
    return b[0] + b[1] * x;
}
bool LinearModel::jacobian(std::span<const double>, double x, std::span<double> o) const {
    o[0] = 1.0;
    o[1] = x;
    return true;
}
std::vector<double> LinearModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                              std::span<const double>) const {
    double sx = 0, sy = 0, sxx = 0, sxy = 0;
...
std::unique_ptr<FitModel> makeModel(std::string_view id) {
    if (id == "exp_decay")
        return std::make_unique<ExpDecayModel>();
    if (id == "ramsey")
        return std::make_unique<RamseyModel>();
    if (id == "echo")
        return std::make_unique<EchoModel>(1.0);
    if (id == "echo_gauss")
        return std::make_unique<EchoModel>(2.0);
    if (id == "rabi_amp")
        return std::make_unique<RabiAmpModel>();
    if (id == "rabi_time")
        return std::make_unique<RabiTimeModel>();
    if (id == "lorentzian")
        return std::make_unique<LorentzianModel>();
    if (id == "gaussian")
        return std::make_unique<GaussianModel>();
    if (id == "resonator_notch")
        return std::make_unique<ResonatorNotchModel>();
    if (id == "rb_decay")
        return std::make_unique<RbDecayModel>(1, false);
    if (id == "rb_decay_2q")
        return std::make_unique<RbDecayModel>(2, false);
    if (id == "rb_interleaved")
        return std::make_unique<RbDecayModel>(1, true);
    if (id == "linear")
        return std::make_unique<LinearModel>();
    return nullptr;
}
std::vector<std::string> modelIds() {
    return {"exp_decay",       "ramsey",    "echo",        "echo_gauss",
            "rabi_amp",        "rabi_time", "lorentzian",  "gaussian",
            "resonator_notch", "rb_decay",  "rb_decay_2q", "rb_interleaved",
            "linear",          "gmm2"};
}
