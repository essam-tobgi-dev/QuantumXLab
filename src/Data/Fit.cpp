...
double dominantFrequency(std::span<const double> x, std::span<const double> y, double* phase,
                         double* amplitude) {
    std::size_t n = x.size();
    if (n < 4) {
        if (phase)
    ...
    if (span <= 0)
        return 0.0;
    // Minimum spacing sets the Nyquist limit; scan frequencies from 1/(2 span) to 1/(2 dxmin) on a
    // fine grid.
    ...
    for (std::size_t i = 1; i < n; ++i)
        if (xs[i] - xs[i - 1] > 0)
            dxmin = std::min(dxmin, xs[i] - xs[i - 1]);
    ...
    for (std::size_t k = 1; k <= m; ++k) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            re += (y[i] - mean) * std::cos(a);
            im -= (y[i] - mean) * std::sin(a);
        }
        double mag = re * re + im * im;
        if (mag > best) {
            best = mag;
            bestF = f;
            bestPh = std::atan2(im, re) - 2.0 * std::numbers::pi * f * xmin;
        }
    }
    if (phase) {
        double p = std::fmod(bestPh, 2 * std::numbers::pi);
        if (p > std::numbers::pi)
            p -= 2 * std::numbers::pi;
        if (p < -std::numbers::pi)
            p += 2 * std::numbers::pi;
    ...
    if (amplitude)
        *amplitude = 2.0 * std::sqrt(best) / static_cast<double>(n);
    return bestF;
}
...
struct Transform {
    ...
    double toBeta(double th) const {
        ...
        case BType::Free:
            return th;
        case BType::Box:
            return lo + (hi - lo) / (1.0 + std::exp(-th));
        case BType::Lower:
            return lo + std::exp(th);
        case BType::Upper:
            return hi - std::exp(th);
        }
        return th;
    }
    double toTheta(double b) const {
        ...
        case BType::Free:
            return b;
        case BType::Box: {
            double u = std::clamp((b - lo) / (hi - lo), 1e-9, 1 - 1e-9);
            return std::log(u / (1 - u));
        }
        case BType::Lower:
            return std::log(std::max(b - lo, 1e-300));
        case BType::Upper:
            return std::log(std::max(hi - b, 1e-300));
        }
        return b;
    }
...
Result<FitResult> fitModel(const FitModel& model, std::span<const double> x,
    ...
    if (n == 0 || y.size() != n)
        return fail(ErrorCode::Data_ + 10, "x/y size mismatch or empty");
    if (cplx && yIm.size() != n)
        return fail(ErrorCode::Data_ + 11, "complex model requires yIm");
    if (!sigma.empty() && sigma.size() != n)
        return fail(ErrorCode::Data_ + 12, "sigma size mismatch");
    const std::size_t m = cplx ? 2 * n : n;
    if (m <= p)
        return fail(ErrorCode::Data_ + 13, std::format("{} residuals for {} parameters", m, p));

    auto infos = model.params();
    std::vector<double> beta = opt.initial ? *opt.initial : model.initialGuess(x, y, yIm);
    if (beta.size() != p)
        return fail(ErrorCode::Data_ + 14, "initial guess size mismatch");
    ...
    for (std::size_t j = 0; j < std::min(p, opt.fixed.size()); ++j)
        fixed[j] = opt.fixed[j];
    std::vector<double> theta(p);
    for (std::size_t j = 0; j < p; ++j)
        theta[j] = tr[j].toTheta(beta[j]);
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            for (std::size_t j = 0; j < p; ++j) {
                ...
                if (ok) {
                    ...
                    bp[j] = b[j];
                    dre = (fp.real() - fm.real()) / (2 * h);
                    dim = (fp.imag() - fm.imag()) / (2 * h);
                }
                double chain = fixed[j] ? 0.0 : tr[j].dBetaDTheta(theta[j]);
                if (cplx) {
                    J[2 * i][j] = -dre * chain * w(i);
                    J[2 * i + 1][j] = -dim * chain * w(i);
                } else
                    J[i][j] = -dre * chain * w(i);
            }
        }
    ...
    for (int it = 0; it < opt.maxIterations; ++it) {
        ...
        for (std::size_t i = 0; i < m; ++i)
            for (std::size_t j = 0; j < p; ++j) {
                Jtr[j] -=
                    J[i][j] * r[i]; // gradient direction: minimize sum r^2, r = y - f => -J^T r
        ...
        for (int tries = 0; tries < 30 && !accepted; ++tries) {
            ...
            for (std::size_t j = 0; j < p; ++j)
                if (fixed[j])
                    rhs[j] = 0;
            if (!linalg::solveSpd(A, rhs, delta)) {
                lambda *= opt.lambdaUp;
                continue;
            }
            for (std::size_t j = 0; j < p; ++j) {
                thetaNew[j] = theta[j] + delta[j];
                betaNew[j] = tr[j].toBeta(thetaNew[j]);
            }
            ...
            if (std::isfinite(chi2New) && chi2New < chi2) {
                ...
                accepted = true;
                if (dchi < opt.chi2RelTol * std::max(chi2, 1e-300) ||
                    std::sqrt(dn) < opt.stepRelTol * std::max(std::sqrt(tn), 1e-300)) {
                    res.converged = true;
                    msg = "converged";
                }
            } else
        ...
        if (!accepted) {
            res.converged = true;
            msg = "no improving step (converged at tolerance)";
            break;
        }
        if (res.converged)
            break;
    ...
    res.chi2 = chi2;
    res.chi2ndf = res.ndf > 0 ? chi2 / res.ndf : 0;
    ...
    res.sigma.resize(p);
    for (std::size_t j = 0; j < p; ++j)
        res.sigma[j] = std::sqrt(std::max(0.0, cov[j][j]));
    res.residuals = r;
    res.message = msg;
    for (auto& pi : infos)
        res.paramNames.push_back(pi.name);
    res.atBound.resize(p);
    for (std::size_t j = 0; j < p; ++j) {
        double lo = tr[j].lo, hi = tr[j].hi, sc = std::max(1.0, std::abs(beta[j]));
        res.atBound[j] = (std::isfinite(lo) && std::abs(beta[j] - lo) < 1e-6 * sc) ||
                         (std::isfinite(hi) && std::abs(hi - beta[j]) < 1e-6 * sc);
    }
