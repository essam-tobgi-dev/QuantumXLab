...
struct Marker { double x; std::string label; };
...
public:
    ...
    ChannelId addDerived(ChannelDesc desc, ChannelId source,
    ...
    Status push(ChannelId id, double x, double y) { return push(id, x, std::span<const double>(&y, 1)); }
...
private:
    struct Entry {
        ...
        std::vector<Marker> markers;
        std::vector<std::pair<ChannelId, std::function<std::vector<double>(double, std::span<const double>)>>> derived;
    };
