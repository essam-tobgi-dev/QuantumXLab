...
std::vector<ParamInfo> ResonatorNotchModel::params() const {
    return {{"f_r", "Hz", {0.0, kInf}}, {"Q_l", "", {1.0, kInf}}, {"Q_c", "", {1.0, kInf}},
            {"phi", "rad", {-kPi, kPi}}, {"a", "", {0.0, kInf}}, {"alpha", "rad", {}}, {"tau", "s", {}}};
}
...
bool ResonatorNotchModel::jacobianC(std::span<const double> b, double f, std::span<Complex> o) const {
    const double fr = b[0], Ql = b[1], Qc = b[2], phi = b[3], a = b[4], alpha = b[5], tau = b[6];
    const Complex env = a * std::exp(kI * (alpha - 2.0 * kPi * f * tau));
    const Complex den = 1.0 + 2.0 * kI * Ql * (f - fr) / fr;
...
std::vector<double> ResonatorNotchModel::initialGuess(std::span<const double> x, std::span<const double> y,
                                                       std::span<const double> yIm) const {
    const std::size_t n = x.size();
    std::vector<double> mag(n);
    for (std::size_t i = 0; i < n; ++i) mag[i] = std::hypot(y[i], yIm.empty() ? 0.0 : yIm[i]);
    // Baseline amplitude from the edges; f_r from the minimum of |S21|.
    std::size_t edge = std::max<std::size_t>(1, n / 10);
    double a = 0; for (std::size_t i = 0; i < edge; ++i) a += mag[i] + mag[n - 1 - i]; a /= static_cast<double>(2 * edge);
    if (a <= 0) a = 1.0;
    std::size_t imin = static_cast<std::size_t>(std::min_element(mag.begin(), mag.end()) - mag.begin());
    double fr = x[imin];
    ...
    while (lo > 0 && mag[lo] < half) --lo;
    while (hi + 1 < n && mag[hi] < half) ++hi;
...
std::vector<std::pair<std::string, double>> ResonatorNotchModel::derived(std::span<const double> b) const {
