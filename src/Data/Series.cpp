...
std::vector<std::pair<double, double>> decimate(std::span<const double> x,
    ...
    if (x.empty() || maxPoints == 0)
        return out;
    auto lo = std::lower_bound(x.begin(), x.end(), x0);
    auto hi = std::upper_bound(x.begin(), x.end(), x1);
    std::size_t b = static_cast<std::size_t>(lo - x.begin()),
                e = static_cast<std::size_t>(hi - x.begin());
    if (e <= b)
        return out;
    std::size_t n = e - b;
    if (n <= maxPoints) {
    ...
    if (span <= 0)
        span = x[e - 1] - x[b];
    if (span <= 0) {
        out.emplace_back(x[b], y[b]);
        return out;
    }
    out.reserve(2 * buckets);
    std::size_t i = b;
    for (std::size_t k = 0; k < buckets && i < e; ++k) {
        double bx1 = (k + 1 == buckets)
                         ? std::numeric_limits<double>::infinity()
                         : x0 + span * static_cast<double>(k + 1) / static_cast<double>(buckets);
        ...
        if (j == i)
            continue;
        if (imin == imax)
            out.emplace_back(x[imin], y[imin]);
        else if (imin < imax) {
            out.emplace_back(x[imin], y[imin]);
            out.emplace_back(x[imax], y[imax]);
        } else {
            out.emplace_back(x[imax], y[imax]);
            out.emplace_back(x[imin], y[imin]);
        }
...
ChannelId Recorder::add(ChannelDesc desc) {
    ...
    Entry e;
    e.desc = desc;
    e.y.assign(std::max<std::size_t>(1, desc.arity), {});
...
ChannelId
Recorder::addDerived(ChannelDesc desc, ChannelId source,
...
Status Recorder::push(ChannelId id, double x, std::span<const double> y) {
    std::vector<std::pair<ChannelId, std::vector<double>>> cascade;
    {
        std::lock_guard lk(mu_);
        if (frozen_)
            return fail(ErrorCode::Data_ + 1, "recorder is frozen");
        if (id.get() >= entries_.size())
            return fail(ErrorCode::Data_ + 2, "unknown channel");
        Entry& e = entries_[id.get()];
        if (y.size() != e.y.size())
            return fail(ErrorCode::Data_ + 3,
                        std::format("channel '{}' arity {} but {} values pushed", e.desc.id,
                                    e.y.size(), y.size()));
        e.x.push_back(x);
        for (std::size_t k = 0; k < y.size(); ++k)
            e.y[k].push_back(y[k]);
        for (auto& [did, fn] : e.derived)
            cascade.emplace_back(did, fn(x, y));
    }
    for (auto& [did, vals] : cascade) {
        auto st = push(did, x, vals);
        if (!st)
            return st;
    }
    return {};
...
void Recorder::mark(ChannelId id, double x, std::string label) {
    std::lock_guard lk(mu_);
    if (id.get() < entries_.size())
        entries_[id.get()].markers.push_back({x, std::move(label)});
}
void Recorder::reserve(ChannelId id, std::size_t n) {
    std::lock_guard lk(mu_);
    if (id.get() >= entries_.size())
        return;
    auto& e = entries_[id.get()];
    e.x.reserve(n);
    for (auto& c : e.y)
        c.reserve(n);
}
