...
public:
    ...
    std::vector<double> initialGuess(std::span<const double> x, std::span<const double> y, std::span<const double>) const override;
};
...
public:
    ...
    std::string latex() const override { return n_ == 2.0 ? "A e^{-(t/T_{2E})^2} + c" : "A e^{-t/T_{2E}} + c"; }
    std::vector<ParamInfo> params() const override;
    double eval(std::span<const double> b, double x) const override;
    bool jacobian(std::span<const double> b, double x, std::span<double> out) const override;
    std::vector<double> initialGuess(std::span<const double> x, std::span<const double> y, std::span<const double>) const override;
...
public:
    ...
    std::string latex() const override { return "A\\frac{(\\Gamma/2)^2}{(x-x_0)^2+(\\Gamma/2)^2} + c"; }
    std::vector<ParamInfo> params() const override;
    double eval(std::span<const double> b, double x) const override;
    bool jacobian(std::span<const double> b, double x, std::span<double> out) const override;
    std::vector<double> initialGuess(std::span<const double> x, std::span<const double> y, std::span<const double>) const override;
...
public:
    explicit RbDecayModel(int nqubits = 1, bool interleaved = false) : nq_(nqubits), il_(interleaved) {}
...
private:
    int nq_; bool il_; double pRef_ = 1.0;
};
...
// Parameters: f_r, Q_l, Q_c (|Q_c|), phi, a, alpha, tau.  Derived: Q_i = 1/(1/Q_l - Re(1/Q_c e^{-iφ})).
...
public:
    ...
    std::string latex() const override {
        return "S_{21}(f) = a e^{i\\alpha} e^{-2\\pi i f\\tau}\\left[1 - \\frac{(Q_l/|Q_c|)e^{i\\phi}}{1 + 2iQ_l(f-f_r)/f_r}\\right]";
    }
    std::vector<ParamInfo> params() const override;
    ...
    std::vector<double> initialGuess(std::span<const double> x, std::span<const double> y, std::span<const double> yIm) const override;
    std::vector<std::pair<std::string, double>> derived(std::span<const double> b) const override;
