...
void Histogram::add(std::uint64_t index, std::uint64_t count) {
    add(labelFromIndex(index, nbits_), count);
}
...
Histogram Histogram::marginal(std::span<const std::size_t> qubits) const {
    ...
    for (auto& [label, c] : counts_) {
        ...
        // Output label convention: position j (from the right, i.e. output qubit j) is input qubit
        // qubits[j].
...
Histogram1D Histogram1D::build(std::span<const double> values, std::size_t bins) {
    ...
    if (bins == 0) {
        ...
        if (w <= 0 || hi <= lo)
            ...
            bins = std::clamp<std::size_t>(static_cast<std::size_t>(std::ceil((hi - lo) / w)), 1,
                                           4096);
    }
    if (hi <= lo) {
        hi = lo + 1.0;
...
Histogram2D Histogram2D::build(std::span<const double> xs, std::span<const double> ys,
                               std::size_t nx, std::size_t ny,
                               std::optional<std::array<double, 4>> range) {
    Histogram2D h;
    h.nx = nx;
    h.ny = ny;
    h.counts.assign(nx * ny, 0);
