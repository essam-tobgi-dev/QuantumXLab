...
public:
    DistanceSearch(const StabilizerCode& code, std::uint64_t budget) : n_(code.n), budget_(budget) {
        for (const PauliString& g : code.stabilizers) gens_.push_back(detail::toMask(g));
    ...
    Result<Found> minimum(Letters letters) {
        ...
        for (std::uint32_t w = 1; w <= n_; ++w) {
            for (std::uint64_t s = (std::uint64_t{1} << w) - 1; s != 0; s = detail::nextSubset(s, limit)) {
                Mask hit;
                QXL_TRY_ASSIGN(const bool found, scanSupport(s, w, letters, hit));
                if (found) return Found{w, hit};
            }
        }
        return Found{};
    }
...
private:
    bool isLogical(Mask m) const {
        for (Mask g : gens_)
            if (!detail::commute(m, g)) return false;
    ...
    Result<bool> scanSupport(std::uint64_t support, std::uint32_t w, Letters letters, Mask& hit) {
        ...
        for (;;) {
            ...
            if (isLogical(m)) { hit = m; return true; }
            std::uint32_t i = 0;
    ...
    bool spend(std::uint64_t c) {
        if (budget_ < c) return false;
        budget_ -= c;
        return true;
    }
    static std::unexpected<Error> exhausted() {
        return fail(err::TooLarge, "exhaustive distance search exceeds its budget of candidate strings");
    }
...
Result<DistanceReport> computeDistance(const StabilizerCode& code) {
    if (code.n == 0 || code.n > 62)
        return fail(err::TooLarge, std::format("exhaustive distance search supports 1 ≤ n ≤ 62, code '{}' has n = {}",
                                               code.id, code.n));
    for (const PauliString& g : code.stabilizers)
        if (g.n != code.n)
            return fail(err::BadPauli, std::format("code '{}': generator '{}' has length {}, expected {}", code.id,
                                                   g.str(), g.n, code.n));
    ...
    if (code.isCss()) {
        ...
        // Z-type: if X(a)Z(b) is a non-trivial logical, X(a) and Z(b) are both in the normalizer and
        // at least one of them is outside the stabilizer group (T09 §4.6).
        ...
        if (fx.weight == 0 && fz.weight == 0)
            return fail(err::BadLogical, std::format("code '{}' has no non-trivial logical operator", code.id));
        ...
        const Found declared = (hasZ && !hasX && fx.weight != 0) ? fx : (hasX && !hasZ && fz.weight != 0) ? fz : best;
