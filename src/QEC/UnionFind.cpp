...
UnionFindDecoder::UnionFindDecoder(DecodingGraph graph, UnionFindOptions options)
    ...
    if (graph_.incident.size() != real_) graph_.finalize(false);
    ...
    for (const GraphEdge& ge : graph_.edges) minWeight = minWeight == 0.0 ? ge.weight : std::min(minWeight, ge.weight);
    const double unit = std::max<std::uint32_t>(1u, options_.resolution) / (minWeight > 0.0 ? minWeight : 1.0);
    std::uint32_t common = 0;
    for (std::uint32_t e = 0; e < nEdges && options_.weighted; ++e) {
        capacity_[e] = std::max<std::uint32_t>(1u, static_cast<std::uint32_t>(std::lround(unit * graph_.edges[e].weight)));
        common = std::gcd(common, capacity_[e]);
    }
    for (std::uint32_t e = 0; e < nEdges; ++e) {
        capacity_[e] = options_.weighted ? 2 * (capacity_[e] / std::max<std::uint32_t>(1u, common)) : 2;
        if (!graph_.edges[e].boundary()) continue;
    ...
    for (auto* flags : {&inCluster_, &fired_, &odd_, &atBoundary_, &visited_}) flags->assign(total, 0);
...
std::uint32_t UnionFindDecoder::farEnd(std::uint32_t edge, std::uint32_t from) const {
    ...
    if (e.boundary()) return from == e.a ? virtualOf_[edge] : e.a;
    return from == e.a ? e.b : e.a;
}

void UnionFindDecoder::enter(std::uint32_t v) {
    if (inCluster_[v]) return;
    inCluster_[v] = 1;
    parent_[v] = v;
    size_[v] = 1;
    odd_[v] = fired_[v];
    ...
    frontier_[v].clear();
    if (v < real_) frontier_[v].push_back(v);   // a virtual vertex has nothing further to grow into
...
Status UnionFindDecoder::grow() {
    ...
    for (std::uint32_t root : active_) {
        ...
        for (std::uint32_t v : frontier_[root]) {
            ...
            for (std::uint32_t e : graph_.incident[v]) {
                if (growth_[e] >= capacity_[e]) continue;
                if (growth_[e] == 0) touchedEdges_.push_back(e);
                grew = true;
                if (++growth_[e] >= capacity_[e]) newlyFull_.push_back(e);
                else open = true;
            }
            if (open) keep_.push_back(v);
        }
        frontier_[root].swap(keep_);
    }
    if (!grew)
        return fail(err::DecodeFailed, "union-find: an odd cluster can neither grow nor reach a boundary (the detection "
    ...
    for (std::uint32_t& root : active_) root = find(root);
...
Status UnionFindDecoder::peel(Correction& out) {
    ...
    auto sweep = [&](bool useBoundary) {
        for (; head < order_.size(); ++head) {
            ...
            auto visit = [&](std::uint32_t e) {
                if (growth_[e] < capacity_[e] || (!useBoundary && graph_.edges[e].boundary())) return;
                const std::uint32_t w = farEnd(e, v);
                if (visited_[w]) return;
                visited_[w] = 1;
                treeEdge_[w] = e;
                treeParent_[w] = v;
                order_.push_back(w);
            };
            if (v >= real_) visit(virtualEdge_[v - real_]);
            else for (std::uint32_t e : graph_.incident[v]) visit(e);
        }
    };
    for (std::uint32_t v : touchedVertices_)
        if (v >= real_ && odd_[find(v)]) seed(v);
    sweep(true);
    for (std::uint32_t v : firedList_)
        if (!visited_[v]) { seed(v); sweep(false); }
    ...
    for (std::size_t i = order_.size(); i-- > 0;) {
        ...
        if (treeEdge_[v] == kNoIndex) {
            if (v < real_ && fired_[v]) return fail(err::DecodeFailed, "union-find: a cluster without boundary kept odd parity");
            continue;
        }
        if (!fired_[v]) continue;
        const GraphEdge& e = graph_.edges[treeEdge_[v]];
        fired_[v] = 0;
        if (treeParent_[v] < real_) fired_[treeParent_[v]] ^= 1u;
        out.edges.push_back({e.a, e.b});
        out.observableMask ^= e.observableMask;
        for (const auto& [q, letter] : e.correction) {
            if (q >= graph_.nData) continue;
            if (letter == 'X' || letter == 'Y') x[q] ^= 1u;
            if (letter == 'Z' || letter == 'Y') z[q] ^= 1u;
        }
    }
...
void UnionFindDecoder::reset() {
    ...
    for (std::uint32_t e : touchedEdges_) growth_[e] = 0;
...
Result<Correction> UnionFindDecoder::decode(const SyndromeLattice& lattice) {
    if (lattice.events.size() != real_)
        return fail(err::BadSyndrome, std::format("union-find decoder expects {} detection events, got {}", real_, lattice.events.size()));
    reset();
    growthRounds_ = 0;
    ...
    while (!active_.empty()) {
        ++growthRounds_;
        if (auto ok = grow(); !ok) { reset(); return std::unexpected(std::move(ok.error())); }
    }
    Status peeled = peel(out);
    reset();
    if (!peeled) return std::unexpected(std::move(peeled.error()));
    return out;
}

Result<std::unique_ptr<IDecoder>> makeCodeCapacityDecoder(std::string_view name, const StabilizerCode& code) {
    if (name == "lookup") {
    ...
    return fail(ErrorCode::Unsupported, std::format("decoder '{}' is not available (spec 16 §5: lookup, union_find; mwpm is MAY)", name));
}
