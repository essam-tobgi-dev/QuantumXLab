...
// inversion (7.2), the T09 §9 recipe with 15-to-1 factories, and the sequential-T model of spec 16 §7.
...
Status requirePositive(double v, std::string_view what) {
    if (!(std::isfinite(v) && v > 0.0)) return fail(err::BadOptions, std::format("{} must be positive and finite, got {}", what, v));
    return {};
}
} // namespace
...
double ResourceEstimator::factoryQubits(std::uint32_t dF) const { return model_.factoryQubitsPerD2 * double(dF) * double(dF); }
double ResourceEstimator::factoryCyclesPerT(std::uint32_t dF) const { return model_.factoryCyclesPerD * double(dF); }

double ResourceEstimator::distilledError(double pIn, std::uint32_t levels) const {
    double p = pIn;
    for (std::uint32_t l = 0; l < levels; ++l) p = std::min(1.0, model_.distillationPrefactor * p * p * p);
    return p;
}
...
Result<std::uint32_t> ResourceEstimator::requiredDistance(double target, double p) const {
    ...
    if (p >= model_.pThreshold)
        return fail(err::AboveThreshold, std::format("physical error rate {} is not below the threshold {}: no distance "
                                                     "reaches the target (T09 §7)", p, model_.pThreshold));
    ...
    if (half > (kMaxDistance + 1) / 2.0)
        return fail(err::TooLarge, std::format("target {} needs a distance above {}", target, kMaxDistance));
...
Result<ResourceEstimate> ResourceEstimator::estimate(const ResourceInput& in) const {
    ...
    if (in.physicalErrorRate >= model_.pThreshold)
        return fail(err::AboveThreshold, std::format("physical error rate {} is not below the threshold {}",
    ...
    for (std::uint32_t d = 3; d <= kMaxDistance; d += 2) {
        ...
        const std::uint32_t nF = in.factories != 0 ? in.factories : std::max<std::uint32_t>(1u, automatic);
        ...
        const double target = in.failureBudget / (in.logicalQubits * cycles);    // union bound, T12 (6.1)
        if (logicalErrorPerCycle(d, in.physicalErrorRate) > target * kSlack) continue;
        r.distance = d;
    ...
    if (r.distance == 0) return fail(err::TooLarge, std::format("no distance up to {} meets the failure budget", kMaxDistance));
    r.logicalErrorPerCycle = logicalErrorPerCycle(r.distance, in.physicalErrorRate);
    r.syndromeCycles = r.logicalCycles * double(r.distance);   // one logical cycle = d rounds (T09 §8.2)
    ...
    r.factoryQubits = in.factoryQubitsOverride ? *in.factoryQubitsOverride : double(r.factories) * factoryQubits(r.factoryDistance);
    ...
    r.assumptions = {"rotated surface code, 2d² − 1 physical qubits per logical qubit",
                     std::format("(7.1) heuristic scaling p_L = A (p/p_th)^((d+1)/2) with A = {}, p_th = {} (MWPM-class decoding)",
                     ...
                     "cycle time from the device (qec.cycle_time_us); errors independent, p = mean two-qubit gate error"};
    return r;
}
...
Result<ResourceEstimate> ResourceEstimator::estimateSequential(const SequentialInput& in) const {
    ...
    QXL_TRY(requirePositive(in.failureBudget, "failure budget"));
    if (in.factories == 0) return fail(err::BadOptions, "the sequential model needs at least one factory");
    if (in.physicalErrorRate >= model_.pThreshold)
        return fail(err::AboveThreshold, std::format("physical error rate {} is not below the threshold {}",
                                                     in.physicalErrorRate, model_.pThreshold));
    ResourceEstimate r;
    for (std::uint32_t d = 3; d <= kMaxDistance; d += 2) {
        ...
        if (volume * logicalErrorPerCycle(d, in.physicalErrorRate) > in.failureBudget * kSlack) continue;   // step 3
        r.distance = d;
    ...
    r.factoryQubits = double(in.factories) * in.factoryQubitsPerD2 * double(r.distance) * double(r.distance);
...
Result<double> cycleTimeFromDevice(const core::Json& deviceData) {
    ...
    if (t == qec->end() || !t->is_number() || !(t->get<double>() > 0.0))
        return fail(err::BadJson, "device: field 'data.qec.cycle_time_us' is missing or is not a positive number");
