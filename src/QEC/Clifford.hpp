...
inline void flipSign(PauliString& p) {
    p.phase = static_cast<std::int8_t>((p.phase + 2) & 3);
}

inline void conjugateCx(PauliString& p, std::uint32_t c, std::uint32_t t) {
    if (p.x[c] & p.z[t] & (p.x[t] ^ p.z[c] ^ 1u))
        flipSign(p); // T09 §3.1 CNOT rule
    p.x[t] ^= p.x[c];
    p.z[c] ^= p.z[t];
}
...
inline bool conjugate(PauliString& p, const Op& op) {
    if (op.marker())
        return true;
    if (op.a >= p.n || (op.twoQubit() && op.b >= p.n))
        return false;
