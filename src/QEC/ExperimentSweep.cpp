...
std::uint32_t roundsOf(const LogicalErrorExperiment& e) {
    if (e.rounds != 0) return e.rounds;
    return e.noise == NoiseSetting::CodeCapacity ? 1u : std::max<std::uint32_t>(1u, e.code.d);   // T09 §5.3: d rounds
}
...
Result<LogicalErrorPoint> runPoint(const LogicalErrorExperiment& e, std::size_t index, double p) {
    ...
    QXL_TRY_ASSIGN(const ExperimentRunner runner, ExperimentRunner::create(e.code, extraction, noise, e.decoder, e.unionFind));
    ...
    auto body = [&](std::size_t begin, std::size_t end) {
        ...
        for (std::size_t i = begin; i < end; ++i) {
            ...
            if (!failed) {
                ...
                if (!firstError) firstError = std::move(failed.error());
                return;
            }
    ...
    if (e.parallel) core::JobSystem::global().parallelFor(e.samples, 256, body);
    else body(0, e.samples);
    if (firstError) return std::unexpected(std::move(*firstError));
...
std::optional<double> crossingOf(const std::vector<LogicalErrorPoint>& small, const std::vector<LogicalErrorPoint>& large) {
    const std::size_t n = std::min(small.size(), large.size());
    for (std::size_t i = 0; i + 1 < n; ++i) {
        if (small[i].pL <= 0 || large[i].pL <= 0 || small[i + 1].pL <= 0 || large[i + 1].pL <= 0) continue;
        if (small[i].p <= 0 || small[i + 1].p <= small[i].p) continue;
        const double g0 = std::log(large[i].pL / small[i].pL), g1 = std::log(large[i + 1].pL / small[i + 1].pL);
        if (g0 < 0.0 && g1 >= 0.0) {   // below threshold the larger code wins, above it loses
            const double t = g0 / (g0 - g1);
            return std::exp(std::log(small[i].p) + t * (std::log(small[i + 1].p) - std::log(small[i].p)));
        }
    }
    return std::nullopt;
}
...
Result<std::vector<LogicalErrorPoint>> runLogicalErrorExperiment(const LogicalErrorExperiment& e) {
    if (e.samples == 0) return fail(err::BadOptions, "a logical-error experiment needs at least one sample per point");
    if (e.p.empty()) return fail(err::BadOptions, "a logical-error experiment needs at least one physical error rate");
...
Result<ThresholdResult> runThresholdSweep(const ThresholdSweep& sweep) {
    if (sweep.codes.size() < 2) return fail(err::BadOptions, "a threshold sweep needs at least two code distances");
    ...
    std::sort(result.curves.begin(), result.curves.end(), [](const auto& a, const auto& b) { return a.d < b.d; });
    double sum = 0.0;
    ...
    if (found > 0) result.crossing = sum / double(found);
    ...
    case NoiseSetting::CodeCapacity:
        ...
        result.referenceNote = "code capacity, independent X/Z flips: MWPM ≈ 10.3 %, union-find ≈ 9.9 %";
        if (sweep.settings.dataError == DataErrorKind::Depolarizing) {
            ...
            result.referenceNote += "; depolarizing data errors put 2p/3 on each component, so the crossing sits at 1.5× in p";
        }
        break;
    ...
    case NoiseSetting::CircuitLevel:
        ...
        result.referenceNote = "circuit-level depolarizing: ≈ 0.5–1 % depending on the noise model details";
        break;
    }
    return result;
}
