...
// Code-capacity picture: a syndrome and the correction a decoder returned for it (detector =
// check).
...
void showShot(CodeView& view, const MemoryExperiment& experiment, const ShotResult& shot,
              std::uint32_t round);
...
SpaceTimeLattice makeSpaceTimeLattice(const StabilizerCode& code,
                                      const MemoryExperiment& experiment, const ShotResult& shot);
