...
struct ExtractionOptions {
    ...
    // CSS codes: reset every data qubit into the +1 eigenstate of the memory-basis logical operator;
    // the first extraction round then projects onto the code space. Other codes: reset and run the
    // explicit encoder of `planEncoder` on |0⟩ (Z basis) or |+⟩ (X basis) — spec 16 §3.
    bool includeLogicalPrep = true;
    // Only without the prep: the input is already a code state (e.g. after `buildEncoder`), so every
    // check is deterministic in round 0 and gets a first-layer detector.
    bool assumeCodeStateInput = false;
};

// Gate set of the generated circuits. Two-qubit kinds act on (a = control, b = target).
enum class OpKind : std::uint8_t { H, S, Sdg, X, Y, Z, CX, CY, CZ, Swap, Reset, Measure, Tick, RoundStart };
...
struct Op {
    ...
    constexpr bool twoQubit() const { return kind == OpKind::CX || kind == OpKind::CY || kind == OpKind::CZ || kind == OpKind::Swap; }
...
struct MemoryExperiment {
    ...
    std::uint32_t nData = 0, nAncilla = 0;   // qubit q < nData is data; nData + j measures generator j
    Schedule schedule;
    std::vector<char> dataBasis;             // per data qubit 'X' | 'Y' | 'Z': prep and readout basis
    std::vector<Detector> detectors;
    // One per logical qubit: data-readout bits whose parity is the logical outcome (0 without faults).
    ...
    std::uint32_t syndromeBit(std::uint32_t round, std::uint32_t check) const { return round * nAncilla + check; }
    std::uint32_t dataBit(std::uint32_t q) const { return options.rounds * nAncilla + q; }
    std::uint32_t layers() const { return options.rounds + (options.finalDataMeasurement ? 1u : 0u); }
    ...
    std::vector<std::uint32_t> detectorIndex;   // [layer · nAncilla + check] → detector, kNoIndex if none
};
