...
inline Mask toMask(const PauliString& p) {
    ...
    for (std::uint32_t q = 0; q < p.n && q < 64; ++q) {
        if (p.x[q])
            m.x |= std::uint64_t{1} << q;
        if (p.z[q])
            m.z |= std::uint64_t{1} << q;
    }
    return m;
}
...
inline bool commute(Mask a, Mask b) {
    return (std::popcount((a.x & b.z) ^ (a.z & b.x)) & 1) == 0;
}
inline int weight(Mask m) {
    return std::popcount(m.x | m.z);
}
