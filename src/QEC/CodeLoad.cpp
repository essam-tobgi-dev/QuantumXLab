...
std::unexpected<Error> missing(std::string_view path, std::string_view expected) {
    return fail(err::BadJson,
                std::format("qec.code: field '{}' is missing or is not {}", path, expected));
}
...
Result<std::uint32_t> readUint(const Json& obj, std::string_view key, std::string_view path) {
    ...
    if (it == obj.end() || !it->is_number_unsigned())
        return missing(path, "a non-negative integer");
    return it->get<std::uint32_t>();
}

Result<std::vector<PauliString>> readPaulis(const Json& obj, std::string_view key,
                                            std::string_view path) {
    const auto it = obj.find(key);
    if (it == obj.end() || !it->is_array())
        return missing(path, "an array of Pauli strings");
    std::vector<PauliString> out;
    for (std::size_t j = 0; j < it->size(); ++j) {
        const Json& e = (*it)[j];
        if (!e.is_string())
            return missing(std::format("{}[{}]", path, j), "a Pauli string");
...
Result<Coord2> readCoord(const Json& j, std::string_view path) {
    if (!j.is_array() || j.size() != 2 || !j[0].is_number() || !j[1].is_number())
        return missing(path, "an [x, y] pair");
    return Coord2{j[0].get<double>(), j[1].get<double>()};
}

CodeFamily familyFromName(std::string_view s) {
    for (CodeFamily f : {CodeFamily::Repetition, CodeFamily::Shor, CodeFamily::Steane,
                         CodeFamily::FiveQubit, CodeFamily::SurfaceRotated})
        if (familyName(f) == s)
            return f;
...
Result<AncillaSpec> readAncilla(const Json& j, const StabilizerCode& code, std::size_t index) {
    ...
    if (!j.is_object())
        return missing(path, "an object");
    AncillaSpec a;
    const auto type = j.find("type");
    if (type == j.end() || !type->is_string())
        return missing(path + ".type", "one of \"X\", \"Z\", \"M\"");
    ...
    if (t == "X")
        ...
        a.type = CheckType::Mixed;
    else
        return missing(path + ".type", "one of \"X\", \"Z\", \"M\"");
    const auto coord = j.find("coord");
    if (coord == j.end())
        return missing(path + ".coord", "an [x, y] pair");
    QXL_TRY_ASSIGN(a.coord, readCoord(*coord, path + ".coord"));
    const auto order = j.find("order");
    if (order == j.end() || !order->is_array())
        return missing(path + ".order", "an array of data-qubit indices");
    for (std::size_t i = 0; i < order->size(); ++i) {
        const Json& q = (*order)[i];
        if (!q.is_number_unsigned() || q.get<std::uint64_t>() >= code.n)
            return missing(std::format("{}.order[{}]", path, i),
                           std::format("a data-qubit index below n = {}", code.n));
    ...
    if (const auto pauli = j.find("pauli"); pauli != j.end()) {
        if (!pauli->is_string())
            return missing(path + ".pauli", "a Pauli string");
        ...
        if (index < code.stabilizers.size() && !p.sameLetters(code.stabilizers[index]))
            return fail(err::BadLayout,
                        std::format("qec.code: {}.pauli = '{}' differs from stabilizers[{}] = '{}'",
                                    path, p.str(), index, code.stabilizers[index].str()));
    }
    return a;
}
...
Result<StabilizerCode> codeFromJson(const Json& d) {
    if (!d.is_object())
        return missing("data", "an object");
    StabilizerCode c;
    const auto id = d.find("id");
    if (id == d.end() || !id->is_string())
        return missing("data.id", "a string");
    ...
    if (const auto f = d.find("family"); f != d.end() && f->is_string())
        c.family = familyFromName(f->get<std::string>());
    if (const auto t = d.find("theory"); t != d.end() && t->is_array())
        for (const Json& e : *t)
            if (e.is_string())
                c.theoryRefs.push_back(e.get<std::string>());
    if (const auto n = d.find("notes"); n != d.end() && n->is_string())
        c.notes = n->get<std::string>();
    ...
    if (layout == d.end()) {
        ...
        if (!layout->is_object())
            return missing("data.layout", "an object");
        const auto data = layout->find("data");
        if (data == layout->end() || !data->is_array())
            return missing("data.layout.data", "an array of [x, y] pairs");
        for (std::size_t q = 0; q < data->size(); ++q) {
            QXL_TRY_ASSIGN(const Coord2 xy,
                           readCoord((*data)[q], std::format("data.layout.data[{}]", q)));
            c.dataLayout.push_back(xy);
        }
        const auto anc = layout->find("ancilla");
        if (anc == layout->end() || !anc->is_array())
            return missing("data.layout.ancilla", "an array of ancilla objects");
    ...
    if (const auto dec = d.find("decoder"); dec != d.end() && dec->is_string()) {
        ...
        if (c.defaultDecoder != "lookup" && c.defaultDecoder != "union_find" &&
            c.defaultDecoder != "mwpm")
...
Json codeToJson(const StabilizerCode& c) {
    auto strings = [](const std::vector<PauliString>& ps) {
        Json a = Json::array();
        for (const PauliString& p : ps)
            a.push_back(p.str(p.phase != 0));
        return a;
    };
    Json d = Json::object();
    d["id"] = c.id;
    ...
    for (const Coord2& xy : c.dataLayout)
        data.push_back(Json::array({xy.x, xy.y}));
    for (std::size_t j = 0; j < c.ancillas.size(); ++j) {
        const AncillaSpec& a = c.ancillas[j];
        Json e = {{"type", std::string(checkTypeName(a.type))},
                  {"coord", Json::array({a.coord.x, a.coord.y})},
                  {"order", a.order}};
        if (a.type == CheckType::Mixed && j < c.stabilizers.size())
            e["pauli"] = c.stabilizers[j].str();
...
Result<StabilizerCode> loadCode(const std::filesystem::path& path, const VerifyOptions& options) {
    ...
    if (!code)
        return annotate(std::move(code.error()));
    if (auto ok = verifyCode(*code, options); !ok)
        return annotate(std::move(ok.error()));
    return code;
}
...
const std::vector<std::string>& shippedCodeIds() {
    static const std::vector<std::string> ids = {
        "repetition_bitflip_3", "repetition_phaseflip_3", "shor_9",        "steane_7",
