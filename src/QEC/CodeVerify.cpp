...
bool isOdd(double v) {
    return isInteger(v) && (static_cast<long long>(std::llround(v)) & 1LL) != 0;
}
bool isEven(double v) {
    return isInteger(v) && (static_cast<long long>(std::llround(v)) & 1LL) == 0;
}

Status verifyShapes(const StabilizerCode& c) {
    if (c.n == 0 || c.k > c.n)
        return fail(err::BadJson, std::format("code '{}': need n ≥ 1 and k ≤ n, got n = {}, k = {}",
                                              c.id, c.n, c.k));
    if (c.stabilizers.size() != c.n - c.k)
        return fail(err::NotIndependent,
                    std::format("code '{}': {} generators given, [[{},{}]] needs n − k = {}", c.id,
                                c.stabilizers.size(), c.n, c.k, c.n - c.k));
    if (c.logicalX.size() != c.k || c.logicalZ.size() != c.k)
        return fail(err::BadLogical,
                    std::format("code '{}': {} logical X and {} logical Z operators given, k = {}",
    ...
    auto sized = [&](const PauliString& p, std::string_view what, std::size_t j) -> Status {
        if (p.n != c.n)
            return fail(err::BadPauli,
                        std::format("code '{}': {}[{}] = '{}' has length {}, expected n = {}", c.id,
                                    what, j, p.str(), p.n, c.n));
        if (!p.isHermitian())
            return fail(err::BadPauli, std::format("code '{}': {}[{}] = '{}' is not Hermitian",
                                                   c.id, what, j, p.str(true)));
        return {};
    };
    for (std::size_t j = 0; j < c.stabilizers.size(); ++j)
        QXL_TRY(sized(c.stabilizers[j], "stabilizers", j));
    for (std::size_t j = 0; j < c.k; ++j)
        QXL_TRY(sized(c.logicalX[j], "logical_x", j));
    for (std::size_t j = 0; j < c.k; ++j)
        QXL_TRY(sized(c.logicalZ[j], "logical_z", j));
    return {};
}
...
Status verifyGroup(const StabilizerCode& c) {
    for (std::size_t a = 0; a < c.stabilizers.size(); ++a) {
        if (c.stabilizers[a].isIdentity())
            return fail(err::NotIndependent,
                        std::format("code '{}': generator {} is the identity", c.id, a));
        for (std::size_t b = a + 1; b < c.stabilizers.size(); ++b)
            if (!c.stabilizers[a].commutesWith(c.stabilizers[b]))
                return fail(err::NotCommuting,
                            std::format("code '{}': generators {} '{}' and {} '{}' anticommute",
                                        c.id, a, c.stabilizers[a].str(), b,
                                        c.stabilizers[b].str()));
    }
    const std::uint32_t rank = symplecticRank(c.stabilizers);
    if (rank != c.n - c.k)
        return fail(err::NotIndependent,
                    std::format("code '{}': generators have rank {} over F2, expected n − k = {}",
                                c.id, rank, c.n - c.k));
    ...
    auto normal = [&](const PauliString& l, std::string_view what, std::size_t j) -> Status {
        for (std::size_t g = 0; g < c.stabilizers.size(); ++g)
            if (!l.commutesWith(c.stabilizers[g]))
                return fail(
                    err::BadLogical,
                    std::format("code '{}': {}[{}] = '{}' anticommutes with generator {} '{}'",
                                c.id, what, j, l.str(), g, c.stabilizers[g].str()));
        return {};
    };
    for (std::size_t j = 0; j < c.k; ++j)
        QXL_TRY(normal(c.logicalX[j], "logical_x", j));
    for (std::size_t j = 0; j < c.k; ++j)
        QXL_TRY(normal(c.logicalZ[j], "logical_z", j));
    for (std::size_t i = 0; i < c.k; ++i)
        for (std::size_t j = 0; j < c.k; ++j) {
            const bool commute = c.logicalX[i].commutesWith(c.logicalZ[j]);
            if (commute == (i == j))
                return fail(err::BadLogical,
                            std::format("code '{}': logical_x[{}] and logical_z[{}] must {}", c.id,
                                        i, j, i == j ? "anticommute" : "commute"));
            if (i < j && (!c.logicalX[i].commutesWith(c.logicalX[j]) ||
                          !c.logicalZ[i].commutesWith(c.logicalZ[j])))
                return fail(
                    err::BadLogical,
                    std::format("code '{}': logical operators {} and {} of one type must commute",
                                c.id, i, j));
        }
    return {};
}
...
Status verifyLayout(const StabilizerCode& c) {
    if (c.dataLayout.size() != c.n)
        return fail(err::BadLayout,
                    std::format("code '{}': layout.data has {} coordinates, expected n = {}", c.id,
                                c.dataLayout.size(), c.n));
    if (c.ancillas.size() != c.stabilizers.size())
        return fail(
            err::BadLayout,
            std::format("code '{}': layout.ancilla has {} entries, expected one per generator ({})",
        ...
        for (std::size_t q = 0; q < c.n; ++q)
            if (!isOdd(c.dataLayout[q].x) || !isOdd(c.dataLayout[q].y))
                return fail(
                    err::BadLayout,
                    std::format(
                        "code '{}': data qubit {} must sit at odd coordinates (spec 16 §2.1)", c.id,
                        q));
    for (std::uint32_t j = 0; j < c.ancillas.size(); ++j) {
        ...
        if (a.type != CheckType::Mixed && a.type != actual)
            return fail(
                err::BadLayout,
                std::format("code '{}': ancilla {} is typed '{}' but generator '{}' is '{}'-type",
        ...
        if (sorted != c.stabilizers[j].support())
            return fail(
                err::BadLayout,
                std::format(
                    "code '{}': ancilla {} order does not list the support of generator '{}'", c.id,
                    j, c.stabilizers[j].str()));
        if (!surface)
            continue;
        if (!isEven(a.coord.x) || !isEven(a.coord.y))
            return fail(
                err::BadLayout,
                std::format("code '{}': ancilla {} must sit at even coordinates (spec 16 §2.1)",
                            c.id, j));
        const bool evenCell =
            ((std::llround(a.coord.x) / 2 + std::llround(a.coord.y) / 2) & 1LL) == 0;
        if (actual == CheckType::Mixed || (actual == CheckType::X) != evenCell)
            return fail(
                err::BadLayout,
                std::format("code '{}': ancilla {} breaks the checkerboard (X at even i + j)", c.id,
                            j));
        int previous = -1;
        for (std::uint32_t q : a.order) {
            ...
            if (layer <= previous)
                return fail(err::BadLayout,
                            std::format("code '{}': ancilla {} order is not the Tomita–Svore order "
                                        "(X: NW,NE,SW,SE; Z: NW,SW,NE,SE) at data qubit {}",
                                        c.id, j, q));
            previous = layer;
        }
    }
...
CheckType StabilizerCode::checkType(std::uint32_t j) const {
    if (j >= stabilizers.size())
        return CheckType::Mixed;
    if (stabilizers[j].isXType())
        return CheckType::X;
...
bool StabilizerCode::isCss() const {
    for (std::uint32_t j = 0; j < checkCount(); ++j)
        if (checkType(j) == CheckType::Mixed)
            return false;
    return true;
}
...
bool StabilizerCode::isMatchable() const {
    ...
    for (std::uint32_t q = 0; q < n; ++q)
        for (char letter : {'X', 'Z'}) {
            ...
            if (std::count(s.begin(), s.end(), std::uint8_t{1}) > 2)
                return false;
        }
    return true;
}
...
int tomitaSvoreLayer(CheckType type, Coord2 ancilla, Coord2 data) {
    ...
    if (type == CheckType::Mixed || std::abs(std::abs(dx) - 1.0) > 1e-9 ||
        std::abs(std::abs(dy) - 1.0) > 1e-9)
        return -1;
    ...
    if (options.checkDistance && code.n <= options.exhaustiveDistanceMaxN) {
        ...
        if (report.declared != code.d)
            return fail(err::BadDistance,
                        std::format("code '{}': declared d = {} but the minimum-weight logical "
                                    "operator '{}' has weight {}",
