...
struct SyndromeLattice {
    ...
    static SyndromeLattice fromRecord(const MemoryExperiment& experiment, std::span<const std::uint8_t> bits);
    ...
    static SyndromeLattice foldLayers(const MemoryExperiment& experiment, std::span<const std::uint8_t> events);
};
...
// T09 §6.1: minimum-weight correction for every syndrome, O(1) per decode. A CSS code gets one table
// per check type (X errors from the Z checks, Z errors from the X checks: 2^{m_Z} + 2^{m_X} entries
// instead of 2^{n−k}); any other code one joint table over all 3^w letter assignments. Ties go to
// the first error in enumeration order (increasing weight, then increasing support as an integer).
...
public:
    ...
    std::unique_ptr<IDecoder> clone() const override { return std::make_unique<LookupDecoder>(*this); }
...
struct UnionFindOptions {
    ...
    bool weighted = true;
    // resolution = 1 is "integer units of the minimum weight": every ratio in [1, 1.5) becomes 1 and
    // [1.5, 2.5) becomes 2. Measured on surface_rot_5, circuit level, p = 1e-3, 2·10^5 shots:
...
public:
    ...
    std::unique_ptr<IDecoder> clone() const override { return std::make_unique<UnionFindDecoder>(*this); }
...
private:
    ...
    std::vector<std::uint32_t> virtualOf_;         // edge → its virtual boundary vertex (kNoIndex for inner edges)
    ...
    std::vector<std::uint32_t> active_, firedList_, touchedVertices_, touchedEdges_, newlyFull_, keep_;
