...
Status checkSchedule(const Schedule& s) {
    if (s.qubits == 0)
        return fail(err::BadOptions, "schedule has no qubits");
    for (std::size_t i = 0; i < s.ops.size(); ++i) {
        ...
        if (op.a >= s.qubits || (op.twoQubit() && (op.b >= s.qubits || op.b == op.a)))
            return fail(err::BadOptions,
                        std::format("schedule op {} ({}) addresses a qubit outside 0..{}", i,
                                    opName(op.kind), s.qubits - 1));
        if (op.kind == OpKind::Measure && op.bit != kNoIndex && op.bit >= s.bits)
            return fail(err::BadOptions,
                        std::format("schedule op {} writes bit {} of {}", i, op.bit, s.bits));
    }
    return {};
}
...
Status validateFaults(std::span<const FaultEvent> faults, const Schedule& s) {
    ...
    for (const FaultEvent& f : faults) {
        if (f.afterOp < previous || f.afterOp >= s.ops.size())
            return fail(err::BadOptions,
                        "faults must be ordered by afterOp and refer to schedule operations");
        previous = f.afterOp;
        const bool badA = f.qubitA != kNoIndex && f.qubitA >= s.qubits,
                   badB = f.qubitB != kNoIndex && f.qubitB >= s.qubits;
        if (badA || badB || (f.flipBit != kNoIndex && f.flipBit >= s.bits))
            return fail(
                err::BadOptions,
                std::format("fault after op {} addresses a qubit or bit outside the schedule",
                            f.afterOp));
    }
    return {};
}
...
Status TableauSampler::run(std::span<const FaultEvent> faults, core::Random& rng,
...
Status TableauSampler::run(std::span<const FaultEvent> faults, core::Random& rng,
                           std::vector<std::uint8_t>& bits, qsim::StabilizerBackend& st) const {
    ...
    for (std::uint32_t i = 0; i < schedule_.ops.size(); ++i) {
        ...
        case OpKind::CY:
            ...
            st.cnot(op.a, op.b);
            st.s(op.b);
            break; // CY = S_t · CX · S†_t
        case OpKind::Swap:
            st.swap(op.a, op.b);
            break;
        case OpKind::Reset:
            QXL_TRY(st.reset(std::span<const QubitIndex>(&qa, 1), rng));
            break;
        case OpKind::Measure: {
            QXL_TRY_ASSIGN(const qsim::Outcome out,
                           st.measure(std::span<const QubitIndex>(&qa, 1), rng));
            if (op.bit != kNoIndex && !out.bits.empty())
                bits[op.bit] = out.bits[0];
            break;
        }
        case OpKind::Tick:
        ...
        for (; next < faults.size() && faults[next].afterOp == i; ++next) {
            ...
            if (f.qubitA != kNoIndex)
                applyPauli(st, f.qubitA, f.pauliA);
            if (f.qubitB != kNoIndex)
                applyPauli(st, f.qubitB, f.pauliB);
            if (f.flipBit != kNoIndex)
                recordFlips[f.flipBit] ^= 1u;
        }
    }
    for (std::size_t b = 0; b < bits.size(); ++b)
        bits[b] ^= recordFlips[b];
    return {};
}
