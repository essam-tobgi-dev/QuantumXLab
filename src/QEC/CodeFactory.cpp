...
Status requireOddDistance(std::uint32_t d, std::string_view family) {
    if (d < 3 || d % 2 == 0)
        return fail(err::BadOptions,
                    std::format("{} code needs an odd distance d ≥ 3, got {}", family, d));
    return {};
}
...
Result<StabilizerCode> makeRepetitionCode(std::uint32_t d, bool phaseFlip) {
    ...
    for (std::uint32_t q = 0; q + 1 < d; ++q) {
        ...
        c.ancillas.push_back(
            {phaseFlip ? CheckType::X : CheckType::Z, {double(q) + 0.5, 1.0}, {q, q + 1}});
    }
    // T09 §4.1–4.2: the protected logical is the full string, its partner a single letter on qubit
    // 0.
    ...
    c.theoryRefs = {phaseFlip ? "T09#4.2-three-qubit-phase-flip-code"
                              : "T09#4.1-three-qubit-bit-flip-code"};
    c.notes = phaseFlip ? "Hadamard-conjugated repetition code; corrects (d-1)/2 Z errors; "
                          "distance 1 against X."
...
Result<StabilizerCode> makeRotatedSurfaceCode(std::uint32_t d) {
    ...
    if (d > 99)
        return fail(
            err::TooLarge,
            std::format("rotated surface code distance {} exceeds the generator limit 99", d));
    StabilizerCode c;
    ...
    for (std::uint32_t i = 0; i < d; ++i)
        for (std::uint32_t j = 0; j < d; ++j)
            c.dataLayout.push_back({2.0 * i + 1.0, 2.0 * j + 1.0});
    auto dataAt = [d](int x, int y) -> int {
        if (x < 1 || y < 1 || x > int(2 * d - 1) || y > int(2 * d - 1))
            return -1;
    ...
    for (std::uint32_t i = 0; i <= d; ++i)
        for (std::uint32_t j = 0; j <= d; ++j) {
            ...
            if (edgeX && edgeY)
                continue;
            if (edgeX && xType)
                continue; // left/right edges: Z checks only
            if (edgeY && !xType)
                continue; // top/bottom edges: X checks only
            ...
            for (int q :
                 xType ? std::vector<int>{nw, ne, sw, se} : std::vector<int>{nw, sw, ne, se})
                if (q >= 0)
                    a.order.push_back(static_cast<std::uint32_t>(q));
    ...
    c.notes = "Rotated planar layout; X checks on top/bottom boundaries, Z checks on left/right; "
              "CNOT order X: "
