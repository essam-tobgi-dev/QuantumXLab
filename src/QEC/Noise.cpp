...
Status validate(const NoiseParams& n) {
    ...
    if (!probability(n.p))
        return fail(err::BadOptions,
                    std::format("physical error rate p = {} is not a probability", n.p));
    if (!probability(n.measurementFlip()))
        return fail(err::BadOptions,
                    std::format("syndrome flip probability q = {} is not a probability", n.q));
    if (!probability(n.pIdle))
        return fail(err::BadOptions,
                    std::format("idle error rate {} is not a probability", n.pIdle));
    return {};
}
...
NoiseSite dataError(std::uint32_t afterOp, std::uint32_t qubit, const NoiseParams& n) {
    ...
    switch (n.dataError) {
    case DataErrorKind::Depolarizing:
        s.px = s.py = s.pz = n.p / 3.0;
        break; // spec 16 §4
    case DataErrorKind::BitFlip:
        s.px = n.p;
        break;
    case DataErrorKind::PhaseFlip:
        s.pz = n.p;
        break;
    }
    return s;
...
void circuitSites(const Schedule& s, const NoiseParams& n, std::vector<NoiseSite>& sites) {
    ...
    auto closeMoment = [&](std::uint32_t lastOp) {
        if (n.pIdle > 0.0 && momentHasOps)
            for (std::uint32_t q = 0; q < s.qubits; ++q)
                if (!touched[q])
                    sites.push_back(depolarize1(lastOp, q, n.pIdle));
    ...
    for (std::uint32_t i = 0; i < s.ops.size(); ++i) {
        ...
        if (op.a < s.qubits)
            touched[op.a] = 1;
        if (op.twoQubit() && op.b < s.qubits)
            touched[op.b] = 1;
        if (n.p <= 0.0)
            continue;
        if (op.twoQubit())
            sites.push_back({i, SiteKind::Depolarize2, op.a, op.b, n.p});
        else if (op.kind == OpKind::Reset)
            sites.push_back({i, SiteKind::Pauli1, op.a, 0, n.p}); // prepared in |1⟩
        else if (op.kind == OpKind::Measure) {
            if (op.bit != kNoIndex)
                sites.push_back({i, SiteKind::RecordFlip, op.bit, 0, n.p});
        } else
            sites.push_back(depolarize1(i, op.a, n.p));
    }
    if (!s.ops.empty())
        closeMoment(static_cast<std::uint32_t>(s.ops.size() - 1));
}
...
Result<NoisePlan> planNoise(const MemoryExperiment& ex, const NoiseParams& params) {
    if (params.setting == NoiseSetting::CircuitLevel)
        return planCircuitNoise(ex.schedule, params);
    QXL_TRY(validate(params));
    NoisePlan plan{params, {}};
    ...
    for (std::uint32_t i = 0; i < ops.size(); ++i) {
        ...
        if (op.kind == OpKind::RoundStart && (everyRound || op.a == 0) && params.p > 0.0)
            for (std::uint32_t q = 0; q < ex.nData; ++q)
                plan.sites.push_back(dataError(i, q, params));
...
void drawFaults(const NoisePlan& plan, core::Random& rng, std::vector<FaultEvent>& out) {
    ...
    for (std::uint32_t i = 0; i < plan.sites.size(); ++i) {
        ...
        case SiteKind::Depolarize2: {
            ...
            const auto index =
                std::min<std::uint32_t>(14u, static_cast<std::uint32_t>(u / s.px * 15.0)) + 1u;
            f.qubitA = s.a;
            f.qubitB = s.b;
