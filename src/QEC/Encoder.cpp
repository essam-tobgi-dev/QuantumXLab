...
class Synthesizer {
  public:
    ...
    Result<Schedule> run() {
        ...
        for (std::uint32_t l = 0; l < code_.k; ++l) {
            ...
            if (rows_[zRow].phase != 0)
                apply({OpKind::X, q});
            if (rows_[xRow].phase != 0)
                apply({OpKind::Z, q});
  ...
  private:
    void apply(Op op) {
        for (PauliString& r : rows_)
            detail::conjugate(r, op);
        ops_.push_back(op);
    }
    ...
    // Removes every letter of `row` except the one on `pivot`, whose letter must be X: X_p ⊗ L_t →
    // X_p by cx (L = X), cz (L = Z) or sdg + cx (L = Y). All of these fix Z_p.
    ...
    Result<std::uint32_t> reduceToZ(std::uint32_t row) {
        ...
        if (support.empty() || support.size() != rows_[row].weight())
            return fail(ErrorCode::Internal,
                        std::format("code '{}': encoder synthesis lost row {} (is the code valid?)",
                                    code_.id, row));
        ...
        if (pivot == kNoIndex) {
            ...
            for (std::uint32_t t : support)
                if (t != pivot)
                    apply({OpKind::CX, t, pivot});
        } else {
            if (rows_[row].letter(pivot) == 'Y')
                apply({OpKind::Sdg, pivot}); // S† Y S = X
    ...
    Status reduceToX(std::uint32_t row, std::uint32_t pivot) {
        if (!rows_[row].x[pivot])
            return fail(ErrorCode::Internal,
                        std::format("code '{}': logical X and Z do not anticommute on the pivot",
                                    code_.id));
        if (rows_[row].letter(pivot) == 'Y')
            apply({OpKind::Sdg, pivot});
        absorbInto(row, pivot);
        return {};
    }
...
Result<PauliString> conjugated(const PauliString& p, std::span<const Op> gates) {
    ...
    for (const Op& op : gates)
        if (!detail::conjugate(out, op))
            return fail(err::NotClifford, std::format("cannot conjugate through '{}' on qubit {}",
                                                      opName(op.kind), op.a));
    return out;
}
...
Result<Schedule> planEncoder(const StabilizerCode& code) {
    ...
    for (const PauliString& g : code.stabilizers)
        if (g.phase != 0)
            return fail(
                err::BadOptions,
                std::format("code '{}': generators with a sign are not supported", code.id));
    ...
    for (std::uint32_t q = 0; q < code.n; ++q) {
        ...
        if (q < code.k)
            image = image * code.logicalZ[q];
        int sign = 0;
        if (!inGroup(code.stabilizers, image, &sign) || sign != 1)
            return fail(ErrorCode::Internal,
                        std::format("code '{}': encoder maps Z_{} outside its target", code.id, q));
    }
    for (std::uint32_t l = 0; l < code.k; ++l) {
        ...
        if (!inGroup(code.stabilizers, image * code.logicalX[l], &sign) || sign != 1)
            return fail(ErrorCode::Internal,
                        std::format("code '{}': encoder maps X_{} outside its target", code.id, l));
    }
    return s;
}
