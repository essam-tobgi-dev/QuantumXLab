...
//  TableauSampler  the reference engine: the full circuit on qsim::StabilizerBackend, faults
//  applied
...
class TableauSampler {
  public:
    ...
    Status run(std::span<const FaultEvent> faults, core::Random& measurementRng,
               std::vector<std::uint8_t>& bits) const;
    // The same shot, leaving the final state in `state` (for correction and readback in tests).
    Status run(std::span<const FaultEvent> faults, core::Random& measurementRng,
               std::vector<std::uint8_t>& bits, qsim::StabilizerBackend& state) const;
...
class FrameSampler {
  public:
    ...
    // Record bits flipped by these faults (ordered by `afterOp`); `flips` gets `schedule.bits`
    // entries.
    ...
    void propagate(std::uint32_t afterOp, std::uint32_t qubit, char pauli,
