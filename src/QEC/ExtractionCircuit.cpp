...
Status appendOps(ir::Circuit& c, const Schedule& s) {
    for (const Op& op : s.ops) {
        switch (op.kind) {
        case OpKind::Tick:
        case OpKind::RoundStart: c.add(ir::Barrier{{}, {}}); break;
        case OpKind::Reset: c.add(ir::Reset{ir::Wire{op.a}, std::nullopt, {}}); break;
        case OpKind::Measure:
            c.add(ir::Measure{ir::Wire{op.a}, op.bit == kNoIndex ? ir::kNoBit : ir::ClassicalBit{op.bit}, std::nullopt, {}});
            break;
        default: {
...
Json coordArray(std::span<const Coord2> coords) {
    Json a = Json::array();
    for (const Coord2& xy : coords) a.push_back(Json::array({xy.x, xy.y}));
    return a;
}

std::unexpected<Error> notClifford(std::string_view what) {
    return fail(err::NotClifford, std::format("{} is outside the QEC gate set (id, x, y, z, h, s, sdg, cx, cy, cz, swap, "
                                              "measure, reset, barrier)", what));
}

Result<OpKind> kindOf(const ir::Gate& g) {
    static constexpr std::pair<std::string_view, OpKind> table[] = {
        {"h", OpKind::H},   {"s", OpKind::S},   {"sdg", OpKind::Sdg}, {"x", OpKind::X},   {"y", OpKind::Y},      {"z", OpKind::Z},
        {"cx", OpKind::CX}, {"CX", OpKind::CX}, {"cy", OpKind::CY},   {"cz", OpKind::CZ}, {"swap", OpKind::Swap}};
    if (!g.controls.empty() || g.adjoint || g.opaque || g.custom) return notClifford(std::format("modified gate '{}'", g.name));
    for (const auto& [name, kind] : table)
        if (g.name == name) return kind;
...
Result<ir::Circuit> toCircuit(const MemoryExperiment& ex) {
    ...
    c.meta()["interactionHint"] = {{"data", coordArray(coords.first(nData))}, {"ancilla", coordArray(coords.subspan(nData))}};
    Json detectors = Json::array();
    for (const Detector& d : ex.detectors)
        detectors.push_back({{"check", d.check}, {"layer", d.layer}, {"type", std::string(checkTypeName(d.type))}, {"bits", d.bits}});
...
Result<ir::Circuit> buildMemoryExperiment(const StabilizerCode& code, const ExtractionOptions& options) {
...
Result<Schedule> compileClifford(const ir::Circuit& circuit) {
    ...
    for (ir::NodeId id : circuit.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&node)) {
            if (g->name == "id") continue;
            QXL_TRY_ASSIGN(const OpKind kind, kindOf(*g));
            Op op{kind};
            if (g->targets.size() != (op.twoQubit() ? 2u : 1u)) return notClifford(std::format("gate '{}' with {} operands", g->name, g->targets.size()));
            op.a = g->targets[0].index;
            if (op.twoQubit()) op.b = g->targets[1].index;
            s.ops.push_back(op);
        } else if (const auto* m = std::get_if<ir::Measure>(&node)) {
            s.ops.push_back({OpKind::Measure, m->qubit.index, 0, m->discards() ? kNoIndex : m->bit.index});
            ...
            if (b->wires.empty() || b->wires.size() == circuit.qubitCount()) s.ops.push_back({OpKind::Tick});
