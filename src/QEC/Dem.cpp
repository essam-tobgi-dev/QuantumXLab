...
// What a fault does to the record: detectors fired, logical readouts flipped, data readouts flipped.
struct Signature {
    std::vector<std::uint32_t> detectors, dataFlips;   // sorted
    std::uint64_t observables = 0;
};

std::vector<std::uint32_t> symmetricDifference(const std::vector<std::uint32_t>& a, const std::vector<std::uint32_t>& b) {
...
double xorProbability(double p, double q) { return p * (1.0 - q) + q * (1.0 - p); }
...
class ModelBuilder {
public:
    ModelBuilder(const MemoryExperiment& ex, const NoisePlan& noise) : ex_(ex), noise_(noise), frame_(ex.schedule) {
        bitDetectors_.assign(ex.schedule.bits, {});
        bitObservables_.assign(ex.schedule.bits, 0);
        for (std::uint32_t d = 0; d < ex.detectors.size(); ++d)
            for (std::uint32_t b : ex.detectors[d].bits) bitDetectors_[b].push_back(d);
        for (std::uint32_t l = 0; l < ex.observables.size() && l < 64; ++l)
            for (std::uint32_t b : ex.observables[l]) bitObservables_[b] ^= std::uint64_t{1} << l;
        ...
        const bool allZ = std::all_of(ex.dataBasis.begin(), ex.dataBasis.end(), [](char c) { return c == 'Z'; });
        const bool allX = std::all_of(ex.dataBasis.begin(), ex.dataBasis.end(), [](char c) { return c == 'X'; });
        readoutType_ = allZ ? CheckType::Z : (allX ? CheckType::X : CheckType::Mixed);
        split_ = readoutType_ != CheckType::Mixed &&
                 std::none_of(ex.detectors.begin(), ex.detectors.end(), [](const Detector& d) { return d.type == CheckType::Mixed; });
    }
    ...
    DecodingGraph build() {
        ...
        for (const FaultEvent& f : enumerateFaults(noise_)) {
            if (f.site != site) mergeSite();
            site = f.site;
            ++g.faults;
            const Signature s = signatureOf(f);
            for (Signature& part : partsOf(s)) {
                if (part.detectors.empty()) {
                    if (part.observables != 0) g.undetectedLogicalProbability = xorProbability(g.undetectedLogicalProbability, f.probability);
        ...
        mergeSite();
        for (const auto& [part, p] : hyper) decompose(part, p, g);
        for (auto& [key, cls] : classes_) {
            GraphEdge e;
            e.a = key.first;
            e.b = key.second;
            e.probability = cls.probability;
            const auto best = std::max_element(cls.actions.begin(), cls.actions.end(),
                                               [](const auto& l, const auto& r) { return l.second.first < r.second.first; });
            if (best != cls.actions.end()) {
                e.observableMask = best->first;
                for (std::uint32_t q : best->second.second) e.correction.emplace_back(q, ex_.dataBasis[q] == 'X' ? 'Z' : 'X');
            }
            if (cls.actions.size() > 1) ++g.ambiguousEdges;
            g.edges.push_back(std::move(e));
        }
        const bool exact = g.hyperedgesDecomposed == 0 && g.hyperedgesDropped == 0 && g.ambiguousEdges == 0;
...
private:
    Signature ofBits(const std::vector<std::uint32_t>& bits) const {
        ...
        for (std::uint32_t b : bits) {
            ...
            if (ex_.options.finalDataMeasurement && b >= ex_.dataBit(0)) s.dataFlips.push_back(b - ex_.dataBit(0));
        }
        std::sort(detectors.begin(), detectors.end());
        for (std::size_t i = 0; i < detectors.size();) {   // keep odd multiplicities
            std::size_t j = i;
            while (j < detectors.size() && detectors[j] == detectors[i]) ++j;
            if ((j - i) % 2 == 1) s.detectors.push_back(detectors[i]);
            i = j;
        }
    ...
    const Signature& elementary(std::uint32_t afterOp, std::uint32_t qubit, bool zLetter) {
        const std::uint64_t key = (std::uint64_t(afterOp) << 32) | (std::uint64_t(qubit) << 1) | (zLetter ? 1u : 0u);
    ...
    Signature signatureOf(const FaultEvent& f) {
        ...
        auto add = [&](std::uint32_t q, char letter) {
            if (q == kNoIndex) return;
            if (letter == 'X' || letter == 'Y') combine(s, elementary(f.afterOp, q, false));
            if (letter == 'Z' || letter == 'Y') combine(s, elementary(f.afterOp, q, true));
        };
        add(f.qubitA, f.pauliA);
        add(f.qubitB, f.pauliB);
        if (f.flipBit != kNoIndex) combine(s, ofBits({f.flipBit}));
        return s;
    }
    ...
    std::vector<Signature> partsOf(const Signature& s) const {
        if (!split_) return {s};
        Signature x, z;
        for (std::uint32_t d : s.detectors) (ex_.detectors[d].type == CheckType::X ? x : z).detectors.push_back(d);
    ...
    void file(const Signature& part, double p) {
        const EdgeKey key{part.detectors[0], part.detectors.size() == 2 ? part.detectors[1] : kBoundary};
        EdgeClass& cls = site_[key];
        cls.probability += p;
        auto& action = cls.actions[part.observables];
        if (action.first == 0.0) action.second = part.dataFlips;
        action.first += p;
    }
    ...
    void mergeSite() {
        for (auto& [key, local] : site_) {
            ...
            for (auto& [mask, action] : local.actions) {
                ...
                if (total.first == 0.0) total.second = std::move(action.second);
    ...
    bool partition(std::vector<std::uint32_t> rest, std::uint64_t wantAction, std::vector<EdgeKey>& out, bool exactAction) {
        if (rest.empty()) return !exactAction || wantAction == 0;
        ...
        for (std::size_t i = 0; i <= rest.size(); ++i) {
            ...
            if (i < rest.size()) remaining.erase(remaining.begin() + static_cast<std::ptrdiff_t>(i));
            const std::uint64_t action = it->second.actions.empty() ? 0 : dominantAction(it->second);
            out.push_back(key);
            if (partition(std::move(remaining), wantAction ^ action, out, exactAction)) return true;
            out.pop_back();
        }
        return false;
    }
    ...
    static std::uint64_t dominantAction(const EdgeClass& cls) {
        return std::max_element(cls.actions.begin(), cls.actions.end(),
                                [](const auto& l, const auto& r) { return l.second.first < r.second.first; })->first;
    }
    ...
    void decompose(const Signature& part, double p, DecodingGraph& g) {
        ...
        if (!partition(part.detectors, part.observables, components, true)) {
            components.clear();
            if (!partition(part.detectors, part.observables, components, false)) { ++g.hyperedgesDropped; return; }
        }
        ++g.hyperedgesDecomposed;
        for (const EdgeKey& key : components) classes_[key].probability = xorProbability(classes_[key].probability, p);
    }
...
Result<DecodingGraph> buildDecodingGraph(const MemoryExperiment& experiment, const NoisePlan& noise) {
    if (experiment.detectors.empty()) return fail(err::BadOptions, "the experiment defines no detector to decode");
