...
// 10^5-sample sweeps at d = 7 interactive (spec 16 §5). With either engine the Monte-Carlo loop does
// not simulate a shot in which no fault was drawn: `ExperimentRunner::create` has checked on the
// backend that the fault-free run fires no detector and reads the prepared logical value.
...
Result<std::vector<LogicalErrorPoint>> runLogicalErrorExperiment(const LogicalErrorExperiment& experiment);

// One decoded shot with everything the lattice views need (spec 16 §8).
struct ShotResult {
    std::vector<FaultEvent> faults;
    std::vector<std::uint8_t> bits;       // measurement record (Stabilizer engine) or its flips (PauliFrame)
...
public:
    ...
    // reliable syndromes.
    static Result<ExperimentRunner> create(const StabilizerCode& code, const ExtractionOptions& extraction,
                                           const NoiseParams& noise, std::string_view decoder = {},
                                           UnionFindOptions unionFind = {});
    // The same from a planned experiment and an explicit noise plan — the entry point for noise that
    // is not one of the three uniform settings, e.g. per-gate rates of a device calibration (spec 16
    // §4, spec 08 §6): the caller fills `NoisePlan::sites` for `experiment.schedule`.
    static Result<ExperimentRunner> fromPlan(const StabilizerCode& code, MemoryExperiment experiment, NoisePlan noise,
    ...
    public:
        ...
        Result<ShotResult> shot(const core::Random& master, std::uint64_t index, SampleEngine engine);
        // A shot with explicit faults (ordered by `afterOp`) instead of sampled ones.
        Result<ShotResult> shotWithFaults(std::vector<FaultEvent> faults, core::Random& measurementRng, SampleEngine engine);
        // Failure flag of shot `index` only — the Monte-Carlo inner loop. Same streams as `shot`, so
        // the two agree; a shot without faults returns false without being simulated.
        Result<bool> failed(const core::Random& master, std::uint64_t index, SampleEngine engine, bool* decoderRejected = nullptr);
