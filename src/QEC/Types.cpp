...
std::string_view familyName(CodeFamily f) {
    ...
    case CodeFamily::Shor: return "Shor";
    case CodeFamily::Steane: return "Steane";
    case CodeFamily::FiveQubit: return "FiveQubit";
    case CodeFamily::SurfaceRotated: return "SurfaceRotated";
    case CodeFamily::Other: return "Other";
    }
    return "Other";
}
...
std::string_view checkTypeName(CheckType t) {
    switch (t) {
    case CheckType::X: return "X";
    case CheckType::Z: return "Z";
    case CheckType::Mixed: return "M";
    }
    return "M";
}

std::string_view basisName(LogicalBasis b) { return b == LogicalBasis::Z ? "Z" : "X"; }

std::string_view noiseSettingName(NoiseSetting s) {
    switch (s) {
    case NoiseSetting::CodeCapacity: return "code_capacity";
    case NoiseSetting::Phenomenological: return "phenomenological";
    case NoiseSetting::CircuitLevel: return "circuit_level";
    }
    return "circuit_level";
}

std::optional<NoiseSetting> noiseSettingFromName(std::string_view name) {
    for (NoiseSetting s : {NoiseSetting::CodeCapacity, NoiseSetting::Phenomenological, NoiseSetting::CircuitLevel})
        if (noiseSettingName(s) == name) return s;
    return std::nullopt;
}
