...
Result<ExperimentRunner> ExperimentRunner::create(const StabilizerCode& code, const ExtractionOptions& extraction,
...
Result<ExperimentRunner> ExperimentRunner::fromPlan(const StabilizerCode& code, MemoryExperiment experiment, NoisePlan noise,
                                                    std::string_view decoder, UnionFindOptions unionFind) {
    if (!experiment.options.finalDataMeasurement || experiment.observables.empty())
        return fail(err::BadOptions, "a logical-error experiment needs the final data readout (spec 16 §6)");
    if (experiment.nData != code.n || experiment.nAncilla != code.checkCount())
        return fail(err::BadOptions, std::format("the experiment was not planned for code '{}'", code.id));
    ExperimentRunner r;
    r.code_ = code;
    ...
    if (r.decoderName_ == "union_find") {
        if (!code.isMatchable())
            return fail(err::NotMatchable, std::format("code '{}' is not matching-type; use the lookup decoder", code.id));
        ...
        return fail(ErrorCode::Unsupported, std::format("decoder '{}' is not available (lookup, union_find)", r.decoderName_));
    }
    ...
    const auto events = r.experiment_.detectionEvents(bits), logical = r.experiment_.observableValues(bits);
    const auto isSet = [](std::uint8_t b) { return b != 0; };
    if (std::any_of(events.begin(), events.end(), isSet) || std::any_of(logical.begin(), logical.end(), isSet))
        return fail(ErrorCode::Internal, std::format("code '{}': the fault-free experiment fires a detector or flips the "
                                                     "logical readout", code.id));
    return r;
}

ExperimentRunner::Worker ExperimentRunner::worker() const { return Worker(*this, prototype_->clone()); }
...
Result<ShotResult> ExperimentRunner::Worker::shot(const core::Random& master, std::uint64_t index, SampleEngine engine) {
    ShotResult s;
...
Result<ShotResult> ExperimentRunner::Worker::shotWithFaults(std::vector<FaultEvent> faults, core::Random& measurementRng,
                                                            SampleEngine engine) {
    ShotResult s;
...
Result<bool> ExperimentRunner::Worker::failed(const core::Random& master, std::uint64_t index, SampleEngine engine,
                                             bool* decoderRejected) {
    core::Random noiseRng = master.stream(2 * index), measurementRng = master.stream(2 * index + 1);
    drawFaults(runner_->noise_, noiseRng, scratch_.faults);
    if (decoderRejected) *decoderRejected = false;
    if (scratch_.faults.empty()) return false;   // fault-free: checked once in ExperimentRunner::create
    QXL_TRY(finish(scratch_, engine, measurementRng));
    if (decoderRejected) *decoderRejected = scratch_.decoderRejected;
    return scratch_.failure;
}

Status ExperimentRunner::Worker::finish(ShotResult& s, SampleEngine engine, core::Random& measurementRng) {
    ...
    if (std::any_of(s.events.begin(), s.events.end(), [](std::uint8_t e) { return e != 0; })) {
        ...
        auto decoded = decoder_->decode(lookup ? SyndromeLattice::foldLayers(ex, s.events) : SyndromeLattice{s.events});
        if (!decoded) {
            s.decoderRejected = true;
        } else {
            s.correction = std::move(*decoded);
            s.predictedFlips = s.correction.observableMask;
            if (lookup)   // the table returns a Pauli: it flips readout l iff it anticommutes with logical l
                for (std::uint32_t l = 0; l < runner_->code_.k && l < 64; ++l)
                    if (!s.correction.pauli.commutesWith(runner_->code_.logical(ex.options.basis, l)))
