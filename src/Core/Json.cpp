...
struct KindInfo {
    int schema;
    std::vector<Upgrader> up;
};
std::map<std::string, KindInfo, std::less<>>& registry() {
    static std::map<std::string, KindInfo, std::less<>> r;
    return r;
}
std::mutex& regMutex() {
    static std::mutex m;
...
Result<Envelope> JsonEnvelope::parse(const std::string& text, std::string_view expectedKind) {
    ...
    if (j.is_discarded())
        return fail(ErrorCode::Parse, "malformed JSON");
    ...
    if (!expectedKind.empty() && e.kind != expectedKind)
        return fail(ErrorCode::InvalidArgument,
                    std::format("expected kind '{}', found '{}'", expectedKind, e.kind));
    KindInfo info{1, {}};
    {
        std::lock_guard lk(regMutex());
        auto it = registry().find(e.kind);
        if (it != registry().end())
            info = it->second;
    }
    if (e.schema > info.schema)
        return fail(ErrorCode::Unsupported,
                    std::format("'{}' schema {} is newer than supported {} (written by app {})",
                                e.kind, e.schema, info.schema, e.app));
    while (e.schema < info.schema) {
        std::size_t idx = static_cast<std::size_t>(e.schema - 1);
        if (idx >= info.up.size())
            return fail(ErrorCode::Unsupported,
                        std::format("no upgrader from schema {} for '{}'", e.schema, e.kind));
...
std::string JsonEnvelope::serialize(std::string_view kind, const Json& data, int schema) {
    Json j;
    j["qxl"] = {{"kind", std::string(kind)},
                {"schema", schema < 0 ? currentSchema(kind) : schema},
                {"app", std::string(version())},
                {"created", isoNow()}};
    j["data"] = data;
    return j.dump(2);
}
Status JsonEnvelope::save(const std::filesystem::path& path, std::string_view kind,
                          const Json& data) {
...
Result<std::string> readTextFile(const std::filesystem::path& p) {
    ...
    if (!f)
        return fail(ErrorCode::Io, "cannot open " + p.string());
    std::stringstream ss;
    ss << f.rdbuf();
    return ss.str();
}
...
Result<double> jsonQuantity(const Json& j, std::string_view expectedUnit) {
    ...
    if (j.is_object() && j.contains("value")) {
        if (!expectedUnit.empty() && j.contains("unit") &&
            j["unit"].get<std::string>() != expectedUnit)
            return fail(ErrorCode::InvalidArgument,
                        std::format("unit '{}' where '{}' expected", j["unit"].get<std::string>(),
                                    expectedUnit));
