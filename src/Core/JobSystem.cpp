...
void JobSystem::workerLoop(std::stop_token st) {
    for (;;) {
        Job job;
        {
            ...
            cv_.wait(lk, [&] {
                return shutdown_ || st.stop_requested() || !interactive_.empty() || !batch_.empty();
            });
            if ((shutdown_ || st.stop_requested()) && interactive_.empty() && batch_.empty())
                return;
            if (!interactive_.empty()) {
                job = std::move(interactive_.front());
                interactive_.pop_front();
            } else {
                job = std::move(batch_.front());
                batch_.pop_front();
            }
        }
...
void JobSystem::parallelFor(std::size_t n, std::size_t grain,
    ...
    auto work = [&] {
        ...
        {
            std::lock_guard lk(dmu);
            ++done;
        }
        dcv.notify_all();
    };
    for (std::size_t i = 0; i < helpers; ++i)
        enqueue([work](std::stop_token) { work(); }, JobPriority::Interactive);
    work();
