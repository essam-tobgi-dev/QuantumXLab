...
constexpr double kRailSetback_m = 0.030;  // front rails behind the cabinet's front edge (RackMeshes.cpp)
...
double unitRelief_m(const ComponentDescriptor& d) {
    return parseRackPanel(GenParams::merged(d.geometry, core::Json::object())).handles ? 0.034 : 0.013;
}
} // namespace
...
Status SceneBuilder::buildRacks(ComponentId root) {
    ...
    for (const auto& rack : layout_.racks) QXL_TRY(buildRack(root, rack, instanceOf, outputLines));
    return {};
}

Status SceneBuilder::buildRack(ComponentId root, const RackSpec& rack, std::map<std::string, int>& instanceOf,
    ...
    ComponentId group = addGroup(root, std::format("rack_{}", rack.id), std::format("Rack {}", rack.id), Group::Rack,
                                 Transform::at(rack.position_m + glm::dvec3(0.0, 0.5 * H, 0.0)));
    {   // The cabinet carries the rack's descriptor, so clicking the frame or a rail selects it.
        const gfx::MeshData enclosure = props::rackEnclosure(static_cast<float>(kRackWidth_m), static_cast<float>(H),
    ...
    int slot = 0;
    std::vector<std::pair<double, double>> unitRears; // (y centre, z of the chassis rear) for the loom stubs
    for (const auto& [unitId, count] : rack.units) {
        ...
        for (int k = 0; k < std::max(1, count); ++k) {
            ...
            spec.instance = count > 1 ? std::format("rack_{}.{}[{}]", rack.id, unitId, index) : std::format("rack_{}.{}", rack.id, unitId);
            spec.display = count > 1 ? std::format("{} {}", d->name, index + 1) : d->name;
            spec.group = Group::Rack;
            spec.material = "panel_flat";   // vertex colours, no powder-coat normal over the engraving
    ...
    if (scene_.catalog().contains("cable_loom") && !unitRears.empty()) {
        gfx::MeshData loom;
        const float xBar = static_cast<float>(0.5 * kRackWidth_m - 0.09), zBar = static_cast<float>(-0.5 * D + 0.08);
        const float brushZ = static_cast<float>(-0.5 * D + 0.12), roof = static_cast<float>(0.5 * H);
        ...
        for (const auto& [y, zRear] : unitRears)
            for (int k = 0; k < 4; ++k) {
                ...
                glm::vec3 a{0.10f - 0.03f * static_cast<float>(k), static_cast<float>(y) + dy, static_cast<float>(zRear)};
        ...
        for (int k = 0; k < 12; ++k) { // the vertical bundle: 12 runs in a 3 × 4 pack
            glm::vec3 off{0.007f * static_cast<float>(k % 3) - 0.007f, 0.0f, 0.007f * static_cast<float>(k / 3) - 0.0105f};
            std::vector<glm::vec3> path{{xBar, bottom, zBar}, {xBar, roof - 0.15f, zBar}, {0.10f, roof - 0.02f, brushZ}, {0.0f, roof + 0.25f, brushZ},
                                        {0.0f, trayY, 0.0f}};
            for (auto& p : path) p += off;
