...
class Router {
  public:
        ...
        for (int j = 0; j < ny_; ++j)
            for (int i = 0; i < nx_; ++i) {
                ...
                for (const auto& q : L.qubits)
                    b = b || glm::length(c - q.pos_um) < q.radius_um() + cw;
                for (const auto& r : L.resonators)
                    b = b || r.footprint.contains(c, cw);
                for (const auto& p : L.bondPads) {
                    glm::dvec2 d = glm::abs(c - p.pos_um);
                    b = b ||
                        (d.x < 0.5 * o.bondPadSize_um + cw && d.y < 0.5 * o.bondPadSize_um + cw);
                }
    ...
    void blockRect(const ChipRect& r, double inflate) {
        for (int j = 0; j < ny_; ++j)
            for (int i = 0; i < nx_; ++i)
                if (r.contains(centre(i, j), inflate))
                    blocked_[idx(i, j)] = 1;
    }
    ...
    std::vector<glm::dvec2> route(glm::dvec2 from, glm::dvec2 to, double freeRadius) {
        ...
        const int S = nx_ * ny_;
        std::vector<double> g(static_cast<std::size_t>(S) * 5,
        ...
        while (!open.empty()) {
            ...
            if (f - h(i, j) > g[static_cast<std::size_t>(st)] + 1e-9)
                continue;
            if (i == gi && j == gj) {
                goalState = st;
                break;
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (ni < 0 || nj < 0 || ni >= nx_ || nj >= ny_)
                    continue;
                glm::dvec2 c = centre(ni, nj);
                bool free =
                    glm::length(c - from) <= freeRadius || glm::length(c - to) <= freeRadius;
                if (blocked_[idx(ni, nj)] && !free)
                    continue;
                double step = cell_ * (1.0 + 3.0 * usage_[idx(ni, nj)]) +
                              ((dir != 4 && dir != d) ? 0.6 * cell_ : 0.0);
        ...
        for (int st = goalState; st >= 0; st = parent[static_cast<std::size_t>(st)])
            cells.push_back(st / 5);
        std::reverse(cells.begin(), cells.end());
        for (int c : cells)
            usage_[static_cast<std::size_t>(c)] =
                static_cast<std::uint8_t>(std::min(200, usage_[static_cast<std::size_t>(c)] + 1));
        path.push_back(from);
        for (std::size_t k = 1; k + 1 < cells.size(); ++k) { // keep corner cells only
            glm::dvec2 a = centre(cells[k - 1] % nx_, cells[k - 1] / nx_),
                       b = centre(cells[k] % nx_, cells[k] / nx_),
                       c = centre(cells[k + 1] % nx_, cells[k + 1] / nx_);
            if (glm::length(glm::normalize(b - a) - glm::normalize(c - b)) > 1e-9)
                path.push_back(b);
        }
        path.push_back(to);
        return path;
...
int takePad(ChipLayout& L, int edge, glm::dvec2 near, int signal) {
    ...
    for (int pass = 0; pass < 2 && best < 0; ++pass)
        for (std::size_t k = 0; k < L.bondPads.size(); ++k) {
            ...
            if (b.signal >= 0 || (pass == 0 && b.edge != edge))
                continue;
            double dist = glm::length(b.pos_um - near);
            if (dist < bestD)
                bestD = dist, best = static_cast<int>(k);
        }
    if (best >= 0)
        L.bondPads[static_cast<std::size_t>(best)].signal = signal;
    return best;
}
...
void routeLines(ChipLayout& L, const hw::Device& dev, const ChipLayoutOptions& o) {
    ...
    auto connect = [&](std::vector<glm::dvec2>& path, glm::dvec2 a, glm::dvec2 b, double freeR,
                       bool& ok) {
    ...
    for (std::size_t g = 0; g < groups.size(); ++g) {
        ...
        for (const auto& r : L.resonators)
            if (r.feedline == f.id)
                taps.push_back(r.tap_um);
        if (taps.empty())
            continue;
        std::sort(taps.begin(), taps.end(),
                  [](glm::dvec2 a, glm::dvec2 b) { return a.x != b.x ? a.x < b.x : a.y < b.y; });
        ...
        glm::dvec2 in = f.padIn >= 0 ? L.bondPads[static_cast<std::size_t>(f.padIn)].pos_um
                                     : glm::dvec2(0.0, taps.front().y);
        glm::dvec2 out = f.padOut >= 0 ? L.bondPads[static_cast<std::size_t>(f.padOut)].pos_um
                                       : glm::dvec2(L.size_um.x, taps.back().y);
        // λ/4 Purcell filter in series at the feedline input, folded along the left edge above the
        // pad
        double fMean = 0.0;
        for (auto q : f.qubits)
            fMean += q < dev.readout.resonatorFrequencies.size()
                         ? dev.readout.resonatorFrequencies[q].v
                         : 7e9;
        fMean /= static_cast<double>(std::max<std::size_t>(1, f.qubits.size()));
        std::vector<glm::dvec2> local =
            meanderCenterline(quarterWaveLength_m(fMean, o.epsEff) * 1e6, o.meanderPitch_um,
                              o.meanderAmplitude_um, o.meanderLead_um);
        double ampEff = 0.0;
        for (const auto& p : local)
            ampEff = std::max(ampEff, std::abs(p.y));
        glm::dvec2 origin = in + glm::dvec2(padReach + o.routeGrid_um, 0.0);
        for (const auto& p : local)
            f.purcell.path_um.push_back(origin + glm::dvec2(ampEff + p.y, p.x));
        ChipRect pf;
        ...
        if (!ok)
            L.diagnostics.push_back(
                std::format("feedline {}: a segment could not be routed around obstacles", f.id));
    ...
    auto addLine = [&](std::uint32_t target, bool flux) {
        ...
        if (flux)
            edge = (edge + 2) % 4;
        glm::dvec2 terminal =
            s.pos_um + edgeAxis(edge) * (s.radius_um() + o.clearance_um +
                                         2.0 * (0.5 * o.cpwWidth_um + o.cpwGap_um));
        for (int turn = 1; turn < 4;
             ++turn) { // step around the pad if a resonator sits on that side
            bool inside =
                std::any_of(L.resonators.begin(), L.resonators.end(), [&](const ResonatorSite& r) {
                    return r.footprint.contains(terminal, o.clearance_um);
                });
            if (!inside)
                break;
            terminal =
                s.pos_um + edgeAxis(edge + turn) * (s.radius_um() + o.clearance_um +
                                                    2.0 * (0.5 * o.cpwWidth_um + o.cpwGap_um));
        }
        ControlLine c;
        c.target = target;
        ...
        glm::dvec2 start =
            c.bondPad >= 0 ? L.bondPads[static_cast<std::size_t>(c.bondPad)].pos_um : terminal;
        glm::dvec2 inward = c.bondPad >= 0
                                ? -L.bondPads[static_cast<std::size_t>(c.bondPad)].outward
                                : glm::dvec2(0.0);
        std::vector<glm::dvec2> seg = router.route(start + inward * padReach, terminal, freePad);
        c.routed = !seg.empty();
        c.route.path_um =
            c.routed ? seg : std::vector<glm::dvec2>{start + inward * padReach, terminal};
        if (!c.routed)
            L.diagnostics.push_back(std::format("{} line of q{}: drawn straight (unroutable)",
                                                flux ? "flux" : "drive", target));
        (flux ? L.fluxLines : L.driveLines).push_back(std::move(c));
    };
    for (const auto& s : L.qubits)
        if (!s.coupler)
            addLine(s.index, false);
    for (const auto& s : L.qubits)
        if (s.tunable || s.coupler)
            addLine(s.index, true);
}
