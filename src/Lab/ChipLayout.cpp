...
Key keyOf(const hw::QubitInfo& q) {
    return {std::lround(q.pos[0]), std::lround(q.pos[1])};
}
} // namespace
...
ChipTopology classifyTopology(const hw::Device& dev) {
    ...
    for (const auto& q : dev.qubits)
        if (q.kind != hw::QubitKind::Coupler)
            data.push_back(&q);
    ...
    for (const auto* q : data)
        maxDeg = std::max(maxDeg, adj[q->index].size());
    bool allIntegral = std::all_of(data.begin(), data.end(), [](auto* q) {
        return integral(q->pos[0]) && integral(q->pos[1]);
    });
    std::map<Key, std::uint32_t> at;
    for (const auto* q : data)
        at[keyOf(*q)] = q->index;
    auto posOf = [&](std::uint32_t i) { return keyOf(dev.qubits[i]); };
    bool unitEdges = allIntegral && at.size() == n &&
                     std::all_of(dev.edges.begin(), dev.edges.end(), [&](const hw::EdgeInfo& e) {
    ...
    if (unitEdges) { // full R × C rectangle with every Manhattan neighbour coupled
        ...
        auto C = static_cast<std::size_t>(maxX - minX + 1),
             R = static_cast<std::size_t>(maxY - minY + 1);
        if (R * C == n && E == R * (C - 1) + C * (R - 1) && R > 1 && C > 1)
            return ChipTopology::Grid;
    }
...
double CpwRoute::length_um() const {
    double s = 0.0;
    for (std::size_t i = 1; i < path_um.size(); ++i)
        s += glm::length(path_um[i] - path_um[i - 1]);
    return s;
}

namespace {
glm::dvec2 axisOf(const ChipRect& r) {
    return {std::cos(r.angle_rad), std::sin(r.angle_rad)};
}
...
bool separated(const ChipRect& A, const ChipRect& B, double clearance) {
    glm::dvec2 axes[4] = {
        axisOf(A), {-axisOf(A).y, axisOf(A).x}, axisOf(B), {-axisOf(B).y, axisOf(B).x}};
    glm::dvec2 d = B.center - A.center;
    for (const auto& ax : axes) {
        auto extent = [&](const ChipRect& r) {
            glm::dvec2 u = axisOf(r), v{-u.y, u.x};
            return r.halfSize.x * std::abs(glm::dot(u, ax)) +
                   r.halfSize.y * std::abs(glm::dot(v, ax));
        };
        if (std::abs(glm::dot(d, ax)) > extent(A) + extent(B) + clearance)
            return true;
    }
    return false;
}
...
bool ChipRect::overlaps(const ChipRect& o, double clearance) const {
    return !separated(*this, o, clearance);
}

bool ChipRect::contains(glm::dvec2 p, double inflate) const {
    glm::dvec2 u = axisOf(*this), v{-u.y, u.x}, d = p - center;
    return std::abs(glm::dot(d, u)) <= halfSize.x + inflate &&
           std::abs(glm::dot(d, v)) <= halfSize.y + inflate;
}
...
std::vector<std::string> ChipLayout::overlaps(double clearance) const {
    ...
    for (std::size_t i = 0; i < qubits.size(); ++i)
        for (std::size_t j = i + 1; j < qubits.size(); ++j)
            if (glm::length(qubits[i].pos_um - qubits[j].pos_um) <
                qubits[i].radius_um() + qubits[j].radius_um() + clearance)
    ...
    for (const auto& r : resonators) {
        for (const auto& q : qubits)
            if (r.footprint.overlapsCircle(q.pos_um, q.radius_um() + clearance))
                out.push_back(
                    std::format("resonator of q{} overlaps the pad of q{}", r.qubit, q.index));
...
Result<ChipLayout> layoutChip(const hw::Device& dev, const hw::Calibration* cal,
    ...
    if (dev.qubits.empty())
        return fail(kErrChip, std::format("device '{}' has no qubits", dev.id));
    ChipLayout L;
    L.device = dev.id;
    ...
    for (const auto& q : L.qubits)
        signals += (q.coupler ? 0 : 1) + (q.tunable ? 1 : 0);
