...
const BenchItem kBenchItems[]{{"microscope", {-0.45, 0.0, 0.05}, 0.0}, {"wire_bonder", {0.30, 0.0, 0.0}, 0.0},
                              {"sample_box", {-0.05, 0.0, 0.25}, 15.0}, {"power_meter", {-0.05, 0.0, -0.22}, 0.0}};
} // namespace
    ...
    for (const auto& prop : layout_.props) {
        ...
        for (int k = 0; k < std::max(1, prop.count); ++k) {
            ...
            if (prop.id == "workstation") { // desk at 0.75 m with two monitors on arms (spec 17 §2)
                const auto w = static_cast<float>(d ? d->geometryNumber("w_m", 1.6) : 1.6), depth = static_cast<float>(d ? d->geometryNumber("d_m", 0.8) : 0.8);
                mesh = props::workstation(w, depth);
                at.y += 0.75; // monitors along the desk's −z edge, the chair on the +z side facing them
                addScenery(props, "office_chair", "Task chair", Group::Room, Transform::rotated(prop.position_m + glm::dvec3(0.0, 0.0, 0.75), {0.0, 1.0, 0.0}, 180.0),
                           props::officeChair(), "vertex");
            } else if (prop.id == "bench") { // optical-breadboard assembly bench, 0.9 m high
                const auto w = static_cast<float>(d ? d->geometryNumber("w_m", 1.8) : 1.8), h = static_cast<float>(d ? d->geometryNumber("h_m", 0.9) : 0.9);
                ...
                const auto r = static_cast<float>(d ? d->geometryNumber("r_m", 0.25) : 0.25), h = static_cast<float>(d ? d->geometryNumber("h_m", 1.3) : 1.3);
                mesh = props::heDewar(r, h);
                spec.material = "stainless";
                at.x += 0.55 * k; // the pair stands side by side by the wall, behind the GHS bookmark's
            ...
            if (simple.triangleCount() > 0) spec.authoredSimple = &simple;
            const std::string propId = prop.id;
            const ComponentId propNode = addProp(props, std::move(spec), mesh);
            if (propId == "bench" && propNode.value != 0) QXL_TRY(buildBenchInstruments(propNode));
        }
    }
    if (layout_.compressor) { // pulse-tube compressor package; outside the −X wall when the layout says so,
        // seen through the wall window (BuildRoomDetail.cpp), its helium flex lines through the wall
        const ComponentDescriptor* d = scene_.catalog().find("pt_compressor");
        const auto w = static_cast<float>(d ? d->geometryNumber("w_m", 0.7) : 0.7), h = static_cast<float>(d ? d->geometryNumber("h_m", 1.0) : 1.0);
        const auto depth = static_cast<float>(d ? d->geometryNumber("d_m", 0.8) : 0.8);
        glm::dvec3 at = *layout_.compressor;
        if (layout_.compressorOutside) at.x = std::min(at.x, -0.5 * layout_.roomWidth_m - 0.1 - 0.5 * static_cast<double>(w) - 0.25);
        NodeSpec spec;
        ...
        spec.group = Group::Room;
        spec.material = "vertex";
        spec.local = Transform::rotated(at, {0.0, 1.0, 0.0}, 90.0); // control panel toward the window
...
Status SceneBuilder::buildBenchInstruments(ComponentId bench) {
    for (const BenchItem& item : kBenchItems) {
        ...
        if (!d) continue;
        const auto w = static_cast<float>(d->geometryNumber("w_m", 0.2)), h = static_cast<float>(d->geometryNumber("h_m", 0.2));
        const auto depth = static_cast<float>(d->geometryNumber("d_m", 0.2));
        gfx::MeshData mesh;
        const std::string id = item.id;
        if (id == "microscope") mesh = props::microscope(w, h, depth);
        else if (id == "wire_bonder") mesh = props::wireBonder(w, h, depth);
        else if (id == "sample_box") mesh = props::sampleBox(w, h, depth);
            ...
            mesh::appendColored(mesh, mesh::box({0.0f, 0.6f * h, 0.5f * depth + 0.001f}, {0.7f * w, 0.35f * h, 0.002f}), {0.05f, 0.12f, 0.08f, 1.0f});
            mesh::appendColored(mesh, gfx::shapes::cylinder(0.012f, 0.06f, 12, true), {0.25f, 0.25f, 0.27f, 1.0f},
                                mesh::translate({0.7f * w, 0.012f, 0.1f}) * mesh::alignY({0.0f, 0.0f, 1.0f}));
            mesh::appendColored(mesh, gfx::shapes::tube({{0.5f * w, 0.5f * h, 0.0f}, {0.62f * w, 0.02f, 0.06f}, {0.7f * w, 0.012f, 0.07f}}, 0.003f, 6, true),
