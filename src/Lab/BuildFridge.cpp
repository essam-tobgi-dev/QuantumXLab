...
NodeSpec part(std::string descriptor, std::string instance, Group group, std::string material,
              Transform local) {
    NodeSpec s;
...
double SceneBuilder::plateThickness(cryo::Stage s) const {
    ...
    return d ? d->geometryNumber("t_m", s == cryo::Stage::RT ? 0.02 : 0.012)
             : (s == cryo::Stage::RT ? 0.02 : 0.012);
}

ComponentId SceneBuilder::stageNode(cryo::Stage s) const {
    return scene_.stageNodes()[static_cast<std::size_t>(s)];
}
...
Status SceneBuilder::buildFridge(ComponentId root) {
    ...
    if (!hasStage(Stage::RT))
        return fail(kErrLayout, "layout has no RT stage");
    ...
    NodeSpec motor =
        part("pt_remote_motor", "pt_remote_motor", Group::FridgeExterior, "plastic_grey",
             Transform::at(0.45 * layout_.frameWidth_m, 0.5 * frameH - 0.075,
                           0.45 * layout_.frameDepth_m));
    ...
    if (scene_.catalog().contains("rt_electronics_box")) { // thermometry bridge on the frame side
        NodeSpec box =
            part("rt_electronics_box", "rt_electronics_box", Group::FridgeExterior, "plastic_grey",
    ...
    NodeSpec ptHead = part(
        "pulse_tube_head", "pulse_tube_head", Group::FridgeExterior, "stainless",
    ...
    {
        std::vector<glm::dvec3> path{{0.0, 0.5 * headH, 0.0},
                                     {0.0, 0.5 * headH + 0.25, 0.0},
                                     {-0.8, 0.5 * headH + 0.45, 0.6}};
        glm::dvec3 target = layout_.compressor.value_or(glm::dvec3(-3.2, 0.4, 2.6));
        glm::dvec3 headWorld =
            layout_.fridgePosition_m + polar(kPulseTubeAzimuth_deg, kPulseTubeRadius_m,
                                             stageHeight(Stage::RT) + 0.5 * rtT + 0.5 * headH);
        path.push_back(target + glm::dvec3(0.0, 0.6, 0.0) - headWorld);
        path.push_back(target + glm::dvec3(0.0, 0.4, 0.0) - headWorld);
        NodeSpec flex =
            part("pt_flex_lines", "pt_flex_lines", Group::FridgeExterior, "stainless", {});
    ...
    for (const auto& sb : kInterior) {
        if (!hasStage(sb.stage))
            continue;
        std::string id(stageDescriptorId(sb.stage));
        NodeSpec plate = part(id, id, Group::FridgeInterior, sb.material,
                              Transform::at(0.0, stageHeight(sb.stage), 0.0));
        plate.overrides = {{"r_m", stageRadius(sb.stage)},
                           {"t_m", plateThickness(sb.stage)},
                           {"holes", "bolt_circle"}};
        if (sb.pumpHole)
            plate.overrides["through_hole_m"] = pumpHole;
        ...
        for (int k = 0; k < kPostsPerStage && L > 0.01; ++k) {
            ...
            NodeSpec post = part(
                "support_post", std::format("support_post.{}[{}]", stageLayoutName(sb.stage), k),
                Group::FridgeInterior, stainless ? "stainless" : "g10",
                Transform::at(polar(az, ring, -0.5 * plateThickness(above) - 0.5 * L)));
            post.display = std::format("Support post {} ({} → {})", k + 1, cryo::stageName(above),
                                       cryo::stageName(sb.stage));
            post.overrides = {{"d_m", stainless ? 0.020 : 0.012},
                              {"L_m", L},
                              {"wall_m", stainless ? 0.0005 : 0.006}};
    ...
    const double magHeight =
        scene_.catalog().contains("mag_shield_mumetal")
    ...
    if (layout_.shields) {
        const struct {
            ...
            {Stage::STILL, "shield_still"}, {Stage::PT2, "shield_4K"}, {Stage::PT1, "shield_50K"}};
        for (const auto& sh : kShields) {
            ...
            if (std::abs(authored - height) > 0.01)
                note(std::format(
                    "{}: height fitted to {:.3f} m for nesting (descriptor says {:.3f} m)", sh.id,
                    height, authored));
            NodeSpec shield = part(sh.id, sh.id, Group::FridgeInterior, "aluminium",
                                   Transform::at(0.0, -0.5 * plateThickness(sh.stage), 0.0));
            shield.overrides = {{"h_m", height},
                                {"r_m", r},
                                {"flange_bolts", static_cast<int>(std::lround(
                                                     2.0 * glm::pi<double>() * r / 0.06))}};
    ...
    { // outer vacuum can: long enough to close over the outermost shield
        ...
        double height =
            std::max(d->geometryNumber("h_m", 1.35), rim - (innerBottom - kShieldClearance_m));
        NodeSpec ovc = part("ovc", "ovc", Group::FridgeExterior, "stainless",
                            Transform::at(0.0, -0.5 * rtT, 0.0));
