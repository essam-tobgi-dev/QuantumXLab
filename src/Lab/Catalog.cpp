...
data::FidelityClass fidelityFromName(std::string_view s) {
    ...
    if (s == "Exact") return F::Exact;
    if (s == "Numerical") return F::Numerical;
    if (s == "Statistical") return F::Statistical;
    if (s == "Illustrative") return F::Illustrative;
    return F::Model;
}
...
std::string ComponentDescriptor::geometryString(std::string_view key, std::string_view fallback) const {
    auto it = geometry.find(key);
...
Detail ComponentDescriptor::detailAt(double distance_m) const {
    for (const auto& r : lod)
        if (distance_m <= r.maxDistance_m) return r.detail;
...
Result<std::string> requiredString(const core::Json& j, const char* key, const std::filesystem::path& from) {
    auto it = j.find(key);
    if (it == j.end() || !it->is_string() || it->get<std::string>().empty())
        return fail(kErrDescriptor, std::format("{}: missing required string '{}'", from.string(), key));
...
Result<SpecRow> parseRow(const core::Json& r, const std::filesystem::path& from) {
    ...
    if (row.field.empty())
        return fail(kErrDescriptor, std::format("{}: spec_sheet row without 'field'", from.string()));
...
Result<ComponentDescriptor> ComponentCatalog::parse(const core::Json& j, const std::filesystem::path& from) {
    ...
    if (auto ins = j.find("instrument"); ins != j.end() && !ins->is_null()) {
        ...
        if (info.kind.empty())
            return fail(kErrDescriptor, std::format("{}: 'instrument' has no 'class'", from.string()));
    ...
    if (sheet == j.end() || !sheet->is_array() || sheet->empty())
        return fail(kErrDescriptor, std::format("{}: 'spec_sheet' must have at least one row", from.string()));
    ...
    if (lod == j.end() || !lod->is_array() || lod->empty())
        return fail(kErrDescriptor, std::format("{}: 'lod' must have at least one level", from.string()));
    for (const auto& l : *lod) {
        ...
        if (!detailFromName(det, r.detail))
            return fail(kErrDescriptor, std::format("{}: unknown lod detail '{}'", from.string(), det));
        d.lod.push_back(r);
    }
    std::stable_sort(d.lod.begin(), d.lod.end(),
                     [](const LodRule& a, const LodRule& b) { return a.maxDistance_m < b.maxDistance_m; });
    return d;
}
...
Status ComponentCatalog::validate(const ComponentDescriptor& d) {
    ...
    if (words < 40 || words > 200)
        problems.push_back(std::format("'function' has {} words, spec 17 §4 requires 40..200", words));
    std::size_t chars = utf8Length(d.tooltip);
    if (chars == 0 || chars > 90)
        problems.push_back(std::format("'tooltip' has {} characters, spec 17 §4 requires 1..90", chars));
    ...
    for (const auto& r : d.specSheet) {
        if (!r.unitDeclared) problems.push_back(std::format("row '{}' has no 'unit'", r.field));
        if (!r.binding) continue;
        if (!r.classDeclared) problems.push_back(std::format("live row '{}' has no 'class'", r.field));
        if (auto root = splitBindingRoot(*r.binding); !root)
            problems.push_back(std::format("row '{}': {}", r.field, root.error().message));
    }
    if (d.lod.empty()) problems.push_back("'lod' is empty");
    if (problems.empty()) return {};
    Error e(kErrDescriptor, std::format("component '{}' fails lint", d.id));
    for (auto& p : problems) e.withNote(std::move(p));
...
Result<ComponentCatalog> ComponentCatalog::load(const std::filesystem::path& dirIn) {
    ...
    for (const auto& e : std::filesystem::directory_iterator(dir, ec)) {
        ...
        if (std::filesystem::exists(f, ec)) files.push_back(f);
    }
    ...
    for (const auto& f : files) {
        ...
        if (d.id != f.parent_path().filename().string())
            return fail(kErrDescriptor, std::format("{}: id '{}' does not match its directory", f.string(), d.id));
        if (cat.byId_.contains(d.id)) return fail(kErrDescriptor, std::format("duplicate component id '{}'", d.id));
        cat.byId_.emplace(d.id, cat.items_.size());
        cat.items_.push_back(std::move(d));
    }
    if (cat.items_.empty()) return fail(kErrDescriptor, "no component.json found under " + dir.string());
    return cat;
}
...
std::vector<std::string> ComponentCatalog::ids() const {
    ...
    for (const auto& d : items_) v.push_back(d.id);
    return v;
}
