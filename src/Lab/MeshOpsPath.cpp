...
std::vector<glm::vec3> dedupe(const std::vector<glm::vec3>& pts, float eps) {
    ...
    for (const auto& p : pts)
        if (out.empty() || glm::length(p - out.back()) > eps) out.push_back(p);
    return out;
}
...
MeshData ribbon(const std::vector<glm::vec2>& path, float width, float y0, float y1) {
    ...
    if (p.size() < 2 || width <= 0.0f) return m;
    std::vector<glm::vec2> L = offsetPolyline(p, 0.5f * width), R = offsetPolyline(p, -0.5f * width);
    ...
    quad(at(L.front(), y0), at(R.front(), y0), at(R.front(), y1), at(L.front(), y1), {-d0.x, 0, -d0.y});
...
double polylineLength(const std::vector<glm::dvec3>& pts) {
    double s = 0.0;
    for (std::size_t i = 1; i < pts.size(); ++i) s += glm::length(pts[i] - pts[i - 1]);
    return s;
}
...
std::vector<glm::dvec3> catmullRom(const std::vector<glm::dvec3>& p, int sub) {
    ...
    for (std::size_t i = 0; i + 1 < p.size(); ++i) {
        ...
        for (int s = 0; s < sub; ++s) out.push_back(cr(p0, p[i], p[i + 1], p3, static_cast<double>(s) / sub));
    }
...
glm::dvec3 pointAt(const std::vector<glm::dvec3>& pts, double s, glm::dvec3* tangent) {
    if (pts.empty()) return glm::dvec3(0.0);
    if (pts.size() == 1) {
        if (tangent) *tangent = glm::dvec3(0, -1, 0);
        return pts[0];
    }
    s = std::max(0.0, s);
...
MeshData clipByPlane(const MeshData& m, const glm::dvec4& plane) {
    MeshData out;
    auto dist = [&](const gfx::Vertex& v) { return glm::dot(glm::dvec3(plane), glm::dvec3(v.position)) + plane.w; };
    ...
    for (std::size_t i = 0; i + 2 < m.indices.size(); i += 3) {
        std::array<gfx::Vertex, 3> tri{m.vertices[m.indices[i]], m.vertices[m.indices[i + 1]], m.vertices[m.indices[i + 2]]};
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            double da = d[static_cast<std::size_t>(k)], db = d[static_cast<std::size_t>((k + 1) % 3)];
            if (da >= 0) poly.push_back(a);
            if ((da >= 0) != (db >= 0)) poly.push_back(lerpV(a, b, static_cast<float>(da / (da - db))));
        }
        auto base = static_cast<std::uint32_t>(out.vertices.size());
        out.vertices.insert(out.vertices.end(), poly.begin(), poly.end());
        for (std::uint32_t k = 1; k + 1 < poly.size(); ++k) out.indices.insert(out.indices.end(), {base, base + k, base + k + 1});
    }
    return out;
}

std::string validate(const MeshData& m, double normalTol) {
    if (m.vertices.empty() || m.indices.empty()) return "empty mesh";
    if (m.indices.size() % 3 != 0) return std::format("index count {} is not a triangle list", m.indices.size());
    for (std::size_t i = 0; i < m.indices.size(); ++i)
        if (m.indices[i] >= m.vertices.size())
            return std::format("index {} = {} out of range ({} vertices)", i, m.indices[i], m.vertices.size());
