...
std::string_view stageLayoutName(cryo::Stage s) {
    return kStageNames[static_cast<std::size_t>(s)].layout;
}
std::string_view stageDescriptorId(cryo::Stage s) {
    return kStageNames[static_cast<std::size_t>(s)].descriptor;
}

std::filesystem::path resolveLayoutDir(const std::filesystem::path& p) {
    std::error_code ec;
    if (std::filesystem::exists(p / "layout.json", ec))
        return p;
...
Result<LayoutSpec> loadLayout(const std::filesystem::path& dirOrId) {
    ...
    if (auto f = d.find("fridge"); f != d.end() && f->is_object()) {
        ...
        if (auto p = f->find("position_m"); p != f->end())
            L.fridgePosition_m = vec3(*p);
        ...
        const core::Json* heights =
            f->contains("stage_height_m") ? &(*f)["stage_height_m"] : nullptr;
        ...
        if (auto st = f->find("stages"); st != f->end() && st->is_array())
            for (const auto& s : *st) {
                ...
                if (!stageFromLayoutName(spec.name, spec.stage))
                    return fail(kErrLayout,
                                std::format("layout '{}': unknown stage '{}'", L.id, spec.name));
                spec.height_m = heights && heights->contains(spec.name)
                                    ? (*heights)[spec.name].get<double>()
                                    : 2.0;
                spec.radius_m =
                    radii && radii->contains(spec.name) ? (*radii)[spec.name].get<double>() : 0.2;
                L.stages.push_back(std::move(spec));
            }
    }
    L.device = d.value("device", std::string{});
    if (d.contains("wiring"))
        L.wiringFile = resolveAssetPath(d["wiring"].get<std::string>());
    L.routingFile = d.contains("routing") ? resolveAssetPath(d["routing"].get<std::string>())
                                          : L.dir / "routing.json";
    if (auto racks = d.find("racks"); racks != d.end() && racks->is_array())
        for (const auto& r : *racks) {
            RackSpec rk;
            rk.id = r.value("id", std::string{});
            if (auto p = r.find("position_m"); p != r.end())
                rk.position_m = vec3(*p);
    ...
    if (auto g = d.find("ghs"); g != d.end() && g->is_object() && g->contains("position_m"))
        L.ghs = vec3((*g)["position_m"]);
    if (auto cm = d.find("compressor"); cm != d.end() && cm->is_object()) {
        if (cm->contains("position_m"))
            L.compressor = vec3((*cm)["position_m"]);
        L.compressorOutside = cm->value("outside", false);
    }
    if (auto ch = d.find("chamber"); ch != d.end() && ch->is_object()) {
        if (ch->contains("position_m"))
            L.chamberPosition_m = vec3((*ch)["position_m"]);
        L.chamberRadius_m = ch->value("radius_m", 0.12);
    }
    if (auto ls = d.find("lasers"); ls != d.end() && ls->is_array())
        for (const auto& l : *ls)
            L.lasers.push_back({l.value("id", std::string("laser_path")),
                                l.value("wavelength_nm", 369.0), l.value("role", std::string{})});
        ...
        for (const auto& p : *props) {
            ...
            if (p.is_string()) {
                ...
                if (p.contains("position_m"))
                    ps.position_m = vec3(p["position_m"]);
    ...
    if (auto bm = d.find("bookmarks"); bm != d.end() && bm->is_object())
        for (auto it = bm->begin(); it != bm->end(); ++it) {
            ...
            b.view.name = it.key();
            b.view.position =
                vec3(it.value().value("pos", core::Json::array()), glm::dvec3(5, 3, 6));
    ...
    if (auto ly = d.find("layers"); ly != d.end() && ly->is_array())
        for (const auto& l : *ly)
            L.layers.push_back(l.get<std::string>());
    if (auto lights = d.find("lights"); lights != d.end() && lights->is_array())
        for (const auto& l : *lights) {
            LightSpec spec;
            if (auto pos = l.find("position_m"); pos != l.end())
                spec.position_m = vec3(*pos);
            if (auto sz = l.find("size_m"); sz != l.end() && sz->is_array() && sz->size() >= 2)
                spec.size_m = {(*sz)[0].get<double>(), (*sz)[1].get<double>()};
            if (auto col = l.find("color"); col != l.end())
                spec.color = glm::vec3(vec3(*col, glm::dvec3(1.0)));
            spec.lumens = l.value("lumens", 4000.0);
            if (spec.lumens <= 0.0)
                return fail(kErrLayout,
                            std::format("layout '{}': light with non-positive lumens", L.id));
...
Result<RoutingSpec> loadRouting(const std::filesystem::path& file) {
    ...
    if (auto so = d.find("stage_order"); so != d.end() && so->is_array())
        for (const auto& s : *so)
            R.stageOrder.push_back(s.get<std::string>());
    ...
    if (auto lb = d.find("line_bundles"); lb != d.end() && lb->is_array())
        for (const auto& b : *lb) {
            ...
            if (auto sec = b.find("sector_deg");
                sec != b.end() && sec->is_array() && sec->size() >= 2) {
            ...
            if (auto rad = b.find("radius_m"); rad != b.end() && rad->is_object())
                for (auto it = rad->begin(); it != rad->end(); ++it)
                    bundle.radius_m[it.key()] = it.value().get<double>();
            R.bundles.push_back(std::move(bundle));
        }
    if (auto sl = d.find("element_slots"); sl != d.end() && sl->is_object())
        for (auto it = sl->begin(); it != sl->end(); ++it)
            if (it.value().contains("stage_offset_m"))
                R.slots[it.key()] = vec3(it.value()["stage_offset_m"]);
    if (auto seg = d.find("segment_lengths_m"); seg != d.end() && seg->is_object())
        for (auto it = seg->begin(); it != seg->end(); ++it)
            R.segmentLengths_m[it.key()] = it.value().get<double>();
