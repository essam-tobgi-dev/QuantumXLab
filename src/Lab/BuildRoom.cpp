...
SceneBuilder::WallOpening SceneBuilder::doorOpening() const {
    return WallOpening{0.3 * layout_.roomDepth_m, 0.90, 0.0, 2.10}; // spec 17 §2: door in the −X wall at z = 0.3 D
}

std::optional<SceneBuilder::WallOpening> SceneBuilder::windowOpening() const {
    if (!layout_.compressor || !layout_.compressorOutside) return std::nullopt;
    // A 0.6 × 0.9 m window at eye height, centred on the compressor and clear of the door frame.
    const WallOpening door = doorOpening();
    const double zMin = door.z + 0.5 * door.width + 0.08 + 0.30, zMax = 0.5 * layout_.roomDepth_m - 0.08 - 0.30;
    if (zMin > zMax) return std::nullopt;
...
Status SceneBuilder::buildRoom(ComponentId root) {
    ...
    {   // −X wall as pieces around the door (and window) openings, so they are real openings
        ...
        if (auto win = windowOpening()) openings.push_back(*win);
        std::sort(openings.begin(), openings.end(), [](const WallOpening& a, const WallOpening& b) { return a.z < b.z; });
        gfx::MeshData wall;
        auto piece = [&](double z0, double z1, double y0, double y1) {
            if (z1 - z0 < 1e-6 || y1 - y0 < 1e-6) return;
            mesh::append(wall, mesh::box({0.0f, static_cast<float>(0.5 * (y0 + y1)), static_cast<float>(0.5 * (z0 + z1))},
        ...
        piece(z, 0.5 * D, 0.0, H);
        addScenery(room, "wall_x", "Wall (−X)", Group::Room, Transform::at(-0.5 * W - 0.05, 0.0, 0.0), std::move(wall), "wall");
    }
    {   // skirting board along both walls, interrupted at the door
        const WallOpening door = doorOpening();
        gfx::MeshData skirting;
        const float dz0 = static_cast<float>(door.z - 0.5 * door.width - 0.08), dz1 = static_cast<float>(door.z + 0.5 * door.width + 0.08);
        mesh::append(skirting, mesh::box({-0.5f * w + 0.008f, 0.05f, 0.5f * (-0.5f * d + dz0)}, {0.016f, 0.1f, dz0 + 0.5f * d}));
        mesh::append(skirting, mesh::box({-0.5f * w + 0.008f, 0.05f, 0.5f * (dz1 + 0.5f * d)}, {0.016f, 0.1f, 0.5f * d - dz1}));
    ...
    if (scene_.catalog().contains("cable_tray") && !layout_.racks.empty()) {
        ...
        std::vector<glm::dvec3> route{{fridge.x, ceiling, fridge.z}, {first.x, ceiling, fridge.z}, {first.x, ceiling, first.z}};
        if (std::abs(last.x - first.x) > 1e-6) route.push_back({last.x + 0.25, ceiling, first.z});
        for (glm::dvec3 p : route) pts.push_back({p.x, p.y, p.z});
