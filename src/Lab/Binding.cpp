...
std::string Binding::fullPath() const {
    return std::format("{}.{}", bindingRootName(root), path);
}
...
std::optional<std::string> substitutePath(std::string_view path, const InstanceParams& p) {
    ...
    for (std::size_t i = 0; i < path.size();) {
        ...
        std::size_t j = i + 1;
        while (j < path.size() && placeholderChar(path[j]))
            ++j;
...
bool isSimulatorOnlyPath(std::string_view path) {
    for (std::string_view key : {".bloch", ".purity", ".statevector", ".entropy"}) {
        ...
        while (at != std::string_view::npos) {
            ...
            if (end == path.size() || !placeholderChar(path[end]))
                return true;
...
std::string formatBindingValue(const std::optional<BindingValue>& v,
    ...
    if (!v || !v->available())
        return "—";
    if (const std::string* s = v->asText())
        return *s;
