...
MeshData rackEnclosure(float W, float H, float D, int heightU) {
    ...
    for (float sx : {-1.0f, 1.0f})
        for (float sz : {-1.0f, 1.0f})
            mesh::appendColored(m, mesh::box({sx * (0.5f * W - 0.5f * post), 0.0f, sz * (0.5f * D - 0.5f * post)}, {post, H, post}), kPanel);
    for (float sx : {-1.0f, 1.0f}) // side panels with a shallow louvre band near the bottom
        mesh::appendColored(m, mesh::box({sx * (0.5f * W - 0.005f), 0.0f, 0.0f}, {0.01f, H - 2.0f * post, D - 2.0f * post}), kPanel);
    mesh::appendColored(m, mesh::box({0.0f, -0.5f * H + 0.5f * plinth, 0.0f}, {W, plinth, D}), kPanel);
    mesh::appendColored(m, mesh::box({0.0f, 0.5f * H - 0.5f * roof, 0.0f}, {W, roof, D}), kPanel);
    mesh::appendColored(m, mesh::box({0.0f, 0.0f, -0.5f * D + 0.005f}, {W - 2.0f * post, H - 2.0f * post, 0.01f}), kPanel);
    ...
    mesh::appendColored(m, mesh::box({0.0f, 0.5f * H + 0.004f, brushZ}, {0.32f, 0.008f, 0.08f}), kBrush);
    for (float side : {-1.0f, 1.0f})
        mesh::appendColored(m, mesh::box({0.0f, 0.5f * H + 0.010f, brushZ + side * 0.016f}, {0.30f, 0.004f, 0.028f}), kBristle);
    ...
    const float railY = 0.5f * (plinth - roof), zFront = 0.5f * D - 0.030f, zRear = -0.5f * D + 0.10f;
    for (float side : {-1.0f, 1.0f})
        for (float z : {zFront, zRear}) {
            mesh::appendColored(m, mesh::box({side * railX, railY, z - 0.002f}, {0.022f, railH, 0.004f}), kRail);
            mesh::appendColored(m, mesh::box({side * (railX + 0.011f * side), railY, z - 0.014f}, {0.004f, railH, 0.028f}), kRail); // L-flange
        }
    ...
    for (float side : {-1.0f, 1.0f})
        for (int u = 0; u < heightU; ++u) {
            ...
            if (yc - 0.02f < -0.5f * H + plinth) break;
            for (int k = 0; k < 3; ++k) {
                float y = yc + 0.015875f * (1.0f - static_cast<float>(k));
                mesh::appendColored(m, mesh::box({side * railX, y, zFront + 0.0002f}, {0.0095f, 0.0095f, 0.0006f}), kHole);
            }
        }
    // Vertical cable-tie bar behind the rear rails, where the looms are dressed.
    mesh::appendColored(m, mesh::box({0.5f * W - 0.07f, railY, -0.5f * D + 0.05f}, {0.02f, railH, 0.02f}), kRail);
    return m;
}
