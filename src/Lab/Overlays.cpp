...
std::vector<glm::dvec3> worldPath(const Scene& scene, ComponentId id) {
    ...
    for (const auto& a : n->spline->points)
        control.push_back(glm::dvec3(n->world * glm::dvec4(a.restLocal, 1.0)));
    return mesh::catmullRom(control, 6);
}
} // namespace

LabOverlays::LabOverlays(const Scene& scene, const BindingRegistry& bindings)
    : scene_(&scene), bindings_(&bindings) {
    legend_.ticks = {{0.01, "10 mK"}, {0.1, "100 mK"},  {1.0, "1 K"},
                     {10.0, "10 K"},  {100.0, "100 K"}, {300.0, "300 K"}};
}
...
void LabOverlays::update(double timeS) {
    ...
    if (onTemperature_) {
        ...
        for (const Node& n : scene_->nodes()) {
            for (const Binding& b : n.bindings) {
                if (b.root != BindingRoot::Cryo || !b.path.starts_with("stage.") ||
                    !b.path.ends_with(".T"))
                    continue;
                if (b.field != "temperature" && b.field != "nominal temperature")
                    continue;
                auto value = bindings_->resolve(b);
                if (!value || !value->available() || !value->isNumber())
                    break;
                StageTemperature st;
                st.node = n.id;
    ...
    if (onPulses_)
        for (const auto& run : scene_->wiringRuns()) {
            if (run.kind == cryo::LineKind::ReadoutOut || run.kind == cryo::LineKind::DC)
                continue;
            ...
            if (!t || !t->available() || !amp || !amp->available())
                continue;
            ...
            for (ComponentId seg : run.segments) {
                ...
                for (const auto& [node, dB] : run.attenuators)
                    if (const Node* an = scene_->node(node); an && an->world[3].y > p.position.y)
                        attenuation += dB;
                p.amplitude = amp->asNumber() * std::pow(10.0, -attenuation / 20.0);
                if (auto sigma = bindings_->query(BindingRoot::Run, base + ".sigma_s");
                    sigma && sigma->available())
    ...
    for (std::size_t q = 0; q < scene_->qubitNodes().size(); ++q) {
        ...
        if (onBloch_) {
            ...
            if (x && y && z && x->available() && y->available() && z->available()) {
                ...
                m.purity = purity && purity->available()
                               ? std::clamp(purity->asNumber(), 0.0, 1.0)
        ...
        if (pop && pop->available())
            visuals_.emissive[pad->id.value] =
                static_cast<float>(3.0 * std::clamp(pop->asNumber(), 0.0, 1.0));
    }
    ...
    if (onGlow_)
        for (std::size_t q = 0; q < scene_->resonatorNodes().size(); ++q) {
            ...
            g.intensity = static_cast<float>(std::clamp(
                std::log10(1.0 + g.photons) / std::log10(1.0 + kPhotonReference), 0.0, 1.0));
