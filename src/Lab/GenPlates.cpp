...
// origin (axis +Y) with a 1 mm chamfer on both rims, an optional real through-hole
// (`through_hole_m` [x, z, r]: the still pumping line), and at full detail the bolt circle of M6
// counterbores near the rim, radial rows of M4 tapped holes (shallow dimples), four loom
// pass-through slots and, on the RT plate, the KF-40 ports and the viewport ring. The simple level
// is a plain disc.
Result<MeshData> plate(const GenParams& p, const GenContext& c) {
    ...
    if (square) {
        if (!full)
            return gfx::shapes::box({2.0f * r, t, 2.0f * r});
        ...
        for (int side = 0; side < 4; ++side)
            for (int k = 0; k < 4; ++k) {
                ...
                glm::vec3 center = side % 2 == 0 ? glm::vec3(along, 0.5f * t - 0.5f * tt,
                                                             (side == 0 ? 1.0f : -1.0f) * 0.7f * r)
                                                 : glm::vec3((side == 1 ? 1.0f : -1.0f) * 0.7f * r,
                                                             0.5f * t - 0.5f * tt, along);
                glm::vec3 size = side % 2 == 0 ? glm::vec3(0.04f * r, tt, 0.55f * r)
                                               : glm::vec3(0.55f * r, tt, 0.04f * r);
    ...
    auto portAt = [&](int k) {
        float a = static_cast<float>(p.number("start_angle_deg", 0.0)) * glm::pi<float>() / 180.0f +
                  glm::radians(55.0f + 25.0f * static_cast<float>(k));
    ...
    if (!full) {
        ...
        for (int k = 0; k < ports; ++k)
            mesh::append(m, gfx::shapes::cylinder(c.F(0.0225), portH, 12, true),
                         mesh::translate(portAt(k) + glm::vec3(0, 0.5f * portH, 0)));
        return m;
    }
    const int seg = 96;
    ...
    if (hr > 0.0f)
        mesh::append(m, mesh::wall(hr, -0.5f * t, 0.5f * t, 32, false),
                     mesh::translate({hx, 0.0f, hz}));
    ...
    auto marker = [&](float x, float z, float radius, const glm::vec4& color, bool both) {
        mesh::appendColored(m, mesh::disc(radius, 0.5f * t + lift, 12, true), color,
                            mesh::translate({x, 0.0f, z}));
        if (both)
            mesh::appendColored(m, mesh::disc(radius, -0.5f * t - lift, 12, false), color,
                                mesh::translate({x, 0.0f, z}));
    };
    ...
    const float start =
        static_cast<float>(p.number("start_angle_deg", 0.0)) * glm::pi<float>() / 180.0f;
    if (holes ==
        "bolt_circle") { // M6 counterbores on the bolt circle: Ø 6.6 clearance in an Ø 11 recess
        ...
        for (int k = 0; k < count; ++k) {
            float a =
                start + glm::two_pi<float>() * static_cast<float>(k) / static_cast<float>(count);
            float x = ring * std::cos(a), z = ring * std::sin(a);
            if (inHole(x, z, c.F(0.006)))
                continue;
            mesh::appendColored(m, mesh::disc(c.F(0.0055), 0.5f * t + lift, 16, true), kRecess,
                                mesh::translate({x, 0.0f, z}));
            marker(x, z, c.F(0.0033), kHole, true);
        }
    } else if (holes == "feedthrough_ring") { // SMA bulkhead clearance holes (count/radius from
                                              // routing.json)
    ...
    for (int k = 0; k < ports; ++k) { // KF-40: tube, clamp-ring flange and the dark bore
        ...
        mesh::append(m, gfx::shapes::cylinder(c.F(0.0225), portH, 32, true),
                     mesh::translate(at + glm::vec3(0, 0.5f * portH, 0)));
        mesh::append(m, gfx::shapes::cylinder(c.F(0.0275), c.F(0.006), 32, true),
                     mesh::translate(at + glm::vec3(0, portH - c.F(0.003), 0)));
        mesh::appendColored(m, mesh::disc(c.F(0.020), portH + lift, 24, true), kHole,
                            mesh::translate(at));
    }
    if (p.boolean("viewport", false)) { // viewport ring with its glass window (RT plate), in the
        ...
        mesh::appendColored(m, mesh::disc(c.F(0.030), 0.5f * h, 32, true), kGlass,
                            mesh::translate(at));
    }
    return m;
}
...
Result<MeshData> plateWithHoles(const GenParams& p, const GenContext& c) {
    float w = c.F(p.length("w", 0.008)), d = c.F(p.length("d", 0.008)),
          t = c.F(p.length("t", kFilmGround_m));
    MeshData m;
    if (c.detail == Detail::Simple)
        return mesh::box({0, 0.5f * t, 0}, {w, t, d});
    struct Hole {
        float x0, x1, z0, z1;
    };
    ...
    for (const auto& h : p.points3("holes")) {
        ...
        Hole hole{std::max(-0.5f * w, x - r), std::min(0.5f * w, x + r), std::max(-0.5f * d, z - r),
                  std::min(0.5f * d, z + r)};
        if (hole.x1 <= hole.x0 || hole.z1 <= hole.z0)
            continue;
    ...
    for (std::size_t j = 0; j + 1 < zs.size(); ++j) {
        ...
        if (runStart != xs.size())
            quad(xs[runStart], xs.back(), zs[j], zs[j + 1]);
    }
    ...
    for (const auto& h : holes)
        mesh::appendColored(m,
                            mesh::box({0.5f * (h.x0 + h.x1), 0.05f * t, 0.5f * (h.z0 + h.z1)},
                                      {h.x1 - h.x0, 0.1f * t, h.z1 - h.z0}),
                            kGap);
    return m;
}
