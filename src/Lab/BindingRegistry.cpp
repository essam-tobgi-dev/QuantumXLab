...
std::optional<BindingValue> BindingRegistry::resolve(const InstanceParams& params,
                                                     const SpecRow& row) const {
    if (!row.binding)
        return std::nullopt;
    ...
    if (split->first == BindingRoot::Static)
        if (auto n = params.number(split->second))
            v = BindingValue::number(*n, row.unit, row.cls);
    if (!v)
        v = query(split->first, split->second);
    if (!v)
        return std::nullopt;
    if (v->unit.empty())
        v->unit = row.unit;
...
void StaticProvider::setNumber(std::string key, double v, std::string unit,
