...
bool GenParams::has(std::string_view key) const {
...
int GenParams::integer(std::string_view key, int fallback) const {
    ...
    if (it->is_array())
        return static_cast<int>(it->size()); // e.g. "ports": ["signal", "pump"]
    return fallback;
}
...
std::optional<double> GenParams::length(std::string_view stem) const {
    ...
    for (const auto& [suffix, factor] : kSuffix) {
        ...
        auto it = j_.find(key);
        if (it != j_.end() && it->is_number())
            return it->get<double>() * factor;
    }
    return std::nullopt;
}

double GenParams::length(std::string_view stem, double fallback) const {
    return length(stem).value_or(fallback);
}

namespace {
std::optional<glm::dvec3> toPoint(const core::Json& e, double f) {
    if (!e.is_array() || e.size() < 2 || !e[0].is_number() || !e[1].is_number())
        return std::nullopt;
...
const core::Json* findScaled(const core::Json& j, std::string_view stem, double& factor) {
    for (auto [suffix, f] :
         {std::pair<const char*, double>{"_m", 1.0}, {"_um", 1e-6}, {"_mm", 1e-3}}) {
...
constexpr std::array<Entry, 24> kGenerators{{
    {"Box", &gen::box},
    {"Plate", &gen::plate},
    {"PlateWithHoles", &gen::plateWithHoles},
    {"RackUnit", &gen::rackUnit},
    {"Cylinder", &gen::cylinder},
    {"CylinderSma", &gen::cylinderSma},
    {"SmaConnector", &gen::smaConnector},
    {"Tube", &gen::tube},
    {"Can", &gen::can},
    {"Torus", &gen::torus},
    {"Sphere", &gen::sphere},
    {"Frame", &gen::frame},
    {"Octagon", &gen::octagon},
    {"ExtrudedU", &gen::extrudedU},
    {"Spiral", &gen::spiral},
    {"Cpw", &gen::cpw},
    {"Meander", &gen::meander},
    {"Xmon", &gen::xmon},
    {"Junction", &gen::junction},
    {"SquidLoop", &gen::squidLoop},
    {"Airbridge", &gen::airbridge},
    {"WirebondArc", &gen::wirebondArc},
    {"TrapChip", &gen::trapChip},
    {"Post", &gen::post},
}};
constexpr auto kNames = [] {
    std::array<std::string_view, kGenerators.size()> n{};
    for (std::size_t i = 0; i < kGenerators.size(); ++i)
        n[i] = kGenerators[i].name;
    return n;
}();
} // namespace
...
Result<gfx::MeshData> generateMesh(std::string_view generator, const GenParams& params,
    ...
    for (const auto& e : kGenerators) {
        ...
        if (std::string why = mesh::validate(m); !why.empty())
            return fail(kErrGeometry,
                        std::format("generator '{}' produced an invalid mesh: {}", generator, why));
        return m;
    }
