...
Result<TourStep> parseStep(const core::Json& j, std::size_t index) {
    ...
    if (!j.is_object())
        return fail(kErrTour, where + " is not an object");
    TourStep s;
    const auto str = [&](const char* key, std::string& into, bool required) -> Status {
        const auto it = j.find(key);
        if (it == j.end())
            return required ? fail(kErrTour, std::format("{}.{} is missing", where, key))
                            : Status{};
        if (!it->is_string())
            return fail(kErrTour, std::format("{}.{} is not a string", where, key));
    ...
    if (const auto it = j.find("bookmark"); it != j.end() && it->is_string())
        s.bookmark = it->get<std::string>();
    if (s.focusInstance.empty() && s.focusDescriptor.empty() && !s.bookmark)
        return fail(kErrTour,
                    where + " names neither focus_instance, focus_descriptor nor bookmark");
    if (s.narration.size() < 80)
        return fail(kErrTour, where + ".narration is too short to explain anything");
    ...
    if (s.dwell_s <= 0.0 || s.marginFactor <= 0.0)
        return fail(kErrTour, where + ": dwell_s and margin_factor must be positive");
    if (const auto it = j.find("azimuth_deg"); it != j.end() && it->is_number())
        s.azimuthDeg = it->get<double>();
    if (const auto it = j.find("elevation_deg"); it != j.end() && it->is_number())
        s.elevationDeg = it->get<double>();
    if (const auto v = j.find("view"); v != j.end()) {
        if (!v->is_object())
            return fail(kErrTour, where + ".view is not an object");
        if (const auto it = v->find("xray"); it != v->end() && it->is_boolean())
            s.view.xray = it->get<bool>();
        if (const auto it = v->find("cutaway"); it != v->end() && it->is_boolean())
            s.view.cutaway = it->get<bool>();
        if (const auto it = v->find("cans"); it != v->end() && it->is_boolean())
            s.view.cans = it->get<bool>();
        ...
        if (const auto it = v->find("layers"); it != v->end()) {
            if (!it->is_object())
                return fail(kErrTour, where + ".view.layers is not an object");
            for (const auto& [name, on] : it->items()) {
                Group g;
                if (!groupFromName(name, g))
                    return fail(kErrTour,
                                std::format("{}.view.layers: unknown layer '{}'", where, name));
                if (!on.is_boolean())
                    return fail(kErrTour,
                                std::format("{}.view.layers.{} is not a boolean", where, name));
    ...
    if (const auto it = j.find("highlight"); it != j.end()) {
        if (!it->is_array())
            return fail(kErrTour, where + ".highlight is not an array");
        for (const core::Json& h : *it) {
            if (!h.is_string())
                return fail(kErrTour, where + ".highlight entries must be strings");
...
ComponentId resolve(const Scene& scene, std::string_view instance, std::string_view descriptor) {
    if (!instance.empty())
        if (ComponentId id = scene.findByInstance(instance); id.value != 0)
            return id;
    if (!descriptor.empty()) {
...
Result<Tour> Tour::load(const std::filesystem::path& json, const Scene& scene, Interaction& ui,
    ...
    if (!d.is_object())
        return fail(kErrTour, json.string() + ": `data` is not an object");
    Tour t;
    t.scene_ = &scene;
    t.ui_ = &ui;
    ...
    if (steps == d.end() || !steps->is_array() || steps->empty())
        return fail(kErrTour, json.string() + ": `steps` is missing or empty");
    ...
    const auto anchors =
        corpus.empty() ? std::map<std::string, std::set<std::string>>{} : theoryAnchorsIn(corpus);
    if (!theoryDir.empty() && anchors.empty())
        return fail(kErrTour, "no theory documents under " + theoryDir.string());
    std::vector<std::string> problems;
    std::set<std::string> ids;
    for (std::size_t i = 0; i < t.steps_.size(); ++i) {
        const TourStep& s = t.steps_[i];
        if (!ids.insert(s.id).second)
            problems.push_back(std::format("step '{}' is listed twice", s.id));
        ...
        if (hash == std::string::npos || hash != 3 || s.theory[0] != 'T')
            problems.push_back(std::format(
                "step '{}': theory anchor '{}' is not of the form Tnn#anchor", s.id, s.theory));
            ...
            if (doc == anchors.end() || !doc->second.contains(s.theory.substr(hash + 1)))
                problems.push_back(
                    std::format("step '{}': unresolved theory anchor '{}'", s.id, s.theory));
        }
    ...
    if (!t.steps_.empty() &&
        std::all_of(t.unresolved_.begin(), t.unresolved_.end(), [](bool u) { return u; }))
...
bool Tour::stepSkipped(std::size_t step) const {
    if (step < unresolved_.size() && unresolved_[step])
        return true;
...
double Tour::stepProgress() const {
    ...
    switch (phase_) {
    case Phase::Flying:
        return std::clamp(t_ / total, 0.0, 1.0);
    case Phase::Dwelling:
        return std::clamp((kFlight_s + t_) / total, 0.0, 1.0);
    case Phase::Paused:
        return std::clamp(pausedProgress_, 0.0, 1.0);
    case Phase::Idle:
        break;
    }
...
void Tour::update(double dt, gfx::Camera& camera) {
    ...
    if (phase_ == Phase::Flying) {
        ...
        t_ += dt;
        // The orbit takes pixel deltas at 0.005 rad/px (Camera::orbit); a slow drift around the
        // focus.
        const double radians = glm::radians(s.orbitDegPerS) * dt;
        if (radians != 0.0)
            camera.orbit(radians / 0.005, 0.0);
        if (t_ >= s.dwell_s) {
            next();
            if (!playing())
                return;
...
bool Tour::cameraMovedExternally(const gfx::Camera& camera) const {
    ...
    return glm::length(camera.position() - lastCamera_.position) > scale ||
           ...
           std::abs(camera.fovDeg() - lastCamera_.fovDeg) > 1e-9 ||
           camera.ortho() != lastCamera_.ortho;
}
...
void Tour::arrive(gfx::Camera& camera) {
    ...
    t_ = 0.0;
    if (id.value != 0)
        ui_->select(id); // the Inspector follows the tour (spec 17 §7.2)
    ...
    const auto frameFocus = [&]() -> gfx::Bookmark {
        ...
        if (box.valid()) {
            if (s.azimuthDeg || s.elevationDeg) {
                const double a = glm::radians(s.azimuthDeg.value_or(35.0)),
                             e = glm::radians(s.elevationDeg.value_or(20.0));
                const glm::dvec3 dir{std::cos(e) * std::sin(a), std::sin(e),
                                     std::cos(e) * std::cos(a)};
                ...
                framed.lookAt(box.center() +
                                  glm::dvec3(0.6, 0.35, 0.7) * std::max(box.radius(), 1e-6),
                              box.center());
            }
    ...
    if (s.bookmark) {
        ...
        (void)ui_->applyBookmark(*s.bookmark, camera,
                                 kFlight_s); // chip bookmarks switch the layers
...
TourStep::View Tour::foldedView(std::size_t upTo) const {
    ...
    for (std::size_t k = 0; k <= upTo && k < steps_.size(); ++k) {
        ...
        if (v.xray)
            out.xray = v.xray;
        if (v.cutaway)
            out.cutaway = v.cutaway;
        if (v.cans)
            out.cans = v.cans;
        if (v.explode)
            out.explode = v.explode;
        for (const auto& [group, on] : v.layers) {
            ...
            if (!found)
                out.layers.emplace_back(group, on);
        }
    }
    return out;
}
...
void Tour::applyView(const TourStep::View& v, const gfx::Bookmark& target) {
    if (v.xray)
        ui_->setXray(*v.xray);
    if (v.cans)
        ui_->setCansVisible(*v.cans);
    if (v.explode)
        ui_->setExplode(Assembly::FridgeStages, *v.explode);
    if (v.cutaway) {
        // The kept half-space is dot(n, p) + w >= 0 about the fridge axis
        // (Interaction::cutawayPlane): point n away from the camera so the half facing the viewer
        // is the one cut away.
        glm::dvec3 toCamera = target.position - scene_->layout().fridgePosition_m;
        toCamera.y = 0.0;
        const double angle = glm::length(toCamera) > 1e-9
                                 ? glm::degrees(std::atan2(-toCamera.z, -toCamera.x))
                                 : ui_->cutawayAngleDeg();
        ui_->setCutaway(*v.cutaway, *v.cutaway ? angle : ui_->cutawayAngleDeg());
    }
    for (const auto& [group, on] : v.layers)
        ui_->setLayerVisible(group, on);
}

// ---------------------------------------------------------------- what the card shows

ComponentId Tour::focusId(std::size_t step) const {
    return step < focus_.size() ? focus_[step] : ComponentId{0};
}

std::string Tour::focusName() const {
    if (steps_.empty())
        return {};
    if (const Node* n = scene_->node(focusId()))
        return n->displayName.empty() ? n->instanceName : n->displayName;
