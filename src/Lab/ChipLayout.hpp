...
struct ChipLayoutOptions {
    ...
    double edgeMargin_um = 800.0;      // outermost footprint → chip edge (bond pads, Purcell filters, routing)
...
struct CouplerSite {
    ...
    std::vector<CpwRoute> stubs; // fixed: one pad-to-pad segment; tunable: coupler pad to each qubit
};
...
Result<ChipLayout> layoutChip(const hw::Device& dev, const hw::Calibration* cal, const ChipLayoutOptions& opt = {});
