...
Status SceneBuilder::buildPulseTubeStages() {
    if (!scene_.catalog().contains("pt_regenerator") || ptHead_.value == 0) return {};
    ...
    for (const auto& st : kTubes) {
        if (!hasStage(st.from) || !hasStage(st.to)) continue;
        ...
        const double parentY = underHead ? stageHeight(Stage::RT) + 0.5 * plateThickness(Stage::RT) + 0.5 * headH : stageHeight(st.from);
        NodeSpec tube = part("pt_regenerator", std::format("pt_regenerator[{}]", index), "stainless",
                             Transform::at(underHead ? 0.0 : pt.x, bottom + 0.5 * L - parentY, underHead ? 0.0 : pt.z));
        ...
        if (scene_.catalog().contains("thermal_braid")) { // three braid bundles from the flange to the plate
            ...
            for (int k = 0; k < 3; ++k) {
                ...
                std::vector<glm::dvec3> path{pt + dir * st.radius + glm::dvec3(0.0, y0 + 0.014, 0.0),
                                             pt + dir * (st.radius + 0.025) + glm::dvec3(0.0, y0 + 0.022, 0.0),
                                             pt + dir * (st.radius + 0.055) + glm::dvec3(0.0, y0 + 0.003, 0.0)};
                NodeSpec braid = part("thermal_braid", std::format("thermal_braid.{}[{}]", stageLayoutName(st.to), k), "copper", {});
...
Status SceneBuilder::buildCirculation() {
    if (!hasStage(Stage::STILL)) return {};
    ...
    const double stillTop = plateTop(Stage::STILL) + kStillBodyHeight_m; // world y of the still's lid
    ComponentId stillId{0};
    if (scene_.catalog().contains("still")) {
        NodeSpec body = part("still", "still", "copper", Transform::at(still.x, 0.5 * tStill + 0.5 * kStillBodyHeight_m, still.z));
        ...
        if (scene_.catalog().contains("still_pumping_line") && hasStage(Stage::RT)) {
            ...
            line.overrides = {{"points_m", pointList({{0.0, 0.5 * kStillBodyHeight_m, 0.0}, {0.0, 0.5 * kStillBodyHeight_m + top, 0.0}})},
                              {"r_m", kPumpLineRadius_m}};
            line.params.setNumber("length_m", top);
            QXL_TRY(addComponent(stillId, std::move(line)));
        }
        NodeSpec heater = part("still_heater", "still_heater.still", "plastic_grey", Transform::at(kStillBodyRadius_m + 0.005, -0.008, 0.0));
    ...
    if (hasStage(Stage::PT2)) { // condensing line 4 K → still with its flow impedance
        ...
        const double dyTop = stillTop - 0.012 - stageHeight(Stage::PT2); // enters the still's upper wall
        const glm::dvec3 entry = still + polar(kStillAzimuth_deg + 90.0, kStillBodyRadius_m);
        std::vector<glm::dvec3> path{{a.x, y0, a.z}, {a.x, y0 + 0.45 * (dyTop - y0), a.z},
                                     {entry.x + 0.02 * (entry.x - still.x) / kStillBodyRadius_m, dyTop, entry.z + 0.02 * (entry.z - still.z) / kStillBodyRadius_m},
        ...
        NodeSpec imp = part("flow_impedance", "flow_impedance", "cuni", Transform::at(a.x, y0 + 0.45 * (dyTop - y0), a.z));
    ...
    if (hasStage(Stage::CP)) { // continuous heat exchanger coil under the still, on its return tube
        ...
        NodeSpec hx = part("hx_continuous", "hx_continuous", "cuni",
    ...
    if (hasStage(Stage::MXC) && scene_.catalog().contains("mixing_chamber")) {
        ...
        NodeSpec mc = part("mixing_chamber", "mixing_chamber", "copper", Transform::at(0.0, 0.5 * tMxc + 0.5 * kMcHeight_m, 0.0));
        ...
        NodeSpec heater = part("mxc_heater", "mxc_heater.mxc", "plastic_grey", Transform::at(kMcRadius_m + 0.005, -0.015, 0.0));
        heater.display = "Mixing-chamber heater";
        heater.params.setToken("stage", "mxc");
        QXL_TRY(addComponent(mcId, std::move(heater)));
    }
    if (hasStage(Stage::CP) && hasStage(Stage::MXC)) { // step exchangers stacked under the cold plate
        ...
        const double h = std::min(d->geometryNumber("h_m", 0.03), (gap - 0.02) / count - 0.004), spacing = 0.004;
        double y = -0.5 * plateThickness(Stage::CP) - 0.006;
        for (int i = 0; i < count; ++i) {
            NodeSpec step = part("hx_step", std::format("hx_step[{}]", i), "stainless", Transform::at(still.x, y - 0.5 * h, still.z));
        ...
        if (mcId.value != 0) { // concentrated-side capillary from the last exchanger into the mixing chamber
            const double mcTop = plateTop(Stage::MXC) + kMcHeight_m - stageHeight(Stage::CP);
            std::vector<glm::dvec3> path{{still.x, y + spacing, still.z}, {0.6 * still.x, 0.5 * (y + mcTop), 0.6 * still.z}, {0.012, mcTop, 0.012}};
...
Status SceneBuilder::buildThermometry() {
    ...
    const Aux kAux[]{{Stage::MXC, "thermometer_ruo2", 0}, {Stage::CP, "thermometer_ruo2", 1}, {Stage::STILL, "thermometer_ruo2", 2},
                     {Stage::PT2, "thermometer_cernox", 3}, {Stage::PT1, "thermometer_cernox", 4}};
    for (const auto& a : kAux) {
        ComponentId parent = stageNode(a.stage);
        if (parent.value == 0) continue;
        NodeSpec spec = part(a.id, std::format("{}.{}", a.id, stageLayoutName(a.stage)), "plastic_grey",
                             Transform::at(polar(75.0, 0.55 * stageRadius(a.stage), 0.5 * plateThickness(a.stage) + 0.0025)));
        spec.display = std::format("{} ({})", scene_.catalog().find(a.id)->name, cryo::stageName(a.stage));
...
Status SceneBuilder::buildSampleStage() {
    ...
    if (layout_.magShield) { // Cryoperm can with its mounting flange, superconducting Al can inside
        NodeSpec mu = part("mag_shield_mumetal", "mag_shield_mumetal", "mu_metal", Transform::at(0.0, -0.5 * t, 0.0));
        mu.overrides = {{"flange", true}, {"flange_t_m", 0.004}, {"flange_w_m", 0.008}, {"flange_bolts", 12}};
        mu.params.setToken("stage", "mxc");
        QXL_TRY_ASSIGN(ComponentId muId, addComponent(mxc, std::move(mu)));
        NodeSpec al = part("mag_shield_al", "mag_shield_al", "aluminium", Transform::at(0.0, -0.004, 0.0));
    ...
    NodeSpec finger = part("cold_finger", "cold_finger", "gold_plated_cu", Transform::at(0.0, -0.5 * t - 0.5 * fingerH, 0.0));
    ...
    NodeSpec puck = part("sample_puck", "sample_puck", "gold_plated_cu", Transform::at(0.0, -0.5 * fingerH - 0.5 * puckH, 0.0));
