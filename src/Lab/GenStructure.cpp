...
Result<MeshData> frame(const GenParams& p, const GenContext& c) {
    ...
    if (p.length("span")) { // gantry: two rails along z, cross beam, trolley, hoist cable and hook
        ...
        if (c.detail == Detail::Simple)
            return mesh::box({0, 0, 0}, {span, H, travel});
        ...
        mesh::appendColored(
            m, mesh::box({0, yTop - 2.0f * rail, 0}, {3.0f * rail, rail, 2.0f * rail}), kKnob);
    ...
    float w = c.F(p.length("width", 1.4)), d = c.F(p.length("depth", 1.4)),
          h = c.F(p.length("height", 2.6));
    if (c.detail == Detail::Simple) {
    ...
    auto member = [&](glm::vec3 a, glm::vec3 b, float size) {
        if (tslot && std::abs(size - s) < 1e-6f)
            mesh::append(m, mesh::extrudeBetween(prof, a, b));
        else
            mesh::append(m, bar(a, b, size));
    };
    ...
    for (float x : {-hx, hx})
        for (float z : {-hz, hz}) {
            ...
            mesh::appendColored(
                m, mesh::box({x, -0.5f * h + 0.5f * feet, z}, {1.4f * s, feet, 1.4f * s}), kRubber);
        }
    for (float y : {0.5f * h - 0.5f * s, 0.05f * h,
                    -0.5f * h + feet + 0.5f * s}) { // top ring, mid brace, base
        ...
        for (float x : {-hx, hx}) // corner brackets: gusset plates on the inner faces of the joints
            for (float z : {-hz, hz})
                mesh::appendColored(
                    m,
                    mesh::box({x - std::copysign(0.55f * s, x), y, z - std::copysign(0.55f * s, z)},
                              {1.1f * s, 0.9f * s, 1.1f * s}),
                    kBracket);
    }
    // Top-plate support: two cross beams under the plate's rim with a rubber isolation block on
    // each end, so the insert hangs on the dampers and not on the frame's resonances (spec 17
    // §3.1).
    ...
    for (float z : {-0.5f * span, 0.5f * span}) {
        ...
        for (float x : {-0.55f * span, 0.55f * span})
            mesh::appendColored(
                m, mesh::box({x, ySupport - 0.5f * block, z}, {1.6f * s, block, 1.6f * s}),
                kRubber);
    }
    return m;
}
...
Result<MeshData> extrudedU(const GenParams& p, const GenContext& c) {
    ...
    if (pts.size() < 2)
        pts = {glm::dvec3(-0.5, 0, 0), glm::dvec3(0.5, 0, 0)};
    MeshData m;
    for (std::size_t i = 0; i + 1 < pts.size(); ++i) {
        ...
        if (c.detail == Detail::Simple) { // box impostor of the whole channel profile
            mesh::append(m, mesh::box(center, alongX ? glm::vec3(L, H, W)
                                                     : (vertical ? glm::vec3(W, len, H)
                                                                 : glm::vec3(W, H, L))));
            continue;
        }
        glm::vec3 floorSize =
            alongX ? glm::vec3(L, t, W) : (vertical ? glm::vec3(W, len, t) : glm::vec3(W, t, L));
        glm::vec3 floorAt = center + (vertical ? glm::vec3(0, 0, -0.5f * H + 0.5f * t)
                                               : glm::vec3(0, -0.5f * H + 0.5f * t, 0));
        mesh::append(m, mesh::box(floorAt, floorSize));
        // The cable bundle the tray carries: `cables` runs laid in the channel (spec 17 §2
        // amended).
        ...
        for (int k = 0; k < cables && !vertical; ++k) {
            ...
            mesh::appendColored(
                m,
                gfx::shapes::tube({center + off - 0.5f * run, center + off + 0.5f * run}, rc, 8,
                                  false),
                kCable);
        }
        for (float side : {-1.0f, 1.0f}) {
            glm::vec3 off = alongX ? glm::vec3(0, 0, side * (0.5f * W - 0.5f * t))
                                   : (vertical ? glm::vec3(side * (0.5f * W - 0.5f * t), 0, 0)
                                               : glm::vec3(side * (0.5f * W - 0.5f * t), 0, 0));
            glm::vec3 size = alongX ? glm::vec3(L, H, t)
                                    : (vertical ? glm::vec3(t, len, H) : glm::vec3(t, H, L));
