...
void springEmbed(ChipLayout& L, const hw::Device& dev, const ChipLayoutOptions& o) {
    ...
    for (auto& q : p)
        q = {rng.uniform(0.0, side), rng.uniform(0.0, side)};
    double t0 = side / 4.0;
    for (int it = 0; it < o.springIterations; ++it) {
        ...
        for (std::size_t v = 0; v < n; ++v) {
            ...
            if (len > 0.0)
                p[v] += disp[v] / len * std::min(len, temp);
        }
    }
    for (std::size_t v = 0; v < n; ++v)
...
Status placeSites(ChipLayout& L, const hw::Device& dev, const hw::Calibration* cal,
    ...
    for (const auto& q : dev.qubits) {
        ...
        double alpha = -300e6;
        if (cal)
            if (const auto* qc = cal->qubit(q.index); qc && qc->anharmonicity.value.v != 0.0)
                alpha = qc->anharmonicity.value.v;
        s.armLength_um = xmonArmLength_um(alpha, o.armWidth_um, o.padGap_um, o.epsEff);
        if (!s.coupler)
            armSum += s.armLength_um, ++armCount;
        s.pos_um = glm::dvec2(q.pos[0], q.pos[1]) * pitch;
    }
    for (auto& s : L.qubits)
        if (s.coupler) { // coupler_tunable: a transmon-like element at `scale` of the mean qubit
                         // pad
    ...
    switch (L.topology) {
    case ChipTopology::HeavyHex: { // site (r, c) → x = a (c + ½ (r mod 2)), y = a (√3/2) r with a =
                                   // 2 pitch
        for (const auto& q : dev.qubits) {
            ...
            if (y % 2 != 0)
                continue;
            long r = y / 2;
            L.qubits[q.index].pos_um = {pitch *
                                            (static_cast<double>(x) + static_cast<double>(r % 2)),
                                        pitch * std::sqrt(3.0) * r};
        }
        for (const auto& q :
             dev.qubits) { // row-edge (bridge) qubits at the midpoint of their neighbours
    ...
    case ChipTopology::Grid:
        break; // x = c·pitch, y = r·pitch; couplers keep their midpoint positions
    ...
    case ChipTopology::SpringEmbedded:
        springEmbed(L, dev, o);
        break;
    }
    if (L.topology == ChipTopology::SpringEmbedded)
        for (std::size_t i = 0; i < L.qubits.size(); ++i)
            for (std::size_t j = i + 1; j < L.qubits.size(); ++j)
                if (glm::length(L.qubits[i].pos_um - L.qubits[j].pos_um) <
                    L.qubits[i].radius_um() + L.qubits[j].radius_um() + o.clearance_um)
                    return fail(
                        kErrChip,
                        std::format("spring-embedded layout of '{}': pads of q{} and q{} overlap",
                                    dev.id, i, j));
    return {};
}
...
void placeResonators(ChipLayout& L, const hw::Device& dev, const ChipLayoutOptions& o) {
    ...
    for (const auto& site : L.qubits) {
        ...
        ResonatorSite r;
        r.qubit = q;
        r.frequency_Hz = q < dev.readout.resonatorFrequencies.size()
        ...
        for (const auto& other : L.qubits)
            if (other.index != q && glm::length(other.pos_um - site.pos_um) < 1.75 * o.pitch_um)
                angles.push_back(
                    std::atan2(other.pos_um.y - site.pos_um.y, other.pos_um.x - site.pos_um.x));
        ...
        if (angles.empty())
            gaps.push_back({-0.5 * pi, 2.0 * pi});
        for (std::size_t i = 0; i < angles.size(); ++i) {
            double a0 = angles[i],
                   a1 = i + 1 < angles.size() ? angles[i + 1] : angles[0] + 2.0 * pi;
            gaps.push_back({wrap(0.5 * (a0 + a1)), a1 - a0});
        }
        std::stable_sort(gaps.begin(), gaps.end(), [&](const Cand& a, const Cand& b) {
            if (std::abs(a.gap - b.gap) > 1e-6)
                return a.gap > b.gap;
            return std::abs(wrap(a.angle - 0.25 * pi)) < std::abs(wrap(b.angle - 0.25 * pi));
        });
        std::vector<double> candidates;
        for (const auto& g : gaps)
            for (double d : {0.0, 15.0, -15.0, 30.0, -30.0})
                candidates.push_back(g.angle + d * pi / 180.0);
        for (int k = 0; k < 8; ++k)
            candidates.push_back(0.25 * pi * k);
        ...
        for (double factor : {1.0, 0.7, 1.4, 2.0, 0.5}) {
            ...
            s.local =
                meanderCenterline(r.length_um, o.meanderPitch_um, s.request, o.meanderLead_um);
            if (s.local.size() < 4)
                continue;
            s.lenX = s.local.back().x;
            for (const auto& p : s.local)
                s.amp = std::max(s.amp, std::abs(p.y));
            shapes.push_back(std::move(s));
        }
        if (shapes.empty()) {
            L.diagnostics.push_back(std::format(
                "resonator of q{}: length {:.0f} µm is too short to meander", q, r.length_um));
            continue;
        }
        ...
        auto conflicts = [&](const ChipRect& f) {
            ...
            for (const auto& other : L.qubits)
                n += other.index != q &&
                             f.overlapsCircle(other.pos_um, other.radius_um() + o.clearance_um)
                         ? 1
                         : 0;
            for (const auto& placed : L.resonators)
                n += f.overlaps(placed.footprint, o.clearance_um) ? 1 : 0;
            return n;
        };
        ...
        for (const auto& sh : shapes) {
            for (double a : candidates) {
                ...
                if (c < bestConflicts)
                    best = a, bestShape = &sh, bestConflicts = c;
                if (c == 0)
                    break;
            }
            if (bestConflicts == 0)
        ...
        if (bestConflicts > 0)
            L.diagnostics.push_back(std::format("resonator of q{}: no conflict-free side", q));
        r.angle_rad = best;
        ...
        for (const auto& p : bestShape->local)
            r.centerline_um.push_back(r.origin_um + dir * p.x + perp * p.y);
    ...
    for (std::size_t i = 0; i < dev.edges.size(); ++i) {
        ...
        if (e.coupler && *e.coupler < L.qubits.size()) {
            ...
            for (std::uint32_t end : {e.a, e.b})
                c.stubs.push_back(CpwRoute{
                    {padEdgeToward(*e.coupler, L.qubits[end].pos_um), padEdgeToward(end, cp)}});
        } else {
            c.stubs.push_back(CpwRoute{{padEdgeToward(e.a, L.qubits[e.b].pos_um),
                                        padEdgeToward(e.b, L.qubits[e.a].pos_um)}});
        }
...
std::vector<std::vector<std::uint32_t>> feedlineGroups(const hw::Device& dev, const ChipLayout& L,
    ...
    for (const auto& f : dev.readout.feedlines)
        groups.push_back(f.qubits);
    if (!groups.empty())
        return groups;
    ...
    for (std::size_t i = 0; i < data.size(); i += per)
        groups.emplace_back(data.begin() + static_cast<std::ptrdiff_t>(i),
                            data.begin() +
                                static_cast<std::ptrdiff_t>(std::min(data.size(), i + per)));
    return groups;
}
