...
ComponentId Scene::add(Node n, ComponentId parent) {
    ...
    if (!n.descriptorId.empty() && n.kind == NodeKind::Component) {
        ...
        auto it = std::find_if(all.begin(), all.end(), [&](const ComponentDescriptor& d) {
            return d.id == n.descriptorId;
        });
    ...
    if (parent.value != 0)
        nodes_[parent.value - 1].children.push_back(id);
    return id;
}
...
Status Scene::finalize(double buildMs) {
    ...
    while (!stack.empty()) {
        ...
        for (auto it = ch.rbegin(); it != ch.rend(); ++it)
            stack.push_back(it->value - 1);
    }
    if (order.size() != n)
        return fail(kErrScene,
                    std::format("scene graph has {} unreachable node(s)", n - order.size()));
    std::vector<std::uint32_t> newIndex(n);
    for (std::size_t k = 0; k < n; ++k)
        newIndex[order[k]] = static_cast<std::uint32_t>(k);
    auto remap = [&](ComponentId& id) {
        if (id.value != 0)
            id = ComponentId{newIndex[id.value - 1] + 1};
    };
    std::vector<Node> sorted;
    sorted.reserve(n);
    for (std::uint32_t old : order)
        sorted.push_back(std::move(nodes_[old]));
    ...
    for (auto& id : qubitNodes_)
        remap(id);
    for (auto& id : resonatorNodes_)
        remap(id);
    for (auto& id : stageNodes_)
        remap(id);
    for (auto& run : runs_) {
        for (auto& id : run.segments)
            remap(id);
        for (auto& a : run.attenuators)
            remap(a.first);
        for (auto& id : run.stageAnchor)
            remap(id);
    }
    remap(chipRoot_);
    ...
    for (std::size_t i = n; i-- > 0;) {
        ...
        if (node.parent.value != 0)
            count[node.parent.value - 1] += count[i];
        nodes_[i].subtreeEnd = static_cast<std::uint32_t>(i + count[i]);
        if (node.parent.value != 0 && node.parent.value - 1 >= i)
            return fail(kErrScene, "parent after child in depth-first order");
    }
    updateTransforms();
...
Node* Scene::node(ComponentId id) {
    return id.value >= 1 && id.value <= nodes_.size() ? &nodes_[id.value - 1] : nullptr;
}

const ComponentDescriptor* Scene::descriptor(const Node& n) const {
    if (n.descriptorIndex < 0 ||
        static_cast<std::size_t>(n.descriptorIndex) >= catalog_.all().size())
        return nullptr;
...
std::vector<ComponentId> Scene::findByDescriptor(std::string_view descriptorId) const {
    ...
    for (const auto& n : nodes_)
        if (n.kind == NodeKind::Component && n.descriptorId == descriptorId)
            out.push_back(n.id);
    return out;
}
...
ComponentId Scene::findByInstance(std::string_view instanceName) const {
    for (const auto& n : nodes_)
        if (n.instanceName == instanceName)
            return n.id;
...
std::vector<ComponentId> Scene::breadcrumb(ComponentId id) const {
    std::vector<ComponentId> out;
    for (const Node* n = node(id); n; n = node(n->parent))
        out.push_back(n->id);
...
std::string Scene::breadcrumbText(ComponentId id, std::string_view separator) const {
    ...
    for (ComponentId c : breadcrumb(id)) {
        if (c == root())
            continue; // the laboratory root is implicit (spec 17 §7.2 example)
        if (!out.empty())
            out += separator;
...
void Scene::updateTransforms() {
    for (auto& n : nodes_) {
        glm::dmat4 parentWorld =
            n.parent.value != 0 ? nodes_[n.parent.value - 1].world : glm::dmat4(1.0);
        glm::dmat4 m = n.local.matrix();
        if (n.explodeOffset != glm::dvec3(0.0))
            m = glm::translate(glm::dmat4(1.0), n.explodeOffset) * m;
    ...
    for (std::size_t i = nodes_.size(); i-- > 0;) {
        ...
        for (ComponentId c : n.children)
            mergeInto(n.subtreeBounds, nodes_[c.value - 1].subtreeBounds);
    }
}
