...
std::vector<glm::dvec2> meanderCenterline(double L, double pitch, double amp, double lead) {
    ...
    // the remainder widens the excursions instead of lengthening a lead, so the footprint stays
    // compact
...
std::vector<glm::vec2> toXZ(const std::vector<glm::dvec2>& chip, double scale) {
    ...
    for (const auto& q : chip)
        out.push_back(chipToXZ(q, scale));
    return out;
}
...
MeshData cpwStrips(const std::vector<glm::vec2>& P, float w, float s, const GenContext& c,
                   bool gaps) {
    MeshData m;
    ...
    for (float side : {-1.0f, 1.0f}) {
        ...
        mesh::appendColored(m, mesh::ribbon(off, s, c.F(kFilmGround_m), c.F(kFilmGapTop_m)),
                            kGapColor);
    }
    return m;
}
...
Result<MeshData> cpw(const GenParams& p, const GenContext& c) {
    ...
    if (path.size() < 2)
        path = {glm::dvec2(0.0), glm::dvec2(1e-3, 0.0)};
    std::vector<glm::vec2> P = mesh::dedupe2(toXZ(path, c.unitScale), 0.0f);
    if (P.size() < 2)
        return fail(kErrGeometry, "Cpw: path collapses to a point");
...
Result<MeshData> meander(const GenParams& p, const GenContext& c) {
    ...
    double pitch = p.length("pitch", 50e-6), amp = p.length("amplitude", 150e-6),
           lead = p.length("lead", 50e-6);
    std::vector<glm::dvec2> centre = meanderCenterline(L, pitch, amp, lead);
    if (centre.size() < 2)
        return fail(kErrGeometry, "Meander: length shorter than one turn plus leads");
...
Result<MeshData> xmon(const GenParams& p, const GenContext& c) {
    ...
    float L = c.F(p.length("arm_length", 150e-6) * scale),
          a = 0.5f * c.F(p.length("arm_width", 24e-6) * scale);
    ...
    if (c.detail == Detail::Full)
        mesh::appendColored(
            m, mesh::prism(cross(L + g, a + g), c.F(kFilmGround_m), c.F(kFilmGapTop_m)), kGapColor);
    return m;
}
