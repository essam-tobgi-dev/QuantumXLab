...
Result<MeshData> junction(const GenParams& p, const GenContext& c) {
    float w = c.F(p.length("w_lead", 1e-6)), ov = c.F(p.length("overlap", 0.15e-6)), t = c.F(kFilmJunction_m);
    float x0 = -10.0f * w, x1 = 0.5f * w + ov, span = x1 - x0;
    if (c.detail == Detail::Simple) return mesh::box({0.5f * (x0 + x1), t, 0.5f * (x0 + x1)}, {span, 2.0f * t, span});
    MeshData m;
    ...
    mesh::appendColored(m, mesh::box({0.25f * ov, t, 0.25f * ov}, {w + 0.5f * ov, 0.2f * t, w + 0.5f * ov}), kOxide);
    return m;
}
...
Result<MeshData> squidLoop(const GenParams& p, const GenContext& c) {
    ...
    float side = c.F(std::sqrt(std::max(area, 1e-14))), w = c.F(p.length("w", 1e-6)), t = c.F(kFilmJunction_m);
    float outer = side + w;
    if (c.detail == Detail::Simple) return mesh::box({0, 0.5f * t, 0}, {outer, t, outer});
    MeshData m;
    float h = 0.5f * side;
    ...
    mesh::appendColored(m, mesh::box({-h, 0.5f * t, 0}, {w, t, side - w}), kLead);
    for (float x : {-h, h}) mesh::appendColored(m, mesh::box({x, 0.6f * t, 0}, {0.9f * w, 0.8f * t, 1.5f * w}), kOxide);
    return m;
}
...
Result<MeshData> airbridge(const GenParams& p, const GenContext& c) {
    float span = c.F(p.length("span", 30e-6)), w = c.F(p.length("w", 10e-6)), h = c.F(p.length("h", 3e-6));
    float tb = c.F(0.3e-6);
    if (c.detail == Detail::Simple) return mesh::box({0, 0.5f * (h + tb), 0}, {span, h + tb, w});
    float foot = 0.15f * span, xa = 0.5f * span - foot;
    const int n = 12;
    std::vector<float> xs{-0.5f * span};
    for (int i = 0; i <= n; ++i) xs.push_back(-xa + 2.0f * xa * static_cast<float>(i) / n);
    ...
    MeshData m;
    auto quad = [&](glm::vec3 a, glm::vec3 b, glm::vec3 cc, glm::vec3 d, glm::vec3 na, glm::vec3 nb) {
    ...
    for (std::size_t i = 0; i + 1 < xs.size(); ++i) {
        ...
        glm::vec3 nA = glm::normalize(glm::vec3(-slope(xA), 1.0f, 0.0f)), nB = glm::normalize(glm::vec3(-slope(xB), 1.0f, 0.0f));
        float bA = yb(xA), bB = yb(xB);
        quad({xA, bA + tb, -hw}, {xB, bB + tb, -hw}, {xB, bB + tb, hw}, {xA, bA + tb, hw}, nA, nB); // top
        quad({xA, bA, hw}, {xB, bB, hw}, {xB, bB, -hw}, {xA, bA, -hw}, -nA, -nB);                   // underside
        quad({xA, bA, hw}, {xA, bA + tb, hw}, {xB, bB + tb, hw}, {xB, bB, hw}, {0, 0, 1}, {0, 0, 1});
        quad({xA, bA, -hw}, {xB, bB, -hw}, {xB, bB + tb, -hw}, {xA, bA + tb, -hw}, {0, 0, -1}, {0, 0, -1});
    }
...
Result<MeshData> trapChip(const GenParams& p, const GenContext& c) {
    ...
    if (c.detail == Detail::Simple) return mesh::box({0, 0, 0}, size);
    MeshData m;
    float film = 0.1f * size.y;
    mesh::appendColored(m, mesh::box({0, -0.5f * film, 0}, {size.x, size.y - film, size.z}), kTrapBody);
    float yTop = 0.5f * size.y - 0.5f * film;
    for (float z : {-0.12f * size.z, 0.12f * size.z})
        mesh::appendColored(m, mesh::box({0, yTop, z}, {0.96f * size.x, film, 0.06f * size.z}), kElectrode);
    ...
    for (int k = 0; k < perSide; ++k) {
        ...
        for (float z : {-0.32f * size.z, 0.32f * size.z})
            mesh::appendColored(m, mesh::box({x, yTop, z}, {0.8f * pitch, film, 0.3f * size.z}), kElectrode);
    }
    return m;
}
