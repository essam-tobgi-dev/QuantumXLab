...
int padsOnEdge(double length, const ChipLayoutOptions& o) {
    ...
    return length > 2.0 * corner
               ? static_cast<int>(std::floor((length - 2.0 * corner) / o.bondPadPitch_um)) + 1
               : 0;
}
...
void frameChip(ChipLayout& L, const ChipLayoutOptions& o, std::size_t signalsNeeded) {
    ...
    for (auto& r : L.resonators) {
        ...
        for (auto& p : r.centerline_um)
            p += shift;
    }
    for (auto& c : L.couplers)
        for (auto& s : c.stubs)
            for (auto& p : s.path_um)
                p += shift;
    L.size_um = size;
    ...
    for (int edge = 0; edge < 4; ++edge) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            switch (edge) {
            case 0:
                b.pos_um = {t, o.bondPadInset_um}, b.outward = {0.0, -1.0};
                break;
            case 1:
                b.pos_um = {size.x - o.bondPadInset_um, t}, b.outward = {1.0, 0.0};
                break;
            case 2:
                b.pos_um = {t, size.y - o.bondPadInset_um}, b.outward = {0.0, 1.0};
                break;
            default:
                b.pos_um = {o.bondPadInset_um, t}, b.outward = {-1.0, 0.0};
                break;
            }
...
void placeAirbridges(ChipLayout& L, const ChipLayoutOptions& o) {
    ...
    for (const auto& f : L.feedlines)
        routes.insert(routes.end(), {&f.route, &f.purcell});
    for (const auto& c : L.driveLines)
        routes.push_back(&c.route);
    for (const auto& c : L.fluxLines)
        routes.push_back(&c.route);
    for (const auto& c : L.couplers)
        for (const auto& s : c.stubs)
            routes.push_back(&s);
    L.airbridges.clear();
    ...
    for (const CpwRoute* r : routes) {
        ...
        for (double s = 0.5 * o.airbridgePitch_um; s < total - 0.5 * o.airbridgePitch_um;
             s += o.airbridgePitch_um) {
            double acc = s;
            for (std::size_t i = 1; i < P.size(); ++i) {
                ...
                L.airbridges.push_back(
                    {P[i - 1] + dir * acc, std::atan2(dir.y, dir.x) + 0.5 * pi, false});
                break;
            }
        }
    }
    ...
    for (const auto& b : L.airbridges)
        taken.insert({std::lround(b.pos_um.x / o.airbridgePitch_um),
                      std::lround(b.pos_um.y / o.airbridgePitch_um)});
        ...
        for (std::size_t b = a + 1; b < routes.size(); ++b) {
            ...
            for (std::size_t i = 1; i < A.size(); ++i)
                for (std::size_t j = 1; j < B.size(); ++j) {
                    glm::dvec2 at;
                    if (!segmentIntersection(A[i - 1], A[i], B[j - 1], B[j], at))
                        continue;
                    if (!taken
                             .insert({std::lround(at.x / o.airbridgePitch_um),
                                      std::lround(at.y / o.airbridgePitch_um)})
                             .second)
                        continue;
                    glm::dvec2 dir =
                        glm::normalize(B[j] - B[j - 1]); // the later route bridges over
