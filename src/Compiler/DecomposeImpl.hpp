...
private:
    ...
    Status controlledPhase(double gamma, std::span<const ir::Wire> controls, const SourceSpan& span, Out& out, int depth);

    const Target& target_;
    bool physical_;
    std::map<std::string, OneQubitSequence, std::less<>> named1q_;   // synthesis of parameterless gates (h, s, t, …)
};
