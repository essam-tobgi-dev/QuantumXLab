...
Vf2Result vf2Layout(const InteractionGraph& ig, const CouplingGraph& g,
                    std::uint64_t stateBudget = 1'000'000, std::uint32_t maxEmbeddings = 256);
...
Layout refineLayout(Layout start, const InteractionGraph& ig, const CouplingGraph& g,
                    std::uint64_t seed, std::uint32_t iterations = 200);
