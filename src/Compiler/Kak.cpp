...
const num::Matrix& magic() {
    static const num::Matrix q = [] {
        ...
        const C i(0, 1);
        return num::scale(
            num::Matrix::fromRows({{1, 0, 0, i}, {0, i, 1, 0}, {0, i, -1, 0}, {1, 0, 0, -i}}), r);
    }();
    return q;
}
    ...
    for (int sweep = 0; sweep < 64; ++sweep) {
        ...
        for (int p = 0; p < 4; ++p)
            for (int q = p + 1; q < 4; ++q) {
                ...
                const double t =
                    (theta >= 0 ? 1.0 : -1.0) / (std::abs(theta) + std::sqrt(theta * theta + 1.0));
...
bool factor(const num::Matrix& m, num::Matrix& hi, num::Matrix& lo) {
    ...
    for (std::size_t i = 0; i < 2; ++i)
        for (std::size_t j = 0; j < 2; ++j) {
            ...
            for (std::size_t k = 0; k < 2; ++k)
                for (std::size_t l = 0; l < 2; ++l)
                    tr += std::conj(lo(k, l)) * m(2 * i + k, 2 * j + l);
            hi(i, j) = tr / 2.0;
        }
...
num::Matrix KakDecomposition::reconstruct() const {
    ...
    return num::scale(num::matmul(num::matmul(left, canonicalGate(a, b, c)), right),
                      std::polar(1.0, phase));
}
...
std::uint32_t KakDecomposition::cxCount(double eps) const {
    ...
    for (double v : {a, b, c})
        if (std::abs(v) > eps) {
            ++nonzero;
            quarter = std::abs(std::abs(v) - kPi / 4) < eps;
        }
    if (nonzero == 0)
        return 0;
    if (nonzero == 1 && quarter)
        return 1;
    return nonzero <= 2 ? 2 : 3;
}

Result<KakDecomposition> kakDecompose(num::ConstMatrixView u) {
    if (u.rows != 4 || u.cols != 4 || !num::isUnitary(u, 1e-9))
        return fail(ErrorCode::InvalidArgument, "kakDecompose takes a 4×4 unitary");
    KakDecomposition k;
    ...
    for (double t : {0.37, 1.23, 2.11, 0.79, 2.77, 1.71}) {
        ...
        for (std::size_t i = 0; i < 4; ++i)
            for (std::size_t j = 0; j < 4; ++j)
                sym[i][j] = 0.5 * (std::cos(t) * (m2(i, j).real() + m2(j, i).real()) +
                                   std::sin(t) * (m2(i, j).imag() + m2(j, i).imag()));
        jacobi4(sym, v);
        for (std::size_t i = 0; i < 4; ++i)
            for (std::size_t j = 0; j < 4; ++j)
                p(i, j) = v[i][j];
    ...
    if (!diagonalised)
        return fail(ErrorCode::Internal, "kakDecompose: no common real eigenbasis found");
    ...
    for (std::size_t j = 0; j < 4; ++j) {
        theta[j] = std::arg(d(j, j)) / 2.0;
        sum += theta[j];
    }
    if (std::cos(sum) < 0.0)
        theta[0] += kPi; // det D^{1/2} = +1, so that K₁ ∈ SO(4)
    num::Matrix rootInverse(4, 4);
    for (std::size_t j = 0; j < 4; ++j)
        rootInverse(j, j) = std::polar(1.0, -theta[j]);
    num::Matrix k1 = num::matmul(num::matmul(m, p), rootInverse);
    for (auto& z : k1.data)
        z = C(z.real(), 0.0); // real up to rounding (unitary and complex orthogonal)
    ...
    for (const Axis& axis : {Axis{&k.a, "x"}, Axis{&k.b, "y"}, Axis{&k.c, "z"}}) {
        ...
        *axis.value = v;
        k.phase += turns * kPi / 2;
        if (std::fmod(std::abs(turns), 2.0) ==
            1.0) { // P⊗P = −(iP)⊗(iP), and det(iP) = 1 keeps A in SU(2)⊗SU(2)
    ...
    if (!num::approxEqual(k.reconstruct(), num::Matrix(u), 1e-8))
        return fail(ErrorCode::Internal, "kakDecompose: reconstruction failed");
    return k;
}
