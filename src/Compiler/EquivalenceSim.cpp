...
Result<std::uint32_t> mapWire(ir::Wire w, const std::vector<std::uint32_t>& wireMap) {
    if (w.index >= wireMap.size() || wireMap[w.index] == kUnmapped)
        return fail(ErrorCode::Internal, std::format("wire {} is used but was not mapped for simulation", w.index));
    return wireMap[w.index];
}

Status compileInto(const ir::Circuit& c, const std::vector<std::uint32_t>& wireMap, std::vector<SimOp>& out, SimProgram& totals) {
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            ...
            for (std::size_t j = 0; j < g->controls.size(); ++j) {
                ...
                if (!g->isNegControl(j)) op.controlPattern |= std::size_t{1} << q;
            }
            ++totals.gates;
...
struct Runner {
    ...
    int take(std::uint32_t wire, int wanted) {   // projects onto `wanted` unless it cannot happen
        ...
        if ((wanted ? p1 : 1.0 - p1) < kImpossible) wanted ^= 1;
    ...
    int likely(std::uint32_t wire) { return take(wire, state.probabilityOfOne(wire) > 0.5 + 1e-6 ? 1 : 0); }

    Status exec(const std::vector<SimOp>& ops) {
        for (const SimOp& op : ops) {
            switch (op.kind) {
            case SimOp::Kind::Gate: state.apply(op.matrix, op.wires, op.controlMask, op.controlPattern); break;
            case SimOp::Kind::Measure:
                if (op.bit == ir::kNoBit) { likely(op.wires[0]); break; }
                if (op.bit.index >= run.bits.size()) return fail(ErrorCode::OutOfRange, "measurement writes a bit outside the circuit");
                run.bits[op.bit.index] = static_cast<std::uint8_t>(
                    take(op.wires[0], static_cast<int>(mix(planSeed ^ mix(op.bit.index + 1) ^ mix(0x5151 + occurrence[op.bit.index]++)) & 1u)));
                break;
            case SimOp::Kind::Reset:
                if (likely(op.wires[0]) == 1) state.flip(op.wires[0]);
                break;
            case SimOp::Kind::Classical: ir::applyClassicalOp(*op.assign, run.bits); break;
            case SimOp::Kind::Branch: QXL_TRY(exec(op.condition->eval(run.bits) != 0 ? op.first : op.second)); break;
            case SimOp::Kind::Loop:
                for (std::uint32_t it = 0; it < std::min(op.maxIterations, kLoopCap) && op.condition->eval(run.bits) != 0; ++it)
                    QXL_TRY(exec(op.first));
                break;
            case SimOp::Kind::Block: QXL_TRY(exec(op.first)); break;
            }
        }
        return {};
    }
...
Result<SimProgram> compileForSimulation(const ir::Circuit& c, const std::vector<std::uint32_t>& wireMap) {
    SimProgram p;
    QXL_TRY(compileInto(c, wireMap, p.ops, p));
    return p;
}

Result<SimRun> execute(const SimProgram& p, StateVec& state, std::uint32_t clbits, std::uint64_t planSeed) {
    SimRun run;
