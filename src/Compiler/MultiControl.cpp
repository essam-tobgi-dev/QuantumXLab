...
bool isDiagonal(const num::Matrix& u) { return std::abs(u(0, 1)) < kEps && std::abs(u(1, 0)) < kEps; }
// γ with U = e^{iγ} X, if U has that form.
std::optional<double> phaseIfX(const num::Matrix& u) {
    if (std::abs(u(0, 0)) > kEps || std::abs(u(1, 1)) > kEps || std::abs(u(0, 1) - u(1, 0)) > kEps) return std::nullopt;
    return std::arg(u(0, 1));
}
bool negligible(double angle) { return std::abs(wrapAngle(angle)) < kEps; }
} // namespace

Status Decomposer::lowerControlled(const ir::Gate& g, Out& out, int depth) {
    const std::size_t n = g.controls.size();
    if (n > kMaxControls) return fail(lang::Diagnostics::make("QL4050", g.span, g.name, n).error);
    if (g.opaque) return fail(notDecomposable(g));

    std::vector<ir::Wire> negative;   // spec 14 §4.3: negctrl = ctrl conjugated by x
    for (std::size_t j = 0; j < n; ++j)
        if (g.isNegControl(j)) negative.push_back(g.controls[j]);
    for (ir::Wire w : negative) QXL_TRY(emitNamed("x", {w}, {}, g.span, out, depth + 1));
    ...
    if (!base.adjoint && !base.custom) joined = ir::gates::joinControlled(base.name, base.params, n);
    if (joined) {   // ctrl @ x = cx, ctrl(2) @ x = ccx, ctrl @ swap = cswap, … (T02 §3)
        ...
        if (base.name != "gphase" || base.params.size() != 1) return fail(notDecomposable(g));
        QXL_TRY(controlledPhase(base.adjoint ? -base.params[0] : base.params[0], controls, g.span, out, depth + 1));
    } else if (base.targets.size() == 1) {
        auto u = ir::baseMatrixOf(base);
        if (!u) return fail(notDecomposable(g));
        QXL_TRY(controlledU(*u, controls, base.targets[0], g.span, out, depth + 1));
    } else {
        auto expansion = expandToCx(base);
        if (!expansion) return fail(notDecomposable(g));
...
Status Decomposer::controlledPhase(double gamma, std::span<const ir::Wire> controls, const SourceSpan& span,
                                   Out& out, int depth) {
    if (controls.empty() || negligible(gamma)) return {};
    if (controls.size() == 1) return emitNamed("p", {controls[0]}, {wrapAngle(gamma)}, span, out, depth);
    return controlledU(phaseGate(gamma), controls.first(controls.size() - 1), controls.back(), span, out, depth);
}

Status Decomposer::controlledU(const num::Matrix& u, std::span<const ir::Wire> controls, ir::Wire target,
                               const SourceSpan& span, Out& out, int depth) {
    if (depth > kMaxLoweringDepth) return fail(ErrorCode::Internal, "controlled-gate lowering does not terminate");
    ...
    if (n == 1) {
        ...
        if (isDiagonal(u)) {   // ctrl @ diag(e^{iα}, e^{iβ}) = p(α)_c · cp(β − α)
            ...
            if (!negligible(delta)) QXL_TRY(emitNamed("cp", {c, target}, {wrapAngle(delta)}, span, out, depth + 1));
        ...
        return emitNamed("cu", {c, target}, {e.theta, e.phi, e.lambda, e.phase}, span, out, depth + 1);
    }
    ...
    auto root = ir::gates::unitaryPower(u.view(), 1.0 / static_cast<double>(std::size_t{1} << (n - 1)));
        ...
        if (const std::uint32_t diff = pattern ^ last; diff != 0) {
            ...
            if (pos != lm) {
                ...
                for (std::uint32_t idx = 0; idx < lm; ++idx)
                    if ((pattern >> idx) & 1u) QXL_TRY(emitNamed("cx", {controls[idx], controls[lm]}, {}, span, out, depth + 1));
            }
        }
        const bool odd = (std::popcount(pattern) & 1) != 0;
        QXL_TRY(controlledU(odd ? v : vDagger, controls.subspan(lm, 1), target, span, out, depth + 1));
        last = pattern;
    }
    return {};
}
