...
struct PairRef {
    std::uint32_t a, b, node;
}; // logical qubits of a blocked or upcoming two-qubit gate
} // namespace
...
Sabre::Sabre(const CouplingGraph& g, const RouteOptions& o, std::uint64_t seed)
    ...
    for (std::uint32_t a : g.dataQubits())
        for (std::uint32_t b : g.dataQubits()) {
            if (g.hops(a, b) == CouplingGraph::kUnreachable)
                continue;
            dist_[a * n_ + b] =
                o.noiseAware ? g.reliabilityDistance(a, b) : static_cast<double>(g.hops(a, b));
        }
}

Result<std::uint32_t> Sabre::run(const Dag& dag, bool reversed, FullLayout& layout,
                                 const OnNode& onNode, const OnSwap& onSwap, std::stop_token stop) {
    const auto& nodes = dag.nodes;
    const auto count = static_cast<std::uint32_t>(nodes.size());
    auto next = [&](std::uint32_t i) -> const std::vector<std::uint32_t>& {
        return reversed ? nodes[i].pred : nodes[i].succ;
    };
    std::vector<std::uint32_t> remaining(count), front;
    for (std::uint32_t k = 0; k < count; ++k) {
        const std::uint32_t i = reversed ? count - 1 - k : k;
        remaining[i] =
            static_cast<std::uint32_t>((reversed ? nodes[i].succ : nodes[i].pred).size());
        if (remaining[i] == 0)
            front.push_back(i);
    }
    ...
    std::vector<std::uint32_t> seenEdge(static_cast<std::size_t>(n_) * n_,
                                        0); // stamp per qubit pair
    ...
    while (!front.empty()) {
        ...
        for (std::size_t idx = 0; idx < front.size();) {
            ...
            if (nd.twoQubit && !g_.adjacent(layout.l2p[nd.wires[0]], layout.l2p[nd.wires[1]])) {
                ++idx;
                continue;
            }
            if (onNode)
                QXL_TRY(onNode(i, layout));
        ...
        if (stale) {
            ...
            for (std::size_t head = 0; head < queue.size() && ePairs.size() < o_.lookahead; ++head)
                for (std::uint32_t s : next(queue[head])) {
                    if (visited[s] == generation)
                        continue;
                    visited[s] = generation;
                    queue.push_back(s);
                    if (nodes[s].twoQubit && ePairs.size() < o_.lookahead)
                        ePairs.push_back({nodes[s].wires[0], nodes[s].wires[1], s});
                }
            for (const PairRef& p : fPairs) {
                fAdj[p.a].push_back(p.b);
                fAdj[p.b].push_back(p.a);
            }
            for (const PairRef& p : ePairs) {
                eAdj[p.a].push_back(p.b);
                eAdj[p.b].push_back(p.a);
            }
            stale = false;
        }
        if (sinceProgress > stallLimit) { // release valve: walk the closest blocked gate together
            ...
            for (const PairRef& p : fPairs)
                if (g_.hops(layout.l2p[p.a], layout.l2p[p.b]) <
                    g_.hops(layout.l2p[best->a], layout.l2p[best->b]))
                    best = &p;
            while (!g_.adjacent(layout.l2p[best->a], layout.l2p[best->b])) {
                ...
                if (g_.hops(p, target) == CouplingGraph::kUnreachable)
                    return fail(
                        err::BadLayout,
                        std::format("${} and ${} are in different components of the coupling graph",
                                    p, target));
                std::uint32_t step = p;
                for (std::uint32_t q : g_.neighbours(p))
                    if (g_.hops(q, target) < g_.hops(step, target))
                        step = q;
        ...
        for (const PairRef& pr : fPairs)
            for (std::uint32_t l : {pr.a, pr.b}) {
                ...
                for (std::uint32_t q : g_.neighbours(p)) {
                    ...
                    if (seenEdge[lo * n_ + hi] == swapRound)
                        continue;
        ...
        if (candidates.empty())
            return fail(err::BadLayout, "a program qubit sits on a device qubit without couplings");

        auto delta = [&](const std::vector<std::vector<std::uint32_t>>& adj, std::uint32_t a,
                         std::uint32_t b, std::uint32_t p,
                         std::uint32_t q) { // logical a on p moves to q, logical b on q moves to p
            double d = 0.0;
            for (std::uint32_t o : adj[a])
                if (o != b)
                    d += dist(q, layout.l2p[o]) - dist(p, layout.l2p[o]);
            for (std::uint32_t o : adj[b])
                if (o != a)
                    d += dist(p, layout.l2p[o]) - dist(q, layout.l2p[o]);
            return d;
        };
        ...
        for (const auto& [p, q] : candidates) {
            ...
            if (!ePairs.empty())
                h += o_.lookaheadWeight * delta(eAdj, a, b, p, q) /
                     static_cast<double>(ePairs.size());
            h += o_.decay * static_cast<double>(decay[p] + decay[q]);
            if (ties.empty() || h < bestScore - 1e-12) {
                bestScore = h;
                ties.assign(1, {p, q});
            } else if (h <= bestScore + 1e-12)
                ties.emplace_back(p, q);
        }
        const auto [p, q] = ties[rng_.uniformInt(ties.size())];
        std::uint32_t enabled = fPairs.front().node; // the blocked gate this swap works for
        for (const PairRef& pr : fPairs)
            if (pr.a == layout.p2l[p] || pr.b == layout.p2l[p] || pr.a == layout.p2l[q] ||
                pr.b == layout.p2l[q]) {
                enabled = pr.node;
                break;
            }
        ...
        ++decay[p];
        ++decay[q];
        if (o_.decayReset > 0 && swaps % o_.decayReset == 0)
            std::fill(decay.begin(), decay.end(), 0u);
    }
    return swaps;
}
