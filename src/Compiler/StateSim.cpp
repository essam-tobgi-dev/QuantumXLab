...
StateVec::StateVec(std::uint32_t qubits) : n_(qubits), amp_(std::size_t{1} << qubits, num::Complex{}) { amp_[0] = 1.0; }
...
void StateVec::apply(const num::Matrix& u, std::span<const std::uint32_t> wires, std::size_t cmask, std::size_t cpat) {
    ...
    for (std::size_t t = 0; t < k; ++t) targetMask |= std::size_t{1} << wires[t];
    for (std::size_t m = 0; m < block; ++m)
        for (std::size_t t = 0; t < k; ++t)
            if ((m >> t) & 1u) offset[m] |= std::size_t{1} << wires[t];
    std::vector<num::Complex> in(block);
    for (std::size_t base = 0; base < dim; ++base) {
        if ((base & targetMask) != 0 || (base & cmask) != cpat) continue;
        for (std::size_t m = 0; m < block; ++m) in[m] = a[base | offset[m]];
        for (std::size_t r = 0; r < block; ++r) {
            num::Complex s{};
            for (std::size_t m = 0; m < block; ++m) s += u(r, m) * in[m];
            a[base | offset[r]] = s;
        }
    }
}
...
Status StateVec::applyGate(const ir::Gate& g, std::span<const std::uint32_t> wireMap) {
    ...
    auto mapped = [&](ir::Wire w) -> Result<std::uint32_t> {
        if (w.index >= wireMap.size() || wireMap[w.index] >= n_)
            return fail(ErrorCode::OutOfRange, std::format("gate '{}' uses wire {} outside the simulated register", g.name, w.index));
    ...
    for (std::size_t j = 0; j < g.controls.size(); ++j) {
        ...
        if (!g.isNegControl(j)) cpat |= std::size_t{1} << q;
    }
...
double StateVec::project(std::uint32_t wire, int outcome) {
    ...
    for (std::size_t i = 0; i < amp_.size(); ++i) {
        if (((i & bit) != 0) == (outcome != 0)) amp_[i] *= scale;
        else amp_[i] = 0.0;
    }
    return p;
}
...
void StateVec::flip(std::uint32_t wire) {
    const std::size_t bit = std::size_t{1} << wire;
    for (std::size_t i = 0; i < amp_.size(); ++i)
        if (!(i & bit)) std::swap(amp_[i], amp_[i | bit]);
}

double StateVec::norm() const {
    double s = 0.0;
    for (const auto& z : amp_) s += std::norm(z);
    return std::sqrt(s);
}
...
num::Complex overlap(std::span<const num::Complex> a, std::span<const num::Complex> b) {
    num::Complex s{};
    for (std::size_t i = 0; i < a.size() && i < b.size(); ++i) s += std::conj(a[i]) * b[i];
    return s;
}
