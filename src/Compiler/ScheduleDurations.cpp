...
Durations::Durations(const hw::Device& dev, const hw::Calibration& cal, const PulseSource* source)
    : device(dev), calibration(cal), pulses(source), dt(Picoseconds{dev.timing.dtPs}),
      granule(Picoseconds{dev.timing.dtPs * std::max(1, dev.timing.granularitySamples)}), latency(Picoseconds{0}),
      granularity(std::max(1, dev.timing.granularitySamples)) {
    latency = nearest(dev.control.feedbackLatency.si());
}

Picoseconds Durations::nearest(double seconds) const { return pulse::quantise(seconds, dt, granularity, 0); }

Picoseconds Durations::ceilToGrid(Picoseconds p) const {
    if (p.get() <= 0) return Picoseconds{0};
    const std::int64_t g = granule.get();
    return Picoseconds{(p.get() + g - 1) / g * g};
}

Error Durations::missing(std::string_view what, std::initializer_list<std::uint32_t> qubits, const SourceSpan& span) const {
    std::string where;
    for (std::uint32_t q : qubits) where += std::format("{}${}", where.empty() ? "" : ", ", q);
    return lang::Diagnostics::make("QL4080", span, what, where).error;
}

Result<Picoseconds> Durations::gate(const ir::Gate& g) {
    if (g.width() == 0) return Picoseconds{0};                                 // gphase
    ...
    if (pulses && (pulses->library() || pulses->hasProgramDefcal(g.name, qubits))) {
        ...
        if (g.params.empty()) {
            key = g.name + "|";
            for (std::uint32_t q : qubits) key += std::format("{},", q);
            if (auto it = blockCache_.find(key); it != blockCache_.end()) return it->second;
        }
        QXL_TRY_ASSIGN(const pulse::Schedule block, pulses->gateBlock(g));
        const Picoseconds length = ceilToGrid(block.duration());
        if (!key.empty()) blockCache_.emplace(std::move(key), length);
        return length;
    }
    if (plain && g.name == "rz") return Picoseconds{0};                        // virtual Z (§5.4)
    if (g.opaque || !plain || qubits.size() > 2) {
        if (qubits.size() == 1) return fail(missing(g.name, {qubits[0]}, g.span));
        if (qubits.size() == 2) return fail(missing(g.name, {qubits[0], qubits[1]}, g.span));
        return fail(Error(err::Unsupported, std::format("gate '{}' on {} qubits has no calibrated duration; decompose before scheduling", g.name, qubits.size())).withSpan(g.span));
    }
    auto seconds = calibration.gateDuration(g.name, qubits);
    if (!seconds) return qubits.size() == 1 ? fail(missing(g.name, {qubits[0]}, g.span)) : fail(missing(g.name, {qubits[0], qubits[1]}, g.span));
...
Result<Picoseconds> Durations::measure(std::uint32_t q, const SourceSpan& span) {
    ...
    if (!qc) return fail(missing("measure", {q}, span));
...
Result<Picoseconds> Durations::reset(std::uint32_t q, const SourceSpan& span) {
    ...
    const hw::QubitCal* qc = calibration.qubit(q);
    if (!qc) return fail(missing("reset", {q}, span));
    ...
    case hw::ResetPolicy::Passive: return nearest(device.control.passiveMultiplier * qc->t1.value.si());
    case hw::ResetPolicy::Cooling: return nearest(device.control.coolingTime.si());
    }
