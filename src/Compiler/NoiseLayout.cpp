...
double localScore(const std::vector<std::uint32_t>& v2p, const InteractionGraph& ig,
                  const CouplingGraph& g, std::uint32_t a, std::uint32_t b) {
    double s = 0.0;
...
double layoutScore(const Layout& l, const InteractionGraph& ig, const CouplingGraph& g) {
    ...
    for (const auto& e : ig.edges)
        s += e.weight * pairCost(g, l.v2p[e.a], l.v2p[e.b]);
    for (std::uint32_t v = 0; v < ig.qubits; ++v)
        s += nodeCost(ig, g, v, l.v2p[v]);
    return s;
}

Layout refineLayout(Layout start, const InteractionGraph& ig, const CouplingGraph& g, std::uint64_t,
    ...
    for (std::uint32_t it = 0; it < iterations; ++it) {
        ...
        auto consider = [&](std::uint32_t v,
                            std::uint32_t target) { // exchange v with whatever sits on `target`
        ...
        for (std::uint32_t v = 0; v < ig.qubits; ++v) {
            if (!ig.used[v])
                continue;
            for (std::uint32_t p : g.neighbours(v2p[v]))
                consider(v, p);
            if (allPairs)
        ...
        if (bestV == kFree)
            break; // no exchange improves the score
...
Result<LayoutChoice> chooseLayout(const ir::Circuit& c, const CouplingGraph& g, LayoutPolicy policy,
                                  std::uint64_t seed, std::uint64_t vf2StateBudget,
    ...
    if (policy == LayoutPolicy::Trivial) {
        ...
        if (choice.vf2.layout) {
            ...
            if (choice.vf2.budgetExceeded && diagnostics)
                diagnostics->push_back(lang::Diagnostics::make("QL4060", SourceSpan{}));
            QXL_TRY_ASSIGN(choice.layout, denseLayout(ig, g, seed));
            if (policy == LayoutPolicy::Vf2)
                choice.policy = LayoutPolicy::Dense;
        }
        if (policy == LayoutPolicy::NoiseAware)
            choice.layout = refineLayout(std::move(choice.layout), ig, g, seed);
    }
