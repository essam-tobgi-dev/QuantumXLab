...
WorkList::WorkList(std::vector<ir::Node> list, std::uint32_t qubitCount) : nodes(std::move(list)), qubits(qubitCount) {
    ...
    for (std::uint32_t i = 0; i < n; ++i) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&nodes[i])) {   // targets first, then controls: `Gate::wires()` order
            std::size_t k = 0;
            for (ir::Wire w : g->targets) place(i, w.index, wireAction(*g, k++));
            for (ir::Wire w : g->controls) place(i, w.index, wireAction(*g, k++));
            continue;
        }
        ...
        for (ir::Wire w : ir::nodeWiresIn(nodes[i], qubits)) place(i, w.index, 0);
    }
    begin_.push_back(static_cast<std::uint32_t>(wireFlat_.size()));
}
...
std::vector<ir::Node> WorkList::flush() {
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        if (alive[i]) out.push_back(std::move(nodes[i]));
        for (ir::Node& n : after[i]) out.push_back(std::move(n));
    }
    return out;
}
...
bool WorkList::commutes(std::uint32_t i, std::uint32_t j) const {
    for (std::uint32_t a = begin_[i]; a < begin_[i + 1]; ++a)
        for (std::uint32_t b = begin_[j]; b < begin_[j + 1]; ++b)
            if (wireFlat_[a] == wireFlat_[b] && (action_[a] & action_[b]) == 0) return false;
    return true;
}

bool WorkList::anyDirty(std::uint32_t i) const {
    for (std::uint32_t a = begin_[i]; a < begin_[i + 1]; ++a)
        if (dirty[wireFlat_[a]]) return true;
    return false;
}

void WorkList::touch(std::uint32_t i) {
    for (std::uint32_t a = begin_[i]; a < begin_[i + 1]; ++a) touched[wireFlat_[a]] = 1;
}

bool isFusable(const ir::Gate& g) { return !g.opaque && g.controls.empty() && g.targets.size() == 1; }
