...
double Affine::eval(std::span<const double> p) const {
    double v = c0;
    for (std::size_t k = 0; k < c.size() && k < p.size(); ++k)
        v += c[k] * p[k];
    return v;
}
        ...
        for (std::uint8_t k : st.operands) {
            if (k >= source.targets.size())
                return fail(ErrorCode::Internal,
                            std::format("rule '{}' uses operand {} of a {}-qubit gate", rule.source,
                                        k, source.targets.size()));
            wires.push_back(source.targets[k]);
        }
        std::vector<double> params;
        params.reserve(st.params.size());
        for (const Affine& a : st.params)
            params.push_back(a.eval(source.params));
        QXL_TRY_ASSIGN(ir::Gate made,
                       gate(st.gate, std::move(wires), std::move(params), source.span));
...
std::string decompositionRulesJson() {
    ...
    for (const Rule& r : decompositionRules()) {
        ...
        for (const RuleStep& st : r.steps) {
            ...
            for (const Affine& a : st.params)
                params.push_back(a.text());
            seq.push_back({{"gate", std::string(st.gate)},
                           {"qubits", st.operands},
    ...
    core::Json data{
        {"convention",
         "sequence in time order; source = exp(i*phase) * product; p<k> = k-th source parameter; "
