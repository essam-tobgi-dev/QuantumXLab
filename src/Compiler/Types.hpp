...
// `pragma qlab.layout physical` is a property of the circuit (`Circuit::isPhysical()`), not a
// policy.
...
struct CompileOptions {
    ...
    std::uint64_t equivalenceWorkLimit =
        20'000'000;                           // amplitude updates spent by the automatic check
...
struct CompileContext {
    ...
    static CompileContext from(const CompileOptions& o, const hw::Device* device,
...
inline constexpr ErrorCode BadLayout =
    ErrorCode::Compiler_ + 101; // layout not injective / off-device
inline constexpr ErrorCode NoDevice =
    ErrorCode::Compiler_ + 102; // pass needs a device or calibration
inline constexpr ErrorCode BadCalibrationBlock =
    ErrorCode::Compiler_ + 103; // program cal/defcal not lowerable
