...
std::uint64_t mixText(std::uint64_t h, std::string_view s) {
    return mix(h, programHash(s));
}
template <class T> std::uint64_t opt(const std::optional<T>& o) {
    return o ? 1 + static_cast<std::uint64_t>(*o) : 0;
}

template <class Cache>
auto lookup(Cache& cache, std::uint64_t key) -> decltype(cache.front().second) {
    const auto it =
        std::find_if(cache.begin(), cache.end(), [&](const auto& e) { return e.first == key; });
    if (it == cache.end())
        return nullptr;
    std::rotate(it, it + 1, cache.end()); // most recently used last
    return cache.back().second;
}
template <class Cache, class Value>
void store(Cache& cache, std::uint64_t key, Value value, std::size_t capacity) {
    cache.emplace_back(key, std::move(value));
    if (cache.size() > std::max<std::size_t>(capacity, 1))
        cache.erase(cache.begin());
}
} // namespace

IncrementalCompiler::IncrementalCompiler(core::JobSystem& jobs)
    : IncrementalCompiler(jobs, Settings{}) {}
IncrementalCompiler::IncrementalCompiler(core::JobSystem& jobs, Settings settings)
...
void IncrementalCompiler::setTarget(const hw::Device* device, const hw::Calibration* calibration,
                                    CompileOptions options) {
    std::lock_guard lock(mutex_);
    device_ = device;
...
std::uint64_t IncrementalCompiler::submit(std::string source, std::string filename) {
    ...
    auto task = [this, generation, source = std::move(source), filename = std::move(filename),
                 snapshot, token, previous](std::stop_token pool) {
        if (previous.valid())
            previous.wait();
        work(generation, source, filename, snapshot, token, pool);
    };
    std::shared_future<void> next =
        jobs_.submit(std::move(task), core::JobPriority::Interactive).share();
    std::lock_guard lock(mutex_);
...
std::shared_ptr<const IncrementalCompiler::Update> IncrementalCompiler::wait() {
    for (;;) {
        ...
        std::lock_guard lock(mutex_);
        if (seen == generation_)
            return latest_; // nothing newer arrived while waiting
    }
}
...
void IncrementalCompiler::work(std::uint64_t generation, const std::string& source,
                               const std::string& filename, const Snapshot& target,
                               std::stop_token superseded, std::stop_token pool) {
    ...
    frontKey =
        mix(frontKey, opt(o.level.transform([](OptimizeLevel l) { return static_cast<int>(l); })));
    frontKey = mix(mix(frontKey, o.loopUnrollBound), o.kak ? 1 : 0);
    frontKey =
        mixText(mixText(frontKey, o.twoQubitBasis), target.device ? target.device->id : "{U, cx}");
    for (const auto& [name, value] : o.inputs)
        frontKey = mix(mixText(frontKey, name), std::bit_cast<std::uint64_t>(value));
    std::uint64_t fullKey =
        mix(frontKey, opt(o.layout.transform([](LayoutPolicy p) { return static_cast<int>(p); })));
    fullKey =
        mix(fullKey, opt(o.routing.transform([](RoutingPolicy p) { return static_cast<int>(p); })));
    fullKey = mix(mix(fullKey, opt(o.pulseLevel)), opt(o.seed));
    fullKey =
        mix(mix(fullKey, static_cast<std::uint64_t>(o.schedule)), o.verifyEquivalence ? 1 : 0);
    ...
    std::shared_ptr<const FrontEntry> front;
    {
        ...
        if (!cachedFull)
            front = lookup(frontCache_, frontKey);
    }
    if (cachedFull) {
        *update = *cachedFull;
        update->generation = generation;
        ...
        update->compileTime =
            std::chrono::microseconds(static_cast<std::int64_t>(timer.seconds() * 1e6));
    ...
    auto finish = [&](Result<CompiledProgram> result, const PassManager* pm) {
        ...
        if (pm)
            update->trace = pm->trace();
        if (result)
            for (const auto& d : result->diagnostics)
                update->diagnostics.push_back(d);
        else {
            lang::Diagnostic d;
        ...
        if (update->program) {
            ...
            std::lock_guard lock(mutex_);
            store(fullCache_, fullKey, std::shared_ptr<const Update>(update),
                  settings_.cacheEntries);
        }
        publish(update);
    };
    ...
    if (!front) { // passes 1–4: parse, analyse, Build, Verify, Decompose, Optimize
        ...
        for (const auto& d : program.diagnostics)
            update->diagnostics.push_back(d);
        if (!program.ok()) {
            finish(std::unexpected(program.errors().front()), nullptr);
            return;
        }
        PassManager pm =
            PassManager::standard(resolveContext(program, o, target.device, target.calibration));
        auto built = pm.build(program, o.inputs);
        if (!built) {
            finish(std::unexpected(built.error()), &pm);
            return;
        }
        auto reference = std::make_shared<const ir::Circuit>(*built);
        auto early = pm.runRange(std::move(*built), reference, 0, pm.frontEnd(), stop);
        if (!early) {
            finish(std::unexpected(early.error()), &pm);
            return;
        }
    ...
    if (!o.layout)
        ctx.layout = front->layout;
    if (!o.routing)
        ctx.routing = front->routing;
    if (!o.pulseLevel)
        ctx.pulseLevel = front->pulseLevel && target.device != nullptr;
    if (!o.seed)
        ctx.seed = front->seed;
    ...
    if (ctx.pulseLevel && !ctx.pulses && target.device && !target.device->directory.empty())
        if (auto loaded = pulse::loadPulses(target.device->directory)) {
            auto resolved = target.calibration ? loaded->withCalibration(*target.calibration)
                                               : Result<pulse::PulseLibrary>(*loaded);
