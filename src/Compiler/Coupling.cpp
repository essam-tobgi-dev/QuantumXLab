...
Result<CouplingGraph> CouplingGraph::build(const hw::Device& device,
    ...
    if (g.data_.empty())
        return fail(err::NoDevice, std::format("device '{}' has no data qubits", device.id));

    g.adj_.assign(g.qubits_, {});
    auto addEdge = [&](std::uint32_t a, std::uint32_t b) {
        if (a == b || !g.isData(a) || !g.isData(b))
            return;
        if (std::find(g.adj_[a].begin(), g.adj_[a].end(), b) != g.adj_[a].end())
            return;
    ...
    if (device.allToAll) {
        for (std::size_t i = 0; i < g.data_.size(); ++i)
            for (std::size_t j = i + 1; j < g.data_.size(); ++j)
                addEdge(g.data_[i], g.data_[j]);
    } else {
    ...
    for (std::size_t i = 0; i < d; ++i) {
        slot[g.data_[i]] = static_cast<std::uint32_t>(i);
        dist[i * d + i] = 0.0;
    }
    ...
    for (std::size_t k = 0; k < d; ++k)
        for (std::size_t i = 0; i < d; ++i) {
            ...
            for (std::size_t j = 0; j < d; ++j)
                if (ik + dist[k * d + j] < dist[i * d + j])
                    dist[i * d + j] = ik + dist[k * d + j];
        }
    g.rel_.assign(n * n, inf);
    for (std::size_t i = 0; i < d; ++i)
        for (std::size_t j = 0; j < d; ++j)
            g.rel_[g.data_[i] * n + g.data_[j]] = dist[i * d + j];
    ...
    for (std::uint32_t q : g.data_) {
        ...
        if (qc->t1.value.si() > 0.0)
            g.decay_[q] = 1.0 / qc->t1.value.si();
    }
    return g;
}
