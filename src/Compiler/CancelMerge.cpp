// Spec 14 §5.1 (inverse cancellation), §5.3 (commutation), §5.6 (diagonal merging). For a gate g the
// sweep walks back along g's first wire over gates that commute with g. On meeting a gate h on the
// same operands it checks that everything between h and g on g's other wires commutes with g as
...
double additivePeriod(std::string_view n) {
    if (n == "rx" || n == "ry" || n == "rz" || n == "p" || n == "phase" || n == "u1" || n == "cp" || n == "rxx" ||
        n == "ryy" || n == "rzz" || n == "ms")
        return 2 * kPi;
    if (n == "crx" || n == "cry" || n == "crz") return 4 * kPi;
    return 0.0;
}
double reduce(double a, double period) { return a - period * std::ceil((a - period / 2) / period); }
...
bool clearBetween(const WorkList& w, std::uint32_t j, std::uint32_t i) {
    ...
    for (std::uint32_t slot = 1; slot < wires.size(); ++slot) {
        ...
        for (std::uint32_t steps = 0; k != j; ++steps) {
            if (k == WorkList::npos || steps > kCommuteWindow || !w.gateAt(k) || !w.commutes(k, i)) return false;
...
ir::Gate diagonalGate(Basis1q basis, double angle, const ir::Gate& like) {
    ...
    d.params = basis == Basis1q::U ? std::vector<double>{0.0, 0.0, angle} : std::vector<double>{angle};
...
std::size_t pulsesOf(const ir::Gate& g, Basis1q basis) {
    const bool native = !g.adjoint && !g.custom &&
                        (basis == Basis1q::U     ? g.name == "U"
                         : basis == Basis1q::ZSX ? (g.name == "rz" || g.name == "sx" || g.name == "x" || g.name == "id")
                                                 : (g.name == "rz" || g.name == "rx" || g.name == "ry"));
    if (native) return isFrameChange(g.name) ? 0u : 1u;
...
bool combine(WorkList& w, std::uint32_t j, std::uint32_t i, const MergeRules& rules) {
    ...
    const auto removeBoth = [&] { w.kill(j); w.kill(i); return true; };
    if (isNamedInverseOf(h, g)) return removeBoth();
    ...
    if (!single) return isInverseOf(h, g) ? removeBoth() : false;

    const bool bothDiagonal = (w.actionOf(j, 0) & w.actionOf(i, 0) & kActsZ) != 0;
    const auto dh = bothDiagonal ? diagonalAngle(h) : std::nullopt, dg = bothDiagonal ? diagonalAngle(g) : std::nullopt;
    if (dh && dg) {   // §5.6: two frame changes are one
        const double total = wrapAngle(*dh + *dg);
        w.kill(i);
        if (std::abs(total) < kAngleEps) w.kill(j);
        else { h = diagonalGate(rules.basis, total, h); w.touch(j); }
        return true;
    }
    ...
    if (rules.resynthesize && (w.actionOf(j, 0) & w.actionOf(i, 0) & kActsX) != 0) {
        ...
        if (!(RunCost{s.pulses(), s.gates.size()} < before)) return false;
...
bool addAngles(WorkList& w, std::uint32_t j, std::uint32_t i, const MergeRules& rules) {
    ir::Gate& h = *w.gateAt(j);
    const ir::Gate& g = *w.gateAt(i);
    if (h.params.size() != 1 || h.name != g.name || h.adjoint || g.adjoint || h.custom || g.custom || !h.controls.empty()) return false;
    ...
    // One native entangler pulse covers |θ| ≤ maxEntanglerAngle (T06 §6): larger sums stay two gates.
    if (rules.maxEntanglerAngle > 0.0 && h.targets.size() == 2 && std::abs(total) > rules.maxEntanglerAngle + kAngleEps) return false;
    w.kill(i);
    if (std::abs(total) < kAngleEps) { w.kill(j); return true; }
    h.params[0] = total;
    h.matrixCache.reset();
...
bool sweepCancelMerge(WorkList& w, const MergeRules& rules) {
    ...
    for (std::uint32_t i = 0; i < n; ++i) {
        const ir::Gate* g = w.gateAt(i);
        if (!g || g->opaque || w.wiresOf(i).empty() || !w.anyDirty(i)) continue;
        ...
        for (std::uint32_t steps = 0; j != WorkList::npos && steps < kCommuteWindow; ++steps) {
            ...
            if (!h) break;   // measure, reset, barrier, delay or a control node: nothing passes
            if (sameOperands(*h, *g) && clearBetween(w, j, i) && (addAngles(w, j, i, rules) || combine(w, j, i, rules))) {
                changed = true;
                break;
            }
            if (!rules.lookThrough || !w.commutes(j, i)) break;
