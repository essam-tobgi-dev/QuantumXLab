...
struct Lowering {
    ...
    pulse::ChannelId driveOf(std::uint32_t q) const {
        return source.device().technology == hw::Technology::IonChain ? pulse::ChannelId::raman(q) : pulse::ChannelId::drive(q);
    }
    ...
    Status place(pulse::Schedule block, Picoseconds at) {
        ...
        auto isRepeat = [&](const pulse::Instruction& i) {
            ...
            if (!p || pulse::channelKindArity(p->ch.kind) != 0) return false;
            for (const pulse::Play& s : shared)
                if (s.ch == p->ch && s.t0 == p->t0 && s.wf == p->wf) return true;
            return false;
        };
        for (const auto& i : block.instructions()) repeated = repeated || isRepeat(i);
        if (repeated) {
            pulse::Schedule kept(block.dt());
            for (const auto& f : block.frames()) kept.addFrame(f);
            for (const auto& i : block.instructions())
                if (!isRepeat(i)) kept.insert(i, pulse::instructionStart(i));
            block = std::move(kept);
        }
        for (const auto& i : block.instructions())
            if (const auto* p = std::get_if<pulse::Play>(&i); p && pulse::channelKindArity(p->ch.kind) == 0) shared.push_back(*p);
    ...
    static Result<std::vector<TimedNode>> slotsOf(const ir::Circuit& c) {
        ...
        if (!meta.contains("schedule") || !meta["schedule"].contains("start_ps") || !meta["schedule"].contains("length_ps"))
            return fail(err::Unsupported, "pulse lowering needs a scheduled circuit (run the Schedule pass first)");
        const auto& starts = meta["schedule"]["start_ps"];
        const auto& lengths = meta["schedule"]["length_ps"];
        if (!starts.is_array() || !lengths.is_array() || starts.size() != c.nodeCount() || lengths.size() != c.nodeCount())
        ...
        for (std::size_t i = 0; i < starts.size(); ++i) {
            ...
            out.push_back({Picoseconds{starts[i].get<std::int64_t>()}, Picoseconds{lengths[i].get<std::int64_t>()}});
        }
        return out;
    }

    // `parent` is the top-level index of the Branch whose arm is being lowered (kTopLevel outside).
    Status level(const ir::Circuit& c, const std::vector<TimedNode>& slots, Picoseconds offset, bool topLevel,
                 std::uint32_t parent, const std::string& condition) {
        ...
        for (ir::NodeId id : c.topologicalOrder()) {
            ...
            if (const auto* g = std::get_if<ir::Gate>(&n)) {
                ...
                QXL_TRY(place(std::move(block), at));
            } else if (const auto* d = std::get_if<ir::Delay>(&n)) {   // a frame delay on the drive line (spec 14 §6)
                const Picoseconds length = d->duration.resolve(source.dt());
                pulse::Schedule block(source.dt());
                std::vector<std::uint32_t> wires;
                for (ir::Wire w : d->wires) wires.push_back(w.index);
                if (d->wires.empty()) wires = activeWires;
                for (std::uint32_t w : wires)
                    if (length.get() > 0) block.insert(pulse::Delay{driveOf(w), {}, length}, Picoseconds{0});
                QXL_TRY(place(std::move(block), at));
                ...
                if ((*br->elseBody).nodeCount() > 0)
                    return fail(Error(err::Unsupported, "a pulse schedule has no branch: only 'if' without 'else' can be lowered (its arm is played "
                                                        "conditionally by the backend)").withSpan(br->span));
                if (!condition.empty())
                    return fail(Error(err::Unsupported, "nested conditions cannot be lowered to a pulse schedule").withSpan(br->span));
                QXL_TRY_ASSIGN(const auto inner, slotsOf(*br->thenBody));
                const Picoseconds latency = source.quantise(source.device().control.feedbackLatency.si(), false);
                QXL_TRY(level(*br->thenBody, inner, at + latency, false, topLevel ? index : parent, br->cond.text()));
            } else if (std::holds_alternative<ir::Loop>(n)) {
                return fail(Error(err::Unsupported, "a run-time loop cannot be lowered to a fixed pulse schedule").withSpan(ir::nodeSpan(n)));
            }
            // A window is the node's scheduled slot, so the two time axes agree by construction.
            if (topLevel || !condition.empty()) windows.push_back({index, topLevel ? PulseWindow::kTopLevel : parent, at, end, !condition.empty(), condition});
            ++index;
        }
        return {};
    }
...
Result<PulseProgram> lowerToPulses(const ir::Circuit& c, const ScheduleInfo& timing, const PulseSource& source) {
    if (!c.isPhysical()) return fail(err::Unsupported, "pulse lowering takes a circuit on physical qubits");
    if (timing.nodes.size() != c.nodeCount()) return fail(err::Unsupported, "pulse lowering needs the schedule of this circuit");
    ...
    for (const pulse::FrameDecl& f : source.programFrames()) out.schedule.addFrame(f);   // program frames take precedence
    if (!blocks.empty()) out.schedule.merge(blocks.front());
    out.windows = std::move(low.windows);
    auto verified = out.schedule.verify(source.device(), &out.warnings);
    if (!verified) return fail(verified.error().withNote("while verifying the lowered pulse schedule (spec 10 §5)"));
    return out;
}
