...
struct WorkList {
    ...
    std::span<const std::uint32_t> wiresOf(std::uint32_t i) const {
        return {wireFlat_.data() + begin_[i], begin_[i + 1] - begin_[i]};
    }
    const ir::Gate* gateAt(std::uint32_t i) const {
        return alive[i] ? std::get_if<ir::Gate>(&nodes[i]) : nullptr;
    }
    ir::Gate* gateAt(std::uint32_t i) {
        return alive[i] ? std::get_if<ir::Gate>(&nodes[i]) : nullptr;
    }
    ...
    std::uint8_t actionOf(std::uint32_t i, std::uint32_t slot) const {
        return action_[begin_[i] + slot];
    }
...
struct RunCost {
    ...
    bool operator<(const RunCost& o) const {
        return pulses != o.pulses ? pulses < o.pulses : gates < o.gates;
    }
};
