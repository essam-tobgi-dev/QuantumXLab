...
struct PassContext {
    explicit PassContext(const CompileContext& cc, std::stop_token st = {}) : options(cc), stop(std::move(st)) {}

    const CompileContext& options;
    std::stop_token stop;
    std::vector<lang::Diagnostic> diagnostics;          // of the running pass; the manager moves them into its PassResult
...
public:
    ...
    //   Verify, Decompose, [Optimize ≥O1], then with a device: Layout, Route, Decompose, [Optimize ≥O1],
    //   [VirtualZ ≥O1], Schedule, [PulseLower when pulseLevel]; finally Verify and Equivalence.
    ...
    Result<CompileOutput> run(const lang::Program& program, const ir::ParamMap& inputs = {}, std::stop_token stop = {});
    ...
    // (incremental compilation caches the circuit after pass 4). `reference` is the post-Build circuit.
    Result<CompileOutput> runRange(ir::Circuit circuit, std::shared_ptr<const ir::Circuit> reference, std::size_t first,
    ...
    void setTrace(std::vector<PassResult> earlier) { trace_ = std::move(earlier); }   // resume after cached passes
