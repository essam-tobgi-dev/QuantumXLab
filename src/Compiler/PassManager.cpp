...
Result<CompileOutput> PassManager::run(const lang::Program& program, const ir::ParamMap& inputs,
                                       std::stop_token stop) {
    trace_.clear();
...
Result<ir::Circuit> PassManager::build(const lang::Program& program, const ir::ParamMap& inputs) {
    ...
    normalizeBodies(
        *built); // an absent `else` arrives as an empty virtual body (see CircuitUtil.hpp)
    build.after = measureCircuit(*built);
    trace_.push_back(std::move(build));
    return built;
}

Result<CompileOutput> PassManager::runRange(ir::Circuit circuit,
                                            std::shared_ptr<const ir::Circuit> reference,
    ...
    if (circuit.isPhysical() && first > 0) {
        ctx.initialLayout.v2p =
            metaIndices(circuit, "layout").value_or(std::vector<std::uint32_t>{});
        ctx.finalLayout.v2p = metaIndices(circuit, "final_layout").value_or(ctx.initialLayout.v2p);
        const auto& meta = circuit.meta();
        if (meta.contains("swap_count") && meta["swap_count"].is_number_unsigned())
            ctx.swapCount = meta["swap_count"].get<std::uint32_t>();
    }
    CompileOutput out;
    ...
    out.trace = trace_;
    if (context_.device)
        out.deviceId = context_.device->id;
    if (context_.calibration)
        out.calibrationTimestamp = context_.calibration->timestamp;
    return out;
}
