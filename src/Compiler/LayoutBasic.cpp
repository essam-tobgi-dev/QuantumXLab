            ...
            for (std::size_t i = 0; i < ws.size(); ++i)
                for (std::size_t j = i + 1; j < ws.size(); ++j)
                    if (ws[i].index < ig.qubits && ws[j].index < ig.qubits && ws[i] != ws[j])
                        ++weights[{std::min(ws[i].index, ws[j].index),
                                   std::max(ws[i].index, ws[j].index)}];
        } else if (const auto* m = std::get_if<ir::Measure>(&n)) {
            if (m->qubit.index < ig.qubits)
                ig.measured[m->qubit.index] = 1;
        } else {
...
std::vector<std::uint32_t> bfsOrder(const std::vector<std::uint32_t>& seeds, std::uint32_t space,
                                    Neighbours&& next) {
...
Status validateLayout(const Layout& l, std::uint32_t programQubits, const CouplingGraph& g) {
    if (l.size() != programQubits)
        return fail(err::BadLayout, std::format("layout maps {} qubit(s) but the program has {}",
                                                l.size(), programQubits));
    if (!l.injective())
        return fail(err::BadLayout, "layout maps two program qubits to one physical qubit");
    for (std::uint32_t v = 0; v < l.size(); ++v)
        if (!g.isData(l.v2p[v]))
            return fail(
                err::BadLayout,
                std::format("layout maps q{} to ${}, which is not a data qubit of the device", v,
                            l.v2p[v]));
    return {};
}
...
Result<Layout> trivialLayout(std::uint32_t programQubits, const CouplingGraph& g) {
    if (programQubits > g.dataQubits().size())
        return fail(err::BadLayout,
                    std::format("the program uses {} qubits but the device has {} data qubits",
                                programQubits, g.dataQubits().size()));
    Layout l;
...
Result<Layout> denseLayout(const InteractionGraph& ig, const CouplingGraph& g, std::uint64_t seed) {
    ...
    if (k > data.size())
        return fail(err::BadLayout,
                    std::format("the program uses {} qubits but the device has {} data qubits", k,
                                data.size()));
    if (k == 0)
        return Layout{};
    ...
    const std::uint32_t hub =
        *std::max_element(data.begin(), data.end(), [&](std::uint32_t a, std::uint32_t b) {
        ...
        while (set.size() < k) {
            ...
            for (std::uint32_t p : data) {
                ...
                if (into > bestInto) {
                    bestInto = into;
                    ties.assign(1, p);
                } else if (into == bestInto)
                    ties.push_back(p);
            }
            if (bestInto == 0) { // component exhausted: continue from the closest free qubit
                std::stable_sort(ties.begin(), ties.end(), [&](std::uint32_t a, std::uint32_t b) {
                    return g.hops(set.front(), a) < g.hops(set.front(), b);
                });
                ties.resize(1);
            }
            const std::uint32_t pick =
                restart == 0 ? ties.front() : ties[rng.uniformInt(ties.size())];
            in[pick] = 1;
            set.push_back(pick);
            internal += bestInto;
        }
        if (best.empty() || internal > bestEdges) {
            best = set;
            bestEdges = internal;
        }
    }
    ...
    for (std::uint32_t v = 0; v < k; ++v)
        vSeeds[v] = v;
    auto strength = [&](std::uint32_t v) {
        std::uint64_t s = 0;
        for (auto [u, w] : ig.adj[v]) {
            (void)u;
            s += w;
        }
        return s;
    };
    std::stable_sort(vSeeds.begin(), vSeeds.end(),
                     [&](std::uint32_t a, std::uint32_t b) { return strength(a) > strength(b); });
    ...
    for (std::uint32_t p : best)
        inBest[p] = 1;
    auto internalDegree = [&](std::uint32_t p) {
        std::size_t d = 0;
        for (std::uint32_t q : g.neighbours(p))
            d += inBest[q];
        return d;
    ...
    std::stable_sort(pSeeds.begin(), pSeeds.end(), [&](std::uint32_t a, std::uint32_t b) {
