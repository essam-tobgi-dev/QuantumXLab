...
public:
    ...
    struct Update {
        ...
        Result<CompiledProgram> program = std::unexpected(Error(ErrorCode::Unknown, "not compiled"));
    ...
    void setTarget(const hw::Device* device, const hw::Calibration* calibration, CompileOptions options = {});
...
private:
    struct FrontEntry;
    struct Snapshot;
    void work(std::uint64_t generation, const std::string& source, const std::string& filename, const Snapshot& target,
    ...
    std::vector<std::pair<std::uint64_t, std::shared_ptr<const FrontEntry>>> frontCache_;   // most recent last
