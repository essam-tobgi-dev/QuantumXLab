...
struct Search {
    ...
    Layout complete() const {
        ...
        for (std::uint32_t v = 0; v < ig.qubits; ++v) {
            if (l.v2p[v] != kNone)
                continue;
            while (next < data.size() && used[data[next]])
                ++next;
            if (next == data.size())
                break;
            l.v2p[v] = data[next];
    ...
    bool feasible(std::uint32_t v, std::uint32_t p) const {
        if (taken[p] || g.degree(p) < ig.degree(v))
            return false;
        for (const auto& [u, w] : ig.adj[v]) {
            (void)w;
            if (image[u] != kNone && !g.adjacent(p, image[u]))
                return false;
        }
        return true;
    }
    ...
    void search(std::size_t depth) {
        if (depth == order.size()) {
            ...
            if (!result.layout || s < bestScore) {
                result.layout = l;
                bestScore = s;
            }
            if (result.embeddings >= maxEmbeddings)
                stop = true;
            return;
        }
        const std::uint32_t v = order[depth];
        const auto& candidates =
            anchor[depth] == kNone ? g.dataQubits() : g.neighbours(image[anchor[depth]]);
        for (std::uint32_t p : candidates) {
            if (++result.states > budget) {
...
Vf2Result vf2Layout(const InteractionGraph& ig, const CouplingGraph& g, std::uint64_t stateBudget,
                    std::uint32_t maxEmbeddings) {
    Search s{ig, g,   stateBudget, std::max<std::uint32_t>(maxEmbeddings, 1), {}, {}, {}, {},
             {}, 0.0, false};
    if (ig.qubits > g.dataQubits().size())
        return s.result;
    ...
    for (std::uint32_t v = 0; v < ig.qubits; ++v)
        if (ig.degree(v) > 0)
            want.push_back(ig.degree(v));
    for (std::uint32_t p : g.dataQubits())
        have.push_back(g.degree(p));
    std::sort(want.rbegin(), want.rend());
    std::sort(have.rbegin(), have.rend());
    if (ig.edges.size() > g.edges().size() || want.size() > have.size())
        return s.result;
    ...
    std::stable_sort(seeds.begin(), seeds.end(),
                     [&](std::uint32_t a, std::uint32_t b) { return ig.degree(a) > ig.degree(b); });
    ...
    for (std::uint32_t seed : seeds) {
        ...
        while (!queue.empty()) {
            ...
            queue.pop_front();
            auto nb = ig.adj[u];
            std::stable_sort(nb.begin(), nb.end(), [&](const auto& x, const auto& y) {
                return ig.degree(x.first) > ig.degree(y.first);
            });
