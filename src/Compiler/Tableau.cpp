...
num::Matrix localPauli(std::size_t k, std::size_t a, std::size_t b) {
    ...
    for (std::size_t s = 0; s < dim; ++s)
        m(s ^ a, s) = (std::popcount(b & s) & 1) ? -1.0 : 1.0;
    return m;
}
} // namespace
...
CliffordTableau::CliffordTableau(std::uint32_t qubits)
    : n_(qubits), rows_(2 * static_cast<std::size_t>(qubits)) {
...
const CliffordTableau::GateTable* CliffordTableau::tableFor(const ir::Gate& g) {
    std::string key =
        std::format("{}|{}|{}|{}", g.name, g.adjoint ? 1 : 0, g.targets.size(), g.controls.size());
    for (std::size_t j = 0; j < g.controls.size(); ++j)
        key += g.isNegControl(j) ? 'n' : 'p';
    for (double p : g.params)
        key += std::format("|{:.17g}", p);
    if (g.custom)
        key += std::format("|custom{}", cache_.size()); // an explicit matrix is never shared
    ...
    if (full && k >= 1 && k <= 3 && full->rows == (std::size_t{1} << k)) {
        ...
        for (std::size_t j = 0; j < k && clifford; ++j)
            for (int which = 0; which < 2 && clifford; ++which) { // 0: X_j, 1: Z_j
                const num::Matrix gen = which == 0 ? localPauli(k, std::size_t{1} << j, 0)
                                                   : localPauli(k, 0, std::size_t{1} << j);
                ...
                for (std::size_t a = 0; a < dim && !found; ++a)
                    for (std::size_t b = 0; b < dim && !found; ++b) {
                        num::Complex c{};
                        for (std::size_t s = 0; s < dim; ++s)
                            c += ((std::popcount(b & s) & 1) ? -1.0 : 1.0) * image(s ^ a, s);
                        ...
                        if (std::abs(c - std::polar(1.0, r * std::numbers::pi / 2.0)) > kTol) {
                            clifford = false;
                            break;
                        }
                        (which == 0 ? t.imageX : t.imageZ)
                            .push_back(
                                {static_cast<std::uint8_t>(a), static_cast<std::uint8_t>(b), r});
                        found = true;
                    }
...
bool CliffordTableau::apply(const ir::Gate& g) {
    if (g.opaque)
        return false;
    if (g.width() == 0)
        return true; // gphase: no action on the Pauli group
    ...
    auto multiply = [](LocalPauli a, const LocalPauli& b) {
        a.phase = static_cast<std::uint8_t>(
            (a.phase + b.phase + 2 * std::popcount(static_cast<unsigned>(a.z & b.x))) & 3);
        a.x ^= b.x;
        a.z ^= b.z;
        return a;
    };
    for (PauliString& row : rows_) {
        ...
        for (std::size_t j = 0; j < wires.size(); ++j) {
            ...
            if (row.x[q]) {
                acc = multiply(acc, t->imageX[j]);
                touched = true;
            }
            if (row.z[q]) {
                acc = multiply(acc, t->imageZ[j]);
                touched = true;
            }
        }
        if (!touched)
...
bool applyAll(CliffordTableau& t, const ir::Circuit& c) {
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            ...
            if (!applyAll(t, *box->body))
                return false;
        } else if (!std::holds_alternative<ir::Barrier>(n) &&
