...
std::string pretty(Picoseconds p) {
    ...
    if (ns >= 1e6) return std::format("{:.3f} ms", ns * 1e-6);
    if (ns >= 1e3) return std::format("{:.3f} us", ns * 1e-3);
    return std::format("{:.3f} ns", ns);
}

// Lengths of the nodes of one level; bodies are scheduled recursively on their own time axis.
Result<std::vector<Picoseconds>> lengthsOf(std::vector<ir::Node>& nodes, Durations& d, const ScheduleOptions& o);
...
Result<ScheduleInfo> scheduleLevel(ir::Circuit& c, Durations& d, const ScheduleOptions& o) {
    ...
    struct Spans {
        ...
        std::span<const std::uint32_t> operator[](std::size_t i) const { return {items.data() + offset[i], offset[i + 1] - offset[i]}; }
    } wires, bits;
    for (std::size_t i = 0; i < n; ++i) {
        if (const auto* g = std::get_if<ir::Gate>(&nodes[i])) {   // the common case, without temporaries
            for (ir::Wire w : g->targets) if (w.index < c.qubitCount()) wires.items.push_back(w.index);
            for (ir::Wire w : g->controls) if (w.index < c.qubitCount()) wires.items.push_back(w.index);
        } else {
            for (ir::Wire w : ir::nodeWiresIn(nodes[i], c.qubitCount()))
                if (w.index < c.qubitCount()) wires.items.push_back(w.index);
            for (auto b : ir::nodeWrites(nodes[i])) bits.items.push_back(b.index);
            for (auto b : ir::nodeReads(nodes[i])) bits.items.push_back(b.index);
        }
    ...
    for (std::size_t k = 0; k < n; ++k) {
        ...
        std::int64_t t = 0;
        for (std::uint32_t w : wires[i]) t = std::max(t, wireFree[w]);
        for (std::uint32_t b : bits[i]) if (b < bitFree.size()) t = std::max(t, bitFree[b]);
        begin[i] = t;
        const std::int64_t end = t + (*lengths)[i].get();
        for (std::uint32_t w : wires[i]) wireFree[w] = end;
        for (std::uint32_t b : bits[i]) if (b < bitFree.size()) bitFree[b] = end;
        total = std::max(total, end);
    }
    for (std::size_t i = 0; i < n; ++i) {
        const std::int64_t len = (*lengths)[i].get();
        info.nodes[i] = TimedNode{Picoseconds{alap ? total - begin[i] - len : begin[i]}, Picoseconds{len}};
    }
    ...
    for (std::size_t i = 0; i < n; ++i)
        if (ir::isQuantum(nodes[i]) || isControlNode(nodes[i]))
            for (ir::Wire w : ir::nodeWires(nodes[i])) if (w.index < carries.size()) carries[w.index] = 1;
    for (std::size_t i = 0; i < n; ++i)   // same-wire nodes are time ordered in topological order
        for (std::uint32_t w : wires[i]) {
            if (!carries[w]) continue;
            const std::int64_t s = info.nodes[i].start.get();
            if (lastEnd[w] >= 0 && s > lastEnd[w]) info.idle[w].push_back({Picoseconds{lastEnd[w]}, Picoseconds{s}});
            lastEnd[w] = info.nodes[i].end().get();
        }

    core::Json starts = core::Json::array(), lens = core::Json::array(), idle = core::Json::object();
    for (const TimedNode& t : info.nodes) { starts.push_back(t.start.get()); lens.push_back(t.duration.get()); }
    for (const auto& [w, gaps] : info.idle) {
        core::Json list = core::Json::array();
        for (const IdleInterval& g : gaps) list.push_back({g.start.get(), g.end.get()});
        idle[std::format("{}", w)] = std::move(list);
    }
    setNodes(c, std::move(nodes));
    c.meta()["schedule"] = {{"policy", std::string(schedulePolicyName(o.policy))}, {"dt_ps", d.dt.get()}, {"granule_ps", d.granule.get()},
                            {"duration_ps", total}, {"start_ps", std::move(starts)}, {"length_ps", std::move(lens)}, {"idle", std::move(idle)}};
    return info;
}
...
Result<std::vector<Picoseconds>> lengthsOf(std::vector<ir::Node>& nodes, Durations& d, const ScheduleOptions& o) {
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        ...
        if (auto* g = std::get_if<ir::Gate>(&n)) {
            ...
            dl->duration = ir::Duration{out[i], 0};   // `dt` resolved, on the grid (export makes it explicit)
        } else if (auto* br = std::get_if<ir::Branch>(&n)) {
            QXL_TRY_ASSIGN(const Picoseconds a, bodyLength(*br->thenBody));
            Picoseconds b{0};
            if (br->elseBody.present()) { QXL_TRY_ASSIGN(b, bodyLength(*br->elseBody)); }
            ...
            if (bx->duration) {
                ...
                if (a > declared)
                    return fail(Error(err::BoxOverrun, std::format("the box body lasts {} but the box declares {}", pretty(a), pretty(declared))).withSpan(bx->span));
                out[i] = declared;
...
Picoseconds ScheduleInfo::totalIdle() const {
    std::int64_t sum = 0;
    for (const auto& [w, gaps] : idle) { (void)w; for (const IdleInterval& g : gaps) sum += g.length().get(); }
    return Picoseconds{sum};
}

Result<ScheduleInfo> schedule(ir::Circuit& c, const hw::Device& device, const hw::Calibration& calibration,
                              const ScheduleOptions& options, const PulseSource* pulses) {
    Durations d(device, calibration, pulses);
    QXL_TRY_ASSIGN(ScheduleInfo info, scheduleLevel(c, d, options));
    const Picoseconds limit{static_cast<std::int64_t>(device.control.maxProgramDuration.si() * 1e12)};
    if (options.enforceMaxDuration && limit.get() > 0 && info.duration > limit)
        return fail(lang::Diagnostics::make("QL4090", SourceSpan{}, pretty(info.duration), pretty(limit)).error);
    return info;
}
...
Status requireResolvedDurations(const ir::Circuit& c) {
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if ((dl && dl->duration.symbolic()) || (bx && bx->duration && bx->duration->symbolic())) {
            ...
            d.error.withNote("help: select a device ('pragma qlab.device <id>') or write the duration in ns, us or ms");
            return fail(std::move(d.error));
        }
        Status st;
        forEachBody(n, [&](const ir::Circuit& body) { if (st) st = requireResolvedDurations(body); });
        QXL_TRY(st);
    }
    return {};
}
