...
struct EquivalenceOptions {
    ...
    std::uint32_t maxStateQubits = 24;       // wires simulated by RandomStates (2^24 amplitudes = 256 MiB)
...
struct EquivalenceReport {
    ...
    std::uint32_t inputs = 0;                // basis states, random states × branches, or Pauli generators compared
...
Result<EquivalenceReport> checkEquivalence(const ir::Circuit& reference, const ir::Circuit& compiled,
