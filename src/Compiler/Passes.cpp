// Spec 14 §1.1 — the passes of the standard pipeline as `IPass` adaptors over the module's
// functions.
...
Result<const CouplingGraph*> PassContext::graph() {
    if (!options.device)
        return fail(err::NoDevice, "this pass needs a device");
    if (!coupling) {
...
struct Verify final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        QXL_TRY(ir::verify(c));
        if (!ctx.options.device)
            QXL_TRY(requireResolvedDurations(c)); // QL4010
...
struct Optimize final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        ...
        if (ctx.options.kak ||
            ctx.options.level == OptimizeLevel::O2) { // §5.5: level 2 adds two-qubit resynthesis
            QXL_TRY_ASSIGN(const KakStats k, resynthesizeTwoQubitBlocks(c, ctx.target, ctx.stop));
            if (k.replaced > 0)
                QXL_TRY(optimize(c, ctx.target, OptimizeOptions{},
                                 ctx.stop)); // fuse the new local gates
        }
        return ctx.metrics(c);
    }
};
...
struct LayoutSelect final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        ...
        QXL_TRY_ASSIGN(LayoutChoice choice,
                       chooseLayout(c, *g, ctx.options.layout, ctx.options.seed,
                                    ctx.options.vf2StateBudget, &ctx.diagnostics));
...
struct Route final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        ...
        Result<RoutingResult> routed = [&]() -> Result<RoutingResult> {
            if (c.isPhysical())
                return adoptPhysical(c, *g); // `qlab.layout physical`: validate only
            if (ctx.options.routing == RoutingPolicy::None)
                return applyLayout(c, ctx.initialLayout, *g);
            RouteOptions o;
            o.seed = ctx.options.seed;
            o.trials =
                ctx.options.level == OptimizeLevel::O2 ? 4 : 1; // spec 14 §8: level 2 tries 4 seeds
...
struct ScheduleNodes final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        ...
        if (!o.device || !o.calibration)
            return fail(err::NoDevice, "scheduling needs a device and its calibration");
        const bool calibrated =
            c.meta().contains("calibrations") && !c.meta()["calibrations"].empty();
        if (o.pulseLevel && !o.pulses)
            return fail(err::NoDevice,
                        "a pulse-level compile needs the device's pulse library (pulses.json)");
        if (o.pulseLevel || calibrated) { // block durations come from the pulses themselves
            QXL_TRY_ASSIGN(PulseSource source,
                           PulseSource::create(*o.device, o.pulseLevel ? o.pulses : nullptr, c));
            ctx.pulseSource = std::move(source);
        }
        ScheduleOptions so;
        so.policy = o.schedule;
        QXL_TRY_ASSIGN(ctx.schedule, schedule(c, *o.device, *o.calibration, so,
                                              ctx.pulseSource ? &*ctx.pulseSource : nullptr));
        return ctx.unchanged();
    }
};
...
struct PulseLower final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        const CompileContext& o = ctx.options;
        if (!ctx.pulseSource || !o.device)
            return fail(err::NoDevice,
                        "pulse lowering runs after Schedule on a device with a pulse library");
        if (o.enforcePulseQubitCap) { // spec 15 §2: the Lindblad backend takes 5 transmons or 6
                                      // ions
            const std::uint32_t cap =
                o.pulseQubitCap ? o.pulseQubitCap
                                : (o.device->technology == hw::Technology::IonChain ? 6u : 5u);
            if (usedWires(c).size() > cap)
                return fail(lang::Diagnostics::make("QL4040", SourceSpan{}, cap).error);
        }
        QXL_TRY_ASSIGN(PulseProgram program, lowerToPulses(c, ctx.schedule, *ctx.pulseSource));
        ctx.pulseProgram =
            std::move(program); // non-fatal verifier findings (W_DEAD_PHASE) travel inside it
        return ctx.unchanged();
    }
};
...
struct Equivalence final : IPass {
    ...
    Result<PassMetrics> run(ir::Circuit& c, PassContext& ctx) override {
        if (!ctx.options.verifyEquivalence || !ctx.reference)
            return ctx.unchanged();
        EquivalenceReport skipped;
        if (ctx.reference->qubitCount() >
            10) { // spec 14 §1.1: automatic for n ≤ 10, on demand above
        ...
        if (!report.equivalent && report.method != EquivalenceMethod::Skipped)
            return fail(
                err::NotEquivalent,
                std::format(
                    "internal error: the compiled circuit is not equivalent to the source ({}: {})",
