...
bool isNativeName(const ir::Gate& g, Basis1q basis) {
    if (g.adjoint || g.custom) return false;
    switch (basis) {
    case Basis1q::U: return g.name == "U";
    case Basis1q::ZSX: return g.name == "rz" || g.name == "sx" || g.name == "x" || g.name == "id";
    case Basis1q::ZYZ: return g.name == "rz" || g.name == "rx" || g.name == "ry";
    }
    return false;
}
...
bool alreadyShortest(const WorkList& w, const std::vector<std::uint32_t>& run, Basis1q basis) {
    ...
    if (basis == Basis1q::U || run.size() > 5) return false;
    auto special = [&](double a, std::initializer_list<double> values) {
        for (double v : values)
            if (std::abs(wrapAngle(a - v)) < 1e-9) return true;
        return false;
    };
    std::size_t pulses = 0;
    ...
    for (std::size_t k = 0; k < run.size(); ++k) {
        ...
        if (g.adjoint || g.custom) return false;
        if (g.name == "rz") {
            if (previousRz || special(g.params[0], {0.0})) return false;
            const bool middle = k > 0 && k + 1 < run.size();
            if (middle && basis == Basis1q::ZSX && special(g.params[0], {pi / 2, -pi / 2, pi})) return false;
            previousRz = true;
            continue;
        }
        if (!previousRz && k > 0) return false;                      // two pulses in a row merge
        previousRz = false;
        ++pulses;
        if (basis == Basis1q::ZSX && g.name != "sx") return false;
        if (basis == Basis1q::ZYZ && (g.name != "ry" || special(g.params[0], {0.0, pi}) || run.size() > 2)) return false;
    }
...
bool fuseRun(WorkList& w, const std::vector<std::uint32_t>& run, Basis1q basis) {
    if (run.size() < 2) return false;   // spec 14 §5.2: shorter runs are left alone
    if (alreadyShortest(w, run, basis)) return false;
    ...
    for (std::uint32_t idx : run) {
        ...
        if (isNativeName(g, basis)) before.pulses += isFrameChange(g.name) ? 0u : 1u;
        else before.pulses += synthesize1q(m->view(), basis).pulses();   // what the gate costs once lowered
    }
    ...
    for (ir::Node& n : replacement) w.after[run.front()].push_back(std::move(n));
    for (std::uint32_t idx : run) w.kill(idx);
    return true;
}
} // namespace
...
bool sweepFuse(WorkList& w, Basis1q basis) {
    ...
    for (std::uint32_t wire = 0; wire < w.wireSeq.size(); ++wire) {
        if (!w.dirty[wire]) continue;   // single-qubit runs depend on their own wire only
        run.clear();
        for (std::uint32_t idx : w.wireSeq[wire]) {
            ...
            if (!spliced && !w.alive[idx]) continue;   // removed earlier in this pass: the run continues
            const ir::Gate* g = w.gateAt(idx);
...
bool sweepCommute(WorkList& w) {
    ...
    for (std::uint32_t i = 0; i < n; ++i) {
        ...
        if (!g || !isFusable(*g) || !w.after[i].empty() || !w.anyDirty(i)) continue;
        const std::uint8_t action = w.actionOf(i, 0);
        if (action != kActsZ && action != kActsX) continue;   // `id` commutes with everything: nothing to gain
