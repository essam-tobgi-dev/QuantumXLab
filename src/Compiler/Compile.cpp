...
CompileContext resolveContext(const lang::Program& program, const CompileOptions& options,
    ...
    if (!options.level)
        c.level = static_cast<OptimizeLevel>(std::clamp(pr.optimize, 0, 2));
    if (!options.layout && pr.layout) {
        switch (*pr.layout) {
        case lang::LayoutChoice::Trivial:
            c.layout = LayoutPolicy::Trivial;
            break;
        case lang::LayoutChoice::Dense:
            c.layout = LayoutPolicy::Dense;
            break;
        case lang::LayoutChoice::Vf2:
            c.layout = LayoutPolicy::Vf2;
            break;
        case lang::LayoutChoice::NoiseAware:
            c.layout = LayoutPolicy::NoiseAware;
            break;
        case lang::LayoutChoice::Physical:
            break; // the circuit itself is physical: layout and routing only validate
        }
    }
    if (!options.routing)
        c.routing =
            pr.routing == lang::RoutingChoice::None ? RoutingPolicy::None : RoutingPolicy::Sabre;
    if (!options.pulseLevel && pr.pulseLevel)
        c.pulseLevel = *pr.pulseLevel;
    if (!options.seed && pr.seed)
        c.seed = *pr.seed;
    if (!device)
        c.pulseLevel = false;
    return c;
...
Result<CompiledProgram> compile(const lang::Program& program, const hw::Device& device,
                                const hw::Calibration& calibration, const CompileOptions& options,
                                std::stop_token stop) {
    ...
    if (ctx.pulseLevel && !ctx.pulses) {
        if (device.directory.empty())
            return fail(err::NoDevice,
                        std::format("device '{}' was not loaded from a directory: pass "
                                    "CompileOptions::pulses for a pulse-level compile",
                                    device.id));
        auto library = pulse::loadPulses(device.directory);
        if (!library)
            return fail(library.error().withNote(
                std::format("while loading pulses.json of device '{}'", device.id)));
        auto resolved = library->withCalibration(
            calibration); // `cal.*` references follow the calibration in use
...
Result<CompiledProgram> compileSource(std::string_view text, std::string filename,
    ...
    Result<CompiledProgram> out =
        device && calibration ? compile(program, *device, *calibration, options, std::move(stop))
...
std::string formatWith(const Error& e, std::string_view severity, std::string_view fix) {
    ...
    if (e.span && e.span->line > 0) // whole-program findings (QL4060, QL4090) carry no location
        s += std::format("{}:{}:{}: ", e.span->file.empty() ? "<program>" : e.span->file,
                         e.span->line, e.span->column);
    s += severity;
    if (!e.diagnosticId.empty())
        s += std::format("[{}]", e.diagnosticId);
    s += ": " + e.message;
    const std::string_view hint =
        e.diagnosticId.empty() ? std::string_view{} : lang::Diagnostics::info(e.diagnosticId).hint;
    for (const std::string& note : e.notes) {
        if (note.starts_with("help: "))
            s += "\n  " + note;
        else if (!hint.empty() && note == hint)
...
std::string formatDiagnostic(const lang::Diagnostic& d) {
    const char* severity = d.severity == lang::Severity::Error     ? "error"
