...
std::string_view layoutPolicyName(LayoutPolicy p) {
    switch (p) {
    case LayoutPolicy::Trivial: return "trivial";
    case LayoutPolicy::Dense: return "dense";
    case LayoutPolicy::Vf2: return "vf2";
    case LayoutPolicy::NoiseAware: return "noise_aware";
    }
    return "?";
}
std::string_view routingPolicyName(RoutingPolicy p) { return p == RoutingPolicy::Sabre ? "sabre" : "none"; }
std::string_view schedulePolicyName(SchedulePolicy p) { return p == SchedulePolicy::Asap ? "asap" : "alap"; }
...
Layout Layout::identity(std::uint32_t n) {
    Layout l;
    l.v2p.resize(n);
    for (std::uint32_t i = 0; i < n; ++i) l.v2p[i] = i;
    return l;
}
std::string Layout::text() const {
    std::string s;
    for (std::size_t v = 0; v < v2p.size(); ++v) s += std::format("{}q{}->${}", v ? ", " : "", v, v2p[v]);
    return s;
}

CompileContext CompileContext::from(const CompileOptions& o, const hw::Device* dev, const hw::Calibration* cal) {
    CompileContext c;
    c.device = dev;
    c.calibration = cal;
    c.pulses = o.pulses;
    if (o.level) c.level = *o.level;
    if (o.layout) c.layout = *o.layout;
    if (o.routing) c.routing = *o.routing;
    if (o.pulseLevel) c.pulseLevel = *o.pulseLevel;
...
ir::Circuit shellLike(const ir::Circuit& c) {
    ...
    for (const auto& r : c.qubitRegisters()) out.addQubitRegister(r);
    for (const auto& r : c.bitRegisters()) out.addBitRegister(r);
    out.meta() = c.meta();
    return out;
}
...
std::vector<ir::Node> takeNodes(ir::Circuit& c) {
    ...
    const std::vector<ir::NodeId> order(topo.begin(), topo.end());   // the span dies with the first mutation
    std::vector<ir::Node> nodes;
    nodes.reserve(order.size());
    for (ir::NodeId id : order) nodes.push_back(std::move(c.node(id)));
    return nodes;
}
...
std::vector<ir::Node> copyNodes(const ir::Circuit& c) {
    ...
    for (ir::NodeId id : topo) nodes.push_back(c.node(id));
    return nodes;
}

void setNodes(ir::Circuit& c, std::vector<ir::Node> nodes) {
    ir::Circuit out = shellLike(c);
    for (auto& n : nodes) out.add(std::move(n));
    c = std::move(out);
}

void normalizeBodies(ir::Circuit& c) {
    bool nested = false;
    for (ir::NodeId id : std::as_const(c).topologicalOrder()) nested = nested || isControlNode(std::as_const(c).node(id));
    if (!nested) return;
...
void count(const ir::Circuit& c, PassMetrics& m) {
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            ++m.gateCount;
            if (g->width() == 2) ++m.twoQubitCount;
            if (g->name == "t" || g->name == "tdg") ++m.tCount;
        } else {
...
void collectWires(const ir::Circuit& c, std::set<std::uint32_t>& out) {
    for (ir::NodeId id : c.topologicalOrder())
        for (ir::Wire w : ir::nodeWires(c.node(id))) out.insert(w.index);
}
} // namespace
...
std::optional<std::vector<std::uint32_t>> metaIndices(const ir::Circuit& c, std::string_view key) {
    ...
    if (it == meta.end() || !it->is_array()) return std::nullopt;
...
double wrapAngle(double a, long* turns) {
    ...
    if (turns) *turns = static_cast<long>(k);
    return a - twoPi * k;
}
