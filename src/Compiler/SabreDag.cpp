...
Dag buildDag(const ir::Circuit& c) {
    ...
    auto link = [&](std::uint32_t from, std::uint32_t to) {
        ...
        if (!succ.empty() && succ.back() == to) return;   // edges of one node are added back to back
        succ.push_back(to);
    ...
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            for (ir::Wire w : g->wires()) d.wires.push_back(w.index);
            d.twoQubit = d.wires.size() == 2;
        } else {
            for (ir::Wire w : ir::nodeWires(n)) d.wires.push_back(w.index);
        }
        ...
        for (ir::Wire w : ir::nodeWiresIn(n, c.qubitCount()))
            if (w.index < lastOnWire.size()) { link(lastOnWire[w.index], index); lastOnWire[w.index] = index; }
        auto bits = ir::nodeWrites(n);
        for (auto b : ir::nodeReads(n)) bits.push_back(b);
        for (auto b : bits)
            if (b.index < lastOnBit.size()) { link(lastOnBit[b.index], index); lastOnBit[b.index] = index; }
        ...
        const bool event = !std::holds_alternative<ir::Gate>(n) && !std::holds_alternative<ir::Barrier>(n) &&
                           !std::holds_alternative<ir::Delay>(n);
        if (event) { link(lastEvent, index); lastEvent = index; }
    }
...
FullLayout FullLayout::from(const Layout& program, const CouplingGraph& g) {
    ...
    f.p2l.assign(n, none);
    for (std::uint32_t l = 0; l < f.l2p.size(); ++l) f.p2l[f.l2p[l]] = l;
