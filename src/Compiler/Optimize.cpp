...
std::uint32_t gateCount(const std::vector<ir::Node>& nodes) {
    ...
    for (const auto& node : nodes)
        if (std::holds_alternative<ir::Gate>(node)) ++n;
    return n;
}

Result<OptimizeStats> optimizeIn(ir::Circuit& c, Basis1q basis, const OptimizeOptions& o, std::stop_token stop) {
    ...
    while (bodies && stats.iterations < o.maxIterations) {
        ...
        auto stage = [&](auto&& sweep) {   // runs a sweep on the wires that are dirty or changed so far
            for (std::size_t q = 0; q < dirty.size(); ++q) w.dirty[q] = dirty[q] | changedWires[q];
            std::fill(w.touched.begin(), w.touched.end(), 0);
            const bool changed = sweep();
            for (std::size_t q = 0; q < dirty.size(); ++q) changedWires[q] |= w.touched[q];
            return changed;
        };
        // In-place rewrites first and to their fixpoint, so that a resynthesis (which hides its wire
        // until the next pass) never consumes a cancellation that was available: cx · x · sx · cx.
        if (o.cancel) {
        ...
        if (o.fuse) stage([&] { return detail::sweepFuse(w, basis); });
        if (o.cancel && o.fuse) {
            rules.resynthesize = true;
            stage([&] { return detail::sweepCancelMerge(w, rules); });
        }
        if (o.commute && o.fuse) stage([&] { return detail::sweepCommute(w); });
        nodes = w.flush();
        dirty = std::move(changedWires);
        if (std::find(dirty.begin(), dirty.end(), 1) == dirty.end()) break;   // a pass changed nothing: fixpoint
    }
    ...
    if (cancelled) return fail(ErrorCode::Cancelled, "compile cancelled");
    return stats;
}
} // namespace

Result<OptimizeStats> optimize(ir::Circuit& c, Basis1q basis, const OptimizeOptions& options, std::stop_token stop) {
...
bool isPlainRz(const ir::Gate& g) {
    return g.name == "rz" && g.controls.empty() && !g.opaque && !g.custom && g.targets.size() == 1 && g.params.size() == 1;
}
...
Result<VirtualZInfo> virtualZ(ir::Circuit& c) {
    ...
    std::vector<std::uint32_t> atNode, onWire;   // parallel arrays: one JSON value each, not one per gate
    ...
    for (ir::NodeId id : view.topologicalOrder()) {
        ...
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            if (isPlainRz(*g)) {
                ...
                auto record = [&](ir::Wire wire) {   // the frame every pulse of this gate is played in (T07 §5)
                    if (wire.index >= info.finalPhase.size() || std::abs(info.finalPhase[wire.index]) <= kAngleEps) return;
                    atNode.push_back(index);
                    onWire.push_back(wire.index);
                    framePhase.push_back(info.finalPhase[wire.index]);
                };
                for (ir::Wire wire : g->targets) record(wire);
                for (ir::Wire wire : g->controls) record(wire);
            }
        }
        ++index;
    }
    ...
    for (std::uint32_t q = 0; q < info.finalPhase.size(); ++q)
        if (std::abs(info.finalPhase[q]) > kAngleEps) finalPhases[std::format("{}", q)] = info.finalPhase[q];
    c.meta()["virtual_z"] = {{"frame_changes", info.frameChanges},
                             {"final_phase", std::move(finalPhases)},
                             {"phases", {{"node", std::move(atNode)}, {"wire", std::move(onWire)}, {"phase", std::move(framePhase)}}}};
    return info;
}
