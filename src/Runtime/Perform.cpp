...
// Spec 15 §1: the seed actually used. An unset option takes the program's `pragma qlab.seed`, else a
// value derived from the program hash — deterministic and recorded, never a wall-clock seed (04 §6).
std::uint64_t resolveSeed(const RunContext& ctx) {
    if (ctx.options.seed) return *ctx.options.seed;
    if (ctx.source && ctx.source->pragmas.seed) return *ctx.source->pragmas.seed;
...
data::FidelityClass classOf(qsim::Kind kind, bool stochastic, data::FidelityClass applied) {
    if (kind == qsim::Kind::Lindblad) return data::weakest(applied, data::FidelityClass::Numerical);
    if (stochastic) return data::weakest(applied, data::FidelityClass::Statistical);
    if (kind == qsim::Kind::DensityMatrix) return data::weakest(applied, data::FidelityClass::Numerical);
    return applied;
}
} // namespace

Result<RunResult> performRun(const RunContext& ctx) {
    if (!ctx.program) return fail(err::NotCompiled, "the run needs a compiled program");
    if (!ctx.device || !ctx.calibration) return fail(err::NoDevice, "the run needs a device and a calibration");
    ...
    QXL_TRY_ASSIGN(const ProgramPlan plan, planProgram(ctx.program->circuit, outputNames(ctx.source)));
    ...
    if (ctx.bus) {
        ...
        ctx.bus->post(RunStarted{handle, id, ctx.device->id, selection.reason, selection.kind, options.shots, in.seed});
    }
    ...
    for (const std::string& id : exec.twirled)  // spec 08 §7.3: the twirl changes the model
        out.diagnostics.push_back(note(std::format("channel '{}' was applied as its Pauli twirl (class Model)", id),
                                       lang::Severity::Warning));
    if (!exec.twirled.empty()) out.backendClass = data::weakest(out.backendClass, data::FidelityClass::Model);
    summarize(out, plan, ctx.device, options.computeExpectations);
    if (!options.keepMemory) out.memory.clear();
    ...
    if (options.computeEstimate) {
        ...
        for (std::uint32_t sim : plan.measuredQubits) est.measuredQubits.push_back(plan.qubits[sim]);
        ...
        if (options.compareDevices && ctx.source) { // spec 15 §9: one row per device in the catalog
            if (auto rows = compareDevices(*ctx.source, ctx.compileOptions, options.shots, ctx.device->id, ctx.stop))
                out.estimate.comparison = std::move(*rows);
            else if (rows.error().code != ErrorCode::Cancelled)
                out.diagnostics.push_back(note("the device comparison is unavailable: " + rows.error().message,
                                               lang::Severity::Warning));
        }
        if (options.accurateFidelity) {
            if (auto sim = simulatedFidelity(ctx, out)) out.estimate.fidelity.simulated = std::move(*sim);
            else out.diagnostics.push_back(note("the simulated fidelity is unavailable: " + sim.error().message,
                                                lang::Severity::Warning));
        }
    }
    return out;
