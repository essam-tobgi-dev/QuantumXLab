...
Expectation fromParity(std::string name, std::span<const ShotRecord> memory, std::span<const std::uint32_t> bits) {
    Expectation e;
    e.observable = std::move(name);
    if (memory.empty()) return e;
    double sum = 0.0;
    for (const ShotRecord& r : memory) {
        int parity = 0;
        for (std::uint32_t b : bits)
            if (b < r.bits.size()) parity ^= r.bits[b] & 1;
        sum += parity ? -1.0 : 1.0;
    }
...
std::optional<double> exactParity(const std::optional<std::vector<double>>& dist, std::span<const std::uint32_t> bits) {
    if (!dist) return std::nullopt;
    double v = 0.0;
    for (std::size_t x = 0; x < dist->size(); ++x) {
        int parity = 0;
        for (std::uint32_t b : bits) parity ^= static_cast<int>((x >> b) & 1);
...
std::vector<Expectation> computeExpectations(const RunResult& r, const ProgramPlan& plan, const hw::Device* device) {
    ...
    for (const MeasuredBit& m : plan.measurements)
        if (m.bit != ir::kNoBit) bitOfQubit[m.physical] = m.bit.index;
    ...
    if (device) {
        for (auto a = bitOfQubit.begin(); a != bitOfQubit.end(); ++a)
            for (auto b = std::next(a); b != bitOfQubit.end(); ++b) {
                if (!device->adjacent(a->first, b->first)) continue;
                const std::uint32_t bits[2] = {a->second, b->second};
                Expectation e = fromParity(std::format("Z{}Z{}", a->first, b->first), r.memory, bits);
                e.exact = exactParity(r.exact, bits);
                out.push_back(std::move(e));
            }
    }
...
double RunResult::probability(const std::string& key) const { return counts.probability(key); }

const Expectation* RunResult::expectation(std::string_view name) const {
    for (const Expectation& e : expectations)
        if (e.observable == name) return &e;
    return nullptr;
}
...
std::vector<std::size_t> SweepResult::shape() const {
    ...
    for (const SweepAxis& a : axes) s.push_back(a.values.size());
    return s;
}
