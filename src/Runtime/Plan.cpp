...
bool cliffordGate(const ir::Gate& g) {
    static const std::set<std::string, std::less<>> kNames{"i",  "id",   "x",  "y",    "z",     "h",  "s",
                                                           "sdg", "sx",  "sxdg", "cx", "cy",   "cz", "swap",
                                                           "ecr", "iswap", "sy", "sydg", "barrier"};
    if (!g.controls.empty()) { // ctrl @ Clifford is not Clifford in general (ccx is not)
        if (g.controls.size() != 1) return false;
        if (g.name != "x" && g.name != "y" && g.name != "z") return false;
    }
    if (kNames.contains(g.name)) return true;
    if (g.params.size() == 1 && (g.name == "rz" || g.name == "rx" || g.name == "ry" || g.name == "p")) {
    ...
    void visit(const ir::Circuit& c, bool nested) {
        for (ir::NodeId id : c.topologicalOrder()) {
            ...
            if (const auto* g = std::get_if<ir::Gate>(&n)) {
                ++plan->gateCount;
                if (!cliffordGate(*g)) plan->cliffordOnly = false;
            } else if (const auto* m = std::get_if<ir::Measure>(&n)) {
                if (nested) plan->terminalMeasurementOnly = false;
                (void)m;
                ...
                plan->terminalMeasurementOnly = false;
                ++plan->branchCount;
                if (b->thenBody.present()) visit(*b->thenBody, true);
                if (b->elseBody.present()) visit(*b->elseBody, true);
            } else if (const auto* l = std::get_if<ir::Loop>(&n)) {
                plan->hasFeedforward = true;
                plan->hasLoop = true;
                plan->terminalMeasurementOnly = false;
                ++plan->branchCount;
                if (l->body.present()) visit(*l->body, true);
            } else if (const auto* bx = std::get_if<ir::Box>(&n)) {
                if (bx->body.present()) visit(*bx->body, true);
            }
        }
    }
};
...
bool lastOnWire(const ir::Circuit& c, ir::Wire w, ir::NodeId at) {
    ...
    for (ir::NodeId id : c.onWire(w)) {
        if (id == at) { seen = true; continue; }
        if (!seen) continue;
        const ir::Node& n = c.node(id);
        if (!std::holds_alternative<ir::Barrier>(n) && !std::holds_alternative<ir::Delay>(n)) return false;
    }
    return true;
}
} // namespace
...
std::vector<std::uint32_t> usedQubits(const ir::Circuit& c) {
    ...
    const auto collect = [&](const ir::Circuit& body, auto&& self) -> void {
        for (ir::NodeId id : body.topologicalOrder()) {
            ...
            if (std::holds_alternative<ir::Barrier>(n) || std::holds_alternative<ir::Delay>(n)) continue;
            for (ir::Wire w : ir::nodeWires(n)) used.insert(w.index);
...
Result<ProgramPlan> planProgram(const ir::Circuit& c, std::span<const std::string> outputs) {
    ...
    if (plan.qubits.empty() && c.qubitCount() > 0) plan.qubits.push_back(0); // a program with no quantum op
    plan.toSim.assign(std::max<std::size_t>(c.qubitCount(), 1), -1);
    for (std::size_t s = 0; s < plan.qubits.size(); ++s) {
        if (plan.qubits[s] >= plan.toSim.size()) plan.toSim.resize(plan.qubits[s] + 1, -1);
    ...
    for (ir::NodeId id : c.topologicalOrder()) {
        ...
        if (s < 0) return fail(err::Unsupported, "measurement on a wire outside the circuit");
        plan.measurements.push_back(MeasuredBit{static_cast<std::uint32_t>(s), m->qubit.index, m->bit});
    ...
    if (plan.hasFeedforward) plan.hasMidCircuitMeasure = plan.hasMidCircuitMeasure || !plan.measurements.empty();
    return plan;
}

Picoseconds circuitCriticalPath(const ir::Circuit& c, const compiler::ScheduleInfo& timing) {
    const auto order = c.topologicalOrder();
    if (timing.nodes.size() != order.size()) return timing.duration;
    ...
    for (std::size_t i = 0; i < order.size(); ++i) {
        if (std::holds_alternative<ir::Measure>(c.node(order[i]))) continue;
...
std::vector<IdleGap> idleGaps(const ir::Circuit& c, const compiler::ScheduleInfo& timing) {
    ...
    const auto order = c.topologicalOrder();
    if (timing.nodes.size() != order.size()) return gaps;
    std::unordered_map<std::uint32_t, std::size_t> index;
    for (std::size_t i = 0; i < order.size(); ++i) index[order[i].get()] = i;
