...
RunOptions resolveOptions(RunOptions options, const lang::Program* program) {
    ...
    if (p.shots && options.shots == kDefault.shots)
        options.shots = static_cast<std::uint32_t>(*p.shots);
    if (!options.seed && p.seed)
        options.seed = *p.seed;
    if (options.noise == kDefault.noise && p.noise != lang::NoiseChoice::Calibrated)
        options.noise =
            p.noise == lang::NoiseChoice::Ideal ? NoiseSource::Ideal : NoiseSource::Custom;
    if (options.noiseFile.empty() && !p.noiseFile.empty())
    ...
    if (options.cadence == kDefault.cadence) {
        if (p.snapshotCadence == "none")
            options.cadence = SnapshotCadence::None;
        else if (p.snapshotCadence == "gate")
            options.cadence = SnapshotCadence::Gate;
        else if (p.snapshotCadence == "barrier")
            options.cadence = SnapshotCadence::Barrier;
        else if (p.snapshotCadence == "end")
            options.cadence = SnapshotCadence::End;
    }
    return options;
}
} // namespace
...
Status checkInputs(const lang::Program& program, const ir::ParamMap& bound,
    ...
    for (const lang::InputVar& v : program.inputs) {
        if (v.defaultValue || bound.contains(v.name))
            continue;
        bool swept = false;
        if (sweep)
            for (const SweepAxis& a : sweep->axes)
                swept = swept || a.input == v.name;
        if (swept)
            continue;
...
Result<std::optional<SweepGrid>> Session::sweepGrid(const lang::Program* program,
...
Result<noise::NoiseModel> Session::noiseFor(const RunOptions& options) const {
    if (!device_)
        return fail(err::NoDevice, "the noise model needs a device");
    if (options.noise == NoiseSource::Custom) {
        auto text = core::readTextFile(options.noiseFile);
        if (!text) {
            auto d = diagnostic("QL5050", SourceSpan{}, options.noiseFile.string(),
                                text.error().message);
            return std::unexpected(error(d));
        }
...
Result<RunResult> Session::runSync(const RunRequest& request) {
    ...
    if (!device_)
        return fail(err::NoDevice, "select a device before running");
    RunOptions options = resolveOptions(request.options, request.source);
    if (options.probes.empty() && request.source)
        options.probes = resolveProbes(*request.source, *request.program);
    QXL_TRY_ASSIGN(const std::optional<SweepGrid> grid, sweepGrid(request.source, options));
    if (request.source)
        QXL_TRY(checkInputs(*request.source, request.compileOptions.inputs, grid));
...
Result<RunHandle> Session::run(CompileHandle h, RunOptions options) {
    ...
    if (options.probes.empty() && source)
        options.probes = resolveProbes(*source, *compileState->output);
    ...
    if (source)
        QXL_TRY(checkInputs(*source, compileState->options.inputs, grid));
    ...
    auto body = [this, raw, compileState, source, grid](std::stop_token worker) {
        ...
        if (raw->options.noise != NoiseSource::Ideal) {
            ...
            if (m)
                ...
                noiseStatus = std::unexpected(m.error());
        }
        if (noiseStatus) {
            detail::RunContext ctx;
            ...
            if (out && grid) {
                ...
                if (sweep)
                    ...
                    out = std::unexpected(sweep.error());
            }
        } else {
        ...
        if (raw->ok) {
            record =
                RunRecord{raw->result->id,      raw->handle,
                          raw->program,         raw->result->programHash,
                          raw->result->device,  std::string(qsim::kindName(raw->result->backend)),
                          raw->options.shots,   raw->result->seed,
                          raw->result->partial, raw->result->wallTime};
        }
        {
            std::lock_guard lk(mu_);
            if (raw->ok)
    ...
    if (jobs_)
        raw->future = jobs_->submit(std::move(body), core::JobPriority::Batch);
    else
        body(std::stop_token{});
    return raw->handle;
}
...
Status Session::wait(RunHandle h) {
    ...
    if (!s)
        return fail(err::UnknownHandle, "no such run");
    if (s->future.valid())
        s->future.wait();
    if (!s->ok)
...
void Session::cancel(RunHandle h) {
    if (RunState* s = findRun(h))
        s->stop.request_stop();
}

const RunResult* Session::result(RunHandle h) const {
    const RunState* s = findRun(h);
    if (!s || !s->finished.load(std::memory_order_acquire) || !s->result)
        return nullptr;
    return &*s->result;
}
