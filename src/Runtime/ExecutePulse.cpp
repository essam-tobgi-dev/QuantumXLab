...
std::size_t fullIndex(std::span<const std::uint32_t> dims, std::uint32_t qubitSites,
                      std::size_t word, std::span<const std::uint32_t> extra) {
...
bool advance(std::span<std::uint32_t> extra, std::span<const std::uint32_t> dims,
             std::uint32_t qubitSites) {
    for (std::size_t k = 0; k < extra.size(); ++k) {
        if (++extra[k] < dims[qubitSites + k])
            return true;
        extra[k] = 0;
    }
    return false;
...
double automaticStep(const qsim::SystemModel& m, double durationS, double sampleS) {
    ...
    for (int k = 0; k <= kProbes; ++k) {
        ...
        for (const qsim::DriveTerm& d : m.drives) {
            ...
            drive += std::abs(omega.real()) * num::norm1(d.inPhase) +
                     std::abs(omega.imag()) * num::norm1(d.quadrature);
        }
    ...
    do {
        for (std::size_t a = 0; a < dim; ++a)
            for (std::size_t b = 0; b < dim; ++b)
                out(a, b) += rho(fullIndex(dims, qubitSites, a, extra),
                                 fullIndex(dims, qubitSites, b, extra));
...
Result<ExecutionOutput> executePulse(const ExecutionInput& in) {
    if (!in.program || !in.plan)
        return fail(err::NotCompiled, "executePulse needs a compiled program and its plan");
    if (in.backend != qsim::Kind::Lindblad)
        return fail(err::Unsupported,
                    std::format("the pulse-level model runs on the Lindblad backend, not on {}",
    ...
    // Record on the AWG grid (spec 07 §5), but never more than `maxSnapshots` × 64 samples: a 200
    // µs ion gate on a 1 ns grid would otherwise record 2·10⁵ density matrices.
    ...
    settings.stepS = options.pulseStepS > 0.0
                         ? options.pulseStepS
    ...
    if (in.device) {
        const double repetitionS =
            std::max(pm.durationS, static_cast<double>(in.device->timing.repetitionDelay.v));
        if (auto p = schedulePower(in.program->pulses->schedule, *in.device, repetitionS))
            powers = std::move(*p);
        else
            out.diagnostics.push_back(
                note("no line power was published to cryo: " + p.error().message,
    ...
    const auto publish = [&](double progress) {
        ...
        ev.powers =
            progress >= 1.0 ? cryo::LinePowers{} : powers; // the lines are quiet once the job ends
    ...
    out.memory.assign(in.shots,
                      ShotRecord{std::vector<std::uint8_t>(plan.layout.bits, 0), {}, false});
        ...
        if (plan.layout.bits > 0 && plan.layout.bits <= 20) {
            ...
            for (std::size_t i = 0; i < observed.size(); ++i) {
                ...
                for (std::size_t k = 0; k < cbits.size(); ++k)
                    if (cbits[k] >= 0 && ((i >> k) & 1))
                        index |= std::size_t{1} << static_cast<std::size_t>(cbits[k]);
        ...
        for (std::uint32_t s = 0; s < in.shots; ++s) {
            ...
            for (std::size_t k = 0; k < cbits.size(); ++k)
                if (cbits[k] >= 0 && static_cast<std::size_t>(cbits[k]) < out.memory[s].bits.size())
                    out.memory[s].bits[static_cast<std::size_t>(cbits[k])] =
                        static_cast<std::uint8_t>((pick >> k) & 1);
        }
    }
