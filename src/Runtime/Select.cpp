...
std::optional<qsim::Kind> pinnedKind(BackendChoice c) {
    switch (c) {
    case BackendChoice::StateVector: return qsim::Kind::StateVector;
    case BackendChoice::DensityMatrix: return qsim::Kind::DensityMatrix;
    case BackendChoice::Stabilizer: return qsim::Kind::Stabilizer;
    case BackendChoice::Lindblad: return qsim::Kind::Lindblad;
...
Result<qsim::Selection> chooseBackend(const BackendRequest& req) {
    if (!req.plan) return fail(err::NoBackend, "backend selection needs a program plan");
    ...
    sr.cliffordOnly = req.plan->cliffordOnly &&
                      (n > kStabilizerThreshold || n > qsim::maxQubitsFor(16, false));
    ...
    if (!selection) {
        if (sr.pinned) return selection; // a pinned backend that cannot run: spec 15 §2, verbatim
        auto d = diagnostic("QL5011", SourceSpan{}, n, qsim::maxQubitsFor(req.hasNoise ? 16 : 16, req.hasNoise));
        d.error.withNote(selection.error().message);
        return std::unexpected(error(d));
    }
    if (sr.pinned) return selection;
    ...
    if (selection->kind == qsim::Kind::DensityMatrix && req.levels == 2 &&
        (req.shots < kDensityMatrixShots || n > kDensityMatrixQubits) && n <= qsim::maxQubitsFor(16, false)) {
        ...
        if (auto alt = qsim::selectBackend(sv)) {
            alt->reason = req.shots < kDensityMatrixShots
                              ? std::format("noise model with {} shots (< {}): per-shot trajectories are cheaper than "
                                            "one density-matrix evolution",
                                            req.shots, kDensityMatrixShots)
                              : std::format("noise model above {} qubits: Monte-Carlo noise trajectories, one per shot",
