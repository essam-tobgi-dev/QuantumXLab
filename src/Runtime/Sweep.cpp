...
// its coordinates, recompiles the program (the structure may depend on the inputs) and runs with the
// base seed offset by the point index, so the whole sweep is reproducible from one seed.
...
void firstBitProbability(const RunResult& r, SweepPoint& point) {
    if (r.marginals.empty()) return;
    const Marginal* first = nullptr;
    for (const Marginal& m : r.marginals)
        if (m.qubit != Marginal::kNoQubit) { first = &m; break; }
    if (!first) first = &r.marginals.front();
    point.p1 = first->p1;
...
Result<SweepResult> performSweep(const RunContext& ctx, const SweepGrid& grid) {
    if (!ctx.source) return fail(err::Unsupported, "a sweep needs the analysed program to rebind its inputs");
    ...
    if (grid.points() == 0) return fail(err::Unsupported, "the sweep grid is empty");

    SweepResult out;
    out.axes = grid.axes;
    out.points.reserve(grid.points());
    const std::uint64_t baseSeed = ctx.options.seed.value_or(ctx.program ? ctx.program->programHash : 0);
    for (std::size_t p = 0; p < grid.points(); ++p) {
        if (ctx.stop.stop_requested()) break;
        const std::vector<double> coords = grid.at(p);
        compiler::CompileOptions options = ctx.compileOptions;
        for (std::size_t a = 0; a < grid.axes.size(); ++a) options.inputs[grid.axes[a].input] = coords[a];
        QXL_TRY_ASSIGN(compiler::CompiledProgram compiled,
        ...
        for (const RegisterInfo& reg : r.layout.registers) {
            if (!reg.output) continue;
            double sum = 0.0;
            std::size_t n = 0;
            for (const ShotRecord& s : r.memory)
                if (k < s.outputs.size()) { sum += s.outputs[k]; ++n; }
