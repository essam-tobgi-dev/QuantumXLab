...
bool cancelled(const Error& e) { return e.code == ErrorCode::Cancelled; }
} // namespace

Result<ExecutionOutput> execute(const ExecutionInput& in) {
    if (!in.program || !in.plan) return fail(err::NotCompiled, "execute needs a compiled program and its plan");
    if (in.backend == qsim::Kind::Lindblad) return executePulse(in); // spec 15 §3.5 (c)
    if (in.backend == qsim::Kind::Trajectories)
        return fail(err::Unsupported, "the Trajectories backend is not a run backend (spec 15 §2 selects lindblad)");
    ...
    if (!perShot) {
        ...
        ctx.rng = &rng;
        engine.setProjectMeasurements(false); // the N shots are sampled from the final Born distribution
        QXL_TRY(engine.prepare(ctx));
        if (auto st = engine.runCircuit(in.program->circuit, ctx, true); !st) {
            if (!cancelled(st.error())) return std::unexpected(st.error());
            out.partial = true;
        ...
        for (ShotRecord& r : out.memory) r.outputs = outputsOf(plan.layout, r.bits);
        out.shotsCompleted = in.shots;
        if (in.progress) in.progress(in.shots, in.shots);
    } else {
        ...
        for (std::uint32_t s = 0; s < in.shots; ++s) {
            ...
            if (s == 0) out.finalState = engine.capture();
            ShotRecord rec;
            ...
            if (rec.truncated && out.diagnostics.empty())
                out.diagnostics.push_back(diagnostic("QL5020", SourceSpan{}, options.maxLoopIterations));
            out.memory.push_back(std::move(rec));
            ++out.shotsCompleted;
            if (in.progress && (s % 64 == 0 || s + 1 == in.shots)) in.progress(out.shotsCompleted, in.shots);
        }
        out.branchesPerShot = out.shotsCompleted ? static_cast<double>(branches) / static_cast<double>(out.shotsCompleted) : 0.0;
    }
    if (out.partial) out.diagnostics.push_back(diagnostic("QL5060", SourceSpan{}));
    out.snapshots = std::move(engine.snapshots());
    out.cls = data::weakest(engine.report().cls, in.stochasticUnravelling ? data::FidelityClass::Statistical
                                                                          : data::FidelityClass::Exact);
    if (in.backend == qsim::Kind::DensityMatrix) out.exactClass = data::FidelityClass::Exact;
...
void summarize(RunResult& out, const ProgramPlan& plan, const hw::Device* device, bool expectations) {
    ...
    for (const ShotRecord& r : out.memory) out.counts.add(layout.key(r.bits));
    ...
    for (const MeasuredBit& m : plan.measurements)
        if (m.bit != ir::kNoBit) qubitOfBit[m.bit.index] = m.physical;
    out.marginals.clear();
    for (const RegisterInfo& reg : layout.registers) {
        for (std::uint32_t i = 0; i < reg.size; ++i) {
            ...
            for (const ShotRecord& r : out.memory)
                if (flat < r.bits.size() && r.bits[flat]) ++ones;
            Marginal m;
            ...
            m.stderr_ = n ? std::sqrt(std::max(0.0, m.p1 * (1.0 - m.p1)) / static_cast<double>(n)) : 0.0;
