...
Engine::Engine(const ExecutionInput& in, const ProgramPlan& plan)
    ...
    for (std::uint32_t l = 0; l < layers.size(); ++l)
        for (ir::NodeId id : layers[l])
            layerByNode[id.get()] = l;
    for (std::size_t i = 0; i < order.size(); ++i)
        if (auto it = layerByNode.find(order[i].get()); it != layerByNode.end())
            layerOf_[i] = it->second;
...
std::size_t Engine::stateBytes() const {
    ...
    case qsim::Kind::Lindblad:
        return static_cast<std::size_t>(std::min(1e18, 16.0 * dim * dim));
    case qsim::Kind::Stabilizer:
        return static_cast<std::size_t>(std::max(64.0, n * n / 4.0));
    default:
        return static_cast<std::size_t>(std::min(1e18, 16.0 * dim));
    }
}
...
Status Engine::allocate() {
    ...
    if (!backend_)
        return fail(err::NoBackend, "no such backend");
...
Status Engine::applyChannels(std::span<const noise::AttachedChannel> channels, ShotContext& ctx) {
    if (channels.empty())
        return {};
    apply_.shotDetuningHz =
        perShotDrift_ ? std::span<const double>(detuning_) : std::span<const double>{};
...
Status Engine::prepare(ShotContext& ctx) {
    ...
    for (std::uint32_t q = 0; q < plan_.nQubits(); ++q)
        all.push_back(QubitIndex{q});
    ...
    if (perShotDrift_) {
        ...
        for (std::size_t s = 0; s < plan_.qubits.size(); ++s)
            if (plan_.qubits[s] < perDevice.size())
                detuning_[s] = perDevice[plan_.qubits[s]];
    }
    std::vector<QubitIndex> phys;
    for (std::uint32_t q : plan_.qubits)
        phys.push_back(QubitIndex{q});
...
Result<const noise::ReadoutModel*> Engine::readoutFor(std::uint32_t simQubit) {
    if (auto it = readout1q_.find(simQubit); it != readout1q_.end())
        return &it->second;
...
Result<noise::ReadoutModel> terminalReadoutFor(const ExecutionInput& in, const ProgramPlan& plan) {
    ...
    if (!in.noise)
        return noise::ReadoutModel::ideal(sim);
...
Result<noise::ReadoutModel> Engine::terminalReadout() const {
    return terminalReadoutFor(in_, plan_);
}
...
void writeMeasuredBits(const std::string& key, std::span<const std::int64_t> cbits,
    ...
    for (std::size_t k = 0; k < n && k < key.size(); ++k) {
        ...
        if (bit < 0 || static_cast<std::size_t>(bit) >= bits.size())
            continue;
        bits[static_cast<std::size_t>(bit)] = key[k] == '1' ? 1 : 0;
    }
}

Status Engine::sampleTerminal(std::uint32_t shots, core::Random& rng,
                              std::vector<ShotRecord>& memory,
                              std::optional<std::vector<double>>& exact) {
    if (plan_.measuredQubits.empty()) {
        memory.assign(shots,
                      ShotRecord{std::vector<std::uint8_t>(plan_.layout.bits, 0), {}, false});
        return {};
    }
    ...
    QXL_TRY_ASSIGN(const qsim::Counts counts,
                   noise::sampleWithReadout(*backend_, readout, shots, rng));
