...
void operandQubits(std::string_view text, const lang::Program& program,
                   const compiler::CompiledProgram& compiled, std::vector<std::uint32_t>& out) {
    ...
    const auto physical = [&](std::uint32_t virtualQubit) {
        ...
        if (virtualQubit < compiled.initialLayout.size())
            return compiled.initialLayout.physical(virtualQubit);
        return virtualQubit;
    };
    ...
    if (text.empty())
        return;
    if (text.front() == '$') {
        if (auto q = number(text.substr(1)))
            out.push_back(*q);
        return;
    }
    ...
    if (open == std::string_view::npos) { // a whole register
        for (const lang::QubitRegInfo& r : program.qubitRegs) {
            ...
            for (std::size_t i = 0; i < r.size; ++i)
                if (auto v = program.qubitIndex(r.name, i))
                    out.push_back(physical(*v));
        }
        return;
    }
    if (text.back() != ']')
    ...
    if (!element)
        return;
    if (auto v = program.qubitIndex(text.substr(0, open), *element))
        out.push_back(physical(*v));
}
...
std::vector<QubitIndex> simQubits(const ProbeRequest& probe, const ProgramPlan& plan) {
    ...
    if (probe.qubits.empty()) {
        for (std::uint32_t s = 0; s < plan.nQubits(); ++s)
            sim.push_back(QubitIndex{s});
        return sim;
    }
    for (std::uint32_t q : probe.qubits) {
        const std::int32_t s = q < plan.toSim.size() ? plan.toSim[q] : -1;
        if (s >= 0)
            sim.push_back(QubitIndex{static_cast<std::uint32_t>(s)});
    }
    return sim;
}
...
ProbeValue header(const ProbeRequest& probe, std::span<const QubitIndex> sim) {
    ...
    for (QubitIndex q : sim)
        v.qubits.push_back(q.get());
    v.cls = data::FidelityClass::Exact;
    return v;
}
} // namespace

std::vector<ProbeRequest> resolveProbes(const lang::Program& program,
    ...
    for (const lang::Probe& p : program.pragmas.probes) {
        ...
        for (const std::string& operand : p.qubits)
            operandQubits(operand, program, compiled, r.qubits);
...
std::vector<ProbeValue> evaluateProbes(const num::Matrix& rho, std::uint32_t nQubits,
