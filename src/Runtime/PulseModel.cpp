...
bool lineCovers(std::string_view lineChannel, std::string_view channel) {
    ...
    if (open == std::string_view::npos || dots == std::string_view::npos ||
        lineChannel.back() != ']')
        return false;
    if (channel.compare(0, open, lineChannel, 0, open) != 0 || channel.size() <= open ||
        channel[open] != '[')
        return false;
    const auto number = [](std::string_view s) -> std::optional<std::uint32_t> {
        std::uint32_t v = 0;
        const auto r = std::from_chars(s.data(), s.data() + s.size(), v);
        return r.ec == std::errc{} && r.ptr == s.data() + s.size() ? std::optional{v}
                                                                   : std::nullopt;
    };
...
Result<PulseModel> buildPulseModel(const ExecutionInput& in, const ProgramPlan& plan) {
    if (!in.program || !in.program->pulses)
        return fail(err::Unsupported,
                    "a pulse-level run needs a compiled program with a pulse schedule");
    if (!in.device || !in.calibration)
        return fail(err::NoDevice, "a pulse-level run needs a device and a calibration");
    ...
    QXL_TRY_ASSIGN(hw::SystemModelSpec spec,
                   hw::buildSystemModel(*in.device, *in.calibration, opt));
    ...
    for (const hw::CollapseSpec& c : spec.collapse)
        out.model.collapse.push_back({c.name, c.op.toDense()});
    ...
    for (const hw::DriveSpec& d : spec.drives) {
        ...
        if (!e)
            continue; // no Play on this channel: the term is identically zero
        out.model.drives.push_back(
            qsim::DriveTerm{d.channel, d.inPhase.toDense(), d.quadrature.toDense(), e->envelope()});
        out.channels.push_back(d.channel);
    }
    if (out.model.drives.empty() && !drives.drives.empty()) {
        std::string names;
        for (const pulse::DriveEnvelope& e : drives.drives)
            names += (names.empty() ? "" : ", ") + e.channel;
        return fail(
            err::Unsupported,
            std::format("the device model has no Hamiltonian term for the driven channels ({})",
                        names));
    }
    out.model.durationS = drives.durationS;
    for (const pulse::ScheduleEvent& ev : drives.events)
        if (ev.kind == pulse::ScheduleEvent::Kind::Acquire)
            out.acquireEnd = std::max(out.acquireEnd, ev.t0S + ev.durationS);
    return out;
}

Result<cryo::LinePowers> schedulePower(const pulse::Schedule& schedule, const hw::Device& device,
                                       double repetitionS) {
    cryo::LinePowers powers;
    if (schedule.empty() || repetitionS <= 0.0)
        return powers;
    const double dtS = static_cast<double>(schedule.dt().get()) * 1e-12;
    if (dtS <= 0.0)
        return fail(err::Unsupported, "the schedule has no sample period");
    QXL_TRY_ASSIGN(const pulse::SampledSchedule sampled,
                   pulse::sampleSchedule(schedule, 1.0 / dtS, device.timing.granularitySamples));
    ...
    for (const pulse::ChannelSamples& ch : sampled.channels) {
        ...
        if (energy > 0.0)
            byChannel[ch.channel.toString()] += energy / repetitionS;
    }
    ...
    if (!device.directory.empty() && std::filesystem::exists(wiringPath)) {
        ...
        for (const auto& [channel, watts] : byChannel) {
            ...
            for (const cryo::WiringLine& line : wiring.lines)
                if (lineCovers(line.channel, channel)) {
                    hit = &line;
                    break;
                }
