...
Session::~Session() {
    // Every worker holds a raw pointer into `compiles_`/`runs_`: stop them and join before the maps go.
    std::vector<std::future<void>*> pending;
    {
        std::lock_guard lk(mu_);
        for (auto& [key, c] : compiles_) {
            c->stop.request_stop();
            if (c->future.valid()) pending.push_back(&c->future);
        }
        for (auto& [key, r] : runs_) {
            r->stop.request_stop();
            if (r->future.valid()) pending.push_back(&r->future);
        }
    }
...
void Session::setDevice(hw::LoadedDevice loaded) {
    ...
    if (bus_) bus_->post(DeviceSelected{device_->device.id, device_->calibration.timestamp});
}

void Session::selectBackend(BackendChoice choice) {
    backend_ = choice;
    if (bus_) bus_->post(BackendSelected{choice});
}
...
Result<ProgramId> Session::loadProgram(std::string source, std::filesystem::path origin) {
    ...
    if (!e->program.ok()) {
        ...
        Error err = errors.empty() ? Error(ErrorCode::Parse, "the program has errors") : errors.front();
        for (std::size_t i = 1; i < errors.size(); ++i) err.notes.push_back(errors[i].message);
    ...
    if (bus_) bus_->post(ProgramLoaded{ProgramId{id}, originText});
    return ProgramId{id};
}
...
Result<CompileHandle> Session::compile(ProgramId id, compiler::CompileOptions options) {
    ...
    if (!prog) return fail(err::UnknownHandle, "no such program");
    if (!device_) return fail(err::NoDevice, "select a device before compiling");
    ...
    if (prog->program.pragmas.device && *prog->program.pragmas.device != device_->device.id)
        return fail(err::NoDevice, std::format("the program targets device '{}', the session holds '{}'",
    ...
    if (bus_) bus_->post(CompileStarted{id, raw->handle});
    ...
    auto body = [raw, prog, &dev, &cal, bus = bus_](std::stop_token worker) {
        ...
        for (const lang::Diagnostic& d : raw->diagnostics) (d.isError() ? errors : warnings) += 1;
        ...
        if (bus) bus->post(CompileFinished{raw->program, raw->handle, raw->ok, errors, warnings, ms});
    };
    if (jobs_) raw->future = jobs_->submit(std::move(body), core::JobPriority::Interactive);
...
Status Session::waitCompile(CompileHandle h) {
    ...
    if (!s) return fail(err::UnknownHandle, "no such compile");
...
void Session::cancelCompile(CompileHandle h) {
    if (CompileState* s = findCompile(h)) s->stop.request_stop();
}

const compiler::CompiledProgram* Session::compiled(CompileHandle h) const {
    const CompileState* s = findCompile(h);
    if (!s || !s->finished.load(std::memory_order_acquire) || !s->output) return nullptr;
    return &*s->output;
}

std::vector<lang::Diagnostic> Session::compileDiagnostics(CompileHandle h) const {
    const CompileState* s = findCompile(h);
    return s && s->finished.load(std::memory_order_acquire) ? s->diagnostics : std::vector<lang::Diagnostic>{};
}
