...
void gateInstances(const ir::Circuit& c,
...
struct Ends {
    double typ = 1.0, low = 1.0, high = 1.0;
};
...
Result<FidelityEstimate> estimateFidelityFast(const EstimateInput& in) {
    if (!in.device || !in.calibration || !in.program)
        return fail(err::Unsupported,
                    "the fidelity estimate needs a device, a calibration and a compiled program");
    ...
    for (const auto& [name, qubits] : instances) {
        if (name == "rz" || name == "id" || name == "i" || name == "barrier")
            continue; // virtual / free
        auto e = cal.gateError(name, qubits);
        if (!e)
            continue;
        double sigma = 0.0;
        if (qubits.size() == 1) {
            if (const hw::QubitCal* q = cal.qubit(qubits[0]))
                sigma = q->gateError1q.sigma;
        } else if (qubits.size() == 2) {
            if (const hw::EdgeCal* edge = cal.edge(qubits[0], qubits[1]))
                sigma = edge->gateError2q.sigma;
        }
        multiply(gates, *e, sigma);
        if (*e < 1.0)
            varLog += (sigma / (1.0 - *e)) * (sigma / (1.0 - *e)); // T12 §7 log-domain band
    }
    ...
    f.caveats = {
        "errors are treated as independent and multiplicative: no coherent build-up, no crosstalk",
        "gate errors are average (RB) fidelities, so state-dependent errors are ignored",
        "the idle factor assumes an equal superposition: worst case for dephasing, average for "
        "relaxation",
...
double hostGateCost() {
    ...
    static const double cost = [] {
        ...
        do {
            for (std::size_t i = 0; i + 1 < kN;
                 i += 2) { // one two-amplitude butterfly, as a 1q kernel does
        ...
        return v[0].real() != 0.0 || v[0].real() == 0.0 ? elapsed / static_cast<double>(updates)
                                                        : 0.0;
    }();
    return cost;
}

ClassicalCost classicalCost(std::uint32_t qubits, std::uint64_t gates, double perShotS,
                            std::uint64_t shots) {
    ClassicalCost c;
    ...
    if (gates > 0 && shots > 0 && perShotS > 0.0) {
        const double ratio =
            static_cast<double>(shots) * perShotS / (c.gateCostS * static_cast<double>(gates));
        if (ratio > 0.0)
            c.crossoverQubits = std::log2(ratio);
    }
    return c;
}

Result<qec::ResourceEstimate> estimateQec(const EstimateInput& in, const ResourceSummary& res) {
    if (!in.device || !in.calibration)
        return fail(err::Unsupported, "the QEC estimate needs a device");
    ...
    if (n == 0)
        return fail(err::Unsupported, "the calibration has no two-qubit gate errors");
...
Result<Estimate> estimate(const EstimateInput& in) {
    if (!in.device || !in.calibration || !in.program)
        return fail(err::Unsupported,
                    "an estimate needs a device, a calibration and a compiled program");
    Estimate e;
    ...
    e.classicalCost = classicalCost(e.resources.qubits, in.program->metrics.gateCount,
                                    e.wallTime.perShotS, in.shots);

    e.assumptions = {"queue_time_excluded",
                     std::format("reset_policy:{}",
                                 e.wallTime.resetPolicy == hw::ResetPolicy::Active    ? "active"
