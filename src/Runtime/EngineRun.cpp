...
Status Engine::applyIdle(std::size_t nodeIndex, ShotContext& ctx) {
    if (!in_.noise) return {};
    while (idleCursor_ < idle_.size() && idle_[idleCursor_].nodeIndex < nodeIndex) ++idleCursor_;
...
Status Engine::applyGate(const ir::Gate& g, ShotContext& ctx) {
    ...
    if (in_.noise) {
        ...
        for (ir::Wire w : g.wires()) phys.push_back(QubitIndex{w.index});
        for (auto& ac : in_.noise->channelsFor(g.name, phys))
            (ac.placement == noise::Placement::Before ? before : after).push_back(std::move(ac));
    }
    if (!before.empty()) QXL_TRY(applyChannels(remap(std::move(before)), ctx));
    QXL_TRY(backend_->apply(op));
    if (!after.empty()) QXL_TRY(applyChannels(remap(std::move(after)), ctx));
    return {};
}
...
Status Engine::applyMeasure(const ir::Measure& m, ShotContext& ctx) {
    ...
    if (s < 0) return fail(err::Unsupported, "measurement on an unused wire");
    ...
    // N shots, so the projective collapse must NOT run here — it would pin every shot to one outcome.
    if (!project_) return {};
    ...
    if (m.bit != ir::kNoBit && m.bit.index < ctx.bits.size()) ctx.bits[m.bit.index] = bit;
    return {};
}
...
void Engine::maybeSnapshot(std::size_t nodeIndex, const ir::Node& n, ShotContext& ctx) {
    if (!collect_ || cadence_ == SnapshotCadence::None || cadence_ == SnapshotCadence::End) return;
    if (snapshots_.size() >= options_.maxSnapshots) return;
    bool boundary = false;
    switch (cadence_) {
    case SnapshotCadence::Gate: boundary = ir::isQuantum(n); break;
    case SnapshotCadence::Layer: boundary = ir::isQuantum(n) && layerOf_[nodeIndex] != lastLayer_; break;
    case SnapshotCadence::Barrier: boundary = std::holds_alternative<ir::Barrier>(n); break;
    default: break;
    }
    ...
    snap.timeS = nodeIndex < timing.nodes.size() ? static_cast<double>(timing.nodes[nodeIndex].end().get()) * 1e-12 : 0.0;
...
Status Engine::runCircuit(const ir::Circuit& c, ShotContext& ctx, bool topLevel) {
    ...
    for (std::size_t i = 0; i < order.size(); ++i) {
        if (in_.stop.stop_requested()) return fail(ErrorCode::Cancelled, "run cancelled");
        const ir::Node& n = c.node(order[i]);
        if (topLevel) QXL_TRY(applyIdle(i, ctx));
        if (const auto* g = std::get_if<ir::Gate>(&n)) {
            ...
            if (in_.noise && d->duration.ps.get() > 0) {
                ...
                for (ir::Wire w : wires) {
                    ...
                    if (s < 0) continue;
                    QXL_TRY(applyChannels(remap(in_.noise->idleChannels(QubitIndex{w.index}, d->duration.ps)), ctx));
                }
            }
            ...
            if (body.present()) QXL_TRY(runCircuit(*body, ctx, false));
        } else if (const auto* l = std::get_if<ir::Loop>(&n)) {
            const std::uint32_t bound = l->maxIterations > 0 ? l->maxIterations : options_.maxLoopIterations;
            std::uint32_t iter = 0;
            while (l->cond.eval(ctx.bits) != 0) {
                ...
                ++iter;
                ++ctx.branches;
                if (l->body.present()) QXL_TRY(runCircuit(*l->body, ctx, false));
            }
        } else if (const auto* bx = std::get_if<ir::Box>(&n)) {
            if (bx->body.present()) QXL_TRY(runCircuit(*bx->body, ctx, false));
        }
        if (topLevel) maybeSnapshot(i, n, ctx);
    }
    return {};
}
