...
std::vector<double> distribution(const RunResult& r) {
    ...
    for (const auto& [label, count] : r.counts.all()) {
        ...
        if (index < p.size()) p[index] = static_cast<double>(count) / total;
    }
    return p;
}

// ⟨ψ|ρ|ψ⟩ with |ψ⟩ the ideal amplitudes and ρ the noisy density matrix (T10 §1, squared convention).
std::optional<double> stateFidelity(const RunResult& ideal, const RunResult& noisy) {
    if (!ideal.finalState || !noisy.finalState) return std::nullopt;
    const auto& psi = ideal.finalState->amplitudes;
    const auto& rho = noisy.finalState->densityMatrix;
    if (!psi || !rho || rho->rows != psi->size() || rho->cols != psi->size()) return std::nullopt;
    num::Complex f = 0.0;
...
Result<SimulatedFidelity> simulatedFidelity(const RunContext& ctx, const RunResult& noisy) {
    if (!ctx.noise) return fail(err::Unsupported, "an ideal run has nothing to compare against");
    ...
    if (ctx.choice == BackendChoice::DensityMatrix) ideal.choice = BackendChoice::Auto;
    ...
    double overlap = 0.0;
    for (std::size_t i = 0; i < p.size() && i < q.size(); ++i) overlap += std::sqrt(std::max(0.0, p[i] * q[i]));
    ...
    out.cls = (reference.exact && noisy.exact) ? data::FidelityClass::Numerical : data::FidelityClass::Statistical;
    return out;
}
