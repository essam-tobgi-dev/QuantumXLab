...
constexpr std::array kAssumptions{
    ...
    Assumption{
        ...
        "p_th = 1 %, one logical cycle = d syndrome rounds, 15-to-1 (or Toffoli) factories with "
        "the "
...
double maxT1(const hw::Calibration& cal, std::span<const std::uint32_t> qubits, End end) {
    ...
    for (std::uint32_t q : qubits)
        if (const hw::QubitCal* c = cal.qubit(q))
            t = std::max(t, at(c->t1.value.v, c->t1.sigma.v, end, true));
    return t;
}

// Spec 15 §6 T_ro: max over the measured qubits of the calibrated readout duration, plus the
// device's ring-down.
double readoutTime(const hw::Device& dev, const hw::Calibration& cal,
                   std::span<const std::uint32_t> measured, End end) {
    if (measured.empty())
        return 0.0;
    double t = 0.0;
...
double resetTime(const hw::Device& dev, const hw::Calibration& cal, hw::ResetPolicy policy,
    ...
    case hw::ResetPolicy::Passive:
        return std::max(dev.control.passiveMultiplier * maxT1(cal, used, end),
                        dev.timing.repetitionDelay.v);
    case hw::ResetPolicy::Cooling:
        return dev.control.coolingTime.v;
    }
    return 0.0;
}

WallTimeEstimate evaluate(const EstimateInput& in, hw::ResetPolicy policy, double circuitS,
                          End end) {
    ...
    w.feedbackTotalS =
        in.branchesPerShot * static_cast<double>(in.shots) * dev.control.feedbackLatency.v;
...
std::string_view assumptionText(std::string_view key) {
    const std::size_t colon =
        key.find(':'); // "reset_policy:active" renders the reset_policy sentence
...
Result<WallTimeEstimate> estimateWallTime(const EstimateInput& in) {
    if (!in.device || !in.calibration || !in.program)
        return fail(err::Unsupported,
                    "the wall-time estimate needs a device, a calibration and a compiled program");
    ...
    const double circuitS =
        static_cast<double>(circuitCriticalPath(in.program->circuit, in.program->timing).get()) *
        1e-12;
...
ResourceSummary summarizeResources(const EstimateInput& in) {
    ...
    r.qubits = static_cast<std::uint32_t>(in.usedQubits.empty() ? usedQubits(p.circuit).size()
                                                                : in.usedQubits.size());
        ...
        for (ir::NodeId id : c.topologicalOrder()) {
            ...
            if (const auto* g = std::get_if<ir::Gate>(&n)) {
                if (g->name == "t" || g->name == "tdg") {
                    ++r.tCount;
                } else if (g->params.size() == 1 &&
                           (g->name == "rz" || g->name == "p" || g->name == "u1")) {
                    const double k = g->params[0] / quarter;
                    const double rounded = std::round(k);
                    if (std::abs(k - rounded) > 1e-9)
                        ++r.rotations; // arbitrary angle
                    else if (std::abs(std::fmod(std::abs(rounded), 2.0) - 1.0) < 1e-9)
                        ++r.tCount; // ±π/4 (mod π/2)
                } else if (!g->params.empty() && (g->name == "rx" || g->name == "ry")) {
                    const double k = g->params[0] / (std::numbers::pi / 2.0);
                    if (std::abs(k - std::round(k)) > 1e-9)
                        ++r.rotations;
                }
            } else if (std::holds_alternative<ir::Branch>(n) ||
                       std::holds_alternative<ir::Loop>(n)) {
                ++r.branches;
